...
--+''',@Is_Digital='''+ISNULL(Cast(@Is_Digital AS varchar), '')
	...
	Begin
		...
		Insert InTo #Temp_Right_Remarks(Acq_Deal_Rights_Code, Restriction_Remarks, Remarks, Rights_Remarks, Sub_Deal_Restriction_Remark, Sub_License_Name,Deal_Type_Name)
		...
		CASE WHEN ISNULL(sl.Sub_License_Code, 0) = @Sub_License_Code_Avail  THEN 'Yes' ELSE sl.Sub_License_Name END,DT.Deal_Type_Name
	...
	Update a Set a.Group_No = b.RowId From #Temp_Main_Ctr a 
		...
		print 'STEP-11 UPDATE Country Names in #Temp_Main_Ctr' + convert(varchar(30),getdate() ,109)	
		BEGIN		
	...
	--	Select  a.Avail_Raw_Code, Title_Code, Platform_Code, Is_Title_Language, Sub_Language_Code, Dub_Language_code, 
	...
	--		) AS a WHERE RowId = 1
	--	) As a 
	--	INNER JOIN #Tbl_Ret b On a.Group_No = b.Group_No AND CASE WHEN @TabName = 'CO' THEN b.Avail_Raw_Code ELSE 1 END = CASE WHEN @TabName = 'CO' THEN a.Avail_Raw_Code ELSE 1 END
   ...
   END		
	...
	BEGIN --------------- ANCILLARY
		...
		SELECT AR.Acq_Deal_Code, a.Title_Code, Platform_Str FROM (
			SELECT DISTINCT Avail_Raw_Code, Title_code, Platform_Str FROM #Temp_Main_Ctr
		) AS a
	...
	--SET @strSQL =
		SELECT DISTINCT 
			...
			trr.Deal_Type_Name COLLATE SQL_Latin1_General_CP1_CI_AS Deal_Type_Name ,
			Case When ar.Episode_From < @Episode_From Then @Episode_From Else ar.Episode_From End As Episode_From, 
			Case When ar.Episode_To > @Episode_To Then @Episode_To Else ar.Episode_To End As Episode_To, 
			--ar.Episode_From, 
			--ar.Episode_To, 
			...
			trr.Sub_License_Name AS Sub_License, 'Yes' Platform_Avail, ISNULL(TA.Ancillary_Rights,'') AS Ancillary_Rights
		FROM 
			#Temp_Main_Ctr tm
			INNER JOIN #Avail_Raw ar On tm.Avail_Raw_Code = ar.Avail_Raw_Code
			INNER JOIN #Temp_Right_Remarks trr ON trr.Acq_Deal_Rights_Code = ar.Acq_Deal_Rights_Code
			INNER JOIN #Temp_Titles trt ON trt.Title_Code = tm.Title_Code
			LEFT JOIN #Temp_Ancillary TA ON trt.Title_Code = TA.Title_Code AND tm.Title_Code = TA.Title_Code AND ar.Acq_Deal_Code = TA.Acq_Deal_Code AND TA.Platform_Str = tm.Platform_Str 
			INNER JOIN Platform pt ON pt.Platform_Code = tm.Platform_Code
		UNION
		SELECT Title_Name COLLATE SQL_Latin1_General_CP1_CI_AS,Deal_Type_Name COLLATE SQL_Latin1_General_CP1_CI_AS, Episode_From, Episode_To,  Platform_Name COLLATE SQL_Latin1_General_CP1_CI_AS, 
			...
			Ancillary_Rights AS Ancillary_Rights
	...
	BEGIN
		...
		INNER JOIN Platform pt ON pt.Platform_Code = tm.Platform_Code
		UNION 
		SELECT Title_Name COLLATE SQL_Latin1_General_CP1_CI_AS,Episode_From,Deal_Type_Name COLLATE SQL_Latin1_General_CP1_CI_AS, Episode_To, Platform_Name COLLATE SQL_Latin1_General_CP1_CI_AS, 
		COL1, COL2, COL3, Right_Start_Date, Rights_End_Date, Title_Language_Names COLLATE SQL_Latin1_General_CP1_CI_AS, 
		Sub_Language_Names, Dub_Language_Names, Genres_Name COLLATE SQL_Latin1_General_CP1_CI_AS, Star_Cast COLLATE SQL_Latin1_General_CP1_CI_AS, 
		Director COLLATE SQL_Latin1_General_CP1_CI_AS, Duration_In_Min, Year_Of_Production, Restriction_Remark, 
		Sub_Deal_Restriction_Remark, Remarks, Rights_Remarks, Exclusive, 'No' AS Sub_License, Platform_Avail,
