...
Begin
	...
	Inner Join STRING_SPLIT(IsNull(@Country_Codes, ''), ',') On IsNull([value], '') <> '' AND Cast([value] AS Int) = rtc.Country_Code
	...
	Select Country_Code, Report_Territory_Code From Report_Territory_Country rtc Where rtc.Country_Code Not In (
		Select Cast([value] AS Int) From STRING_SPLIT(IsNull(@Country_Codes, ''), ',') Where IsNull([value], '') <> ''
	)
