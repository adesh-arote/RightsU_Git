...
-- =============================================
    ...
    FROM BV_Deal BV WHERE ISNULL(BV.Acq_Deal_Code,0) = @Deal_Code
	...
	BEGIN TRY
		...
		BEGIN	
			...
			SELECT 
				DISTINCT 
					Acq_Deal_Code, BV_Deal_Ref_Key, Is_Archived, [RU_Licensor_Code], [RU_Payee_Code], [RU_Currency_Code], [RU_Licensee_Code], 
					[RU_Category_Code], [BV_Licensor_Code], [BV_Payee_Code], [BV_Currency_Code], [BV_Licensee_Code], [BV_Category_Code], 
					(select sum(Deal_Cost) From Acq_Deal_Cost where Acq_Deal_Code = @Deal_Code) [License_Fees],
		...
		BEGIN		
			...
			,BV.License_Fees = (select sum(Deal_Cost) From Acq_Deal_Cost where Acq_Deal_Code = @Deal_Code) 
		...
		INSERT INTO BV_Asset 
				...
				(
					(
					temp.RU_Title_Code IN (select RU_Title_Code From BV_Asset BA)-- WHERE BA.RU_Title_Code = temp.RU_Title_Code)
					AND ISNULL(temp.Episode_Number, 0) NOT IN (select ISNULL(Episode_Number, 0) From BV_Asset BA WHERE BA.RU_Title_Code = temp.RU_Title_Code)
					)
					OR
					temp.RU_Title_Code NOT IN (select RU_Title_Code From BV_Asset BA ) --WHERE BA.RU_Title_Code = temp.RU_Title_Code)
				)
			...
			BEGIN	
				...
				(
					SELECT BV_Asset_Code FROM BV_Deal_Content WHERE BV_Deal_Code IN
					(SELECT BV_Deal_Code FROM BV_Deal WHERE Acq_Deal_Code = @Deal_Code )
				)
			...
			BEGIN
				...
				(
					SELECT BDCR.RU_Channel_Code FROM BV_Deal_Content_Rights BDCR 
					INNER JOIN BV_Deal_Content BDC ON BDCR.BV_Deal_Content_Code = BDC.BV_Deal_Content_Code 
					INNER JOIN BV_Deal BD ON BDC.BV_Deal_Code = BD.BV_Deal_Code 
					WHERE BD.Acq_Deal_Code = @Deal_Code
				) 
				OR
				TD.CurIden_BV_Asset NOT IN
				(
					SELECT BDCR.BV_Asset_Code FROM BV_Deal_Content_Rights BDCR 
					INNER JOIN BV_Deal_Content BDC ON BDCR.BV_Deal_Content_Code = BDC.BV_Deal_Content_Code 
			...
			(
				SELECT BDCR.YearWise_No FROM BV_Deal_Content_Rights BDCR
...
--EXEC [dbo].[USP_BV_Deal_Data_Generation] 9193