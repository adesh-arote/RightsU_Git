...
BEGIN
	...
	from AT_Acq_Deal AAD
	INNER JOIN AT_Acq_Deal_Movie AADM ON AAD.AT_Acq_Deal_Code = AADM.AT_Acq_Deal_Code
	...
	from Acq_Deal AAD
	INNER JOIN Acq_Deal_Movie AADM ON AAD.Acq_Deal_Code = AADM.Acq_Deal_Code
	INNER JOIN Title t ON t.Title_Code = AADM.Title_Code
	INNER JOIN Users USR ON USR.Users_Code = CASE WHEN version = '0001' THEN 
				AAD.Inserted_By
			  ELSE
...
BEGIN
	...
	FROM AT_Acq_Deal_Rights AADR 
	INNER JOIN AT_Acq_Deal AAD ON AADR.AT_Acq_Deal_Code = AAD.AT_Acq_Deal_Code
	...
	ISNULL(AADR.Milestone_Unit_Type,'') Milestone_Unit_Type,AADR.Is_ROFR,ISNULL(Convert(varchar(20),AADR.ROFR_Date,106),'') ROFR_Date,ISNULL(AADR.Restriction_Remarks,'') Restriction_Remarks,ISNULL(Convert(varchar(20),AADR.Effective_Start_Date,106),'') Effective_Start_Date
	,ISNULL(R.ROFR_Type,'') ROFR_Type, 'UnChanged' [Status],cast(AAD.Version as decimal), '',USR.Login_Name
	from Acq_Deal_Rights AADR 
	INNER JOIN Acq_Deal AAD ON AADR.Acq_Deal_Code = AAD.Acq_Deal_Code
	LEFT JOIN Sub_License S ON AADR.Sub_License_Code = S.Sub_License_Code
	LEFT JOIN Milestone_Type MT ON AADR.Milestone_Type_Code = MT.Milestone_Type_Code
	LEFT JOIN ROFR R ON AADR.ROFR_Code = R.ROFR_Code
	INNER JOIN Users USR ON USR.Users_Code = CASE WHEN version = '0001' THEN 
	...
	Declare CUR_Right Cursor For Select AT_Acq_Deal_Rights_Code,Version,VersionOld,Acq_Deal_Rights_Code,[Index] From #TempRightsHistory
			...
			Begin
				...
				BEGIN
					Set @Codes = ''
					Select @Codes = @Codes + Cast(Platform_Code as varchar) + ',' From AT_Acq_Deal_Rights_Platform Where AT_Acq_Deal_Rights_Code = @AT_Acq_Deal_Rights_Code
					SET  @Platform_Name = ''
					...
					Select @Title_Names = @Title_Names + Title_Name + ', ' From (
						...
						From Title t 
						Inner Join AT_Acq_Deal_Rights_Title adrt On t.Title_Code = adrt.Title_Code 
				...
				BEGIN
					...
					Select @Title_Names = @Title_Names + Title_Name + ', ' From (
						...
						Else t.Title_Name End As Title_Name
						From Title t 
						Inner Join Acq_Deal_Rights_Title adrt On t.Title_Code = adrt.Title_Code 
...
BEGIN
	...
	FROM AT_Acq_Deal_Pushback AADR 
	INNER JOIN AT_Acq_Deal AAD ON AADR.AT_Acq_Deal_Code = AAD.AT_Acq_Deal_Code
	LEFT JOIN Milestone_Type MT ON AADR.Milestone_Type_Code = MT.Milestone_Type_Code
	INNER JOIN Users USR ON USR.Users_Code = CASE WHEN version = '0001' THEN 
				AAD.Inserted_By
	...
	Declare CUR_Pushback Cursor For Select AT_Acq_Deal_Pushback_Code,Version,VersionOld,Acq_Deal_Pushback_Code,[Index] From #TempPushbackHistory
		...
		Begin
			...
			BEGIN
				Set @Codes = ''
				Select @Codes = @Codes + Cast(Platform_Code as varchar) + ',' From AT_Acq_Deal_Pushback_Platform Where AT_Acq_Deal_Pushback_Code = @AT_Acq_Deal_Pushback_Code
				SET  @Platform_Name = ''
				select @Platform_Name = @Platform_Name + Platform_Hiearachy + ',' from dbo.UFN_Get_Platform_With_Parent (@Codes)
				set @Platform_Name = reverse(stuff(reverse(@Platform_Name), 1, 1, ''))
				...
				Select @Title_Names = @Title_Names + Title_Name + ', ' From (
					...
					Else t.Title_Name End As Title_Name
					From Title t 
					Inner Join AT_Acq_Deal_Pushback_Title adrt On t.Title_Code = adrt.Title_Code 
			...
			BEGIN
				Set @Codes = ''
				Select @Codes = @Codes + Cast(Platform_Code as varchar) + ',' From Acq_Deal_Pushback_Platform Where Acq_Deal_Pushback_Code = @Acq_Deal_Pushback_Code
				SET  @Platform_Name = ''
				select @Platform_Name = @Platform_Name + Platform_Hiearachy + ',' from dbo.UFN_Get_Platform_With_Parent (@Codes)
				set @Platform_Name = reverse(stuff(reverse(@Platform_Name), 1, 1, ''))
				...
				Select @Title_Names = @Title_Names + Title_Name + ', ' From (
					...
					Else t.Title_Name End As Title_Name
					From Title t 
					Inner Join Acq_Deal_Pushback_Title adrt On t.Title_Code = adrt.Title_Code 
...
BEGIN
	...
	FROM AT_Acq_Deal_Payment_Terms AAPT 
	INNER JOIN AT_Acq_Deal AAD ON AAPT.AT_Acq_Deal_Code = AAD.AT_Acq_Deal_Code
	LEFT JOIN Cost_Type CT ON AAPT.Cost_Type_Code = CT.Cost_Type_Code
	LEFT JOIN Payment_Terms PT ON AAPT.Payment_Term_Code = PT.Payment_Terms_Code
	INNER JOIN Users USR ON USR.Users_Code = CASE WHEN version = '0001' THEN 
				AAD.Inserted_By
			  ELSE
				AAD.Last_Action_By
	...
	FROM Acq_Deal_Payment_Terms AAPT 
	INNER JOIN Acq_Deal AAD ON AAPT.Acq_Deal_Code = AAD.Acq_Deal_Code
	LEFT JOIN Cost_Type CT ON AAPT.Cost_Type_Code = CT.Cost_Type_Code
	LEFT JOIN Payment_Terms PT ON AAPT.Payment_Term_Code = PT.Payment_Terms_Code
	INNER JOIN Users USR ON USR.Users_Code = CASE WHEN version = '0001' THEN 
				AAD.Inserted_By
...
--------------------------------------------------------------- Code Start for Deal Attachment History--------------------------------------------------------
	...
	FROM AT_Acq_Deal_Attachment AADA 
	INNER JOIN AT_Acq_Deal AAD ON AADA.AT_Acq_Deal_Code = AAD.AT_Acq_Deal_Code
	...
	RIGHT('000' + cast( (cast(AAD.Version as decimal) +1) as varchar(50)),4) VersionOld,ISNULL(AADA.Acq_Deal_Attachment_Code, 0)Acq_Deal_Attachment_Code,
	T.Title_Name,AADA.Attachment_Title,AADA.Attachment_File_Name,AADA.System_File_Name,DT.Document_Type_Name,AADA.Episode_From,AADA.Episode_To,'UnChanged' [Status]	,cast(AAD.Version as decimal), '',USR.Login_Name
	FROM Acq_Deal_Attachment AADA 
	INNER JOIN Acq_Deal AAD ON AADA.Acq_Deal_Code = AAD.Acq_Deal_Code
	LEFT JOIN Document_Type DT ON AADA.Document_Type_Code = DT.Document_Type_Code
	LEFT JOIN Title T ON AADA.Title_Code = T.Title_Code
	INNER JOIN Users USR ON USR.Users_Code = CASE WHEN version = '0001' THEN 
				AAD.Inserted_By
...
BEGIN
	...
	FROM AT_Acq_Deal_Material AADM 
	INNER JOIN AT_Acq_Deal AAD ON AADM.AT_Acq_Deal_Code = AAD.AT_Acq_Deal_Code
	...
	RIGHT('000' + cast( (cast(AAD.Version as decimal) +1) as varchar(50)),4) VersionOld,ISNULL(AADM.Acq_Deal_Material_Code, 0)Acq_Deal_Material_Code,
	T.Title_Name,MM.Material_Medium_Name,MT.Material_Type_Name,AADM.Quantity,AADM.Episode_From,AADM.Episode_To,'UnChanged' [Status],cast(AAD.Version as decimal), '',USR.Login_Name
	FROM Acq_Deal_Material AADM 
	INNER JOIN Acq_Deal AAD ON AADM.Acq_Deal_Code = AAD.Acq_Deal_Code
	LEFT JOIN Material_Medium MM ON AADM.Material_Medium_Code= MM.Material_Medium_Code
	LEFT JOIN Material_Type MT ON AADM.Material_Type_Code = MT.Material_Type_Code
	LEFT JOIN Title T ON AADM.Title_Code = T.Title_Code
	INNER JOIN Users USR ON USR.Users_Code = CASE WHEN version = '0001' THEN 
...
-- Code start for Deal Ancillary History	
	...
	FROM AT_Acq_Deal_Ancillary AADA
		...
		Begin
			...
			BEGIN
				...
				Select @Title_Names = @Title_Names + Title_Name + ', ' From (
					...
					Else t.Title_Name End As Title_Name
					From Title t 
					Inner Join AT_Acq_Deal_Ancillary_Title adrt On t.Title_Code = adrt.Title_Code 
			...
			BEGIN
				...
				Select @Title_Names = @Title_Names + Title_Name + ', ' From (
					...
					Else t.Title_Name End As Title_Name
					From Title t 
					Inner Join Acq_Deal_Ancillary_Title adrt On t.Title_Code = adrt.Title_Code 
...
--	
	...
	FROM AT_Acq_Deal_Sport AADS 
	INNER JOIN AT_Acq_Deal AAD ON AADS.AT_Acq_Deal_Code = AAD.AT_Acq_Deal_Code
	INNER JOIN Users USR ON USR.Users_Code = CASE WHEN version = '0001' THEN 
				AAD.Inserted_By
			  ELSE
				AAD.Last_Action_By
	...
	AADS.Sys_File_Name,AADS.Remarks,AADS.MBO_Note,'UnChanged' [Status],cast(AAD.Version as decimal), '',USR.Login_Name
	FROM Acq_Deal_Sport AADS 
	INNER JOIN Acq_Deal AAD ON AADS.Acq_Deal_Code = AAD.Acq_Deal_Code
	INNER JOIN Users USR ON USR.Users_Code = CASE WHEN version = '0001' THEN 
				AAD.Inserted_By
			  ELSE
				AAD.Last_Action_By
		...
		Begin
			...
			BEGIN
				...
				Select @Title_Names = @Title_Names + Title_Name + ', ' From (
					...
					Else t.Title_Name End As Title_Name
					From Title t 
					Inner Join AT_Acq_Deal_Sport_Title adrt On t.Title_Code = adrt.Title_Code 
			...
			BEGIN
				...
				Select @Title_Names = @Title_Names + Title_Name + ', ' From (
					...
					Else t.Title_Name End As Title_Name
					From Title t 
					Inner Join Acq_Deal_Sport_Title adrt On t.Title_Code = adrt.Title_Code 
...
BEGIN
	...
	FROM AT_Acq_Deal_Sport_Ancillary AADSA 
	INNER JOIN AT_Acq_Deal AAD ON AADSA.AT_Acq_Deal_Code = AAD.AT_Acq_Deal_Code
	LEFT JOIN Sport_Ancillary_Type SAT ON AADSA.Sport_Ancillary_Type_Code = SAT.Sport_Ancillary_Type_Code
	LEFT JOIN Sport_Ancillary_Periodicity SAP ON AADSA.Broadcast_Periodicity_Code = SAP.Sport_Ancillary_Periodicity_Code
	LEFT JOIN Sport_Ancillary_Periodicity SAP1 ON AADSA.Sport_Ancillary_Periodicity_Code = SAP1.Sport_Ancillary_Periodicity_Code
	INNER JOIN Users USR ON USR.Users_Code = CASE WHEN version = '0001' THEN 
				AAD.Inserted_By
			  ELSE
				AAD.Last_Action_By
	...
	FROM Acq_Deal_Sport_Ancillary AADSA 
	INNER JOIN Acq_Deal AAD ON AADSA.Acq_Deal_Code = AAD.Acq_Deal_Code
	LEFT JOIN Sport_Ancillary_Type SAT ON AADSA.Sport_Ancillary_Type_Code = SAT.Sport_Ancillary_Type_Code
	LEFT JOIN Sport_Ancillary_Periodicity SAP ON AADSA.Broadcast_Periodicity_Code = SAP.Sport_Ancillary_Periodicity_Code
	LEFT JOIN Sport_Ancillary_Periodicity SAP1 ON AADSA.Sport_Ancillary_Periodicity_Code = SAP1.Sport_Ancillary_Periodicity_Code
	INNER JOIN Users USR ON USR.Users_Code = CASE WHEN version = '0001' THEN 
		...
		Begin
			...
			BEGIN
				...
				Select @Title_Names = @Title_Names + Title_Name + ', ' From (
					...
					Else t.Title_Name End As Title_Name
					From Title t 
					Inner Join AT_Acq_Deal_Sport_Ancillary_Title adrt On t.Title_Code = adrt.Title_Code 
			...
			BEGIN
				...
				Select @Title_Names = @Title_Names + Title_Name + ', ' From (
					...
					Else t.Title_Name End As Title_Name
					From Title t 
					Inner Join Acq_Deal_Sport_Ancillary_Title adrt On t.Title_Code = adrt.Title_Code 
...
BEGIN
	...
	FROM AT_Acq_Deal_Sport_Sales_Ancillary AADSA 
	INNER JOIN AT_Acq_Deal AAD ON AADSA.AT_Acq_Deal_Code = AAD.AT_Acq_Deal_Code
	INNER JOIN Users USR ON USR.Users_Code = CASE WHEN version = '0001' THEN 
				AAD.Inserted_By
			  ELSE
	...
	AADSA.Last_Matching_Rights_Title_Sponsor,AADSA.Last_Matching_Rights_Official_Sponsor,AADSA.Title_Last_Matching_Rights_Validity,
	AADSA.Official_Last_Matching_Rights_Validity,AADSA.Remarks,'UnChanged' [Status],cast(AAD.Version as decimal), '',USR.Login_Name
	FROM Acq_Deal_Sport_Sales_Ancillary AADSA 
	INNER JOIN Acq_Deal AAD ON AADSA.Acq_Deal_Code = AAD.Acq_Deal_Code
	INNER JOIN Users USR ON USR.Users_Code = CASE WHEN version = '0001' THEN 
				AAD.Inserted_By
			  ELSE
		...
		Begin
			...
			BEGIN
				...
				Select @Title_Names = @Title_Names + Title_Name + ', ' From (
					...
					Else t.Title_Name End As Title_Name
					From Title t 
					Inner Join AT_Acq_Deal_Sport_Sales_Ancillary_Title adrt On t.Title_Code = adrt.Title_Code 
					...
					From Sponsor s 
					Inner Join AT_Acq_Deal_Sport_Sales_Ancillary_Sponsor adrt On s.Sponsor_Code = adrt.Sponsor_Code 
			...
			BEGIN
				...
				Select @Title_Names = @Title_Names + Title_Name + ', ' From (
					...
					Else t.Title_Name End As Title_Name
					From Title t 
					Inner Join Acq_Deal_Sport_Sales_Ancillary_Title adrt On t.Title_Code = adrt.Title_Code 
					...
					Select @Title_Sponsor = @Title_Sponsor + Sponsor_Name + ', ' 
					From Sponsor s 
					Inner Join Acq_Deal_Sport_Sales_Ancillary_Sponsor adrt On s.Sponsor_Code = adrt.Sponsor_Code 
...
BEGIN
	...
	FROM AT_Acq_Deal_Sport_Monetisation_Ancillary AADSA 
	INNER JOIN AT_Acq_Deal AAD ON AADSA.AT_Acq_Deal_Code = AAD.AT_Acq_Deal_Code
	INNER JOIN Users USR ON USR.Users_Code = CASE WHEN version = '0001' THEN 
				AAD.Inserted_By
			  ELSE
	...
	CASE WHEN AADSA.Appoint_Broadcast_Sponsor ='Y' THEN 'Yes' ELSE 'No' END Appoint_Broadcast_Sponsor
	,AADSA.Remarks,'UnChanged' [Status],cast(AAD.Version as decimal), '',USR.Login_Name
	FROM Acq_Deal_Sport_Monetisation_Ancillary AADSA 
	INNER JOIN Acq_Deal AAD ON AADSA.Acq_Deal_Code = AAD.Acq_Deal_Code
	INNER JOIN Users USR ON USR.Users_Code = CASE WHEN version = '0001' THEN 
				AAD.Inserted_By
			  ELSE
		...
		Begin
			...
			BEGIN
				...
				Select @Title_Names = @Title_Names + Title_Name + ', ' From (
					...
					Else t.Title_Name End As Title_Name
					From Title t 
					Inner Join AT_Acq_Deal_Sport_Monetisation_Ancillary_Title adrt On t.Title_Code = adrt.Title_Code 
			...
			BEGIN
				...
				Select @Title_Names = @Title_Names + Title_Name + ', ' From (
					...
					Else t.Title_Name End As Title_Name
					From Title t 
					Inner Join Acq_Deal_Sport_Monetisation_Ancillary_Title adrt On t.Title_Code = adrt.Title_Code 
					...
					Select @Monetisation_Type = @Monetisation_Type + s.Monetisation_Type_Name +':'+ Cast(adrt.Monetisation_Rights as VARCHAR(3)) + ', ' 
					From Monetisation_Type s 
					Inner Join Acq_Deal_Sport_Monetisation_Ancillary_Type adrt On s.[Monetisation_Type_Code] = adrt.[Monetisation_Type_Code] 
					WHERE adrt.Acq_Deal_Sport_Monetisation_Ancillary_Code = @Acq_Deal_Sport_Monetisation_Ancillary_Code
					Order by s.Monetisation_Type_Name
...
BEGIN
	...
	FROM AT_Acq_Deal_Budget AADSA 
	INNER JOIN AT_Acq_Deal AAD ON AADSA.AT_Acq_Deal_Code = AAD.AT_Acq_Deal_Code
	INNER JOIN SAP_WBS SW ON AADSA.SAP_WBS_Code = SW.SAP_WBS_Code
	INNER JOIN Users USR ON USR.Users_Code = CASE WHEN version = '0001' THEN 
				AAD.Inserted_By
			  ELSE
				AAD.Last_Action_By
		...
		Begin
			...
			BEGIN
				...
				Select @Title_Names = @Title_Names + Title_Name + ', ' From (
					...
					Else t.Title_Name End As Title_Name
					From Title t 
					Inner Join AT_Acq_Deal_Budget adrt On t.Title_Code = adrt.Title_Code 
			...
			BEGIN
				...
				Select @Title_Names = @Title_Names + Title_Name + ', ' From (
					...
					Else t.Title_Name End As Title_Name
					From Title t 
					Inner Join Acq_Deal_Sport_Budget adrt On t.Title_Code = adrt.Title_Code 
