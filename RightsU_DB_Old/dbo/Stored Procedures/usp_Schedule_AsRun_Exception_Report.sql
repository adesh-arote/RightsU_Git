...
SET NOCOUNT ON;  
	...
	BEGIN
		...
		BEGIN
			...
			from Email_Notification_Schedule ENS 
			LEFT OUTER JOIN Title T ON T.title_code = ENS.Title_Code
			INNER JOIN Channel C ON C.channel_code = ENS.Channel_Code
	...
	BEGIN
		...
		BEGIN
			...
			from Email_Notification_AsRun ENA 
			LEFT OUTER JOIN Title T ON T.title_code = ENA.Title_Code
			INNER JOIN Channel C ON C.channel_code = ENA.Channel_Code 
			WHERE 1=1 ' + @filter
		...
		BEGIN
			...
			ENA.ON_AIR, ENA.Dt_Tm, ENA.DUration, ENA.ID, 
			C.channel_code, C.channel_name, ENA.Email_Notification_Msg
			from Email_Notification_AsRun_Shows ENA 
			LEFT OUTER JOIN Title T ON T.title_code = ENA.Title_Code
			INNER JOIN Channel C ON C.channel_code = ENA.Channel_Code 
			WHERE 1=1 ' + @filter
