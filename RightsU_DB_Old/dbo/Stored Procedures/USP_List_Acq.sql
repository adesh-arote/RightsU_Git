...
IF OBJECT_ID('tempdb..#Temp') IS NOT NULL DROP TABLE #Temp
	...
	Set @Sql = N'SELECT * FROM (
		...
		,(SELECT DISTINCT count(ISNULL(Title_Name,Original_Title)) FROM Acq_Deal_Movie ADM 
			INNER JOIN Title Tit On Tit.title_code = ADM.Title_Code WHERE ADM.Acq_Deal_Code = AD.Acq_Deal_Code ) Cnt_DealTitles
		,dbo.fn_IsShow_Deal_Reopen_Btn(AD.Acq_Deal_Code) IsShowReopenBtn
		,(SELECT COUNT(*) from Acq_Deal_Movie where Acq_Deal_Code in (AD.Acq_Deal_Code)  and ISNULL(is_closed,''N'')=''Y'') IsCountMovieClosed
	    ,(select COUNT(*) from Acq_Deal_Movie where Acq_Deal_Code  in (AD.Acq_Deal_Code)  AND ISNULL(is_closed,''N'') = ''N'') IsShowDealMovieCloseBtn	    
	    ,(SELECT COUNT(Deal_Code) from Syn_Acq_Mapping where Deal_Code in (AD.Acq_Deal_Code)) isShowAmmendmentBtn
	    ,(select Deal_Tag_Description from Deal_Tag where Deal_Tag_Code = AD.Deal_Tag_Code) DealTagDescription
	    ,(SELECT TOP 1 DT.Deal_Type_Name FROM Deal_Type DT WHERE  DT.Deal_Type_Code=AD.Deal_Type_Code ) DealTypeName
	    ...
	    ,(select count(AT_Acq_Deal_Code) from AT_Acq_Deal where Acq_Deal_Code = AD.Acq_Deal_Code) parentDealCode		
		...
		,(SELECT Max(Version) FROM AT_Acq_Deal WHERE Acq_Deal_Code = AD.Acq_Deal_Code) [Previous Version]
		...
		,AD.Status
		,(Select Top 1 Action From Deal_Process dp Where dp.Deal_Code = AD.Acq_Deal_Code AND DP.Record_Status <> ''D'') AT_Status
		...
		,(select Business_Unit_Name from Business_Unit where Business_Unit_Code = AD.Business_Unit_Code) Business_Unit_Name
		,AD.Last_Updated_Time
		...
		FROM Acq_Deal AD
		INNER JOIN Vendor v on AD.Vendor_Code = v.vendor_code
		Inner Join Role r On AD.Role_Code = r.Role_Code
