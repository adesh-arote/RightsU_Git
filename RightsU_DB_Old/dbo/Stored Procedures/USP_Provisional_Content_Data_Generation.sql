...
--Update  Process_Provisional_Deal SET Record_Status='D' where Record_Status='W'
	...
	IF NOT EXISTS(SELECT TOP 1 Process_Provisional_Deal_Code FROM  Process_Provisional_Deal WHERE --Acq_Deal_Code=13565 AND 
	Record_Status = 'W')
	BEGIN
		BEGIN TRY	
		BEGIN TRAN	
			DECLARE CUS_Deal_Process CURSOR FOR
			SELECT Provisional_Deal_Code FROM  Process_Provisional_Deal WHERE Record_Status = 'P' --AND Acq_Deal_Code=13565 
			...
			UPDATE Process_Provisional_Deal SET Record_Status = 'W',Process_Start = GETDATE() where Provisional_Deal_Code = @Deal_Code
				...
				* 
				...
				(
					...
					FROM Provisional_Deal PD 
					INNER JOIN Provisional_Deal_Title PDT ON PDT.Provisional_Deal_Code = PD.Provisional_Deal_Code
					INNER JOIN Title_Content_Mapping TCM ON TCM.Provisional_Deal_Title_Code = PDT.Provisional_Deal_Title_Code
					INNER JOIN Title T ON PDT.Title_Code = T.Title_Code
					INNER JOIN Language Lang ON T.Title_Language_Code = Lang.Language_Code
					INNER JOIN Provisional_Deal_Run PDRun ON PDT.Provisional_Deal_Title_Code = PDRun.Provisional_Deal_Title_Code --AND ADRun.Is_Channel_Definition_Rights = 'Y'
					INNER JOIN Provisional_Deal_Run_Channel PDRC ON PDRun.Provisional_Deal_Run_Code = PDRC.Provisional_Deal_Run_Code
					INNER JOIN Channel CH ON PDRC.Channel_Code = CH.Channel_Code
					LEFT JOIN Right_Rule RR ON PDRun.Right_Rule_Code = RR.Right_Rule_Code
					WHERE 1=1 AND 
				...
				INTO #TempData_New
				from #TempData TD
				INNER JOIN Title_Content_Mapping TCM ON TCM.Provisional_Deal_Title_Code=TD.Provisional_Deal_Title_Code
				...
				FROM Content_Channel_Run CCR WHERE CCR.Provisional_Deal_Code = @Deal_Code --Provisional_Deal_Code=2083--@Deal_Code
