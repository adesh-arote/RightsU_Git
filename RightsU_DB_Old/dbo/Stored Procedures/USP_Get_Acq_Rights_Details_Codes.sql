...
BEGIN
	...
	BEGIN
		...
		SELECT SDM.Title_Code, SDM.Episode_From, SDM.Episode_End_To FROM Syn_Deal_Movie SDM WHERE SDM.Syn_Deal_Movie_Code IN (select number from fn_Split_withdelemiter(@Title_Code, ','))
	END
	ELSE
	BEGIN
		PRINT 'Deal type is not Content'
		INSERT INTO #Deal_Movie_Temp (Title_Code, Episode_From, Episode_To)
		SELECT SDM.Title_Code, SDM.Episode_From, SDM.Episode_End_To FROM Syn_Deal_Movie SDM WHERE SDM.Title_Code IN (select number from fn_Split_withdelemiter(@Title_Code, ','))
	END
	...
	(
		...
		LEFT JOIN Territory_Details TD ON  ISNULL(C.Territory_Code,0) = TD.Territory_Code
	) AS Maintbl
	...
	LEFT JOIN Language_Group_Details LGD ON ISNULL(ADRS.Language_Group_Code,0) = LGD.Language_Group_Code
	...
	LEFT JOIN Language_Group_Details LGD ON ISNULL(ADRD.Language_Group_Code,0) = LGD.Language_Group_Code
