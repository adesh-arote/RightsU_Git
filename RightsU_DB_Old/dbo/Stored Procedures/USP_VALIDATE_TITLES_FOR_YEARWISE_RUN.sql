...
BEGIN
	...
	BEGIN
		...
		BEGIN
			...
			SELECT  DISTINCT ADRT.Title_Code from Acq_Deal_Rights ADR --ADRT.Title_Code
		...
		BEGIN
			INSERT INTO @tableTemp(Title_Code) 
			SELECT  DISTINCT ADM.Acq_Deal_Movie_Code from Acq_Deal_Rights ADR --ADM.Acq_Deal_Movie_Code
			inner join Acq_Deal_Rights_Title ADRT on ADRT.Acq_Deal_Rights_Code = ADR.Acq_Deal_Rights_Code
			INNER JOIN Acq_Deal_Movie ADM on ADM.Acq_Deal_Code = @DEAL_CODE AND ADM.Episode_Starts_From = ADRT.Episode_From 
			INNER JOIN Acq_Deal_Rights_Platform ADRP on ADR.Acq_Deal_Rights_Code = ADRP.Acq_Deal_Rights_Code
			INNER JOIN Platform P ON P.Platform_Code = ADRP.Platform_Code AND p.Is_No_Of_Run = 'Y'
...
Exec [dbo].[USP_VALIDATE_TITLES_FOR_YEARWISE_RUN] '3105', 342
