--USP_Get_Termination_Title_Data 8284,'A'
CREATE PROC USP_Get_Termination_Title_Data
(
	@Deal_Code INT,
...
BEGIN
	...
	BEGIN
		...
		BEGIN
			...
			SELECT @Schedule_Item_Log_Date = Max(BVST.Schedule_Item_Log_Date) FROM Content_Channel_Run CCR
			INNER JOIN BV_Schedule_Transaction BVST ON CCR.Content_Channel_Run_Code = BVST.Content_Channel_Run_Code 
			...
			(
				SELECT SAM.Syn_Deal_Rights_Code From Syn_Acq_Mapping SAM WHERE SAM.Deal_Code = @Deal_Code
			...
			(
				...
				(
					...
					FROM Acq_Deal_Termination_Details A 
		...
		BEGIN
			...
			(
				...
				SELECT 
					BVST.Title_Code, Max(BVST.Program_Episode_Number) AS Eps_No, Max(BVST.Schedule_Item_Log_Date) AS Schedule_Date 
				FROM 
					BV_Schedule_Transaction BVST Where BVST.Content_Channel_Run_Code In (
					Select Content_Channel_Run_Code From Content_Channel_Run Where Acq_Deal_Code = @Deal_Code
				) Group By BVST.Title_Code
			...
			(
				...
				WHERE SDR.Syn_Deal_Rights_Code In 
						(
							...
							FROM SYN_ACQ_MAPPING SAM WHERE SAM.DEAL_CODE = @DEAL_CODE
...
/*
SELECT AD.Acq_Deal_Code, AD.Agreement_No, AD.Deal_Type_Code, ADM.Title_Code, COUNT(ADM.Title_Code) FROM Acq_Deal AD
INNER JOIN Acq_Deal_Movie ADM ON AD.Acq_Deal_Code = ADM.Acq_Deal_Code
WHERE AD.Deal_Workflow_Status = 'A'
GROUP BY AD.Acq_Deal_Code, AD.Agreement_No, AD.Deal_Type_Code, ADM.Title_Code
