ALTER PROCEDURE [dbo].[USPValidateMHRequestConsumption]
	--DECLARE
...
BEGIN
	...
	select DISTINCT MD.Music_Deal_Code, MD.Link_Show_Type, MD.Music_Label_Code, MRD.MHRequestDetailsCode, MR.TitleCode, MD.Rights_Start_Date, MD.Rights_End_Date, 
		MR.TelecastFrom, MR.TelecastTo, MD.No_Of_Songs, MR.ChannelCode, MDC.Defined_Runs--, ISNULL(MDC.Scheduled_Runs,0) + ISNULL(MDC.RequestedSchedule,0) Scheduled_Runs
	INTO #MusicDealTemp 
	from MHRequest MR
	...
	--Deal not found
	BEGIN
		Update #MHRequestTemp SET IsValid = 'N', Message = 'Deal not found' WHERE MHRequestDetailsCode NOT IN (SELECT MHRequestDetailsCode FROM #MusicDealTemp )
	...
	BEGIN
		...
		INNER JOIN Music_Deal_LinkShow MDL ON MDL.Music_Deal_Code = T.Music_Deal_Code
