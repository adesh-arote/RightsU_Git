...
Update System_Parameter_new SET PArameter_Value='24' WHERE Parameter_Name='Run_Expiry_ChannelCode'
Select *  From Channel
EXEC [USP_Email_Run_Expiry]  
...
-- =============================================
	...
	DECLARE curOuter cursor For select distinct Business_Unit_Code,Users_Email_id from Deal_Expiry_Email where Business_Unit_Code Is not Null And Alert_Type = 'E'
	OPEN curOuter 
	...
	Begin		
		...
		SELECT @Deal_Expiry_Email_Template_Desc =REPLACE(Template_Desc ,'{Link}',@DefaultSiteUrl) FROM  Email_Template WHERE Template_For= 'USP_Email_Run_Expiry'			
