 ...
 --   @Country_Code VARCHAR(MAX) = '0', 
	...
	BEGIN
		...
		(
			...
			Sub_License_Code INT,
			Is_Exclusive CHAR(1),
			Title_Language_Names Varchar(200),
			Episode_From INT,
			Episode_To INT
		)
		...
		(
			...
			Episode_From INT,
			Episode_To INT,
			Sub_Language_Names VARCHAR(MAX)
		)
		...
		(
			...
			Episode_From INT,
			Episode_To INT,
			Dubb_Language_Names VARCHAR(MAX)
		)
		...
		BEGIN
			...
			Select Distinct ',' + 
			CAST(Case When Language_Type = 'G' Then lgd.Language_Code Else adrs.Language_Code End AS VARCHAR) From Syn_Deal_Rights_Subtitling adrs
		...
		BEGIN
			Select Distinct Syn_Deal_Rights_Code, STUFF((
			Select Distinct ',' + 
			CAST(Case When Language_Type = 'G' Then lgd.Language_Code Else adrd.Language_Code End AS VARCHAR) From Syn_Deal_Rights_Dubbing adrd
		...
		BEGIN
			...
			ALTER TABLE #Temp_Main_Ctr ADD Platform_Str VARCHAR(2000)
		   ...
		   END		
			...
			CAST('' AS VARCHAR(MAX)) Remarks, CAST('' AS VARCHAR(MAX)) Rights_Remarks, CAST('' AS VARCHAR(MAX)) Restriction_Remarks
