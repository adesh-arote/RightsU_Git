...
{
    public partial class Show_Availability_Report : ParentPage
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            ((Home)this.Page.Master).setVal("ShowAvailability");
        ...
        {
            //rdQueryNo.Checked = true;
            //rdQueryYes.Checked = false;
        ...
        {
            System_Parameter_New_Service objSystemParamService = new System_Parameter_New_Service(ObjLoginEntity.ConnectionStringName);
            string SelectedBU = objSystemParamService.SearchFor(p => p.Parameter_Name == "Title_Avail_Show_BU").ToList().FirstOrDefault().Parameter_Value;
        ...
        private void BindGridMovie()
        {
            ...
            string strSearch = "";
            SearchString = " AND IS_Active='Y' AND Deal_Type_Code NOT IN(" + Deal_Type_Movie + ") AND Title_Code IN (SELECT adm.Title_Code FROM Acq_Deal_Movie adm INNER JOIN Acq_Deal ad ON ad.Acq_Deal_Code = adm.Acq_Deal_Code WHERE ad.Is_Master_Deal='Y' AND ad.Business_Unit_Code=" + BU_Code + ")";
        ...
        public void BindTitleLanguage()
        {
            ...
            System_Parameter_New_Service objSystemParamService = new System_Parameter_New_Service(ObjLoginEntity.ConnectionStringName);
            int Deal_Type_Movie = Convert.ToInt32(objSystemParamService.SearchFor(p => p.Parameter_Name == "Deal_Type_Movie").FirstOrDefault().Parameter_Value);
            List<int> lstTitleLanguageCode = titleService.SearchFor(T => T.Is_Active == "Y" && T.Acq_Deal_Movie.Any(AM => AM.Acq_Deal.Business_Unit_Code ==
                                             BU_Code && AM.Acq_Deal.Is_Master_Deal == "Y") && T.Deal_Type_Code != Deal_Type_Movie)
                                             .Select(t => t.Title_Language_Code.Value).Distinct().ToList();
        ...
        protected void ShowResults(string isSaveQry, int intCode = 0)
        {
            ...
            string StartDate = "", EndDate = "", StartMonth, EndYear, EpisodeFrom = "", EpisodeTo = "", Digital = "true";
            ...
            if (txtEFrom.Text != "")
            {
                EpisodeFrom = txtEFrom.Text;
            }
            else
                EpisodeFrom = "NULL";
            hdnEpisodeFrom.Value = txtEFrom.Text;
            if (txtETo.Text != "")
            {
                EpisodeTo = txtETo.Text;
            }
            else
                EpisodeTo = "NULL";
            hdnEpisodeTo.Value = txtETo.Text;
            if (StartDate != "")
                hdnStartDate.Value = StartDate.Replace('-', '/');
            else
                hdnStartDate.Value = "";
            ...
            string strInsertVal = "'" + sbMovie.ToString() + "', '" + platformCodes + "','" + sbTerritory.ToString() + "','"
                                      ...
                                      + BU_Code.ToString() + "','SQ','Y'," + EpisodeFrom + "," + EpisodeTo + ",'" + Digital + "','" + chkMetadata.Checked.ToString() + "'";
            ...
            objAvail_Report_Schedule_UDT.Module_Code = Convert.ToInt32("206");
            ...
            //    objUSP.USP_Get_Title_Avail_Language_Data("T", objLoginedUser.Users_Code, strInsertVal, 0, "", "", 0, objRecordCount, "");
            ...
            if (isSaveQry == "R")
            {
                ...
                dynamic list;
                list = objUSP.USP_Get_Title_Avail_Language_Data_Show(lst_Avail_Report_Schedule_UDT, objLoginedUser.Users_Code, "F", strFilter, PageNo, "", "N", 0).ToList();

                 //   objUSP.USP_Get_Title_Avail_Language_Data_Shows(, objLoginedUser.Users_Code, "F", strFilter, PageNo, "", "N", 0).ToList();
                if (list.Count > 0)
                {
                    ...
                    hdnEpisodeFrom.Value = Convert.ToString(list[0].Episode_From);
                    hdnEpisodeTo.Value = Convert.ToString(list[0].Episode_To);
                    BindPlatform("");
                    int[] arrPlatform;
            ...
            if (hdnIsCriteriaChange.Value == "Y")
            {
                ...
                ReportParameter[] parm = new ReportParameter[41];
                ...
                parm[18] = new ReportParameter("Start_Date", GlobalUtil.MakedateFormat(StartDate));
                parm[19] = new ReportParameter("End_Date", GlobalUtil.MakedateFormat(EndDate));
                parm[20] = new ReportParameter("Restriction_Remarks", strRestRemarks);
                parm[21] = new ReportParameter("Others_Remarks", chkOtherRemarks.Checked.ToString());
                parm[22] = new ReportParameter("Platform_MustHave", (chkExact.SelectedValue == "MH") ? uctabSelectedplt.PlatformCodes_Selected_Out : "0");
                parm[23] = new ReportParameter("Include_Metadata", Metadata);
                parm[24] = new ReportParameter("Is_IFTA_Cluster", "N");
                parm[25] = new ReportParameter("Platform_Group_Code", "0");
                parm[26] = new ReportParameter("Subtitling_Group_Code", "0");
                parm[27] = new ReportParameter("Subtitling_ExactMatch", "0");
                parm[28] = new ReportParameter("Subtitling_MustHave", "0");
                parm[29] = new ReportParameter("Subtitling_Exclusion", "0");
                parm[30] = new ReportParameter("Dubbing_Group_Code", "0");
                parm[31] = new ReportParameter("Dubbing_ExactMatch", "0");
                parm[32] = new ReportParameter("Dubbing_MustHave", "0");
                parm[33] = new ReportParameter("Dubbing_Exclusion", "0");
                parm[34] = new ReportParameter("Territory_Code", "0");
                parm[35] = new ReportParameter("Episode_From", txtEFrom.Text);
                parm[36] = new ReportParameter("Episode_To", txtETo.Text);
                parm[37] = new ReportParameter("Show_EpisodeWise", "N");
                parm[38] = new ReportParameter("Country_Level", "N");
                parm[39] = new ReportParameter("Territory_Level", "Y");
                parm[40] = new ReportParameter("TabName", "IF");
                ReportCredential();
                ...
                if (ReportViewer1.ServerReport.ReportPath == "")
                {
                    ...
                    ReportViewer1.ServerReport.ReportPath = objRS.GetReport("Title_Availability_Show_3_V18_Demo");
                }
        ...
        private void BindGridView(bool showNorecAlert = true)
        {
            ...
            ObjectParameter objRecordCount = new ObjectParameter("RecordCount", RecordCount);
            USP_Service objUSP = new USP_Service(ObjLoginEntity.ConnectionStringName);
            List<USP_Get_Title_Avail_Language_Data_Show> objAvailResultShow = new List<USP_Get_Title_Avail_Language_Data_Show>();
            ...
            strFilter = "AND ((Visibility='PR' AND User_Code = " + objLoginedUser.Users_Code +
                    ...
                    "AND BU_CODE =" + ddlBusinessUnit.SelectedValue + "AND Report_Type = 'SQ' AND IndiaCast = 'N' AND Module_Code = " + Convert.ToInt32(206) + " ";


            gvSchedule.DataSource = objUSP.USP_Get_Title_Avail_Language_Data_Show(lst_Avail_Report_Schedule_UDT, objLoginedUser.Users_Code, "F", strFilter, PageNo, "", isPaging, pageSize).ToList();
            //RecordCount = Convert.ToInt32(objRecordCount.Value);
            objAvailResultShow = objUSP.USP_Get_Title_Avail_Language_Data_Show(lst_Avail_Report_Schedule_UDT, objLoginedUser.Users_Code, "F", strFilter, PageNo, "", isPaging, pageSize).ToList();
            RecordCount = objAvailResultShow.Select(s => s.Recordcount).FirstOrDefault();
        ...
        protected void gvSchedule_RowDeleting(object sender, GridViewDeleteEventArgs e)
        {
            ...
            try
            {
                ...
                ObjectParameter objRecordCount = new ObjectParameter("RecordCount", 10);
                USP_Service objUSP = new USP_Service(ObjLoginEntity.ConnectionStringName);
                objUSP.USP_Get_Title_Avail_Language_Data_Show(lst_Avail_Report_Schedule_UDT, objLoginedUser.Users_Code, "D", intCode, 0, "", "", 0);
            }
            catch (Exception) { }
            finally
            {
        ...
        protected void btnSaveQuery_Click(object sender, EventArgs e)
        {
            ...
            string query = "SELECT Parameter_value FROM System_Parameter_New WHERE Parameter_Name = 'Reports_DB_Show'";
            ...
            else
            {
                ...
                try
                {
                    //hdnstrCriteria.Value += ",'" + txtReportName.Text + "','" + rblVisibility.SelectedValue + "'";
                    ...
                    string txt_From = txtEFrom.Text == "" ? "0" : txtEFrom.Text;
                    string txt_To = txtETo.Text == "" ? "0" : txtETo.Text;
                    ObjectParameter objRecordCount = new ObjectParameter("RecordCount", 10);
                    List<Avail_Report_Schedule_UDT> lst_Avail_Report_Schedule_UDT = new List<Avail_Report_Schedule_UDT>();
                    Avail_Report_Schedule_UDT objAvail_Report_Schedule_UDT = new Avail_Report_Schedule_UDT();
                    objAvail_Report_Schedule_UDT.Title_Code = string.Join(",", sbMovie.ToString());
                    ...
                    objAvail_Report_Schedule_UDT.MustHave_Promoter = "";
                    objAvail_Report_Schedule_UDT.Module_Code = Convert.ToInt32("206");
                    objAvail_Report_Schedule_UDT.Episode_From = Convert.ToInt32(txt_From);
                    objAvail_Report_Schedule_UDT.Episode_To = Convert.ToInt32(txt_To);
                    objAvail_Report_Schedule_UDT.ReportName = txtReportName.Text.Trim();
                    lst_Avail_Report_Schedule_UDT.Add(objAvail_Report_Schedule_UDT);
                    objUSP.USP_Get_Title_Avail_Language_Data_Show(lst_Avail_Report_Schedule_UDT, objLoginedUser.Users_Code, "T", hdnstrCriteria.Value, 0, "", "", 0);

                    // objUSP.USP_Get_Title_Avail_Language_Data_Show("S", objLoginedUser.Users_Code, hdnstrCriteria.Value, 0, "", "", 0, objRecordCount, "", txtReportName.Text, "");
