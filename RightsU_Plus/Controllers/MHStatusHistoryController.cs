...
{
    public class MHStatusHistoryController : BaseController
    {
        ...
        private List<RightsU_Entities.USPMHRequisitionList_Result> lst
        {
            get
            {
                if (Session["lstMHStatusHistory"] == null)
                    Session["lstMHStatusHistory"] = new List<RightsU_Entities.USPMHRequisitionList_Result>();
                return (List<RightsU_Entities.USPMHRequisitionList_Result>)Session["lstMHStatusHistory"];
            }
            set { Session["lstMHStatusHistory"] = value; }
        }
        private MHStatusHistory_Search objPage_Properties
        {
            get
            {
                if (Session["MHStatusHistory_Search_Page_Properties"] == null)
                    Session["MHStatusHistory_Search_Page_Properties"] = new MHStatusHistory_Search();
                return (MHStatusHistory_Search)Session["MHStatusHistory_Search_Page_Properties"];
            }
            set { Session["MHStatusHistory_Search_Page_Properties"] = value; }
        }
        public ActionResult Index()
        {
            string moduleCode = GlobalParams.ModuleCodeForStatusReport.ToString();
            ViewBag.UserModuleRights = GetUserModuleRights();
            ViewBag.Code = moduleCode;
            ViewBag.LangCode = objLoginUser.System_Language_Code.ToString();
            return View();
        }
        public PartialViewResult BindStatusReport(string prodHouseCode = "", string statusCode = "", string titleCodes = "", string usersCodes = "", string requestId = "", string fromDate = "", string toDate = "", int pageNo = 1, int recordPerPage = 10)
        {
            ...
            lst = new USP_Service(objLoginEntity.ConnectionStringName).USPMHRequisitionList(prodHouseCode, "", 0, fromDate, toDate, statusCode, titleCodes, "","", pageNo, recordPerPage, usersCodes, requestId, objRecordCount).ToList();
            ...
            return PartialView("~/Views/MHStatusHistory/_MHStatusHistoryList.cshtml", lst);
        }
        ...
        public JsonResult GetShowName(string Searched_Title)
        {
            ...
            var result = new MHRequest_Service(objLoginEntity.ConnectionStringName)
                .SearchFor(x => x.Title.Title_Name.ToUpper().Contains(searchString.ToUpper()))
        ...
        {
            List<string> lstRights = new USP_Service(objLoginEntity.ConnectionStringName).USP_MODULE_RIGHTS(Convert.ToInt32(UTOFrameWork.FrameworkClasses.GlobalParams.ModuleCodeForStatusReport), objLoginUser.Security_Group_Code, objLoginUser.Users_Code).ToList();
            string rights = "";
        ...
        public void ExportToExcel(string prodHouseCode = "", string statusCode = "", string titleCodes = "", string usersCodes = "", string requestId = "", string fromDate = "", string toDate = "", int pageNo = 1, int recordPerPage = 10)
        {
            titleCodes = titleCodes.Replace('﹐', ',');
            usersCodes = usersCodes.Replace('﹐', ',');
            int RecordCount = 0;
            ...
            parm[6] = new ReportParameter("TitleCode", string.IsNullOrEmpty(titleCodes) ? " " : titleCodes);
            ...
            Response.AddHeader("Content-disposition", "filename=MHStatusHistory.xls");
    ...
    public class MHStatusHistory_Search
    {
