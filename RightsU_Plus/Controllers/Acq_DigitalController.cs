...
namespace RightsU_Plus.Controllers
{
    public class Acq_DigitalController : BaseController
    {
        ...
        public JsonResult BindDigital(int page_index, int page_size)
        {
            ...
            foreach (string Dlttl in Digital_Tab_Title)
            {

                List<USP_Acq_Deal_Digital_List_Result> objDigital_ListtitleData = objDigital_List.Where(x => x.title_name == Dlttl).ToList();
                strList = strList + "<TR><TD>" + Dlttl + "</TD>";
                string Title_Code = ""; string Acq_Deal_Digital_Code = "";
                foreach (USP_Acq_Deal_Digital_List_Result dg in objDigital_ListtitleData)
                {
                    Title_Code = dg.Title_Code.ToString();
                    Acq_Deal_Digital_Code = dg.Acq_Deal_Digital_Code.ToString();
                    strList = strList + "<TD>" + dg.Remarks + "</TD>";

                }
                //strList = strList + "<TR><TD>" + sl.title_name + "</TD><TD>" + sl.SocialMedia + "</TD><TD>" + sl.Commitments + "</TD><TD>" + sl.OpeningClosingCredits + "</TD><TD>" + sl.EssentialClauses + "</TD><TD><a title=\"Edit\" class=\"glyphicon glyphicon-pencil\" onclick=\"Edit(" + Convert.ToString(sl.Supplementary_code) + "," + Convert.ToString(sl.title_code) + ",'');\" ></a><a title=\"Edit\" class=\"glyphicon glyphicon-eye-open\" onclick=\"Edit(" + Convert.ToString(sl.Supplementary_code) + "," + Convert.ToString(sl.title_code) + ",'VIEW');\" ></a><a title=\"Delete\" class=\"glyphicon glyphicon-trash\" onclick=\"Delete(" + Convert.ToString(sl.Supplementary_code) + ',' + Convert.ToString(sl.title_code) + ");\"></a></TD></Tr>";
                if (objDeal_Schema.Mode != "V" && objDeal_Schema.Mode != "APRV")
                {
                    strList = strList + "<TD><a title=\"Edit\" class=\"glyphicon glyphicon-pencil\" onclick=\"Edit(" + Convert.ToString(Acq_Deal_Digital_Code) + "," + Convert.ToString(Title_Code) + ",'');\" ></a><a title=\"VIEW\" class=\"glyphicon glyphicon-eye-open\" onclick=\"Edit(" + Convert.ToString(Acq_Deal_Digital_Code) + "," + Convert.ToString(Title_Code) + ",'VIEW');\" ></a><a title=\"Delete\" class=\"glyphicon glyphicon-trash\" onclick=\"Delete(" + Convert.ToString(Acq_Deal_Digital_Code) + ',' + Convert.ToString(Title_Code) + ");\"></a></TD></Tr>";
                }
                else
                {
                    strList = strList + "<TD><a title=\"VIEW\" class=\"glyphicon glyphicon-eye-open\" onclick=\"Edit(" + Convert.ToString(Acq_Deal_Digital_Code) + "," + Convert.ToString(Title_Code) + ",'VIEW');\" ></a></TD></Tr>";
                }
            }
        ...
        public JsonResult BindAllPreReq_Async()
        {
            ...
            List<RightsU_Entities.Digital_Tab> objDigital_Tab = objService.SearchFor(x => x.Module_Code.Value == GlobalParams.ModuleCodeForAcqDeal).Where(x => x.Is_Show == "Y" || x.Is_Show == "N").OrderBy(a => a.Order_No).ToList();
