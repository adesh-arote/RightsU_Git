...
{
    public class Acq_AncillaryController : BaseController
    {
        #region --- Properties ---
        ...
        public PartialViewResult Index()
        {
            ...
            List<USP_List_Acq_Ancillary_Result> lst = new List<USP_List_Acq_Ancillary_Result>();
        ...
        public ActionResult Cancel()
        {
            ...
            //return DependencyResolver.Current.GetService<RightsU_Plus.Controllers.GlobalController>().RedirectToControl("");
        }
        ...
        public PartialViewResult Bind_Grid_List(int txtpageSize = 1, int page_index = 0, string IsCallFromPaging = "N")
        {
            ...
            if (IsCallFromPaging == "N")
                objDeal_Schema.List_Ancillary.Clear();
            Fill_Session(txtpageSize, PageNo);
            if (objDeal_Schema.List_Ancillary.Count == 0 || objDeal_Schema.Deal_Type_Condition == GlobalParams.Deal_Program || objDeal_Schema.Deal_Type_Condition == GlobalParams.Deal_Music)
                objDeal_Schema.List_Ancillary = new USP_Service(objLoginEntity.ConnectionStringName).USP_List_Acq_Ancillary(objDeal_Schema.Deal_Code).OrderBy(a => a.TitleName).ToList();
            AssignPaging(txtpageSize);
            List<USP_List_Acq_Ancillary_Result> lst = Get_Paging_List(txtpageSize, PageNo);
            if (PageNo > 1 && lst.Count == 0 && objDeal_Schema.List_Ancillary.Count > 0)
            {
                PageNo = 1;
                lst = Get_Paging_List(txtpageSize, PageNo);
            }
            ViewBag.RecordCount = objDeal_Schema.List_Ancillary.Where(r => r.PageNo == PageNo).Count();
            ViewBag.RecordCount_Display = objDeal_Schema.List_Ancillary.Select(i => i.Acq_Deal_Ancillary_Code).Distinct().Count();
            ...
            return PartialView("~/Views/Acq_Deal/_List_Ancillary.cshtml", lst);
        }
        private MultiSelectList BindTitle(string selected_Title_Code = "")
        {
            return DependencyResolver.Current.GetService<RightsU_Plus.Controllers.GlobalController>().BindTitle_List(objDeal_Schema.Deal_Code, objDeal_Schema.Deal_Type_Code, selected_Title_Code);
        }
        #endregion
        #region --- Create / Edit / Delete ---
        private void BindAllViewBag(string selected_Title_Code, int selected_Type_Code, string selected_Acq_Deal_Ancillary_Code, string selected_Medium_Code)
        {
            ViewBag.Title_List_Popup = BindTitle(selected_Title_Code);
            ViewBag.Type_List_Popup = BindAncillaryType(selected_Type_Code);
            if (selected_Acq_Deal_Ancillary_Code != "")
                ViewBag.Rights_List_Popup = Bindddl_Right(selected_Type_Code, selected_Acq_Deal_Ancillary_Code);
            else
            ...
            if (selected_Medium_Code != "")
                ViewBag.Medium_List_Popup = Bindddl_Medium(selected_Acq_Deal_Ancillary_Code, selected_Medium_Code);
            else
            {
        ...
        {
            ViewBag.Mode = Mode;
            Acq_Deal_Ancillary obj = new Acq_Deal_Ancillary_Service(objLoginEntity.ConnectionStringName).GetById(selected_Acq_Deal_Ancillary_Code);
            ...
            if (objDeal_Schema.Deal_Type_Condition == GlobalParams.Deal_Program || objDeal_Schema.Deal_Type_Condition == GlobalParams.Deal_Music)
                selected_Title_Code = string.Join(",", objDeal_Schema.Title_List.Where(x => obj.Acq_Deal_Ancillary_Title.Where(y => y.Title_Code == x.Title_Code && y.Episode_From == x.Episode_From && y.Episode_To == x.Episode_To).Count() > 0).Select(s => s.Acq_Deal_Movie_Code).ToArray());
            else
                selected_Title_Code = string.Join(",", obj.Acq_Deal_Ancillary_Title.Select(i => i.Title_Code).ToArray());
            int selected_Type_Code = obj.Ancillary_Type_code ?? 0;
            string selected_Platform_Code = string.Join(",", obj.Acq_Deal_Ancillary_Platform.Select(i => i.Ancillary_Platform_code).Distinct().ToArray());
            string selected_Medium_Code = "";
            if (obj.Acq_Deal_Ancillary_Platform.Select(i => i.Acq_Deal_Ancillary_Platform_Medium).Count() > 0)
                selected_Medium_Code = string.Join(",", obj.Acq_Deal_Ancillary_Platform.SelectMany(i => i.Acq_Deal_Ancillary_Platform_Medium).Select(i => i.Ancillary_Platform_Medium_Code).Distinct().ToArray());
        ...
        {
            Acq_Deal_Ancillary_Service objAcq_Deal_Ancillary_Service = new Acq_Deal_Ancillary_Service(objLoginEntity.ConnectionStringName);
            Acq_Deal_Ancillary objAcq_Deal_Ancillary = objAcq_Deal_Ancillary_Service.GetById(acq_Deal_Ancillary_Code ?? 0);
            try
            {
        ...
        private string Save(Acq_Deal_Ancillary objAcqDeal_AncillaryTemp, ref Acq_Deal_Ancillary_Service objAcq_Deal_Ancillary_Service, bool isDelete)//Final Save
        {
            dynamic resultSet;
            string msg = "E";
            string strMode = objAcqDeal_AncillaryTemp.Acq_Deal_Ancillary_Code > 0 ? "U" : "A";
            if (!isDelete)
        ...
        public JsonResult Save_Ancillary(Acq_Deal_Ancillary objAcq_Deal_Ancillary_Temp, string Ancillary_Mode, string CatchUp, string hdnTitles, string hdnTVCodes, string hdnAncillaryRightCode, string hdnMedium, int? page_index, int? txtpageSize)//
        {
            Acq_Deal_Ancillary_Service objAcq_Deal_Ancillary_Service = new Acq_Deal_Ancillary_Service(objLoginEntity.ConnectionStringName);
            Acq_Deal_Ancillary objAcq_Deal_Ancillary = new Acq_Deal_Ancillary();
            string msg = "";
            ...
            try
            {
                if (hdnTitles != "")
                {
                    ...
                    objAcq_Deal_Ancillary.Acq_Deal_Code = objDeal_Schema.Deal_Code;
                    ...
                    #region --- Title ---
                    if (objAcq_Deal_Ancillary.Acq_Deal_Ancillary_Title == null)
                        objAcq_Deal_Ancillary.Acq_Deal_Ancillary_Title = new HashSet<Acq_Deal_Ancillary_Title>();
                    objAcq_Deal_Ancillary.Acq_Deal_Ancillary_Title.ToList<Acq_Deal_Ancillary_Title>().ForEach(x => x.EntityState = State.Deleted);
                    foreach (string strCode in arrCodes)
                    {
                        Acq_Deal_Ancillary_Title objAcq_Deal_Ancillary_Title = new Acq_Deal_Ancillary_Title();
                        ...
                        if (code > 0)
                        {
                            ...
                            objAcq_Deal_Ancillary.Acq_Deal_Ancillary_Title.Add(objAcq_Deal_Ancillary_Title);
                        }
                        Deal_Ancillary_Title_UDT objATUDT = new Deal_Ancillary_Title_UDT();
                        objATUDT.Deal_Ancillary_Code = objAcq_Deal_Ancillary.Acq_Deal_Ancillary_Code;
                    ...
                    if (isAdvAncillary == "N")
                    {
                        ...
                        if (hdnAncillaryRightCode.Trim() != "")
                        {
                            ...
                            objAcq_Deal_Ancillary.Acq_Deal_Ancillary_Platform.ToList<Acq_Deal_Ancillary_Platform>().ForEach(x => x.EntityState = State.Deleted);
                            objAcq_Deal_Ancillary.Acq_Deal_Ancillary_Platform.SelectMany(i => i.Acq_Deal_Ancillary_Platform_Medium).ToList<Acq_Deal_Ancillary_Platform_Medium>().ForEach(x => x.EntityState = State.Deleted);
                            foreach (string strPlatformCode in arrAncillaryPlafrom)
                            {
                                ...
                                Acq_Deal_Ancillary_Platform objAcqDealAncillary_Platform = null;
                                if (objAcq_Deal_Ancillary.Acq_Deal_Ancillary_Code > 0)
                                    objAcq_Deal_Ancillary.Acq_Deal_Ancillary_Platform.Select(i => i.Ancillary_Platform_code == Ancillary_PlatformCode).FirstOrDefault();
                                if (objAcqDealAncillary_Platform == null)
                                {
                                    objAcqDealAncillary_Platform = new Acq_Deal_Ancillary_Platform();
                                    objAcqDealAncillary_Platform.EntityState = State.Added;//Set Entity State Added
                                    objAcqDealAncillary_Platform.Acq_Deal_Ancillary_Code = objAcq_Deal_Ancillary.Acq_Deal_Ancillary_Code;
                                ...
                                objAPUDT.Deal_Ancillary_Code = objAcq_Deal_Ancillary.Acq_Deal_Ancillary_Code;
                                    ...
                                    if (arrGet_Ancillary_Platform_Medium_Code != null && arrGet_Ancillary_Platform_Medium_Code.Count() > 0)
                                        if (arrGet_Ancillary_Platform_Medium_Code != null && arrGet_Ancillary_Platform_Medium_Code.Count() > 0)
                                            foreach (string strPlatformMedium_Code in arrGet_Ancillary_Platform_Medium_Code)
                                            {
                                                ...
                                                Acq_Deal_Ancillary_Platform_Medium objAcqDealAncillaryPlatform_Medium = null;
                                                if (objAcqDealAncillary_Platform != null && objAcq_Deal_Ancillary.Acq_Deal_Ancillary_Code > 0)
                                                    objAcqDealAncillaryPlatform_Medium = objAcqDealAncillary_Platform.Acq_Deal_Ancillary_Platform_Medium.Where(i => i.Ancillary_Platform_Medium_Code == Ancillary_Platform_MediumCode).Select(i => i).FirstOrDefault();
                                                if (objAcqDealAncillaryPlatform_Medium == null)
                                                {
                                                    objAcqDealAncillaryPlatform_Medium = new Acq_Deal_Ancillary_Platform_Medium();
                                                    objAcqDealAncillaryPlatform_Medium.EntityState = State.Added;
                                                    objAcqDealAncillaryPlatform_Medium.Acq_Deal_Ancillary_Platform_Code = objAcqDealAncillary_Platform.Acq_Deal_Ancillary_Platform_Code;
                                                    objAcqDealAncillaryPlatform_Medium.Ancillary_Platform_Medium_Code = Ancillary_Platform_MediumCode;
                                                    objAcqDealAncillary_Platform.Acq_Deal_Ancillary_Platform_Medium.Add(objAcqDealAncillaryPlatform_Medium);
                                                }
                                                else
                                                ...
                                                objAPMUDT.Deal_Ancillary_Code = objAcq_Deal_Ancillary.Acq_Deal_Ancillary_Code;
                                ...
                                objAcq_Deal_Ancillary.Acq_Deal_Ancillary_Platform.Add(objAcqDealAncillary_Platform);
                            }
                        }
                        #endregion
                    }
                        ...
                        if (hdnTVCodes.Trim() != "")
                        {
                            ...
                            foreach (string platformCode in PTitle_Codes)
                            {
                                if (platformCode != "" && platformCode != "0")
                                {
                                    Acq_Deal_Ancillary_Platform objAcqDealAncillary_Platform = null;
                                    if (objAcq_Deal_Ancillary.Acq_Deal_Ancillary_Code > 0)
                                        objAcq_Deal_Ancillary.Acq_Deal_Ancillary_Platform.Where(t => t.Platform_Code == Convert.ToInt32(platformCode)).Select(i => i).FirstOrDefault();
                                    if (objAcqDealAncillary_Platform == null)
                                    {
                                        objAcqDealAncillary_Platform = new Acq_Deal_Ancillary_Platform();
                                        objAcqDealAncillary_Platform.EntityState = State.Added;//Set Entity State Added
                                        objAcqDealAncillary_Platform.Acq_Deal_Ancillary_Code = objAcq_Deal_Ancillary.Acq_Deal_Ancillary_Code;
                                        objAcqDealAncillary_Platform.Platform_Code = Convert.ToInt32(platformCode);
                                    }
                                    else
                                        objAcqDealAncillary_Platform.EntityState = State.Unchanged;
        ...
        {
            Acq_Deal_Ancillary objADR = new Acq_Deal_Ancillary_Service(objLoginEntity.ConnectionStringName).GetById(acqDealAncillaryCode);
            Platform_Tree_View objPTV = new Platform_Tree_View(objLoginEntity.ConnectionStringName);
            string strPlatform = string.Join(",", objADR.Acq_Deal_Ancillary_Platform.Select(s => s.Platform_Code).ToArray());
        ...
        private bool AssignPaging(int txtPageSize)
        {
            ...
            int pNo = 0;
            objDeal_Schema.List_Ancillary.ForEach(r =>
                {
                    if (Acq_Ancillary_Code != r.Acq_Deal_Ancillary_Code)
                    {
                        Acq_Ancillary_Code = r.Acq_Deal_Ancillary_Code;
                        r.PageNo = pNo + 1;
                        pNo = r.PageNo ?? 0;
                    }
                    else
