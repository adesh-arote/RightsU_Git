...
namespace RightsU_Plus.Controllers
{
    public class System_LanguageController : BaseController
    {
        ...
        private List<RightsU_Entities.System_Language> lstSystem_Language
        {
            get
            {
                if (Session["lstSystem_Language"] == null)
                    Session["lstSystem_Language"] = new List<RightsU_Entities.System_Language>();
                return (List<RightsU_Entities.System_Language>)Session["lstSystem_Language"];
            }
        ...
        public PartialViewResult AddEdit(int SystemLanguageCode, int RecordLockingCode)
        {
            ...
            System_Language objL = objService.GetById(SystemLanguageCode);
        ...
        public PartialViewResult BindSystem_LanguageList(int moduleCode, string formId)
        {
            ...
            TempData.Keep();
            ViewBag.GetModules = new SelectList(new System_Module_Service(objLoginEntity.ConnectionStringName).SearchFor(x => x.Is_Active == "Y" && x.Is_Sub_Module == "N").ToList().OrderBy(x => x.Module_Name), "Module_Code", "Module_Name", moduleCode);
            ViewBag.GetFormId = new MultiSelectList(new System_Module_Message_Service(objLoginEntity.ConnectionStringName).SearchFor(x => true).Select(i => new { Form_ID = i.Form_ID }).Distinct().OrderBy(x => x.Form_ID).ToList(), "Form_ID", "Form_ID", formId);
            USP_Service service = new USP_Service(objLoginEntity.ConnectionStringName);
            List<USP_GetSystem_Language_Message_ByModule_Result> list = service.USP_GetSystem_Language_Message_ByModule(moduleCode, formId, SysLanguageCode).ToList();
        ...
        public ActionResult Save(System_Language objMVC)
        {
            ...
            {
                //Update start
                System_Language objSL = objService.GetById(objMVC.System_Language_Code);
                ...
                if (!objService.Save(objSL, out resultSet))
                {
                    status = "E";
        ...
        {  
            dynamic list = new System_Module_Message_Service(objLoginEntity.ConnectionStringName).SearchFor(x => (x.Module_Code ?? 0) == moduleCode)
        ...
        public PartialViewResult BindSystemLanguageList(int pageNo, int recordPerPage, string commandName)
        {
            ...
            if (RecordCount > 0)
            {
                ...
                ViewBag.TotalMessages = new System_Module_Message_Service(objLoginEntity.ConnectionStringName).SearchFor(x => true).Count();
            }
            ViewBag.UserModuleRights = GetUserModuleRights();
        ...
        public JsonResult SaveSystemLanguage(int SystemLanguageCode, string SystemLanguageName, string Direction, int Record_Code)
        {
            ...
            objSystemLanguage = new RightsU_Entities.System_Language();
            ...
            dynamic resultSet;
            bool isValid = objService.Save(objSystemLanguage, out resultSet);
            if (isValid)
            {
