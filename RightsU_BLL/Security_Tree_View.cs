...
{
    public class Security_Tree_View
    {
        public string _Connection_Str;

        public Security_Tree_View(string connectionstring)
        {
        ...
        public string PopulateTreeNode(string IsView, string Security_Hiearachy_Search = "")
        {
            ...
            List<USP_Get_Security_Group_Tree_Hierarchy_Result> lstSecurity = objUS.USP_Get_Security_Group_Tree_Hierarchy(SecurityCodes_Display, Security_Hiearachy_Search).ToList();
            ...
            List<USP_Get_Security_Group_Tree_Hierarchy_Result> arr = lstSecurity.Where(x => x.Parent_Module_Code == 0).ToList();

            bool IsMenuReference = false, IsMenuChecked = false;
            for (int i = 0; i < arr.Count; i++)
            {
                USP_Get_Security_Group_Tree_Hierarchy_Result objSecurity = arr[i];
        ...
        private bool AddChildDetailForGroup(int Security_Code, int? RightsCount, List<USP_Get_Security_Group_Tree_Hierarchy_Result> lstSecurity, string IsView, out bool IsReference, out string strChild, string Security_Hiearachy_Search = "")
        {
            RightsU_Entities.System_Right objSR = new System_Right();

            List<USP_Get_Security_Group_Tree_Hierarchy_Result> arr;
