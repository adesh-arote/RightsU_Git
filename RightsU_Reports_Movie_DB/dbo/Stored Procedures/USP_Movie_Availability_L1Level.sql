CREATE PROCEDURE [dbo].[USP_Movie_Availability_L1Level]
(
...
--)
	...
	''', @Is_Digital=''' + CAST(ISNULL(@Is_Digital, '') AS VARCHAR(MAX)), 'MAvailL1'
	...
	SELECT REPLACE(number, 'L', '') FROM fn_Split_withdelemiter(ISNULL(@Subtitling_MustHave,'0'), ',') --WHERE number LIKE 'L%' AND number NOT IN('0', '')
	
	INSERT INTO #Temp_Dub_MH(Language_Code)
	SELECT REPLACE(number, 'L', '') FROM fn_Split_withdelemiter(ISNULL(@Dubbing_MustHave,'0'), ',') --WHERE number LIKE 'L%' AND number NOT IN('0', '')
	
	SELECT CAST(number AS INT) SubLicense_Code INTO #Tmp_SL FROM fn_Split_withdelemiter(ISNULL(@SubLicense_Code,0), ',') WHERE number NOT IN('0', '')
	------------------ END
	...
	CREATE  TABLE #Temp_Main_Ctr_obl
	(
		DenseRank BIGINT,
		Avail_Raw_Code Numeric(38,0),
		Title_Code Int, 
  ...
  DROP TABLE #TMP_MAIN  
   ...
   END		
	...
	BEGIN /*New Logic*/
		...
		INSERT INTO #Temp_Main_Ctr_obl(DenseRank,Avail_Raw_Code,Title_Code,Platform_Code,Country_Code,Cluster_Names,Right_Start_Date,Rights_End_Date,Country_Names,Acq_Deal_Rights_Holdback_Code,Holdback_Type,Holdback_Release_Date,Holdback_On_Platform_Code,Reverse_Holdback,Group_no)
	...
	BEGIN
		...
		--SELECT * 
		UPDATE ctr SET ctr.Base_Platform_Code = CASE WHEN ISNULL(pl.Base_Platform_Code, 0) = 0 THEN pl.Platform_Code ELSE pl.Base_Platform_Code END
		FROM #Temp_Main_Ctr_obl ctr
		INNER JOIN Platform pl ON ctr.Platform_Code = pl.Platform_Code

		--SELECT * FROM #Temp_Main_Ctr_obl
		...
		FROM #Temp_Main_Ctr_obl ctr
		INNER JOIN (
			SELECT Avail_Raw_Code, Title_Code, Country_Code, Is_Title_Language, Sub_Language_Code, Dub_Language_code, Country_Cd_Str, 
				   Cluster_Names, Right_Start_Date, Rights_End_Date, Country_Names, Platform_Str, Title_Language_Names, Sub_Language_Names, Dub_Language_Names, 
				   Acq_Deal_Rights_Holdback_Code, Holdback_Type, Holdback_Release_Date, Holdback_On_Platform_Code, Reverse_Holdback, Group_no, Objection_Platform, 
				   Objection_Region, Objection_Start_Date, Objection_End_Date, Under_Litigation, Base_Platform_Code, Actual_Platform_Count, COUNT(*) AvailCnt
			FROM #Temp_Main_Ctr_obl
			GROUP BY Avail_Raw_Code, Title_Code, Country_Code, Is_Title_Language, Sub_Language_Code, Dub_Language_code, Country_Cd_Str, 
					 Cluster_Names, Right_Start_Date, Rights_End_Date, Country_Names, Platform_Str, Title_Language_Names, Sub_Language_Names, Dub_Language_Names, 
					 Acq_Deal_Rights_Holdback_Code, Holdback_Type, Holdback_Release_Date, Holdback_On_Platform_Code, Reverse_Holdback, Group_no, Objection_Platform, 
					 Objection_Region, Objection_Start_Date, Objection_End_Date, Under_Litigation, Base_Platform_Code, Actual_Platform_Count
		) AS avail ON ISNULL(ctr.Avail_Raw_Code, 0) = ISNULL(avail.Avail_Raw_Code, 0) AND 
		ISNULL(ctr.Title_Code, 0) = ISNULL(avail.Title_Code, 0) AND 
		ISNULL(ctr.Country_Code, 0) = ISNULL(avail.Country_Code, 0) AND 
		ISNULL(ctr.Is_Title_Language, '') = ISNULL(avail.Is_Title_Language, '') AND 
		ISNULL(ctr.Sub_Language_Code, '') = ISNULL(avail.Sub_Language_Code, '') AND 
		ISNULL(ctr.Dub_Language_code, '') = ISNULL(avail.Dub_Language_code, '') AND 
		ISNULL(ctr.Country_Cd_Str, '') = ISNULL(avail.Country_Cd_Str, '') AND 
		ISNULL(ctr.Cluster_Names, '') = ISNULL(avail.Cluster_Names, '') AND 
		ISNULL(ctr.Right_Start_Date, '') = ISNULL(avail.Right_Start_Date, '') AND 
		ISNULL(ctr.Rights_End_Date, '') = ISNULL(avail.Rights_End_Date, '') AND 
		ISNULL(ctr.Country_Names, '') = ISNULL(avail.Country_Names, '') AND 
		ISNULL(ctr.Platform_Str, '') = ISNULL(avail.Platform_Str, '') AND 
		ISNULL(ctr.Title_Language_Names, '') = ISNULL(avail.Title_Language_Names, '') AND 
		ISNULL(ctr.Sub_Language_Names, '') = ISNULL(avail.Sub_Language_Names, '') AND 
		ISNULL(ctr.Dub_Language_Names, '') = ISNULL(avail.Dub_Language_Names, '') AND 
		ISNULL(ctr.Acq_Deal_Rights_Holdback_Code, '') = ISNULL(avail.Acq_Deal_Rights_Holdback_Code, '') AND 
		...
		ISNULL(ctr.Holdback_On_Platform_Code, '') = ISNULL(avail.Holdback_On_Platform_Code, '') AND 
		ISNULL(ctr.Reverse_Holdback, '') = ISNULL(avail.Reverse_Holdback, '') AND 
		ISNULL(ctr.Group_no, '') = ISNULL(avail.Group_no, '') AND 
		ISNULL(ctr.Objection_Platform, '') = ISNULL(avail.Objection_Platform, '') AND 
		ISNULL(ctr.Objection_Region, '') = ISNULL(avail.Objection_Region, '') AND 
		ISNULL(ctr.Objection_Start_Date, '') = ISNULL(avail.Objection_Start_Date, '') AND 
		ISNULL(ctr.Objection_End_Date, '') = ISNULL(avail.Objection_End_Date, '') AND 
		ISNULL(ctr.Under_Litigation, '') = ISNULL(avail.Under_Litigation, '') AND 
		ISNULL(ctr.Base_Platform_Code, '') = ISNULL(avail.Base_Platform_Code, '')

		INSERT INTO #Temp_Main_Ctr_New(Avail_Raw_Code, Title_Code, Country_Code, Is_Title_Language, Sub_Language_Code, Dub_Language_code, Country_Cd_Str, 
				   Cluster_Names, Right_Start_Date, Rights_End_Date, Country_Names, Platform_Str, Title_Language_Names, Sub_Language_Names, Dub_Language_Names, 
				   Acq_Deal_Rights_Holdback_Code, Holdback_Type, Holdback_Release_Date, Holdback_On_Platform_Code, Reverse_Holdback, Group_no, Objection_Platform, 
				   Objection_Region, Objection_Start_Date, Objection_End_Date, Under_Litigation, 
				   Platform_Code, Available)
		SELECT DISTINCT Avail_Raw_Code, Title_Code, Country_Code, Is_Title_Language, Sub_Language_Code, Dub_Language_code, Country_Cd_Str, 
			   Cluster_Names, Right_Start_Date, Rights_End_Date, Country_Names, Platform_Str, Title_Language_Names, Sub_Language_Names, Dub_Language_Names, 
			   Acq_Deal_Rights_Holdback_Code, Holdback_Type, Holdback_Release_Date, Holdback_On_Platform_Code, Reverse_Holdback, Group_no, Objection_Platform, 
			   Objection_Region, Objection_Start_Date, Objection_End_Date, Under_Litigation, 
	...
	print 'ad'
		SELECT DISTINCT
			...
			trr.Sub_License_Name AS Sub_License, Available AS Platform_Avail,
...
Exec USP_Movie_Availability	@Title_Code='12336', 
	@Is_Original_Language=1, 
	@Title_Language_Code='',
	@Date_Type ='FL',
