CREATE PROCEDURE [dbo].[USP_Movie_Availability_IFTA_SAVE]
(
...
BEGIN 
	...
	--select * from title where title_Name like '%dell%'
	--select 	
	--@Title_Code='', 
 ...
 --   @Country_Code='1, 10, 100, 101, 102, 103, 104, 105, 106, 107, 108, 109, 110, 111, 112, 113, 114, 115, 116, 117, 118, 119, 12, 120, 121, 122, 123, 124, 125, 127, 128, 13, 130, 131, 132, 133, 134, 135, 137, 138, 139, 14, 140, 141, 142, 143, 145, 146, 147, 148, 149, 15, 16, 160, 164, 166, 167, 168, 169, 17, 170, 18, 182, 183, 184, 188, 189, 19, 190, 191, 192, 193, 2, 20, 200, 204, 205, 206, 209, 210, 211, 212, 213, 214, 215, 216, 217, 218, 219, 22, 220, 221, 222, 223, 224, 225, 226, 227, 228, 229, 23, 230, 231, 232, 235, 236, 237, 238, 239, 24, 246, 247, 248, 249, 250, 251, 252, 255, 256, 257, 258, 264, 265, 266, 267, 268, 269, 27, 270, 271, 272, 273, 275, 276, 277, 278, 279, 28, 280, 281, 282, 283, 284, 285, 286, 287, 288, 289, 293, 294, 295, 296, 299, 3, 30, 300, 301, 302, 304, 305, 306, 307, 308, 309, 31, 310, 311, 312, 313, 32, 35, 36, 37, 38, 39, 4, 40, 41, 42, 44, 47, 48, 5, 50, 51, 52, 53, 54, 55, 57, 58, 59, 6, 60, 61, 62, 65, 66, 67, 73, 74, 75, 76, 77, 8, 82, 83, 84, 85, 93, 94, 95, 96, 97, 98, 99, 314, 315, 316, 317, 318, 319, 320, 321, 322, 324, 325, 326, 327, 328, 329, 330, 331, 332, 333, 334, 335, 336, 337, 338, 341', 
	...
	--@Subtitling_ExactMatch = 'false', --MH/EM/false
	--@Subtitling_MustHave = '',
	--@Subtitling_Exclusion = '0',
	
	--@Dubbing_Language_Code='', 
	--@Dubbing_ExactMatch = 'false',
	--@Dubbing_MustHave = '',
	--@Dubbing_Exclusion = '0',
...
--)
 ...
 --   ',@Country_Code='+@Country_Code+ 
	...
	Set @StartDate = Case When IsNull(@StartDate, '') = '' Then REPLACE(CONVERT(VARCHAR,getdate()-1,106), ' ','') Else @StartDate End
	Set @EndDate = Case When IsNull(@EndDate, '') = '' Then '31Mar9999' Else @EndDate End
	...
	Begin
		...
		Insert InTo #TMP_MAIN(Title_Code, Platform_Code, Country_Code, Avail_Acq_Code, Avail_Raw_Code, Is_Title_Language, Sub_Language_Code, Dub_Language_Code)
		Select AA.Title_Code, AA.Platform_Code, AA.Country_Code, asd.Avail_Acq_Code, asd.Avail_Raw_Code, Case When 1 = 1 Then asd.Is_Title_Language Else 0 End, asd.Sub_Language_Code, asd.Dub_Language_Code
		From #Avail_Acq AA
		Inner Join Avail_Acq_Details asd On asd.Avail_Acq_Code = AA.Avail_Acq_Code
		Inner Join #Avail_Raw ar On ar.Avail_Raw_Code = asd.Avail_Raw_Code
		Where 
	...
	BEGIN
		...
		--SELECT Country_Cd_Str FROM #TMP_MAIN 
   ...
   END		
	...
	BEGIN
	print 'ad'
		INSERT INTO Movie_Avail_Data(Title_Name,Platform_Name,COL1,COL2,COL3,Right_Start_Date,Rights_End_Date,Title_Language_Names,Sub_Language_Names,Dub_Language_Names,Genres_Name,Star_Cast,Director,Duration_In_Min,Year_Of_Production,Restriction_Remark,Sub_Deal_Restriction_Remark,Remarks,Rights_Remarks,Exclusive,Sub_License,Platform_Avail,HoldbackOn,Holdback_Type,Holdback_Release_Date,Reverse_Holdback,Ancillary_Rights,BU_Code)
		SELECT DISTINCT
			...
			Reverse_Holdback COLLATE SQL_Latin1_General_CP1_CI_AS As Reverse_Holdback, '' AS Ancillary_Rights,CAST(@BU_Code AS VARCHAR)
		FROM
			#Temp_Main_Ctr tm
		...
		Reverse_Holdback AS Reverse_Holdback, '' AS Ancillary_Rights,CAST(@BU_Code AS VARCHAR)
...
--Select * from Movie_Avail_Data
--CREATE TABLE Movie_Avail_Data(
...
--	Rights_End_Date NVARCHAR(MAX),
--	Title_Language_Names NVARCHAR(MAX),
--	Sub_Language_Names NVARCHAR(MAX),
--	Dub_Language_Names NVARCHAR(MAX),
