BEGIN
	...
	DECLARE @Condition NVARCHAR(MAX) = 'BUSINESS_UNIT_CODE~AND~=~1|EXPIRED~AND~=~N|IS_THEATRICAL_RIGHT~=~N|SDR.IS_HOLDBACK~AND~IN~Y|'
	DECLARE @ColNames NVARCHAR(MAX) = 'Holdback,'


	--DECLARE @Condition NVARCHAR(MAX) = 'BUSINESS_UNIT_CODE~AND~=~1|EXPIRED~AND~=~Y|SD.DEAL_DESCRIPTION~AND~LIKE~EROS%|'
	--DECLARE @ColNames NVARCHAR(MAX) = 'Title,agreement no,Deal Description'
	...
	DECLARE @tblTitle TABLE(Title_Code INT, Original_Title NVARCHAR(MAX), Original_Dubbed NVARCHAR(MAX), Year_of_Release INT)
	...
	BEGIN
		...
		BEGIN
			...
			SELECT Title_Code, Original_Title, ISNULL(OD.Columns_Value,'NA'), Year_Of_Production FROM Title t 
			LEFT JOIN #OriDub_Title_Code OD ON OD.Record_Code = T.Title_Code
			
			INSERT INTO #tempRights(Syn_Deal_Code, Syn_Deal_Rights_Code, Title_Code, Right_Start_Date, Right_End_Date, [Sub-Licensing], Exclusive, Term, Episode_From, Episode_To)
		...
		BEGIN
			...
			BEGIN
				...
				BEGIN
					...
					EXEC ('Select Distinct t.Title_Code,t.Original_Title,ISNULL(OD.Columns_Value,''NA''),t.Year_Of_Production 
						   FROM Title t 
						   LEFT JOIN #OriDub_Title_Code OD ON OD.Record_Code = T.Title_Code
					...
					INSERT INTO #tempTitle(Title_Code, Original_Title,Original_Dubbed, Year_of_Release)
					SELECT Title_Code,Original_Title,ISNULL(OD.Columns_Value,'NA'),Year_of_Release FROM @tblTitle T
					LEFT JOIN #OriDub_Title_Code OD ON OD.Record_Code = T.Title_Code
			...
			BEGIN
				...
				BEGIN
					...
					INSERT INTO @tblTitle(Title_Code,Original_Title,Original_Dubbed,Year_of_Release)
					EXEC ('Select Distinct tg.Title_Code,t.Original_Title,ISNULL(OD.Columns_Value,''NA''),t.Year_Of_Production FROM Title_Talent tg 
			...
			BEGIN
				...
				BEGIN
					...
					INSERT INTO @tblTitle(Title_Code,Original_Title,Original_Dubbed,Year_of_Release)
					EXEC ('Select Distinct tg.Title_Code,t.Original_Title,ISNULL(OD.Columns_Value,''NA''),t.Year_Of_Production FROM Title_Geners tg 
						   INNER JOIN Title t ON t.Title_Code = tg.Title_Code
						   LEFT JOIN #OriDub_Title_Code OD ON OD.Record_Code = T.Title_Code
			...
			BEGIN
				...
				BEGIN
					...
					INSERT INTO @tblTitle (Title_Code, Original_Title,Original_Dubbed, Year_of_Release)
					EXEC ('SELECT DISTINCT t.Title_Code, t.Original_Title,ISNULL(OD.Columns_Value,''NA''), t.Year_Of_Production FROM Title T
							LEFT JOIN #OriDub_Title_Code OD ON OD.Record_Code = T.Title_Code
		...
		BEGIN
			...
			BEGIN
				...
				BEGIN
					...
					INSERT INTO @tblDeal(Syn_Deal_Code, Agreement_Date, Agreement_No, Deal_Description, Deal_Workflow_Status, Currency_Code, Deal_Category_Code, Entity_Code, Vendor_Code,Sales_Agent_Code,Deal_Tag_Code, Deal_Type_Code, Customer_Type)
					EXEC ('SELECT AD.Syn_Deal_Code, Agreement_Date, Agreement_No, Deal_Description, Deal_Workflow_Status, ad.Currency_Code, ad.Category_Code, ad.Entity_Code, ad.Vendor_Code,ad.Sales_Agent_Code,ad.Deal_Tag_Code, ad.Deal_Type_Code, ad.Customer_Type
						   FROM Syn_Deal ad
					...
					INSERT INTO #tempTitle(Title_Code, Original_Title,Original_Dubbed, Year_of_Release)
					SELECT Distinct tr.Title_Code,t.Original_Title,ISNULL(OD.Columns_Value,'NA'),t.Year_Of_Production FROM #tempRights tr
			...
			BEGIN
				...
				BEGIN
					...
					INSERT INTO #tempTitle(Title_Code, Original_Title, Original_Dubbed,Year_of_Release)-- Title_Language_Code
					SELECT Distinct tr.Title_Code,t.Original_Title,ISNULL(OD.Columns_Value,'NA'),t.Year_Of_Production FROM #tempRights tr  --, Title_Language_Code
					INNER JOIN Title t ON t.Title_Code = tr.Title_Code
					LEFT JOIN #OriDub_Title_Code OD ON OD.Record_Code = T.Title_Code
					WHERE tr.Title_Code NOT IN (Select Distinct Title_Code from #tempTitle)
		...
		BEGIN
			...
			BEGIN
				...
				BEGIN
					...
					INSERT INTO @tblRights_New(Syn_Deal_Code, Syn_Deal_Rights_Code, Title_Code, Right_Start_Date, Right_End_Date, [Sub-Licensing], Exclusive, Term, Episode_From, Episode_To)
					...
					INSERT INTO #tempRights(Syn_Deal_Rights_Code, Syn_Deal_Code, Title_Code, Right_Start_Date, Right_End_Date, [Sub-Licensing], Exclusive, Term, Episode_From ,Episode_To)
					SELECT Syn_Deal_Rights_Code, Syn_Deal_Code, Title_Code, Right_Start_Date, Right_End_Date, [Sub-Licensing], Exclusive, Term ,Episode_From ,Episode_To
					FROM @tblRights_New
			...
			--	IF(@logicalConnect = 'AND')
			--	BEGIN
			--		EXEC ('DELETE FROM #tempDeal WHERE Is_Holdback '+@theOp+' ('+@Value+') ')
			...
			--		INSERT INTO @tblDeal(Syn_Deal_Code, Agreement_Date, Agreement_No, Deal_Description, Deal_Workflow_Status, Currency_Code, Deal_Category_Code, Entity_Code, Vendor_Code,Sales_Agent_Code,Deal_Tag_Code, Deal_Type_Code, Customer_Type,Is_HoldBack)
			--		EXEC ('SELECT AD.Syn_Deal_Code, Agreement_Date, Agreement_No, Deal_Description, Deal_Workflow_Status, ad.Currency_Code, ad.Category_Code, ad.Entity_Code, ad.Vendor_Code,ad.Sales_Agent_Code,ad.Deal_Tag_Code, ad.Deal_Type_Code, ad.Customer_Type,VSD.IS_HoldBack
			--		   FROM Syn_Deal ad
			--		   INNER JOIN #BUWiseSyn_Deal_Code tbu ON tbu.Syn_deal_Code = ad.Syn_deal_Code
			--		   INNER JOIN VW_Syn_Deals vsd ON vsd.Vendor_Code =ad.Vendor_Code
			...
			--		INSERT INTO #tempDeal(Syn_Deal_Code, Agreement_Date, Agreement_No, Deal_Description, Deal_Workflow_Status, Currency_Code, Deal_Category_Code, Entity_Code, Vendor_Code,Sales_Agent_Code,Deal_Tag_Code, Deal_Type_Code, Customer_Type,Is_HoldBack)
			--		SELECT AD.Syn_Deal_Code, Agreement_Date, Agreement_No, Deal_Description, Deal_Workflow_Status, ad.Currency_Code, ad.Deal_Category_Code, ad.Entity_Code, ad.Vendor_Code,ad.Sales_Agent_Code,ad.Deal_Tag_Code, ad.Deal_Type_Code, ad.Customer_Type,ad.Is_HoldBack
			--		FROM @tblDeal AD
			...
			BEGIN
				...
				END
			END
			IF(@LeftColDbname = 'Milestone')
			BEGIN
				IF(@logicalConnect = 'AND')
				BEGIN
					EXEC ('DELETE FROM #tempDeal WHERE Milestone '+@theOp+' ('+@Value+') ')
				...
				BEGIN
					...
					INSERT INTO @tblDeal(Syn_Deal_Code, Agreement_Date, Agreement_No, Deal_Description, Deal_Workflow_Status, Currency_Code, Deal_Category_Code, Entity_Code, Vendor_Code,Sales_Agent_Code,Deal_Tag_Code, Deal_Type_Code, Customer_Type,Is_HoldBack,Milestone)
					EXEC ('SELECT AD.Syn_Deal_Code, Agreement_Date, Agreement_No, Deal_Description, Deal_Workflow_Status, ad.Currency_Code, ad.Category_Code, ad.Entity_Code, ad.Vendor_Code,ad.Sales_Agent_Code,ad.Deal_Tag_Code, ad.Deal_Type_Code, ad.Customer_Type,Milestone
					   FROM Syn_Deal ad
					   INNER JOIN #BUWiseSyn_Deal_Code tbu ON tbu.Syn_deal_Code = ad.Syn_deal_Code
					   INNER JOIN Title_Milestone TM On TM.Title_code=
					   
					   WHERE Milestone '+@theOp+' ('+@Value+') ')
				...
				END
			END 
			IF(@LeftColDbname = 'Deal Workflow Status')
			BEGIN
				IF(@logicalConnect = 'AND')
				BEGIN
				...
				BEGIN
					...
					EXEC ('SELECT AD.Syn_Deal_Code, Agreement_Date, Agreement_No, Deal_Description, Deal_Workflow_Status, ad.Currency_Code, ad.Category_Code, ad.Entity_Code, ad.Vendor_Code,ad.Sales_Agent_Code,ad.Deal_Tag_Code, ad.Deal_Type_Code, ad.Customer_Type
					   ...
					   WHERE Deal_workflow_status '+@theOp+' ('+@Value+') ')
			...
			BEGIN
				IF(@logicalConnect = 'AND')
				BEGIN
					EXEC ('DELETE FROM #tempDeal WHERE Category_Name '+@theOp+' ('+@Value+') ')
				...
				END
			END 
			IF(@LeftColDbname = 'CBFC Rating')
			BEGIN
				IF(@logicalConnect = 'AND')
				BEGIN
					EXEC ('DELETE FROM #tempDeal WHERE CBFC_rating '+@theOp+' ('+@Value+') ')
				...
				BEGIN
					...
					INSERT INTO @tblDeal(Syn_Deal_Code, Agreement_Date, Agreement_No, Deal_Description, Deal_Workflow_Status, Currency_Code, Deal_Category_Code, Entity_Code, Vendor_Code,Sales_Agent_Code,Deal_Tag_Code, Deal_Type_Code, Customer_Type,Cbfc_Rating)
					EXEC ('SELECT AD.Syn_Deal_Code, Agreement_Date, Agreement_No, Deal_Description, Deal_Workflow_Status, ad.Currency_Code, ad.Category_Code, ad.Entity_Code, ad.Vendor_Code,ad.Sales_Agent_Code,ad.Deal_Tag_Code, ad.Deal_Type_Code, ad.Customer_Type,DT.Cbfc_Rating
					   FROM Syn_Deal ad
					   INNER JOIN #BUWiseSyn_Deal_Code tbu ON tbu.Syn_deal_Code = ad.Syn_deal_Code
					   INNER JOIN VW_ Syn_Deals  ON VSD.Vendor_Code = ad.Vendor_Code
					   INNER JOIN DM_Title DT ON DT.Dm_Title_Code= VSD.Title_Code
	...
	BEGIN
		...
		BEGIN
			...
			BEGIN 
				...
				INSERT INTO #tempRights (Syn_Deal_Code, Syn_Deal_Rights_Code, Title_Code, Country, Dubbing, Subtitling, Territory , Right_Start_Date, Right_End_Date, [Sub-Licensing], Exclusive, Term, [Platform], Episode_From, Episode_To, Title_Name)
				SELECT Syn_Deal_Code, Syn_Deal_Rights_Code, Title_Code, Country, Dubbing, Subtitling, Territory , Right_Start_Date, Right_End_Date, [Sub-Licensing], Exclusive, Term, [Platform], Episode_From, Episode_To	, Title_Name
				FROM @tblRights_New
			END
		END
		...
		BEGIN
			...
			IF (UPPER(@OutputColsNames) =  'Is_Holdback')
			BEGIN
				IF(@TableColumns <> '')
					SET @TableColumns = @TableColumns + ', '
				SET @TableColumns = @TableColumns + 'td.Is_HoldBack [Is_Holdback]'
		
				UPDATE td SET td.Is_Holdback = VSD.Is_HoldBack
				FROM #tempDeal td
				INNER JOIN VW_ Syn_Deals ON VSD.Vendor_Code = td.Vendor_Code
			END
			...
			BEGIN
				...
				UPDATE td SET td.Variable_Cost_Type = VSD.Variable_Cost_Type
				FROM #tempDeal td
				INNER JOIN VW_ Syn_Deals  ON VSD.Vendor_Code = td.Vendor_Code
			END
			...
			--	UPDATE td SET td.ROFR = VSD.ROFR
			--	FROM #tempDeal td
			--	--INNER JOIN VW_Syn_Deals  ON VSD.Vendor_Code = td.Vendor_Code
			--END
			--IF (UPPER(@OutputColsNames) =  'Restriction Remarks')
			--BEGIN
			--	IF(@TableColumns <> '')
			--		SET @TableColumns = @TableColumns + ', '
			--	SET @TableColumns = @TableColumns + 'td.Restriction Remarks [Restriction Remarks]'
		
			--	UPDATE td SET td.ROFR = VSD.ROFR
			--	FROM #tempDeal td
			--	INNER JOIN Syn_Deal_rights On
			--	--INNER JOIN VW_ Syn_Deals VSD ON VSD.Vendor_Code = td.Vendor_Code
			--END
			--IF (UPPER(@OutputColsNames) = 'Cat_Name')
			--BEGIN
			--	IF(@TableColumns <> '')
			--		SET @TableColumns = @TableColumns + ', '
			--	SET @TableColumns = @TableColumns + 'td.Deal Category [Deal Category]'
		
			--	UPDATE td SET td.Deal_Category = C.Category_Name
			--	FROM #tempDeal td
			--	INNER JOIN Syn_Deal SD On SD.Vendor_Code =td.Vendor_Code
			...
			BEGIN
				IF(@TableColumns <> '')
					SET @TableColumns = @TableColumns + ', '
				SET @TableColumns = @TableColumns + 'td.CBFC_Rating [CBFC Rating]'
		
				UPDATE td SET td.CBFC_Rating = C.Category_Name
				FROM #tempDeal td
				INNER JOIN VW_ Syn_Deals  ON VSD.Vendor_Code = td.Vendor_Code
				INNER JOIN DM_Title DT ON DT.Dm_Title_Code= VSD.Title_Code
			...
			--	IF(@TableColumns <> '')
			--		SET @TableColumns = @TableColumns + ', '
			--	SET @TableColumns = @TableColumns + 'td.Self_Utilization_Group [Self Utilization Group]'
...
--select * from Report_Column_Setup where display_name like '%Lic%'


--delete from Report_Column_Setup where Column_Code in(156,208,217)

--Update Report_Column_Setup set IsPartofSelectOnly = 'A' WHERE Column_Code in(180,224)

--select * from Report_Column_Setup where View_Name = 'VW_SYN_DEALS' and IsPartofSelectOnly = 'Y' AND Display_Type is null

--select * from Report_Column_Setup where View_Name = 'VW_SYN_DEALS' and IsPartofSelectOnly <> 'Y' AND Display_Type is null
--select * from Report_Column_Setup where View_Name = 'VW_Acq_DEALS' and IsPartofSelectOnly <> 'Y' AND Display_Type is null
...
----select Customer_Type, sales_agent_code, * from syn_deal where isnull(Sales_Agent_Code,'') not in (172)
----select * from #tempDeal
--select * from Dm_Title
--select * from INFORMATION_SCHEMA.COLUMNS 
--where COLUMN_NAME like '%category_code%' 
--order by TABLE_NAME

--select * from report_column_setup where IsPartofSelectOnly in ('Y','B') AND View_Name = 'VW_Syn_DEALS' and Alternate_Config_Code is null
--select * from report_column_setup where IsPartofSelectOnly in ('N','B')  AND View_Name = 'VW_Syn_DEALS'

--select Rofr,variable_cost_type,is_holdback,Restriction_Remarks,Self_Utilization_Group,Self_Utilization_Remarks
--,Deal_Category
--,CBFC_Rating from VW_Syn_deals

--select CAtegory_name from syn_deal sd
--inner join category c on c.category_code =sd.category_code
--select  * from Dm_title where CBFC_rating is not null DM_title_code IN(select Title_code from title)

--select * from Syn_Deal_Rights where Milestone_unit_type is not null--_Error_Details
--select top 10 * from Report_query  where  View_Name = 'VW_Syn_DEALS'
--select * from title_milestone
--select top 10 * from vW_Syn_deals_rights
