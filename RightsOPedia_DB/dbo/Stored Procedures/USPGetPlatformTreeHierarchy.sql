CREATE PROCEDURE USPGetPlatformTreeHierarchy
(
...
BEGIN
	...
	--Declare @Acq_Deal_Rights_Code INT = 36376
	...
	SELECT Platform_Code, Platform_Name, Parent_Platform_Code, Is_Last_Level, Is_Active,Module_Position  FROM RightsU_Plus_Testing..Platform

	IF(@TabName = 'Rights Tab')
	BEGIN
		INSERT INTO #TempAcqRightsPlatform
		SELECT Acq_Deal_Rights_Code,Platform_Code FROM RightsU_Plus_Testing..Acq_Deal_Rights_Platform
		WHERE Acq_Deal_Rights_Code = @Acq_Deal_Rights_Code
	END
	ELSE
	BEGIN
		INSERT INTO #TempAcqRightsPlatform
		SELECT Syn_Deal_Rights_Code,Platform_Code FROM RightsU_Plus_Testing..Syn_Deal_Rights_Platform
		WHERE Syn_Deal_Rights_Code = @Acq_Deal_Rights_Code
	END
	...
	--(Select Count(*) From #TempPF tp Where tp.Parent_Platform_Code = #TempPlatform.Platform_Code) ChildCount
	...
	--Select Platform_Code, Platform_Name, IsNull(Parent_Platform_Code, 0) Parent_Platform_Code, Is_Last_Level, Module_Position From #TempPlatform Where Is_Active = 'Y' Order By Module_Position
	--Drop Table #TempPF
	IF OBJECT_ID('tempdb..#TempPF') IS NOT NULL DROP TABLE #TempPF
	IF OBJECT_ID('tempdb..#TempPlatform') IS NOT NULL DROP TABLE #TempPlatform
	IF OBJECT_ID('tempdb..#TempAcqRightsPlatform') IS NOT NULL DROP TABLE #TempAcqRightsPlatform
