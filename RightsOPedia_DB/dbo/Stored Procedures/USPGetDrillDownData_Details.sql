CREATE PROCEDURE [dbo].[USPGetDrillDownData_Details]
@BUCode INT, 
@UsersCode INT,
@Widget NVARCHAR(MAX),
...
IF OBJECT_ID('tempdb..#tempLanguageWithOthers') IS NOT NULL DROP TABLE #tempLanguageWithOthers
	...
	IF(@Widget = 'Acq')
	BEGIN
		SELECT  ISNULL(CAST(t.Year_Of_Production AS NVARCHAR(MAX)),'NA') AS YOR, dt.Deal_Type_Name AS TitleType, t.Title_Name as TitleName
		...
		GROUP BY dt.Deal_Type_Name,t.Year_Of_Production,t.Title_Name
	...
	IF(@Widget = 'Syn')
	BEGIN
		SELECT ISNULL(CAST(t.Year_Of_Production AS NVARCHAR(MAX)),0) AS YOR, dt.Deal_Type_Name AS TitleType,t.Title_Name as TitleName
