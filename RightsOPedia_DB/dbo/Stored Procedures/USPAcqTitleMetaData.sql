CREATE PROCEDURE [dbo].[USPAcqTitleMetaData]
(
...
--@TitleCode VARCHAR(MAX) = '7677'	
	...
	BEGIN
		...
		SET @OutputSQL = 'INSERT INTO #TempOutput(Title_Code, Vendor_Name, Acq_Deal_Code,' + @OutputCols + ')
		SELECT DISTINCT tt.Title_Code, v.Vendor_Name, tr.Acq_Deal_Code, ' + @TableColumns + '
		FROM #TempTitle tt
		INNER JOIN TitleRights tr ON tr.Title_Code = tt.Title_Code
		...
		GROUP BY tt.Title_Code,tt.Title_Code, v.Vendor_Name, tr.Acq_Deal_Code, Title_Name, Title_Language, YOR, Duration, [StarCast], Genres, Columns_Value, IMDB'

		EXEC(@OutputSQL)
		
		SELECT Title_Code AS TitleCode, Acq_Deal_Code AS DealCode, Licensor, KeyField, ValueFields AS ValueField, FieldOrder FROM (
			SELECT u.Title_Code, u.Acq_Deal_Code, u.Vendor_Name AS Licensor, u.ColValues, u.ValueFields  FROM (
				SELECT Title_Code, Vendor_Name, Acq_Deal_Code,
