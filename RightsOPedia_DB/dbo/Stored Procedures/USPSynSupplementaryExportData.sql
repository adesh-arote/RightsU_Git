CREATE PROCEDURE [dbo].[USPSynSupplementaryExportData]
(
	@DepartmentCode NVARCHAR(1000),
	@BVCode NVARCHAR(1000),
	@TitleCode VARCHAR(MAX),
	@SynDealCode VARCHAR(1000),
...
BEGIN
	...
	BEGIN
		...
		SELECT * FROM
			...
			WHERE sds.title_code = @TitleCode AND sds.Syn_Deal_Code = @SynDealCode ) AS T WHERE Data_Desc IS NOT NULL
	...
	SELECT 1 AS GroupOrder, tsc.Title_Code AS TitleCode, tsc.Syn_Deal_Code AS Deal_Code, ISNULL(tsc.Data_Desc,'NA') AS KeyField,
	       ISNULL(tsc.Supplementary_Tab_Description,'NA') AS DataValue, ISNULL(tsc.Data_Value,'NA') AS ValueField, tsc.Supplementary_Tab_Code AS FieldOrder, t.Title_Name AS TitleName, 
