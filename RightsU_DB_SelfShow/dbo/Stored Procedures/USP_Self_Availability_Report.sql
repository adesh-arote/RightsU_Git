ALTER PROCEDURE [dbo].[USP_Self_Availability_Report]
(
--declare
	@Title_Code VARCHAR(MAX)='30028', 
	@Is_Original_Language bit=1, 
	@Title_Language_Code VARCHAR(MAX)='0',
	@Episode_From VARCHAR(20)='1',
	@Episode_To VARCHAR(20)='1',
	@Show_EpisodeWise CHAR(1)='N',
	
	@Date_Type VARCHAR(2)='FL',
	@Start_Date VARCHAR(20)='2019-03-01',
	@End_Date VARCHAR(20)='2024-02-29',
	...
	@Restriction_Remarks VARCHAR(10)='Y',
	@Others_Remarks VARCHAR(10)='Y',
	@Exclusivity VARCHAR(1)='B' ,   --B-Both, E-Exclusive,N-NonExclusive 
	@SubLicense_Code VARCHAR(MAX)='0', --Comma   Separated SubLicensing Code. 0-No Sub Licensing ,
	@BU_Code VARCHAR(20)='1',
	@Country_Level CHAR(1)='Y',
	@Territory_Level CHAR(1)='N',
	@TabName CHAR(2)='IF',
	@Promoter_Groups varchar(max) = '',  
	@Promoter_ExactMatch char(10) = '',  
	@Promoter_MustHave varchar(max) = '' 
)
AS
BEGIN
--TRUNCATE TABLE SelfParam
	--INSERT INTO SelfParam
	--select @Title_Code, 
 ...
 --   @Platform_ExactMatch='' , 
	...
	--@Subtit_Language_Code  ='1,37', 
	--@Subtitling_Group_Code   = null,  
	--@Subtitling_ExactMatch   = null,  
	--@Subtitling_MustHave   = null,  
	--@Subtitling_Exclusion   = null,  

	--@Dubbing_Group_Code   = null,  
	--@Dubbing_ExactMatch   = null,  
	--@Dubbing_MustHave   = null,  
	--@Dubbing_Exclusion   = null,  
	--@Dubbing_Language_Code  ='0', 
	
	--@Restriction_Remarks ='Y',
	--@Others_Remarks ='Y',
	...
	--@Promoter_Groups   = '44,45,46,47,48,49,6',  
	--@Promoter_ExactMatch   = '',  
	--@Promoter_MustHave   = '' 
	
	IF(ISNULL(@Promoter_Groups,'') = '' OR ISNULL(@Promoter_Groups,'') = '0')
	BEGIN
		SELECT @Promoter_Groups = ',1,2,44,45,46,47,48,49,6,'
	END
	...
	BEGIN
		...
		CREATE TABLE #Temp_Promoter(
			Promoter_Group_Code INT
		)
		CREATE TABLE #Temp_Promoter_MH(
			Promoter_Group_Code INT
		)
		...
		----------------- Promoter Group Population
		INSERT INTO #Temp_Promoter
		SELECT CAST(number AS INT) FROM dbo.fn_Split_withdelemiter(@Promoter_Groups,',') WHERE number NOT IN('0', '')
	
		IF(@Promoter_MustHave <> '')
		BEGIN
			INSERT INTO #Temp_Promoter_MH
			SELECT CAST(number AS INT)  FROM dbo.fn_Split_withdelemiter(@Promoter_MustHave,',') WHERE number NOT IN('0', '')
		END
		-----------------END

		CREATE TABLE #Temp_Avail_Promoter_Groups(
		Avail_Promoter_Groups_Code NUMERIC(38,0),
		Promoter_Group_Code VARCHAR(MAX),
		...
		IF( @Promoter_Groups != '')
		BEGIN
		DECLARE @Promoter_Group_Code INT

			DECLARE Cur_Promoter CURSOR FOR SELECT Promoter_Group_Code FROM #Temp_Promoter ORDER BY Cast(Promoter_Group_Code AS VARCHAR(10)) ASC
			OPEN Cur_Promoter
			FETCH NEXT FROM Cur_Promoter INTO @Promoter_Group_Code
			WHILE (@@FETCH_STATUS = 0)
			BEGIN
				DELETE FROM #Temp_Promoter_MH WHERE Promoter_Group_Code = 0

				SET @MHCnt = 0
				IF Exists (SELECT TOP 1 * FROM #Temp_Promoter_MH WHERE Promoter_Group_Code = @Promoter_Group_Code)
					SET @MHCnt = @MHCnt + 1

				INSERT INTO #Temp_Avail_Promoter_Groups
				select Avail_Promoter_Groups_Code, CAST(@Promoter_Group_Code AS VARCHAR), '', @MHCnt FROM Avail_Promoter_Groups a1 
				where Promoter_Group_Codes Like '%,' + CAST(@Promoter_Group_Code AS VARCHAR) + ',%'

				FETCH NEXT FROM Cur_Promoter INTO @Promoter_Group_Code	
			END
			Close Cur_Promoter
			Deallocate Cur_Promoter
		
			IF(UPPER(@Promoter_ExactMatch) = 'EM')
			BEGIN
				DECLARE @ProStr VARCHAR(1000) = ''
				SELECT @ProStr = 
				STUFF
				(
					(
						SELECT DISTINCT ',' + CAST(Promoter_Group_Code AS VARCHAR) FROM #Temp_Promoter
						FOR XML PATH('')
					), 1, 1, ''
				)
			
				DELETE FROM #Temp_Avail_Promoter_Groups WHERE Promoter_Group_Code <> @ProStr 
			END
		
			IF(UPPER(@Promoter_ExactMatch) = 'MH')
			BEGIN
				SELECT @MHAllCnt = COUNT(*) FROM #Temp_Promoter_MH
				DELETE FROM #Temp_Avail_Promoter_Groups WHERE Promo_Cnt <> @MHAllCnt
			END

		END

	END
	--select '#Temp_Avail_Promoter_Groups',* from #Temp_Avail_Promoter_Groups
	...
	BEGIN
		...
		--select '#Avail_Dates',* from #Avail_Dates
		Select ar.Avail_Raw_Code, ar.Acq_Deal_Code, ar.Acq_Deal_Rights_Code, ar.Avail_Dates_Code, ar.Is_Exclusive, ar.Episode_From, ar.Episode_To, ad.Start_Date, 
		ad.End_Date 
		InTo #Avail_Raw 
		From Avail_Raw ar 
	...
	--select '#Avail_Raw',* FROM  #Avail_Raw
	--Over here
	CREATE TABLE #Temp_Avail_Promoter_Remarks(
		Avail_Raw_Code NUMERIC(38,0),
		Avail_Promoter_Remarks_Code NUMERIC(38,0),
		Avail_Promoter_Groups_Code NUMERIC(38,0),
		Promoter_Remark_Code NUMERIC(38,0),
		Promoter_Remark NVARCHAR(MAX)
	)

	BEGIN
		--select * from Avail_Acq_Show_Details
		--select * from Avail_Promoter_Remarks
		DECLARE @Avail_Raw_Code_cr NUMERIC(38,0), @Promoter_Groups_Code_cr NUMERIC(38,0), @Promoter_Remarks_Code_cr NUMERIC(38,0), @Promoter_Remark_Codes_cr VARCHAR(4000)
		--SELECT TAR.Avail_Raw_Code, ASD.Promoter_Groups_Code, ASD.Promoter_Remarks_Code, APR.Promoter_Remark_Codes FROM #Avail_Raw TAR 
		--INNER JOIN Avail_Acq_Show_Details ASD ON ASD.Avail_Raw_Code = TAR.Avail_Raw_Code
		--INNER JOIN Avail_Promoter_Remarks APR ON APR.Avail_Promoter_Remarks_Code = ASD.Promoter_Remarks_Code

		DECLARE Cur_Promoter_Remark CURSOR FOR SELECT TAR.Avail_Raw_Code, ASD.Promoter_Groups_Code, ASD.Promoter_Remarks_Code, APR.Promoter_Remark_Codes FROM #Avail_Raw TAR 
		INNER JOIN Avail_Acq_Show_Details ASD ON ASD.Avail_Raw_Code = TAR.Avail_Raw_Code
		INNER JOIN Avail_Promoter_Remarks APR ON APR.Avail_Promoter_Remarks_Code = ASD.Promoter_Remarks_Code
		
		OPEN Cur_Promoter_Remark
		FETCH NEXT FROM Cur_Promoter_Remark INTO @Avail_Raw_Code_cr, @Promoter_Groups_Code_cr, @Promoter_Remarks_Code_cr, @Promoter_Remark_Codes_cr
		WHILE (@@FETCH_STATUS = 0)
		BEGIN
			INSERT INTO #Temp_Avail_Promoter_Remarks(Avail_Raw_Code, Avail_Promoter_Remarks_Code, Avail_Promoter_Groups_Code, Promoter_Remark_Code)
			SELECT @Avail_Raw_Code_cr, @Promoter_Remarks_Code_cr, @Promoter_Groups_Code_cr,  CAST(number AS INT) FROM dbo.fn_Split_withdelemiter(@Promoter_Remark_Codes_cr,',') WHERE number NOT IN('0', '')

			FETCH NEXT FROM Cur_Promoter_Remark INTO @Avail_Raw_Code_cr, @Promoter_Groups_Code_cr, @Promoter_Remarks_Code_cr, @Promoter_Remark_Codes_cr
		END
		Close Cur_Promoter_Remark
		Deallocate Cur_Promoter_Remark
	END

	PRINT 'STEP-12 Query to get Promoter_Group_Name' + convert(varchar(30),getdate() ,109)	
	BEGIN
		update TMC SET TMC.Promoter_Remark = PR.Promoter_Remark_Desc FROM #Temp_Avail_Promoter_Remarks TMC 
		INNER JOIN Promoter_Remarks PR ON PR.Promoter_Remarks_Code = TMC.Promoter_Remark_Code
	END
	--select DISTINCT '#Temp_Avail_Promoter_Remarks',* from #Temp_Avail_Promoter_Remarks
	SELECT DISTINCT T1.Avail_Raw_Code, T1.Avail_Promoter_Remarks_Code, T1.Avail_Promoter_Groups_Code,
	STUFF
				(
					(
						SELECT DISTINCT ',' + CAST(T2.Promoter_Remark AS VARCHAR) FROM #Temp_Avail_Promoter_Remarks T2
						Where T2.Avail_Raw_Code = T1.Avail_Raw_Code AND T2.Avail_Promoter_Remarks_Code = T1.Avail_Promoter_Remarks_Code 
						AND T2.Avail_Promoter_Groups_Code = T1.Avail_Promoter_Groups_Code
						FOR XML PATH('')
					), 1, 1, ''
				) AS Promoter_Remark
				INTO #Temp_Avail_Promoter_RemarksNew
	FROM #Temp_Avail_Promoter_Remarks T1
	--select '#Temp_Avail_Promoter_RemarksNew',* from #Temp_Avail_Promoter_RemarksNew 
	IF OBJECT_ID('tempdb..#Temp_Avail_Promoter_Remarks') IS NOT NULL 
	BEGIN 
		DROP TABLE #Temp_Avail_Promoter_Remarks
	END
	--select '#Temp_Avail_Promoter_Remarks',* from #Temp_Avail_Promoter_Remarks
	...
	BEGIN
		...
		Begin
			...
			SELECT ar.Acq_Deal_Rights_Code, ar.Restriction_Remarks, ad.Remarks, ad.Rights_Remarks, 
				(
	...
	END
	--select '#Temp_Avail_Promoter_RemarksNew',* from #Temp_Avail_Promoter_RemarksNew
	PRINT 'STEP-4 Populate Title Avail for Title Languages ' + convert(varchar(30),getdate() ,109)
	BEGIN-----------------Populate Title Avail for Title Languages
	--select '#Temp_Right_Remarks',* from #Temp_Right_Remarks
	...
	Create Table #TMP_MAIN(
		...
		Promoter_Groups_Code Varchar(Max),
		Promoter_Remark Varchar(Max)
	)
	--SELECT '#Temp_Avail_Languages',* FROM #Temp_Avail_Languages WHERE Lang_Type = 'S'
	...
	Begin
		...
		INSERT INTO #TMP_MAIN(Title_Code, Platform_Code, Country_Code, Avail_Acq_Show_Code, Avail_Raw_Code, Is_Title_Language, Sub_Language_Code, Dub_Language_Code,
			Promoter_Groups_Code, Promoter_Remark)
		SELECT AA.Title_Code, AA.Platform_Code, AA.Country_Code, asd.Avail_Acq_Show_Code, asd.Avail_Raw_Code, 
			CASE WHEN @Is_Original_Language = 1 THEN asd.Is_Title_Language Else 0 END, asd.Sub_Language_Code, asd.Dub_Language_Code, 
			TAPG.Promoter_Group_Code, TAPR.Promoter_Remark
		...
		INNER JOIN #Temp_Avail_Promoter_Groups TAPG ON TAPG.Avail_Promoter_Groups_Code = asd.Promoter_Groups_Code
		INNER JOIN #Temp_Avail_Promoter_RemarksNew TAPR ON TAPR.Avail_Promoter_Remarks_Code = asd.Promoter_Remarks_Code
		WHERE 
		(
	...
	Begin
		...
		INSERT INTO #TMP_MAIN(Title_Code, Platform_Code, Country_Code, Avail_Acq_Show_Code, Avail_Raw_Code, Is_Title_Language, Sub_Language_Code, Dub_Language_Code,
		Promoter_Groups_Code, Promoter_Remark)
		SELECT DISTINCT AA.Title_Code, AA.Platform_Code, AA.Country_Code, asd.Avail_Acq_Show_Code, asd.Avail_Raw_Code, 
		CASE WHEN @Is_Original_Language = 1 THEN asd.Is_Title_Language Else 0 END, asd.Sub_Language_Code, asd.Dub_Language_Code, 
		TAPG.Promoter_Group_Code, TAPR.Promoter_Remark 
		FROM #Avail_Acq AA
		INNER JOIN Avail_Acq_Show_Details asd ON asd.Avail_Acq_Show_Code = AA.Avail_Acq_Show_Code
		...
		--AND
		--(
		--	@Promoter_Groups = '' OR asd.Promoter_Groups_Code IN (SELECT Promoter_Groups_Code FROM #Temp_Avail_Promoter_Groups)
		--)
	End
		...
		BEGIN 
			DROP TABLE #Temp_Avail_Promoter_Remarks
		END
		IF OBJECT_ID('tempdb..#Temp_Avail_Promoter_RemarksNew') IS NOT NULL 
		BEGIN 
			DROP TABLE #Temp_Avail_Promoter_RemarksNew
		END
	END
	--SELECT '#TMP_MAIN TEP-4',* FROM #TMP_MAIN
	...
	END
	--select '#TMP_MAIN1',* from  #TMP_MAIN
	PRINT 'STEP-5 UPDATE Language Names ' + convert(varchar(30),getdate() ,109)	
	BEGIN
	...
	PRINT 'STEP-6 Country Exact Match/ Must Have, Country Names' + convert(varchar(30),getdate() ,109)
	BEGIN
		...
		(
			...
			Reporting_Cluster NVARCHAR(MAX),
			...
			Group_no Int,
			Promoter_Groups_Code numeric(38,0),
			Promoter_Remarks_Code numeric(38,0),
			Promoter_Group_Name Varchar(4000),
			Promoter_Remark Varchar(4000),
			Acq_Deal_Rights_Holdback_Code INT, 
			Holdback_Type		Char(1), 
			Holdback_Release_Date Date, 
			Holdback_On_Platform_Code Int,
		...
		--select '#TMP_MAIN',* FROM #TMP_MAIN
		INSERT INTO #Temp_Main_Ctr(Title_Code, Platform_Code, Avail_Raw_Code, Is_Title_Language, Sub_Language_Code, Dub_Language_code, Promoter_Groups_Code, 
			Promoter_Remark, Acq_Deal_Rights_Holdback_Code, Holdback_Type, Holdback_Release_Date, Holdback_On_Platform_Code)
		SELECT DISTINCT Title_Code, Platform_Code, Avail_Raw_Code, Is_Title_Language, Sub_Language_Code, Dub_Language_code, Promoter_Groups_Code, Promoter_Remark,
			Acq_Deal_Rights_Holdback_Code, Holdback_Type, Holdback_Release_Date, Holdback_On_Platform_Code
		FROM #TMP_MAIN

		--For All Promoter Groups
		--INSERT INTO #Temp_Main_Ctr(Title_Code, Platform_Code, Avail_Raw_Code, Is_Title_Language, Sub_Language_Code, Dub_Language_code, Promoter_Groups_Code, Promoter_Remark)
		--select DISTINCT TMR.Title_Code, TMR.Platform_Code, TMR.Avail_Raw_Code, TMR.Is_Title_Language, TMR.Sub_Language_Code, TMR.Dub_Language_code, TMR.Promoter_Groups_Code,
		--PG.Promoter_Group_Code, '' Promoter_Remark
		--from Promoter_Group PG, #Temp_Main_Ctr TMR Where Is_Last_Level='Y' and Promoter_Group_Code NOT IN (select DISTINCT Promoter_Group_Code FROM #TMP_MAIN)
		--For All Promoter Groups

		--make Country Codes Comma Saperate
		...
		BEGIN
			...
			--	SELECT DISTINCT b.RowId, b.Country_Codes, a.Avail_Raw_Code FROM #Temp_Main_Ctr a
			...
			--	Insert InTo #Tbl_Ret
			--	Select DISTINCT c.Region_Code, c.Region_Name_In, c.Region_Name_NOTIn, NULL, tc.RowId, tc.Avail_Raw_Code
			--	From #tblCountries tc
			--	Cross Apply DBO.UFN_Get_Report_Country(tc.Country_Codes, @Country_Code, tc.RowId, tc.Acq_Rights_Code) c
			--	IF OBJECT_ID('tempdb..#tblCountries') IS NOT NULL 
			--	BEGIN
			...
			PRINT 'A'
				...
				Insert InTo #Tbl_Ret
				Select c.Region_Code, c.Cluster_Name, Region_Name, tc.RowId, NULL
				--select *
		...
		BEGIN
			Insert InTo #Tbl_Ret
			Select c.Region_Code, c.Full_Cluster_Name, Region_Name, tc.RowId, NULL
			From #Temp_Country_Names tc
			Cross Apply DBO.UFN_Get_Report_Cluster_Territory(tc.Country_Codes, tc.RowId, @Territory_Code, @Is_IFTA_Cluster) c
		END
	
		print 'STEP-9.4 UPDATE Country Names in #Temp_Main_Ctr' + convert(varchar(30),getdate() ,109)	
		BEGIN		
		
			UPDATE tms
			SET tms.Reporting_Cluster = b.Reporting_Cluster, tms.Country = b.Country, tms.Country_Code = b.Region_Code
			FROM #Temp_Main_Ctr tms
			INNER JOIN #Tbl_Ret b On tms.Group_no = b.Group_No AND CASE WHEN @TabName = 'CO' THEN b.Avail_Raw_Code ELSE 1 END = CASE WHEN @TabName = 'CO' THEN tms.Avail_Raw_Code ELSE 1 END
		END
	END
	--select '#TMP_MAINSTEP-6',* FROM #TMP_MAIN
	...
	BEGIN
		...
		END
	END
	--select '#TMP_MAINSTEP-10',* FROM #TMP_MAIN
   ...
   END
	...
	PRINT 'STEP-12 Query to get Promoter_Group_Name' + convert(varchar(30),getdate() ,109)	
	BEGIN
		update TMC SET TMC.Promoter_Group_Name = 
		CASE WHEN Parent.Promoter_Group_Code IS NULL THEN PG.Promoter_Group_Name ELSE Parent.Promoter_Group_Name + ' -- ' + PG.Promoter_Group_Name END 
		FROM #Temp_Main_Ctr TMC 
		INNER JOIN Promoter_Group PG ON PG.Promoter_Group_Code = TMC.Promoter_Groups_Code
		LEFT JOIN Promoter_Group Parent ON Parent.Promoter_Group_Code = PG.Parent_Group_Code 

		--select PG.Promoter_Group_Name, PG.Parent_Group_Code FROM #Temp_Main_Ctr TMC 
		--INNER JOIN Promoter_Group PG ON PG.Promoter_Group_Code = TMC.Promoter_Groups_Code
		--LEFT JOIN Promoter_Group Parent ON Parent.Promoter_Group_Code = PG.Parent_Group_Code 
	END
	...
	BEGIN
		...
		SELECT DISTINCT 
			...
			Reporting_Cluster, Country, 
			CAST(AR.Start_Date AS DATE) Right_Start_Date, 
			CAST(ISNULL(AR.End_Date, '31Dec9999') AS DATE) Rights_End_Date,
			...
			Sub_Language_Names, ISNULL(Dub_Language_Names,'') Dub_Language_Names, trt.Genres_Name COLLATE SQL_Latin1_General_CP1_CI_AS Genres_Name, trt.Star_Cast COLLATE SQL_Latin1_General_CP1_CI_AS Star_Cast, 
			...
			trr.Sub_License_Name AS Sub_License, 'Yes' Platform_Avail, tm.Promoter_Group_Name, tm.Promoter_Remark,
			CASE WHEN ISNULL(pt.Platform_Hiearachy,'') = '' OR ar.Start_Date > tm.Holdback_Release_Date THEN '' ELSE hb.HBComments + pt.Platform_Hiearachy END 
			...
			CASE 
				WHEN ISNULL(CONVERT(varchar(50), Holdback_Release_Date ),'') = '' OR ar.Start_Date > Holdback_Release_Date 
					THEN '' 
				ELSE ISNULL(CONVERT(VARCHAR(20), Holdback_Release_Date, 103),'') END 
			COLLATE SQL_Latin1_General_CP1_CI_AS AS  Holdback_Release_Date
			--
		FROM 
		#Temp_Main_Ctr tm
		...
		INNER JOIN Platform pt ON pt.Platform_Code = tm.Platform_Code --where AR.End_Date !='9999-12-31 00:00:00.000'
	...
	BEGIN
		SELECT DISTINCT 
			...
			, tm.Holdback_Type COLLATE SQL_Latin1_General_CP1_CI_AS  AS Holdback_Type, 
				...
				WHEN ISNULL(CONVERT(varchar(50), Holdback_Release_Date ),'') = '' OR ar.Start_Date > Holdback_Release_Date 
					THEN '' 
				ELSE ISNULL(CONVERT(VARCHAR(20), Holdback_Release_Date, 103),'') END COLLATE SQL_Latin1_General_CP1_CI_AS AS Holdback_Release_Date
		...
		LEFT JOIN #MainAH hb On tm.Acq_Deal_Rights_Holdback_Code = hb.Acq_Deal_Rights_Holdback_Code
	END
	--select * FROM #Temp_Right_Remarks tm
	print 'STEP-16 DROP TABLES ' + convert(varchar(30),getdate() ,109)	
	BEGIN
	IF OBJECT_ID('tempdb..#Temp_Country_Names') IS NOT NULL 
	BEGIN
		DROP TABLE #Temp_Country_Names
	...
	BEGIN
		DROP TABLE #tblCountries
	END
	IF OBJECT_ID('tempdb..#Avail_Raw') IS NOT NULL 
	BEGIN
	...
	END 
	IF OBJECT_ID('tempdb..#MainAH') IS NOT NULL 
	BEGIN 
		DROP TABLE #MainAH
	END 
	IF OBJECT_ID('tempdb..#Temp_Sub_Language') IS NOT NULL 
	BEGIN 
		DROP TABLE #Temp_Sub_Language
	END 
	IF OBJECT_ID('tempdb..#Temp_Dub_Language') IS NOT NULL 
	BEGIN 
		DROP TABLE #Temp_Dub_Language
	END 
	IF OBJECT_ID('tempdb..#Temp_Sub_MH') IS NOT NULL 
	BEGIN 
		DROP TABLE #Temp_Sub_MH
	END 
	IF OBJECT_ID('tempdb..#Temp_Dub_MH') IS NOT NULL 
	BEGIN 
	...
	BEGIN 
		DROP TABLE #Tmp_SL
	END 
	IF OBJECT_ID('tempdb..#Temp_Avail_Languages') IS NOT NULL 
	BEGIN 
		DROP TABLE #Temp_Avail_Languages
	END 
	IF OBJECT_ID('tempdb..#Temp_Right_Remarks') IS NOT NULL 
	BEGIN 
		DROP TABLE #Temp_Right_Remarks
	END 
	IF OBJECT_ID('tempdb..#Tbl_Ret') IS NOT NULL 
	BEGIN 
		DROP TABLE #Tbl_Ret 
	END
	IF OBJECT_ID('tempdb..#Temp_Promoter') IS NOT NULL 
	BEGIN 
		DROP TABLE #Temp_Promoter 
	END
	IF OBJECT_ID('tempdb..#Temp_Promoter_MH') IS NOT NULL 
	BEGIN 
		DROP TABLE #Temp_Promoter_MH 
	END
	IF OBJECT_ID('tempdb..#Temp_Avail_Promoter_Groups') IS NOT NULL 
	BEGIN 
		DROP TABLE #Temp_Avail_Promoter_Groups
	END
...
--exec USP_Self_Availability_Report
--@Title_Code ='35278,', 
--@Is_Original_Language=1 , 
--@Title_Language_Code  ='0',
--@Episode_From  ='1',
...
--@Promoter_Groups   = '44,45,46,47,48,49',  
--@Promoter_ExactMatch   = '',  
--@Promoter_MustHave   = '' 