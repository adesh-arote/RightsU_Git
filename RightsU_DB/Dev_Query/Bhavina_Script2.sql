...
BEGIN
	--SET NOCOUNT ON
	--DECLARE
	--@TitleCode BIGINT = 30626, 
	--@EpisodeNo INT = 1, 
	--@BV_Schedule_Transaction_Code BIGINT = 0, 
	--@MusicScheduleTransactionCode BIGINT = 0,
	--@CallFrom VARCHAR(10) = 'AR'
	...
	DECLARE @DefaultVersionCode INT = 1
	...
	BEGIN
		...
		BEGIN
			...
			LEFT JOIN Music_Title_Label MTL ON MTL.Music_Title_Code = CML.Music_Title_Code 
				AND TSD.Schedule_Date BETWEEN MTL.Effective_From AND ISNULL(MTL.Effective_To, GETDATE())
			WHERE TC.Title_Code = @TitleCode AND TC.Episode_No = @EpisodeNo 
		END
		...
		*/
		...
		CASE 
			...
			ELSE 'N'
		END AS Show_Linked, MD.Inserted_On AS DealCreatedOn, 'N' AS [Is_Active_Deal]
		...
		BEGIN
			...
			INNER JOIN #TempMusicScheduleTransaction TMST ON TMST.BV_Schedule_Transaction_Code = MST.BV_Schedule_Transaction_Code
				AND TMST.Content_Music_Link_Code = MST.Content_Music_Link_Code AND TMST.Music_Label_Code = MST.Music_Label_Code
				AND TMST.Channel_Code = MST.Channel_Code
		...
		BEGIN
			...
			INNER JOIN #TempMusicScheduleTransaction TMST ON TMST.BV_Schedule_Transaction_Code = MST.BV_Schedule_Transaction_Code
				...
				--AND TMST.Is_Processed COLLATE SQL_Latin1_General_CP1_CI_AS = MST.Is_Processed COLLATE SQL_Latin1_General_CP1_CI_AS 
		END
		...
		BEGIN
			...
			SET @stepNo += 1
			DECLARE @MusicLabelCode BIGINT = 0, @MusicTitleCode BIGINT = 0, @IsError CHAR(1) = 'N',  @MusicDealCode BIGINT = 0, 
					@ErrorCode BIGINT = 0, @RightRuleCode BIGINT = 0,  
					@NoOfHours INT = 24, @NoOfSongs INT = 0,  @IsIgnore CHAR(1) = 'N', @RunType CHAR(1) = '', @ChannelType CHAR(1) = '', 
					@Schedule_Date DATETIME = NULL, @ChannelCode BIGINT = 0, @isValid CHAR(1) = 'Y', @AutoResolvedErrCodes VARCHAR(MAX) = ''

			PRINT ' ---------------------------------------------------------'
			SET @MusicScheduleTransactionCode  = 0
			SELECT TOP 1 @MusicScheduleTransactionCode = MusicScheduleTransactionCode, @MusicLabelCode = ISNULL(Music_Label_Code, 0), 
			@MusicTitleCode = ISNULL(Music_Title_Code ,0) FROM #TempMusicScheduleTransaction WHERE Is_Processed = 'N'
			...
			BEGIN
				...
				SELECT  @MusicDealCode = 0, @ErrorCode = 0, @Schedule_Date = NULL, @ChannelCode = 0, @isValid  = 'Y', @AutoResolvedErrCodes = ''

				SELECT TOP 1 @Schedule_Date = CONVERT(DATETIME, Schedule_Item_Log_Date, 121), @ChannelCode =BST.Channel_Code
				FROM BV_Schedule_Transaction BST
				INNER JOIN #TempMusicScheduleTransaction TMST ON TMST.MusicScheduleTransactionCode = @MusicScheduleTransactionCode
					AND TMST.BV_Schedule_Transaction_Code = BST.BV_Schedule_Transaction_Code
				...
				BEGIN
					...
					SELECT @IsError  = 'Y', @isValid = 'N', @ErrorCode = 0
					SELECT TOP 1 @ErrorCode = Error_Code FROM #ExistingException 
					WHERE MusicScheduleTransactionCode = @MusicScheduleTransactionCode AND Error_Key = 'MLBLNF'
					IF (@ErrorCode = 0)
					BEGIN
						INSERT INTO Music_Schedule_Exception(Music_Schedule_Transaction_Code, Error_Code, [Status], [Is_Mail_Sent])
						SELECT TOP 1 @MusicScheduleTransactionCode, Error_Code, 'E', 'N' FROM Error_Code_Master WHERE Error_For = 'MSE' AND Upload_Error_Code = 'MLBLNF'
					END
					ELSE
					BEGIN
						UPDATE Music_Schedule_Exception SET [Status] = 'E' 
						WHERE Music_Schedule_Transaction_Code = @MusicScheduleTransactionCode AND Error_Code = @ErrorCode
					END
				...
				BEGIN
					...
					INSERT INTO #CurrentMusicLabelDealData(
						...
						Channel_Code, No_Of_Songs, Defined_Runs, Scheduled_Runs, Show_Linked, DealCreatedOn, Is_Active_Deal
					)
					SELECT 
						Music_Deal_Code, Agreement_No, Deal_Workflow_Status, Rights_Start_Date, Rights_End_Date, Run_Type, Right_Rule_Code, Channel_Type, 
						Channel_Code, No_Of_Songs, Defined_Runs, Scheduled_Runs, Show_Linked, DealCreatedOn, Is_Active_Deal
					FROM #AllMusicLabelDealData WHERE Music_Deal_Code IS NOT NULL AND Music_Label_Code = @MusicLabelCode
				END
				...
				BEGIN
					...
					BEGIN
						...
						SELECT @IsError  = 'Y', @isValid = 'N', @ErrorCode = 0
						SELECT TOP 1 @ErrorCode = Error_Code FROM #ExistingException 
						WHERE MusicScheduleTransactionCode = @MusicScheduleTransactionCode AND Error_Key = 'DNF'
						IF (@ErrorCode = 0)
						BEGIN
							INSERT INTO Music_Schedule_Exception(Music_Schedule_Transaction_Code, Error_Code, [Status], [Is_Mail_Sent])
							SELECT TOP 1 @MusicScheduleTransactionCode, Error_Code, 'E', 'N' FROM Error_Code_Master WHERE Error_For = 'MSE' AND Upload_Error_Code = 'DNF'
						END
						ELSE
						BEGIN
							UPDATE Music_Schedule_Exception SET [Status] = 'E' 
				...
				BEGIN
					...
					WHERE Deal_Workflow_Status = 'A' AND Channel_Code = @ChannelCode AND Show_Linked = 'Y' AND @Schedule_Date BETWEEN Rights_Start_Date  AND Rights_End_Date
					...
					BEGIN
						SELECT TOP 1 @MusicDealCode = Music_Deal_Code FROM #CurrentMusicLabelDealData 
						WHERE Deal_Workflow_Status = 'A' AND Show_Linked = 'Y' AND @Schedule_Date BETWEEN Rights_Start_Date  AND Rights_End_Date
						...
						BEGIN
							...
							SELECT @IsError  = 'Y', @isValid = 'N', @ErrorCode = 0
							SELECT TOP 1 @ErrorCode = Error_Code FROM #ExistingException 
							WHERE MusicScheduleTransactionCode = @MusicScheduleTransactionCode AND Error_Key = 'CNF'
							IF (@ErrorCode = 0)
							BEGIN
								INSERT INTO Music_Schedule_Exception(Music_Schedule_Transaction_Code, Error_Code, [Status], [Is_Mail_Sent])
								SELECT TOP 1 @MusicScheduleTransactionCode, Error_Code, 'E', 'N' FROM Error_Code_Master WHERE Error_For = 'MSE' 
									AND Upload_Error_Code = 'CNF'
							END
							ELSE
							BEGIN
								UPDATE Music_Schedule_Exception SET [Status] = 'E' 
					...
					END
					ELSE 
						SET @AutoResolvedErrCodes = @AutoResolvedErrCodes + ',CNF'
					...
					BEGIN
						...
						BEGIN
							...
							SELECT @IsError  = 'Y', @isValid = 'N', @ErrorCode = 0
							SELECT TOP 1 @ErrorCode = Error_Code FROM #ExistingException 
							WHERE MusicScheduleTransactionCode = @MusicScheduleTransactionCode AND Error_Key = 'IRP'
							IF (@ErrorCode = 0)
							BEGIN
								INSERT INTO Music_Schedule_Exception(Music_Schedule_Transaction_Code, Error_Code, [Status], [Is_Mail_Sent])
								SELECT TOP 1 @MusicScheduleTransactionCode, Error_Code, 'E', 'N' FROM Error_Code_Master WHERE Error_For = 'MSE' 
									AND Upload_Error_Code = 'IRP'
							END
							ELSE
							BEGIN
								UPDATE Music_Schedule_Exception SET [Status] = 'E' 
					...
					END
					ELSE
						SET @AutoResolvedErrCodes = @AutoResolvedErrCodes + ',IRP'

					IF(@MusicDealCode = 0)
					BEGIN
						SELECT TOP 1 @MusicDealCode = Music_Deal_Code FROM #CurrentMusicLabelDealData 
						WHERE Channel_Code = @ChannelCode AND Show_Linked = 'Y'
						AND @Schedule_Date BETWEEN Rights_Start_Date  AND Rights_End_Date

						IF (@MusicDealCode = 0)
						BEGIN
							SELECT TOP 1 @MusicDealCode = Music_Deal_Code FROM #CurrentMusicLabelDealData 
							WHERE Show_Linked = 'Y' AND @Schedule_Date BETWEEN Rights_Start_Date  AND Rights_End_Date
						END
						
						IF (@MusicDealCode = 0)
						BEGIN
							SELECT TOP 1 @MusicDealCode = Music_Deal_Code FROM #CurrentMusicLabelDealData WHERE Channel_Code = @ChannelCode AND Show_Linked = 'Y'
						END
						
						IF (@MusicDealCode = 0)
						BEGIN
							SELECT TOP 1 @MusicDealCode = Music_Deal_Code FROM #CurrentMusicLabelDealData WHERE Show_Linked = 'Y'
						END
						...
						BEGIN
							...
							SELECT @IsError  = 'Y', @isValid = 'N', @ErrorCode = 0
							SELECT TOP 1 @ErrorCode = Error_Code FROM #ExistingException 
							WHERE MusicScheduleTransactionCode = @MusicScheduleTransactionCode AND Error_Key = 'DNA'
							IF (@ErrorCode = 0)
							BEGIN
								INSERT INTO Music_Schedule_Exception(Music_Schedule_Transaction_Code, Error_Code, [Status], [Is_Mail_Sent])
								SELECT TOP 1 @MusicScheduleTransactionCode, Error_Code, 'E', 'N' FROM Error_Code_Master WHERE Error_For = 'MSE' AND Upload_Error_Code = 'DNA'
							END
							ELSE
							BEGIN
								UPDATE Music_Schedule_Exception SET [Status] = 'E' 
					...
					IF(@MusicDealCode = 0)
					BEGIN
						SELECT TOP 1 @MusicDealCode = Music_Deal_Code FROM #CurrentMusicLabelDealData WHERE Deal_Workflow_Status = 'A'
						IF (@MusicDealCode = 0)
						BEGIN
							SELECT TOP 1 @MusicDealCode = Music_Deal_Code FROM #CurrentMusicLabelDealData
						END
						...
						SELECT @IsError  = 'Y', @isValid = 'N', @ErrorCode = 0
						SELECT TOP 1 @ErrorCode = Error_Code FROM #ExistingException 
						WHERE MusicScheduleTransactionCode = @MusicScheduleTransactionCode AND Error_Key = 'SNL'
						IF (@ErrorCode = 0)
						BEGIN
							INSERT INTO Music_Schedule_Exception(Music_Schedule_Transaction_Code, Error_Code, [Status], [Is_Mail_Sent])
							SELECT TOP 1 @MusicScheduleTransactionCode, Error_Code, 'E', 'N' FROM Error_Code_Master WHERE Error_For = 'MSE' 
								AND Upload_Error_Code = 'SNL'
						END
						ELSE
						BEGIN
							UPDATE Music_Schedule_Exception SET [Status] = 'E' 
				...
				BEGIN
					...
					BEGIN
						...
						SELECT @RightRuleCode = 0, @NoOfHours = 24, @NoOfSongs = 0,  @IsIgnore = 'N', @RunType = '', @ChannelType = ''
						...
						BEGIN
							...
							SELECT @MaxDateTime = DATEADD(SECOND,-1,DATEADD(HOUR,ISNULL(@NoOfHours,0),@MinDateTime)) 
						...
						BEGIN
							...
							BEGIN
								...
								SELECT @IsError  = 'Y', @ErrorCode = 0
								SELECT TOP 1 @ErrorCode = Error_Code FROM #ExistingException 
								WHERE MusicScheduleTransactionCode = @MusicScheduleTransactionCode AND Error_Key = 'EAR'
								IF (@ErrorCode = 0)
								BEGIN
									INSERT INTO Music_Schedule_Exception(Music_Schedule_Transaction_Code, Error_Code, [Status], [Is_Mail_Sent])
									SELECT TOP 1 @MusicScheduleTransactionCode, Error_Code, 'E', 'N' FROM Error_Code_Master WHERE Error_For = 'MSE' 
										AND Upload_Error_Code = 'EAR'
								END
								ELSE
								BEGIN
									UPDATE Music_Schedule_Exception SET [Status] = 'E' 
							...
							BEGIN
								...
								BEGIN
									...
									SELECT @IsError  = 'Y', @ErrorCode = 0
									SELECT TOP 1 @ErrorCode = Error_Code FROM #ExistingException 
									WHERE MusicScheduleTransactionCode = @MusicScheduleTransactionCode AND Error_Key = 'ECAR'
									IF (@ErrorCode = 0)
									BEGIN
										INSERT INTO Music_Schedule_Exception(Music_Schedule_Transaction_Code, Error_Code, [Status], [Is_Mail_Sent])
										SELECT TOP 1 @MusicScheduleTransactionCode, Error_Code, 'E', 'N' FROM Error_Code_Master WHERE Error_For = 'MSE' 
											AND Upload_Error_Code = 'ECAR'
									END
									ELSE
									BEGIN
										UPDATE Music_Schedule_Exception SET [Status] = 'E' 
				...
				PRINT ' ---------------------------------------------------------'
				SELECT @MusicScheduleTransactionCode = 0, @MusicLabelCode = 0, @IsError  = 'N', @MusicTitleCode = 0
				SELECT TOP 1 @MusicScheduleTransactionCode = MusicScheduleTransactionCode, @MusicLabelCode = ISNULL(Music_Label_Code, 0), 
				@MusicTitleCode = ISNULL(Music_Title_Code ,0) FROM #TempMusicScheduleTransaction WHERE Is_Processed = 'N'
			END
	...
	BEGIN
		...
		INSERT INTO #TempMailData(Channel_Codes, User_Email_Id, Record_Count, Email_Data)
		VALUES('~0~', 'jatin@uto.in', 0, '')

		IF(@IsChannelwiseMail = 'Y')
		BEGIN
			PRINT ' Send seperate mail for every channel'
			INSERT INTO #TempMailData(Channel_Codes, User_Email_Id, Record_Count, Email_Data)
			SELECT '~' + CAST(BVEC.Channel_Code AS VARCHAR) + '~', 
			STUFF(
			(
				SELECT DISTINCT '~' +  U.Email_Id FROM BVException_Users BVEU 
				INNER JOIN Users U ON U.Users_Code = BVEU.Users_Code
				WHERE BVEU.Bv_Exception_Code = BVE.Bv_Exception_Code
				FOR XML PATH('')), 1, 1, ''
			), 
			0, '' FROM BVException BVE
			INNER JOIN BVException_Channel BVEC ON BVEC.Bv_Exception_Code = BVE.Bv_Exception_Code
			WHERE BVE.Bv_Exception_Type = 'M'
		END
		ELSE
		BEGIN
			...
			'~' + STUFF(
			(
				SELECT DISTINCT '~' +  CAST(BVEC.Channel_Code AS VARCHAR) FROM BVException_Channel BVEC 
				WHERE BVEC.Bv_Exception_Code = BVE.Bv_Exception_Code
				FOR XML PATH('')), 1, 1, ''
			)  + '~', 
			STUFF(
			(
		...
		SET @trTable = '<tr>      
			<td align="center" width="15%" class="tblHead"><b>Exception<b></td>    
			<td align="center" width="15%" class="tblHead"><b>Content<b></td>      
			<td align="center" width="5%" class="tblHead"><b>Episode No<b></td>      
			<td align="center" width="10%" class="tblHead"><b>Airing Date<b></td>      
			<td align="center" width="10%" class="tblHead"><b>Airing Time<b></td>      
			<td align="center" width="10%" class="tblHead"><b>Channel Name<b></td>
			<td align="center" width="15%" class="tblHead"><b>Music Track<b></td>
			<td align="center" width="10%" class="tblHead"><b>Movie / Album<b></td>
			<td align="center" width="10%" class="tblHead"><b>Music Label<b></td>
		</tr>'
		...
		BEGIN
			...
			BEGIN
				...
				IF NOT EXISTS (SELECT * FROM #TempMailData WHERE RowNo = @RowNo AND Email_Data LIKE '%'+ @trTable +'%')
					UPDATE #TempMailData SET Email_Data = (Email_Data +  @trTable), Record_Count = (Record_Count + 1) WHERE RowNo = @RowNo
...
END
*/
select * from System_Language_Message SLM
inner join system_module_message smm on SLM.system_module_message_code = smm.system_module_message_code
where system_language_code=1 
AND smm.module_code = 27
select * from System_message where message_key like'%layout%'

select * from system_module_message
select * from system_module where module_name like'%title%'

update system_module_message set module_code = null where form_id = 'Common'