...
BEGIN      
 ...
 INTO #TempTC FROM Title_Content AS TC1 where TC1.Title_Content_Code IN (SELECT NUMBER FROM fn_Split_withdelemiter(@Title_Content_Code, ','))  
 ...
 , (select MA.Music_Album_Name from Music_Album MA Where MA.Music_Album_Code = MT.Music_Album_Code) AS Music_Album
 ...
 INNER JOIN BV_Schedule_Transaction BST ON BST.Content_Channel_Run_Code = CCR.Content_Channel_Run_Code --ON BST.Deal_Movie_Code = TCM.Acq_Deal_Movie_Code AND BST.Program_Episode_Number = TC.Episode_No      
...
--select * from title_Content where Title_Code=17788
