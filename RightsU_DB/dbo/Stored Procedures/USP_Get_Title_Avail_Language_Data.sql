CREATE PROCEDURE [dbo].[USP_Get_Title_Avail_Language_Data]    
	 @Mode CHAR(1),                  
	 @UserCode INT,                  
...
BEGIN   
	...
	if(@Loglevel< 2)Exec [USPLogSQLSteps] '[USP_Get_Title_Avail_Language_Data]', 'Step 1', 0, 'Started Procedure', 0, ''
	-- =============================================    
	-- Author:  Faisal Khan    
	...
	-- =============================================    
		...
		DECLARE @ReportDB VARCHAR(50)
		SET @ReportDB = ''    
		SELECT  @ReportDB = Parameter_value FROM System_Parameter_New WHERE Parameter_Name = 'Reports_DB'    
		...
		BEGIN    
			...
			,Dubbing_Language_Code,BU_Code ,Report_Type,IsDisital ,IncludeMetadata,Is_IFTA_Cluster,Platform_Group_Code ,Subtitling_Group_Code ,Subtitling_ExactMatch,
			...
			Include_Ancillary, Promoter_Code, MustHave_Promoter, Promoter_ExactMatch, Module_Code) 
			SELECT
		...
		SELECT @RecCount = COUNT(*) FROM #Temp where Module_Code = @ModuleCode and Report_Status <> 'X'
		...
		print 'reshma-1.1'  
			SET @SqlQuery = '    
			SELECT 0 AS RowNum, Avail_Report_Schedule_Code, Title_Code, Platform_Code, Country_Code, CASE Is_Original_Language  WHEN ''1'' THEN ''Yes'' ELSE ''No'' END Is_Original_Language,     
			Dubbing_Subtitling, CASE GroupBy  WHEN ''C'' THEN ''Country'' WHEN ''L'' THEN ''Language'' ELSE '''' END GroupBy,     
			CASE Node WHEN ''P'' THEN ''Parent'' WHEN ''C'' THEN ''Child'' ELSE '''' END Node, Language_Code,     
			CASE Date_Type WHEN ''FL'' THEN ''Flexi'' WHEN ''FI'' THEN ''Fixed'' ELSE ''MI'' END Date_Type,     
			...
			(select dbo.UFN_GetRegionNames(isnull(Country_Code, 0))) Country_Names, (select dbo.UFN_GetLanguageNames(isnull(Language_Code, 0))) Language_Names,    
			CASE WHEN Dubbing_Subtitling LIKE ''%D%'' THEN ''Yes'' ELSE ''No'' END Is_Dubbing, CASE WHEN Dubbing_Subtitling LIKE ''%S%'' THEN ''Yes'' ELSE ''No'' END Is_Subtitling,    
	...
	if(@Loglevel< 2)Exec [USPLogSQLSteps] '[USP_Get_Title_Avail_Language_Data]', 'Step 2', 0, 'Procedure Excution Completed', 0, ''
END