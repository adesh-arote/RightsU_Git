...
BEGIN
	...
	if(@Loglevel<2)Exec [USPLogSQLSteps] '[USP_Report_PlatformWise_Syndication_Neo]', 'Step 1', 0, 'Started Procedure', 0, ''
	 ...
	 --   @Country_Code VARCHAR(MAX) = '0', 
		...
		BEGIN
			...
			BEGIN
				...
				SELECT DISTINCT Title_Code FROM Syn_Deal_Movie (NOLOCK)
			END
		...
		BEGIN
			...
			SELECT SR.Syn_Deal_Code, SR.Syn_Deal_Rights_Code, SR.Restriction_Remarks, SDRP.Platform_Code, SDRT.Title_Code,
				   ...
				   (STUFF((
					...
					FROM Syn_Deal SD_Tmp (NOLOCK)
					INNER JOIN Syn_Deal_Rights ADR_Tmp (NOLOCK) ON adr_Tmp.Syn_Deal_Code = SD_Tmp.Syn_Deal_Code 
			...
			FROM Syn_Deal_Rights AS SR (NOLOCK)
			INNER JOIN Syn_Deal_Rights_Platform AS SDRP (NOLOCK) ON SDRP.Syn_Deal_Rights_Code = SR.Syn_Deal_Rights_Code
			INNER JOIN Syn_Deal_Rights_Title AS SDRT (NOLOCK) ON SDRT.Syn_Deal_Rights_Code = SR.Syn_Deal_Rights_Code
			INNER JOIN Syn_Deal SD (NOLOCK) ON SR.Syn_Deal_Code = SD.Syn_Deal_Code
		...
		BEGIN
			...
			FROM 
			#Temp_Right tr 
			INNER JOIN Syn_Deal_Rights_Territory SDRTER (NOLOCK) on SDRTER.Syn_Deal_Rights_Code = tr.Syn_Deal_Rights_Code
			LEFT JOIN Territory_Details td (NOLOCK) on (td.Territory_Code = SDRTER.Territory_Code AND SDRTER.Territory_Type = 'G')
		...
		BEGIN
			...
			BEGIN
				...
				Select Distinct ',' + 
				CAST(Case When Language_Type = 'G' Then lgd.Language_Code Else adrs.Language_Code End AS NVARCHAR) From Syn_Deal_Rights_Subtitling adrs (NOLOCK)
			...
			BEGIN
				Select Distinct Syn_Deal_Rights_Code, STUFF((
				Select Distinct ',' + 
				CAST(Case When Language_Type = 'G' Then lgd.Language_Code Else adrd.Language_Code End AS NVARCHAR) From Syn_Deal_Rights_Dubbing adrd (NOLOCK)
			...
			BEGIN
			   ...
			   END		
				...
				INNER JOIN Syn_Deal_Rights SDR (NOLOCK) ON SDR.Syn_Deal_Rights_Code = TR.Syn_Deal_Rights_Code
				INNER JOIN Syn_Deal SD (NOLOCK) ON SD.Syn_Deal_Code = SDR.Syn_Deal_Code
				INNER JOIN Syn_Deal_Movie SDM (NOLOCK) ON SDM.Syn_Deal_Code = SDM.Syn_Deal_Code AND SDM.Title_Code = TM.Title_Code
					AND TM.Episode_From = SDM.Episode_From AND TM.Episode_To = SDM.Episode_End_To
				INNER JOIN Vendor V (NOLOCK) ON V.Vendor_Code = SD.Vendor_Code
				INNER JOIN Title T (NOLOCK) ON T.Title_Code = SDM.Title_Code
				INNER JOIN [Platform] P (NOLOCK) ON P.Platform_Code = TM.Platform_Code
	...
	if(@Loglevel<2)Exec [USPLogSQLSteps] '[USP_Report_PlatformWise_Syndication_Neo]', 'Step 2', 0, 'Procedure Excuting Completed', 0, ''
END