...
BEGIN   
	...
	if(@Loglevel < 2)Exec [USPLogSQLSteps] '[USP_GetPromoterCodes]', 'Step 1', 0, 'Started Procedure', 0, ''
		 --DECLARE   
		...
		BEGIN  
				...
				SELECT Title_Code, Episode_From, Episode_End_To INTO #Temp_Syn_Deal_Movie FROM Syn_Deal_Movie (NOLOCK)
				...
				BEGIN  
						...
						SELECT Title_Code, Episode_FROM, Episode_To FROM Acq_Deal_Rights_Title  (NOLOCK)     
		...
		BEGIN
			  ...
			  SELECT Acq_Deal_Rights_Code  FROM ( 
				  ...
				  SELECT Acq_Deal_Rights_Code from Acq_Deal_Rights_Platform (NOLOCK) WHERE Platform_Code IN(SELECT NUMBER FROM fn_Split_withdelemiter(@Platform_Code, ','))                   
				  INTERSECT   
				  SELECT ADR.Acq_Deal_Rights_Code   from Acq_Deal_Rights_Territory ADR (NOLOCK)
				  LEFT JOIN  Territory_Details TD (NOLOCK) ON TD.Territory_Code = ADR.Territory_Code
				  WHERE TD.country_code  IN(SELECT NUMBER FROM fn_Split_withdelemiter(@Country_Code ,','))            
				  UNION
				  SELECT DISTINCT ADR.Acq_Deal_Rights_Code   from Acq_Deal_Rights_Territory ADR (NOLOCK)
				  LEFT JOIN  Territory_Details TD (NOLOCK) ON TD.Territory_Code = ADR.Territory_Code
		...
		BEGIN
			...
			SELECT DISTINCT Country_Code INTO #CountryHoldback FROM Territory_Details (NOLOCK) WHERE Territory_Code  IN (SELECT NUMBER FROM fn_Split_withdelemiter(@Country_Code, ','))

			PRINT 'Getting all the Territory Code which has been assigned with the Country Code'
			SELECT DISTINCT td.Territory_Code INTO #TerritoryHoldback FROM Territory_Details td (NOLOCK) WHERE td.country_code IN (SELECT country_code FROM  #CountryHoldback)
			...
			BEGIN
				SELECT Acq_Deal_Rights_Code INTO #Temp_ADRC FROM Acq_Deal_Rights_Territory  (NOLOCK)       
		...
		INNER JOIN Acq_Deal_Rights AR (NOLOCK) On T.Acq_Deal_Rights_Code = AR.Acq_Deal_Rights_Code       
		...
		SELECT
		 ...
		 INNER JOIN Acq_Deal_Rights ADR (NOLOCK) On ADR.Acq_Deal_Rights_Code = T.Acq_Deal_Rights_Code             
		 INNER JOIN Acq_Deal_Rights_Title RT (NOLOCK) On AR.Acq_Deal_Rights_Code = RT.Acq_Deal_Rights_Code       
		...
		BEGIN         
			...
			BEGIN 
					...
					SELECT @TitleLanguage = Is_Title_Language_Right FROM Acq_Deal_Rights (NOLOCK) WHERE Acq_Deal_Rights_Code = @Acq_Deal_Rights_Code   
					...
					BEGIN       
						SELECT @Rights_Start_Date = Convert(date,Actual_Right_Start_Date,101) from Acq_Deal_Rights (NOLOCK) where Acq_Deal_Rights_Code = @Acq_Deal_Rights_Code       
					...
					BEGIN 
						 ...
						 SELECT Language_Code FROM (     
							SELECT DISTINCT Language_Code FROM Language_Group_Details (NOLOCK) WHERE Language_Group_Code IN
							(SELECT Language_Group_Code FROM Acq_Deal_Rights_Subtitling (NOLOCK) WHERE  Acq_Deal_Rights_Code = @Acq_Deal_Rights_Code)
							UNION
							SELECT DISTINCT Language_Code FROM Acq_Deal_Rights_Subtitling (NOLOCK) WHERE  Acq_Deal_Rights_Code = @Acq_Deal_Rights_Code
						 ) AS Language_Code
					...
					BEGIN
						...
						SELECT DISTINCT Language_Code FROM Language_Group_Details (NOLOCK) WHERE Language_Group_Code IN (SELECT NUMBER FROM fn_Split_withdelemiter(@Subtitle_Code, ','))

						INSERT INTO #Language_GroupHoldback
						SELECT DISTINCT td.Language_Group_Code FROM Language_Group_Details td (NOLOCK) WHERE td.Language_Code IN
						...
						BEGIN   
							IF EXISTS ( SELECT Acq_Deal_Rights_Code  FROM Acq_Deal_Rights_Subtitling (NOLOCK)  WHERE Language_Group_Code IN
							(SELECT Language_Group_Code FROM #Language_GroupHoldback) and Acq_Deal_Rights_Code = @Acq_Deal_Rights_Code)
							BEGIN
								 SET @Subtitling = 'Y'
							END 
							IF EXISTS ( SELECT Acq_Deal_Rights_Code  FROM Acq_Deal_Rights_Subtitling (NOLOCK) WHERE Language_Code IN
					...
					BEGIN
						 ...
						 SELECT Language_Code FROM (     
							SELECT DISTINCT Language_Code FROM Language_Group_Details (NOLOCK) WHERE Language_Group_Code IN
							(SELECT Language_Group_Code FROM Acq_Deal_Rights_Dubbing (NOLOCK) WHERE  Acq_Deal_Rights_Code = @Acq_Deal_Rights_Code)
							UNION
							SELECT DISTINCT Language_Code FROM Acq_Deal_Rights_Dubbing (NOLOCK) WHERE  Acq_Deal_Rights_Code = @Acq_Deal_Rights_Code
						 ) AS Language_Code
					...
					BEGIN       
						...
						INSERT INTO #LanguageHoldback
						SELECT DISTINCT Language_Code FROM Language_Group_Details (NOLOCK) WHERE Language_Group_Code IN (SELECT NUMBER FROM fn_Split_withdelemiter(@Dubbing_Code, ','))

						INSERT INTO #Language_GroupHoldback
						SELECT DISTINCT td.Language_Group_Code FROM Language_Group_Details td (NOLOCK) WHERE td.Language_Code IN (SELECT Language_Code FROM  #LanguageHoldback)
						...
						BEGIN   
							IF EXISTS ( SELECT Acq_Deal_Rights_Code  FROM Acq_Deal_Rights_Dubbing (NOLOCK) WHERE Language_Group_Code IN
							(SELECT Language_Group_Code FROM #Language_GroupHoldback) and Acq_Deal_Rights_Code = @Acq_Deal_Rights_Code)
							BEGIN
								 SET @Dubbing = 'Y'
							END    
							 IF EXISTS ( SELECT Acq_Deal_Rights_Code  FROM Acq_Deal_Rights_Dubbing (NOLOCK)  WHERE Language_Code IN
							(SELECT Language_Code FROM #LanguageHoldback) and Acq_Deal_Rights_Code = @Acq_Deal_Rights_Code)
							BEGIN
								 SET @Dubbing = 'Y'
							END                
					...
					BEGIN
						...
						BEGIN            
							...
							FROM Acq_Deal_Rights_Promoter (NOLOCK) WHERE Acq_Deal_Rights_Code IN (SELECT NUMBER FROM fn_Split_withdelemiter(CAST(@ADR_Codes AS VARCHAR(1000)), ','))          
							FOR XML PATH('')),1,1,'')
						END
						ELSE
						BEGIN
							SELECT @Acq_Deal_Rights_Promoter_Code = STUFF((SELECT Distinct ',' + CAST(Acq_Deal_Rights_Promoter_Code AS NVARCHAR(MAX)) 
							FROM Acq_Deal_Rights_Promoter (NOLOCK) WHERE Acq_Deal_Rights_Code = @Acq_Deal_Rights_Code
							FOR XML PATH('')),1,1,'')
						END
	...
	if(@Loglevel < 2)Exec [USPLogSQLSteps] '[USP_GetPromoterCodes]', 'Step 2', 0, 'Procedure Excution Completed', 0, ''     
END