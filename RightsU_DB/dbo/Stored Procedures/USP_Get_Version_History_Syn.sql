...
BEGIN  
	...
	if(@Loglevel < 2)Exec [USPLogSQLSteps] '[USP_Get_Version_History_Syn]', 'Step 1', 0, 'Started Procedure', 0, '' 
	 ...
	 Select @Selected_Deal_Type_Code = Deal_Type_Code From Acq_Deal (NOLOCK) Where Acq_Deal_Code = @Syn_Deal_Code  
	...
	*/  
	 ...
	 cast(AAD.Version as decimal),'',USR.Login_Name, DS.Deal_Segment_Name, RV.Revenue_Vertical_Name
	  ...
	  LEFT OUTER JOIN Vendor_Contacts VC (NOLOCK) ON AAD.Vendor_Contact_Code = VC.Vendor_Contacts_Code  
	  INNER JOIN Currency CR (NOLOCK) ON AAD.Currency_Code = CR.Currency_Code  
	  INNER JOIN Syn_Deal AD (NOLOCK) ON AD.Syn_Deal_Code = AAD.Syn_Deal_Code
	...
	BEGIN  
	 ...
	 from AT_Syn_Deal AAD (NOLOCK)  
	 INNER JOIN AT_Syn_Deal_Movie AADM (NOLOCK) ON AAD.AT_Syn_Deal_Code = AADM.AT_Syn_Deal_Code  
	 ...
	 from Syn_Deal AAD  (NOLOCK) 
	 INNER JOIN Syn_Deal_Movie AADM (NOLOCK) ON AAD.Syn_Deal_Code = AADM.Syn_Deal_Code  
	 INNER JOIN Title t (NOLOCK) ON t.Title_Code = AADM.Title_Code  
	  INNER JOIN Users USR (NOLOCK) ON USR.Users_Code = CASE WHEN version = '0001' THEN 
					AAD.Inserted_By
				  ELSE
	...
	*/  
	...
	BEGIN  
	 ...
	 FROM AT_SYN_Deal_Rights AADR (NOLOCK)   
	 INNER JOIN AT_Syn_Deal AAD (NOLOCK) ON AADR.AT_SYN_Deal_Code = AAD.AT_Syn_Deal_Code  
	 ...
	 ISNULL(AADR.Milestone_Unit_Type,'') Milestone_Unit_Type,AADR.Is_ROFR,ISNULL(Convert(varchar(20),AADR.ROFR_Date,106),'') ROFR_Date,ISNULL(AADR.Restriction_Remarks,'') Restriction_Remarks,ISNULL(Convert(varchar(20),AADR.Effective_Start_Date,106),'') Effective_Start_Date  
	 ,ISNULL(R.ROFR_Type,'') ROFR_Type, 'UnChanged' [Status],cast(AAD.Version as decimal), '',USR.Login_Name  
	 from Syn_Deal_Rights AADR (NOLOCK)   
	 INNER JOIN Syn_Deal AAD (NOLOCK) ON AADR.Syn_Deal_Code = AAD.Syn_Deal_Code  
	 LEFT JOIN Sub_License S (NOLOCK) ON AADR.Sub_License_Code = S.Sub_License_Code  
	 LEFT JOIN Milestone_Type MT (NOLOCK) ON AADR.Milestone_Type_Code = MT.Milestone_Type_Code  
	 LEFT JOIN ROFR R (NOLOCK) ON AADR.ROFR_Code = R.ROFR_Code  
	 INNER JOIN Users USR (NOLOCK) ON USR.Users_Code = CASE WHEN version = '0001' THEN   
	 ...
	 Declare CUR_Right Cursor For Select AT_Syn_Deal_Rights_Code,Version,VersionOld,Syn_Deal_Rights_Code,[Index] From #TempRightsHistory  
	   ...
	   Begin  
		...
		BEGIN  
		 Set @Codes = ''  
		 Select @Codes = @Codes + Cast(Platform_Code as varchar) + ',' From AT_Syn_Deal_Rights_Platform (NOLOCK) Where AT_Syn_Deal_Rights_Code = @AT_Syn_Deal_Rights_Code  
		 SET  @Platform_Name = ''  
		 ...
		 Select @Title_Names = @Title_Names + Title_Name + ', ' From (  
		  ...
		  From Title t (NOLOCK)   
		  Inner Join AT_Syn_Deal_Rights_Title adrt (NOLOCK) On t.Title_Code = adrt.Title_Code   
		...
		BEGIN  
		 Set @Codes = ''  
		 Select @Codes = @Codes + Cast(Platform_Code as varchar) + ',' From Syn_Deal_Rights_Platform (NOLOCK) Where Syn_Deal_Rights_Code = @Syn_Deal_Rights_Code  
		 SET  @Platform_Name = ''  
		 select @Platform_Name = @Platform_Name + Platform_Hiearachy + ',' from dbo.UFN_Get_Platform_With_Parent (@Codes)  
		 set @Platform_Name = reverse(stuff(reverse(@Platform_Name), 1, 1, ''))  
		 ...
		 Select @Title_Names = @Title_Names + Title_Name + ', ' From (  
		  ...
		  Else t.Title_Name End As Title_Name  
		  From Title t (NOLOCK)   
		  Inner Join Syn_Deal_Rights_Title adrt (NOLOCK) On t.Title_Code = adrt.Title_Code   
	...
	BEGIN  
	 ...
	 Declare CUR_Right Cursor For Select AT_Syn_Deal_Rights_Code,Version,VersionOld,Syn_Deal_Rights_Code,[Index] From #TempPushbackHistory  
	   ...
	   Begin  
		...
		BEGIN  
		 Set @Codes_Pushback = ''  
		 Select @Codes_Pushback = @Codes_Pushback + Cast(Platform_Code as varchar) + ',' From AT_Syn_Deal_Rights_Platform (NOLOCK) Where AT_Syn_Deal_Rights_Code = @AT_Syn_Deal_Pushback_Code  
		...
		BEGIN  
		 Set @Codes_Pushback = ''  
		 Select @Codes_Pushback = @Codes_Pushback + Cast(Platform_Code as varchar) + ',' From Syn_Deal_Rights_Platform (NOLOCK) Where Syn_Deal_Rights_Code = @Syn_Deal_Pushback_Code  
		 SET  @Platform_Name_Pushback = ''  
		 select @Platform_Name_Pushback = @Platform_Name_Pushback + Platform_Hiearachy + ',' from dbo.UFN_Get_Platform_With_Parent (@Codes_Pushback)  
		 set @Platform_Name_Pushback = reverse(stuff(reverse(@Platform_Name_Pushback), 1, 1, ''))  
	...
	*/  
	...
	BEGIN  
	 ...
	 FROM AT_Syn_Deal_Payment_Terms AAPT (NOLOCK)   
	 INNER JOIN AT_Syn_Deal AAD (NOLOCK) ON AAPT.AT_Syn_Deal_Code = AAD.AT_Syn_Deal_Code  
	 LEFT JOIN Cost_Type CT (NOLOCK) ON AAPT.Cost_Type_Code = CT.Cost_Type_Code  
	 LEFT JOIN Payment_Terms PT (NOLOCK) ON AAPT.Payment_Terms_Code = PT.Payment_Terms_Code  
	 INNER JOIN Users USR (NOLOCK) ON USR.Users_Code = CASE WHEN version = '0001' THEN   
		AAD.Inserted_By  
		 ELSE  
		AAD.Last_Action_By  
	...
	,AAPT.Percentage,CONVERT(VARCHAR(20),AAPT.Due_Date,106) Due_Date, 'UnChanged' [Status],cast(AAD.Version as decimal), '',USR.Login_Name  
	 FROM Syn_Deal_Payment_Terms AAPT (NOLOCK)   
	 INNER JOIN Syn_Deal AAD (NOLOCK) ON AAPT.Syn_Deal_Code = AAD.Syn_Deal_Code  
	 LEFT JOIN Cost_Type CT (NOLOCK) ON AAPT.Cost_Type_Code = CT.Cost_Type_Code  
	 LEFT JOIN Payment_Terms PT (NOLOCK) ON AAPT.Payment_Terms_Code = PT.Payment_Terms_Code  
	 INNER JOIN Users USR (NOLOCK) ON USR.Users_Code = CASE WHEN version = '0001' THEN   
		AAD.Inserted_By  
	...
	*/  
	 ...
	 FROM AT_Syn_Deal_Attachment AADA (NOLOCK)   
	 INNER JOIN AT_Syn_Deal AAD (NOLOCK) ON AADA.AT_Syn_Deal_Code = AAD.AT_Syn_Deal_Code  
	 ...
	 RIGHT('000' + cast( (cast(AAD.Version as decimal) +1) as varchar(50)),4) VersionOld,ISNULL(AADA.Syn_Deal_Attachment_Code, 0)Syn_Deal_Attachment_Code,  
	 T.Title_Name,AADA.Attachment_Title,AADA.Attachment_File_Name,AADA.System_File_Name,DT.Document_Type_Name,AADA.Episode_From,AADA.Episode_To,'UnChanged' [Status] ,cast(AAD.Version as decimal), ''  
	 FROM Syn_Deal_Attachment AADA  (NOLOCK)  
	 INNER JOIN Syn_Deal AAD (NOLOCK) ON AADA.Syn_Deal_Code = AAD.Syn_Deal_Code  
	 LEFT JOIN Document_Type DT (NOLOCK) ON AADA.Document_Type_Code = DT.Document_Type_Code  
	 LEFT JOIN Title T (NOLOCK) ON AADA.Title_Code = T.Title_Code  
	...
	*/  
	...
	BEGIN  
	 ...
	 FROM AT_Syn_Deal_Material AADM  (NOLOCK)  
	 INNER JOIN AT_Syn_Deal AAD (NOLOCK) ON AADM.AT_Syn_Deal_Code = AAD.AT_Syn_Deal_Code  
	 ...
	 RIGHT('000' + cast( (cast(AAD.Version as decimal) +1) as varchar(50)),4) VersionOld,ISNULL(AADM.Syn_Deal_Material_Code, 0)Syn_Deal_Material_Code,  
	 T.Title_Name,MM.Material_Medium_Name,MT.Material_Type_Name,AADM.Quantity,AADM.Episode_From,AADM.Episode_To,'UnChanged' [Status],cast(AAD.Version as decimal), '',USR.Login_Name  
	 FROM Syn_Deal_Material AADM (NOLOCK)   
	 INNER JOIN Syn_Deal AAD (NOLOCK) ON AADM.Syn_Deal_Code = AAD.Syn_Deal_Code  
	 LEFT JOIN Material_Medium MM (NOLOCK) ON AADM.Material_Medium_Code= MM.Material_Medium_Code  
	 LEFT JOIN Material_Type MT (NOLOCK) ON AADM.Material_Type_Code = MT.Material_Type_Code  
	 LEFT JOIN Title T (NOLOCK) ON AADM.Title_Code = T.Title_Code  
	 INNER JOIN Users USR (NOLOCK) ON USR.Users_Code = CASE WHEN version = '0001' THEN   
	...
	*/  
	 ...
	 'UnChanged' [Status],cast(AAD.Version as decimal), '',USR.Login_Name  
	 FROM AT_Syn_Deal_Ancillary AADA (NOLOCK)  
	 ...
	 AADA.Duration,AADA.[Day],AADA.Remarks,  
	 'UnChanged' [Status],cast(AAD.Version as decimal), '',USR.Login_Name  
	 FROM Syn_Deal_Ancillary AADA (NOLOCK)   
	 INNER JOIN Ancillary_Type AT (NOLOCK) ON AADA.Ancillary_Type_code = AT.Ancillary_Type_code  
	 INNER JOIN Syn_Deal AAD (NOLOCK) ON AADA.Syn_Deal_Code = AAD.Syn_Deal_Code  
	 INNER JOIN Users USR (NOLOCK) ON USR.Users_Code = CASE WHEN version = '0001' THEN   
		AAD.Inserted_By  
	  ...
	  Begin  
	   ...
	   BEGIN  
		...
		Select @Title_Names = @Title_Names + Title_Name + ', ' From (  
		 ...
		 Else t.Title_Name End As Title_Name  
		 From Title t (NOLOCK)   
		 Inner Join AT_Syn_Deal_Ancillary_Title adrt (NOLOCK) On t.Title_Code = adrt.Title_Code   
	   ...
	   BEGIN  
		...
		Select @Title_Names = @Title_Names + Title_Name + ', ' From (  
		 ...
		 Else t.Title_Name End As Title_Name  
		 From Title t  (NOLOCK)  
		 Inner Join Syn_Deal_Ancillary_Title adrt (NOLOCK) On t.Title_Code = adrt.Title_Code   
	...
	if(@Loglevel < 2)Exec [USPLogSQLSteps] '[USP_Get_Version_History_Syn]', 'Step 2', 0, 'Procedure Excution Completed', 0, '' 
END