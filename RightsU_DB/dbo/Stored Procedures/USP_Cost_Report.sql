...
Begin     
	...
	if(@Loglevel < 2)Exec [USPLogSQLSteps] '[USP_Cost_Report]', 'Step 1', 0, 'Started Procedure', 0, '' 
	...
	begin  
	  ...
	  ,STUFF((SELECT distinct ',' +  CAST(v.Vendor_Name AS VARCHAR)  FROM Vendor  v  (NOLOCK)
	...
	End  
	  ...
	  inner join Title T (NOLOCK) on ADCT.Title_Code = T.Title_Code   
	  inner join Deal_Type DT (NOLOCK) on Ad.Deal_Type_Code = DT.Deal_Type_Code    
	  inner join Syn_Deal_Rights ADR (NOLOCK) on Ad.Syn_Deal_Code= ADR.Syn_Deal_Code    
	...
	if(@Loglevel < 2)Exec [USPLogSQLSteps] '[USP_Cost_Report]', 'Step 2', 0, 'Procedure Excution Completed', 0, '' 
End