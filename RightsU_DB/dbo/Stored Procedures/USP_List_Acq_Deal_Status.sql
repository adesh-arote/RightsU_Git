...
BEGIN
	...
	if(@Loglevel  < 2)Exec [USPLogSQLSteps] '[USP_List_Acq_Deal_Status]', 'Step 1', 0, 'Started Procedure', 0, ''
		SET FMTONLY OFF
		SET NOCOUNT ON;
		...
		SELECT  @Deal_Complete_Flag = ','+Deal_Complete_Flag+',' FROM Acq_Deal (NOLOCK)  Where Acq_Deal_Code = @Acq_Deal_Code
	...
	if(@Loglevel  < 2)Exec [USPLogSQLSteps] '[USP_List_Acq_Deal_Status]', 'Step 2', 0, 'Procedure Excution Completed', 0, ''
END