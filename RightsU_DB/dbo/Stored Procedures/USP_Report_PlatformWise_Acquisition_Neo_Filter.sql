CREATE PROCEDURE [dbo].[USP_Report_PlatformWise_Acquisition_Neo_Filter]
(
...
BEGIN
	...
	if(@Loglevel<2)Exec [USPLogSQLSteps] '[USP_Report_PlatformWise_Acquisition_Neo_Filter]', 'Step 1', 0, 'Started Procedure', 0, ''
		...
		DECLARE @TitleNames NVARCHAR(MAX), @PlatformNames NVARCHAR(MAX), @TerritoryNames NVARCHAR(MAX), @CountryNames NVARCHAR(MAX),
		 @MustHavePlatformsNames NVARCHAR(MAX)
		, @LanguageGroupNames NVARCHAR(MAX), @Subtit_LanguageNames NVARCHAR(MAX), @MustHaveCountryNames NVARCHAR(MAX),
		 @Dubbing_LanguageNames NVARCHAR(MAX)
		DECLARE @TitleLanguage_Names NVARCHAR(MAX)='', @ExclusionCountryNames NVARCHAR(MAX)='', @Tmp_MustHaveCountryNames NVARCHAR(MAX)='',
		 @SubDub NVARCHAR(MAX) =''
		/*---Start Title---*/
	...
	if(@Loglevel<2)Exec [USPLogSQLSteps] '[USP_Report_PlatformWise_Acquisition_Neo_Filter]', 'Step 2', 0, 'Procedure Excuting Completed', 0, ''
END