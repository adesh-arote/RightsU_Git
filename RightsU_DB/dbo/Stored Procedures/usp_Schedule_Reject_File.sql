...
BEGIN  
	...
	if(@Loglevel<2)Exec [USPLogSQLSteps] '[usp_Schedule_Reject_File]', 'Step 1', 0, 'Started Procedure', 0, ''
	-- =============================================
	/*	
	...
	*/
	-- =============================================
		...
		SELECT @OffsetTime = ISNUll(OffsetTime_Schedule,'00:00') FROM Channel (NOLOCK) WHERE channel_code = @ChannelCode
		...
		FROM Temp_BV_Schedule (NOLOCK) WHERE 1=1 AND File_Code = @File_Code --AND Channel_Code IN (@ChannelCode) --Commented By Bhavesh on 24 Dec 2014
		...
		FROM Temp_BV_Schedule (NOLOCK) WHERE 1 = 1 AND File_Code = @File_Code AND Channel_Code IN (@ChannelCode) 
	...
	if(@Loglevel<2)Exec [USPLogSQLSteps] '[usp_Schedule_Reject_File]', 'Step 2', 0, 'Procedure Excuting Completed', 0, ''
END
