CREATE PROC [dbo].[USP_Get_Title_WBS_Data]
(
	@Acq_Deal_Code INT,
...
BEGIN
	...
	if(@Loglevel < 2)Exec [USPLogSQLSteps] '[USP_Get_Title_WBS_Data]', 'Step 1', 0, 'Started Procedure', 0, ''
		...
		SELECT @VersionNo = Cast(Version as INT) FROM Acq_Deal (NOLOCK) WHERE Acq_Deal_Code = @Acq_Deal_Code
		...
		INNER JOIN SAP_WBS SW (NOLOCK) ON SW.SAP_WBS_Code = ADB.SAP_WBS_Code
		INNER JOIN Acq_Deal_Rights ADR (NOLOCK) ON ADR.Acq_Deal_Code = ADB.Acq_Deal_Code 
		...
		WHERE ADRP.Platform_Code IN (SELECT Platform_Code FROM Platform (NOLOCK) WHERE ISNULL(Is_No_Of_Run, '') = 'Y') AND
		...
		IF(@VersionNo > 1)
		BEGIN
			SELECT @AT_Acq_Deal_Code = AT_Acq_Deal_Code FROM AT_Acq_Deal (NOLOCK) WHERE Acq_Deal_Code = @Acq_Deal_Code AND [Version] =  (@VersionNo - 1)
			...
			FROM AT_Acq_Deal_Budget ADB (NOLOCK)
			INNER JOIN SAP_WBS SW (NOLOCK) ON SW.SAP_WBS_Code = ADB.SAP_WBS_Code
			INNER JOIN AT_Acq_Deal_Rights ADR (NOLOCK) ON ADR.AT_Acq_Deal_Code = ADB.AT_Acq_Deal_Code 
			INNER JOIN AT_Acq_Deal_Rights_Title ADRT (NOLOCK) ON ADRT.AT_Acq_Deal_Rights_Code = ADR.AT_Acq_Deal_Rights_Code 
			INNER JOIN AT_Acq_Deal_Rights_Platform ADRP (NOLOCK) ON ADRP.AT_Acq_Deal_Rights_Code = ADR.AT_Acq_Deal_Rights_Code 
			AND ADRT.Title_Code = ADB.Title_Code AND ADRT.Episode_From = ADB.Episode_From AND ADRT.Episode_To = ADB.Episode_To
			WHERE ADRP.Platform_Code IN (SELECT Platform_Code FROM Platform (NOLOCK) WHERE ISNULL(Is_No_Of_Run, '') = 'Y') AND
	...
	if(@Loglevel < 2)Exec [USPLogSQLSteps] '[USP_Get_Title_WBS_Data]', 'Step 2', 0, 'Procedure Excution Completed', 0, ''
END