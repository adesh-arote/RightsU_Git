...
BEGIN
	...
	if(@Loglevel < 2)Exec [USPLogSQLSteps] '[USP_DELETE_Syn_DEAL]', 'Step 1', 0, 'Started Procedure', 0, ''
		SET NOCOUNT ON
		BEGIN TRY
		BEGIN TRANSACTION;
		...
		IN (SELECT Syn_Deal_Run_Code FROM [dbo].[Syn_Deal_Run] (NOLOCK) WHERE Syn_Deal_Code = @Syn_Deal_Code)
		...
		(
			SELECT Syn_Deal_Rights_Holdback_Code FROM Syn_Deal_Rights_Holdback  (NOLOCK)
			WHERE Syn_Deal_Rights_Code IN
			(
				SELECT Syn_Deal_Rights_Code FROM Syn_Deal_Rights  (NOLOCK)
		...
		(
			SELECT Syn_Deal_Rights_Blackout_Code FROM Syn_Deal_Rights_Blackout (NOLOCK)
		WHERE Syn_Deal_Rights_Code IN
			(
				SELECT Syn_Deal_Rights_Code FROM Syn_Deal_Rights (NOLOCK) 
				WHERE Syn_Deal_Code =@Syn_Deal_Code
		...
		(
		SELECT Syn_Deal_Rights_Promoter_Code FROM Syn_Deal_Rights_Promoter (NOLOCK)
		WHERE Syn_Deal_Rights_Code IN
			(
				SELECT Syn_Deal_Rights_Code FROM Syn_Deal_Rights (NOLOCK) 
				WHERE Syn_Deal_Code =@Syn_Deal_Code
		...
		(	
			SELECT Syn_Deal_Ancillary_Code From Syn_Deal_Ancillary (NOLOCK) WHERE Syn_Deal_Code = @Syn_Deal_Code
		) 
		DELETE FROM Syn_Deal_Ancillary_Platform_Medium WHERE  Syn_Deal_Ancillary_Platform_Code IN
		(
			SELECT  Syn_Deal_Ancillary_Platform_Code From Syn_Deal_Ancillary_Platform (NOLOCK) WHERE  Syn_Deal_Ancillary_Code IN
			(	
				SELECT Syn_Deal_Ancillary_Code From Syn_Deal_Ancillary (NOLOCK) WHERE Syn_Deal_Code = @Syn_Deal_Code
			) 
		)
		...
		(	
			SELECT Syn_Deal_Revenue_Code From Syn_Deal_Revenue (NOLOCK) WHERE Syn_Deal_Code = @Syn_Deal_Code
		) 
		...
		(	
			SELECT Syn_Deal_Revenue_Costtype_Code From Syn_Deal_Revenue_Costtype (NOLOCK) WHERE Syn_Deal_Revenue_Code IN
			(	
				SELECT Syn_Deal_Revenue_Code From Syn_Deal_Revenue (NOLOCK) WHERE Syn_Deal_Code = @Syn_Deal_Code
			)
		) 
		...
		DELETE FROM [dbo].[Syn_Deal_Supplementary_detail] WHERE Syn_Deal_Supplementary_Code in (SELECT Syn_Deal_Supplementary_Code FROM [dbo].[Syn_Deal_Supplementary] (NOLOCK) 
		WHERE Syn_Deal_Code = @syn_Deal_Code)

		DELETE FROM [dbo].[Syn_Deal_Supplementary] WHERE Syn_Deal_Code = @Syn_Deal_Code

		/*Delete from supplementary*/

		/*Delete from Digital*/
		
		DELETE FROM [dbo].[Syn_Deal_Digital_Detail] WHERE Syn_Deal_Digital_Code in (SELECT Syn_Deal_Digital_Code FROM [dbo].[Syn_Deal_Digital] (NOLOCK) 
		WHERE Syn_Deal_Code = @syn_Deal_Code)

		DELETE FROM [dbo].[Syn_Deal_Digital] WHERE Syn_Deal_Code = @Syn_Deal_Code
	...
	if(@Loglevel < 2)Exec [USPLogSQLSteps] '[USP_DELETE_Syn_DEAL]', 'Step 2', 0, 'Procedure Excution Completed', 0, ''
END