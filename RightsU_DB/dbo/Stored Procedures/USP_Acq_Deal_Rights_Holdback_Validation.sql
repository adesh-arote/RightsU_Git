...
BEGIN
	...
	if(@Loglevel < 2)Exec [USPLogSQLSteps] '[USP_Acq_Deal_Rights_Holdback_Validation]', 'Step 1', 0, 'Started Procedure', 0, ''
		...
		BEGIN
			...
			BEGIN
				...
				SELECT Title_Code, Episode_Starts_FROM, Episode_End_To FROM Acq_Deal_Movie (NOLOCK) 
			...
			SELECT Acq_Deal_Rights_Holdback_Code INTO #TempHoldbackCodes FROM (
				SELECT DISTINCT Acq_Deal_Rights_Holdback_Code FROM Acq_Deal_Rights_Holdback_Platform (NOLOCK)
				WHERE Platform_Code IN(SELECT NUMBER FROM fn_Split_withdelemiter(@Platform_Code, ','))
				INTERSECT
				SELECT DISTINCT Acq_Deal_Rights_Holdback_Code FROM Acq_Deal_Rights_Holdback_Territory  (NOLOCK)
			...
			INNER JOIN Acq_Deal_Rights_Holdback HB (NOLOCK) On T.Acq_Deal_Rights_Holdback_Code = HB.Acq_Deal_Rights_Holdback_Code
			...
			INNER JOIN Acq_Deal_Rights_Title RT (NOLOCK) On HB.Acq_Deal_Rights_Code = RT.Acq_Deal_Rights_Code
			...
			BEGIN
				...
				BEGIN
					...
					INNER JOIN Acq_Deal_Rights_Holdback_Subtitling ADRHS (NOLOCK) ON ADRHS.Acq_Deal_Rights_Holdback_Code = THB.Acq_Deal_Rights_Holdback_Code
				...
				BEGIN
					...
					SELECT @Rec_Count = COUNT(THB.Acq_Deal_Rights_Code) FROM #TempHoldback_TL THB
					INNER JOIN Acq_Deal_Rights_Holdback_Dubbing ADRHD (NOLOCK) ON ADRHD.Acq_Deal_Rights_Holdback_Code = THB.Acq_Deal_Rights_Holdback_Code
	...
	if(@Loglevel < 2)Exec [USPLogSQLSteps] '[USP_Acq_Deal_Rights_Holdback_Validation]', 'Step 2', 0, 'Procedure Excution Completed', 0, ''
END