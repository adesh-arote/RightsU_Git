CREATE PROCEDURE [dbo].[USP_Acq_Bulk_Update_Validation]
(
...
BEGIN
	...
	if(@Loglevel < 2)Exec [USPLogSQLSteps] '[USP_Acq_Bulk_Update_Validation]', 'Step 1', 0, 'Started Procedure', 0, ''
		...
		BEGIN
			...
			BEGIN
				...
				BEGIN
					...
					from #SelectedCodeTab AS t 
					LEFT JOIN 
					Acq_Deal_Rights_Dubbing AS SDRP (NOLOCK)ON SDRP.Acq_Deal_Rights_Code = @RCODE AND SDRP.Language_Group_Code = t.I_Code
			...
			BEGIN
				...
				BEGIN
					IF(ISNULL(@IS_SYN_ACQ_MAP,'') = 'Y')
					BEGIN
						if(EXISTS(Select Syn_Deal_Rights_Code FROM Syn_Acq_Mapping (NOLOCK) where Deal_Rights_Code=@RCODE))
					BEGIN
						...
						BEGIN
							...
							BEGIN
								INSERT INTO #Result(Msg_Type,Right_Codes)
								Select 'Can not be set to Tentative as it is already syndicated' AS Msg_Type,Deal_Rights_Code FROM Syn_Acq_Mapping (NOLOCK) where Deal_Rights_Code = @RCODE
							END
						...
						END
						ELSE IF(@Rights_Type = (SELECT Right_Type FROM Acq_Deal_Rights (NOLOCK) WHERE Acq_Deal_Rights_Code = @RCODE))
						BEGIN
				...
				BEGIN
					...
					BEGIN
						INSERT INTO #RCodeForSynMap(Right_Code)
						select DISTINCT  SAM.Deal_Rights_Code FROM Syn_Acq_Mapping SAM (NOLOCK) INNER JOIN Syn_Deal_Rights SDR (NOLOCK) ON SAM.Syn_Deal_Rights_Code=SDR.Syn_Deal_Rights_Code
						where SAM.Deal_Rights_Code IN(@RCODE) 
					END
					INSERT INTO ##Error_Record (Title_Name,Platform_Name,Right_Start_Date,Right_End_Date,Right_Type,Is_Sub_License,
					Is_Title_Language_Right,Country_Name,Subtitling_Language,Dubbing_Language,Agreement_No,ErrorMSG,Episode_From,Episode_To)
	...
	if(@Loglevel < 2)Exec [USPLogSQLSteps] '[USP_Acq_Bulk_Update_Validation]', 'Step 2', 0, 'Procedure Excution Completed ', 0, ''
END
