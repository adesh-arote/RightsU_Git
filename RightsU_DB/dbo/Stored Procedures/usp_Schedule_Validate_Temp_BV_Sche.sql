...
BEGIN
	...
	if(@Loglevel< 2)Exec [USPLogSQLSteps] '[usp_Schedule_Validate_Temp_BV_Sche]', 'Step 1', 0, 'Started Procedure', 0, ''
	SET NOCOUNT ON;
	...
	--BEGIN TRANSACTION		--  Transaction begins here       
		...
		DELETE FROM Temp_BV_Schedule WHERE ISNULL(Program_Category,'') NOT IN (SELECT BV_Program_Category_Name FROM BV_Program_Category (NOLOCK) WHERE Is_Active = 'Y')
		...
		BEGIN	
			...
			FOR               
				...
				Program_Category
				FROM Temp_BV_Schedule tbs (NOLOCK) WHERE tbs.File_Code = @File_Code
			...
			SELECT @EmailMsg_InvalidHID = Email_Msg FROM Email_Notification_Msg (NOLOCK) WHERE LTRIM(RTRIM(Email_Msg_For)) = 'HouseIDNotFound' AND [Type] = 'S'
			...
			SET Channel_Name = (SELECT Channel_Name AS Channel_Name FROM Channel (NOLOCK) WHERE Channel_Code = Email.Channel_Code)
		...
		PRINT '8'		
			...
			SELECT @Count = COUNT(*) FROM Temp_BV_Schedule (NOLOCK) WHERE File_Code = @File_Code
			IF(@Count > 0)
			BEGIN
				SET @IsErrYN = 'Y'
			END
	...
	if(@Loglevel< 2)Exec [USPLogSQLSteps] '[usp_Schedule_Validate_Temp_BV_Sche]', 'Step 2', 0, 'Procedure Excuting Completed', 0, ''
END
