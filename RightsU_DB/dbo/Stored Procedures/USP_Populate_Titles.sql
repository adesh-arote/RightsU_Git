...
BEGIN
	...
	BEGIN
		...
		BEGIN
			...
			SELECT TOP 1 @Title_Code = Title_Code FROM Acq_Deal_Movie where Acq_Deal_Movie_Code = @Master_Deal_Movie_Code
			SELECT TOP 1 @Deal_Type_Name = Deal_Type_Name FROM Deal_Type WHERE Deal_Type_Code = @Deal_Type_Code
			...
			SELECT DISTINCT TitTal.Title_Code, TitTal.Title_Name FROM Map_Extended_Columns MEC 
			INNER JOIN Map_Extended_Columns_Details MECD ON MEC.Map_Extended_Columns_Code = MECD.Map_Extended_Columns_Code
			INNER JOIN Talent TAT ON TAT.Talent_Code = MECD.Columns_Value_Code
			Inner Join Title TitTal On TAT.Talent_Code = TitTal.Reference_Key
			...
			INSERT INTO #Populated_Titles(Title_Code, Title_Name)
			SELECT DISTINCT TIT.Title_Code, TIT.Title_Name FROM Title_Talent TT 
			INNER JOIN [Role] R ON R.Role_Code = TT.Role_Code AND R.Deal_Type_Code = @Deal_Type_Code
			INNER JOIN Talent TAT ON TAT.Talent_Code = TT.Talent_Code
			INNER JOIN Title TIT ON TIT.Title_Name = TAT.Talent_Name AND TIT.Deal_Type_Code = @Deal_Type_Code 
			AND ISNULL(TIT.Reference_Flag, '') = 'T' AND ISNULL(TIT.Is_Active, '') = 'Y'
			AND TIT.Title_Code NOT IN (SELECT Title_Code FROM #Selected_Titles)
		...
		BEGIN
			INSERT INTO #Populated_Titles(Title_Code, Title_Name)
			SELECT DISTINCT Title_Code, Title_Name FROM Title WHERE ISNULL(Reference_Flag, '') <> 'T'  AND ISNULL(Is_Active, '') = 'Y'
