CREATE PROC [dbo].[USP_GetSystem_Language_Message_ByModule]          
(              
 @Module_Code INT,              
 @Form_ID VARCHAR(MAX),                    
...
BEGIN    
	...
	if(@Loglevel < 2)Exec [USPLogSQLSteps] '[USP_GetSystem_Language_Message_ByModule]', 'Step 1', 0, 'Started Procedure', 0, ''          
		 SET FMTONLY OFF             
		-- DECLARE           
		 --@Module_Code INT = 163,          
		 ...
		 SELECT TOP 1 @defalultSysLangCode = System_Language_Code FROM System_Language (NOLOCK) WHERE Is_Default = 'Y'          
		 ...
		 LEFT JOIN System_Module_Message SMM (NOLOCK) ON SMM.System_Message_Code = SM.System_Message_Code          
		 LEFT JOIN System_Language_Message SLM (NOLOCK) ON SLM.System_Module_Message_Code = SMM.System_Module_Message_Code           
		 ...
		 INNER JOIN System_Module_Message SMM (NOLOCK) ON SMM.System_Message_Code = T.System_Message_Code          
		 INNER JOIN System_Language_Message SLM (NOLOCK) ON SLM.System_Module_Message_Code = SMM.System_Module_Message_Code           
	...
	if(@Loglevel < 2)Exec [USPLogSQLSteps] '[USP_GetSystem_Language_Message_ByModule]', 'Step 2', 0, 'Procedure Excution Completed', 0, ''
END