...
BEGIN  
	...
	select @Loglevel = Parameter_Value from System_Parameter_New where Parameter_Name='loglevel'
	 if(@Loglevel < 2)Exec [USPLogSQLSteps] '[USP_Title_Objection_ADV_List]', 'Step 1', 0, 'Started Procedure', 0, ''   
	 --SET @OrderByCndition = N'Last_Updated_Time DESC'      
		 ...
		 Create table #Filter(    
		  ...
		  set @SqlPageNo = N'      
					...
					INNER JOIN Title_Objection_Status TOS (NOLOCK) ON TOB.Title_Objection_Status_Code = TOS.Title_Objection_Status_Code       
		 ...
		 EXEC (@SqlPageNo)      
		  ...
		  BEGIN    
			  set @SqlPageNo = N'      
					...
					,T.Title_Name      
					FROM Title_Objection TOB WITH(NOLOCK)      
					INNER JOIN Acq_Deal AD (NOLOCK) ON TOB.Record_Code =  AD.Acq_Deal_Code    
			...
			Inner Join (Select dense_Rank() over(order by Sort Asc, Last_Updated_Time desc, agreement_no ASC) Row_Num, ID From #TempData    
			 ...
			 End     
			  ...
			  FROM Title_Objection TOB   (NOLOCK)       
			  INNER JOIN Title_Objection_Type TOT (NOLOCK) ON TOB.Title_Objection_Type_Code = TOT.Objection_Type_Code         
			  INNER JOIN Title_Objection_Status TOS (NOLOCK) ON TOB.Title_Objection_Status_Code = TOS.Title_Objection_Status_Code        
			  INNER JOIN Title T (NOLOCK) ON TOB.Title_Code = T.Title_Code        
			  INNER JOIN Acq_Deal AD (NOLOCK) ON TOB.Record_Code = AD.Acq_Deal_Code        
		 ...
		 ELSE IF(@Type = 'S')    
		  BEGIN    
			  set @SqlPageNo = N'      
					...
					,T.Title_Name      
					FROM Title_Objection TOB WITH(NOLOCK)      
					INNER JOIN Syn_Deal SD (NOLOCK) ON TOB.Record_Code =  SD.Syn_Deal_Code    
			...
			BEGIN
			 ...
			 dbo.UFN_Get_Title_Objection_Territory(TOB.Title_Objection_Code) CountryDetails,TOS.Objection_Status_Name Status ,Tm.Sort,TOB.Last_Updated_Time      
			  ...
			  INNER JOIN Title_Objection_Status TOS (NOLOCK) ON TOB.Title_Objection_Status_Code = TOS.Title_Objection_Status_Code        
			  INNER JOIN Title T (NOLOCK) ON TOB.Title_Code = T.Title_Code        
			  INNER JOIN Syn_Deal AD (NOLOCK) ON TOB.Record_Code = AD.Syn_Deal_Code        
			  INNER JOIN Vendor v WITH(NOLOCK) on AD.Vendor_Code = v.vendor_code        
			  INNER JOIN #TempData Tm on TOB.Title_Objection_Code = Tm.Title_Objection_Code        
			  WHERE TOB.Record_Type Collate Latin1_General_CI_AI IN(SELECT Deal_Type FROM #Type) '+@StrSearch+' ORDER BY Sort,TOB.Last_Updated_Time'     
	 ...
	 if(@Loglevel < 2)Exec [USPLogSQLSteps] '[USP_Title_Objection_ADV_List]', 'Step 2', 0, 'Procedure Excuting Completed', 0, ''   
END