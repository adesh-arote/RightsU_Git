...
BEGIN        
	...
	if(@Loglevel< 2)Exec [USPLogSQLSteps] '[USP_Title_List]', 'Step 1', 0, 'Started Procedure', 0, '' 
		 DECLARE @Condition NVARCHAR(MAX) = ' AND ISNULL(T.Reference_Key,'''') = '''' AND  ISNULL(T.Reference_Flag,'''') = '''' '      
		 DECLARE @delimt NVARCHAR(2) = N'﹐'      
      
			if(@TitleName != '')      
				set @Condition  += ' AND T.Title_code in (select Title_Code from Title (NOLOCK) where Title_Name IN (SELECT number FROM fn_Split_withdelemiter(N'''+@TitleName+''', N'''+ @delimt +''') where number != ''''))'      
			 print @Condition  
         
				 IF(@OriginalTitleName != '')      
				SET @Condition  += ' AND T.Title_code in (select Title_Code FROM Title (NOLOCK) WHERE Original_Title IN (SELECT number FROM fn_Split_withdelemiter(N'''+@OriginalTitleName+''', N'''+ @delimt +''') WHERE number != '''')) '      
	...
	if(@Loglevel< 2)Exec [USPLogSQLSteps] '[USP_Title_List]', 'Step 2', 0, 'Procedure Excuting Completed', 0, '' 
END