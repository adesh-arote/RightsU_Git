...
Begin
	...
	if(@Loglevel< 2)Exec [USPLogSQLSteps] '[USP_Syn_List_Runs]', 'Step 1', 0, 'Started Procedure', 0, '' 
		...
		Title_Name,
		 STUFF((SELECT distinct ',' + p.Platform_Hiearachy 
			from Syn_Deal_Run SDR (NOLOCK)
			INNER JOIN Title t (NOLOCK) ON SDR.Title_Code = TBL.Title_Code
			INNER JOIN Syn_Deal_Movie SDM (NOLOCK) ON SDR.Syn_Deal_Code = SDM.Syn_Deal_Code AND 
					SDR.Episode_From = SDM.Episode_From AND SDR.Episode_To = SDM.Episode_End_To AND SDR.Title_Code = SDM.Title_Code
			Inner JOIN Syn_Deal_Run_Platform SDRP (NOLOCK) ON tbl.Syn_Deal_Run_Code = SDRP.Syn_Deal_Run_Code
		...
		FROM (	
			...
			from Syn_Deal_Run SDR (NOLOCK)
			INNER JOIN Title t (NOLOCK) ON SDR.Title_Code = t.Title_Code                 
			INNER JOIN Syn_Deal_Movie SDM (NOLOCK) ON SDR.Syn_Deal_Code = SDM.Syn_Deal_Code AND 
						SDR.Episode_From = SDM.Episode_From AND SDR.Episode_To = SDM.Episode_End_To AND SDR.Title_Code = SDM.Title_Code
			Inner JOIN Syn_Deal_Run_Platform SDRP (NOLOCK) ON SDR.Syn_Deal_Run_Code = SDRP.Syn_Deal_Run_Code
			INNER JOIN Platform P (NOLOCK) ON SDRP.Platform_Code = p.Platform_Code
			WHERE SDR.Syn_Deal_Code = @Deal_Code                               
	...
	if(@Loglevel< 2)Exec [USPLogSQLSteps] '[USP_Syn_List_Runs]', 'Step 2', 0, 'Procedure Excuting Completed', 0, '' 
End