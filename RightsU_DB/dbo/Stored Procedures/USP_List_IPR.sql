...
BEGIN
	...
	if(@Loglevel < 2)Exec [USPLogSQLSteps] '[USP_List_IPR]', 'Step 1', 0, 'Started Procedure', 0, ''
	SET FMTONLY OFF
		--DECLARE 
		...
		(
			...
			INNER JOIN IPR_REP_CLASS IRC (NOLOCK) ON IRC.IPR_Rep_Code = IR.IPR_Rep_Code
			INNER JOIN IPR_CLASS IC (NOLOCK) ON IC.IPR_Class_Code = IRC.IPR_Class_Code
			INNER JOIN IPR_Country IT_C (NOLOCK) ON IR.Country_Code=IT_C.IPR_Country_Code
			INNER JOIN IPR_APP_STATUS IAS (NOLOCK) ON IR.Application_Status_Code=IAS.IPR_App_Status_Code
			INNER JOIN IPR_ENTITY IE  (NOLOCK) ON IR.Applicant_Code=IE.IPR_Entity_Code
			LEFT JOIN IPR_Rep_Business_Unit IPR_BU (NOLOCK) ON IR.IPR_Rep_Code=IPR_BU.IPR_Rep_Code
			LEFT JOIN IPR_Rep_Channel IPR_C (NOLOCK) ON IR.IPR_Rep_Code=IPR_C.IPR_Rep_Code
			Where 1= 1  '+@StrSearch+'
	...
	if(@Loglevel < 2)Exec [USPLogSQLSteps] '[USP_List_IPR]', 'Step 2', 0, 'Procedure Excution Completed', 0, ''
END