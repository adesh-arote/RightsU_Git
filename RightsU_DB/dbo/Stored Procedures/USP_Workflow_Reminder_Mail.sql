...
BEGIN
	...
	if(@Loglevel< 2)Exec [USPLogSQLSteps] '[USP_Workflow_Reminder_Mail]', 'Step 1', 0, 'Started Procedure', 0, ''
		SET NOCOUNT ON;
		...
		SET @EmailFooter = '&nbsp;</br>&nbsp;</br><FONT FACE=''verdana'' SIZE=''2'' COLOR=''Black''>
			...
			SELECT @Email_Config_Code= Email_Config_Code FROM Email_Config (NOLOCK) WHERE [Key]='ARE'
		...
		SELECT DISTINCT MWD.Record_Code, ISNULL(DEE.Users_Email_id,'')  +
			STUFF(( SELECT DISTINCT  ';' + U.Email_Id FROM Users U (NOLOCK)
			INNER JOIN Users_Business_Unit ubu (NOLOCK) on ubu.Users_Code = u.Users_Code and ubu.Business_Unit_Code = bu.Business_Unit_Code
		...
		INNER JOIN Module_Status_History MSH (NOLOCK) ON MSH.Record_Code = MWD.Record_Code AND MSH.Module_Code = MWD.Module_Code AND MSH.Module_Code = 30
		INNER JOIN Acq_Deal AD (NOLOCK) ON MSH.Record_Code=AD.Acq_Deal_Code
		INNER JOIN Workflow_Module WM  (NOLOCK) ON WM.Workflow_Code=AD.Work_Flow_Code AND WM.Module_Code=MSH.Module_Code AND WM.Business_Unit_Code=AD.Business_Unit_Code 
		...
		INNER JOIN Business_Unit BU (NOLOCK) ON BU.Business_Unit_Code=AD.Business_Unit_Code AND BU.Is_Active='Y'
		INNER JOIN Workflow_Module_Role WMR (NOLOCK) ON WMR.Workflow_Module_Code=WM.Workflow_Module_Code AND WMR.Group_Code=MWD.Group_Code AND WMR.Group_Level=MWD.Role_Level
		LEFT JOIN #TmpEmail_Config DEE ON DEE.BUCode= AD.Business_Unit_Code
		WHERE  BU.Business_Unit_Code in (SELECT BUCode FROM @ENL) AND  MWD.Module_Workflow_Detail_Code IN(  
		  SELECT MIN(Module_Workflow_Detail_Code) FROM Module_Workflow_Detail  (NOLOCK) 
		  WHERE Is_Done='N' and Module_Code = 30  
		  GROUP BY Record_Code,Module_Code,Is_Done  
		 ) AND MSH.Module_Status_Code IN (  
		  SELECT MAX(Module_Status_Code) FROM Module_Status_History (NOLOCK) WHERE Module_Code = 30 AND Record_Code IN (  
		   SELECT Acq_Deal_Code FROM Acq_Deal (NOLOCK) WHERE Deal_Workflow_Status = 'W'  
		  )  
		...
		INTO #TempS
		FROM Module_Workflow_Detail MWD (NOLOCK)
		INNER JOIN Module_Status_History MSH (NOLOCK) ON MSH.Record_Code=MWD.Record_Code AND MSH.Module_Code = MWD.Module_Code AND MSH.Module_Code=35
		INNER JOIN Syn_Deal SD (NOLOCK) ON MSH.Record_Code=SD.Syn_Deal_Code
		INNER JOIN Workflow_Module WM (NOLOCK) ON WM.Workflow_Code=SD.Work_Flow_Code AND WM.Module_Code=MSH.Module_Code AND WM.Business_Unit_Code=SD.Business_Unit_Code
										 AND (MSH.Status_Changed_On>=WM.Effective_Start_Date AND MSH.Status_Changed_On<=ISNULL(WM.System_End_Date,CONVERT(datetime,'31 Dec 9999')))  
		INNER JOIN Business_Unit BU (NOLOCK) ON BU.Business_Unit_Code=SD.Business_Unit_Code AND BU.Is_Active='Y'
		INNER JOIN Workflow_Module_Role WMR (NOLOCK) ON WMR.Workflow_Module_Code=WM.Workflow_Module_Code AND WMR.Group_Code=MWD.Group_Code AND WMR.Group_Level=MWD.Role_Level
	  ...
	  SELECT MAX(Module_Status_Code) FROM Module_Status_History (NOLOCK) WHERE Module_Code = 35 AND Record_Code IN (  
	   SELECT Syn_Deal_Code FROM Syn_Deal (NOLOCK) WHERE Deal_Workflow_Status = 'W'  
	  )  
	  GROUP BY Record_Code  
	 ) --AND MSH.Status = 'W')  
	 AND (MSH.Status_Changed_On <= getdate())  
	 --AND DEE.Alert_Type='W'  
			...
			--SET @EmailBody2=''
				...
				BEGIN
					...
					BEGIN
						...
						(SELECT Vendor_Name from Vendor (NOLOCK) where Vendor_Code=
						...
						REVERSE(
							STUFF(REVERSE(STUFF((
								...
								INNER JOIN Acq_Deal AD1 (NOLOCK) ON AD1.Acq_Deal_Code=ADR.Acq_Deal_Code
								INNER JOIN Deal_Type DT (NOLOCK) ON DT.Deal_Type_Code=AD1.Deal_Type_Code
						...
						FROM #TempA t
						INNER JOIN Acq_Deal AD (NOLOCK) ON AD.Acq_Deal_Code=t.Record_Code
						WHERE t.Email_Id=@Email_Id AND t.Business_Unit_Name=@Bu_Name
		...
		BEGIN
				...
				INNER JOIN Syn_Deal SD (NOLOCK) ON SD.Syn_Deal_Code=t.Record_Code
				WHERE t.Email_Id=@Email_Id AND t.Business_Unit_Name=@Bu_Name)
					BEGIN
						SET @EmailBody1 = @EmailBody1 + '<tr><td colspan=''6'' style=''padding: 8px 0px 3px 0px;''>
											<b>Business Unit:'+@Bu_Name+' </b></td></tr>'
				...
				REVERSE(
					STUFF(REVERSE(STUFF((
						...
						INNER JOIN Syn_Deal SD1 (NOLOCK) ON SD1.Syn_Deal_Code=SDR.Syn_Deal_Code
						INNER JOIN Deal_Type DT (NOLOCK) ON DT.Deal_Type_Code=SD1.Deal_Type_Code
	...
	if(@Loglevel< 2)Exec [USPLogSQLSteps] '[USP_Workflow_Reminder_Mail]', 'Step 2', 0, 'Procedure Excution Completed', 0, ''
END
