...
BEGIN
	...
	if(@Loglevel < 2)Exec [USPLogSQLSteps] '[USP_Validate_Run]', 'Step 1', 0, 'Started Procedure', 0, ''			
		...
		BEGIN
			...
			INNER JOIN Acq_Deal_Run ADRun (NOLOCK)  ON ADM.Acq_Deal_Code = ADRun.Acq_Deal_Code AND 
				ADRun.Acq_Deal_Run_Code NOT IN (@ACQ_DEAL_RUN_CODE) AND ADM.Acq_Deal_Code IN (@ACQ_DEAL_CODE)
			INNER JOIN Acq_Deal_Run_Title ADRunT  (NOLOCK) ON ADRunT.Acq_Deal_Run_Code = ADRun.Acq_Deal_Run_Code AND
			...
			INNER JOIN Acq_Deal_Run_Channel ADRunC (NOLOCK)  ON ADRunC.Acq_Deal_Run_Code = ADRun.Acq_Deal_Run_Code AND
				ADRunC.Channel_Code IN (SELECT number FROM dbo.fn_Split_withdelemiter(@CHANNEL_CODES, ','))
			INNER JOIN Channel C (NOLOCK)  ON C.Channel_Code = ADRunC.Channel_Code
		...
		BEGIN
			...
			INNER JOIN Acq_Deal_Rights_Title ADRT (NOLOCK)  ON ADRT.Acq_Deal_Rights_Code = ADR.Acq_Deal_Rights_Code 
				AND ADRT.Episode_From = ADM.Episode_Starts_From AND ADRT.Episode_To = ADM.Episode_End_To AND ADRT.Title_Code = ADM.Title_Code
			LEFT JOIN Acq_Deal_Run_Channel ADRunC (NOLOCK)  ON ADRunC.Acq_Deal_Run_Code = ADRun.Acq_Deal_Run_Code
			LEFT JOIN Channel C (NOLOCK)  ON C.Channel_Code = ADRunC.Channel_Code
		END
		...
		FROM #TempData TD
		INNER JOIN Title T (NOLOCK)  ON T.Title_Code = TD.Title_Code

		--DROP TABLE #TempData

		IF OBJECT_ID('tempdb..#TempData') IS NOT NULL DROP TABLE #TempData
	 
	if(@Loglevel < 2)Exec [USPLogSQLSteps] '[USP_Validate_Run]', 'Step 3', 0, 'Procedure Excution Completed', 0, ''	
END