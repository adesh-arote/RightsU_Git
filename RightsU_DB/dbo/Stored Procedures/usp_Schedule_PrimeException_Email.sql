...
BEGIN
	...
	if(@Loglevel<2)Exec [USPLogSQLSteps] '[usp_Schedule_PrimeException_Email]', 'Step 1', 0, 'Started Procedure', 0, ''
		...
		BEGIN TRY
			...
			BEGIN
				...
				SElect @TitleName = T.Title_Name  from Acq_Deal_Movie ADM  (NOLOCK)
				inner join Title T (NOLOCK) on T.Title_Code = ADM.Acq_Deal_Movie_Code
				where ADm.Acq_Deal_Movie_Code = @DEAL_MOVIE_CODE
		
		
				select @ChannelName = Channel_Name from Channel (NOLOCK) where Channel_Code = @ChannelName
		
		
				DECLARE @Exception_Cnt AS INT;	SET @Exception_Cnt = 0
				SELECT @Exception_Cnt = COUNT(*) FROM Email_Notification_Schedule (NOLOCK) WHERE File_Code = @File_Code AND ISNULL(IsMailSent,'N') = 'N' AND isnull(IsPrimeException,'N') = 'Y'
				Declare @DeletedCnt Int=0
				Select @DeletedCnt = COUNT(*) From Temp_BV_Schedule_DeletedRecords (NOLOCK) Where File_Code = @File_Code AND ISNULL(IsMailSent,'N') = 'N'
				...
				BEGIN
					...
					SELECT @AiringChannelName = channel_name FROM Channel (NOLOCK) WHERE channel_code in
					(
						select top 1 ChannelCode from Upload_Files (NOLOCK) WHERE File_Code = @File_Code
					)
					...
					BEGIN					
							...
							FROM Email_Notification_Schedule e (NOLOCK) WHERE e.File_Code = @File_Code AND ISNULL(IsMailSent,'N') = 'N'
					END
	...
	if(@Loglevel<2)Exec [USPLogSQLSteps] '[usp_Schedule_PrimeException_Email]', 'Step 2', 0, 'Procedure Excuting Completed', 0, ''
END