CREATE Proc [dbo].[USP_Get_Mapping_Countries]
(
	@Acq_Deal_Right_Code Int
)
As
Begin
	...
	if(@Loglevel < 2)Exec [USPLogSQLSteps] '[USP_Get_Mapping_Countries]', 'Step 1', 0, 'Started Procedure', 0, '' 
		...
		(
			...
			WHERE Syn_Deal_Rights_Code In(
				Select Syn_Deal_Rights_Code From Syn_Acq_Mapping (NOLOCK) Where Deal_Rights_Code = @Acq_Deal_Right_Code
			)
		) AS tbl
		...
		If((Select Is_Theatrical_Right From Acq_Deal_Rights (NOLOCK) Where Acq_Deal_Rights_Code = @Acq_Deal_Right_Code) = 'N')
		Begin
	...
	if(@Loglevel < 2)Exec [USPLogSQLSteps] '[USP_Get_Mapping_Countries]', 'Step 2', 0, 'Procedure Excution Completed', 0, '' 
End