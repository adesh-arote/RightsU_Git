...
BEGIN
	...
	if(@Loglevel < 2)Exec [USPLogSQLSteps] '[USP_Validate_Platform_Run_UDT]', 'Step 1', 0, 'Started Procedure', 0, ''
		SET NOCOUNT ON
		SET FMTONLY OFF
		...
		BEGIN
			...
			BEGIN
				...
				SELECT DISTINCT RunT.Title_Code, RunT.Episode_From, RunT.Episode_To, 'R' FROM  Acq_Deal_Run Run (NOLOCK)
				INNER JOIN Acq_Deal_Run_Title RunT (NOLOCK) ON RunT.Acq_Deal_Run_Code = Run.Acq_Deal_Run_Code AND Run.Acq_Deal_Code = @Deal_Code
				INNER JOIN Acq_Deal_Rights_Title RightT (NOLOCK) ON RightT.Acq_Deal_Rights_Code = @Deal_Rights_Code AND RightT.Title_Code = RunT.Title_Code
					AND RightT.Episode_From = RunT.Episode_From AND RightT.Episode_To = RunT.Episode_To

				INSERT INTO #Deal_Rights_Title_New(Title_Code, Episode_From, Episode_To, Title_For)
				SELECT DISTINCT Budget.Title_Code, Budget.Episode_From, Budget.Episode_To, 'B' FROM  Acq_Deal_Budget Budget (NOLOCK)
				INNER JOIN Acq_Deal_Rights_Title RightT (NOLOCK) ON RightT.Acq_Deal_Rights_Code = @Deal_Rights_Code AND RightT.Title_Code = Budget.Title_Code
			...
			BEGIN
				INSERT INTO #Deal_Rights_Platform_New (Platform_Code)
				SELECT DISTINCT Platform_Code FROM Acq_Deal_Rights_Platform (NOLOCK) WHERE Acq_Deal_Rights_Code = @Deal_Rights_Code
			...
			BEGIN
				...
				SELECT DISTINCT ADRT.Title_Code, ADRT.Episode_From, ADRT.Episode_To, ADRP.Platform_Code, Title_For FROM Acq_Deal_Rights ADR (NOLOCK)
				...
				INNER JOIN Platform P (NOLOCK) ON ADRP.Platform_Code = p.Platform_Code AND P.Is_No_Of_Run = 'Y'
				...
				LEFT JOIN Platform P (NOLOCK) ON DRP.Platform_Code = P.Platform_Code AND P.Is_No_Of_Run = 'Y'
	...
	if(@Loglevel < 2)Exec [USPLogSQLSteps] '[USP_Validate_Platform_Run_UDT]', 'Step 2', 0, 'Procedure Excuting Completed', 0, ''
END