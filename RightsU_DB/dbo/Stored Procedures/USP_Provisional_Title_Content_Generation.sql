CREATE PROC [dbo].[USP_Provisional_Title_Content_Generation]
(
--DECLARE
	@Pro_Deal_Code INT,
	@User_Code INT = 143
...
BEGIN
	...
	if(@Loglevel <2)Exec [USPLogSQLSteps] '[USP_Provisional_Title_Content_Generation]', 'Step 1', 0, 'Started Procedure', 0, ''
		SET NOCOUNT ON
		SET FMTONLY OFF
		...
		BEGIN TRY
			...
			SELECT TOP 1 @Selected_Deal_Type_Code = Deal_Type_Code, @Deal_Workflow_Status = Deal_Workflow_Status FROM Provisional_Deal  (NOLOCK)
			...
			INTO #TempProDealTitle FROM Provisional_Deal_Title PDT  (NOLOCK)
			...
			BEGIN
				...
				IF EXISTS (SELECT * FROM Provisional_Deal (NOLOCK) WHERE Provisional_Deal_Code = @Pro_Deal_Code AND LTRIM(RTRIM(Deal_Workflow_Status)) = 'A')
				BEGIN
				...
				BEGIN
					...
					(
						SELECT  MAX(TC.Episode_No) AS MaxEpisode_Id FROM Title_Content TC  (NOLOCK)
					...
					BEGIN
						...
						IF NOT EXISTS(SELECT Title_Content_Code FROM Title_Content (NOLOCK) WHERE Title_Code = @TitleCode AND Episode_No = @Episode_No)
						BEGIN
							...
							SELECT Title_Content_Code, @DefaultVersionCode, Duration FROM Title_Content (NOLOCK) WHERE Title_Code = @TitleCode AND Episode_No = @Episode_No
						...
						SELECT TOP 1 @Title_Content_Code = Title_Content_Code FROM Title_Content (NOLOCK) WHERE Title_Code = @TitleCode AND Episode_No = @Episode_No

						PRINT '  Episode No : ' + CAST(@Episode_No AS VARCHAR)
						PRINT '  Title_Content_Code : ' + CAST(@Title_Content_Code AS VARCHAR)
						IF NOT EXISTS (
							SELECT * FROM Title_Content_Mapping (NOLOCK) 
					...
					WHERE ISNULL(TC.Ref_BMS_Content_Code, '') = '' AND TC.Title_Content_Code NOT IN (
						SELECT DISTINCT Title_Content_Code FROM Title_Content_Mapping (NOLOCK)
					) 
					...
					Select Distinct Title_Content_Code From Title_Content (NOLOCK) Where ISNULL(Ref_BMS_Content_Code, '') = '' AND Title_Content_Code NOT IN (
						SELECT DISTINCT Title_Content_Code FROM Title_Content_Mapping  (NOLOCK)
					) AND Title_Content_Code NOT IN (
						SELECT DISTINCT Title_Content_Code FROM Content_Music_Link (NOLOCK)
	...
	if(@Loglevel <2)Exec [USPLogSQLSteps] '[USP_Provisional_Title_Content_Generation]', 'Step 2', 0, 'Procedure Excuting Completed', 0, ''
END