...
BEGIN
	...
	BEGIN
		...
		,'' as [CHANNEL_NAME]
		FROM ACQ_DEAL_RUN ADRR
		INNER JOIN Acq_Deal_Movie ADM ON ADM.Acq_Deal_Code = @ACQ_DEAL_CODE
		INNER JOIN ACQ_DEAL_RUN_YEARWISE_RUN  ADYR ON ADYR.ACQ_DEAL_RUN_CODE = ADRR.ACQ_DEAL_RUN_CODE
		INNER JOIN  ACQ_DEAL_RIGHTS ADR ON ADR.ACQ_DEAL_CODE = ADRR.ACQ_DEAL_CODE
		INNER JOIN  ACQ_DEAL_RIGHTS_TITLE ADRT ON ADRT.ACQ_DEAL_RIGHTS_CODE = ADR.ACQ_DEAL_RIGHTS_CODE AND
		ADRT.Episode_From = ADM.Episode_Starts_From AND ADRT.Episode_To = ADM.Episode_End_To AND ADRT.Title_Code = ADM.Title_Code
		INNER JOIN Acq_Deal_Run_Title ADRNT ON ADRT.Episode_From = ADRNT.Episode_From AND ADRT.Episode_To = ADRNT.Episode_To AND ADRT.Title_Code = ADRNT.Title_Code AND ADRR.Acq_Deal_Run_Code = ADRNT.Acq_Deal_Run_Code
...
select * from ACQ_DEAL_RUN where ACQ_DEAL_CODE in (20)

 EXEC [DBO].[USP_VALIDATE_YEARWISE_RIGHT_FOR_RUN] 19,'1889','2014-10-22 00:00:00.000','2014-10-22 00:00:00.000'
 EXEC [DBO].[USP_VALIDATE_YEARWISE_RIGHT_FOR_RUN] 20,'1874','10/1/2014 00:00:00.000','10/1/2014 00:00:00.000'
