...
CREATE PROC [dbo].[USP_AutoPushAcqDeal]
(
	@SynDealCode INT, 
	@UserCode INT,
...
BEGIN
	DECLARE 
	--@SynDealCode INT = 1522, 
	--@UserCode INT = 1284,
	--@StatusFlag VARCHAR(1)= '',
	--@ErrMessage NVARCHAR(MAX) = '',
	@NewAcqDealCode VARCHAR(10), @ExistsCounter INT = 0
	DECLARE @AutopushAcqDeal CHAR(1) = 'N', @BusinessUnitCode_P INT = 0, @Message NVARCHAR(MAX), @AutoPushLicensor VARCHAR(10), @IntrBuCode VARCHAR(10), @NewAcqDealRightsCode INT, @NewAcqDealMovieCode INT, @SynDealMovieCode INT ,@VersionNo INT, @ATSynDealCode INT,@DealPushUser VARCHAR(10)
	SELECT @StatusFlag = '', @ErrMessage = '',@DealPushUser = 0

	SELECT @DealPushUser = Parameter_Value FROM System_Parameter_New Where Parameter_Name = 'DealPushUser'
	...
	BEGIN TRY
		...
		BEGIN
			...
			BEGIN /* Get Mapped Promoter Group Code */
				PRINT 'Get Mapped Group'

			IF(OBJECT_ID('TEMPDB..#AcqDealPromoterCodeVMP') IS NOT NULL)
			DROP TABLE #AcqDealPromoterCodeVMP
			CREATE TABLE #AcqDealPromoterCodeVMP(Acq_Deal_Rights_Code INT, Syn_Deal_Rights_Code INT, Promoter_Group_Code INT)
			INSERT INTO #AcqDealPromoterCodeVMP( Acq_Deal_Rights_Code,Syn_Deal_Rights_Code,Promoter_Group_Code)
			SELECT DISTINCT SAM.Deal_Rights_Code, SAM.Syn_Deal_Rights_Code, ADRPG.Promoter_Group_Code FROM Syn_Acq_Mapping SAM
			INNER JOIN Acq_Deal_Rights_Promoter ADRP ON ADRP.Acq_Deal_Rights_Code = SAM.Deal_Rights_Code
			...
			WHERE Syn_Deal_Code = @SynDealCode 

				IF(OBJECT_ID('TEMPDB..#CurrentPromoterGroupCode') IS NOT NULL)
					DROP TABLE #CurrentPromoterGroupCode

				CREATE TABLE #CurrentPromoterGroupCode
				(
					Syn_Deal_Rights_Code INT,
					Promoter_Group_Code	INT,
					NewPromoterGroupCode INT DEFAULT(0)
				)
				INSERT INTO #CurrentPromoterGroupCode(Syn_Deal_Rights_Code,Promoter_Group_Code)
				SELECT Syn_Deal_Rights_Code,  Promoter_Group_Code FROM #AcqDealPromoterCodeVMP 
				WHERE  Promoter_Group_Code NOT IN
				(
					SELECT  DISTINCT  SDRPG.Promoter_Group_Code FROM Syn_Deal_Rights_Promoter_Group SDRPG
					INNER JOIN Syn_Deal_Rights_Promoter SDRP ON SDRP.Syn_Deal_Rights_Promoter_Code = SDRPG.Syn_Deal_Rights_Promoter_Code
					INNER JOIN Syn_Deal_Rights SDR ON SDR.Syn_Deal_Rights_Code = SDRP.Syn_Deal_Rights_Code
					--INNER JOIN #AcqDealPromoterCodeVMP ADPCV ON ADPCV.Promoter_Group_Code = SDRPG.Promoter_Group_Code 
					WHERE SDR.Syn_Deal_Code = @SynDealCode
				)     

				UPDATE CPGR SET CPGR.NewPromoterGroupCode = ISNULL(MD.SecondaryDataCode, 0) FROM #CurrentPromoterGroupCode CPGR
				INNER JOIN AcqPreReqMappingData MD ON MD.MappingFor = 'PROM' AND CPGR.Promoter_Group_Code = MD.PrimaryDataCode
				
				IF EXISTS(SELECT TOP 1 * FROM #CurrentPromoterGroupCode WHERE NewPromoterGroupCode = 0)
				BEGIN
					PRINT 'Found some unmapped Platform'
				END
			...
			BEGIN --------------- DEAL MOVE PROCESS TO DESTINATION DATABASE
				...
				FROM Syn_Deal Where Syn_Deal_Code = @SynDealCode
					...
					,@Last_Updated_Time ,@UserCode ,NULL,@RoleCode_Deal ,NULL ,'Y', NULL, NULL,NULL
				...
				BEGIN
					...
					UPDATE RightsU_Broadcast.dbo.Acq_Deal SET
						...
						Acq_Deal.Category_Code = ISNULL(@CategoryCode_S,0),
				...
				BEGIN
					...
					BEGIN
						...
						BEGIN
							...
							DELETE FROM RightsU_Broadcast.dbo.Acq_Deal_Rights_Promoter_Group WHERE Acq_Deal_Rights_Promoter_Code IN (
							Select Acq_Deal_Rights_Promoter_Code From RightsU_Broadcast.dbo.Acq_Deal_Rights_Promoter where Acq_Deal_Rights_Code = @DestAcqDealRightsCode)

							DELETE FROM RightsU_Broadcast.dbo.Acq_Deal_Rights_Promoter_Remarks WHERE Acq_Deal_Rights_Promoter_Code IN (
							Select Acq_Deal_Rights_Promoter_Code From RightsU_Broadcast.dbo.Acq_Deal_Rights_Promoter where Acq_Deal_Rights_Code = @DestAcqDealRightsCode)

							DELETE FROM RightsU_Broadcast.dbo.Acq_Deal_Rights_Promoter WHERE Acq_Deal_Rights_Code = @DestAcqDealRightsCode
						END
					...
					BEGIN
						...
						BEGIN
							...
							BEGIN
								...
								/**************Insertion of self utilization**************/
									IF EXISTS(SELECT TOP 1 * FROM #CurrentPromoterGroupCode WHERE Promoter_Group_Code > 0)
									BEGIN
										PRINT 'Insert data in ''Acq_Deal_Rights_Promoter'' table'
										INSERT INTO RightsU_Broadcast.dbo.Acq_Deal_Rights_Promoter(Acq_Deal_Rights_Code,Inserted_On,Inserted_By,Last_Updated_Time,Last_Action_By)
										SELECT DISTINCT @DestAcqDealRightsCode ,GETDATE(),@UserCode,GETDATE(),@UserCode
										FROM #CurrentPromoterGroupCode CPGC
										WHERE CPGC.Syn_Deal_Rights_Code = @SynDealRightsCode
									
										PRINT 'Insert data in ''Acq_Deal_Rights_Promoter_Group'' table'
										INSERT INTO RightsU_Broadcast.dbo.Acq_Deal_Rights_Promoter_Group(Acq_Deal_Rights_Promoter_Code,Promoter_Group_Code)
										SELECT DISTINCT ADRP.Acq_Deal_Rights_Promoter_Code, 
										CPGC.NewPromoterGroupCode
										FROM Syn_Deal_Rights_Promoter_Group SDRPG
										INNER JOIN Syn_Deal_Rights_Promoter SDRP ON SDRP.Syn_Deal_Rights_Promoter_Code = SDRPG.Syn_Deal_Rights_Promoter_Code 
										INNER JOIN RightsU_Broadcast.dbo.Acq_Deal_Rights_Promoter ADRP ON ADRP.Acq_Deal_Rights_Code  = @DestAcqDealRightsCode 
										INNER JOIN #AcqDealPromoterCodeVMP ADPCV ON ADPCV.Syn_Deal_Rights_Code = @SynDealRightsCode
										INNER JOIN #CurrentPromoterGroupCode CPGC ON CPGC.Promoter_Group_Code = ADPCV.Promoter_Group_Code
										WHERE 
										ISNULL(CPGC.NewPromoterGroupCode, 0) > 0 
										--And SDRP.Syn_Deal_Rights_Code = @SynDealRightsCode
										
										PRINT 'Insert data in ''Acq_Deal_Rights_Promoter_Remarks'' table'
										INSERT INTO RightsU_Broadcast.dbo.Acq_Deal_Rights_Promoter_Remarks(Acq_Deal_Rights_Promoter_Code,Promoter_Remarks_Code)
										SELECT DISTINCT ADRP.Acq_Deal_Rights_Promoter_Code, 
										SDRPR.Promoter_Remarks_Code
										FROM Syn_Deal_Rights_Promoter_Remarks SDRPR 
										INNER JOIN Syn_Deal_Rights_Promoter 
										SDRP ON SDRP.Syn_Deal_Rights_Promoter_Code = SDRPR.Syn_Deal_Rights_Promoter_Code 
										INNER JOIN RightsU_Broadcast.dbo.Acq_Deal_Rights_Promoter ADRP ON ADRP.Acq_Deal_Rights_Code  = @DestAcqDealRightsCode 
										INNER JOIN Promoter_Remarks PR ON PR.Promoter_Remarks_Code = SDRPR.Promoter_Remarks_Code
										--Where SDRP.Syn_Deal_Rights_Code = @SynDealRightsCode

									
										PRINT '    Insert / update Program and map with current Promoter Remarks in ''Promoter_Remarks'' table'
										IF(OBJECT_ID('TEMPDB..#Promoter_Remarks') IS NOT NULL)
										DROP TABLE #Promoter_Remarks

										CREATE TABLE #Promoter_Remarks
										(
											Promoter_Remarks_Code INT, 
											New_Promoter_remarks_Code INT, 
										...
										DELETE FROM #Promoter_Remarks
										INSERT INTO #Promoter_Remarks(Promoter_Remarks_Code, Promoter_Remark_Desc, Inserted_on, Inserted_By, Last_Updated_Time, Last_Action_By, Is_Active)
										SELECT PR.Promoter_Remarks_Code,PR.Promoter_Remark_Desc, PR.Inserted_On, @UserCode, GETDATE(),@UserCode, PR.Is_Active FROM Syn_Deal_Rights SDR
										INNER JOIN Syn_Deal_Rights_Promoter SDRP ON SDRP.Syn_Deal_Rights_Code = SDR.Syn_Deal_Rights_Code
										...
										INNER JOIN Promoter_Remarks PR ON PR.Promoter_Remarks_Code = SDRPR.Promoter_Remarks_Code
										WHERE SDR.Syn_Deal_Rights_Code = @SynDealRightsCode
										

										UPDATE PR SET PR.New_Promoter_remarks_Code = ISNULL(MD.SecondaryDataCode, 0) FROM #Promoter_Remarks PR
										INNER JOIN AcqPreReqMappingData MD ON MD.MappingFor = 'PRMK' AND PR.Promoter_Remarks_Code = MD.PrimaryDataCode

										--UPDATE PR_P SET PR_P.New_Promoter_remarks_Code = PR_S.Promoter_Remarks_Code  FROM #Promoter_Remarks PR_P
										--INNER JOIN RightsU_Broadcast.dbo.Promoter_Remarks PR_S ON PR_S.Promoter_Remarks_Code = PR_P.New_Promoter_remarks_Code --AND PR_S.Title_Code = @NewTitleCode
										--INNER JOIN RightsU_Broadcast.dbo.Acq_Deal_Rights_Promoter_Remarks ADRPR ON ADRPR.Promoter_Remarks_Code = PR_S.Promoter_Remarks_Code
										--INNER JOIN RightsU_Broadcast.dbo.Acq_Deal_Rights_Promoter ADRP ON ADRP.Acq_Deal_Rights_Promoter_Code = ADRPR.Acq_Deal_Rights_Promoter_Code AND 
										--ADRP.Acq_Deal_Rights_Code = @DestAcqDealRightsCode
										--WHERE PR_P.New_Promoter_remarks_Code > 0
										...
										INSERT INTO RightsU_Broadcast.dbo.Promoter_Remarks(Promoter_Remark_Desc, Inserted_On, Inserted_By,  Last_Updated_Time, Last_Action_By,Is_Active)
										SELECT RTRIM(LTRIM(PR.Promoter_Remark_Desc)),GETDATE(),@UserCode, GETDATE(), @UserCode, 'Y'
										FROM #Promoter_Remarks PR WHERE ISNULL(PR.New_Promoter_remarks_Code, 0) = 0 
										AND NOT EXISTS (
												SELECT TOP 1 Promoter_Remarks_Code FROM RightsU_Broadcast.dbo.Promoter_Remarks PRV18 
												WHERE RTRIM(LTRIM(PRV18.Promoter_Remark_Desc)) COLLATE SQL_Latin1_General_CP1_CI_AS = RTRIM(LTRIM(PR.Promoter_Remark_Desc)) COLLATE SQL_Latin1_General_CP1_CI_AS
										)

										UPDATE PR SET PR.New_Promoter_remarks_Code = PRV18.Promoter_Remarks_Code  FROM #Promoter_Remarks PR
										INNER JOIN RightsU_Broadcast.dbo.Promoter_Remarks PRV18 ON 
											RTRIM(LTRIM(PRV18.Promoter_Remark_Desc)) COLLATE SQL_Latin1_General_CP1_CI_AS = RTRIM(LTRIM(PR.Promoter_Remark_Desc)) COLLATE SQL_Latin1_General_CP1_CI_AS
										WHERE ISNULL(PR.New_Promoter_remarks_Code, 0) = 0 AND NOT EXISTS (
											SELECT TOP 1 * FROM AcqPreReqMappingData WHERE SecondaryDataCode = PRV18.Promoter_Remarks_Code AND MappingFor = 'PRMK'
										)				

										PRINT '----------------Update New Promoter Remarks In Destination ''Acq_Deal_Rights_Promoter_Remarks'' Table'
										UPDATE ADRPR SET ADRPR.Promoter_Remarks_Code = PR.New_Promoter_remarks_Code
										from RightsU_Broadcast.dbo.Acq_Deal_Rights_Promoter_Remarks ADRPR
										INNER JOIN RightsU_Broadcast.dbo.Acq_Deal_Rights_Promoter ADRP On ADRP.Acq_Deal_Rights_Promoter_Code = ADRPR.Acq_Deal_Rights_Promoter_Code
										INNER JOIN #Promoter_Remarks PR ON PR.Promoter_Remarks_Code = ADRPR.Promoter_Remarks_Code 
										WHERE ADRP.Acq_Deal_Rights_Code = @DestAcqDealRightsCode AND ADRPR.Promoter_Remarks_Code IN (Select Promoter_Remarks_Code FROm #Promoter_Remarks) 

										--SELECT ADRPR.Promoter_Remarks_Code  
										----(SELECT TOP 1 New_Promoter_remarks_Code From #Promoter_Remarks)
										--from RightsU_Broadcast.dbo.Acq_Deal_Rights_Promoter_Remarks ADRPR
										--INNER JOIN RightsU_Broadcast.dbo.Acq_Deal_Rights_Promoter ADRP On ADRP.Acq_Deal_Rights_Promoter_Code = ADRPR.Acq_Deal_Rights_Promoter_Code
										----INNER JOIN #Promoter_Remarks PR ON PR.Promoter_Remarks_Code = ADRPR.Promoter_Remarks_Code 
										--WHERE ADRP.Acq_Deal_Rights_Code = 12960  AND ADRPR.Promoter_Remarks_Code IN (Select Promoter_Remarks_Code FROm #Promoter_Remarks) 

										--Select * from #Promoter_Remarks

										INSERT INTO AcqPreReqMappingData(MappingFor, PrimaryDataCode, SecondaryDataCode)
										SELECT 'PRMK', PR.Promoter_Remarks_Code, PR.New_Promoter_remarks_Code FROM #Promoter_Remarks PR
										WHERE NOT EXISTS (
											SELECT TOP 1 * FROM AcqPreReqMappingData 
											WHERE PrimaryDataCode = PR.Promoter_Remarks_Code AND SecondaryDataCode = PR.New_Promoter_remarks_Code AND MappingFor = 'PRMK'
										)
									
									END
									ELSE
									BEGIN
										PRINT 'Promoter Group Not Inserted'
									END

							
								/**************END Insertion of self utilization**************/
							END
							ELSE
							BEGIN
					...
					INSERT INTO RightsU_Broadcast.dbo.Module_Status_History(
						Module_Code, Record_Code, [Status], Status_Changed_By, Status_Changed_On, Remarks, Version_No
					)VALUES(
						30, @NewAcqDealCode, 'AP', CAST(@DealPushUser AS INT), GETDATE(), 'Auto pushed deal from ''' + @dbName + ''' database, Ref Syn Agreement No is ''' + @AgreementNo +'''',@VersionNo
					)
				END
				END
				ELSE
