...
BEGIN
	...
	if(@Loglevel  < 2)Exec [USPLogSQLSteps] '[USP_Mass_Territory_Update_Schedule]', 'Step 1', 0, 'Started Procedure', 0, '' 
		...
		inner join Acq_Deal_Rights adr (NOLOCK) on adr.Acq_Deal_Code=admtu.Acq_Deal_Code
		...
		BEGIN
			...
			BEGIN
				...
				from 
					Acq_Deal_Rights_Territory adrt  (NOLOCK)
					inner join Acq_Deal_Rights adr (NOLOCK) on adr.Acq_Deal_Rights_Code=adrt.Acq_Deal_Rights_Code
					inner join Acq_Deal_Mass_Territory_Update_Details admtud (NOLOCK) on admtud.Territory_Code=adrt.Territory_Code
				where 
					adr.Acq_Deal_Rights_Code=@Acq_Deal_Rights_Code and admtud.Acq_Deal_Mass_Update_Code=@Acq_Deal_Mass_Update_Code
			
				select @cntCountry=COUNT(*) from  Acq_Deal_Mass_Territory_Update_Details admtud (NOLOCK)
				inner join Territory_Details td (NOLOCK) on td.Territory_Code=admtud.Territory_Code
				inner join 
				(
				...
				(
					select adrt.Country_Code from Acq_Deal_Rights_Territory adrt (NOLOCK)
					inner join Acq_Deal_Mass_Territory_Update_Details admtud (NOLOCK) on admtud.Territory_Code=adrt.Territory_Code
					inner join Acq_Deal_Mass_Territory_Update admtu (NOLOCK) on admtu.Acq_Deal_Mass_Update_Code=admtud.Acq_Deal_Mass_Update_Code
				...
				begin
					 ...
					 print @cntDealRun
						...
						select @Acq_Deal_Rights_Code,'G',td.Country_Code,admtud.Territory_Code from Acq_Deal_Mass_Territory_Update_Details admtud (NOLOCK)
						inner join Territory_Details td (NOLOCK) on td.Territory_Code=admtud.Territory_Code
						inner join 
						(
							select  distinct adrtt.Acq_Deal_Rights_Code,adrtt.Territory_Code 
						...
						begin
						   ...
						   select @isDone=count(*) from Acq_Deal_Mass_Territory_Update (NOLOCK) where Acq_Deal_Mass_Update_Code=@Acq_Deal_Mass_Update_Code 
						   and Can_Process='D'
						   ...
						   begin
							   ...
							   select * from Module_Status_History (NOLOCK)
		...
		inner join Syn_Deal_Rights adr (NOLOCK) on adr.Syn_Deal_Code=admtu.Syn_Deal_Code
		...
		BEGIN
			...
			BEGIN
				...
				from 
					Syn_Deal_Rights_Territory adrt (NOLOCK)
					inner join Syn_Deal_Rights adr (NOLOCK) on adr.Syn_Deal_Rights_Code=adrt.Syn_Deal_Rights_Code
					inner join Syn_Deal_Mass_Territory_Update_Details admtud (NOLOCK) on admtud.Territory_Code=adrt.Territory_Code
				where 
					adr.Syn_Deal_Rights_Code=@Syn_Deal_Rights_Code and admtud.Syn_Deal_Mass_Update_Code=@Syn_Deal_Mass_Update_Code
			
				select @cntSynCountry=COUNT(*) from Syn_Deal_Mass_Territory_Update_Details admtud  (NOLOCK)
				inner join Territory_Details td (NOLOCK) on td.Territory_Code=admtud.Territory_Code
				inner join 
				(
					select  distinct adrtt.Syn_Deal_Rights_Code,adrtt.Territory_Code 
					from Syn_Deal_Rights_Territory adrtt (NOLOCK)) a 
				...
				and td.Country_Code not in 
				(
					select adrt.Country_Code from Syn_Deal_Rights_Territory adrt (NOLOCK)
					inner join Syn_Deal_Mass_Territory_Update_Details admtud (NOLOCK) on admtud.Territory_Code=adrt.Territory_Code
					inner join Syn_Deal_Mass_Territory_Update admtu (NOLOCK) on admtu.Syn_Deal_Mass_Update_Code=admtud.Syn_Deal_Mass_Update_Code
				...
				begin
					 ...
					 print @cntSynDealRun
						...
						select @Syn_Deal_Rights_Code,'G',td.Country_Code,admtud.Territory_Code from Syn_Deal_Mass_Territory_Update_Details admtud (NOLOCK)
						inner join Territory_Details td (NOLOCK) on td.Territory_Code=admtud.Territory_Code
						inner join 
						(
							select  distinct adrtt.Syn_Deal_Rights_Code,adrtt.Territory_Code 
						...
						begin
						   ...
						   select @isSynMassDone=count(*) from Syn_Deal_Mass_Territory_Update (NOLOCK) where Syn_Deal_Mass_Update_Code=@Syn_Deal_Mass_Update_Code 
						   and Can_Process='D'
	...
	if(@Loglevel  < 2)Exec [USPLogSQLSteps] '[USP_Mass_Territory_Update_Schedule]', 'Step 2', 0, 'Procedure Excution Completed', 0, '' 
END