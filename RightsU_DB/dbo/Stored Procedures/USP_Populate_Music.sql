CREATE PROCEDURE USP_Populate_Music
(
...
BEGIN
	...
	BEGIN
		PRINT 'SELECT DATA FROM MUSIC_TITLE FOR MUSIC DEAL'

		SELECT TOP 1 @Deal_Code = Acq_Deal_Code FROM Acq_Deal_Movie WHERE Acq_Deal_Movie_Code = @Acq_Deal_Movie_Code
		...
		FROM Music_Title MT 
		LEFT JOIN Music_Title_Talent MTT ON MTT.Music_Title_Code = MT.Music_Title_Code 
			AND MTT.Role_Code IN (@RoleCode_Singer, @RoleCode_MusicComposer, @RoleCode_Lyricist)
		LEFT JOIN Talent T ON T.Talent_Code = MTT.Talent_Code 
		WHERE MT.Music_Title_Code NOT IN (
			SELECT DISTINCT ADMM.Music_Title_Code FROM Acq_Deal_Movie_Music ADMM WHERE ADMM.Acq_Deal_Movie_Code IN ( 
	...
	BEGIN
		PRINT 'SELECT DATA FROM ACQ_DEAL FOR LINK SHOW'
		...
		BEGIN
			...
			(
				(
					...
					FROM Acq_Deal_Movie_Music_Link ADMML_I
	...
	ELSE
	BEGIN
		PRINT 'SELECT DATA FROM ACQ_DEAL_MOVIE_MUSIC FOR NON-MUSIC DEAL'
		...
		(
			(
				...
				FROM Music_Title_Label MTL_I 
				INNER JOIN Music_Label ML_I ON ML_I.Music_Label_Code = MTL_I.Music_Label_Code
		...
		INNER JOIN Acq_Deal_Movie ADM ON ADM.Acq_Deal_Movie_Code = ADMM.Acq_Deal_Movie_Code
		INNER JOIN Acq_Deal AD ON AD.Acq_Deal_Code = ADM.Acq_Deal_Code
		INNER JOIN Music_Title MT ON MT.Music_Title_Code = ADMM.Music_Title_Code
		INNER JOIN Title T ON T.Title_Code = ADM.Title_Code
