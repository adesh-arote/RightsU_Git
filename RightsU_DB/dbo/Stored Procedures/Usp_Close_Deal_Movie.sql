...
BEGIN
	...
	if(@Loglevel < 2)Exec [USPLogSQLSteps] '[Usp_Close_Deal_Movie]', 'Step 1', 0, 'Started Procedure', 0, ''
		...
		*/	
		...
		SELECT Acq_Deal_Movie_Code, Title_Code, Movie_Closed_Date FROM Acq_Deal_Movie (NOLOCK) WHERE Is_Closed = 'X' AND Acq_Deal_Code = @Acq_Deal_Code
		DECLARE 
		...
		FROM Acq_Deal_Rights adr (NOLOCK)
			...
			INNER JOIN Acq_Deal_Movie adm (NOLOCK) ON adm.Title_Code = adrt.Title_Code AND adm.Episode_Starts_From = adrt.Episode_From AND adm.Episode_End_To = adrt.Episode_To AND adm.Acq_Deal_Code = adr.Acq_Deal_Code
		...
		BEGIN	
			...
			BEGIN
				...
				IF((SELECT COUNT(*) FROM Acq_Deal_Rights_Title (NOLOCK) WHERE Acq_Deal_Rights_Code = @Acq_Deal_Rights_Code AND Title_Code <> @Title_Code ) > 0)
				BEGIN
				...
				DELETE FROM Acq_Deal_Run_Yearwise_Run_Week WHERE Acq_Deal_Run_Code in (SELECT ADR.Acq_Deal_Run_Code FROM Acq_Deal_Run_Title ADRT (NOLOCK)
				INNER JOIN Acq_Deal_Run ADR (NOLOCK) ON ADRT.Acq_Deal_Run_Code = ADR.Acq_Deal_Run_Code
				WHERE Title_Code = @Title_Code 
				AND ADR.Acq_Deal_Code = @Acq_Deal_Code)

				DELETE FROM Acq_Deal_Run_Channel WHERE Acq_Deal_Run_Code in (SELECT ADR.Acq_Deal_Run_Code FROM Acq_Deal_Run_Title ADRT (NOLOCK)
				INNER JOIN Acq_Deal_Run ADR (NOLOCK) ON ADRT.Acq_Deal_Run_Code = ADR.Acq_Deal_Run_Code
				WHERE Title_Code = @Title_Code 
				AND ADR.Acq_Deal_Code = @Acq_Deal_Code
				)
		
				DELETE FROM Acq_Deal_Run_Repeat_On_Day WHERE Acq_Deal_Run_Code in (SELECT ADR.Acq_Deal_Run_Code FROM Acq_Deal_Run_Title ADRT (NOLOCK)
				INNER JOIN Acq_Deal_Run ADR (NOLOCK) ON ADRT.Acq_Deal_Run_Code = ADR.Acq_Deal_Run_Code
				WHERE Title_Code = @Title_Code 
				AND ADR.Acq_Deal_Code = @Acq_Deal_Code)

				DELETE FROM Acq_Deal_Run_Yearwise_Run WHERE Acq_Deal_Run_Code in (SELECT ADR.Acq_Deal_Run_Code FROM Acq_Deal_Run_Title ADRT (NOLOCK)
				INNER JOIN Acq_Deal_Run ADR  (NOLOCK) ON ADRT.Acq_Deal_Run_Code = ADR.Acq_Deal_Run_Code
				WHERE Title_Code = @Title_Code 
				AND ADR.Acq_Deal_Code = @Acq_Deal_Code)
				...
				DELETE FROM Acq_Deal_Run WHERE Acq_Deal_Run_Code IN (SELECT Acq_Deal_Run_Code FROM Acq_Deal_Run_Title (NOLOCK) WHERE Title_Code = @Title_Code)
			...
			BEGIN
				...
				---------------------------------------Delete Run Definition for Title-------------
				UPDATE Acq_Deal_Run_Yearwise_Run SET End_Date = @Movie_Closed_Date WHERE Acq_Deal_Run_Code in (SELECT ADR.Acq_Deal_Run_Code FROM Acq_Deal_Run_Title ADRT (NOLOCK)
				...
				INNER JOIN Acq_Deal_Run ADR (NOLOCK) ON ADR.Acq_Deal_Run_Code = ADRYR.Acq_Deal_Run_Code
	...
	if(@Loglevel < 2)Exec [USPLogSQLSteps] '[Usp_Close_Deal_Movie]', 'Step 2', 0, 'Procedure Excution Completed', 0, ''
END
