...
BEGIN	
	...
	if(@Loglevel < 2)Exec [USPLogSQLSteps] '[USP_Email_Run_Utilization]', 'Step 1', 0, 'Started Procedure', 0, '' 
		--DECLARE 
		--@Call_From CHAR(1) ='',
		--@Title_Codes VARCHAR(1000) ='',
		--@BU_Code INT=2,
		...
		SELECT @Email_Config_Code = Email_Config_Code FROM Email_Config (NOLOCK) where [Key] = 'LMR'
		...
		SELECT @Deal_Run_Email_Template_Desc =REPLACE(Template_Desc ,'{Link}',@DefaultSiteUrl) FROM  Email_Template (NOLOCK) 
	...
	if(@Loglevel < 2)Exec [USPLogSQLSteps] '[USP_Email_Run_Utilization]', 'Step 2', 0, 'Procedure Excution Completed', 0, '' 
END