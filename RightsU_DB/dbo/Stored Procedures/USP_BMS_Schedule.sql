...
BEGIN   
	...
	if(@Loglevel < 2)Exec [USPLogSQLSteps] '[USP_BMS_Schedule]', 'Step 1', 0, 'Started Procedure', 0, ''
		SET NOCOUNT ON;
		IF(@Is_Reprocess = 'Y')
		BEGIN
		...
		DECLARE CR_BMS_Schedule_Process_Data_Temp CURSOR FOR 
			SELECT DISTINCT BSPDT.Channel_Code, C.Order_For_schedule FROM BMS_Schedule_Process_Data_Temp BSPDT (NOLOCK)
			INNER JOIN Channel C (NOLOCK) ON C.Channel_Code = BSPDT.Channel_Code
		...
		BEGIN
			...
			BEGIN
				BEGIN TRY
					...
					--(SELECT BT.Date_Time FROm BMS_Schedule_Process_Data_Temp BT (NOLOCK) WHERE
					...
					--	select Timeline_ID from BMS_Schedule_Process_Data_Temp BSPDT (NOLOCK)  where BSPDT.Channel_Code=@Channel_Code_CV 
					...
					(
						Select Top 1 Date_Time From BMS_Schedule_Process_Data_Temp t2 (NOLOCK) Where t2.Timeline_ID = t1.Timeline_ID And t2.Record_Status <> 'W' 
					...
					Where BSPD.BMS_Schedule_Process_Data_Temp_Code In (
						Select Max(BMS_Schedule_Process_Data_Temp_Code) From BMS_Schedule_Process_Data_Temp t2 (NOLOCK) Where t2.Timeline_ID = BSPDT.Timeline_ID 
					...
					( SELECT TOP 1 CCR.Content_Channel_Run_Code FROM Content_Channel_Run CCR (NOLOCK) WHERE CCR.Title_Content_Code = TC.Title_Content_Code 
					...
					SELECT DISTINCT TBSPD.BMS_Asset_Ref_Key, TBSPD.BMS_Schedule_Process_Data_Temp_Code, TBSPD.Timeline_ID FROM BMS_Schedule_Process_Data_Temp TBSPD  (NOLOCK)
					...
					(
						...
						(
							...
							row_number() over (partition by BMS_Asset_Ref_Key order by Timeline_ID desc ) rn from #UnMapped_Titles (NOLOCK)
						) AS a where a.rn = 1
					)
					...
					(
						SELECT BH.Program_Episode_ID FROM BV_HouseId_Data BH (NOLOCK) WHERE ISNULL(BH.Program_Episode_ID,0)  > 0
					)
				
					/*******************Insert into Exception Table (Asset id null or Empty)************/			
					IF EXISTS (SELECT TOP 1 TBSPD.BMS_Schedule_Process_Data_Temp_Code FROM BMS_Schedule_Process_Data_Temp TBSPD (NOLOCK) WHERE TBSPD.Record_Status = 'W' 
					...
					BEGIN
						...
						FROM Email_Notification_Msg (NOLOCK) WHERE UPPER(Email_Msg_For) = 'BMS_SCHEDULE_ASSET_ID_NULL' AND [Type] = 'S' 
						...
						FROM BMS_Schedule_Process_Data_Temp TBSPD  (NOLOCK)
						LEFT JOIN BMS_Schedule_Exception BSE (NOLOCK) ON TBSPD.BMS_Schedule_Process_Data_Temp_Code = BSE.BMS_Schedule_Process_Data_Temp_Code
					...
					BEGIN
						...
						SET @Email_Notification_Msg_Code = 0
						SELECT TOP 1 @Email_Notification_Msg_Code = Email_Notification_Msg_Code 
						FROM Email_Notification_Msg (NOLOCK) WHERE UPPER(Email_Msg_For) = 'PROGRAM_ASSET' AND [Type] = 'S'
						...
						LEFT JOIN BMS_Schedule_Exception BSE (NOLOCK) ON UT.BMS_Schedule_Process_Data_Temp_Code = BSE.BMS_Schedule_Process_Data_Temp_Code
					...
					BEGIN
						DROP TABLE #TEMP_BV_Schedule
					END
					SELECT DISTINCT Timeline_ID INTO #TEMP_BV_Schedule FROM BMS_Schedule_Process_Data_Temp  (NOLOCK)
					WHERE Channel_Code  = @Channel_Code_CV AND ISNULL(Record_Status,'P') = 'P'
					...
					INNER JOIN 		
					(select DISTINCT T.BMS_Asset_Ref_Key, ISNULL( (select MAX(CAST(A.Schedule_Item_Log_Time AS DATETIME)) From BV_Schedule_Transaction A  (NOLOCK)
					...
					AND A.Timeline_ID NOT IN (select Timeline_ID from BMS_Schedule_Process_Data_Temp (NOLOCK) WHERE Record_Status='W')
					),X.Date_Time ) AS  Schedule_Item_Log_Time from BMS_Schedule_Process_Data_Temp T INNER JOIN

					(select BMS_Asset_Ref_Key, Min(CAST(CAST(Date_Time AS DATETIME2) AS DATETIME)) AS Date_Time from BMS_Schedule_Process_Data_Temp (NOLOCK) 
					...
					INNER JOIN BV_Schedule_Transaction BV 
					 (NOLOCK)ON BV.Program_Episode_ID = Y.BMS_Asset_Ref_Key AND BV.Timeline_ID = BSPDT.Timeline_ID AND 
					CAST(BV.Schedule_Item_Log_Time AS DATETIME) = CAST(CAST(BSPDT.Date_Time AS DATETIME2) AS DATETIME)
					AND BSPDT.Timeline_ID NOT IN(select Timeline_ID from BMS_Schedule_Process_Data_Temp (NOLOCK) WHERE Record_Status='W')
					...
					FROM Content_Channel_Run VWB (NOLOCK)
					INNER JOIN BMS_Schedule_Process_Data_Temp TBSPD (NOLOCK) ON 
					...
					INNER JOIN BMS_Asset BA (NOLOCK) ON
					TBSPD.BMS_Asset_Ref_Key = BA.BMS_Asset_Ref_Key
					INNER JOIN Title_Content TC
					 (NOLOCK) ON VWB.Title_Content_Code= TC.Title_Content_Code AND TC.Ref_BMS_Content_Code=TBSPD.BMS_Asset_Ref_Key
					--AND ((CAST(CAST(
					--TBSPD.Date_Time 
					...
					BEGIN
						...
						SET  @Email_Notification_Msg_Code = 0
						SELECT TOP 1 @Email_Notification_Msg_Code = Email_Notification_Msg_Code
						FROM Email_Notification_Msg (NOLOCK) WHERE  UPPER(Email_Msg_For) = 'Invalid_Channel' AND [Type] = 'S'
						...
						FROM BMS_Schedule_Process_Data_Temp T (NOLOCK)
						LEFT JOIN BMS_Schedule_Exception BSE (NOLOCK) ON T.BMS_Schedule_Process_Data_Temp_Code = BSE.BMS_Schedule_Process_Data_Temp_Code
					...
					BEGIN
						...
						WHERE Record_Status = 'W' AND BMS_Schedule_Process_Data_Temp_Code IN(
							SELECT BMS_Schedule_Process_Data_Temp_Code FROM BMS_Schedule_Exception BSE (NOLOCK) WHERE ISNULL(BSE.BMS_Schedule_Process_Data_Temp_Code,0) > 0
						)
						PRINT 'Updated Is Exception Flag Yes in BMS_Schedule_Process_Data_Temp'
						-------------------Insert Error------------------------------------------------------------------------
						DECLARE @Count INT
						Select @Count = COUNT(*) from BMS_Schedule_Process_Data_Temp (NOLOCK) Where Record_Status = 'E'
						...
						Select DISTINCT GETDATE(),null,null,'USP_BMS_Schedule', CAST(@Count AS VARCHAR)+' '+'Record''s are in error state for Channel'+' '+ C.Channel_Name,'NA','NA','DB' from BMS_Schedule_Process_Data_Temp BSPDT  (NOLOCK)
						INNER JOIN Channel C (NOLOCK) ON BSPDT.Channel_Code = C.Channel_Code 
					...
					(
						SELECT BST.Timeline_ID FROM BV_Schedule_Transaction BST (NOLOCK) WHERE BST.Channel_Code = @Channel_Code_CV
					)) AND TBSPD.Channel_Code = @Channel_Code_CV
				...
				BEGIN CATCH				
					...
					FROM Email_Notification_Msg (NOLOCK) WHERE  UPPER(Email_Msg_For) = 'BMS_SCHEDULE_USP_FAIL' AND [Type] = 'S'
					...
					FROM BMS_Schedule_Process_Data_Temp BSPDT (NOLOCK) WHERE ISNULL(BSPDT.Record_Status,'N') = 'W'
	...
	if(@Loglevel < 2)Exec [USPLogSQLSteps] '[USP_BMS_Schedule]', 'Step 2', 0, 'Procedure Excution Completed', 0, ''
END