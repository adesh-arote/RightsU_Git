...
BEGIN
	...
	if(@Loglevel< 2)Exec [USPLogSQLSteps] '[USP_Validate_SYN_YEARWISE_RIGHT_FOR_RUN]', 'Step 1', 0, 'Started Procedure', 0, ''
		...
		BEGIN
			...
			,'' as [CHANNEL_NAME]
			FROM Syn_DEAL_RUN ADRR (NOLOCK)
			INNER JOIN Syn_Deal_Movie ADM (NOLOCK) ON ADM.Syn_Deal_Code = @SYN_DEAL_CODE
			INNER JOIN Syn_DEAL_RUN_YEARWISE_RUN ADYR (NOLOCK) ON ADYR.Syn_DEAL_RUN_CODE = ADRR.Syn_DEAL_RUN_CODE
			INNER JOIN  Syn_DEAL_RIGHTS ADR (NOLOCK) ON ADR.Syn_DEAL_CODE = ADRR.Syn_DEAL_CODE
			INNER JOIN  Syn_DEAL_RIGHTS_TITLE ADRT (NOLOCK) ON ADRT.Syn_DEAL_RIGHTS_CODE = ADR.Syn_DEAL_RIGHTS_CODE AND
	...
	if(@Loglevel< 2)Exec [USPLogSQLSteps] '[USP_Validate_SYN_YEARWISE_RIGHT_FOR_RUN]', 'Step 2', 0, 'Procedure Excution Completed', 0, ''
END
