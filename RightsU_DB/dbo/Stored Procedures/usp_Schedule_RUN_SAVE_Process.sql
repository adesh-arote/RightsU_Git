CREATE PROC [dbo].[usp_Schedule_RUN_SAVE_Process]
(
	@AcqDealCode INT
)
AS  
BEGIN
	...
	if(@Loglevel< 2)Exec [USPLogSQLSteps] '[usp_Schedule_RUN_SAVE_Process]', 'Step 1', 0, 'Started Procedure', 0, ''
		...
		INNER JOIN #tempTitle T ON BST.Title_Code = T.Title_Code 
		AND (((Convert(DATE,BST.Schedule_Item_Log_Date,101) BETWEEN T.Right_Start_Date AND T.Right_End_Date) AND BST.Deal_Movie_Code IS NULL)
		OR (BST.Deal_Movie_Code IN(select Acq_Deal_Movie_Code From Acq_Deal_Movie (NOLOCK) Where Acq_Deal_Code = @AcqDealCode)))
		AND BST.Channel_Code = T.Channel_Code
		...
		BEGIN
			...
			select X.File_Code, X.Channel_Code FROM
				...
				FROM #BV_Schedule_Transaction BST 
				INNER JOIN Channel C (NOLOCK) ON C.Channel_Code = BST.Channel_Code) AS X
		...
		DROP TABLE #BV_Schedule_Transaction
		IF OBJECT_ID('tempdb..#BV_Schedule_Transaction') IS NOT NULL DROP TABLE #BV_Schedule_Transaction
		IF OBJECT_ID('tempdb..#tempTitle') IS NOT NULL DROP TABLE #tempTitle
	 
	if(@Loglevel< 2)Exec [USPLogSQLSteps] '[usp_Schedule_RUN_SAVE_Process]', 'Step 2', 0, 'Procedure Excuting Completed', 0, ''
END