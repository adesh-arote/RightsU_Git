...
BEGIN
	...
	if(@Loglevel<2)Exec [USPLogSQLSteps] '[USP_Report_PlatformWise_Acquisition_Neo_Avail_Indiacast]', 'Step 1', 0, 'Started Procedure', 0, ''
	 ...
	 --   @country_code='0', 
		...
		BEGIn
			INSERT INTO #Temp_Country_Neo
			SELECT DISTINCT Country_Code FROM Territory_Details td (NOLOCK)  WHERE td.Territory_Code IN (
		...
		BEGIN
			INSERT INTO #Temp_Country_Neo
			SELECT c.Country_Code FROM Country c (NOLOCK)  WHERE ISNULL(c.Is_Active, 'N') = 'Y' and c.Country_Code <> 3
		END
	...
	if(@Loglevel<2)Exec [USPLogSQLSteps] '[USP_Report_PlatformWise_Acquisition_Neo_Avail_Indiacast]', 'Step 2', 0, 'Procedure Excuting Completed', 0, ''
END