...
BEGIN
	...
	if(@Loglevel<2)Exec [USPLogSQLSteps] '[usp_Schedule_PkgExecutionFail]', 'Step 1', 0, 'Started Procedure', 0, ''
	-- =============================================
	/*
	Steps:-
	...
	*/
	...
	SET NOCOUNT ON;
		...
		----- 1.0 And 2.0 -----
			...
			BEGIN TRY
				...
				SELECT @EmailMsg_New = Email_Msg FROM Email_Notification_Msg (NOLOCK) WHERE LTRIM(RTRIM(Email_Msg_For)) = 'PkgFail' and [Type] = 'S'

				DECLARE @ChannelCode INT
				select @ChannelCode= ChannelCode from Upload_Files (NOLOCK) Where File_code = @FileCode 
	...
	if(@Loglevel<2)Exec [USPLogSQLSteps] '[usp_Schedule_PkgExecutionFail]', 'Step 2', 0, 'Procedure Excuting Completed', 0, ''
END
