...
BEGIN
	...
	if(@Loglevel < 2)Exec [USPLogSQLSteps] '[USP_DELETE_AT_ACQ_Deal]', 'Step 1', 0, 'Started Procedure', 0, ''
		SET NOCOUNT ON
		BEGIN TRY
		BEGIN TRANSACTION;
			/* START : Rights Tab */
			...
			(
				SELECT AT_Acq_Deal_Rights_Holdback_Code FROM AT_Acq_Deal_Rights_Holdback  (NOLOCK)
				WHERE AT_Acq_Deal_Rights_Code IN
				(
					SELECT AT_Acq_Deal_Rights_Code FROM AT_Acq_Deal_Rights (NOLOCK) 
			...
			(
				SELECT AT_Acq_Deal_Rights_Blackout_Code FROM AT_Acq_Deal_Rights_Blackout  (NOLOCK)
			WHERE AT_Acq_Deal_Rights_Code IN
				(
					SELECT AT_Acq_Deal_Rights_Code FROM AT_Acq_Deal_Rights  (NOLOCK)
					WHERE AT_Acq_Deal_Code =@AT_Acq_Deal_Code
	...
	(
		...
		WHERE AT_Acq_Deal_Code =@AT_Acq_Deal_Code
			...
			(
				SELECT AT_Acq_Deal_Pushback_Code FROM AT_Acq_Deal_Pushback  (NOLOCK)
				WHERE AT_Acq_Deal_Code =@AT_Acq_Deal_Code
			)
			...
			(
				SELECT AT_Acq_Deal_Run_Yearwise_Run_Code From AT_Acq_Deal_Run_Yearwise_Run (NOLOCK) WHERE AT_Acq_Deal_Run_Code IN
				(
					SELECT AT_Acq_Deal_Run_Code FROM AT_Acq_Deal_Run (NOLOCK) WHERE AT_Acq_Deal_Code = @AT_Acq_Deal_Code
				)
			)
			...
			(	
				SELECT AT_Acq_Deal_Ancillary_Code From AT_Acq_Deal_Ancillary (NOLOCK) WHERE AT_Acq_Deal_Code = @AT_Acq_Deal_Code
			) 
			DELETE FROM AT_Acq_Deal_Ancillary_Platform_Medium WHERE  AT_Acq_Deal_Ancillary_Platform_Code IN
			(
				SELECT  AT_Acq_Deal_Ancillary_Platform_Code From AT_Acq_Deal_Ancillary_Platform (NOLOCK) WHERE  AT_Acq_Deal_Ancillary_Code IN
				(	
					SELECT AT_Acq_Deal_Ancillary_Code From AT_Acq_Deal_Ancillary (NOLOCK) WHERE AT_Acq_Deal_Code = @AT_Acq_Deal_Code
				) 
			)
			...
			(	
				SELECT AT_Acq_Deal_Cost_Code From AT_Acq_Deal_Cost (NOLOCK) WHERE AT_Acq_Deal_Code = @AT_Acq_Deal_Code
			) 
			...
			(	
				SELECT AT_Acq_Deal_Cost_Costtype_Code From AT_Acq_Deal_Cost_Costtype (NOLOCK) WHERE AT_Acq_Deal_Cost_Code IN
				(	
					SELECT AT_Acq_Deal_Cost_Code From AT_Acq_Deal_Cost (NOLOCK) WHERE AT_Acq_Deal_Code = @AT_Acq_Deal_Code
				)
			) 
			...
			(	
				SELECT AT_Acq_Deal_Sport_Code FROM AT_Acq_Deal_Sport (NOLOCK) WHERE AT_Acq_Deal_Code = @AT_Acq_Deal_Code
			) 
			...
			(	
				SELECT AT_Acq_Deal_Sport_Ancillary_Code FROM AT_Acq_Deal_Sport_Ancillary (NOLOCK) WHERE AT_Acq_Deal_Code = @AT_Acq_Deal_Code
			) 
			...
			(	
				SELECT AT_Acq_Deal_Sport_Monetisation_Ancillary_Code FROM AT_Acq_Deal_Sport_Monetisation_Ancillary (NOLOCK) WHERE AT_Acq_Deal_Code = @AT_Acq_Deal_Code
			)  
			...
			(	
				SELECT AT_Acq_Deal_Sport_Sales_Ancillary_Code FROM AT_Acq_Deal_Sport_Sales_Ancillary (NOLOCK) WHERE AT_Acq_Deal_Code = @AT_Acq_Deal_Code
			) 
			...
			(
				Select AT_Acq_Deal_Movie_Code from AT_Acq_Deal_Movie (NOLOCK) Where AT_Acq_Deal_Code = @AT_Acq_Deal_Code
			)
	...
	if(@Loglevel < 2)Exec [USPLogSQLSteps] '[USP_DELETE_AT_ACQ_Deal]', 'Step 2', 0, 'Procedure Excution Completed', 0, ''
END
