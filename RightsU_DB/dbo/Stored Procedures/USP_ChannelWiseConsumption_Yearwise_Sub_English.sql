...
BEGIN
	...
	CCR.Prime_Runs Prime_No_Of_Runs, 
		(
			SELECT COUNT(BV.BV_Schedule_Transaction_Code) FROM BV_Schedule_Transaction BV WHERE 
	...
	INNER JOIN Title_Content TC ON TC.Title_Content_Code = CCR.Title_Content_Code AND CCR.Title_Content_Code = @Title_Content_Code
		...
		BEGIN
			...
			(
				SELECT 'Prime Time Runs' Description
						...
						,(SELECT  CHANNEL_NAME FROM CHANNEL C WHERE C.Channel_Code = @ChannelCode) CHANNEL_NAME
						,0 Prime_No_Of_Runs
						,0 Prime_Provision
						,0 Prime_Consume
						,0 Prime_Balance 
		...
		SELECT * FROM
			(
				...
				SELECT DISTINCT Channel_Code,
						(SELECT  CHANNEL_NAME FROM CHANNEL C WHERE C.Channel_Code = T.Channel_Code) CHANNEL_NAME,
