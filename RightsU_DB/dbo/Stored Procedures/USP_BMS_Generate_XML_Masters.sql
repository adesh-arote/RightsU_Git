...
BEGIN	
	...
	if(@Loglevel < 2)Exec [USPLogSQLSteps] '[USP_BMS_Generate_XML_Masters]', 'Step 1', 0, 'Started Procedure', 0, ''
		SET NOCOUNT ON;
		--=======================================================
		...
		DECLARE @i INT = 1
			...
			PRINT '@i : ' + CAST(@i AS VARCHAR)
				...
				--PRINT  '@recordCount : '+ CAST (@recordCount AS VARCHAR)
					SET @no_Of_Record_In_Batch = @no_Of_Record_In_Batch_Master
					SELECT @recordCount = COUNT(*)
					FROM Extended_Columns_Value bvECV (NOLOCK)
					...
					AND bvECV.Columns_Value_Code IN (
						SELECT DISTINCT RU_ProgramCategory_Code FROM BMS_Asset (NOLOCK) WHERE 
				...
				BEGIN            
					...
					FROM BMS_WBS bvW   (NOLOCK)              
					INNER JOIN SAP_WBS SW (NOLOCK) ON bvW.SAP_WBS_Code = SW.SAP_WBS_Code
	...
	-- ==========================================================================================================================
					...
					BEGIN                
						...
						FROM BMS_All_Masters (NOLOCK) WHERE Module_Name = @moduleName AND Is_Active = 'Y' AND Method_Type = 'P'
							...
							SELECT @xmlData = (
								...
								FROM (
									SELECT TOP(@totalTopOuter) * FROM (
										...
										AND bvECV.Columns_Value_Code IN (
											SELECT DISTINCT RU_ProgramCategory_Code FROM BMS_Asset  (NOLOCK)
											WHERE 
											ISNULL(Record_Status, '') IN ('P') AND 
											ISNULL(RU_ProgramCategory_Code,0) > 0
										)
							...
							SELECT @xmlData = (
								...
								FROM (
									SELECT TOP(@totalTopOuter) * FROM (
										...
										FROM BMS_WBS bvW (NOLOCK)
										INNER JOIN SAP_WBS SW (NOLOCK) ON SW.SAP_WBS_Code = bvW.Sap_WBS_Code
										WHERE (bvW.BMS_WBS_Code = @Code OR @Code = 0)
										AND (
												(ISNULL(LTRIM(RTRIM(bvW.BMS_Key)), 0) = 0 AND @i = 1) OR
												(ISNULL(LTRIM(RTRIM(bvW.BMS_Key)), 0) > 0 AND @i = 2)
	...
	if(@Loglevel < 2)Exec [USPLogSQLSteps] '[USP_BMS_Generate_XML_Masters]', 'Step 2', 0, 'Procedure Excution Completed', 0, ''
END