...
BEGIN
	...
	if(@Loglevel< 2)Exec [USPLogSQLSteps] '[USPValidateMHRequestConsumption]', 'Step 1', 0, 'Started Procedure', 0, ''
		...
		INNER JOIN MHRequestDetails MRD (NOLOCK)  ON 
		...
		INTO #MusicDealTemp 
		from MHRequest MR (NOLOCK) 
		INNER JOIN MHRequestDetails MRD (NOLOCK)  ON MR.MHRequestCode = MRD.MHRequestCode AND ISNULL(MRD.IsApprove,'P') = 'P'
		INNER JOIN Music_Title_Label MTL (NOLOCK)  ON MTL.Music_Title_Code = MRD.MusicTitleCode AND MR.MHRequestTypeCode=1 AND MR.MHRequestCode = @MHRequestCode
		INNER JOIN Music_Deal MD (NOLOCK)  ON MD.Music_Label_Code = MTL.Music_Label_Code
		INNER JOIN Music_Deal_Channel MDC (NOLOCK)  ON MD.Music_Deal_Code = MDC.Music_Deal_Code AND MDC.Channel_Code = MR.ChannelCode
		...
		BEGIN
			...
			LEFT JOIN Music_Deal_LinkShow MDL (NOLOCK)  ON MDL.Music_Deal_Code = T.Music_Deal_Code
			AND ((T.Link_Show_Type = 'SP' AND MDL.Title_Code = T.TitleCode) OR (T.Link_Show_Type = 'AF' AND T.TitleCode IN(select Title_Code from Title (NOLOCK)  Where Deal_Type_Code=18))
			OR (T.Link_Show_Type = 'AS') OR (T.Link_Show_Type = 'AN' AND T.TitleCode IN(select Title_Code from Title (NOLOCK)  Where Deal_Type_Code != 18))
			OR (T.Link_Show_Type = 'AE' AND T.TitleCode IN(select Title_Code from Title (NOLOCK)  Where Deal_Type_Code=22))
			)
			)
			AND IsValid != 'N'
		END
	...
	if(@Loglevel< 2)Exec [USPLogSQLSteps] '[USPValidateMHRequestConsumption]', 'Step 2', 0, 'Procedure Excution Completed', 0, ''
END