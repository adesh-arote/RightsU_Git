...
BEGIN
	...
	if(@Loglevel < 2)Exec [USPLogSQLSteps] '[USP_BMS_Schedule_Process]', 'Step 1', 0, 'Started Procedure', 0, ''
	SET NOCOUNT ON;	
	BEGIN TRY	
			...
			BEGIN
				...
				BEGIN
					...
					BEGIN
						...
						BEGIN
							...
							FROM Email_Notification_Msg (NOLOCK) WHERE  UPPER(Email_Msg_For) = 'Right_Period' AND [Type] = 'S'
						END					
					...
					FROM BMS_Schedule_Process_Data_Temp BSPDT  (NOLOCK)
					INNER JOIN BMS_Asset BA (NOLOCK) ON BA.BMS_Asset_Ref_Key = BSPDT.BMS_Asset_Ref_Key AND BSPDT.Channel_Code=@Channel_Code
					INNER JOIN Title_Content TC (NOLOCK) ON TC.Ref_BMS_Content_Code = BSPDT.BMS_Asset_Ref_Key
					INNER JOIN Content_Channel_Run CCR (NOLOCK) ON CCR.Title_Content_Code = TC.Title_Content_Code AND ISNULL(CCR.Is_Archive, 'N') = 'N' 
					...
					(
						...
						(
							SELECT ISNULL(BSP.Timeline_ID,0) FROM BV_Schedule_Transaction BSP (NOLOCK) WHERE ISNULL(BSP.Timeline_ID,0) > 0 AND ISNULL(BSP.File_Code,0) > 0
							UNION 
					...
					IF EXISTS(	
								...
								INNER JOIN Content_Channel_Run CCR (NOLOCK) ON CCR.Content_Channel_Run_Code = UDT.Content_Channel_Run_Code
								INNER JOIN Acq_Deal_Rights ADR (NOLOCK) ON ADR.Acq_Deal_Code = CCR.Acq_Deal_Code AND (CCR.Rights_Start_Date BETWEEN ADR.Right_Start_Date 
								AND ADR.Right_End_Date) AND (CCR.Rights_End_Date BETWEEN ADR.Right_Start_Date AND ADR.Right_End_Date)
								INNER JOIN Acq_Deal_Rights_Title ADRT (NOLOCK) ON ADR.Acq_Deal_Rights_Code = ADRT.Acq_Deal_Rights_Code AND ADRT.Title_Code = UDT.Title_Code
					...
					BEGIN
						...
						SELECT TOP 1 @Email_Notification_Msg_Code = Email_Notification_Msg_Code ,@Is_Error = 'Y'
						FROM Email_Notification_Msg (NOLOCK) WHERE  UPPER(Email_Msg_For) = 'BLACKOUT_RIGHT_PERIOD' AND [Type] = 'S'													
					END
					...
					BEGIN	
						...
						FROM BV_Schedule_Transaction BST (NOLOCK)
						LEFT JOIN BMS_Schedule_Exception BSE (NOLOCK) ON BST.BV_Schedule_Transaction_Code = BSE.BV_Schedule_Transaction_Code
						...
						BEGIN
							...
							ISNULL(@Email_Notification_Msg_Code,0) AS Email_Notification_Msg_Code, 'N' IsMailSent
							FROM BMS_Schedule_Process_Data_Temp BST (NOLOCK)
							LEFT JOIN BMS_Schedule_Exception BSE (NOLOCK) ON BST.BMS_Schedule_Process_Data_Temp_Code = BSE.BV_Schedule_Transaction_Code
					...
					FROM 
					Content_Channel_Run CCR (NOLOCK)
					INNER JOIN Title_Content TC (NOLOCK) ON TC.Title_Content_Code=CCR.Title_Content_Code AND TC.Ref_BMS_Content_Code=CAST(@BMS_Asset_Ref_Key_CV AS VARCHAR)
				...
				ELSE IF EXISTS (
					...
					BEGIN -- Start SimulCast
						IF EXISTS(SELECT Channel_Code FROM Channel (NOLOCK) WHERE Channel_Code = @Channel_Code AND ISNULL(Is_Parent_Child,'') = 'C')
						BEGIN
							...
							SELECT @Parent_Channel_Code = ISNULL(C.Parent_Channel_Code,0) FROM Channel C (NOLOCK) WHERE C.Channel_Code = @Channel_Code
							...
							IF EXISTS (SELECT TOP 1 * FROM BV_Schedule_Transaction BST (NOLOCK) WHERE BST.Channel_Code = @Parent_Channel_Code AND 
					...
					IF EXISTS(SELECT TOP 1 * FROM  BV_Schedule_Transaction BST (NOLOCK) WHERE ISNULL(BST.Timeline_ID,0) = ISNULL(@Timeline_ID_CV,0))
					BEGIN
					...
					BEGIN
						...
						BEGIN
							...
							FROM Email_Notification_Msg (NOLOCK) WHERE UPPER(Email_Msg_For) = 'Right_Rule_Exceeeds' AND [Type] = 'S'
						END
						/***********************************Exception Right Rule Exceeeds**************/
					END
					ELSE 
					...
					BEGIN											
						SELECT TOP 1 @Email_Notification_Msg_Code = ISNULL(@Email_Notification_Msg_Code,'') +  ',' +
						CAST(ISNULL(Email_Notification_Msg_Code,'') AS VARCHAR),@Is_Error = 'Y' 
						FROM Email_Notification_Msg (NOLOCK) WHERE UPPER(Email_Msg_For) = 'TOTAL_OVER_RUN' AND [Type] = 'S'
					END
					...
					BEGIN
						...
						PRINT 'START PRIMETIME_OVER_RUN'
							SELECT TOP 1 @Email_Notification_Msg_Code = ISNULL(@Email_Notification_Msg_Code,'') +   ',' +
							CAST(ISNULL(Email_Notification_Msg_Code,'') AS VARCHAR) ,@Is_Error = 'Y' 
							FROM Email_Notification_Msg (NOLOCK) WHERE UPPER(Email_Msg_For) = 'PRIMETIME_OVER_RUN' AND [Type] = 'S'
					...
					BEGIN
						...
						FROM Email_Notification_Msg (NOLOCK) WHERE UPPER(Email_Msg_For) = 'OUTSIDE OFF PRIME TIME' AND [Type] = 'S'
					END					
					...
					BEGIN						
						...
						BEGIN
							SELECT TOP 1 @Email_Notification_Msg_Code = ISNULL(@Email_Notification_Msg_Code,'') +  ',' +
							CAST(ISNULL(Email_Notification_Msg_Code,'') AS VARCHAR),@Is_Error = 'Y'  
							FROM Email_Notification_Msg (NOLOCK) WHERE UPPER(Email_Msg_For) = 'OFFPRIMETIME_OVER_RUN' AND [Type] = 'S'
						END
					END
						...
						IF EXISTS(SELECT TOP 1 ISNULL(T.Is_Prime_Time,'N') FROM #Temp_BMS_Acq_Run_Process_Data T 
							...
							SELECT TOP 1 @Email_Notification_Msg_Code = ISNULL(@Email_Notification_Msg_Code,'') +  ',' +
							CAST(ISNULL(Email_Notification_Msg_Code,'') AS VARCHAR) 
							FROM Email_Notification_Msg  (NOLOCK) WHERE UPPER(Email_Msg_For) = 'OUTSIDE PRIME TIME' AND [Type] = 'S'												
						END					
						...
						BEGIN
							...
							IF NOT EXISTS(select * from BMS_Schedule_Exception (NOLOCK) WHERE BV_Schedule_Transaction_Code = @BMS_Schedule_Process_Data_Code)
							BEGIN
	...
	if(@Loglevel < 2)Exec [USPLogSQLSteps] '[USP_BMS_Schedule_Process]', 'Step 2', 0, 'Procedure Excution Complete', 0, ''
END