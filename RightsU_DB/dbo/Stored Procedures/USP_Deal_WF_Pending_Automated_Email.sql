CREATE PROCEDURE [dbo].[USP_Deal_WF_Pending_Automated_Email]
AS
----------------------------
--Author: Ayush Dubey
...
BEGIN
	...
	if(@Loglevel < 2)Exec [USPLogSQLSteps] '[USP_Deal_WF_Pending_Automated_Email]', 'Step 1', 0, 'Started Procedure', 0, ''
		SET NOCOUNT ON
		...
		SELECT @Email_Config_Code=Email_Config_Code FROM Email_Config (NOLOCK) WHERE [Key]='AMR'
		...
		BEGIN
			SELECT @Email_Template = Template_Desc FROM Email_Template (NOLOCK) WHERE Template_For = 'Acq_Syn_Deal_Pending'
	...
	if(@Loglevel < 2)Exec [USPLogSQLSteps] '[USP_Deal_WF_Pending_Automated_Email]', 'Step 2', 0, 'Procedure Excution Completed', 0, ''
END				