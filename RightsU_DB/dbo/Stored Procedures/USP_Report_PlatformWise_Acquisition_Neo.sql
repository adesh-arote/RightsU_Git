...
BEGIN
	...
	if(@Loglevel<2)Exec [USPLogSQLSteps] '[USP_Report_PlatformWise_Acquisition_Neo]', 'Step 1', 0, 'Started Procedure', 0, ''
		...
		BEGIN
			...
			SELECT AR.Acq_Deal_Code, AR.Acq_Deal_Rights_Code, ISNULL(AD.Master_Deal_Movie_Code_ToLink, 0),  AR.Restriction_Remarks, ADRP.Platform_Code, ADRT.Title_Code,
				   ...
				   (STUFF((
					...
					(SELECT adm_Tmp.Acq_Deal_Movie_Code FROM Acq_Deal_Movie adm_Tmp (NOLOCK) WHERE adm_Tmp.Acq_Deal_Code = AD.Acq_Deal_Code)
			...
			INNER JOIN Acq_Deal_Rights AR (NOLOCK) ON AR.Acq_Deal_Code = AD.Acq_Deal_Code
			...
			AND (
				ADRT.Title_Code IN (SELECT tt.Title_Code FROM #Temp_Title tt) OR (
					@Include_Sub_Deal = 'Y' AND AD.Master_Deal_Movie_Code_ToLink IN (
						Select Acq_Deal_Movie_Code From Acq_Deal_Movie (NOLOCK) Where Title_Code IN (SELECT Title_Code FROM #Temp_Title)
					)
				)
			)
		...
		BEGIN
			...
			BEGIN
				...
				SELECT AA.Acq_Deal_Rights_Code, AA.Rights_Start_Date AS Rights_Start_Date, AA.Rights_End_Date AS Right_End_Date, tit.Title_Language_Code
					, AA.Sub_License_Code, AA.Is_Exclusive, Episode_From ,Episode_To
				FROM #Avail_Acq AA
				INNER JOIN Acq_Deal_Movie ADM (NOLOCK) ON ADM.Acq_Deal_Movie_Code = AA.Master_Deal_Movie_Code
				INNER JOIN Title tit (NOLOCK) ON ADM.Title_Code = tit.Title_Code
				INNER JOIN #Temp_Title_Language TTL ON TTL.number = tit.Title_Language_Code
			END
		...
		*/
		...
		INNER JOIN Acq_Deal_Rights ADR (NOLOCK) ON ADR.Acq_Deal_Rights_Code = TR.Acq_Deal_Rights_Code
		INNER JOIN Acq_Deal AD (NOLOCK) ON AD.Acq_Deal_Code = ADR.Acq_Deal_Code
		INNER JOIN Acq_Deal_Movie ADM (NOLOCK) ON ADM.Acq_Deal_Code = ADM.Acq_Deal_Code AND ADM.Title_Code = TM.Title_Code 
	...
	if(@Loglevel<2)Exec [USPLogSQLSteps] '[USP_Report_PlatformWise_Acquisition_Neo]', 'Step 2', 0, 'Procedure Excuting Completed', 0, ''
END