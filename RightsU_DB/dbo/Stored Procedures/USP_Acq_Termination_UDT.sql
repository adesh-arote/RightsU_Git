...
BEGIN
	...
	if(@Loglevel < 2)Exec [USPLogSQLSteps] '[USP_Acq_Termination_UDT]', 'Step 1', 0, 'Started Procedure', 0, ''
		...
		*/
		...
		SELECT DISTINCT ADM.Acq_Deal_Code, ADMML.Title_Code, ADMML.Episode_No  FROM Acq_Deal_Movie ADM (NOLOCK)
		INNER JOIN #Termination_Deals_Status TDS ON ADM.Title_Code = TDS.Title_Code
		INNER JOIN Acq_Deal_Movie_Music_Link ADMML (NOLOCK) ON ADM.Acq_Deal_Movie_Code = ADMML.Link_Acq_Deal_Movie_Code
		...
		BEGIN
			...
			BEGIN
				...
				BEGIN
					IF NOT EXISTS (
						SELECT * FROM Acq_Deal_Termination_Details (NOLOCK) WHERE Acq_Deal_Code = @dealCode AND ISNULL(Title_Code, 0) = @titleCode AND
						(
					...
					BEGIN
						...
						BEGIN
							...
							INNER JOIN Syn_Acq_Mapping SAM (NOLOCK) ON SAM.Deal_Code = @dealCode AND SDR.Syn_Deal_Rights_Code = SAM.Syn_Deal_Rights_Code 
							INNER JOIN Syn_Deal_Rights_Title SDRT (NOLOCK) ON SDRT.Syn_Deal_Rights_Code = SDR.Syn_Deal_Rights_Code AND SDRT.Title_Code = @titleCode
						END
						ELSE
						BEGIN
						...
						BEGIN
							...
							BEGIN
								...
								INNER JOIN Syn_Acq_Mapping SAM (NOLOCK) ON SAM.Deal_Code = @dealCode AND SDR.Syn_Deal_Rights_Code = SAM.Syn_Deal_Rights_Code 
								INNER JOIN Syn_Deal_Rights_Title SDRT (NOLOCK) ON SDRT.Syn_Deal_Rights_Code = SDR.Syn_Deal_Rights_Code AND (
									(SDRT.Title_Code = @titleCode AND @titleCode > 0) OR 
									(SDRT.Title_Code IN (SELECT Title_Code FROM Acq_Deal_Movie (NOLOCK) WHERE Acq_Deal_Code = @dealCode) AND @titleCode = 0)
									)
								INNER JOIN Entity E (NOLOCK) ON E.Entity_Code = SD.Entity_Code
				...
				BEGIN
					...
					BEGIN
						...
						SELECT TOP 1 @ccMailId = Email_Id FROM Users (NOLOCK) WHERE Users_Code = @Login_User_Code
			...
			BEGIN
				...
				BEGIN
					...
					SELECT ADR.Acq_Deal_Rights_Code, ADR.Actual_Right_Start_Date, ISNULL(ADR.Actual_Right_End_Date, '9999-12-31'), ADR.Right_Type FROM Acq_Deal_Rights ADR (NOLOCK)
					...
					BEGIN
						...
						SELECT @titleCount = COUNT( DISTINCT Title_Code) FROM Acq_Deal_Rights_Title (NOLOCK) WHERE Acq_Deal_Rights_Code = @Acq_Deal_Rights_Code 
						AND Title_Code <> @Title_Code

						SELECT @minEpisodeFrom  = MIN(Episode_From) FROM Acq_Deal_Rights_Title (NOLOCK) WHERE Acq_Deal_Rights_Code = @Acq_Deal_Rights_Code 
						AND (Title_Code = @Title_Code OR @Title_Code = 0)

						SELECT @maxEpisodeTo  = MAX(Episode_To) FROM Acq_Deal_Rights_Title (NOLOCK) WHERE Acq_Deal_Rights_Code = @Acq_Deal_Rights_Code 
						AND (Title_Code = @Title_Code OR @Title_Code = 0)
						...
						BEGIN
							...
							BEGIN
								...
								(
									SELECT Acq_Deal_Rights_Holdback_Code FROM Acq_Deal_Rights_Holdback (NOLOCK) WHERE Acq_Deal_Rights_Code = @Acq_Deal_Rights_Code
								)
								...
								(
									SELECT Acq_Deal_Rights_Blackout_Code FROM Acq_Deal_Rights_Blackout (NOLOCK) WHERE Acq_Deal_Rights_Code = @Acq_Deal_Rights_Code
								) 
							...
							BEGIN
								...
								BEGIN
									...
									SELECT 
										...
										INNER JOIN Acq_Deal_Rights_Title AtADRT (NOLOCK) On 
									...
									FROM Acq_Deal_Rights_Blackout_Dubbing ADRBD (NOLOCK)
										INNER JOIN Acq_Deal_Rights_Blackout ADRB (NOLOCK) ON ADRBD.Acq_Deal_Rights_Blackout_Code = ADRB.Acq_Deal_Rights_Blackout_Code
						...
						BEGIN
							SET @ADRun_Code = ''
							SELECT @ADRun_Code = @ADRun_Code + CAST(ADR.Acq_Deal_Run_Code AS VARCHAR) + ',' FROM Acq_Deal_Run ADR (NOLOCK)
						...
						isnull(Is_Channel_Definition_Rights, 'N') AS Is_Channel_Defined, Run_Definition_Type, 'N' AS Process_Done INTO #TEMP_Run_Def FROM Acq_Deal_Run ADRun (NOLOCK)
						INNER JOIN Acq_Deal_Run_Title ADRunT (NOLOCK) ON ADRun.Acq_Deal_Run_Code = ADRunT.Acq_Deal_Run_Code AND (ADRunT.Title_Code = @Title_Code OR @Title_Code = 0)
						...
						BEGIN
							...
							BEGIN
								...
								BEGIN
									...
									FROM Acq_Deal_Run_Yearwise_Run ADRY (NOLOCK) where ADRY.Acq_Deal_Run_Code = @Acq_Deal_Run_Code
									...
									IF NOT EXISTS (SELECT * FROM Acq_Deal_Run_Yearwise_Run (NOLOCK) WHERE Acq_Deal_Run_Code = @Acq_Deal_Run_Code)
									BEGIN
									...
									BEGIN
										...
										SELECT @No_Of_Run_New = SUM(ISNULL(No_Of_Runs, 0)) FROM Acq_Deal_Run_Yearwise_Run (NOLOCK) WHERE Acq_Deal_Run_Code = @Acq_Deal_Run_Code
										...
										BEGIN
											...
											BEGIN
												...
												BEGIN
													...
													SELECT @totalRunConsumed = SUM(ISNULL(No_Of_Runs_Sched, 0)) FROM Acq_Deal_Run_Channel (NOLOCK) WHERE Acq_Deal_Run_Code = @Acq_Deal_Run_Code

													SET @changePercentage = ((@No_Of_Run_New * 100) / @No_Of_Run_Old)

													IF EXISTS (
														SELECT * FROM Acq_Deal_Run_Channel (NOLOCK) WHERE ISNULL(No_Of_Runs_Sched, 0) > ((Min_Runs * @changePercentage) / 100)
													...
													FROM Acq_Deal_Run_Channel (NOLOCK) WHERE Acq_Deal_Run_Code  = @Acq_Deal_Run_Code 
									
													SET @channelCode = 0

													IF(@sumOfMin > @sumOfMax)
													BEGIN
														SELECT @channelCode = MAX(Acq_Deal_Run_Channel_Code) FROM Acq_Deal_Run_Channel  (NOLOCK)
											...
											BEGIN
												...
												FROM Acq_Deal_Run (NOLOCK) WHERE Acq_Deal_Run_Code = @Acq_Deal_Run_Code
												...
												IF(@Is_Prime_OffPrime_Defined = 'Y')
													SELECT @sumOfRuns_Run = (ISNULL(Prime_Run, 0) + ISNULL(Off_Prime_Run,0)) FROM Acq_Deal_Run (NOLOCK) WHERE Acq_Deal_Run_Code = @Acq_Deal_Run_Code
												ELSE
													SELECT @sumOfRuns_Run = No_Of_Runs FROM Acq_Deal_Run (NOLOCK) WHERE Acq_Deal_Run_Code = @Acq_Deal_Run_Code

												SELECT @sumOfRuns_YWR = SUM(ISNULL(No_Of_Runs,0)) FROM Acq_Deal_Run_Yearwise_Run (NOLOCK) WHERE Acq_Deal_Run_Code = @Acq_Deal_Run_Code
												SELECT @sumOfRuns_Channel = SUM(ISNULL(Min_Runs,0)) FROM Acq_Deal_Run_Channel (NOLOCK) WHERE Acq_Deal_Run_Code = @Acq_Deal_Run_Code
												...
												BEGIN
													...
													BEGIN
														IF(
															...
															FROM Acq_Deal_Run (NOLOCK) WHERE Acq_Deal_Run_Code = @Acq_Deal_Run_Code) < @maxRun
														)
														BEGIN
												...
												BEGIN
													...
													SELECT @channelCode = MAX(Acq_Deal_Run_Yearwise_Run_Code) FROM Acq_Deal_Run_Yearwise_Run (NOLOCK)
													WHERE Acq_Deal_Run_Code = @Acq_Deal_Run_Code
												...
												IF(@maxRun > @sumOfRuns_Channel)
												BEGIN
													SELECT TOP 1 @channelCode = Acq_Deal_Run_Channel_Code FROM Acq_Deal_Run_Channel (NOLOCK)
													WHERE Acq_Deal_Run_Code = @Acq_Deal_Run_Code
					...
					SELECT DISTINCT ADRun.Acq_Deal_Run_Code INTO #RunDef FROM Acq_Deal_Run ADRun (NOLOCK)
					INNER JOIN Acq_Deal_Run_Title ADRunT (NOLOCK) ON ADRun.Acq_Deal_Run_Code = ADRunT.Acq_Deal_Run_Code 
					AND ADRun.Acq_Deal_Code = @Deal_Code
					INNER JOIN Acq_Deal_Rights ADR (NOLOCK) ON ADR.Acq_Deal_Code = ADRun.Acq_Deal_Code
					INNER JOIN Acq_Deal_Rights_Title ADRT (NOLOCK) ON ADR.Acq_Deal_Rights_Code = ADRT.Acq_Deal_Rights_Code AND 
					...
					INNER JOIN Acq_Deal_Rights_Platform ADRP (NOLOCK) ON ADR.Acq_Deal_Rights_Code = ADRP.Acq_Deal_Rights_Code AND
						ADRP.Platform_Code IN (SELECT DISTINCT Platform_Code FROM [Platform] (NOLOCK) WHERE Is_No_Of_Run = 'Y')

					SELECT Acq_Deal_Run_Code INTO #RunDef_Delete 
					FROM Acq_Deal_Run (NOLOCK) WHERE Acq_Deal_Code = @Deal_Code AND Acq_Deal_Run_Code NOT IN (SELECT Acq_Deal_Run_Code from #RunDef)
	...
	if(@Loglevel < 2)Exec [USPLogSQLSteps] '[USP_Acq_Termination_UDT]', 'Step 2', 0, 'Procedure Excution Completed', 0, ''
END
