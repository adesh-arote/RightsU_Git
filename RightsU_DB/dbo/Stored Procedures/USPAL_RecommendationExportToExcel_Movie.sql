CREATE PROCEDURE [dbo].[USPAL_RecommendationExportToExcel_Movie]
...
--Select top 1 AL_Vendor_Rule_Code from AL_Recommendation_Content WHERE Al_Recommendation_Code = 1450--@AL_Recommendation_Code
	...
	ISNULL(vwale_Tom2.Column_Value,'') AS RottenTomatoes_Rating, ISNULL(vwale_ter.Column_Value,'') AS Rights, ISNULL(vwale_aw.Column_Value,'') AS Awards, ISNULL(vwale_nom.Column_Value,'') AS Nominations, 
	ISNULL(vwale_box.Column_Value,'') AS BoxOffice, '' AS GeneralRemarks, alrc.Content_Type, alrc.Content_Status, CASE WHEN alrc.Content_Type = 'H' THEN 'Holdover' WHEN  alrc.Content_Type = 'D' THEN 'Remove' ELSE 'New' END AS Content_Record_Status, alrc.AL_Recommendation_Content_Code 
	...
	LEFT JOIN VWALTitleRecomExt vwale_ter ON vwale_ter.Record_Code = t.Title_Code AND vwale_ter.Table_Name = 'TITLE' AND vwale_ter.Columns_Code = 52
