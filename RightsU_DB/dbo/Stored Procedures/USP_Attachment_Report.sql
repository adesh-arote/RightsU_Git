...
BEGIN      
	...
	if(@Loglevel < 2)Exec [USPLogSQLSteps] '[USP_Attachment_Report]', 'Step 1', 0, 'Started Procedure', 0, ''
		 DECLARE  
		 ...
		 BEGIN  
		  ...
		  ,STUFF((  
		   ...
		   INNER JOIN Acq_deal_Licensor ADL (NOLOCK) on ADA.Acq_Deal_Code = ADL.Acq_Deal_Code        
		  ...
		  INNER JOIN Acq_Deal_Attachment ADA (NOLOCK) on AD.Acq_Deal_Code = ADA.Acq_Deal_Code        
		  INNER JOIN Document_Type DT (NOLOCK) on ADA.Document_Type_Code = DT.Document_Type_Code          
		  INNER JOIN Deal_Type DTC (NOLOCK) on AD.Deal_Type_Code = DTC.Deal_Type_Code     
		  ...
		  INNER JOIN Title t (NOLOCK) ON t.Title_Code = adm.Title_Code  AND   
		 ...
		 BEGIN    
		  ...
		  INNER JOIN Syn_Deal_Attachment SDA (NOLOCK) on SD.Syn_Deal_Code = SDA.Syn_Deal_Code    
		  INNER JOIN Document_Type DT (NOLOCK) on SDA.Document_Type_Code = DT.Document_Type_Code     
		  INNER JOIN Deal_Type DTC (NOLOCK) on SD.Deal_Type_Code = DTC.Deal_Type_Code          
		  INNER JOIN Syn_Deal_Movie sdm (NOLOCK) On sdm.Syn_Deal_Code = sd.Syn_Deal_Code    
		   AND (@Title_Codes = '' OR ISNULL(sdm.Title_Code, 0) in (select number from fn_Split_withdelemiter(@Title_Codes,',')))  
		  INNER JOIN Title t (NOLOCK) on t.Title_Code = sdm.Title_Code  AND ((isnull(sda.Title_Code, 0) = 0) OR ((isnull(sda.Title_Code, 0) <> 0 AND sdm.Title_Code = sda.Title_Code)))    
	...
	if(@Loglevel < 2)Exec [USPLogSQLSteps] '[USP_Attachment_Report]', 'Step 2', 0, 'Procedure Excution Completed', 0, ''
END
