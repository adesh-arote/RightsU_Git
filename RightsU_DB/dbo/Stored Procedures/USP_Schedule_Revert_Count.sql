...
BEGIN      
	...
	if(@Loglevel< 2)Exec [USPLogSQLSteps] '[USP_Schedule_Revert_Count]', 'Step 1', 0, 'Started Procedure', 0, ''
	-- =============================================
	/*       
	...
	*/      
	-- =============================================      
		...
		BEGIN ------------------------------------------- GLOBAL VARIBALES -------------------------------------------
			...
			SELECT @ScheStartDate = StartDate, @ScheEndDate = EndDate FROM Upload_Files (NOLOCK) WHERE File_code =  @File_Code
		...
		PRINT 'RN'
			...
			FROM BV_Schedule_Transaction bst (NOLOCK) WHERE 1=1
		...
		PRINT 'RY'
			...
			FROM BV_Schedule_Transaction bst (NOLOCK) WHERE 1=1 AND bst.Deal_Code = @Deal_Code AND Deal_Type = @Deal_Type      
		...
		BEGIN
			...
			-- ,pd.Right_Start_Date
				...
				FROM Content_Channel_Run CCR (NOLOCK)
				INNER JOIN Provisional_Deal pd (NOLOCK) ON pd.Provisional_Deal_Code = CCR.Provisional_Deal_Code WHERE      
				...
				FROM Content_Channel_Run  (NOLOCK)     
				...
				IF((SELECT
						...
						WHERE  AD.Deal_Workflow_Status NOT IN ('AR', 'WA') AND ((AD.Business_Unit_Code IN ( SELECT Business_Unit_Code FROM Business_Unit (NOLOCK) WHERE  
						Business_Unit_Name LIKE '%English Movies%') AND ISNULL(CCR.Deal_Type, 'A') = 'A')
						OR (PD.Business_Unit_Code IN ( SELECT Business_Unit_Code FROM Business_Unit (NOLOCK) WHERE  Business_Unit_Name LIKE '%English Movies%')
				...
				BEGIN
					...
					SELECT
						...
						FROM Content_Channel_Run (NOLOCK) WHERE Acq_Deal_Run_Code = @VW_DMR_Run_Code_Cr AND Is_Archive = 'N' AND Channel_Code = @Channel_Code
	...
	if(@Loglevel< 2)Exec [USPLogSQLSteps] '[USP_Schedule_Revert_Count]', 'Step 2', 0, 'Procedure Excuting Completed', 0, ''
END