...
BEGIN   
	...
	if(@Loglevel< 2)Exec [USPLogSQLSteps] '[USP_Syn_Deal_Title_Platform_Report]', 'Step 1', 0, 'Started Procedure', 0, '' 
	--PRINT 'Show Platform wise report'           	
		...
		SELECT
			...
			INNER JOIN Syn_Deal_Movie SDM (NOLOCK) On SD.Syn_Deal_Code = SDM.Syn_Deal_Code AND SD.Business_Unit_Code IN (SELECT CAST(number as INT) from [dbo].[fn_Split_withdelemiter](@Business_Unit_code,','))
			...
			AND SDM.Title_Code = SDRT.Title_Code   INNER JOIN Title t (NOLOCK) On SDRT.Title_Code = t.Title_Code  
	...
	if(@Loglevel< 2)Exec [USPLogSQLSteps] '[USP_Syn_Deal_Title_Platform_Report]', 'Step 2', 0, 'Procedure Excuting Completed', 0, '' 
END