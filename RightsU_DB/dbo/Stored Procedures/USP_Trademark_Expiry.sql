...
BEGIN
	...
	if(@Loglevel < 2)Exec [USPLogSQLSteps] '[USP_Trademark_Expiry]', 'Step 1', 0, 'Started Procedure', 0, ''  
		SET NOCOUNT ON
		...
		SELECT @Email_Config_Code = Email_Config_Code FROM Email_Config (NOLOCK) where [Key] = 'TE'
		...
		BEGIN
			...
			SET @Email_footer =
			 '</table><br/><div class="textFont">Thanks & Regards,</br>RightsU Support</div> </br></br><div id="divFooter" class="textFont">This email is generated by RightsU (Rights Management System)</div></body></html>'
			...
			FROM IPR_Rep IR (NOLOCK)
				...
				INNER JOIN Users U (NOLOCK) ON IR.Created_By = U.Users_Code
	...
	if(@Loglevel < 2)Exec [USPLogSQLSteps] '[USP_Trademark_Expiry]', 'Step 2', 0, 'Procedure Excuting Completed', 0, ''  
END
