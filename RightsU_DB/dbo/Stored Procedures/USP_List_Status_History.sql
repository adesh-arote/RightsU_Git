...
BEGIN	
	...
	if(@Loglevel < 2)Exec [USPLogSQLSteps] '[USP_List_Status_History]', 'Step 1', 0, 'Started Procedure', 0, '' 
		SET FMTONLY OFF
		SET NOCOUNT ON;
		...
		IF EXISTS (SELECT Module_Status_Code FROM Module_Status_History (NOLOCK) WHERE Record_Code = @Record_Code AND Module_Code = @Module_Code)
		BEGIN
			SELECT 
		...
		BEGIN
			...
			BEGIN
				...
				SET @dynamicQuery += 'SELECT U.Users_Code AS ID,AD.Version,''Created'' AS [Status],
							...
							INNER JOIN Users U (NOLOCK) ON AD.Inserted_By = U.Users_Code
							INNER JOIN Security_Group SG (NOLOCK) ON U.Security_Group_Code=SG.Security_Group_Code where Acq_Deal_Code = '+ CAST(@Record_Code AS VARCHAR)	
			...
			BEGIN
				...
				SET @dynamicQuery += ' SELECT U.Users_Code as ID,AD.Version,''Created'' AS [Status],
							...
							INNER JOIN Users U (NOLOCK) ON AD.Inserted_By = U.Users_Code
							INNER JOIN Security_Group SG (NOLOCK) ON U.Security_Group_Code=SG.Security_Group_Code  where Syn_Deal_Code = '+ CAST( @Record_Code AS VARCHAR)
			...
			BEGIN
				...
				SET @dynamicQuery += 'SELECT U.users_code as ID,'''' as [Version],''Open'' AS [Status],
							...
							FROM Module_Status_History  MSH (NOLOCK)
							INNER JOIN Users U (NOLOCK) ON MSH.status_changed_by = U.users_code
							INNER JOIN Security_Group SG (NOLOCK) ON U.Security_Group_Code=SG.Security_Group_Code WHERE MSH.Record_Code = '+ CAST(@Record_Code AS VARCHAR)	
			...
			BEGIN
				...
				SET @dynamicQuery += ' SELECT U.Users_Code as ID,MD.Version, ''Created'' AS [Status],
							...
							INNER JOIN Users U  (NOLOCK) ON MD.Inserted_By = U.Users_Code
							INNER JOIN Security_Group SG (NOLOCK) ON U.Security_Group_Code=SG.Security_Group_Code  where MD.Music_Deal_Code = '+ CAST( @Record_Code AS VARCHAR)
	...
	if(@Loglevel < 2)Exec [USPLogSQLSteps] '[USP_List_Status_History]', 'Step 2', 0, 'Procedure Excution Completed', 0, '' 
END
