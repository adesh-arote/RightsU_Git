...
BEGIN  
	...
	if(@Loglevel <2)Exec [USPLogSQLSteps] '[USP_Reports_Header_Data]', 'Step 1', 0, 'Started Procedure', 0, ''
		...
		BEGIN
			...
			BEGIN
				SELECT   
				 ...
				 FROM System_Message SM   (NOLOCK) 
				 INNER JOIN System_Module_Message SMM (NOLOCK)  ON SMM.System_Message_Code = SM.System_Message_Code AND (SMM.Module_Code = @Module_Code OR ISNULL(SMM.Module_Code, 0)= 0)  
	...
	if(@Loglevel <2)Exec [USPLogSQLSteps] '[USP_Reports_Header_Data]', 'Step 2', 0, 'Procedure Excuting Completed', 0, ''
END