...
BEGIN	
	...
	BEGIN
		...
		--BEGIN
			...
			FROM (
					...
					--	ELSE '' END AS Status,
					CASE   WHEN MSH.Status = 'C' THEN 'Deal Creation Date' ELSE 'Deal '+DWS.Deal_Workflow_Status_Name + ' Date' END AS Status,
		...
		--BEGIN
			...
			AD.Version, 'Deal Creation Date' AS [Status], AD.Last_Updated_Time AS [Last Updated On],u.First_Name +' '+ u.Last_Name AS [User], Ad.Remarks,
		...
		UNION
		Select * from #tempVersion_1_Acq WHERE [Agreement Number] NOT IN (Select [Agreement Number] from #tempMain_Acq)
	...
	BEGIN
		...
		UNION
		Select * from #tempVersion_1_Syn WHERE [Agreement Number] NOT IN (Select [Agreement Number] from #tempMain_Syn)
	) AS A order by A.Record_Code desc, A.Status_Changed_On desc
	END
