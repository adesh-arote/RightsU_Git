...
BEGIN	
	...
	if(@Loglevel< 2)Exec [USPLogSQLSteps] '[USPDealStatusReport]', 'Step 1', 0, 'Started Procedure', 0, ''
		--DECLARE
		--@ModuleCode INT=35,
		--@BusinessUnitcode VARCHAR(MAX)='',	
		...
		DECLARE
		@DealWorkflowStatusCnt INT = (Select COUNT (DISTINCT Deal_WorkflowFlag) from Deal_Workflow_Status (NOLOCK) ),
		...
		BEGIN
			...
			--BEGIN
				...
				FROM (
						Select Record_Code,AD.Agreement_No, BU.Business_Unit_Name, V.Vendor_Name,AD.Agreement_Date AS 'Agreement Date', '000' + CAST(MSH.Version_No AS NVARCHAR(MAX)) AS Version_No  ,
						...
						INNER JOIN Users U1 (NOLOCK) ON MSH.Status_Changed_By = U1.Users_Code
						INNER JOIN Acq_Deal_Rights adr (NOLOCK) ON adr.Acq_Deal_Code = AD.Acq_Deal_Code
						LEFT JOIN Deal_Workflow_Status DWS (NOLOCK) ON DWS.Deal_WorkflowFlag = msh.Status
						WHERE 
			...
			--BEGIN
				...
				INNER JOIN Vendor V WITH (NOLOCK) ON V.Vendor_Code = AD.Vendor_Code
				INNER JOIN Business_Unit BU WITH (NOLOCK) ON AD.Business_Unit_Code = BU.Business_Unit_Code
				INNER JOIN Users u (NOLOCK) ON u.Users_Code = AD.Last_Action_By
				WHERE 
				(@BusinessUnitcode='' OR (AD.Business_Unit_Code IN(SELECT number FROM  dbo.fn_Split_withdelemiter(@BusinessUnitcode, ',')))) 
			...
			(
				...
				UNION
				Select * from #tempVersion_1_Acq WHERE Record_Code NOT IN (Select Distinct Record_Code from Module_Status_History (NOLOCK))--(Select [Agreement Number] from #tempMain_Acq)
			) AS A 
			order by A.Record_Code desc, A.Status_Changed_On desc
		END
		ELSE
		BEGIN
			...
			--BEGIN
				...
				FROM (
					...
					INNER JOIN Module_Status_History MSH  (NOLOCK) ON MSH.Record_Code = AD.Syn_Deal_Code AND MSH.Module_Code= @ModuleCode
					INNER JOIN Users U1 (NOLOCK) ON MSH.Status_Changed_By = U1.Users_Code
			...
			(
				...
				UNION
				Select * from #tempVersion_1_Syn WHERE Record_Code NOT IN (Select Distinct Record_Code from Module_Status_History (NOLOCK)) --WHERE [Agreement Number] NOT IN (Select [Agreement Number] from #tempMain_Syn)
			) AS A 
			order by A.Record_Code desc, A.Status_Changed_On desc
	...
	if(@Loglevel< 2)Exec [USPLogSQLSteps] '[USPDealStatusReport]', 'Step 2', 0, 'Procedure Excution Completed', 0, ''
END
