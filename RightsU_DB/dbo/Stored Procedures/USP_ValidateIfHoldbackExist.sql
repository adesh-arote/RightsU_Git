CREATE PROC [dbo].[USP_ValidateIfHoldbackExist](
...
BEGIN
	...
	if(@Loglevel< 2)Exec [USPLogSQLSteps] '[USP_ValidateIfHoldbackExist]', 'Step 1', 0, 'Started Procedure', 0, ''
		SET FMTONLY OFF
		IF EXISTS(Select TOP 1 * FROM Syn_Deal_Rights_Holdback (NOLOCK) WHERE Syn_Deal_Rights_Code = @Syn_Deal_Rights_Code AND Acq_Deal_Rights_Holdback_Code IS NOT NULL)
		BEGIN
		...
		BEGIN
			...
			END
			IF NOT EXISTS(SELECT TOP 1 * FROM Acq_Deal_Rights_Holdback (NOLOCK) WHERE Acq_Deal_Rights_Code IN(
			...
			(
				...
				FROM Acq_Deal_Rights_Holdback_Platform ADRP (NOLOCK)
			...
			(
				...
				FROM Acq_Deal_Rights_Holdback_Territory ADRT (NOLOCK)
			...
			BEGIN
				...
				(
					...
					FROM Acq_Deal_Rights_Holdback_Subtitling ADRS (NOLOCK)
			...
			BEGIN
				...
				SELECT DISTINCT ADRH.Acq_Deal_Rights_Holdback_Code AS Holdback_Code FROM Acq_Deal_Rights_Holdback ADRH  (NOLOCK)
				INNER JOIN Acq_Deal_Rights_Holdback_Platform ADRHP (NOLOCK) ON ADRH.Acq_Deal_Rights_Holdback_Code = ADRHP.Acq_Deal_Rights_Holdback_Code
				...
				LEFT JOIN Acq_Deal_Rights_Holdback_Subtitling ADRHS (NOLOCK) ON ADRH.Acq_Deal_Rights_Holdback_Code = ADRHS.Acq_Deal_Rights_Holdback_Code
				LEFT JOIN Acq_Deal_Rights_Holdback_Dubbing ADRHD (NOLOCK) ON ADRH.Acq_Deal_Rights_Holdback_Code = ADRHD.Acq_Deal_Rights_Holdback_Code
	...
	if(@Loglevel< 2)Exec [USPLogSQLSteps] '[USP_ValidateIfHoldbackExist]', 'Step 2', 0, 'Procedure Excution Completed', 0, ''
END