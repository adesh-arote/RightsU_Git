...
BEGIN
	...
	SELECT TOP 1 @Version=[Version] FROM Acq_Deal_Tab_Version WHERE Acq_Deal_Code=@Acq_Deal_Code ORDER BY Acq_Deal_Tab_Version_Code DESC
	...
	BEGIN
		...
		IF(NOT EXISTS(SELECT * FROM Acq_Deal WHERE Deal_Workflow_Status NOT IN ('AR', 'WA') AND  Acq_Deal_Code=@Acq_Deal_Code AND Deal_Workflow_Status='EO'))
		BEGIN
	...
	BEGIN
		IF(EXISTS(SELECT * FROM Acq_Deal WHERE Deal_Workflow_Status NOT IN ('AR', 'WA') AND  Acq_Deal_Code=@Acq_Deal_Code AND Deal_Workflow_Status='EO'))
		BEGIN
			UPDATE Acq_Deal_Tab_Version SET Remarks=@Remarks WHERE Acq_Deal_Tab_Version_Code IN(SELECT TOP 1 Acq_Deal_Tab_Version_Code FROM Acq_Deal_Tab_Version
	...
	--	INSERT INTO Acq_Deal_Tab_Version([Version],Remarks,Acq_Deal_Code,Inserted_On,Inserted_By,Approved_On,Approved_By)
	--	VALUES(@Version,@Remarks,@Acq_Deal_Code,GETDATE(),@User_Code,GETDATE(),@User_Code)
		UPDATE Acq_Deal_Tab_Version SET Remarks='' WHERE Acq_Deal_Tab_Version_Code IN(SELECT TOP 1 Acq_Deal_Tab_Version_Code FROM Acq_Deal_Tab_Version
		WHERE Acq_Deal_Code=@Acq_Deal_Code ORDER BY Acq_Deal_Tab_Version_Code DESC)
	...
	BEGIN 
		...
		IN(SELECT TOP 1 Acq_Deal_Tab_Version_Code FROM Acq_Deal_Tab_Version WHERE Acq_Deal_Code=@Acq_Deal_Code ORDER BY Acq_Deal_Tab_Version_Code DESC)
				
		DELETE FROM Module_Status_History WHERE Module_Status_Code IN(SELECT TOP 1 Module_Status_Code FROM Module_Status_History WHERE Record_Code=@Acq_Deal_Code 
