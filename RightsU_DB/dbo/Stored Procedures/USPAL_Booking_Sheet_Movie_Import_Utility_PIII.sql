CREATE PROCEDURE [dbo].[USPAL_Booking_Sheet_Movie_Import_Utility_PIII]
(
...
BEGIN
	...
	if(@Loglevel< 2)Exec [USPLogSQLSteps] '[USPAL_Booking_Sheet_Movie_Import_Utility_PIII]', 'Step 1', 0, 'Started Procedure', 0, '' 
		SET NOCOUNT ON
...
--DECLARE @DM_Master_Import_Code INT = 119
		...
		WHERE DM_Master_Import_Code = @DM_Master_Import_Code AND Data_Type NOT LIKE '%H%' AND Sheet_Name = 'Movie' 
		...
		WHERE DM_Master_Import_Code = @DM_Master_Import_Code AND Sheet_Name = 'Movie'
		...
		BEGIN
			...
			SELECT DISTINCT ttp.DMBookingSheetDataCode, ttp.DMMasterImportCode, absd.Title_Code, absd.Title_Content_Code FROM 
				...
				INNER JOIN Title t ON LTRIM(RTRIM(ttp.TitleName)) COLLATE SQL_Latin1_General_CP1_CI_AS = LTRIM(RTRIM(t.Title_Name))
				INNER JOIN (SELECT Title_Code, Title_Content_Code FROM AL_Booking_Sheet_Details WHERE AL_Booking_Sheet_Code = @Record_Code) AS absd ON absd.Title_Code = t.Title_Code 
			...
			--	INNER JOIN Title t ON LTRIM(RTRIM(ttp.TitleData)) COLLATE SQL_Latin1_General_CP1_CI_AS = LTRIM(RTRIM(t.Title_Name))
			--	INNER JOIN (SELECT Title_Code, Title_Content_Code FROM AL_Booking_Sheet_Details WHERE AL_Booking_Sheet_Code = @Record_Code) AS absd ON absd.Title_Code = t.Title_Code 
		...
		UPDATE DM_Booking_Sheet_Data SET Record_Status = 'C', Error_Message = NULL WHERE DM_Master_Import_Code = @DM_Master_Import_Code AND  Data_Type <> 'H' AND Sheet_Name = 'Movie' AND Record_Status IS NULL
	...
	if(@Loglevel< 2)Exec [USPLogSQLSteps] '[USPAL_Booking_Sheet_Movie_Import_Utility_PIII]', 'Step 2', 0, 'Procedure Excuting Completed', 0, '' 
END