...
BEGIN
	...
	if(@Loglevel < 2)Exec [USPLogSQLSteps] '[USP_List_Syn]', 'Step 1', 0, 'Started Procedure', 0, '' 
		SET FMTONLY OFF
		...
		Set @Sql = N'SELECT * FROM (
			...
			,(SELECT DISTINCT count(ISNULL(Title_Name,Original_Title)) FROM Syn_Deal_Movie SDM  
				INNER JOIN Title Tit (NOLOCK) On Tit.title_code = SDM.Title_Code WHERE SDM.Syn_Deal_Code = SD.Syn_Deal_Code ) Cnt_DealTitles			
	...
	if(@Loglevel < 2)Exec [USPLogSQLSteps] '[USP_List_Syn]', 'Step 2', 0, 'Procedure Excution Completed', 0, '' 
End