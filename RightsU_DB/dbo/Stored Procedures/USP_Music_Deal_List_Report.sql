CREATE PROCEDURE USP_Music_Deal_List_Report        
(        
...
BEGIN                
	...
	SELECT 
		...
		STUFF((        
			...
			INNER JOIN Music_Deal_Vendor MDV ON MD.Music_Deal_Code = MDV.Music_Deal_Code        
		...
		STUFF((        
			SELECT DISTINCT ', ' + CAST(ML.Language_Name AS VARCHAR)  FROM Music_Language  ML        
			LEFT JOIN Music_Deal_Language MDL ON MD.Music_Deal_Code = MDL.Music_Deal_Code        
		...
		STUFF((        
			SELECT DISTINCT ', ' + CAST(C.Channel_Name AS VARCHAR)  FROM Channel C        
			INNER JOIN Music_Deal_Channel MDC ON MD.Music_Deal_Code = MDC.Music_Deal_Code        
		...
		ELSE        
		STUFF((        
			SELECT DISTINCT ', ' + CAST(T.Title_Name AS VARCHAR)  FROM Title T        
			INNER JOIN Music_Deal_LinkShow MDLS ON MD.Music_Deal_Code= MDLS.Music_Deal_Code        
