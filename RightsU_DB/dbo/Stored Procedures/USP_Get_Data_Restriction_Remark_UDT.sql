...
BEGIN
	...
	if(@Loglevel < 2)Exec [USPLogSQLSteps] '[USP_Get_Data_Restriction_Remark_UDT]', 'Step 1', 0, 'Started Procedure', 0, '' 
		SET FMTONLY OFF
		SET NOCOUNT ON
		-- =============================================Delete Temp Tables =============================================
		...
		-- =============================================Insert Into Temp Tables ========================================		
			...
			LEFT JOIN Territory_Details TD (NOLOCK) ON DRT.Territory_Code = TD.Territory_Code  			
			...
			INNER JOIN #Temp_Selected_Region_Code TSRC ON 
				...
				(
					...
					(
						...
						(
							SELECT C.Parent_Country_Code FROM Country C (NOLOCK) WHERE C.Country_Code=TSRC.Country_Code
						)
					)
				)
				OR
			...
			BEGIN
				...
				LEFT JOIN Language_Group_Details LGD (NOLOCK)  ON DRS.Language_Group_Code =  LGD.Language_Group_Code
			...
			BEGIN
				...
				LEFT JOIN Language_Group_Details LGD (NOLOCK) ON DRD.Language_Group_Code =  LGD.Language_Group_Code
				...
				BEGIN            
					...
					BEGIN 
						...
						IF(@Deal_Type_Code=0)
							...
							FROM Acq_Deal (NOLOCK) WHERE  Deal_Workflow_Status NOT IN ('AR', 'WA') AND Acq_Deal_Code = (SELECT TOP 1 Acq_Deal_Code FROM Acq_Deal_Rights (NOLOCK) WHERE Acq_Deal_Rights_Code = @Cur_Rights_Code)
							...
							BEGIN
								...
								FROM Territory T (NOLOCK)
								INNER JOIN Territory_Details TD (NOLOCK) ON T.Territory_Code = TD.Territory_Code
							...
							BEGIN
								IF EXISTS(SELECT TOP 1 DRS.Language_Group_Code FROM @Deal_Rights_Subtitling DRS  WHERE Language_Type = 'G')							
									...
									FROM  Language_Group  LG (NOLOCK)								
									INNER JOIN  Language_Group_Details  LGD (NOLOCK) ON LG.Language_Group_Code = LGD.Language_Group_Code
	...
	if(@Loglevel < 2)Exec [USPLogSQLSteps] '[USP_Get_Data_Restriction_Remark_UDT]', 'Step 2', 0, 'Procedure Excution Completed', 0, '' 
END