...
BEGIN
	...
	if(@Loglevel< 2)Exec [USPLogSQLSteps] '[USPMHRequisitionList]', 'Step 1', 0, 'Started Procedure', 0, ''
		...
		SELECT DISTINCT MR.MHRequestCode, MR.MHRequestTypeCode, MR.RequestID, (select TOP 1 T.Title_Name from Title T (NOLOCK) Where T.Title_Code = MR.TitleCode)
			...
			ELSE '' END
			,C.Channel_Name
			,CAST((SELECT COUNT(*) FROM MHRequestDetails MRD (NOLOCK) WHERE MRD.MHRequestCode = MR.MHRequestCode ) AS NVARCHAR)
			,STUFF((SELECT DISTINCT ', ' + CAST(ML.Music_Label_Name AS NVARCHAR) FROM MHRequestDetails MRD1 (NOLOCK)
			...
			ELSE '' END
			,( SELECT COUNT(*) FROM MHRequestDetails mrd1 (NOLOCK) WHERE MR.MHRequestCode = mrd1.MHRequestCode AND ISNULL(mrd1.IsApprove, 'P') = 'P') AS PendingCount
			,( SELECT COUNT(*) FROM MHRequestDetails mrd1 (NOLOCK) WHERE MR.MHRequestCode = mrd1.MHRequestCode AND mrd1.IsApprove = 'Y') AS AuthorisedSongsCount
		 FROM MHRequest MR (NOLOCK)
			...
			LEFT JOIN MHUsers MU (NOLOCK) ON MR.UsersCode = MU.Users_Code AND MR.VendorCode = MU.Vendor_Code
			LEFT JOIN Users U (NOLOCK) ON MU.Users_Code = U.Users_Code
			LEFT JOIN Vendor V (NOLOCK) ON V.Vendor_Code = MU.Vendor_Code
			LEFT JOIN Music_Deal_LinkShow MDL (NOLOCK) ON MDL.Title_Code = T.Title_Code
			LEFT JOIN Music_Deal MD (NOLOCK) ON MD.Music_Deal_Code = MDL.Music_Deal_Code
		Where 
		(@MusicLabel = '' OR ((select COUNT(*) from Music_Title_Label MTL (NOLOCK)
		INNER JOIN MHRequestDetails MRD (NOLOCK) ON MRD.MusicTitleCode = MTL.Music_Title_Code
	...
	if(@Loglevel< 2)Exec [USPLogSQLSteps] '[USPMHRequisitionList]', 'Step 2', 0, 'Procedure Excution Completed', 0, ''
END