...
BEGIN
	...
	if(@Loglevel < 2)Exec [USPLogSQLSteps] '[USP_DELETE_AT_Syn_Deal]', 'Step 1', 0, 'Started Procedure', 0, ''
	SET NOCOUNT ON
	BEGIN TRY
	BEGIN TRANSACTION;
	...
	IN (SELECT AT_Syn_Deal_Run_Code FROM [dbo].[AT_Syn_Deal_Run] (NOLOCK) WHERE AT_Syn_Deal_Code = @AT_Syn_Deal_Code)
	...
	(
		SELECT AT_Syn_Deal_Rights_Holdback_Code FROM AT_Syn_Deal_Rights_Holdback (NOLOCK)
		WHERE AT_Syn_Deal_Rights_Code IN
		(
			SELECT AT_Syn_Deal_Rights_Code FROM AT_Syn_Deal_Rights (NOLOCK)
	...
	(
		SELECT AT_Syn_Deal_Rights_Blackout_Code FROM AT_Syn_Deal_Rights_Blackout (NOLOCK)
	WHERE AT_Syn_Deal_Rights_Code IN
		(
			SELECT AT_Syn_Deal_Rights_Code FROM AT_Syn_Deal_Rights (NOLOCK)
			WHERE AT_Syn_Deal_Code IN (Select number FROM fn_Split_withdelemiter(@AT_Syn_Deal_Code,','))
	...
	(
	SELECT AT_Syn_Deal_Rights_Promoter_Code FROM AT_Syn_Deal_Rights_Promoter (NOLOCK) 
	WHERE AT_Syn_Deal_Rights_Code IN
		(
			SELECT AT_Syn_Deal_Rights_Code FROM AT_Syn_Deal_Rights  (NOLOCK)
			WHERE AT_Syn_Deal_Code IN (Select number FROM fn_Split_withdelemiter(@AT_Syn_Deal_Code,','))
	...
	(	
		SELECT AT_Syn_Deal_Ancillary_Code From AT_Syn_Deal_Ancillary (NOLOCK) WHERE AT_Syn_Deal_Code IN (Select number FROM fn_Split_withdelemiter(@AT_Syn_Deal_Code,','))
	) 
	DELETE FROM AT_Syn_Deal_Ancillary_Platform_Medium WHERE  AT_Syn_Deal_Ancillary_Platform_Code IN
	(
		SELECT  AT_Syn_Deal_Ancillary_Platform_Code From AT_Syn_Deal_Ancillary_Platform  (NOLOCK) WHERE  AT_Syn_Deal_Ancillary_Code IN
		(	
			SELECT AT_Syn_Deal_Ancillary_Code From AT_Syn_Deal_Ancillary (NOLOCK) WHERE AT_Syn_Deal_Code IN (Select number FROM fn_Split_withdelemiter(@AT_Syn_Deal_Code,','))
		) 
	)
	...
	(	
		SELECT AT_Syn_Deal_Revenue_Code From AT_Syn_Deal_Revenue (NOLOCK) WHERE AT_Syn_Deal_Code IN (Select number FROM fn_Split_withdelemiter(@AT_Syn_Deal_Code,','))
	) 
	...
	(
		SELECT  AT_Syn_Deal_Revenue_Costtype_Code FROM AT_Syn_Deal_Revenue_Costtype (NOLOCK) WHERE AT_Syn_Deal_Revenue_Code IN
		(	
			SELECT AT_Syn_Deal_Revenue_Code From AT_Syn_Deal_Revenue (NOLOCK) WHERE AT_Syn_Deal_Code IN (Select number FROM fn_Split_withdelemiter(@AT_Syn_Deal_Code,','))
		)
	)
	...
	if(@Loglevel < 2)Exec [USPLogSQLSteps] '[USP_DELETE_AT_Syn_Deal]', 'Step 2', 0, 'Procedure Excution Completed', 0, ''
END