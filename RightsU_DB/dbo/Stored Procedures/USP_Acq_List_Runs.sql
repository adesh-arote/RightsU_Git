...
Begin
	...
	if(@Loglevel < 2)Exec [USPLogSQLSteps] '[USP_Acq_List_Runs]', 'Step 1', 0, 'Started Procedure', 0, ''
		--DECLARE
		--@Deal_Code Int=15375,
		...
		STUFF(
				...
				INNER JOIN Acq_Deal_Run_Channel ADRC (NOLOCK) ON ADRC.Channel_Code = C.Channel_Code
		...
		FROM Acq_Deal_Run ADR (NOLOCK)
		  ...
		  INNER JOIN Acq_Deal_Run_Channel ADRC1 (NOLOCK) ON ADRT.Acq_Deal_Run_Code=ADRC1.Acq_Deal_Run_Code
		  INNER JOIN Title t (NOLOCK) ON ADRT.Title_Code = t.Title_Code
		  INNER JOIN Acq_Deal_Movie ADM (NOLOCK) ON ADR.Acq_Deal_Code = ADM.Acq_Deal_Code AND ADRT.Episode_From = ADM.Episode_Starts_From AND ADRT.Episode_To = ADM.Episode_End_To AND ADRT.Title_Code = ADM.Title_Code
		  INNER JOIN Acq_Deal AD (NOLOCK) ON ADM.Acq_Deal_Code=AD.Acq_Deal_Code
		  LEFT JOIN Acq_Deal_Run_Shows ADRS (NOLOCK) ON ADR.Acq_Deal_Run_Code = ADRS.Acq_Deal_Run_Code	
	...
	if(@Loglevel < 2)Exec [USPLogSQLSteps] '[USP_Acq_List_Runs]', 'Step 2', 0, 'Procedure Excution Completed', 0, ''
END
