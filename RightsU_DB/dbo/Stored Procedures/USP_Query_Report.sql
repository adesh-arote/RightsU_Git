...
BEGIN
--DECLARE
  --@Sql_Select VARCHAR(MAX) = 'Title_Name [Title], CBFC_Rating [CBFC Rating], Co_Production [Co Production]',  
  --@Sql_Where NVARCHAR(MAX) = 'BUSINESS_UNIT_CODE~AND~IN~1|EXPIRED~AND~=~N|IS_THEATRICAL_RIGHT~=~N|TIT.TITLE_CODE~AND~IN~175|',  
  --@Column_Count INT = 3,  
  --@Column_Names VARCHAR(MAX) = 'Title,CBFC Rating,Co Production,',  
  --@Report_Query_Code INT=0,  
  --@Subtitling_Codes VARCHAR(MAX) = ' ',  
  --@Dubbing_Codes VARCHAR(MAX) = '',  
  --@Country_Codes VARCHAR(MAX) = ' ',  
  ...
  --@Category_Codes VARCHAR(MAX) = ' '
	...
	--''', @Channel_Codes=''' + CAST(ISNULL(@Channel_Codes, '') AS VARCHAR(MAX)) +
	--''', @Category_Codes=''' + CAST(ISNULL(@Category_Codes, '') AS VARCHAR(MAX)) + ''''

	Declare @Loglevel int
	select @Loglevel = Parameter_Value from System_Parameter_New where Parameter_Name='loglevel'
	if(@Loglevel<2)Exec [USPLogSQLSteps] '[USP_Query_Report]', 'Step 1', 0, 'Started Procedure', 0, ''
		...
		SELECT @ColNames = REPLACE(@ColNames,'Exclusivity','Exclusive')
		...
		DECLARE @tblTitle TABLE(Title_Code INT, Original_Title NVARCHAR(MAX), Original_Dubbed NVARCHAR(MAX), Year_of_Release INT,CBFC_Rating INT, Co_Production INT)
		...
		PRINT '----------'
			...
			BEGIN
				...
				EXEC('SELECT Acq_Deal_code FROM Acq_Deal (NOLOCK) WHERE  Deal_Workflow_Status NOT IN (''AR'', ''WA'') AND Business_Unit_Code '+@theOp+' ('+@Value+')' )
				...
				SELECT Title_Code, Original_Title, ISNULL(OD.Columns_Value,'NA'), Year_Of_Production, mec.Columns_Value_Code as CBFC_Rating, mec1.Columns_Value_Code as Co_Production, t.Duration_In_Min 
				FROM Title t  (NOLOCK)
				LEFT JOIN #OriDub_Title_Code OD ON OD.Record_Code = T.Title_Code
				LEFT JOIN Map_Extended_Columns mec (NOLOCK) on mec.Record_Code = t.Title_Code and mec.Columns_Code IN (5)
				...
				SELECT distinct ad.Acq_Deal_Code, adr.Acq_Deal_Rights_Code, adrt.Title_Code, adr.Actual_Right_Start_Date, adr.Actual_Right_End_Date,
					...
					CASE WHEN (SELECT COUNT(Acq_Deal_Rights_Holdback_Code) FROM Acq_Deal_Rights_Holdback ADRH (NOLOCK) WHERE ADRH.Acq_Deal_Rights_Code = ADR.Acq_Deal_Rights_Code) > 0 THEN 'Y' ELSE 'N' END AS Is_Holdback
				...
				FROM Acq_Deal ad (NOLOCK)
				INNER JOIN #BUWiseAcq_Deal_Code BUAD ON BUAD.Acq_Deal_code = AD.Acq_Deal_code
				INNER JOIN Role DC (NOLOCK)  on AD.Role_Code = DC.Role_Code
				LEFT JOIN Acq_Deal_Movie adm (NOLOCK)  on adm.Acq_Deal_Code = ad.Acq_Deal_Code
				LEFT JOIN Acq_Deal_Cost adc (NOLOCK)  on adc.Acq_Deal_Code = ad.Acq_Deal_Code
			...
			BEGIN
				...
				BEGIN	
					IF(@logicalConnect = 'AND')
					BEGIN
						EXEC ('DELETE FROM #tempTitle WHERE Title_Code '+@theOp+' ( Select Title_Code FROM Title_Talent (NOLOCK) WHERE Talent_Code IN ('+@Value+') AND Role_Code = 1)')
					...
					BEGIN
						...
						INSERT INTO @tblTitle(Title_Code,Original_Title,Original_Dubbed,Year_of_Release, CBFC_Rating)
						EXEC ('Select Distinct tg.Title_Code,t.Original_Title,ISNULL(OD.Columns_Value,''NA''),t.Year_Of_Production, mec.Columns_Value_Code as CBFC_Rating FROM Title_Talent tg  (NOLOCK)
							   INNER JOIN Title t (NOLOCK)  ON t.Title_Code = tg.Title_Code AND tg.Role_Code = 1
							   LEFT JOIN #OriDub_Title_Code OD ON OD.Record_Code = T.Title_Code
							   LEFT JOIN Map_Extended_Columns mec (NOLOCK)  on mec.Record_Code = t.Title_Code
				...
				BEGIN
					IF(@logicalConnect = 'AND')
					BEGIN
						EXEC ('DELETE FROM #tempTitle WHERE Title_Code '+@theOp+' ( Select Title_Code FROM Title_Talent (NOLOCK) WHERE Talent_Code IN ('+@Value+') AND Role_Code = 2)')
				...
				BEGIN
					IF(@logicalConnect = 'AND')
					BEGIN
						EXEC ('DELETE FROM #tempTitle WHERE Title_Code '+@theOp+' (SELECT Title_Code FROM Title_Geners (NOLOCK) WHERE Genres_Code IN ('+@Value+'))')
					...
					BEGIN
						...
						INSERT INTO @tblTitle(Title_Code,Original_Title,Original_Dubbed,Year_of_Release, CBFC_Rating)
						EXEC ('Select Distinct tg.Title_Code,t.Original_Title,ISNULL(OD.Columns_Value,''NA''),t.Year_Of_Production, mec.Columns_Value_Code as CBFC_Rating FROM Title_Geners tg (NOLOCK) 
				...
				BEGIN
					...
					BEGIN
						...
						INSERT INTO @tblTitle (Title_Code, Original_Title,Original_Dubbed, Year_of_Release, CBFC_Rating)
						EXEC ('SELECT DISTINCT t.Title_Code, t.Original_Title,ISNULL(OD.Columns_Value,''NA''), t.Year_Of_Production, mec.Columns_Value_Code as CBFC_Rating FROM Title T (NOLOCK)
								LEFT JOIN #OriDub_Title_Code OD ON OD.Record_Code = T.Title_Code
								LEFT JOIN Map_Extended_Columns mec (NOLOCK)  on mec.Record_Code = t.Title_Code
			...
			BEGIN
				...
				BEGIN
					...
					BEGIN
						...
						INNER JOIN Role DC (NOLOCK)  on AD.Role_Code = DC.Role_Code AND tr.Acq_Deal_Code NOT IN (Select Acq_Deal_Code FROM #tempDeal)
						LEFT JOIN Acq_Deal_Movie adm (NOLOCK)  on adm.Acq_Deal_Code = ad.Acq_Deal_Code
						LEFT JOIN Acq_Deal_Cost adc (NOLOCK)  on adc.Acq_Deal_Code = ad.Acq_Deal_Code
				...
				BEGIN
					...
					BEGIN
						...
						Acq_Deal_Rights_Code
								' + @theOp + ' (SELECT Acq_Deal_Rights_Code FROM Acq_Deal_Rights_Territory (NOLOCK) WHERE Acq_Deal_Rights_Code Is Not Null 
						AND (( 
				...
				BEGIN
					...
					BEGIN
						EXEC ('DELETE FROM #tempRights WHERE Acq_Deal_Rights_Code
								' + @theOp + ' (SELECT Acq_Deal_Rights_Code FROM Acq_Deal_Rights_Subtitling (NOLOCK) WHERE Acq_Deal_Rights_Code Is Not Null 
							AND (( 
				...
				BEGIN
					...
					BEGIN
						...
						EXEC ('DELETE FROM #tempRights WHERE 
						Acq_Deal_Rights_Code 
								 '+@theOp+' (SELECT Acq_Deal_Rights_Code FROM Acq_Deal_Rights_Dubbing (NOLOCK) WHERE Acq_Deal_Rights_Code Is Not Null 
							AND (( 
									Language_Code IN ('+@Value+')
									AND Language_Type = ''L''
								) 
				...
				BEGIN
					...
					INNER JOIN #BUWiseAcq_Deal_Code BUAD ON BUAD.Acq_Deal_code = ad.Acq_Deal_code
					INNER JOIN Acq_Deal_Rights adr  (NOLOCK) ON adr.Acq_Deal_Code = ad.Acq_Deal_Code 
					INNER JOIN Acq_Deal_Rights_Platform P (NOLOCK)  ON P.Acq_Deal_Rights_Code = ADR.Acq_Deal_Rights_Code
			...
			BEGIN
				...
				BEGIN
					...
					SELECT DISTINCT adr.Acq_Deal_Code,adrt.Title_Code FROM Acq_Deal_Run_Channel adrc (NOLOCK)
		...
		BEGIN
			...
			BEGIN
				...
				BEGIN
					...
					BEGIN	
						...
						FROM #tempRights TR
						INNER JOIN Acq_Deal_Rights_Platform ADRP (NOLOCK)  ON ADRP.Acq_Deal_Rights_Code = TR.Acq_Deal_Rights_Code
			...
			BEGIN
				...
				BEGIN
					...
					UPDATE tt SET tt.Duration_In_Min = CASE WHEN t.Duration_In_Min IS NULL THEN '' ELSE t.Duration_In_Min END
					FROM #tempTitle tt
					LEFT JOIN Title t ON t.Title_Code = tt.Title_Code
				END
			END
			...
			BEGIN
				...
				IF (UPPER(@OutputColsNames) =  'PUBLIC NOTICE')
				BEGIN
					Print 'Executing Column ->' + @OutputColsNames
					Print convert(varchar, getdate(), 114)
					IF(@TableColumns <> '')
		...
		--Added by Priyal for exclusivity colname change
		DECLARE @Cid INT = 0
		CREATE TABLE #t (id INT IDENTITY(1,1),colname VARCHAR(50))
		INSERT Into #t (colname) SELECT * FROM fnSplitString(@ColNames,',')

		SELECT @Cid = ISNULL(id,0) FROM #t WHERE colname='Exclusive'
		DROP TABLE #t
		...
		IF(@Cid > 0 )
		BEGIN
			EXEC ( 'UPDATE #tempOutput set COL'+@Cid + '=''Exclusivity'' where COL1 = '''+@ColumnOne+'''')
		END
		--Added by Priyal for exclusivity colname change
	...
	if(@Loglevel<2)Exec [USPLogSQLSteps] '[USP_Query_Report]', 'Step 2', 0, 'Procedure Excuting Completed', 0, ''
END