CREATE PROCEDURE [dbo].[USP_Validate_Rights_Duplication_UDT_Acq]
(
...
Begin
	...
	if(@Loglevel < 2)Exec [USPLogSQLSteps] '[USP_Validate_Rights_Duplication_UDT_Acq]', 'Step 1', 0, 'Started Procedure', 0, ''
			...
			@Right_Start_Date=dr.Right_Start_Date, @Right_End_Date=dr.Right_End_Date,
			--@Right_End_Date=ISNULL(dr.Right_End_Date,DATEADD(YEAR,100,dr.Right_Start_Date)),
			@Right_Type=dr.Right_Type, @Is_Exclusive=dr.Is_Exclusive,@Is_Tentative=dr.Is_Tentative,
			@Is_Sub_License=dr.Is_Sub_License, @Sub_License_Code=dr.Sub_License_Code,
			@Is_Title_Language_Right=dr.Is_Title_Language_Right, @Title_Code=ISNULL(dr.Title_Code,0),
			@Platform_Code=ISNULL(dr.Platform_Code,0), @Check_For = dr.Check_For, @Is_Theatrical_Right=ISNULL(dr.Is_Theatrical_Right,'N'),
			@Buyback_Syn_rights_code = dr.Buyback_Syn_rights_code
			FROM @Deal_Rights dr
			...
			BEGIN  /* insert in temporary tables*/
				...
				Select  ADR.Acq_Deal_Code, ADR.Acq_Deal_Rights_Code, ADR.Right_Type, ADR.Actual_Right_Start_Date, ADR.Actual_Right_End_Date, ADR.Is_Sub_License, ADR.Is_Title_Language_Right
				,(Select Count(*) From Acq_Deal_Rights_Subtitling a (NOLOCK)  Where a.Acq_Deal_Rights_Code = ADR.Acq_Deal_Rights_Code) Sub_Cnt
				,(Select Count(*) From Acq_Deal_Rights_Dubbing a (NOLOCK)  Where a.Acq_Deal_Rights_Code = ADR.Acq_Deal_Rights_Code) Dub_Cnt
				, 'Combination Conflict with other Rights' ErrorMSG
					InTo #Acq_Deal_Rights From Acq_Deal_Rights ADR (NOLOCK) 
				Where ADR.Acq_Deal_Code is not null	AND ISNULL(ADR.Buyback_Syn_rights_code,0) =  ISNULL(@Buyback_Syn_rights_code,0)
				And ADR.Acq_Deal_Code In (
					Select AD.Acq_Deal_Code From Acq_Deal AD (NOLOCK)  Where 
					AD.Deal_Workflow_Status NOT IN ('AR', 'WA') AND
					(
						AD.Is_Master_Deal = 'Y' OR (ISNULL(AD.Deal_Type_Code,1) = @Deal_Type_Code AND Is_Master_Deal = 'N')
					)
				)
				...
				(
					...
					(
						(
							...
							OR ((ADR.Right_Type = 'M'  AND ((@Right_Type = 'Y' AND  @Is_Tentative = 'N') OR @Right_Type = 'U' OR @Right_Type = 'M'))) -- Case 2
							OR ((ADR.Right_Type = 'U' AND ((@Right_Type = 'Y' AND @Is_Tentative = 'N') OR @Right_Type = 'M'))) -- Case 3
						)
						/*
						(
						...
						*/				
						...
						( Convert(DateTime, @Right_Start_Date, 103) between Convert(DateTime, ADR.Actual_Right_Start_Date, 103) and Convert(DateTime, ISNULL(ADR.Actual_Right_End_Date,'31DEC9999'), 103)
							OR Convert(DateTime, ISNULL(@Right_End_Date,'31DEC9999'), 103) between Convert(DateTime, ADR.Actual_Right_Start_Date, 103) and Convert(DateTime, ISNULL(ADR.Actual_Right_End_Date,'31DEC9999'), 103)
							OR Convert(DateTime, ADR.Actual_Right_Start_Date, 103) between Convert(DateTime, @Right_Start_Date, 103) and Convert(DateTime, ISNULL(@Right_End_Date,'31DEC9999'), 103)
							OR Convert(DateTime,ISNULL(ADR.Actual_Right_End_Date,'31DEC9999'), 103) between Convert(DateTime, @Right_Start_Date, 103) and Convert(DateTime, ISNULL(@Right_End_Date,'31DEC9999'), 103)							
						)
						OR
						(				
							/******************Case 4***********************************/
							...
							(
								ADR.Right_Type = 'Y' AND  ADR.Is_Tentative = 'Y' AND 
								(
									@Right_Type = 'U' 
									OR 
									(
										(@Right_Type = 'M' OR (@Right_Type= 'Y' AND @Is_Tentative = 'N')) AND Convert(DateTime, @Right_End_Date, 103) >= Convert(DateTime,ADR.Actual_Right_Start_Date, 103)
									)
									OR (@Right_Type= 'Y' AND @Is_Tentative = 'Y')
								)
							) -- case 4					
							OR
							/******************Case 5***********************************/
							...
							((ADR.Right_Type = 'M' OR (ADR.Right_Type = 'Y' AND ADR.Is_Tentative = 'N')) AND
							(
							 (@Right_Type='Y' AND @Is_Tentative = 'Y') OR (@Right_Type='U')
							)
							...
							OR (ADR.Right_Type = 'U' AND  ((@Right_Type = 'U') OR (@Right_Type = 'Y' AND @Is_Tentative = 'Y')))  --Case 6
							--OR
				...
				AND
				(
					 (ISNULL(@Title_Code,0) = 0 AND ISNULL(@Platform_Code,0) = 0) AND ADR.Acq_Deal_Rights_Code not in (@Deal_Rights_Code) OR (ISNULL(@Title_Code,0) <> 0)
					--OR
				...
				BEGIN
					DELETE FROM #Acq_Deal_Rights_Platform WHERE Platform_Code IN (SELECT Platform_Code FROM [Platform] (NOLOCK)  WHERE ISNULL(Is_No_Of_Run, '') = 'Y')
				END
				...
				From (
					...
					from #Acq_Deal_Rights ADR 
					Inner Join Acq_Deal_Rights_Territory ADRT (NOLOCK)  on ADRT.Acq_Deal_Rights_Code = ADR.Acq_Deal_Rights_Code
				...
				BEGIN
					...
					Select Acq_Deal_Rights_Code, a.Language_Type, a.Language_Group_Code, Language_Code From (
						...
						From #Acq_Deal_Rights ADR 
						inner join Acq_Deal_Rights_Subtitling ADRS (NOLOCK)  on ADRS.Acq_Deal_Rights_Code = ADR.Acq_Deal_Rights_Code
				...
				BEGIN
					...
					From (
						...
						From #Acq_Deal_Rights ADR
						inner join Acq_Deal_Rights_Dubbing ADRD  (NOLOCK) on ADRD.Acq_Deal_Rights_Code = ADR.Acq_Deal_Rights_Code
	...
	if(@Loglevel < 2)Exec [USPLogSQLSteps] '[USP_Validate_Rights_Duplication_UDT_Acq]', 'Step 2', 0, 'Procedure Excuting Completed', 0, ''
END