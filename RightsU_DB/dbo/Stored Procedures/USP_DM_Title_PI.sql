...
BEGIN  
	...
	if(@Loglevel < 2)Exec [USPLogSQLSteps] '[USP_DM_Title_PI]', 'Step 1', 0, 'Started Procedure', 0, ''
	select @Loglevel = Parameter_Value from System_Parameter_New where Parameter_Name='loglevel'
		  ...
		  EXEC USP_Validate_Title_Import  @DM_Master_Import_Code     
		   ...
		   IF EXISTS(SELECT Record_Status FROM DM_Title (NOLOCK) WHERE ISNULL(RTRIM(LTRIM(Record_Status)),'') = 'E' AND DM_Master_Import_Code = @DM_Master_Import_Code)  
			BEGIN
				SELECT   
	...
	if(@Loglevel < 2)Exec [USPLogSQLSteps] '[USP_DM_Title_PI]', 'Step 2', 0, 'Procedure Excution Completed', 0, ''
END