...
BEGIN
	...
	if(@Loglevel<2)Exec [USPLogSQLSteps] '[USP_Report_PlatformWise_Acquisition_Neo_Avail]', 'Step 1', 0, 'Started Procedure', 0, ''
		...
		BEGIn
			...
			AND td.Country_Code IN (SELECT c.Country_Code FROM Country c (NOLOCK) WHERE ISNULL(c.Is_Active,'N')='Y')
		...
		BEGIn
			INSERT INTO #Temp_Country_Neo
			SELECT DISTINCT Country_Code FROM Report_Territory_Country td (NOLOCK) WHERE td.Report_Territory_Code IN (
				SELECT REPLACE(number, 'T', '') FROM fn_Split_withdelemiter(@Country_Code, ',') WHERE number LIKE 'T%' AND number NOT IN('0')
			) AND td.Country_Code NOT IN (
				SELECT tc.Country_Code FROM #Temp_Country_Neo tc
			)
		...
		BEGIN
			INSERT INTO #Temp_Country_Neo
			SELECT c.Country_Code FROM Country c (NOLOCK) WHERE ISNULL(c.Is_Active, 'N') = 'Y'
		END
	
		IF NOT EXISTS(SELECT * FROM #Temp_Language_Neo WHERE Language_Type = 'S')
		BEGIN
			INSERT INTO #Temp_Language_Neo
			SELECT Language_Code, 'S' FROM Language (NOLOCK)
		END
	
		IF NOT EXISTS(SELECT * FROM #Temp_Language_Neo WHERE Language_Type = 'D')
		BEGIN
			INSERT INTO #Temp_Language_Neo
			SELECT Language_Code, 'D' FROM Language (NOLOCK)
		END
	
		IF NOT EXISTS(SELECT * FROM #Temp_Title_Neo)
		BEGIN
			INSERT INTO #Temp_Title_Neo
			SELECT DISTINCT Title_Code FROM Acq_Deal_Movie (NOLOCK)
		END
		...
		BEGIN
			...
			SELECT Sub_License_Code FROM Sub_License (NOLOCK)
		END
		END
		...
		INSERT INTO #Temp_Right_Neo(Acq_Deal_Code, Acq_Deal_Rights_Code, Restriction_Remarks, Platform_Code, Title_Code,
			   Is_Master_Deal, Remarks, Rights_Remarks, Sub_Deal_Restriction_Remark, Rights_Start_Date, Rights_End_Date, Is_Exclusive, Sub_License_Code, Episode_From, Episode_To, Is_Title_Language_Right,Due_Diligence,Category_Name,Deal_Type_Name)
		SELECT ar.Acq_Deal_Code, ar.Acq_Deal_Rights_Code, ar.Restriction_Remarks, adrp.Platform_Code, ADRT.Title_Code,
			   ...
			   (STUFF((
				...
				FROM Acq_Deal AD_Tmp (NOLOCK)
				INNER JOIN Acq_Deal_Movie Adm_Tmp (NOLOCK) ON AD_Tmp.Acq_Deal_Code = Adm_Tmp.Acq_Deal_Code AND AD_Tmp.Master_Deal_Movie_Code_ToLink = ADM.Acq_Deal_Movie_Code
				INNER JOIN Acq_Deal_Rights ADR_Tmp (NOLOCK) ON adr_Tmp.Acq_Deal_Code = AD_Tmp.Acq_Deal_Code 
				INNER JOIN Acq_Deal_Rights_Title ADRT_Tmp (NOLOCK) ON ADR_Tmp.Acq_Deal_Rights_Code = ADRT_Tmp.Acq_Deal_Rights_Code AND ADRT_Tmp.Title_Code = Adm_Tmp.Title_Code 
				...
				, adrt.Episode_From, adrt.Episode_To, ar.Is_Title_Language_Right,ADM.Due_Diligence,C.Category_Name,DT.Deal_Type_Name
		FROM Acq_Deal_Rights AS ar (NOLOCK)
		INNER JOIN Acq_Deal_Rights_Platform AS adrp (NOLOCK) ON adrp.Acq_Deal_Rights_Code = ar.Acq_Deal_Rights_Code
		INNER JOIN Acq_Deal_Rights_Title AS adrt (NOLOCK) ON adrt.Acq_Deal_Rights_Code = ar.Acq_Deal_Rights_Code
		INNER JOIN #Temp_Title_Neo ttn ON adrt.Title_Code = ttn.Title_Code
		INNER JOIN Acq_Deal ad (NOLOCK) ON ar.Acq_Deal_Code = ad.Acq_Deal_Code
		INNER JOIN Acq_Deal_Movie ADM (NOLOCK) ON ad.Acq_Deal_Code = ADM.Acq_Deal_Code AND ADRT.Title_Code = ADM.Title_Code AND ADRT.Episode_From = ADM.Episode_Starts_From AND ADRT.Episode_To = ADM.Episode_End_To
		...
		AND (ad.Business_Unit_Code IN (SELECT CAST(number as INT) from [dbo].[fn_Split_withdelemiter](@BU_Code,',')))
		...
		AND (CAST(ad.Deal_Type_Code AS VARCHAR) IN(SELECT DISTINCT number FROM fn_Split_withdelemiter(@Deal_Type_Code , ','))
		--OR (CAST(ad.Deal_Type_Code AS VARCHAR) NOT IN(SELECT DISTINCT number FROM fn_Split_withdelemiter(@Deal_Type_Code , ','))
		--AND CAST(@Deal_Type_Code AS VARCHAR) NOT IN(SELECT DISTINCT number FROM fn_Split_withdelemiter(@Deal_Type_Code , ','))) OR CAST(@Deal_Type_Code AS VARCHAR)= '0'
		 )
		AND (
		...
		BEGIN
			...
			Sub_License_Code, Episode_From, Episode_To, Is_Title_Language_Right,Due_Diligence,Category_Name,Deal_Type_Name)
			SELECT DISTINCT tr.Acq_Deal_Rights_Code , tr.Title_Code, tr.Platform_Code, 
						   ...
						   , Episode_From, Episode_To, Is_Title_Language_Right,tr.Due_Diligence,tr.Category_Name,tr.Deal_Type_Name
			FROM 
			#Temp_Right_Neo tr 
			INNER JOIN Acq_Deal_Rights_Territory adrter (NOLOCK) on adrter.Acq_Deal_Rights_Code = tr.Acq_Deal_Rights_Code
			LEFT JOIN Territory_Details td (NOLOCK) on (td.Territory_Code = adrter.Territory_Code AND adrter.Territory_Type = 'G')
		...
		BEGIN
			...
			SELECT DISTINCT tr.Acq_Deal_Rights_Code , tr.Title_Code, tr.Platform_Code, 
				   CASE WHEN adrter.Territory_Type = 'G' THEN td.Country_Code ELSE adrter.Country_Code END, 
				   tr.Rights_Start_Date, tr.Rights_End_Date, tr.Is_Exclusive, ISNULL(tr.Sub_License_Code, 0), Episode_From, Episode_To, Is_Title_Language_Right,tr.Due_Diligence,tr.Category_Name,tr.Deal_Type_Name
			FROM 
			#Temp_Right_Neo tr 
			INNER JOIN Acq_Deal_Rights_Territory adrter (NOLOCK) on adrter.Acq_Deal_Rights_Code = tr.Acq_Deal_Rights_Code
			LEFT JOIN Territory_Details td (NOLOCK) on (td.Territory_Code = adrter.Territory_Code AND adrter.Territory_Type = 'G')
		...
		BEGIN
			...
			INNER JOIN Title tit (NOLOCK) ON AA.Title_Code = tit.Title_Code
		...
		BEGIN
			...
			Select Distinct Acq_Deal_Rights_Code, STUFF((
				Select Distinct ',' + 
				CAST(Case When Language_Type = 'G' Then lgd.Language_Code Else adrs.Language_Code End AS VARCHAR) From Acq_Deal_Rights_Subtitling adrs (NOLOCK)
				LEFT JOIN Language_Group_Details lgd (NOLOCK) ON lgd.Language_Group_Code = adrs.Language_Group_Code
		...
		BEGIN
			...
			Select Distinct Acq_Deal_Rights_Code, STUFF((
				Select Distinct ',' + 
				CAST(Case When Language_Type = 'G' Then lgd.Language_Code Else adrd.Language_Code End AS VARCHAR) From Acq_Deal_Rights_Dubbing adrd (NOLOCK)
				LEFT JOIN Language_Group_Details lgd (NOLOCK) ON lgd.Language_Group_Code = adrd.Language_Group_Code
		...
		BEGIN
			...
			-----------------Get Language Names for #Temp_Main_AP
		...
		PRINT 'PA-STEP-8 Populate #Temp_Main_AP ' + convert(varchar(30),getdate() ,109)
		BEGIN
		...
		CAST('' AS CHAR(1)) AS Region_Type , CAST('' AS VARCHAR(2000)) AS Country_Names,AA.Due_Diligence,AA.Category_Name,AA.Deal_Type_Name
		INTO #TMP_MAIN_AP
		FROM #Temp_Dates TD
		INNER JOIN #Avail_Acq_Neo AA ON AA.Acq_Deal_Rights_Code = TD.Acq_Deal_Rights_Code
		LEFT JOIN Acq_Deal_Rights_Holdback hb (NOLOCK) ON hb.Acq_Deal_Rights_Code = TD.Acq_Deal_Rights_Code
		...
		LEFT JOIN Acq_Deal_Rights_Holdback_Platform hbp (NOLOCK) On hb.Acq_Deal_Rights_Holdback_Code = hbp.Acq_Deal_Rights_Holdback_Code and AA.Platform_Code = hbp.Platform_Code
		LEFT JOIN Acq_Deal_Rights_Holdback_Territory hbt (NOLOCK) On hb.Acq_Deal_Rights_Holdback_Code = hbt.Acq_Deal_Rights_Holdback_Code AND AA.Country_Code = hbt.Country_Code
		...
		Select Distinct Acq_Deal_Rights_Holdback_Code, Holdback_Type, Cast('' As NVarchar(Max)) HBComments,  Cast(0 As Int) HB_Run_After_Release_No, Cast(0 As Int) HB_Run_After_Release_Units
			InTo #MainAH_AP 
			From #TMP_MAIN_AP Where Acq_Deal_Rights_Holdback_Code Is Not Null
		...
		Update thb
			...
			INNER JOIN Title_Release_Region trr (NOLOCK) On tr.Title_Release_Code = trr.Title_Release_Code And Country_Code Is Not Null
			UNION
			Select Distinct tr.Title_Release_Code, trd.Country_Code 
			From Title_Release tr (NOLOCK)
			INNER JOIN #MainTit_AP mt On tr.Title_Code = mt.Title_Code
			INNER JOIN Title_Release_Region trr (NOLOCK) On tr.Title_Release_Code = trr.Title_Release_Code And Territory_Code Is Not Null
			INNER JOIN Territory_Details trd (NOLOCK) On trr.Territory_Code = trd.Territory_Code
			...
			INNER JOIN #TMP_MAIN_AP tm On tm.Title_Code = tr.Title_Code AND tm.Title_Code = mt.Title_Code 
		...
		--ALTER TABLE #TMP_MAIN_AP ADD Country_Cd_Str VARCHAR(2000),
		--					  Region_Type CHAR(1),
		...
		(
			(
				SELECT DISTINCT ',' + CAST(Country_Code AS VARCHAR) FROM #TMP_MAIN_AP t1 
				WHERE 
		...
		SELECT DISTINCT Country_Cd_Str, NULL FROM #TMP_MAIN_AP
		...
		BEGIN
			...
			DELETE FROM #TMP_MAIN_AP WHERE Country_Cd_Str <> @CntryStr
		...
		BEGIN
			...
			DELETE FROM #TMP_MAIN_AP WHERE Country_Cd_Str COLLATE SQL_Latin1_General_CP1_CI_AS NOT IN (SELECT Country_Codes COLLATE SQL_Latin1_General_CP1_CI_AS FROM #Temp_Country_Names_Neo)
		END
		...
		Holdback_Release_Date, Holdback_On_Platform_Code,Due_Diligence,Category_Name,Deal_Type_Name
		INTO #Temp_Main_Ctr_Neo
		FROM (
			SELECT ROW_NUMBER() OVER(
										PARTITION BY Acq_Deal_Rights_Code ,Title_Code, Episode_From, Episode_To, Platform_Code, Country_Code, Region_Type, 
		...
		BEGIN
			...
			BEGIN
				...
				INTO #Temp_Plt_Names_AP
				FROM #Temp_Main_Ctr_Neo tm
				

				DELETE TM 
				FROM 
				#Temp_Plt_Names_AP TM
				WHERE TM.Platform_Str NOT IN
				(SELECT DISTINCT Platform_Str
				FROM #Temp_Plt_Names_AP tm
				WHERE 
					(SELECT COUNT(number) 
				...
				DELETE FROM #Temp_Main_Ctr_Neo WHERE Platform_Str NOT IN (SELECT tpn.Platform_Str FROM #Temp_Plt_Names_AP tpn)
			print '--------------2'
			END
			END
		END
	   ...
	   DROP TABLE #Avail_Dubb_Neo
		...
		BEGIN
			SELECT DISTINCT Acq_Deal_Rights_Code, Remarks, Rights_Remarks, 
					Restriction_Remarks,
					Sub_Deal_Restriction_Remark,Due_Diligence,Category_Name,Deal_Type_Name
			INTO #Temp_Right_Remarks_AP
			FROM #Temp_Right_Neo

			--Select * from #Temp_Right_Remarks_AP
		END
		...
		SELECT DISTINCT 
				...
				Reverse_Holdback COLLATE SQL_Latin1_General_CP1_CI_AS As Reverse_Holdback,trr.Due_Diligence,trr.Category_Name,trr.Deal_Type_Name

			FROM 
				#Temp_Main_Ctr_Neo tm
				INNER JOIN #Temp_Right_Remarks_AP trr ON trr.Acq_Deal_Rights_Code = tm.Acq_Deal_Rights_Code
				INNER JOIN #Temp_Titles_Neo trt ON trt.Title_Code = tm.Title_Code
				INNER JOIN Platform pt (NOLOCK) ON pt.Platform_Code = tm.Platform_Code
				LEFT JOIN Sub_License sl (NOLOCK) ON tm.Sub_License_Code = sl.Sub_License_Code
				LEFT JOIN Platform pt1 (NOLOCK) ON pt1.Platform_Code = tm.Holdback_On_Platform_Code
		...
		print 'PA-STEP-13 .1.2'
			SELECT DISTINCT 
				...
				Reverse_Holdback COLLATE SQL_Latin1_General_CP1_CI_AS As Reverse_Holdback,tm.Due_Diligence,tm.Category_Name,tm.Deal_Type_Name
			FROM #Temp_Main_Ctr_Neo tm
			INNER JOIN #Temp_Titles_Neo trt ON trt.Title_Code = tm.Title_Code
			INNER JOIN Platform pt (NOLOCK) ON pt.Platform_Code = tm.Platform_Code
			LEFT JOIN Sub_License sl (NOLOCK) ON ISNULL(tm.Sub_License_Code, 0) = ISNULL(sl.Sub_License_Code, 0)
			LEFT JOIN Platform pt1 (NOLOCK) ON pt1.Platform_Code = tm.Holdback_On_Platform_Code
			LEFT JOIN #MainAH_AP hb On tm.Acq_Deal_Rights_Holdback_Code = hb.Acq_Deal_Rights_Holdback_Code
			WHERE (ISNULL(tm.Holdback_Release_Date,'')<>'' AND CAST(tm.Holdback_Release_Date AS DATETIME) < CAST(ISNULL(tm.Rights_End_Date, '31Dec9999') AS DATETIME)) OR
			ISNULL(tm.Holdback_Release_Date,'') = ''
	...
	END
		...
		IF OBJECT_ID('tempdb..#ReleaseCountry_AP') IS NOT NULL DROP TABLE #ReleaseCountry_AP
		...
		IF OBJECT_ID('tempdb..#Temp_Main_AP') IS NOT NULL DROP TABLE #Temp_Main_AP
		...
		IF OBJECT_ID('tempdb..#Temp_Plt_Names_AP') IS NOT NULL DROP TABLE #Temp_Plt_Names_AP
		IF OBJECT_ID('tempdb..#Temp_Right_Neo') IS NOT NULL DROP TABLE #Temp_Right_Neo
		IF OBJECT_ID('tempdb..#Temp_Right_Remarks_AP') IS NOT NULL DROP TABLE #Temp_Right_Remarks_AP
		...
		IF OBJECT_ID('tempdb..#TempRHB_AP') IS NOT NULL DROP TABLE #TempRHB_AP
	...
	if(@Loglevel<2)Exec [USPLogSQLSteps] '[USP_Report_PlatformWise_Acquisition_Neo_Avail]', 'Step 2', 0, 'Procedure Excuting Completed', 0, ''
END