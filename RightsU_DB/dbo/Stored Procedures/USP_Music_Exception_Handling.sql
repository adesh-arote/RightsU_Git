...
BEGIN
	...
	if(@Loglevel<2)Exec [USPLogSQLSteps] '[USP_Music_Exception_Handling]', 'Step 1', 0, 'Started Procedure', 0, '' 
		...
		;WITH CTE AS(
			...
			INNER JOIN Title_Content AS TC (NOLOCK) ON TC.Title_Content_Code = CML.Title_Content_Code
			...
			INNER JOIN Music_Album AS MA (NOLOCK) ON MT.Music_Album_Code = MA.Music_Album_Code
			...
			LEFT JOIN Music_Label AS ML (NOLOCK) ON ML.Music_Label_Code = MST.Music_Label_Code
	...
	if(@Loglevel<2)Exec [USPLogSQLSteps] '[USP_Music_Exception_Handling]', 'Step 2', 0, 'Procedure Excution Completed', 0, '' 
END