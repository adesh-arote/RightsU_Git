...
BEGIN   
	...
	if(@Loglevel < 2)Exec [USPLogSQLSteps] '[USP_DM_Title_PIV]', 'Step 1', 0, 'Started Procedure', 0, ''   
	   ...
	   -- Director Data      
		...
		(      
			SELECT [Music_Label] MusicLabelName FROM DM_Title (NOLOCK) WHERE ISNULL([Music_Label], '') <> '' AND DM_Master_Import_Code = @DM_Master_Import_Code      
		) AS a      
      
		INSERT INTO #Temp_Language(Name, RoleExists)      
		SELECT DISTINCT a.Language_Name, 'N' FROM      
		(      
			SELECT [Original Language (Hindi)] Language_Name FROM DM_Title (NOLOCK) WHERE ISNULL([Original Language (Hindi)], '') <> '' AND DM_Master_Import_Code = @DM_Master_Import_Code      
		) AS a      

		INSERT INTO #Temp_Original_Language (Name, RoleExists)      
		SELECT DISTINCT a.Language_Name, 'N' FROM      
		(      
			SELECT [Original_Language] Language_Name FROM DM_Title (NOLOCK) WHERE ISNULL([Original_Language], '') <> '' AND DM_Master_Import_Code = @DM_Master_Import_Code      
		) AS a 
      
		INSERT INTO #Temp_Deal_Type(Name, RoleExists)      
		SELECT DISTINCT a.Title_Type, 'N' FROM      
		(      
			SELECT [Title Type] Title_Type FROM DM_Title (NOLOCK) WHERE ISNULL([Title Type], '') <> '' AND DM_Master_Import_Code = @DM_Master_Import_Code      
		) AS a      

		INSERT INTO #Temp_Program_Category(Name, RoleExists)      
		SELECT DISTINCT a.Program_Category, 'N' FROM      
		(      
			SELECT [Program Category] Program_Category FROM DM_Title (NOLOCK) WHERE ISNULL([Program Category], '') <> '' AND DM_Master_Import_Code = @DM_Master_Import_Code      
		) AS a  
		...
		(      
			SELECT [Director Name] DirName FROM DM_Title (NOLOCK) WHERE ISNULL([Director Name], '') <> '' AND DM_Master_Import_Code = @DM_Master_Import_Code      
		) AS a      
		CROSS APPLY      
		(      
		...
		SELECT DISTINCT b.StarCast, 'N' FROM(      
			SELECT [Key Star Cast] StarCast FROM DM_Title (NOLOCK) WHERE ISNULL([Key Star Cast], '') <> '' AND DM_Master_Import_Code = @DM_Master_Import_Code      
		) AS a      
		CROSS APPLY      
		(      
		...
		SELECT @Program_Category_Code = Columns_Code FROM Extended_Columns (NOLOCK) WHERE UPPER(LOWER(Columns_Name)) = 'Program Category'
		...
		BEGIN
			...
			SELECT LTRIM(RTRIM(Name)) As DMNAME, 'PC' As DMMASTER_TYPE FROM #Temp_Program_Category    
				...
				SELECT Columns_Value collate SQL_Latin1_General_CP1_CI_AS FROM Extended_Columns_Value (NOLOCK) 
		...
		Select  @DM_Master_Import_Code, DMNAME, DMMASTER_TYPE, @User_Code, GETDATE(), NULL,'U' From (    
			...
			where Talent_Code  In (select Talent_Code from Talent_Role (NOLOCK) where Role_Code = @DirectorRoleCode)   
		)    
		UNION    
		...
		UNION    
			...
			WHERE Name collate SQL_Latin1_General_CP1_CI_AS NOT IN (    
			SELECT Language_Name collate SQL_Latin1_General_CP1_CI_AS FROM Language (NOLOCK)    
		)   
		UNION    
		...
		UNION      
			...
			WHERE Name collate SQL_Latin1_General_CP1_CI_AS NOT IN (    
			SELECT Deal_Type_Name collate SQL_Latin1_General_CP1_CI_AS FROM Deal_Type (NOLOCK)    
		)    
		UNION   
		...
		BEGIN 
			...
			SELECT DM_Title_Code FROM DM_Title (NOLOCK) 
			...
			FROM (
				...
				FROM (
					...
					INNER JOIN DM_Title DT (NOLOCK) ON DT.DM_Title_Code = D.DM_Title_Code
	...
	if(@Loglevel < 2)Exec [USPLogSQLSteps] '[USP_DM_Title_PIV]', 'Step 2', 0, 'Procedure Excution Completed', 0, '' 
END