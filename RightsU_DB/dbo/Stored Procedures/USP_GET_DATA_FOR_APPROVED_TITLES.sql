...
--SELECT ''''+@Title_Codes +''', ''' + @Platform_Codes +''', ''' + @Platform_Type +''', ''' + @Region_Type +''', ''' + @Subtitling_Type +''', ''' + @Dubbing_Type +''', ''' + Cast(@Syn_Deal_Code AS VARCHAR) + ''''

	Declare @Loglevel int;
	select @Loglevel = Parameter_Value from System_Parameter_New where Parameter_Name='loglevel'
	if(@Loglevel < 2)Exec [USPLogSQLSteps] '[USP_GET_DATA_FOR_APPROVED_TITLES]', 'Step 1', 0, 'Started Procedure', 0, '' 
	...
	-- ============================================= -- ============================================= 
		...
		Begin
			...
			Inner Join Acq_Deal_Rights adr (NOLOCK) ON adrt.Acq_Deal_Rights_Code = adr.Acq_Deal_Rights_Code 
				--And ADR.Is_Theatrical_Right = Case When @Region_Type = 'TPL' Then 'Y' Else 'N' End
			Inner Join Acq_Deal ad (NOLOCK) ON adr.Acq_Deal_Code = ad.Acq_Deal_Code
		...
		Begin
			...
			Begin
				...
				From dbo.Acq_Deal_Rights_Platform adrp (NOLOCK)
				...
				Begin
					...
					InTo #Rights_Country
					From (
						Select * From Acq_Deal_Rights_Territory (NOLOCK) Where Acq_Deal_Rights_Code In (
					...
					Begin
						...
						Begin
							...
							From (
								Select * From Acq_Deal_Rights_Subtitling (NOLOCK) Where Acq_Deal_Rights_Code In (
						...
						Begin
							...
							InTo #Rights_Dubbing
							From (
								Select * From Acq_Deal_Rights_Dubbing (NOLOCK) Where Acq_Deal_Rights_Code In (
									Select rp.Acq_Deal_Rights_Code From #Rights_Platform rp
								)
							) ard
							Left Join Language_Group_Details lgd (NOLOCK) On ard.Language_Group_Code = lgd.Language_Group_Code
	...
	if(@Loglevel < 2)Exec [USPLogSQLSteps] '[USP_GET_DATA_FOR_APPROVED_TITLES]', 'Step 2', 0, 'Procedure Excution Completed', 0, '' 
End