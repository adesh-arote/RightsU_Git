...
BEGIN
	...
	if(@Loglevel< 2)Exec [USPLogSQLSteps] '[USP_Syndication_Deal_List_Report]', 'Step 1', 0, 'Started Procedure', 0, ''

		--exec [USP_Syndication_Deal_List_Report] '', '','', 0, '', 1, 'N','Y', 'N', 1,0,3
		--DECLARE   
		--@Agreement_No Varchar(100)
		...
		BEGIN
			...
			CREATE TABLE #TEMP_Syndication_Deal_List_Report(
				...
				CoExclusive_Remarks nvarchar(4000),
				Co_Production NVARCHAR(MAX),
				Right_Status NVARCHAR(MAX),
				Duration_In_Min DECIMAL(18,2) NULL,
				CBFCRating NVARCHAR(MAX) DEFAULT ''
			)
		END

		DECLARE @IsDealSegment VARCHAR(100), @IsRevenueVertical VARCHAR(100), @IsTypeOfFilm VARCHAR(MAX), @Columns_Code INT, @Is_Allow_Title_Objection CHAR(1) = 'N', @CoProd_Columns_Code INT, @Is_Title_CoProd CHAR(1) = 'N'
		...
		SELECT @CoProd_Columns_Code = Columns_Code FROM Extended_Columns WHERE UPPER(Columns_Name) = 'Co-Production'
		SELECT @Is_Title_CoProd = Parameter_Value FROM System_Parameter_New where Parameter_Name = 'Is_Title_CoProd_AdvSearch'
		...
		BEGIN
			...
			SELECT 
				...
				,CASE WHEN (SELECT count(*) FROM Syn_Deal_Attachment SDT (NOLOCK) WHERE SDT.Syn_Deal_Code = SD.Syn_Deal_Code) > 0 THEN 'Yes'
						   ELSE 'No'
				...
				, C.Category_Name AS Category_Name,MEC.Columns_Value_Code, SDR.CoExclusive_Remarks, ECV.Columns_Value AS Co_Production, SDR.Right_Status
				, T.Duration_In_Min,  REVERSE(stuff(reverse(  stuff(      
									(         
										...
										INNER JOIN Map_Extended_Columns_Details MACD (NOLOCK) ON MACD.Map_Extended_Columns_Code = MEC.Map_Extended_Columns_Code
										INNER JOIN Extended_Columns_Value ECV (NOLOCK) ON ECV.Columns_Value_Code = MACD.Columns_Value_Code
			...
			FROM Syn_Deal SD (NOLOCK)
				...
				INNER JOIN Syn_Deal_Rights SDR (NOLOCK) ON SD.Syn_Deal_Code = SDR.Syn_Deal_Code --AND ISNULL(SDR.Right_Status, '') = 'C'
				INNER JOIN Syn_Deal_Rights_Process_Validation SDRTV (NOLOCK) ON SDRTV.Syn_Deal_Rights_Code = SDR.Syn_Deal_Rights_Code AND LTRIM(RTRIM(SDRTV.Status)) = 'D'
				...
				LEFT JOIN Sub_License SL (NOLOCK) ON SDR.Sub_License_Code = SL.Sub_License_Code
				INNER JOIN Deal_Tag TG (NOLOCK) On SD.Deal_Tag_Code = TG.Deal_Tag_Code
				INNER JOIN Title T (NOLOCK) On SDRT.Title_Code = T.title_code
				...
				INNER JOIN Category C (NOLOCK) ON SD.Category_Code = C.Category_Code
				LEFT JOIN Map_Extended_columns MEC (NOLOCK) ON MEC.Record_Code = T.Title_Code AND MEC.Columns_Code = @Columns_Code
				LEFT JOIN Map_Extended_columns MECPRD (NOLOCK) ON MECPRD.Record_Code = T.Title_Code AND MECPRD.Columns_Code = @CoProd_Columns_Code
				LEFT JOIN Extended_Columns_Value ECV (NOLOCK) ON ECV.Columns_Value_Code=MECPRD.Columns_Value_Code
			WHERE  
				...
				(SELECT SDRP.Syn_Deal_Rights_Code FROM Syn_Deal_Rights_Platform SDRP (NOLOCK)
				Inner Join [Platform] P (NOLOCK) ON SDRP.Platform_Code = P.Platform_Code AND P.Applicable_For_Demestic_Territory = @IS_Theatrical 
				AND @IS_Theatrical = 'Y'
				) OR @IS_Theatrical = 'N')
		...
		BEGIN
			...
			BEGIN
				SELECT * INTO #Title_Objection_Rights_Period FROM Title_Objection_Rights_Period (NOLOCK)
				...
				, B.Rights_Start_Date, B.Rights_End_Date,
				 STUFF((
					...
					FROM Title_Objection_Territory T (NOLOCK)
					left join Country C (NOLOCK) ON C.Country_Code = T.Country_Code
					left join Territory TR (NOLOCK) ON TR.Territory_Code = T.Territory_Code
					WHERE T.Title_Objection_Code = A.Title_Objection_Code
				FOR XML PATH('')
				), 1, 1, '') AS Region,
				STUFF((
					SELECT ',' + CAST(P.Platform_Code AS VARCHAR)
					FROM Title_Objection_Platform P (NOLOCK)
		
					WHERE P.Title_Objection_Code = A.Title_Objection_Code
				FOR XML PATH('')
				), 1, 1, '') AS Platform_Name
				INTO #Title_Objection
				FROM Title_Objection A (NOLOCK)
				...
				FROM #TEMP_Syndication_Deal_List_Report ADLR
				INNER JOIN #Title_Objection TOB ON TOB.Record_Code = ADLR.Syn_Deal_code AND TOB.Title_Code = ADLR.Title_Code 
					AND TOB.Rights_Start_Date =ADLR.Right_Start_Date AND ISNULL(TOB.Rights_End_Date,'') = ISNULL(ADLR.Right_End_Date,'')

			END

			IF(@Is_Title_CoProd != 'Y')
			BEGIN
				UPDATE tsdlr SET Co_Production=null FROM #TEMP_Syndication_Deal_List_Report tsdlr
			END
		...
		BEGIN
			...
			CASE WHEN TEMP_SDLR.Is_PushBack = 'Y' THEN Convert(varchar(12),TEMP_SDLR.Right_Start_Date) ELSE NULL END AS Pushback_Start_Date, 
			CASE WHEN TEMP_SDLR.Is_PushBack = 'Y' THEN Convert(varchar(12),TEMP_SDLR.Right_End_Date) ELSE NULL END AS Pushback_End_Date, 
			...
			(SELECT Deal_Type_Name FROM Deal_Type (NOLOCK) AS DT WHERE DT.Deal_Type_Code=TEMP_SDLR.Deal_Type_Code) AS Deal_Type,
			...
			TEMP_SDLR.Objection_Start_Date,
			TEMP_SDLR.Objection_End_Date,
			TEMP_SDLR.Objection_Platform,
			TEMP_SDLR.Objection_Region,
			TEMP_SDLR.Under_Litigation,
			TEMP_SDLR.CoExclusive_Remarks,
			TEMP_SDLR.Co_Production,
			TEMP_SDLR.Right_Status,
			TEMP_SDLR.Duration_In_Min,
			TEMP_SDLR.CBFCRating
		...
		BEGIN
			SELECT 
				...
				@Col_Head52 = 'Objection Start Date',
				@Col_Head53 = 'Objection End Date',
				@Col_Head54 = 'Objection Region',
				@Col_Head55 = 'Objection Platform',
				@Col_Head56 = 'Under Litigation',
				@Col_Head57 = CASE WHEN  SM.Message_Key = 'CoExclusiveRemarks' AND ISNULL(SLM.Message_Desc,'') <> '' THEN SLM.Message_Desc ELSE @Col_Head57 END,
				@Col_Head58 = CASE WHEN @Is_Title_CoProd = 'Y' THEN 'Co Production' ELSE @Col_Head58 END,
				@Col_Head59 = CASE WHEN  SM.Message_Key = 'RightStatus' AND ISNULL(SLM.Message_Desc,'') <> '' THEN SLM.Message_Desc ELSE @Col_Head57 END,
				@Col_Head60 = 'Duration_In_Min',
				@Col_Head61 = 'CBFCRating',
				@Col_Head62 = 'Rights Expiry Status'
			 ...
			 'CategoryName','CoExclusiveRemarks','RightStatus')  
			 INNER JOIN System_Language_Message SLM (NOLOCK) ON SLM.System_Module_Message_Code = SMM.System_Module_Message_Code AND SLM.System_Language_Code = @SysLanguageCode AND Module_Code = @ModuleCode 
			 ...
			 BEGIN
				 SELECT 
						 ...
						 , [Star Cast],[Genre], [Title Language], [Release Year], [Platform], [Rights Start Date], [Rights End Date], [Rights Expiry Status], [Tentative], [Pushback], [Term], [Region], [Exclusive], [Title Language Right],
						 ...
						 ,[Objection Start Date],[Objection End Date],[Objection Region],[Objection Platform], [Under Litigation], [Co-Exclusive Remarks],[Co Production],[Right Status],[Duration_In_Min],[CBFCRating]
					FROM (
				 SELECT
						sorter = 1,
						...
						CONVERT(DATE,[Rights_End_Date],103)  As [Rights End Date], CASE WHEN Rights_End_Date < GETDATE() THEN 'Expired' ELSE 'Active' END AS [Rights Expiry Status], CAST([Is_Tentative] AS NVARCHAR(MAX)) As [Tentative], CAST([Is_PushBack] AS NVARCHAR(MAX)) As [Pushback], CAST([Term] AS NVARCHAR(MAX)) As [Term], CAST([Country_Territory_Name] AS NVARCHAR(MAX)) As [Region], CAST([Is_Exclusive] AS NVARCHAR(MAX)) As [Exclusive], 
						...
						CAST([Rights_Remarks] AS NVARCHAR(MAX)) As [Rights Remarks], CAST([Pushback_Platform_Name] AS NVARCHAR(MAX)) As [Reverse Holdback Platform], [Pushback_Start_Date] As [Reverse Holdback Start Date],[Pushback_End_Date] As [Reverse Holdback End Date],
						...
						,CONVERT(DATE,Objection_Start_Date,103)  As [Objection Start Date], CONVERT(DATE,Objection_End_Date,103)  As [Objection End Date], CAST(Objection_Region AS NVARCHAR(MAX)) As [Objection Region], CAST(Objection_Platform AS NVARCHAR(MAX)) As [Objection Platform],  CAST(Under_Litigation AS NVARCHAR(MAX)) As [Under Litigation],
						CoExclusive_Remarks As [Co-Exclusive Remarks],CAST(Co_Production AS varchar(max)) As [Co Production], CAST(Right_Status AS varchar(max)) As [Right Status], CAST(Duration_In_Min AS varchar(max)) AS [Duration_In_Min], CAST(CBFCRating AS varchar(max)) As [CBFCRating]
					...
					UNION ALL
						...
						, @Col_Head12, @Col_Head13, @Col_Head14, @Col_Head15,  GETDATE(), GETDATE(), @Col_Head62, @Col_Head18, 'Pushback', @Col_Head19, @Col_Head20, @Col_Head21, @Col_Head22, @Col_Head23, @Col_Head24, @Col_Head25, GETDATE()
						...
						, @Col_Head41, @Col_Head42, @Col_Head43, @Col_Head44, @Col_Head45,@Col_Head49, GETDATE(), GETDATE(), @Col_Head54, @Col_Head55, @Col_Head56, @Col_Head57, @Col_Head58, @Col_Head59, @Col_Head60, @Col_Head61
					) X   
				ORDER BY Sorter
			END
			ELSE
			BEGIN
				SELECT 	CAST(Agreement_No AS VARCHAR(100))  AS [Agreement_No], CAST(Business_Unit_Name AS VARCHAR(100))  AS [Business Unit Name] ,
						CAST([Deal_Type] AS NVARCHAR(MAX)) As [Title Type], CAST([Deal_Description] AS NVARCHAR(MAX)) As [Deal Description], CAST([Reference_No] AS NVARCHAR(MAX)) As [Reference No],
						--CONVERT(VARCHAR(30),[Agreement_Date],103) As [Agreement Date],
						CONVERT(DATE, [Agreement_Date], 103) As [Agreement Date],
						CAST([Deal_Tag_Description] AS NVARCHAR(MAX)) AS [Status],
	...
	if(@Loglevel< 2)Exec [USPLogSQLSteps] '[USP_Syndication_Deal_List_Report]', 'Step 2', 0, 'Procedure Excuting Completed', 0, ''
END
