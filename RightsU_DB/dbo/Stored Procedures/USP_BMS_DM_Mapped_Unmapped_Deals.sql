...
BEGIN
	...
	if(@Loglevel < 2)Exec [USPLogSQLSteps] '[USP_BMS_DM_Mapped_Unmapped_Deals]', 'Step 1', 0, 'Started Procedure', 0, ''
		SET @Acq_Deal_Code = 0
		/*
		...
		*/
		...
		INNER JOIN Acq_Deal AD (NOLOCK) ON BD.Acq_Deal_Code = AD.Acq_Deal_Code  
		INNER JOIN BMS_Deal_Content BDS (NOLOCK) ON BD.BMS_Deal_Code = BDS.BMS_Deal_Code
		INNER JOIN DM_BMS_Deal_Content BDC (NOLOCK) ON BDC.DCO_ASS_ID = BDS.BMS_Asset_Ref_Key
		WHERE
		...
		/****************Insert into Temp table #Temp_BMS_Deal_Content and #Temp_DM_BMS_Deal_Content from BMS_Deal_Content using BMS_Deal_Code****************/				
						...
						FROM BMS_Deal_Content BDS (NOLOCK)
						...
						FROM DM_BMS_Deal_Content DM_BDS (NOLOCK)
		...
		(
			...
			FROM DM_BMS_Mapped_UnMapped_Content_Rights DBMCR1 (NOLOCK)
			INNER JOIN  DM_BMS_Mapped_UnMapped_Content_Rights DBMCR2 (NOLOCK)
	...
	if(@Loglevel < 2)Exec [USPLogSQLSteps] '[USP_BMS_DM_Mapped_Unmapped_Deals]', 'Step 2', 0, 'Procedure Excution Completed', 0, ''
END