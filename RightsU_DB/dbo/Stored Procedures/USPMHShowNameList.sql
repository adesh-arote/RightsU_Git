...
BEGIN
	...
	,(Select Count(MR.TitleCode) AS TitleCount From MHRequest MR where MR.TitleCode = T.Title_Code AND ((CAST(MR.RequestedDate AS DATE)   BETWEEN REPLACE(CONVERT(NVARCHAR,@FromDate, 106),' ', '-') AND REPLACE(CONVERT(NVARCHAR,@ToDate, 106),' ', '-')))) AS TitleCount
	...
	Where AD.Vendor_Code In (Select Vendor_Code From MHUsers Where Users_Code = @UsersCode) AND (ADRC.Channel_Code = @ChannelCode OR @ChannelCode = 0)
