CREATE PROCEDURE [dbo].[USPITGetDrillDownGenreWiseTitle_Details] 
...
BEGIN
	...
	--@SearchText NVARCHAR(MAX)
		...
		ELSE
			...
			SELECT  g.Genres_Name AS Genre, ISNULL(ecv.Columns_Value,'NA') AS CBFCRating, dt.Deal_Type_Name AS TitleType,t.Title_Name
			FROM Title t (NOLOCK) 
			...
			GROUP BY g.Genres_Name, ecv.Columns_Value,dt.Deal_Type_Name,t.Title_Name
			order by 1

		
		--select * from Title where Title_Name like '%Jurassic Park%'
			...
			--FROM Title t (NOLOCK) 
			--LEFT JOIN Map_Extended_Columns mec (NOLOCK)  ON mec.Record_Code = t.Title_Code AND mec.Columns_Code = 5 AND T.Deal_Type_Code IN ((select number from dbo.fn_Split_withdelemiter(''+@Deal_Type_Code+'',',')))  
			--LEFT JOIN Map_Extended_Columns_Details mecd (NOLOCK)  ON mecd.Map_Extended_Columns_Code = mec.Map_Extended_Columns_Code
			--LEFT JOIN Extended_Columns_Value ecv  (NOLOCK) ON ecv.Columns_Value_Code = mecd.Columns_Value_Code
			--INNER JOIN Title_Geners tg  (NOLOCK) ON tg.Title_Code = t.Title_Code
			--INNER JOIN Genres g (NOLOCK)  ON g.Genres_Code = tg.Genres_Code
