...
BEGIN 
	...
	if(@Loglevel < 2)Exec [USPLogSQLSteps] '[USP_Acq_Deal_Clone_UDT]', 'Step 1', 0, 'Started Procedure', 0, ''
		  ...
		  BEGIN try 
			  ...
			  FROM   @Deal_Rights_Title DRT 
					 INNER JOIN acq_deal_movie PADM 
							 (NOLOCK) ON DRT.deal_rights_code = PADM.acq_deal_movie_code 
				...
				BEGIN 
					IF( (SELECT Count(*) 
					  ...
					  BEGIN 
						  ...
						  FROM   acq_deal_rights_title ADRT  (NOLOCK)
								 INNER JOIN #temptitle DRT 
									  (NOLOCK)    ON ADRT.title_code = DRT.title_code 
						  ...
						  FROM   acq_deal_rights_title_eps ADRTE  (NOLOCK)
								 INNER JOIN acq_deal_rights_title ADRT 
									  (NOLOCK) ON ADRTE.acq_deal_rights_title_code = 
								 ...
								 INNER JOIN acq_deal_rights_title AtADRT 
										 (NOLOCK) ON Cast(Isnull(AtADRT.title_code, '') AS 
											VARCHAR) 
											+ '~' 
						  ...
						  SELECT AtADRH.acq_deal_rights_holdback_code, 
								 ADRHD.language_code 
						  FROM   acq_deal_rights_holdback_dubbing ADRHD  (NOLOCK)
								 INNER JOIN acq_deal_rights_holdback ADRH 
										 (NOLOCK) ON ADRHD.acq_deal_rights_holdback_code = 
						  ...
						  SELECT AtADRH.acq_deal_rights_holdback_code, 
								 ADRHP.platform_code 
						  FROM   acq_deal_rights_holdback_platform ADRHP  (NOLOCK)
								 INNER JOIN acq_deal_rights_holdback ADRH 
										(NOLOCK)  ON ADRHP.acq_deal_rights_holdback_code = 
											ADRH.acq_deal_rights_holdback_code 
								 INNER JOIN acq_deal_rights_holdback AtADRH 
									   (NOLOCK)   ON 
								 Cast(Isnull(AtADRH.hb_run_after_release_no, '' 
						  ...
						  SELECT AtADRH.acq_deal_rights_holdback_code, 
								 ADRHS.language_code 
						  FROM   acq_deal_rights_holdback_subtitling ADRHS  (NOLOCK)
								 INNER JOIN acq_deal_rights_holdback ADRH 
										(NOLOCK)  ON ADRHS.acq_deal_rights_holdback_code = 
											ADRH.acq_deal_rights_holdback_code 
								 INNER JOIN acq_deal_rights_holdback AtADRH 
										(NOLOCK)  ON 
								 Cast(Isnull(AtADRH.hb_run_after_release_no, '' 
						  ...
						  SELECT AtADRH.acq_deal_rights_holdback_code, 
								 ...
								 country_code, 
								 territory_code 
						  FROM   acq_deal_rights_holdback_territory ADRHT  (NOLOCK)
								 INNER JOIN acq_deal_rights_holdback ADRH 
								   (NOLOCK) ON ADRHT.acq_deal_rights_holdback_code = 
											ADRH.acq_deal_rights_holdback_code 
						  ...
						  SELECT @New_Acq_Deal_Rights_Code, 
								 ...
								 @User_Code, 
								 Getdate() 
						  FROM   acq_deal_rights_blackout ADRB  (NOLOCK)
						  ...
						  FROM   acq_deal_rights_blackout_dubbing ADRBD  (NOLOCK)
								 INNER JOIN acq_deal_rights_blackout ADRB 
									   (NOLOCK)   ON ADRBD.acq_deal_rights_blackout_code = 
						  ...
						  SELECT AtADRB.acq_deal_rights_blackout_code, 
								 ADRBP.platform_code 
						  FROM   acq_deal_rights_blackout_platform ADRBP  (NOLOCK)
								 INNER JOIN acq_deal_rights_blackout ADRB 
									  (NOLOCK) ON ADRBP.acq_deal_rights_blackout_code = 
											ADRB.acq_deal_rights_blackout_code 
								 INNER JOIN acq_deal_rights_blackout AtADRB 
									  (NOLOCK) ON Cast(Isnull(AtADRB.start_date, '') AS 
											VARCHAR) 
						  ...
						  SELECT AtADRB.acq_deal_rights_blackout_code, 
								 ADRBS.language_code 
						  FROM   acq_deal_rights_blackout_subtitling ADRBS  (NOLOCK)
								 INNER JOIN acq_deal_rights_blackout ADRB 
									   (NOLOCK) ON ADRBS.acq_deal_rights_blackout_code = 
											ADRB.acq_deal_rights_blackout_code 
								 INNER JOIN acq_deal_rights_blackout AtADRB 
									  (NOLOCK) ON Cast(Isnull(AtADRB.start_date, '') AS 
											VARCHAR) 
						  ...
						  FROM   acq_deal_rights_blackout_territory ADRBT  (NOLOCK)
								 INNER JOIN acq_deal_rights_blackout ADRB 
										 (NOLOCK) ON ADRBT.acq_deal_rights_blackout_code = 
											ADRB.acq_deal_rights_blackout_code 
								 INNER JOIN acq_deal_rights_blackout AtADRB 
										 (NOLOCK) ON Cast(Isnull(AtADRB.start_date, '') AS 
											VARCHAR) 
	...
	DECLARE @New_Acq_Deal_Pushback_Code INT = 0, 
	@Acq_Deal_Pushback_Code     INT = 0 
	DECLARE pushback_cursor CURSOR FOR 
	SELECT acq_deal_pushback_code 
	FROM   acq_deal_pushback  (NOLOCK)
	WHERE  acq_deal_code = @Previous_Acq_Deal_Code 
	...
	IF( (SELECT Count(*) 
	FROM   #temptitle tt 
	INNER JOIN acq_deal_pushback_title ADPT  (NOLOCK)
	...
	FROM   acq_deal_pushback_dubbing ADPD  (NOLOCK)
	...
	SELECT @New_Acq_Deal_Pushback_Code, 
	ADPP.platform_code 
	FROM   acq_deal_pushback_platform ADPP  (NOLOCK)
	...
	FROM   acq_deal_pushback_subtitling ADPS  (NOLOCK)
	...
	ADPT.country_code, 
	ADPT.territory_code 
	FROM   acq_deal_pushback_territory ADPT  (NOLOCK)
	WHERE  ADPT.acq_deal_pushback_code = @Acq_Deal_Pushback_Code 
	...
	DECLARE @New_Acq_Deal_Ancillary_Code INT = 0, 
	@Acq_Deal_Ancillary_Code     INT = 0 
	DECLARE ancillary_cursor CURSOR FOR 
	SELECT acq_deal_ancillary_code 
	FROM   acq_deal_ancillary  (NOLOCK)
	WHERE  acq_deal_code = @Previous_Acq_Deal_Code 
	...
	BEGIN 
	IF( (SELECT Count(*) 
	FROM   acq_deal_ancillary_title ADAT  (NOLOCK)
	...
	FROM   acq_deal_ancillary_platform ADAP  (NOLOCK)
	...
	FROM   acq_deal_ancillary_platform_medium ADAPM  (NOLOCK)
	INNER JOIN acq_deal_ancillary_platform ADAP  (NOLOCK)
	ON ADAPM.acq_deal_ancillary_platform_code = 
	ADAP.acq_deal_ancillary_platform_code 
	INNER JOIN acq_deal_ancillary_platform AtADAP  (NOLOCK)
	...
	DECLARE @New_Acq_Deal_Run_Code INT = 0, 
	@Acq_Deal_Run_Code     INT 
	DECLARE run_cursor CURSOR FOR 
	SELECT acq_deal_run_code 
	FROM   acq_deal_run  (NOLOCK)
	WHERE  acq_deal_code = @Previous_Acq_Deal_Code 
	...
	BEGIN 
	IF( (SELECT Count(*) 
	FROM   acq_deal_run_title ADRT  (NOLOCK)
	INNER JOIN #temptitle tt 
	...
	Getdate(), 
	ADRYR.year_no 
	FROM   acq_deal_run_yearwise_run ADRYR  (NOLOCK)
	...
	Getdate() 
	FROM   acq_deal_run_yearwise_run ADRYR  (NOLOCK)
	INNER JOIN acq_deal_run_yearwise_run_week ADRYRW  (NOLOCK)
	ON ADRYRW.acq_deal_run_yearwise_run_code = 
	ADRYR.acq_deal_run_yearwise_run_code 
	INNER JOIN acq_deal_run_yearwise_run AtADRYR  (NOLOCK)
	...
	DECLARE @Acq_Deal_Cost_Code     INT = 0, 
	@New_Acq_Deal_Cost_Code INT = 0 
	DECLARE cur_cost CURSOR FOR 
	SELECT acq_deal_cost_code 
	FROM   acq_deal_cost  (NOLOCK)
	WHERE  acq_deal_code = @Previous_Acq_Deal_Code 
	...
	BEGIN 
	IF( (SELECT Count(*) 
	FROM   acq_deal_cost_title ADCT  (NOLOCK)
	INNER JOIN #temptitle tt 
	...
	Getdate(), 
	@User_Code 
	FROM   acq_deal_cost_additional_exp ADCAE  (NOLOCK)
	WHERE  acq_deal_cost_code = @Acq_Deal_Cost_Code 
	...
	ADCC.percentage, 
	ADCC.amount 
	FROM   acq_deal_cost_commission ADCC  (NOLOCK)
	WHERE  acq_deal_cost_code = @Acq_Deal_Cost_Code 
	...
	Getdate(), 
	@User_Code 
	FROM   acq_deal_cost_variable_cost ADCVC  (NOLOCK)
	WHERE  acq_deal_cost_code = @Acq_Deal_Cost_Code 
	...
	Getdate(), 
	@User_Code 
	FROM   acq_deal_cost_costtype ADCC  (NOLOCK)
	WHERE  acq_deal_cost_code = @Acq_Deal_Cost_Code 
	...
	ADCCE.remarks 
	FROM   acq_deal_cost_costtype ADCC  (NOLOCK)
	INNER JOIN acq_deal_cost_costtype_episode ADCCE  (NOLOCK)
	ON ADCCE.acq_deal_cost_costtype_code = 
	ADCC.acq_deal_cost_costtype_code 
	INNER JOIN acq_deal_cost_costtype AtADCC  (NOLOCK)
	...
	DECLARE @Acq_Deal_Sport_Code     INT = 0, 
	@New_Acq_Deal_Sport_Code INT = 0 
	DECLARE cur_sport CURSOR FOR 
	SELECT acq_deal_sport_code 
	FROM   acq_deal_sport  (NOLOCK)
	WHERE  acq_deal_code = @Previous_Acq_Deal_Code 
	...
	BEGIN 
	IF( (SELECT Count(*) 
	FROM   acq_deal_sport_title ADST  (NOLOCK)
	INNER JOIN #temptitle tt 
	...
	ADSB.[type] 
	FROM   acq_deal_sport_broadcast ADSB  (NOLOCK)
	WHERE  acq_deal_sport_code = @Acq_Deal_Sport_Code 
	...
	ADSP.platform_code, 
	ADSP.[type] 
	FROM   acq_deal_sport_platform ADSP  (NOLOCK)
	WHERE  acq_deal_sport_code = @Acq_Deal_Sport_Code 
	...
	language_group_code, 
	flag 
	FROM   acq_deal_sport_language  (NOLOCK)
	WHERE  acq_deal_sport_code = @Acq_Deal_Sport_Code 
	END 
	...
	DECLARE @Acq_Deal_Sport_Ancillary_Code     INT = 0, 
	@New_Acq_Deal_Sport_Ancillary_Code INT = 0 
	DECLARE cur_sport_ancillary CURSOR FOR 
	SELECT acq_deal_sport_ancillary_code 
	FROM   acq_deal_sport_ancillary  (NOLOCK)
	WHERE  acq_deal_code = @Previous_Acq_Deal_Code 
	...
	BEGIN 
	IF( (SELECT Count(*) 
	FROM   acq_deal_sport_ancillary_title ADSAT  (NOLOCK)
	INNER JOIN #temptitle tt 
	...
	SELECT @New_Acq_Deal_Sport_Ancillary_Code, 
	sport_ancillary_broadcast_code 
	FROM   acq_deal_sport_ancillary_broadcast  (NOLOCK)
	WHERE  acq_deal_sport_ancillary_code = 
	@Acq_Deal_Sport_Ancillary_Code 
	...
	SELECT @New_Acq_Deal_Sport_Ancillary_Code, 
	sport_ancillary_source_code 
	FROM   acq_deal_sport_ancillary_source  (NOLOCK)
	WHERE  acq_deal_sport_ancillary_code = 
	@Acq_Deal_Sport_Ancillary_Code 
	...
	DECLARE @Acq_Deal_Sport_Monetisation_Ancillary_Code     INT = 0, 
	@New_Acq_Deal_Sport_Monetisation_Ancillary_Code INT = 0 
	DECLARE cur_sport_minetisation_ancillary CURSOR FOR 
	SELECT acq_deal_sport_monetisation_ancillary_code 
	FROM   acq_deal_sport_monetisation_ancillary  (NOLOCK)
	WHERE  acq_deal_code = @Previous_Acq_Deal_Code 
	...
	BEGIN 
	IF( (SELECT Count(*) 
	FROM   acq_deal_sport_monetisation_ancillary_title ADSMA  (NOLOCK)
	INNER JOIN #temptitle tt 
	...
	monetisation_type_code, 
	monetisation_rights 
	FROM   acq_deal_sport_monetisation_ancillary_type  (NOLOCK)
	WHERE  acq_deal_sport_monetisation_ancillary_code = 
	@Acq_Deal_Sport_Monetisation_Ancillary_Code 
	END 
	...
	DECLARE @Acq_Deal_Sport_Sales_Ancillary_Code     INT = 0, 
	@New_Acq_Deal_Sport_Sales_Ancillary_Code INT = 0 
	DECLARE cur_sport_sale_ancillary CURSOR FOR 
	SELECT acq_deal_sport_sales_ancillary_code 
	FROM   acq_deal_sport_sales_ancillary  (NOLOCK)
	WHERE  acq_deal_code = @Previous_Acq_Deal_Code 
	...
	BEGIN 
	IF( (SELECT Count(*) 
	FROM   acq_deal_sport_sales_ancillary_title ADSSAT  (NOLOCK)
	INNER JOIN #temptitle tt 
	...
	ADSSAS.sponsor_code, 
	ADSSAS.sponsor_type 
	FROM   acq_deal_sport_sales_ancillary_sponsor ADSSAS  (NOLOCK)
	...
	Getdate(), 
	@User_Code 
	FROM   acq_deal_payment_terms  (NOLOCK)
	WHERE  acq_deal_code = @Previous_Acq_Deal_Code 
	...
	tt.episode_starts_from, 
	tt.episode_end_to 
	FROM   acq_deal_material ADM  (NOLOCK)
	INNER JOIN #temptitle tt 
	...
	if(@Loglevel < 2)Exec [USPLogSQLSteps] '[USP_Acq_Deal_Clone_UDT]', 'Step 2', 0, 'Procedure Excution Completed', 0, ''
END 