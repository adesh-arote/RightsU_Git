...
BEGIN
	...
	if(@Loglevel< 2)Exec [USPLogSQLSteps] '[USP_Syn_Deal_Rights_Holdback_Validation]', 'Step 1', 0, 'Started Procedure', 0, '' 
		...
		BEGIN
			...
			BEGIN
				...
				FROM Syn_Deal_Movie (NOLOCK)  WHERE Syn_Deal_Movie_Code IN(SELECT NUMBER FROM fn_Split_withdelemiter(@Title_Code, ','))
			END
			ELSE 
			BEGIN
				insert into #TempTitle(Title_Code,Episode_From,Episode_To)
				select Title_Code , 1 , 1
				FROM Title (NOLOCK)  WHERE Title_Code IN(SELECT NUMBER FROM fn_Split_withdelemiter(@Title_Code, ','))
			END
		

			SELECT Syn_Deal_Rights_Holdback_Code INTO #TempHoldbackCodes FROM (
				SELECT DISTINCT Syn_Deal_Rights_Holdback_Code FROM Syn_Deal_Rights_Holdback_Platform (NOLOCK) 
				WHERE Platform_Code IN(SELECT NUMBER FROM fn_Split_withdelemiter(@Platform_Code, ','))
				INTERSECT
				SELECT DISTINCT Syn_Deal_Rights_Holdback_Code FROM Syn_Deal_Rights_Holdback_Territory (NOLOCK) 
			...
			INNER JOIN Syn_Deal_Rights_Holdback HB (NOLOCK) On T.Syn_Deal_Rights_Holdback_Code = HB.Syn_Deal_Rights_Holdback_Code
			...
			INNER JOIN Syn_Deal_Rights_Title RT (NOLOCK) On HB.Syn_Deal_Rights_Code = RT.Syn_Deal_Rights_Code
			...
			BEGIN
				...
				BEGIN
					...
					INNER JOIN Syn_Deal_Rights_Holdback_Subtitling SDRHS (NOLOCK) ON SDRHS.Syn_Deal_Rights_Holdback_Code = THB.Syn_Deal_Rights_Holdback_Code
				...
				BEGIN
					...
					SELECT @Rec_Count = COUNT(THB.Syn_Deal_Rights_Code) FROM #TempHoldback_TL THB
					INNER JOIN Syn_Deal_Rights_Holdback_Dubbing SDRHD (NOLOCK)  ON SDRHD.Syn_Deal_Rights_Holdback_Code = THB.Syn_Deal_Rights_Holdback_Code
	...
	if(@Loglevel< 2)Exec [USPLogSQLSteps] '[USP_Syn_Deal_Rights_Holdback_Validation]', 'Step 2', 0, 'Procedure Excuting Completed', 0, '' 
END