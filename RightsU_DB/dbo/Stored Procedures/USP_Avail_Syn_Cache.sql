...
BEGIN
	...
	if(@Loglevel < 2)Exec [USPLogSQLSteps] '[USP_Avail_Syn_Cache]', 'Step 1', 0, 'Started Procedure', 0, ''
		...
		*/
		...
		SELECT 
			...
			INNER JOIN Syn_Deal_Rights_Territory SDRC (NOLOCK) ON SDRC.Syn_Deal_Rights_Code=SDR.Syn_Deal_Rights_Code
			INNER JOIN Country C (NOLOCK) ON SDRC.Country_Code=C.Country_Code AND C.Is_Theatrical_Territory='N'
		...
		from #tmpCache tmp
			 left join Avail_Syn aa (NOLOCK) on aa.Title_Code=tmp.Title_Code
		...
		FROM Syn_Deal_Rights_Dubbing SDRD (NOLOCK)
			inner join Syn_Deal_Rights sdr (NOLOCK) on sdr.Syn_Deal_Rights_Code=SDRD.Syn_Deal_Rights_Code
			inner join Avail_Syn_Details aa (NOLOCK) on aa.Syn_Deal_Code =sdr.Syn_Deal_Code	
		...
		inner join Syn_Deal_Rights sdr (NOLOCK) on sdr.Syn_Deal_Rights_Code=SDRS.Syn_Deal_Rights_Code
		inner join Avail_Syn_Details asd (NOLOCK) on asd.Syn_Deal_Code=sdr.Syn_Deal_Code 
		where sdr.Syn_Deal_Code=@Syn_Deal_Code
	...
	if(@Loglevel < 2)Exec [USPLogSQLSteps] '[USP_Avail_Syn_Cache]', 'Step 2', 0, 'Procedure Excution Completed', 0, ''
END