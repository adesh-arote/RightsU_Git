...
BEGIN
	...
	if(@Loglevel< 2)Exec [USPLogSQLSteps] '[USP_Validate_Title_Objection_Dup]', 'Step 1', 0, 'Started Procedure', 0, ''
		...
		BEGIN
			...
			FROM Title_Objection_Platform  (NOLOCK)
			WHERE
		...
		BEGIN
			...
			SELECT A.Title_Objection_Code, 'LP' FROM (
				...
				FROM Title_Objection_Rights_Period (NOLOCK)
		...
		BEGIN
			...
			BEGIN
				SELECT @CountryCodes = STUFF((
					SELECT DISTINCT ',' + CAST(Country_Code AS VARCHAR) FROM  Territory_Details (NOLOCK) WHERE Territory_Code IN (SELECT number FROM fn_Split_withdelemiter(@CountryCodes,','))
				FOR XML PATH('')
				), 1, 1, '')
			END
			...
			SELECT DISTINCT A.Title_Objection_Code,'CT' FROM (
				...
				FROM Title_Objection_Territory (NOLOCK)
				WHERE Territory_Type = 'I' AND
					Title_Objection_Code IN (SELECT Title_Objection_Code FROM #Title_Objection_Dup_Check) 
				UNION
				SELECT DISTINCT TOT.Title_Objection_Code, TD.Country_Code
				FROM Title_Objection_Territory TOT (NOLOCK)
				INNER JOIN Territory_Details TD (NOLOCK) ON TD.Territory_Code = TOT.Territory_Code
	...
	if(@Loglevel< 2)Exec [USPLogSQLSteps] '[USP_Validate_Title_Objection_Dup]', 'Step 2', 0, 'Procedure Excution Completed', 0, ''
END
