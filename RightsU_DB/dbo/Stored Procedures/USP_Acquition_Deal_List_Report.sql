...
BEGIN
	...
	if(@Loglevel < 2)Exec [USPLogSQLSteps] '[USP_Acquition_Deal_List_Report]', 'Step 1', 0, 'Started Procedure', 0, ''
		--DECLARE 
		...
		--, @DealSegment INT= 0
		--, @TypeOfFilm INT = 3
		--, @ModeOfAcquisition = '27,40'
		--IF CHARINDEX(',',@Business_Unit_code) > 0 
		--	set @Business_Unit_code = '0'
		--exec [USP_Acquition_Deal_List_Report] '','Y', '', '', 0, '', '1', 'Y', 1, 0, 3,'27,40'
		DECLARE 
		...
		BEGIN
			...
			CREATE TABLE #TEMP_Acquition_Deal_List_Report(
				...
				Mode_Of_Acquisition NVARCHAR(MAX) DEFAULT '',
				Co_Production NVARCHAR(MAX),
				Right_Status NVARCHAR(MAX) DEFAULT '',
		...
		DECLARE @IsDealSegment VARCHAR(MAX), @IsRevenueVertical VARCHAR(MAX), @IsTypeOfFilm VARCHAR(MAX), @Columns_Code INT, @Is_Allow_Title_Objection CHAR(1) = 'N', @CoProd_Columns_Code INT, @Is_Title_CoProd CHAR(1) = 'N'
		...
		SELECT @Columns_Code = Columns_Code FROM Extended_Columns (NOLOCK) WHERE UPPER(Columns_Name) = 'TYPE OF FILM'
		...
		SELECT 
			...
			ISNULL(r.Role_Name,''), ECV.Columns_Value AS Co_Production, ADR.Right_Status,
		...
		FROM Acq_Deal AD (NOLOCK)
			...
			INNER JOIN Deal_Tag TG (NOLOCK) On AD.Deal_Tag_Code = TG.Deal_Tag_Code
			...
			LEFT JOIN Acq_Deal_Movie ADM (NOLOCK) on ADRT.Title_Code = ADM.Title_Code AND ADRT.Episode_From = ADM.Episode_Starts_From AND ADRT.Episode_To = ADM.Episode_End_To AND ADM.Acq_Deal_Code = AD.Acq_Deal_Code
		...
		BEGIN
			...
			BEGIN
				UPDATE tadlr SET Co_Production=null FROM #TEMP_Acquition_Deal_List_Report tadlr
			END
		...
		BEGIN
			...
			(
				...
				Due_Diligence,
				Category_Name,
				Columns_Value_Code, Mode_Of_Acquisition, 
				Duration_In_Min,
				CBFCRating
			)
			SELECT 
				...
				CASE UPPER(LTRIM(RTRIM(ISNULL(ADM.Due_Diligence, '')))) 
					...
					ELSE 'No' 
				END AS Due_Diligence,
				C.Category_Name,MEC.Columns_Value_Code, R.Role_Name,
				T.Duration_In_Min,
			...
			FROM Acq_Deal AD (NOLOCK)
				INNER JOIN Business_Unit BU (NOLOCK) ON BU.Business_Unit_Code = AD.Business_Unit_Code
				INNER JOIN Acq_Deal_Pushback ADP (NOLOCK) On AD.Acq_Deal_Code = ADP.Acq_Deal_Code
				INNER JOIN Role R (NOLOCK) ON AD.Role_Code = R.Role_Code 
				INNER JOIN Acq_Deal_Pushback_Title ADPT (NOLOCK) On ADP.Acq_Deal_Pushback_Code = ADPT.Acq_Deal_Pushback_Code 
				INNER JOIN Vendor V (NOLOCK) ON AD.Vendor_Code = V.Vendor_Code
				LEFT JOIN Party_Group PG (NOLOCK) ON v.Party_Group_Code = PG.Party_Group_Code
				INNER JOIN Deal_Tag TG (NOLOCK) On AD.Deal_Tag_Code = TG.Deal_Tag_Code
				INNER JOIN Title T (NOLOCK) On ADPT.Title_Code = T.title_code
				LEFT JOIN Program P (NOLOCK) on T.Program_Code = P.Program_Code
				LEFT JOIN Language L (NOLOCK) on T.Title_Language_Code = L.language_code
				LEFT JOIN Acq_Deal_Movie ADM (NOLOCK) on ADPT.Title_Code = ADM.Title_Code AND ADPT.Episode_From = ADM.Episode_Starts_From AND ADPT.Episode_To = ADM.Episode_End_To AND ADM.Acq_Deal_Code = AD.Acq_Deal_Code
				INNER JOIN Category C  (NOLOCK) ON AD.Category_Code = C.Category_Code
				LEFT JOIN Map_Extended_columns MEC (NOLOCK) ON MEC.Record_Code = T.Title_Code AND MEC.Columns_Code = @Columns_Code
			WHERE  
				AD.Deal_Workflow_Status NOT IN ('AR', 'WA') AND
			...
			BEGIN
				...
				RT.DB_Codes =
				STUFF((Select DISTINCT ',' +  CAST(CASE WHEN AADRD.Language_Code IS NULL THEN AADRD.Language_Group_Code ELSE AADRD.Language_Code END AS NVARCHAR(MAX))
				from  Acq_Deal_Pushback_Dubbing AADRD  (NOLOCK)
			...
			(
				...
				,Is_Pushback, Master_Deal_Movie_Code_ToLink
				,Run_Type,Is_Attachment,[Program_Name],Due_Diligence,Category_Name, Mode_Of_Acquisition, Duration_In_Min, CBFCRating
			)
			SELECT 
				...
				Due_Diligence,Category_Name,Mode_Of_Acquisition, Duration_In_Min, CBFCRating
		...
		SELECT DISTINCT 
			...
			(SELECT Deal_Type_Name FROM Deal_Type AS DT (NOLOCK) WHERE DT.Deal_Type_Code=TEMP_ADLR.Deal_Type_Code) AS Deal_Type,
			...
			TEMP_ADLR.Mode_Of_Acquisition,
			TEMP_ADLR.Co_Production,
			TEMP_ADLR.Right_Status,
			TEMP_ADLR.Duration_In_Min,
			TEMP_ADLR.CBFCRating
		...
		BEGIN
			SELECT 
				...
				@Col_Head58 = CASE WHEN  SM.Message_Key = 'ModeOfAcquisition' AND ISNULL(SLM.Message_Desc,'') <> '' THEN SLM.Message_Desc ELSE @Col_Head49 END,
				@Col_Head59 = CASE WHEN @Is_Title_CoProd = 'Y' THEN 'Co Production' ELSE @Col_Head59 END,
				@Col_Head60 = CASE WHEN  SM.Message_Key = 'RightStatus' AND ISNULL(SLM.Message_Desc,'') <> '' THEN SLM.Message_Desc ELSE @Col_Head49 END,
				@Col_Head61 = 'Duration_In_Min',
				@Col_Head62 = 'CBFCRating',
				@Col_Head63 = 'Rights Expiry Status'
			 ...
			 ,'ModeOfAcquisition','RightStatus')  
			 INNER JOIN System_Language_Message SLM ON SLM.System_Module_Message_Code = SMM.System_Module_Message_Code AND SLM.System_Language_Code = @SysLanguageCode AND Module_code = @ModuleCode 
			...
			BEGIN
				SELECT 
					 ...
					 [Objection Start Date],[Objection End Date],[Objection Region],[Objection Platform], [Under Litigation],[Mode Of Acquisition],[Co Production],[Right Status],[Duration_In_Min],[CBFCRating]
				FROM (
				...
				SELECT
					...
					CAST(Mode_Of_Acquisition AS varchar(max)) As [Mode Of Acquisition],CAST(Co_Production AS varchar(max)) As [Co Production], CAST(Right_Status AS varchar(max)) As [Right Status], CAST(Duration_In_Min AS varchar(max)) AS [Duration_In_Min], CAST(CBFCRating AS varchar(max)) As [CBFCRating]  
				...
				UNION ALL
					...
					, @Col_Head12, @Col_Head13, @Col_Head14, @Col_Head15,  @Col_Head16, @Col_Head17, @Col_Head18, @Col_Head63, @Col_Head19,'', @Col_Head20, @Col_Head21, @Col_Head22, @Col_Head23, @Col_Head24, @Col_Head25, @Col_Head26
					...
					, @Col_Head41, @Col_Head42, @Col_Head43, @Col_Head44, @Col_Head45, @Col_Head46, @Col_Head47,@Col_Head48,@Col_Head49, @Col_Head53, @Col_Head54, @Col_Head55, @Col_Head56, @Col_Head57, @Col_Head58, @Col_Head59, @Col_Head60, @Col_Head61, @Col_Head62
				) X   
				ORDER BY Sorter
			END
			ELSE 
