...
DECLARE 
	...
	ORDER BY TEMP_ADLR.Agreement_No, TEMP_ADLR.Is_Pushback
		...
		SELECT 
			...
			@Col_Head48 = CASE WHEN  SM.Message_Key = 'CategoryName' AND ISNULL(SLM.Message_Desc,'') <> '' THEN SLM.Message_Desc ELSE @Col_Head47 END
		...
		BEGIN
			...
			SELECT
				...
				CAST([Promoter_Group_Name] AS NVARCHAR(MAX)) As[Self Utilization Group], CAST([Promoter_Remarks_Name] AS NVARCHAR(MAX)) As [Self Utilization Remarks],  CAST([Due_Diligence] AS NVARCHAR(MAX)) As [Due Diligence], CAST([Category_Name] AS NVARCHAR(MAX)) AS [Category_Name]
			...
			UNION ALL
				...
				, @Col_Head41, @Col_Head42, @Col_Head43, @Col_Head44, @Col_Head45, @Col_Head46, @Col_Head47,@Col_Head48
			) X   
			ORDER BY Sorter
		END
		ELSE 
