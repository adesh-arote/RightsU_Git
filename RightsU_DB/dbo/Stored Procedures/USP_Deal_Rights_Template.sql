...
BEGIN
	...
	if(@Loglevel < 2)Exec [USPLogSQLSteps] '[USP_Deal_Rights_Template]', 'Step 1', 0, 'Started Procedure', 0, ''
		...
		DECLARE 
			...
			@Term VARCHAR(20), @Right_Type CHAR(1), @Year INT, @Month INT, @Day INT, @Milestone_No_Of_Unit INT, @Milestone_Unit_Type INT, @Is_Tentetive CHAR(1), 
		...
		PRINT '  Select Data from Acq_Rights_Template (NOLOCK) and set in variables'
		SELECT 
		...
		FROM Acq_Rights_Template (NOLOCK) 
		...
		SELECT @Day = number FROM fn_Split_withdelemiter(@Term,'.') WHERE id = 2
		...
		BEGIN
			...
			IF(@Day > 0)
			BEGIN
				SELECT @END_Date =  DATEADD(dd,@Day,@END_Date)
			END
			IF(@ROFR_Days > 0)
			BEGIN
		...
		BEGIN TRY 
			...
			BEGIN
				...
				(
					...
					Inserted_By, Inserted_On, Is_Verified, Original_Right_Type, Right_Status
				)
				SELECT 
					...
					@User_Code, GETDATE(), 'Y' AS Is_Verified, Right_Type, 'P'
				FROM  Acq_Rights_Template (NOLOCK)
				...
				BEGIN
					...
					SELECT @Acq_Deal_Rights_Code , Platform_Code FROM Platform (NOLOCK) where Is_Last_Level = 'Y'
				END
				ELSE
				BEGIN
			...
			BEGIN
				...
				BEGIN
					...
					SELECT @Syn_Deal_Rights_Code , Platform_Code FROM Platform (NOLOCK) where Is_Last_Level = 'Y'
				END
				ELSE
				BEGIN
					PRINT '      Selected Platforms : ' + @Platform_Codes	
	...
	if(@Loglevel < 2)Exec [USPLogSQLSteps] '[USP_Deal_Rights_Template]', 'Step 2', 0, 'Procedure Excution Completed', 0, ''
END