CREATE PROCEDURE [dbo].[USPITGetAvailTitleNeoVirtualScroll]
...
BEGIN
	...
	if(@Loglevel< 2)Exec [USPLogSQLSteps] '[USPITGetAvailTitleNeoVirtualScroll]', 'Step 1', 0, 'Started Procedure', 0, ''
	--DECLARE
	--@IsDDL CHAR(2) = 'Y',
	--@UDT TitleCriteria	
	...
	IF OBJECT_ID('tempdb..#tempAvailTitlesFinal') IS NOT NULL DROP TABLE #tempAvailTitlesFinal
		...
		DELETE FROM #tempAvailTitles WHERE Title_Code NOT IN (Select Title_Code from #tempAvailTitles where (@SearchString = '' OR Title_Name like '%'+@SearchString+'%'))
	...
	if(@Loglevel< 2)Exec [USPLogSQLSteps] '[USPITGetAvailTitleNeoVirtualScroll]', 'Step 2', 0, 'Procedure Excution Completed', 0, ''
END