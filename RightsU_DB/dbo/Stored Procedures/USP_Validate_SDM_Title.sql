...
BEGIN
	...
	if(@Loglevel< 2)Exec [USPLogSQLSteps] '[USP_Validate_SDM_Title]', 'Step 1', 0, 'Started Procedure', 0, ''	
		...
		BEGIN TRY
			...
			--END --DECLARATION PART
				...
				--select * from #Acq_Titles_With_Rights
					...
					Inner Join Acq_Deal_Rights adr (NOLOCK) ON adr.Acq_Deal_Rights_Code = art.Acq_Deal_Rights_Code
					Inner join Acq_Deal_Budget adb (NOLOCK) ON adb.Acq_Deal_Code=adr.Acq_Deal_Code
					...
					INNER JOIN Title T (NOLOCK) ON T.Title_Code=art.Title_Code
					Drop Table #Acq_Titles_With_Rights
					...
					Where DRT.Platform_Code Not In (
						Select DISTINCT Platform_Code From [Platform] ap (NOLOCK)
					)
				...
				Begin ----------------- CHECK SUBTITLING And COUNTRY And PLATFORM And TITLE & EPISODE EXISTS OR NOT
					...
					--select * from @Deal_Rights_Subtitling
						...
						Where DRC.Subtitling_Code Not In (
							Select DISTINCT Language_Code From Language (NOLOCK)
						)
						Select Distinct DRC.Title_Code, DRC.Episode_From, DRC.Episode_To, DRC.Platform_Code, DRC.Country_Code, DRC.Subtitling_Code 
						InTo #NA_Subtitling
						From #Temp_Subtitling DRC
				...
				Begin
					...
					From(
						...
						(
							(
							Select ',' + CAST(l.Language_Code AS VARCHAR) From Language l (NOLOCK)
							Where l.Language_Code In(
				...
				Begin  ------------------------ SYNDICATION DUPLICATION VALIDATION
					...
					Select SDR.Syn_Deal_Rights_Code, SDR.Actual_Right_Start_Date, SDR.Actual_Right_End_Date, SDR.Right_Type, SDR.Is_Title_Language_Right, 
						   SDR.Syn_Deal_Code,(Select Count(*) From Syn_Deal_Rights_Subtitling a (NOLOCK) Where a.Syn_Deal_Rights_Code = SDR.Syn_Deal_Rights_Code) 
						   Sub_Cnt, (Select Count(*) From Syn_Deal_Rights_Dubbing a (NOLOCK) Where a.Syn_Deal_Rights_Code = SDR.Syn_Deal_Rights_Code) Dub_Cnt, 
	...
	if(@Loglevel< 2)Exec [USPLogSQLSteps] '[USP_Validate_SDM_Title]', 'Step 2', 0, 'Procedure Excution Completed', 0, ''
END