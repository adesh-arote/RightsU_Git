...
BEGIN
	...
	if(@Loglevel < 2)Exec [USPLogSQLSteps] '[USP_Validate_Delay_Rights_Duplication_Acq]', 'Step 1', 0, 'Started Procedure', 0, ''  
	  ...
	  SET NOCOUNT ON
	   ...
	   DECLARE 
		...
		BEGIN 	
			...
			SELECT 0, Platform_Code FROM Acq_Deal_Rights_Platform (NOLOCK)  WHERE Acq_Deal_Rights_Code = @Acq_Deal_Rights_Code
			------------------------------------
	...
	if(@Loglevel < 2)Exec [USPLogSQLSteps] '[USP_Validate_Delay_Rights_Duplication_Acq]', 'Step 2', 0, 'Procedure Excuting Completed', 0, ''  
END
