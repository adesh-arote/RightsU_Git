CREATE PROCEDURE [dbo].[USP_Validate_And_Save_Assigned_Music_UDT]
(
	@Deal_Type_Code INT,
	@Login_User_Code INT,
	@Link_Show VARCHAR(1),
...
BEGIN
	...
	select @Loglevel = Parameter_Value from System_Parameter_New where Parameter_Name='loglevel'  
	 if(@Loglevel < 2)Exec [USPLogSQLSteps] '[USP_Validate_And_Save_Assigned_Music_UDT]', 'Step 1', 0, 'Started Procedure', 0, ''  
		--SET FMTONLY OFF
		--DECLARE 
		...
		BEGIN
			...
			BEGIN
				...
				IF EXISTS (SELECT * FROM Acq_Deal_Movie_Music_Link (NOLOCK) WHERE Acq_Deal_Movie_Music_Code IN (SELECT DISTINCT Music_Code FROM #Temp_Combination))
				BEGIN
					...
					INNER JOIN Acq_Deal_Movie_Music_Link ADMML (NOLOCK) ON ADMML.Acq_Deal_Movie_Music_Code = TC.Music_Code
					-- Links Join
					INNER JOIN Acq_Deal_Movie ADM_L (NOLOCK) ON ADM_L.Acq_Deal_Movie_Code = ADMML.Link_Acq_Deal_Movie_Code
					INNER JOIN Acq_Deal AD_L (NOLOCK) ON AD_L.Acq_Deal_Code = ADM_L.Acq_Deal_Code
					INNER JOIN Title T (NOLOCK) ON T.Title_Code = ADM_L.Title_Code
		...
		BEGIN
			...
			BEGIN
				IF EXISTS(
					SELECT ADMML.* FROM Acq_Deal_Movie_Music_Link ADMML (NOLOCK)
				...
				BEGIN
					...
					SELECT ADMML.Link_Acq_Deal_Movie_Code, ADMML.Acq_Deal_Movie_Music_Code, ADMML.Episode_No, 'N', 'Duplicate Record' FROM Acq_Deal_Movie_Music_Link ADMML (NOLOCK)
					INNER JOIN #Temp_Combination TC ON TC.Deal_Movie_Code = ADMML.Link_Acq_Deal_Movie_Code 
						AND TC.Music_Code = ADMML.Acq_Deal_Movie_Music_Code AND TC.Episode_No = ADMML.Episode_No
				...
				BEGIN
					...
					INNER JOIN Acq_Deal_Movie ADM (NOLOCK) ON TC.Deal_Movie_Code = adm.Acq_Deal_Movie_Code
					...
					SELECT DISTINCT ADMM.Acq_Deal_Movie_Code, ADMM.Acq_Deal_Movie_Music_Code, 'N' FROM Acq_Deal_Movie_Music ADMM (NOLOCK)
					...
					SELECT Deal_Movie_Code, Is_Warning, Err_Message FROM (
						...
						INNER JOIN Acq_Deal_Movie ADM (NOLOCK) ON ADM.Acq_Deal_Movie_Code = TDM.Deal_Movie_Code
						INNER JOIN Acq_Deal AD (NOLOCK) ON AD.Acq_Deal_Code = ADM.Acq_Deal_Code AND  AD.Deal_Workflow_Status NOT IN ('AR', 'WA')
						AND AD.[Status] = 'T' 
					...
					BEGIN
						...
						INNER JOIN Acq_Deal_Run ADR (NOLOCK) ON ADR.Acq_Deal_Code = ADM.Acq_Deal_Code AND ADM.Acq_Deal_Movie_Code = @dealMovieCode_MusicLibrary
						...
						INNER JOIN Acq_Deal_Run_Channel ADRC (NOLOCK) ON ADRC.Acq_Deal_Run_Code = ADR.Acq_Deal_Run_Code
						LEFT JOIN Acq_Deal_Run_Shows ADRS (NOLOCK) ON ADRS.Acq_Deal_Run_Code = ADR.Acq_Deal_Run_Code

						IF EXISTS (SELECT * FROM #Temp_Run_Music)
						BEGIN
					
							SELECT @Total_No_Of_Play = ISNULL(SUM(No_Of_Play), 0) FROM Acq_Deal_Movie_Music_Link (NOLOCK) WHERE Acq_Deal_Movie_Music_Code = @musicCode 
							...
							SELECT DISTINCT  ADMML.Link_Acq_Deal_Movie_Code, ADMML.Episode_No, ADRC.Channel_Code, ADMML.No_Of_Play FROM Acq_Deal_Movie_Music_Link ADMML (NOLOCK)
							...
							INNER JOIN Acq_Deal_Run ADR (NOLOCK) ON ADR.Acq_Deal_Code = ADM.Acq_Deal_Code 
							INNER JOIN Acq_Deal_Run_Title ADRT (NOLOCK) ON ADRT.Acq_Deal_Run_Code = ADR.Acq_Deal_Run_Code
								AND ADRT.Episode_From = ADM.Episode_Starts_From AND ADRT.Episode_To = ADM.Episode_End_To
							INNER JOIN Acq_Deal_Run_Channel ADRC (NOLOCK) ON ADRC.Acq_Deal_Run_Code = ADR.Acq_Deal_Run_Code
							...
							INNER JOIN Acq_Deal_Movie ADM_2 (NOLOCK) ON ADM_2.Acq_Deal_Movie_Code = TDM_2.Deal_Movie_Code
							INNER JOIN Acq_Deal_Run ADR_2 (NOLOCK) ON ADR_2.Acq_Deal_Code = ADM_2.Acq_Deal_Code
							INNER JOIN Acq_Deal_Run_Title ADRT_2 (NOLOCK) ON ADRT_2.Acq_Deal_Run_Code = ADR_2.Acq_Deal_Run_Code
								AND ADRT_2.Episode_From = ADM_2.Episode_Starts_From AND ADRT_2.Episode_To = ADM_2.Episode_End_To
							INNER JOIN Acq_Deal_Run_Channel ADRC_2  (NOLOCK) ON ADRC_2.Acq_Deal_Run_Code = ADR_2.Acq_Deal_Run_Code
							...
							SELECT TPC.Deal_Movie_Code, @musicCode, 'Y', 
								...
								' ) for channels ' +  STUFF(
									(
										...
										INNER JOIN Channel C (NOLOCK) ON C.Channel_Code = TRC.Channel_Code
							...
							FROM Warning_Channel WC (NOLOCK)
							...
							CASE 
								...
								(
									...
									INNER JOIN Channel C  (NOLOCK) ON C.Channel_Code = TPC.Channel_Code
							...
							BEGIN
								...
								FROM Acq_Deal_Movie ADM  (NOLOCK)
								INNER JOIN Acq_Deal_Run ADR (NOLOCK) ON ADR.Acq_Deal_Code = ADM.Acq_Deal_Code AND ADM.Acq_Deal_Movie_Code = @dealMovieCode_Link
								INNER JOIN Acq_Deal_Run_Title ADRT (NOLOCK) ON ADRT.Acq_Deal_Run_Code = ADR.Acq_Deal_Run_Code
									AND ADRT.Episode_From = ADM.Episode_Starts_From AND ADRT.Episode_To = ADM.Episode_End_To
								INNER JOIN Acq_Deal_Run_Channel ADRC (NOLOCK) ON ADRC.Acq_Deal_Run_Code = ADR.Acq_Deal_Run_Code
								LEFT JOIN Acq_Deal_Run_Shows ADRS (NOLOCK) ON ADRS.Acq_Deal_Run_Code = ADR.Acq_Deal_Run_Code
								...
								BEGIN
									...
									BEGIN
										...
										SELECT @dealMovieCode_Link, @musicCode, 'Y', 'Channels ' +  STUFF(
											(
												...
												INNER JOIN Channel C (NOLOCK) ON C.Channel_Code = TRDM.Channel_Code
									...
									SELECT DISTINCT TRC.Channel_Code FROM Acq_Deal_Movie ADM (NOLOCK)
			...
			BEGIN
				...
				SELECT TOP 1 @totalNoOfSong = Episode_Starts_From FROM Acq_Deal_Movie (NOLOCK) WHERE Acq_Deal_Movie_Code = @Deal_Movie_Code

				DECLARE @recCount INT = 0;
				IF EXISTS(
					SELECT ADMM.* FROM Acq_Deal_Movie_Music ADMM (NOLOCK)
				...
				SELECT @recCount = COUNT(*) FROM (
					...
					UNION 
					SELECT DISTINCT Acq_Deal_Movie_Code, Music_Title_Code FROM Acq_Deal_Movie_Music (NOLOCK) WHERE Acq_Deal_Movie_Code = @Deal_Movie_Code
				) AS TMP
		...
		BEGIN
			...
			FROM #Temp_Error TE
			LEFT JOIN Acq_Deal_Movie ADM (NOLOCK) ON ADM.Acq_Deal_Movie_Code = TE.Deal_Movie_Code
			LEFT JOIN Title T (NOLOCK) ON T.Title_Code = ADM.Title_Code
			LEFT JOIN Music_Title MT (NOLOCK) ON TE.Music_Code = MT.Music_Title_Code
		END
		...
		BEGIN
			...
			FROM #Temp_Error TE
			LEFT JOIN Acq_Deal_Movie ADM (NOLOCK) ON ADM.Acq_Deal_Movie_Code = TE.Deal_Movie_Code
			LEFT JOIN Acq_Deal AD (NOLOCK) ON AD.Acq_Deal_Code = ADM.Acq_Deal_Code
			LEFT JOIN Deal_Type DT (NOLOCK) ON DT.Deal_Type_Code = AD.Deal_Type_Code
			LEFT JOIN Title T (NOLOCK) ON T.Title_Code = ADM.Title_Code
			--- START Join For Music
			LEFT JOIN Acq_Deal_Movie_Music ADMM (NOLOCK) ON ADMM.Acq_Deal_Movie_Music_Code = TE.Music_Code
			LEFT JOIN Acq_Deal_Movie ADM_L (NOLOCK) ON ADM_L.Acq_Deal_Movie_Code = ADMM.Acq_Deal_Movie_Code
			LEFT JOIN Acq_Deal AD_L (NOLOCK) ON AD_L.Acq_Deal_Code = ADM_L.Acq_Deal_Code
			LEFT JOIN Deal_Type DT_L (NOLOCK) ON DT_L.Deal_Type_Code = AD_L.Deal_Type_Code
			LEFT JOIN Title T_L (NOLOCK) ON T_L.Title_Code = ADM_L.Title_Code
			LEFT JOIN Music_Title MT (NOLOCK) ON MT.Music_Title_Code = ADMM.Music_Title_Code
	 ...
	 if(@Loglevel < 2)Exec [USPLogSQLSteps] '[USP_Validate_And_Save_Assigned_Music_UDT]', 'Step 2', 0, 'Procedure Excuting Completed', 0, ''  
END