CREATE PROCEDURE [dbo].[USP_Music_Deal_Schedule_Validation]
	@Music_Deal_Code INT,
...
BEGIN
	...
	if(@Loglevel<2)Exec [USPLogSQLSteps] '[USP_Music_Deal_Schedule_Validation]', 'Step 1', 0, 'Started Procedure', 0, '' 
		...
		BEGIN
			...
			INSERT INTO #TempErrorData(Error_No, Title, Schedule_Date, [Error_Message])
				SELECT DISTINCT @ErrorCount, T.Title_Name, CONVERT(VARCHAR(11), MIN(CAST(BST.Schedule_Item_Log_Date AS DATETIME)),106), @Msg 
					FROM Music_Schedule_Transaction MSt (NOLOCK)
					INNER JOIN BV_Schedule_Transaction BST (NOLOCK) ON BST.BV_Schedule_Transaction_Code = MST.BV_Schedule_Transaction_Code
					inneR JOIN Title T (NOLOCK) ON T.Title_Code = BST.Title_Code
					WHERE MST.Music_Deal_Code = @Music_Deal_Code AND ISNULL(MST.Is_Exception, '') = 'N'
		...
		SET @ErrorCount = @ErrorCount + 1
				...
				, 'Title(s)  ' + T.Title_Name + ' does not exist for Specific.' FROM Music_Schedule_Transaction MST (NOLOCK)
					INNER JOIN BV_Schedule_Transaction BST (NOLOCK) ON BST.BV_Schedule_Transaction_Code = MST.BV_Schedule_Transaction_Code
					INNER JOIN Title T (NOLOCK) ON T.Title_Code = BST.Title_Code 
		...
		IF(@LinkedShowType = 'AS')
			...
			SET @ErrorCount = @ErrorCount + 1
				...
				INSERT INTO #TempErrorData(Error_No, Title, Schedule_Date, [Error_Message])
					SELECT DISTINCT @ErrorCount, T.Title_Name, CONVERT(VARCHAR(11), MIN(CAST(BST.Schedule_Item_Log_Date AS DATETIME)),106)
						, 'Title(s)  ' + T.Title_Name + ' does not exist for All Shows.' FROM Music_Schedule_Transaction MST (NOLOCK)
						INNER JOIN BV_Schedule_Transaction BST (NOLOCK) ON BST.BV_Schedule_Transaction_Code = MST.BV_Schedule_Transaction_Code
						Inner join Title T (NOLOCK) ON T.Title_Code = BST.Title_Code 
						WHERE MST.Music_Deal_Code = @Music_Deal_Code AND ISNULL(MST.Is_Exception, '') = 'N' 
		...
		IF(@LinkedShowType = 'AF')
			...
			SET @ErrorCount = @ErrorCount + 1
				...
				INSERT INTO #TempErrorData(Error_No, Title, Schedule_Date, [Error_Message])
					SELECT DISTINCT @ErrorCount, T.Title_Name, CONVERT(VARCHAR(11), MIN(CAST(BST.Schedule_Item_Log_Date AS DATETIME)),106)
					, 'Title(s)  ' + T.Title_Name + ' does not exist for All Fiction.' FROM Music_Schedule_Transaction MST (NOLOCK)
						INNER JOIN BV_Schedule_Transaction BST (NOLOCK) ON BST.BV_Schedule_Transaction_Code = MST.BV_Schedule_Transaction_Code
						Inner join Title T (NOLOCK) ON T.Title_Code = BST.Title_Code 
						WHERE MST.Music_Deal_Code = @Music_Deal_Code AND ISNULL(MST.Is_Exception, '') = 'N' 
		...
		IF(@LinkedShowType = 'AN')
			...
			SET @ErrorCount = @ErrorCount + 1
				...
				INSERT INTO #TempErrorData(Error_No, Title, Schedule_Date, [Error_Message])
					SELECT DISTINCT @ErrorCount, T.Title_Name, CONVERT(VARCHAR(11), MIN(CAST(BST.Schedule_Item_Log_Date AS DATETIME)),106)
						, 'Title(s)  ' + T.Title_Name + ' does not exist for All Non Fiction.' FROM Music_Schedule_Transaction MST (NOLOCK)
						INNER JOIN BV_Schedule_Transaction BST (NOLOCK) ON BST.BV_Schedule_Transaction_Code = MST.BV_Schedule_Transaction_Code
						Inner join Title T (NOLOCK) ON T.Title_Code = BST.Title_Code 
						WHERE MST.Music_Deal_Code = @Music_Deal_Code AND ISNULL(MST.Is_Exception, '') = 'N' 
		...
		IF(@LinkedShowType = 'AE')
			BEGIN
				...
				INSERT INTO #TempErrorData(Error_No, Title, Schedule_Date, [Error_Message])
					SELECT DISTINCT @ErrorCount, T.Title_Name, CONVERT(VARCHAR(11), MIN(CAST(BST.Schedule_Item_Log_Date AS DATETIME)),106)
						, 'Title(s)  ' + T.Title_Name + ' does not exist for All Event.' FROM Music_Schedule_Transaction MST (NOLOCK)
						INNER JOIN BV_Schedule_Transaction BST (NOLOCK) ON BST.BV_Schedule_Transaction_Code = MST.BV_Schedule_Transaction_Code
						Inner join Title T (NOLOCK) ON T.Title_Code = BST.Title_Code 
						WHERE MST.Music_Deal_Code = @Music_Deal_Code AND ISNULL(MST.Is_Exception, '') = 'N' 
			...
			INSERT INTO #TempErrorData(Error_No, Title, Schedule_Date, [Error_Message])
				SELECT DISTINCT @ErrorCount, T.Title_Name, CONVERT(VARCHAR(11), MIN(CAST(BST.Schedule_Item_Log_Date AS DATETIME)),106), 
				'Channel(s) '+ C.Channel_Name + ' does not exists as run Scheduled for selected title.' FROM Music_Schedule_Transaction MST (NOLOCK)
					INNER JOIN BV_Schedule_Transaction BST (NOLOCK) ON BST.BV_Schedule_Transaction_Code = MST.BV_Schedule_Transaction_Code
					INNER JOIN Title T (NOLOCK) ON T.Title_Code = BST.Title_Code
	...
	if(@Loglevel<2)Exec [USPLogSQLSteps] '[USP_Music_Deal_Schedule_Validation]', 'Step 2', 0, 'Procedure Excution Completed', 0, '' 
END