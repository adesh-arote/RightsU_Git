...
BEGIN
	...
	if(@Loglevel < 2)Exec [USPLogSQLSteps] '[USP_Insert_Title_Import_UDT]', 'Step 1', 0, 'Started Procedure', 0, ''
			...
			IF EXISTS(SELECT TOP 1 Record_Status FROM DM_Title (NOLOCK) WHERE ISNULL(RTRIM(LTRIM(Record_Status)),'') = 'N')
			BEGIN
			...
			IF NOT EXISTS(SELECT TOP 1 Record_Status FROM DM_Title (NOLOCK) WHERE ISNULL(RTRIM(LTRIM(Record_Status)),'') = 'E')
			BEGIN
				PRINT 'IF Condition'
				...
				(
					SELECT [Director Name] DirName FROM DM_Title (NOLOCK) WHERE ISNULL([Director Name], '') <> ''
				) AS a
				CROSS APPLY
				(
				...
				SELECT DISTINCT b.StarCast, 'N' FROM(
					SELECT [Key Star Cast] StarCast FROM DM_Title (NOLOCK) WHERE ISNULL([Key Star Cast], '') <> ''
				) AS a
				CROSS APPLY
				(
				...
				SELECT LTRIM(RTRIM(Name)), 'N', 'Y' FROM #Temp_Director WHERE Name collate SQL_Latin1_General_CP1_CI_AS NOT IN (SELECT talent_name collate SQL_Latin1_General_CP1_CI_AS FROM Talent (NOLOCK))

				INSERT INTO Talent(talent_name, gender, is_active)
				SELECT LTRIM(RTRIM(Name)), 'N', 'Y' FROM #Temp_S WHERE Name collate SQL_Latin1_General_CP1_CI_AS NOT IN (SELECT talent_name collate SQL_Latin1_General_CP1_CI_AS FROM Talent (NOLOCK))
				...
				SELECT LTRIM(RTRIM(Name)), LTRIM(RTRIM(Name)), LTRIM(RTRIM(Name)), (SELECT Deal_Type_Code FROM [Role] (NOLOCK) WHERE Role_Code = @DirectorRoleCode), GETDATE(), 'Y', TalentCode, 'T' FROM #Temp_Director WHERE RoleExists = 'N'

				INSERT INTO Title(Original_Title, Title_Name, Synopsis, Deal_Type_Code, Inserted_On, Is_Active, Reference_Key, Reference_Flag)  
				SELECT LTRIM(RTRIM(Name)), LTRIM(RTRIM(Name)), LTRIM(RTRIM(Name)), (SELECT Deal_Type_Code FROM [Role]  (NOLOCK) WHERE Role_Code = @StarCastRoleCode), GETDATE(), 'Y', TalentCode, 'T' FROM #Temp_S WHERE RoleExists = 'N'
				...
				DECLARE CUR_Title CURSOR For
					...
					FROM DM_Title (NOLOCK) WHERE Record_Status = 'N' -- AND [Title/ Dubbed Title (Hindi)] = 'Aag Ka Gola'
				OPEN CUR_Title
				...
				BEGIN                                              
					...
					BEGIN																														
						...
						SELECT @Title_Language_code = language_code FROM [Language] (NOLOCK) WHERE Language_Name collate SQL_Latin1_General_CP1_CI_AS = LTRIM(RTRIM(ISNULL(@Original_Language_Name, ''))) collate SQL_Latin1_General_CP1_CI_AS	
						...
						) AS a (NOLOCK) ON LTRIM(RTRIM(td.Name)) collate SQL_Latin1_General_CP1_CI_AS = LTRIM(RTRIM(a.number)) collate SQL_Latin1_General_CP1_CI_AS
						...
						) AS a (NOLOCK) ON LTRIM(RTRIM(ts.Name)) collate SQL_Latin1_General_CP1_CI_AS = LTRIM(RTRIM(a.number)) collate SQL_Latin1_General_CP1_CI_AS
	...
	if(@Loglevel < 2)Exec [USPLogSQLSteps] '[USP_Insert_Title_Import_UDT]', 'Step 2', 0, 'Procedure Excution Completed', 0, ''	
END