...
BEGIN  
 ...
 BEGIN/*INSERT DATA IN TEMP TABLES*/  
  ...
  From BMS_Schedule_Exception BSE  
  INNER JOIN BMS_Schedule_Process_Data_Temp BST ON BST.BMS_Schedule_Process_Data_Temp_Code = BSE.BMS_Schedule_Process_Data_Temp_Code  
  INNER JOIN Email_Notification_Msg EMN ON EMN.Email_Notification_Msg_Code = BSE.Email_Notification_Msg_Code  
  INNER JOIN Channel CH ON CH.Channel_Code = BST.Channel_Code  
  LEFT JOIN Content_Channel_Run CCR ON CCR.Content_Channel_Run_Code = BST.Content_Channel_Run_Code  
  LEFT JOIN Title_Content TC ON TC.Title_Content_Code = CCR.Title_Content_Code  
  LEFT JOIN Title T ON T.Title_Code = TC.Title_Code WHERE IsMailSent= 'N'  
