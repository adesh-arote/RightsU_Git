CREATE PROC [dbo].[USP_UpdateParentHIDCodeForSameTitle]
...
BEGIN    
	...
	if(@Loglevel < 2)Exec [USPLogSQLSteps] '[USP_UpdateParentHIDCodeForSameTitle]', 'Step 1', 0, 'Started Procedure', 0, '' 
	...
	BEGIN
		...
		FROM BV_HouseId_Data (NOLOCK) where BV_Title = @Program  and Is_Mapped = 'Y' and ISNULL(Mapped_Deal_Title_Code,0)  <> ''
		FOR XML PATH('')), 1, 1, '' ))
	...
	BEGIN
		...
		BEGIN
			DECLARE @Cnt INT
			SELECT @Cnt = COUNT(BV_HouseId_Data_Code) FROM BV_HouseId_Data (NOLOCK) WHERE BV_Title = @Program AND Episode_No = @Episode
				AND ISNULL([Type], 'S') = 'S' AND Is_Mapped = 'N'

			IF @Cnt > 0
			BEGIN
				DECLARE @EmailMsg_New NVARCHAR(MAX)
				SELECT @EmailMsg_New = Email_Msg FROM Email_Notification_Msg (NOLOCK) WHERE LTRIM(RTRIM(Email_Msg_For)) = 'SchMapMissing_AsRun' and [Type] = 'A'
	...
	if(@Loglevel < 2)Exec [USPLogSQLSteps] '[USP_UpdateParentHIDCodeForSameTitle]', 'Step 2', 0, 'Procedure Excuting Completed', 0, '' 
END
