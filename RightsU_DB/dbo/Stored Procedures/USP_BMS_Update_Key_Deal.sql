...
BEGIN
	...
	if(@Loglevel  < 2)Exec [USPLogSQLSteps] '[USP_BMS_Update_Key_Deal]', 'Step 1', 0, 'Started Procedure', 0, ''
	--	DECLARE @strXml VARCHAR(MAX), @Type VARCHAR(10) = 'BMS_D'
		...
		BEGIN
			...
			INSERT INTO #Temp_Updated_Keys(BMS_Key, Code)
			SELECT BMS_Key, REPLACE(RU_Code, 'RUBMSBVD', '')  AS Code  FROM BMS_Response_Data (NOLOCK) WHERE Type_For = 'RUBVD'
		...
		BEGIN
			...
			INSERT INTO #Temp_Updated_Keys(BMS_Key, Code)
			SELECT BMS_Key, REPLACE(RU_Code, 'RUBMSBVA', '')  AS Code  FROM BMS_Response_Data (NOLOCK) WHERE Type_For = 'RUBVA'
		...
		BEGIN
			INSERT INTO #Temp_Updated_Keys(BMS_Key, Code)
			SELECT BMS_Key, REPLACE(RU_Code, 'RUBMSBVPV', '')  AS Code  FROM BMS_Response_Data (NOLOCK) WHERE Type_For = 'RUBVPV'
		...
		BEGIN		
			...
			INSERT INTO #Temp_Updated_Keys(BMS_Key, Code)
			SELECT BMS_Key, REPLACE(RU_Code, 'RUBMSBVDC', '')  AS Code  FROM BMS_Response_Data (NOLOCK) WHERE Type_For = 'RUBVDC'
		...
		BEGIN		
			...
			INSERT INTO #Temp_Updated_Keys(BMS_Key, Code)
			SELECT BMS_Key, REPLACE(RU_Code, 'RUBMSBVDCR', '')  AS Code  FROM BMS_Response_Data (NOLOCK) WHERE Type_For = 'RUBVDCR'
	...
	if(@Loglevel  < 2)Exec [USPLogSQLSteps] '[USP_BMS_Update_Key_Deal]', 'Step 2', 0, 'Procedure Excution Completed', 0, ''
END