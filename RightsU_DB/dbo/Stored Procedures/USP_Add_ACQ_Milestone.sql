...
BEGIN
	...
	if(@Loglevel  < 2)Exec [USPLogSQLSteps] '[USP_Add_ACQ_Milestone]', 'Step 1', 0, 'Started Procedure', 0, ''
		SET NOCOUNT ON;
		...
		INNER JOIN [Platform] P (NOLOCK) ON P.Platform_Code = ADRP.Platform_Code AND ISNULL(P.Is_No_Of_Run, '') = 'Y'
		INNER JOIN Title T (NOLOCK) ON T.Title_Code = ADRT.Title_Code
		INNER JOIN Milestone_Type MT (NOLOCK) ON MT.Milestone_Type_Code = ADR.Milestone_Type_Code
		LEFT JOIN Acq_Deal_Run ADRun (NOLOCK) ON ADRun.Acq_Deal_Code = ADR.Acq_Deal_Code
		LEFT JOIN Acq_Deal_Run_Title ADRunT (NOLOCK) ON ADRun.Acq_Deal_Run_Code = ADRunT.Acq_Deal_Run_Code 
		...
		AND ADR.Acq_Deal_Rights_Code NOT IN (
			SELECT DISTINCT ADRP.Acq_Deal_Rights_Code FROM Acq_Deal_Rights_Platform ADRP (NOLOCK) WHERE ADR.Acq_Deal_Rights_Code = ADRP.Acq_Deal_Rights_Code
			AND ADRP.Platform_Code IN (SELECT DISTINCT P.Platform_Code FROM [Platform] P (NOLOCK) WHERE ISNULL(P.Is_No_Of_Run, '') = 'Y')
		)
	
	if(@Loglevel  < 2)Exec [USPLogSQLSteps] '[USP_Add_ACQ_Milestone]', 'Step 2', 0, 'Procedure Excution Completed', 0, ''
	END