CREATE PROCEDURE USPITCuratedPreview_Syn
@BVCode NVARCHAR(MAX),
...
=============================================*/
 ...
 --   INSERT INTO @UDT VALUES('Agreement No','A-2013-00045','EQ','OR','N','4','GRP1')
		...
		*/
		...
		BEGIN---------------RIghts Related Criteria Start--------------
			...
			BEGIN
				...
				FROM Syn_Deal_Rights adr1
				INNER JOIN @tblRights tr ON adr1.Syn_Deal_Rights_Code = tr.Syn_Deal_Rights_Code --AND adr1.PA_Right_Type = 'PR'
				INNER JOIN Syn_Deal_Rights_Territory adrc ON adrc.Syn_Deal_Rights_Code = adr1.Syn_Deal_Rights_Code --AND ISNULL(adr1.Right_Type, '') <> '' AND ISNULL(adr1.Is_Tentative, 'N') = 'N'
			...
			BEGIN
				...
				FROM Syn_Deal_Rights adr1
				INNER JOIN @tblRights tr ON adr1.Syn_Deal_Rights_Code = tr.Syn_Deal_Rights_Code --AND adr1.PA_Right_Type = 'PR'
				INNER JOIN Syn_Deal_Rights_Subtitling adrs ON adrs.Syn_Deal_Rights_Code = adr1.Syn_Deal_Rights_Code AND ISNULL(adr1.Right_Type, '') <> '' AND ISNULL(adr1.Is_Tentative, 'N') = 'N'
			...
			BEGIN
				...
				FROM Syn_Deal_Rights adr1
				INNER JOIN @tblRights tr ON adr1.Syn_Deal_Rights_Code = tr.Syn_Deal_Rights_Code --AND adr1.PA_Right_Type = 'PR'
				INNER JOIN Syn_Deal_Rights_Dubbing adrs ON adrs.Syn_Deal_Rights_Code = adr1.Syn_Deal_Rights_Code AND ISNULL(adr1.Right_Type, '') <> '' AND ISNULL(adr1.Is_Tentative, 'N') = 'N'
			...
			--Select 'a'
				...
				SELECT DISTINCT Syn_Deal_Rights_Code FROM Syn_Deal_Rights_Platform adrp 
	...
	BEGIN
		...
		FROM #TempRights tmd
		INNER JOIN Syn_Deal_Rights_Territory tt ON tmd.Syn_Deal_Rights_Code = tt.Syn_Deal_Rights_Code AND tt.Territory_Type = 'I'
		INNER JOIN Country cn ON tt.Country_Code = cn.Country_Code
		
		INSERT INTO #TempTerritory
		SELECT DISTINCT tmd.Syn_Deal_Rights_Code, cn.Territory_Name 
		FROM #TempRights tmd
		INNER JOIN Syn_Deal_Rights_Territory tt ON tmd.Syn_Deal_Rights_Code = tt.Syn_Deal_Rights_Code AND tt.Territory_Type = 'G'
	...
	BEGIN
		...
		FROM #tempRights tmd
		INNER JOIN Syn_Deal_Rights_Subtitling tb ON tmd.Syn_Deal_Rights_Code= tb.Syn_Deal_Rights_Code AND tb.Language_Type = 'L'
		INNER JOIN Language l ON tb.Language_Code = l.Language_Code
		
		INSERT INTO #TempLang
		SELECT DISTINCT  tmd.Syn_Deal_Rights_Code, l.Language_Group_Name 
		FROM #tempRights tmd
		INNER JOIN Syn_Deal_Rights_Subtitling tb ON tmd.Syn_Deal_Rights_Code = tb.Syn_Deal_Rights_Code AND tb.Language_Type = 'G'
	...
	BEGIN
		...
		SELECT DISTINCT tmd.Syn_Deal_Rights_Code, l.Language_Name 
		FROM #tempRights tmd
		INNER JOIN Syn_Deal_Rights_Dubbing tb ON tmd.Syn_Deal_Rights_Code= tb.Syn_Deal_Rights_Code AND tb.Language_Type = 'L'
		INNER JOIN Language l ON tb.Language_Code = l.Language_Code
		
		INSERT INTO #TempLang
		SELECT DISTINCT tmd.Syn_Deal_Rights_Code, l.Language_Group_Name 
		FROM #tempRights tmd
		INNER JOIN Syn_Deal_Rights_Dubbing tb ON tmd.Syn_Deal_Rights_Code = tb.Syn_Deal_Rights_Code AND tb.Language_Type = 'G'
		INNER JOIN Language_Group l ON tb.Language_Group_Code = l.Language_Group_Code
	...
	BEGIN --------Platform start
		...
		FROM #tempRights tmd
		INNER JOIN Syn_Deal_Rights_Platform tp ON tmd.Syn_Deal_Rights_Code = tp.Syn_Deal_Rights_Code
		
		INSERT INTO #TempPlatformsHL(Syn_Deal_Rights_Code, MediaPlatforms, ExploitationPlatforms)
		SELECT DISTINCT Syn_Deal_Rights_Code, Media_Platform, ExploitatiON_Platform FROM (
