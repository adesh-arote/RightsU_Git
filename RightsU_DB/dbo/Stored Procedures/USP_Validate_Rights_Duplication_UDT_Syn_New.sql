CREATE Proc [dbo].[USP_Validate_Rights_Duplication_UDT_Syn_New]
As
Begin
	Declare @Loglevel int 
	select @Loglevel = Parameter_Value from System_Parameter_New  where Parameter_Name='loglevel'   
	if(@Loglevel < 2)Exec [USPLogSQLSteps] '[USP_Validate_Rights_Duplication_UDT_Syn_New]', 'Step 1', 0, 'Started Procedure', 0, ''

		If((Select Count(*) From Syn_Deal_Rights_Process_Validation (NOLOCK) Where Status = 'W') = 0)
		Begin
			...
			Begin Try
				...
				Select @CallFrom = Case When IsNull(Is_Pushback, 'N') = 'N' Then 'SR' Else 'SP' End  From Syn_Deal_Rights  (NOLOCK)
				...
				Select 
					...
					@Deal_Rights_Code = Syn_Deal_Rights_Code, --Case When @CallFrom = 'SR' Then dr.Deal_Rights_Code Else dr.Deal_Rights_Code End,
				...
				From Syn_Deal_Rights dr (NOLOCK) Where Syn_Deal_Rights_Code = @Syn_Deal_Rights_Code
			...
			Begin Catch
				...
				DECLARE @DatabaseEmail_Profile varchar(200), @EmailBody VARCHAR(max) = ''
	 ...
	 if(@Loglevel < 2) Exec [USPLogSQLSteps] '[USP_Validate_Rights_Duplication_UDT_Syn_New]', 'Step 2', 0, 'Procedure Excution Completed', 0, ''
End