...
BEGIN
	...
	if(@Loglevel< 2)Exec [USPLogSQLSteps] '[USPListResolveConflict]', 'Step 1', 0, 'Started Procedure', 0, ''
	--DECLARE
	...
	--	@PageNo INT = 1,
		...
		INSERT INTO #tempMasterLog(DMMasterLogCode,  MasterType, Type, ImportData, MappedTo, MappedBy, ActionBy, MappedDate, DMMasterImportCode,MUSIC_ALBUM)
		SELECT DISTINCT DML.DM_Master_Log_Code, DML.Master_Type,
			CASE WHEN DML.Master_Type = 'TA' THEN TR.Role
				 ...
				 WHEN DML.Master_Type = 'BA' THEN 'Studio'
				 WHEN DML.Master_Type = 'LA' THEN 'Lab'
				 WHEN DML.Master_Type = 'VE' THEN 'Version'
				 WHEN DML.Master_Type = 'MP' THEN 'MPAA Rating'
			END AS Type, 
			DML.Name, 
			CASE WHEN DML.Master_Type = 'TA' THEN T.Talent_Name
				 ...
				 WHEN DML.Master_Type = 'MA' AND DML.Music_Album = 'Album' THEN MA.Music_Album_Name
				 WHEN DML.Master_Type = 'MA' AND DML.Music_Album = 'Movie' THEN MAT.Title_Name
				 WHEN DML.Master_Type = 'MA' AND DML.Music_Album = 'Show' THEN MAT.Title_Name
				 ...
				 WHEN DML.Master_Type = 'BA' THEN BA.Banner_Name
				 WHEN DML.Master_Type = 'LA' THEN LA.AL_Lab_Name
				 WHEN DML.Master_Type = 'VE' THEN VE.Version_Name
				 WHEN DML.Master_Type = 'MP' THEN E.Columns_Value
			END AS MappedTo,
		...
		CASE WHEN DML.Master_Type = 'MA' AND DML.Music_Album = 'Album' THEN 'Album'
		     WHEN DML.Master_Type = 'MA' AND DML.Music_Album = 'Movie' THEN 'Movie'
			 WHEN DML.Master_Type = 'MA' AND DML.Music_Album = 'Show' THEN 'Show' Else '' END as  MUSIC_ALBUM
		FROM DM_Master_Log DML (NOLOCK)
		LEFT JOIN Users U (NOLOCK) ON DML.Action_By = U.Users_Code
		LEFT JOIN Talent T (NOLOCK) ON T.Talent_Code = DML.Master_Code
		LEFT JOIN Music_Label LB (NOLOCK) ON LB.Music_Label_Code = DML.Master_Code
		LEFT JOIN Genres G (NOLOCK) ON G.Genres_Code = DML.Master_Code
		LEFT JOIN Program PG (NOLOCK) ON PG.Program_Code = DML.Master_Code
		LEFT JOIN Music_Album MA (NOLOCK) ON MA.Music_Album_Code = DML.Master_Code
		LEFT JOIN Title MAT (NOLOCK) ON MAT.Title_Code = DML.Master_Code
		LEFT JOIN Music_Language ML (NOLOCK) ON ML.Music_Language_Code = DML.Master_Code
		LEFT JOIN Music_Theme MT (NOLOCK) ON MT.Music_Theme_Code = DML.Master_Code
		LEFT JOIN Deal_Type DT (NOLOCK) ON DT.Deal_Type_Code = DML.Master_Code
		LEFT JOIN Language TL (NOLOCK) ON TL.Language_Code = DML.Master_Code
		LEFT JOIN Extended_Columns_Value ECV (NOLOCK) ON ECV.Columns_Value_Code = DML.Master_Code
		LEFT JOIN Music_Title TM (NOLOCK) ON TM.Music_Title_Code = DML.Master_Code
		LEFT JOIN Banner BA (NOLOCK) ON BA.Banner_Code = DML.Master_Code
		LEFT JOIN AL_Lab LA (NOLOCK) ON LA.AL_Lab_Code = DML.Master_Code
		LEFT JOIN Version VE (NOLOCK) ON VE.Version_Code = DML.Master_Code
		LEFT JOIN #TempRoles TR ON DML.DM_Master_Log_Code = TR.DM_Master_Log_Code
	    LEFT JOIN Extended_Columns_Value E (NOLOCK) ON E.Columns_Value_Code = DML.Master_Code
		...
		BEGIN
			...
			FROM DM_Music_Title DMT (NOLOCK)
			...
			CASE WHEN (t.MasterType = 'TA' AND t.Type = 'Music Composer')THEN (SELECT TMS.Composers where TMS.Composers COLLATE SQL_Latin1_General_CP1_CI_AS IN (t.ImportData COLLATE SQL_Latin1_General_CP1_CI_AS)) 
				 ...
				 WHEN t.MasterType = 'MA' AND t.MUSIC_ALBUM = 'Album' THEN (SELECT DMT.Movie_Album WHERE DMT.Music_Album_Type = 'Album' AND DMT.Movie_Album COLLATE SQL_Latin1_General_CP1_CI_AS IN(SELECT t.ImportData collate SQL_Latin1_General_CP1_CI_AS))
				 WHEN t.MasterType = 'MA' AND t.MUSIC_ALBUM = 'Show' THEN (SELECT DMT.Movie_Album WHERE DMT.Music_Album_Type = 'Show' AND DMT.Movie_Album COLLATE SQL_Latin1_General_CP1_CI_AS IN(SELECT t.ImportData collate SQL_Latin1_General_CP1_CI_AS))
				 WHEN t.MasterType = 'MA' AND t.MUSIC_ALBUM = 'Movie' THEN (SELECT DMT.Movie_Album WHERE DMT.Music_Album_Type = 'Movie' AND DMT.Movie_Album COLLATE SQL_Latin1_General_CP1_CI_AS IN(SELECT t.ImportData collate SQL_Latin1_General_CP1_CI_AS))
			...
			INNER JOIN DM_Music_Title DMT (NOLOCK) ON DMT.DM_Master_Import_Code = t.DMMasterImportCode
		...
		BEGIN
			...
			FROM DM_Title_Import_Utility_Data (NOLOCK)  DT WHERE DM_Master_Import_Code =  @DM_Master_Import_Code AND DM_Title_Import_Utility_Data_Code = @Code

			INSERT INTO #temp(Type, ImportData, MappedTo, MappedBy, ActionBy, MappedDate)
			SELECT Distinct Type,ImportData,MappedTo, MappedBy, ActionBy, MappedDate
			FROM #tempMasterLog t
			WHERE T.DMMasterLogCode IN (SELECT DM_Master_Log_Code FROM DM_Master_Log (NOLOCK) WHERE DM_Master_Import_Code = @DM_Master_Import_Code and @Str_Contenate like '%'+Name+'%')
		END
		...
		BEGIN
			...
			MappedTo, MappedBy, ActionBy, MappedDate
			FROM #tempMasterLog t
			INNER JOIN DM_Content_Music DCM (NOLOCK) ON DCM.DM_Master_Import_Code = t.DMMasterImportCode   
	...
	if(@Loglevel< 2)Exec [USPLogSQLSteps] '[USPListResolveConflict]', 'Step 2', 0, 'Procedure Excution Completed', 0, ''
END