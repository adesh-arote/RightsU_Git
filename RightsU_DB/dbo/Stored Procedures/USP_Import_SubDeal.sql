...
BEGIN  
	...
	if(@Loglevel < 2)Exec [USPLogSQLSteps] '[USP_Import_SubDeal]', 'Step 1', 0, 'Started Procedure', 0, ''
		 SET NOCOUNT ON  
		 --I- Insert Row  
		 ...
		 WHILE(@RowNo > 0)  
		  BEGIN  
		   ...
		   SELECT @RoleCode = ISNULL(Role_Code,0) FROM Role (NOLOCK) where Role_Name = @ModeOfAcquisition AND Role_Type = 'A'  
		   IF (@RoleCode = 0)  
		   BEGIN  
		   ...
		   SELECT @Business_Unit_Code = ISNULL(Business_Unit_Code,0) FROM Business_Unit (NOLOCK) WHERE Business_Unit_Name = @BusinessUnit AND Is_Active = 'Y'  
		   ...
		   SELECT @Deal_Type_Code_Sub_Deal =  ISNULL(Deal_Type_Code,0) FROM Deal_Type (NOLOCK) WHERE Deal_Type_Name = @Deal_Type_Name AND Is_Active = 'Y'  
		   ...
		   SELECT @MasterDealTitleCode = ISNULL(Title_Code,0) FROM Title (NOLOCK) where Title_name = @MasterDealTitle AND Is_Active = 'Y'  
		   ...
		   IF NOT EXISTS (SELECT Agreement_No FROM Acq_Deal (NOLOCK) WHERE Agreement_No = @MasterDealAgreementNo AND Is_Active = 'Y' AND Business_Unit_Code = @Business_Unit_Code)  
		   BEGIN  
		   ...
		   BEGIN  
			...
			FROM Acq_Deal_Movie ADM (NOLOCK) INNER JOIN Title T  (NOLOCK) on T.Title_Code = ADM.Title_Code   
		   ...
		   INNER JOIN Talent AS S (NOLOCK) ON S.Talent_Name = C.number -- ltrin and rtrin for number  
		   ...
		   BEGIN  
			...
			FROM #temp_Talent_Name AS C  
			INNER JOIN Talent AS S  (NOLOCK) ON S.Talent_Name = C.number)  
			FOR XML PATH ('')), 1, 1, '')   
		   ...
		   BEGIN  
			...
			FROM #temp_Talent_Name AS C   
			INNER JOIN Talent AS S (NOLOCK) ON S.Talent_Name = C.number  
			INNER JOIN Title T (NOLOCK) ON T.Reference_Key  = s.Talent_Code  
		 ...
		 SELECT @Error_Count = COUNT(*)  FROM  ImportSubDeal   (NOLOCK)
		 WHERE  [IsProcessed] = 'E'  
	...
	if(@Loglevel < 2)Exec [USPLogSQLSteps] '[USP_Import_SubDeal]', 'Step 2', 0, 'Procedure Excution Completed', 0, ''
END