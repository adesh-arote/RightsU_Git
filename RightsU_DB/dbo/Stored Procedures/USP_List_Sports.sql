...
BEGIN
	...
	if(@Loglevel < 2)Exec [USPLogSQLSteps] '[USP_List_Sports]', 'Step 1', 0, 'Started Procedure', 0, '' 
		SET FMTONLY OFF
		...
		If(@Title_Codes = '')
		Begin
			Select @Title_Codes = @Title_Codes + ',' + Cast(Title_Code As Varchar) From Acq_Deal_Movie (NOLOCK) Where Acq_Deal_Code = @Acq_Deal_Code
		End

		If(@Deal_Movie_Codes = '')
		Begin
			Select @Deal_Movie_Codes = @Deal_Movie_Codes + ',' + Cast(Acq_Deal_Movie_Code As Varchar) From Acq_Deal_Movie (NOLOCK) Where Acq_Deal_Code = @Acq_Deal_Code
		End

		Declare CUR_Sport Cursor For Select Acq_Deal_Sport_Code From Acq_Deal_Sport (NOLOCK) Where Acq_Deal_Code = @Acq_Deal_Code
		Open CUR_Sport
		...
		Begin
			...
			Begin
				...
				From Acq_Deal_Sport_Title adrt  (NOLOCK)
				Inner Join Title t (NOLOCK) On adrt.Title_Code = t.Title_Code And Acq_Deal_Sport_Code = @Sport_Code 
				INNER JOIN Acq_Deal_Movie ADM (NOLOCK) ON ADM.Acq_Deal_Code = @Acq_Deal_Code AND ADM.Title_Code = adrt.Title_Code 
			...
			Begin
				...
				Select @Title_Names = @Title_Names + Title_Name + ', ' From (
					...
					From Title t (NOLOCK) Inner Join Acq_Deal_Sport_Title adrt (NOLOCK) On t.Title_Code = adrt.Title_Code 
					INNER JOIN Acq_Deal_Movie ADM (NOLOCK) ON ADM.Acq_Deal_Code = @Acq_Deal_Code AND ADM.Title_Code = adrt.Title_Code 
					AND ISNULL(ADM.Episode_Starts_From, 0) = ISNULL(adrt.Episode_From, 0) AND ISNULL(ADM.Episode_End_To, 0) = ISNULL(adrt.Episode_To, 0)
			...
			Begin
				Select @Name = @Name + bm.Broadcast_Mode_Name + ', ' From Acq_Deal_Sport_Broadcast adsb  (NOLOCK)
				Inner Join Broadcast_Mode bm (NOLOCK) On adsb.Broadcast_Mode_Code = bm.Broadcast_Mode_Code
			...
			--#Temp_OB
			If((Select Count(*) From Acq_Deal_Sport_Broadcast (NOLOCK) Where [Type] = 'OB' And Acq_Deal_Sport_Code = @Sport_Code) > 0)
			Begin
				Select @Name = @Name + bm.Broadcast_Mode_Name + ', ' From Acq_Deal_Sport_Broadcast adsb  (NOLOCK)
				Inner Join Broadcast_Mode bm  (NOLOCK) On adsb.Broadcast_Mode_Code = bm.Broadcast_Mode_Code
			...
			--#Temp_ST
			If((Select Count(*) From Acq_Deal_Sport_Platform (NOLOCK) Where [Type] = 'ST' And Acq_Deal_Sport_Code = @Sport_Code) > 0)
			Begin
				Select @Name = @Name + p.Platform_Hiearachy + ', ' From Acq_Deal_Sport_Platform adsp (NOLOCK)
				Inner Join [Platform] p (NOLOCK) On adsp.Platform_Code = p.Platform_Code
			...
			--#Temp_SM
			If((Select Count(*) From Acq_Deal_Sport_Platform (NOLOCK) Where [Type] = 'SM' And Acq_Deal_Sport_Code = @Sport_Code) > 0)
			Begin
				Select @Name = @Name + p.Platform_Hiearachy + ', ' From Acq_Deal_Sport_Platform adsp (NOLOCK)
				Inner Join [Platform] p (NOLOCK) On adsp.Platform_Code = p.Platform_Code
	...
	if(@Loglevel < 2)Exec [USPLogSQLSteps] '[USP_List_Sports]', 'Step 2', 0, 'Procedure Excution Completed', 0, '' 
END