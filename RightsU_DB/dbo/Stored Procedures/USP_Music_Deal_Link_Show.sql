CREATE PROCEDURE [dbo].[USP_Music_Deal_Link_Show]
...
BEGIN
	...
	if(@Loglevel  < 2)Exec [USPLogSQLSteps] '[USP_Music_Deal_Link_Show]', 'Step 1', 0, 'Started Procedure', 0, ''
		--DECLARE
		...
		BEGIN
			...
			LEFT JOIN Music_Deal_LinkShow MDLS (NOLOCK) ON MDLS.Title_Code = T.Title_Code AND MDLS.Music_Deal_Code = @Music_Deal_Code
		...
		BEGIN
			...
			INNER JOIN Acq_Deal_Run_Title ADRT (NOLOCK) ON ADRT.Acq_Deal_Run_Code = ADRC.Acq_Deal_Run_Code
			INNER JOIN Title T (NOLOCK) ON T.Title_Code = ADRT.Title_Code 
			INNER JOIN Music_Deal_LinkShow MDLS (NOLOCK) ON MDLS.Title_Code = T.Title_Code AND MDLS.Music_Deal_Code = @Music_Deal_Code
			INNER JOIN Music_Deal_Channel MDC (NOLOCK) ON MDC.Music_Deal_Code = @Music_Deal_Code and MDC.Channel_Code = ADRC.Channel_Code
		END
		...
		STUFF((
			...
			INNER JOIN Acq_Deal_Run_Title ADRT (NOLOCK) ON ADRT.Title_Code = T.Title_Code
	...
	if(@Loglevel  < 2)Exec [USPLogSQLSteps] '[USP_Music_Deal_Link_Show]', 'Step 2', 0, 'Procedure Excution Completed', 0, ''
END