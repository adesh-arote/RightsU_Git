CREATE PROCEDURE [dbo].[USP_List_Assign_Music]
(
	@Acq_Deal_Code INT
)
AS
BEGIN
	...
	if(@Loglevel < 2)Exec [USPLogSQLSteps] '[USP_List_Assign_Music]', 'Step 1', 0, 'Started Procedure', 0, ''
		...
		SELECT TOP 1 @Deal_Type_Condition = dbo.UFN_GetDealTypeCondition(Deal_Type_Code) FROM Acq_Deal (NOLOCK) WHERE Acq_Deal_Code = @Acq_Deal_Code
		...
		BEGIN
			...
			INNER JOIN Acq_Deal_Movie ADM (NOLOCK) ON ADM.Acq_Deal_Movie_Code = ADMM.Acq_Deal_Movie_Code AND ADM.Acq_Deal_Code = @Acq_Deal_Code
			INNER JOIN Title T (NOLOCK) ON T.Title_Code = ADM.Title_Code
			INNER JOIN Music_Title MT (NOLOCK) ON ADMM.Music_Title_Code = MT.Music_Title_Code
		END
		ELSE
		BEGIN
			...
			(
				(
					...
					FROM Acq_Deal_Movie_Music_Link ADMML (NOLOCK) WHERE ADMML.Link_Acq_Deal_Movie_Code = TEMP.Link_Acq_Deal_Movie_Code 
			...
			(
				(
					...
					FROM Acq_Deal_Movie_Music ADMM (NOLOCK)
					INNER JOIN Music_Title_Label MTL (NOLOCK) ON MTL.Music_Title_Code = ADMM.Music_Title_Code
			...
			INNER JOIN Acq_Deal_Movie ADM (NOLOCK) ON ADM.Acq_Deal_Movie_Code = TEMP.Link_Acq_Deal_Movie_Code AND ADM.Acq_Deal_Code = @Acq_Deal_Code
			INNER JOIN Title T (NOLOCK) ON T.Title_Code = ADM.Title_Code
			--- Join for "Linked Music with Music Library"
			INNER JOIN Acq_Deal_Movie_Music ADMM (NOLOCK) ON ADMM.Acq_Deal_Movie_Music_Code = TEMP.Acq_Deal_Movie_Music_Code
			INNER JOIN Acq_Deal_Movie ADM_L (NOLOCK) ON ADM_L.Acq_Deal_Movie_Code = ADMM.Acq_Deal_Movie_Code
			INNER JOIN Title T_L (NOLOCK) ON T_L.Title_Code = ADM_L.Title_Code
			INNER JOIN Music_Title MT_L (NOLOCK) ON ADMM.Music_Title_Code = MT_L.Music_Title_Code
	...
	if(@Loglevel < 2)Exec [USPLogSQLSteps] '[USP_List_Assign_Music]', 'Step 2', 0, 'Procedure Excution Completed', 0, ''
END