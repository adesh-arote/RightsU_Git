CREATE PROCEDURE [dbo].[USP_Schedule_Email_Notification]
(
...
BEGIN
	...
	if(@Loglevel<2)Exec [USPLogSQLSteps] '[USP_Schedule_Email_Notification]', 'Step 1', 0, 'Started Procedure', 0, ''
		DECLARE @EmailMsg NVARCHAR(200)
		SELECT @EmailMsg = Email_Msg FROM Email_Notification_Msg (NOLOCK) WHERE LTRIM(RTRIM(Email_Msg_For)) = @Email_Msg_For
		AND [Type] = 'S'
	...
	if(@Loglevel<2)Exec [USPLogSQLSteps] '[USP_Schedule_Email_Notification]', 'Step 2', 0, 'Procedure Excuting Completed', 0, ''
END
