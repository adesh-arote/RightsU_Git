...
BEGIN
	...
	if(@Loglevel< 2)Exec [USPLogSQLSteps] '[USP_Title_Import_Utility_PI]', 'Step 1', 0, 'Started Procedure', 0, '' 
		...
		BEGIN
			...
			SELECT @DM_TIU_Count = COUNT(Display_Name) FROM DM_Title_Import_Utility (NOLOCK) WHERE Is_Active='Y' AND Import_Type = @TitleType
			...
			BEGIN
				...
				BEGIN 
					...
					SELECT @ColNameMismatch =  STUFF(( SELECT  ', ' + Display_Name FROM  DM_Title_Import_Utility (NOLOCK) where Display_Name COLLATE SQL_Latin1_General_CP1_CI_AS NOT IN
					(
						SELECT DISTINCT  CN.Column_Name  FROM DM_Title_Import_Utility TIU (NOLOCK)
						INNER JOIN #ColumnName CN ON CN.Column_Name COLLATE SQL_Latin1_General_CP1_CI_AS = TIU.Display_Name
						WHERE TIU.Is_Active = 'Y' AND TIU.Import_Type = @TitleType
					) AND  Is_Active = 'Y' AND Import_Type = @TitleType  FOR XML PATH(''), TYPE).value('.', 'NVARCHAR(MAX)'), 1, 1, '')
	
					SELECT @ColNameMismatch = ISNULL(@ColNameMismatch ,'') + ',' +  ISNULL( STUFF(( SELECT  ', ' + Column_Name
					FROM #ColumnName WHERE Column_Name <> '' AND
					Column_Name COLLATE SQL_Latin1_General_CP1_CI_AS NOT IN (SELECT Display_Name FROM  DM_Title_Import_Utility (NOLOCK) WHERE Is_Active='Y' AND Import_Type = @TitleType)
					FOR XML PATH(''), TYPE).value('.', 'NVARCHAR(MAX)'), 1, 1, ''),'')
	...
	if(@Loglevel< 2)Exec [USPLogSQLSteps] '[USP_Title_Import_Utility_PI]', 'Step 2', 0, 'Procedure Excuting Completed', 0, '' 
END
