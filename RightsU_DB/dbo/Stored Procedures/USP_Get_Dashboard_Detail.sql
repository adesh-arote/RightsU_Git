...
AS
 --=============================================
 --Author:		<Rajesh Godse / Akshay Rane / Rahul Kembhavi
...
--	@DashboardDays INT=30
	...
	if(@Loglevel < 2)Exec [USPLogSQLSteps] '[USP_Get_Dashboard_Detail]', 'Step 1', 0, 'Started Procedure', 0, '' 
		SET NOCOUNT ON;
		--DECLARE 
		...
		BEGIN
			SET @Module_Code = 35
		END
		SELECT @UserSecCode = Security_Group_Code FROM Users (NOLOCK) WHERE Users_Code = @User_Code  
		SELECT DISTINCT @Is_Deal_Rights = CASE WHEN SMR.Module_Code = @Module_Code THEN 'Y' ELSE 'N' END
		FROM System_Module_Right SMR  (NOLOCK)
		INNER JOIN Security_Group_Rel sgr (NOLOCK) ON smr.Module_Right_Code = sgr.System_Module_Rights_Code AND sgr.Security_Group_Code = @UserSecCode
	  ...
	  --SET @DashboardDays = (SELECT ISNULL(Parameter_Value,'') AS Parameter_Value FROM System_Parameter_New WHERE Parameter_Name='Dashboard_Days')
	   ...
	   --  PRINT @SearchFor
		...
		(SELECT SUM(Deal_Cost) AS Deal_Cost FROM Acq_Deal_Cost (NOLOCK) WHERE Acq_Deal_Code = d.Acq_Deal_Code) AS Deal_Movie_Cost,
		...
		END,ADR.Actual_Right_Start_Date FROM Acq_Deal d (NOLOCK)
		INNER JOIN Acq_Deal_Rights AS ADR (NOLOCK) ON ADR.Acq_Deal_Code = d.Acq_Deal_Code
		INNER JOIN dbo.Vendor AS v  (NOLOCK) ON v.vendor_code = d.Vendor_Code
		LEFT OUTER JOIN Acq_Deal_Cost AS ADC (NOLOCK) ON ADC.Acq_Deal_Code = d.Acq_Deal_Code 
		WHERE  d.Deal_Workflow_Status NOT IN (''AR'', ''WA'') AND  d.Business_Unit_Code IN(select Business_Unit_Code from Users_Business_Unit U (NOLOCK) where U.Users_Code = '+CAST(@User_Code AS VARCHAR(50))+'))as Temp
		...
		INNER JOIN Title T (NOLOCK) On T.title_code = SDM.Title_Code    
		WHERE SDM.Syn_Deal_Code = d.Syn_Deal_Code 
		FOR XML PATH('''')),1,1,'''') AS TitleName,
		(SELECT SUM(Deal_Cost) AS Deal_Cost FROM Acq_Deal_Cost (NOLOCK) WHERE Acq_Deal_Code = d.Syn_Deal_Code) AS Deal_Movie_Cost,
		...
		END,SDR.Actual_Right_Start_Date FROM Syn_Deal d (NOLOCK)
		INNER JOIN Syn_Deal_Rights AS SDR  (NOLOCK) ON SDR.Syn_Deal_Code = d.Syn_Deal_Code
		INNER JOIN dbo.Vendor AS v (NOLOCK) ON v.vendor_code = d.Vendor_Code 
		WHERE Business_Unit_Code IN(select Business_Unit_Code from Users_Business_Unit (NOLOCK)  where Users_Code = '+CAST(@User_Code AS VARCHAR(50))+'))as Temp
		WHERE CONVERT(date,Actual_Right_Start_Date,103) BETWEEN '''+ cast(@StartDate as varchar(50)) +''' and '''+ cast(@EndDate as varchar(50))+'''' + @Search
		+ ' Order by Actual_Right_Start_Date ASC'
		...
		INSERT INTO @Temp_Syn EXEC USP_Dashboard_AcqSyn_Expiry '',@DashboardDays,'S','N',@User_Code,'N'
		

		SELECT * INTO #Temp_Syn_Expiry FROM @Temp_Syn

		--DELETE FROM #Temp_Syn_Expiry WHERE Deal_Code IN (
		--	Select DISTINCT tse.Deal_Code from 
		--	Syn_Deal_Rights sdr
		--	INNER JOIN #Temp_Syn_Expiry tse ON tse.Deal_Code = sdr.Syn_Deal_Code
		...
		--Query to Get Deals which Approved in next @DashboardDays days
			...
			SELECT  MD.Record_Code, MAX(AD.Agreement_No), MAX(AD.Vendor_Code), MAX(MD.Status_Changed_On) FROM Acq_Deal AD (NOLOCK)
			INNER JOIN Module_Status_History MD  (NOLOCK) ON MD.Record_Code = AD.Acq_Deal_Code 
			WHERE  AD.Deal_Workflow_Status NOT IN ('AR', 'WA') AND  MD.Module_Code = 30 AND MD.Status = 'A' AND 
			AD.Business_Unit_Code   IN (select Business_Unit_Code from Users_Business_Unit U (NOLOCK) where U.Users_Code = CAST(@User_Code AS VARCHAR(50)))
			GROUP BY  MD.Record_Code 

			SET @SQL = 'SELECT * FROM (SELECT DISTINCT '''+@Is_Deal_Rights+''' AS Is_Deal_Rights, d.Acq_deal_code AS Deal_Code, d.Agreement_No,v.Vendor_Name AS Customer,
					STUFF((SELECT Distinct '','' + ISNULL(Title_Name,Original_Title)
					FROM Acq_Deal_Movie ADM (NOLOCK)  INNER JOIN Title T (NOLOCK) On T.title_code = ADM.Title_Code WHERE ADM.Acq_Deal_Code = d.Acq_Deal_Code
					FOR XML PATH('''')),1,1,'''') AS TitleName,
					(SELECT SUM(Deal_Cost) AS Deal_Cost FROM Acq_Deal_Cost (NOLOCK) WHERE Acq_Deal_Code = d.Acq_Deal_Code) AS Deal_Movie_Cost,
		...
		--Query to Get Deals which Approved in next @DashboardDays days
			...
			SELECT  MD.Record_Code, MAX(SD.Agreement_No), MAX(SD.Vendor_Code), MAX(MD.Status_Changed_On) FROM Syn_Deal SD (NOLOCK)
			INNER JOIN Module_Status_History MD  (NOLOCK) ON MD.Record_Code = SD.Syn_Deal_Code 
			WHERE MD.Module_Code = 35 AND MD.Status = 'A' 
			AND SD.Business_Unit_Code  IN (select Business_Unit_Code from Users_Business_Unit U (NOLOCK) where U.Users_Code = CAST(@User_Code AS VARCHAR(50)))
			GROUP BY  MD.Record_Code 
			...
			STUFF((SELECT DISTINCT '','' + ISNULL(Title_Name,Original_Title)
				FROM Syn_Deal_Movie SDM (NOLOCK)  INNER JOIN Title T (NOLOCK) On T.title_code = SDM.Title_Code WHERE SDM.Syn_Deal_Code = d.Syn_Deal_Code 
				FOR XML PATH('''')),1,1,'''') AS TitleName,
			(SELECT SUM(Deal_Cost) AS Deal_Cost FROM Acq_Deal_Cost (NOLOCK) WHERE Acq_Deal_Code = d.Syn_Deal_Code) AS Deal_Movie_Cost,
			(Cast(Convert(VARCHAR(11),cast(d.Status_Changed_On as datetime),103) as Varchar))  RightPeriod
			FROM  #Syn_Deals d
			INNER JOIN Syn_Deal_Rights AS SDR  (NOLOCK) ON SDR.Syn_Deal_Code = d.Syn_Deal_Code INNER JOIN dbo.Vendor AS v (NOLOCK) ON v.vendor_code = d.Vendor_Code )as Temp
			WHERE CONVERT(date,RightPeriod,103) BETWEEN '''+ cast(@StartDate as varchar(50)) +''' and '''+ cast(@EndDate as varchar(50))+'''' + @Search
			+ ' Order by RightPeriod ASC'
		...
		WHEN ''U'' THEN ''Perpetuity'' 
		ELSE (Cast(Convert(VARCHAR(11),cast(ADR.Actual_Right_Start_Date as datetime),103) as Varchar)+ ''  -  '' + Cast(Convert(VARCHAR(11),cast(ADR.Actual_Right_End_Date as datetime),103) as Varchar))
		END,ADR.ROFR_Date FROM Acq_Deal d (NOLOCK)
		INNER JOIN Acq_Deal_Rights AS ADR  (NOLOCK) ON ADR.Acq_Deal_Code = d.Acq_Deal_Code
		INNER JOIN dbo.Vendor AS v (NOLOCK) ON v.vendor_code = d.Vendor_Code
		LEFT OUTER JOIN Acq_Deal_Cost AS ADC (NOLOCK) ON ADC.Acq_Deal_Code = d.Acq_Deal_Code WHERE  d.Deal_Workflow_Status NOT IN (''AR'', ''WA'') AND   ADR.Is_ROFR = ''Y'' AND
		d.Business_Unit_Code IN(select Business_Unit_Code from Users_Business_Unit (NOLOCK) where Users_Code = '+CAST(@User_Code AS VARCHAR(50))+') AND d.Deal_workflow_status = '+ '''A'''+')as Temp
		...
		INNER JOIN Acq_Deal_Rights AS ADR (NOLOCK) ON ADR.Acq_Deal_Code = d.Acq_Deal_Code
		INNER JOIN dbo.Vendor AS v  (NOLOCK) ON v.vendor_code = d.Vendor_Code
		LEFT OUTER JOIN Acq_Deal_Cost AS ADC (NOLOCK) ON ADC.Acq_Deal_Code = d.Acq_Deal_Code WHERE  d.Deal_Workflow_Status NOT IN (''AR'', ''WA'') AND   ADR.Is_Tentative = ''Y'' AND
		d.Business_Unit_Code IN(select Business_Unit_Code from Users_Business_Unit  (NOLOCK) where Users_Code = '+CAST(@User_Code AS VARCHAR(50))+') )as Temp
		WHERE CONVERT(date,Actual_Right_Start_Date,103) BETWEEN '''+ cast(@StartDate as varchar(50)) +''' and '''+ cast(@EndDate as varchar(50))+'''' + @Search
		+' Order by Actual_Right_Start_Date ASC '
		
			EXEC(@SQL)
		END
		ELSE IF(@DashboardType = 'SRHB' OR @DashboardType = 'ARHB' OR @DashboardType = 'SHB' OR @DashboardType = 'AHB')
		BEGIN
		--Query to Get Deals which expires in next @DashboardDays days

		IF OBJECT_ID('tempdb..#Temp_Syn_Expiry_SRHB') IS NOT NULL
			DROP TABLE #Temp_Syn_Expiry_SRHB

		Declare @Temp_Syn_SRHB Table(
				 Agreement_No NVARCHAR(MAX),
				 Customer NVARCHAR(MAX), 
				 Deal_Code INT,
				 Right_End_Date DATETIME, 
		...
		IF(@DashboardType = 'SRHB')
		BEGIN
			INSERT INTO @Temp_Syn_SRHB EXEC USP_Dashboard_AcqSyn_Expiry '',@DashboardDays,'S','N',@User_Code,'Y'
		END
		ELSE IF(@DashboardType = 'ARHB' OR @DashboardType = 'SHB' OR @DashboardType = 'AHB')
		BEGIN
			INSERT INTO @Temp_Syn_SRHB EXEC USP_Dashboard_AcqSyn_Expiry '',@DashboardDays,@DashboardType,'N',@User_Code,'N'
		END
	
	
		SELECT * INTO #Temp_Syn_Expiry_SRHB FROM @Temp_Syn_SRHB


		SET @SQL = 'SELECT * FROM ( SELECT DISTINCT '''+@Is_Deal_Rights+''' AS Is_Deal_Rights, Agreement_No, Customer, Deal_Code, Right_End_Date, Deal_Movie_Cost, RightPeriod, TitleName 
					FROM #Temp_Syn_Expiry_SRHB ) as Temp
					WHERE CONVERT(date,Right_End_Date,103) BETWEEN '''+ cast(@StartDate as varchar(50)) +''' and '''+ cast(@EndDate as varchar(50))+'''' + @Search
					+' Order by Right_End_Date ASC '
	...
	if(@Loglevel < 2)Exec [USPLogSQLSteps] '[USP_Get_Dashboard_Detail]', 'Step 2', 0, 'Procedure Excution Completed', 0, '' 
END
