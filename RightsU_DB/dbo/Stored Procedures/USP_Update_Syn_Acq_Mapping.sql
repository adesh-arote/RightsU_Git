...
BEGIN
	...
	if(@Loglevel < 2)Exec [USPLogSQLSteps] '[USP_Update_Syn_Acq_Mapping]', 'Step 1', 0, 'Started Procedure', 0, '' 
		SET FMTONLY OFF
		SET NOCOUNT ON;
		/******************************** Delete Temp Tables*********************************/
				...
				BEGIN            
					...
					BEGIN 
						...
						IF EXISTS (SELECT TOP 1 ADRT.Acq_Deal_Rights_Code 
								   ...
								   INNER JOIN Acq_Deal_Rights_Platform  ADRP (NOLOCK) ON ADRT.Acq_Deal_Rights_Code = ADRP.Acq_Deal_Rights_Code
	...
	if(@Loglevel < 2)Exec [USPLogSQLSteps] '[USP_Update_Syn_Acq_Mapping]', 'Step 2', 0, 'Procedure Excuting Completed', 0, '' 
END