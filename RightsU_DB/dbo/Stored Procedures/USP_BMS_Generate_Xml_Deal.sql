...
BEGIN
	...
	if(@Loglevel < 2)Exec [USPLogSQLSteps] '[USP_BMS_Generate_Xml_Deal]', 'Step 1', 0, 'Started Procedure', 0, ''
		...
		BEGIN
			...
			BEGIN
				...
				BEGIN
					...
					FROM BMS_Asset bvA (NOLOCK)
					WHERE (bvA.BMS_Asset_Code IN (SELECT BMS_Asset_Code FROM BMS_Deal_Content (NOLOCK) WHERE BMS_Deal_Code = @Code) OR @Code = 0)
					AND (
				...
				BEGIN
					SET @no_Of_Record_In_Batch = @no_Of_Record_In_Batch_Deal
					SELECT @recordCount = COUNT(*)
					FROM BMS_ProgramVersion bvA (NOLOCK)
					WHERE 
				...
				BEGIN
					SET @no_Of_Record_In_Batch = @no_Of_Record_In_Batch_Deal
					SELECT @recordCount = COUNT(*)
					FROM BMS_Deal_Content bvDC (NOLOCK)
				...
				BEGIN
					SET @no_Of_Record_In_Batch = @no_Of_Record_In_Batch_Deal
					SELECT @recordCount = COUNT(*)
					FROM BMS_Deal_Content_Rights bvDCR (NOLOCK)
					WHERE (bvDCR.BMS_Deal_Content_Code IN (SELECT BMS_Deal_Content_Code FROM BMS_Deal_Content (NOLOCK) WHERE BMS_Deal_Code = @Code) OR @Code = 0)
					AND (
				...
				BEGIN
					...
					(
						...
						FROM BMS_Asset tempAsset  (NOLOCK)	
						WHERE tempAsset.Record_Status = 'P' AND ISNULL(IS_Consider,'Y') <> 'N'
					) AS a INNER JOIN Acq_Deal_Movie ADM (NOLOCK) ON ADM.Title_Code = a.RU_Title_Code
					...
					INNER JOIN BMS_Asset BA (NOLOCK) ON BA.RU_Title_Code = TBL.Title_Code AND temp.Row_Num = BA.Episode_Number COLLATE SQL_Latin1_General_CP1_CI_AS
						...
						SELECT @xmlData = (
							...
							FROM (
								SELECT TOP(@totalTopOuter) * FROM (
									...
									FROM BMS_Deal_Content_Rights bvDCR (NOLOCK)
									INNER JOIN BMS_Deal_Content bvDC (NOLOCK) ON bvDCR.BMS_Deal_Content_Code = bvDC.BMS_Deal_Content_Code
	...
	if(@Loglevel < 2)Exec [USPLogSQLSteps] '[USP_BMS_Generate_Xml_Deal]', 'Step 2', 0, 'Procedure Excution Completed', 0, ''
END