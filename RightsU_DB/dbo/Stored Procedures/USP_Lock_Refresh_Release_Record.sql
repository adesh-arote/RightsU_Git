CREATE PROCEDURE [dbo].[USP_Lock_Refresh_Release_Record]
(
	@Record_Code INT = 0,
	@Module_Code INT = 0,
	@User_Code INT = 0,
...
BEGIN
	...
	if(@Loglevel < 2)Exec [USPLogSQLSteps] '[USP_Lock_Refresh_Release_Record]', 'Step 1', 0, 'Started Procedure', 0, '' 
		...
		BEGIN
			...
			FROM Record_Locking RC (NOLOCK) 
			INNER JOIN Users U (NOLOCK) ON U.Users_Code = RC.User_Code
	...
	if(@Loglevel < 2)Exec [USPLogSQLSteps] '[USP_Lock_Refresh_Release_Record]', 'Step 2', 0, 'Procedure Excution Completed', 0, '' 
END
