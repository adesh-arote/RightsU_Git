...
BEGIN 	
	...
	if(@Loglevel< 2)Exec [USPLogSQLSteps] '[USP_Search_Run_Shows]', 'Step 1', 0, 'Started Procedure', 0, ''
		SET FMTONLY OFF
		SET NOCOUNT ON	 

		IF (@selectedChannelNames <> '')	 
			SELECT DISTINCT C.Channel_Code,C.Channel_Name  INTO #Temp_Channel_Codes 
			FROM Channel C (NOLOCK) WHERE  RTRIM(LTRIM(C.Channel_Name)) IN( SELECT RTRIM(LTRIM(NUmber)) from fn_Split_withdelemiter(@selectedChannelNames,','))	
		...
		IF EXISTS(SELECT TOP  1 Acq_Deal_Run_Shows_Code FROM Acq_Deal_Run_Shows (NOLOCK) WHERE Acq_Deal_Run_Code = @Run_Code AND @Data_For_flag = '')
		BEGIN
			SELECT TOP 1 @Data_For_flag = Data_For FROM Acq_Deal_Run_Shows (NOLOCK) WHERE Acq_Deal_Run_Code = @Run_Code
			ORDER BY Data_For
			PRINT @Data_For_flag
		END	
		...
		FROM Acq_Deal_Run_Shows ADRS   (NOLOCK)
		...
		INNER JOIN Acq_Deal_Run_Channel ADRC  (NOLOCK) ON ADR.Acq_Deal_Run_Code = ADRC.Acq_Deal_Run_Code
			AND ADRC.Channel_Code IN(
					SELECT TC.Channel_Code FROM  #Temp_Channel_Codes  TC						
				)
		INNER JOIN Acq_Deal_Run_Title ADRT (NOLOCK) ON ADRC.Acq_Deal_Run_Code = ADRT.Acq_Deal_Run_Code			
		INNER JOIN Acq_Deal AD  (NOLOCK) ON AD.Acq_Deal_Code=ADR.Acq_Deal_Code and  AD.Deal_Workflow_Status NOT IN ('AR', 'WA') 
			...
			INNER JOIN Acq_Deal_Run_Title ADRT (NOLOCK) ON TS.Acq_Deal_Run_Code=ADRT.Acq_Deal_Run_Code AND  TS.Title_Code = ADRT.Title_Code 
		...
		BEGIN
			...
			INNER JOIN Acq_Deal_Run ADR  (NOLOCK) ON ADM.Acq_Deal_Code =ADR.Acq_Deal_Code
			INNER JOIN Acq_Deal_Run_Channel ADRC (NOLOCK) ON ADR.Acq_Deal_Run_Code =ADRC.Acq_Deal_Run_Code
			...
			INNER JOIN Acq_Deal_Run_Channel ADRC (NOLOCK) ON T.Acq_Deal_Run_Code =ADRC.Acq_Deal_Run_Code
			INNER JOIN Acq_Deal_Run_Title ADRT (NOLOCK) ON ADRT.Acq_Deal_Run_Code = ADRC.Acq_Deal_Run_Code 		
	...
	if(@Loglevel< 2)Exec [USPLogSQLSteps] '[USP_Search_Run_Shows]', 'Step 2', 0, 'Procedure Excuting Completed', 0, ''
END