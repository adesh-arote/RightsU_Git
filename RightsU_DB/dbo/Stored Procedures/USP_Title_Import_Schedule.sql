...
BEGIN    
	...
	if(@Loglevel< 2)Exec [USPLogSQLSteps] '[USP_Title_Import_Schedule]', 'Step 1', 0, 'Started Procedure', 0, '' 
	
		DECLARE @DM_Master_Import_Code INT = 0, @RowCounter INT = 0
		SELECT TOP 1 @DM_Master_Import_Code = DM_Master_Import_Code FROM DM_Master_Import (NOLOCK)  WHERE [Status] NOT IN('S','E','W','N','R','T') AND File_Type = 'T' Order by DM_Master_Import_Code desc
		...
		select @RsolveStatus = status from DM_Master_Import (NOLOCK)  where DM_Master_Import_Code = @DM_Master_Import_Code
			...
			SELECT @COUNT = COUNT(*) FROM DM_Title (NOLOCK)  WHERE Is_Ignore = 'N' AND Record_Status = 'N' AND DM_Master_Import_Code = @DM_Master_Import_Code
			SELECT  @TotalCount = COUNT(*) FROM DM_Title (NOLOCK)  WHERE Is_Ignore = 'N'  AND DM_Master_Import_Code = @DM_Master_Import_Code
			...
			ELSE
				BEGIN
				SELECT @StatusCount = COUNT(*) FROM DM_Title (NOLOCK)  WHERE (Record_Status = 'R') AND IS_Ignore = 'N' AND DM_Master_Import_Code = @DM_Master_Import_Code
	...
	if(@Loglevel< 2)Exec [USPLogSQLSteps] '[USP_Title_Import_Schedule]', 'Step 2', 0, 'Procedure Excuting Completed', 0, '' 
END