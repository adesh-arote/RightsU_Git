...
BEGIN
	...
	FROM BMS_Schedule_Process_Data_Temp BSPD
	INNER JOIN BMS_Schedule_Exception BSE ON ISNULL(BSE.BMS_Schedule_Process_Data_Temp_Code,0) = ISNULL(BSPD.BMS_Schedule_Process_Data_Temp_Code,0)
	...
	INNER JOIN BMS_Schedule_Log BSLL ON ISNULL(BST.File_Code,0) = ISNULL(BSLL.BMS_Schedule_Log_Code,0)
	INNER JOIN BMS_Schedule_Exception BSE ON ISNULL(BSE.BV_Schedule_Transaction_Code,0)  = ISNULL(BST.BV_Schedule_Transaction_Code,0)
	...
	(
		select TOP 1 BDCR.BMS_Deal_Content_Rights_Ref_Key from BMS_Deal BD 
	...
	FROM  BV_Schedule_Transaction BST 
	INNER JOIN BMS_Schedule_Log BSLL ON ISNULL(BST.File_Code,0) = ISNULL(BSLL.BMS_Schedule_Log_Code,0) AND BST.Channel_Code = BSLL.Channel_Code
	AND 
