...
BEGIN
	...
	if(@Loglevel < 2)Exec [USPLogSQLSteps] '[USP_Get_Dashboard_Detail_Material_Type]', 'Step 1', 0, 'Started Procedure', 0, '' 
		SET NOCOUNT ON;
		--DECLARE 
		...
		BEGIN
			...
			SET @SQL = ' INSERT INTO #TempAS (Deal_Code, Agreement_No)
						...
						INNER JOIN Acq_Deal_Rights (NOLOCK) AS ADR ON ADR.Acq_Deal_Code = d.Acq_Deal_Code
			...
			FROM Acq_Deal_Material ADM (NOLOCK)
				...
				INNER JOIN Material_Medium MM (NOLOCK) ON MM.Material_Medium_Code = ADM.Material_Medium_Code
				INNER JOIN Material_Type MT (NOLOCK) ON MT.Material_Type_Code = ADM.Material_Type_Code
				INNER JOIN Title T (NOLOCK) ON T.Title_Code = ADM.Title_Code

			DROP TABLE #TempAS
		END	
	if(@Loglevel < 2)Exec [USPLogSQLSteps] '[USP_Get_Dashboard_Detail_Material_Type]', 'Step 2', 0, 'Procedure Excution Completed', 0, '' 
END
