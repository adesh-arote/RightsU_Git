...
BEGIN
	...
	if(@Loglevel < 2)Exec [USPLogSQLSteps] '[USP_Validate_Episode]', 'Step 1', 0, 'Started Procedure', 0, '' 
				...
				BEGIN TRY
					...
					BEGIN            
						...
						BEGIN  
								...
								IF ((select count(TCM.Acq_Deal_Movie_Code) from Title_Content_Mapping TCM (NOLOCK)
				...
				inner join Title T (NOLOCK) on T.Title_Code  = temp.Title

				--DROP TABLE #Invalid_Episode_ID

				IF OBJECT_ID('tempdb..#Invalid_Episode_ID') IS NOT NULL DROP TABLE #Invalid_Episode_ID
			  
	if(@Loglevel < 2)Exec [USPLogSQLSteps] '[USP_Validate_Episode]', 'Step 2', 0, 'Procedure Excuting Completed', 0, ''
END