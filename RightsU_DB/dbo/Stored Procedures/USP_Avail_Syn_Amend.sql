...
BEGIN	
	...
	if(@Loglevel < 2)Exec [USPLogSQLSteps] '[USP_Avail_Syn_Amend]', 'Step 1', 0, 'Started Procedure', 0, ''
	...
	FOR 
		...
		INNER JOIN Acq_Deal_Rights_Territory ADRTerr (NOLOCK) ON ADRTerr.Acq_Deal_Rights_code = ADR.Acq_Deal_Rights_code
		...
		(
			...
			FROM Avail_Syn  AVS (NOLOCK)
			INNER JOIN Avail_Syn_Details AVSD (NOLOCK) ON AVS.Avail_Syn_Code=AVSD.Avail_Syn_Code
			INNER JOIN Avail_Syn_Language AVAL (NOLOCK) ON AVS.Avail_Syn_Code=AVAL.Avail_Syn_Code 
			INNER JOIN 
			(	SELECT distinct Title_Code,Platform_Code,Country_Code FROM Avail_Syn  AVS (NOLOCK)
				INNER JOIN Avail_Syn_Details AVSD (NOLOCK) ON AVS.Avail_Syn_Code=AVSD.Avail_Syn_Code
	...
	BEGIN
		...
		BEGIN
			...
			FROM Avail_Acq AVA (NOLOCK)
			LEFT JOIN Avail_Acq_Details AVAD (NOLOCK) ON AVA.Avail_Acq_Code=AVAD.Avail_Acq_Code
			LEFT JOIN Avail_Acq_Language AVAL (NOLOCK) ON AVA.Avail_Acq_Code=AVAL.Avail_Acq_Code
			...
			BEGIN
				...
				from Avail_Acq_Details AAD (NOLOCK) where Avail_Acq_Code=@Avail_Acq_Code 
				...
				from Avail_Acq_Language AAL (NOLOCK) where Avail_Acq_Code=@Avail_Acq_Code 
			...
			Print '@SYN_RSD = @ACQ_RSD and @SYN_RED < @ACQ_RED'
				...
				if((select count(*) from Avail_Acq_Details (NOLOCK) where Avail_Acq_Code=@Avail_Acq_Code 
	...
	if(@Loglevel < 2)Exec [USPLogSQLSteps] '[USP_Avail_Syn_Amend]', 'Step 2', 0, 'Procedure Excution Completed', 0, ''
END
