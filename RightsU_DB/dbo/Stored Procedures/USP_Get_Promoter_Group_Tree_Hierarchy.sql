...
Begin  
	...
	if(@Loglevel < 2)Exec [USPLogSQLSteps] '[USP_Get_Promoter_Group_Tree_Hierarchy]', 'Step 1', 0, 'Started Procedure', 0, ''
 
		Insert InTo #TempPF  
		Select Promoter_Group_Code, ISNULL(Parent_Group_Code, 0), Is_Last_Level  
		From [Promoter_Group] (NOLOCK) Where Promoter_Group_Code in (Select number From DBO.fn_Split_withdelemiter(@Promoter_Group_Codes, ',')) order by Promoter_Group_Name   
    
		While((   
		Select Count(*) From [Promoter_Group] (NOLOCK) Where Promoter_Group_Code In (Select Parent_Group_Code From #TempPF) And Promoter_Group_Code Not In (Select Promoter_Group_Code From #TempPF)  
		) > 0)  
			Begin   
				Insert InTo #TempPF  
				Select Promoter_Group_Code, ISNULL(Parent_Group_Code, 0), Is_Last_Level  
				From [Promoter_Group] (NOLOCK) Where Promoter_Group_Code in (Select Parent_Group_Code From #TempPF) And Promoter_Group_Code Not In (Select Promoter_Group_Code From #TempPF)  
			End  
		 End  
		 ...
		 Begin  
			...
			Select Promoter_Group_Code, IsNull(Parent_Group_Code, 0) From [Promoter_Group] (NOLOCK) 
		 End  
		...
		From [Promoter_Group] (NOLOCK) Where Is_Active = 'Y'   
	...
	if(@Loglevel < 2)Exec [USPLogSQLSteps] '[USP_Get_Promoter_Group_Tree_Hierarchy]', 'Step 2', 0, 'Procedure Excution Completed', 0, ''
End