...
AS
	...
	if(@Loglevel  < 2)Exec [USPLogSQLSteps] '[USP_BMS_Schedule_Revert]', 'Step 1', 0, 'Started Procedure', 0, ''
		BEGIN TRY	
			...
			INNER JOIN Title_Content TC (NOLOCK) ON TC.Ref_BMS_Content_Code = BST.Program_Episode_ID
			INNER JOIN Content_Channel_Run CCR (NOLOCK) ON CCR.Title_Content_Code = TC.Title_Content_Code AND CCR.Channel_Code=@Channel_Code
			...
			SELECT  @Acq_Deal_Run_Codes = STUFF((SELECT DISTINCT  ',' +  CAST(ISNULL(Acq_Deal_Run_Code,0) AS VARCHAR)
				FROM BV_Schedule_Transaction (NOLOCK) WHERE Timeline_ID IN(SELECT DISTINCT TimeLine_ID FROM #Temp_TimeLine_ID) FOR XML PATH('')), 1, 1, '')
			...
			SELECT @Parent_ChannelCode = ISNULL(C.Parent_Channel_Code,0) FROM Channel C (NOLOCK) WHERE C.Channel_Code = @Channel_Code
			...
			FROM BV_Schedule_Transaction T2 (NOLOCK) WHERE T2.Content_Channel_Run_Code = T1.Content_Channel_Run_Code
			AND T2.Timeline_ID IN(SELECT DISTINCT TimeLine_ID FROM #Temp_TimeLine_ID)
			FOR XML PATH('')), 1, 1, ''))  AS Content_Channel_Run_Code
				from BV_Schedule_Transaction T1 (NOLOCK) where T1.Timeline_ID IN(SELECT DISTINCT TimeLine_ID FROM #Temp_TimeLine_ID)
		...
		--select '#Temp_Acq_Run_No_Right_Rule_Code',* from #Temp_Acq_Run_No_Right_Rule_Code
			...
			BEGIN
				...
				BEGIN
					...
					BEGIN
						...
						IF EXISTS (SELECT TOP 1 T.Right_Rule_Code FROM Content_Channel_Run T (NOLOCK) WHERE T.Right_Rule_Code IS NOT NULL AND ISNULL(T.Is_Archive, 'N') = 'N')				
						BEGIN
			...
			DELETE FROM BMS_Schedule_Exception WHERE BV_Schedule_Transaction_Code IN(
				...
				INNER JOIN BV_Schedule_Transaction BST (NOLOCK) ON T.TimeLine_ID = BST.Timeline_ID
	...
	if(@Loglevel  < 2)Exec [USPLogSQLSteps] '[USP_BMS_Schedule_Revert]', 'Step 2', 0, 'Procedure Excution Completed', 0, ''
END