CREATE PROCEDURE [dbo].[USP_Get_EmailConfig_Users] 
(
	@Key VARCHAR(5),
	@CallFor CHAR(1) = 'N'
)       
...
BEGIN
	...
	if(@Loglevel < 2)Exec [USPLogSQLSteps] '[USP_Get_EmailConfig_Users]', 'Step 1', 0, 'Started Procedure', 0, '' 

		--DECLARE @Key NVARCHAR(MAX) = 'ACE', @CallFor  CHAR(1) = 'N'
		DECLARE @Tbl2 TABLE (
			Id INT IDENTITY(1,1),
			BuCode INT,
		...
		BEGIN  
			  ...
			  BEGIN
					...
					FROM Users U (NOLOCK)
						INNER JOIN Users_Business_Unit UBU (NOLOCK) ON UBU.Users_Code = U.Users_Code
	...
	if(@Loglevel < 2)Exec [USPLogSQLSteps] '[USP_Get_EmailConfig_Users]', 'Step 2', 0, 'Procedure Excution Completed', 0, '' 
END