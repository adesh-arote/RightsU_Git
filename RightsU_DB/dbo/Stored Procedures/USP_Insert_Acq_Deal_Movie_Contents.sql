	...
	BEGIN TRY
		...
		BEGIN
			...
			BEGIN
				...
				(
					SELECT MAX(Episode_Id) AS MaxEpisode_Id FROM Acq_Deal_Movie_Contents ADMC 
					INNER JOIN Content_Music_Link CML ON CML.Acq_Deal_Movie_Content_Code = ADMC.Acq_Deal_Movie_Content_Code
				...
				BEGIN
					...
					IF NOT EXISTS (SELECT * FROM Acq_Deal_Movie_Contents WHERE Acq_Deal_Movie_Code = @AcqDealMovieCode AND Episode_Id = @Episode_Id)
					BEGIN
