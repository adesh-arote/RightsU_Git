...
BEGIN
	...
	if(@Loglevel < 2)Exec [USPLogSQLSteps] '[USP_Import_SubDeal_Insert]', 'Step 1', 0, 'Started Procedure', 0, ''
		SET NOCOUNT ON
		...
		BEGIN TRY 
			...
			BEGIN
						...
						SELECT 	@Acq_Deal_Code = Acq_Deal_Code , @Vendor_Code = Vendor_Code, @Deal_Type_Code =  Deal_Type_Code FROM Acq_Deal  (NOLOCK)															
						...
						FROM Acq_Deal (NOLOCK) WHERE Acq_Deal_Code = SCOPE_IDENTITY()
						-------------------------------------------------------------
	...
	if(@Loglevel < 2)Exec [USPLogSQLSteps] '[USP_Import_SubDeal_Insert]', 'Step 2', 0, 'Procedure Excution Completed', 0, ''
END