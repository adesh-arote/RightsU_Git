...
BEGIN  
	...
	if(@Loglevel < 2)Exec [USPLogSQLSteps] '[USP_Get_Music_Platform_Tree_Hierarchy]', 'Step 1', 0, 'Started Procedure', 0, ''
		...
		BEGIN  
			INSERT INTO #TEMP_MPF  
				SELECT Music_Platform_Code, ISNULL(Parent_Code, 0), Is_Last_Level  
					 FROM [Music_Platform] (NOLOCK) WHERE Music_Platform_Code IN (SELECT NUMBER FROM DBO.FN_SPLIT_WITHDELEMITER(@MusicPlatformCodes, ',')) ORDER BY Platform_Name  
				WHILE((     
					SELECT COUNT(*) FROM [Music_Platform] (NOLOCK) WHERE Music_Platform_Code IN (SELECT Parent_Code FROM #TEMP_MPF) AND Music_Platform_Code NOT IN (SELECT Music_Platform_Code FROM #TEMP_MPF)  
				) > 0)  
			BEGIN  
				INSERT INTO #TEMP_MPF  
					SELECT Music_Platform_Code, ISNULL(Parent_Code , 0), Is_Last_Level  
	    		FROM [Music_Platform] (NOLOCK) WHERE Music_Platform_Code  IN (SELECT Parent_Code FROM #TEMP_MPF) AND Music_Platform_Code NOT IN (SELECT Music_Platform_Code  FROM #TEMP_MPF)  
			END  
		END  
		 ELSE  
		BEGIN  
			INSERT INTO #TEMP_MPF(Music_Platform_Code, Parent_Code )  
				 SELECT Music_Platform_Code, ISNULL(Parent_Code , 0) FROM [Music_Platform]  (NOLOCK) 
		End  
		...
		--,'<span style="background-color:yellow">'+@Search_Platform_Name+'</span>') AS Platform_Name  
			...
			, ISNULL(Parent_Code, 0) Parent_Code, Is_Last_Level, Module_Position,  
				(SELECT COUNT(*) FROM #TEMP_MPF tmp WHERE tmp.Parent_Code = [Music_Platform].Music_Platform_Code) ChildCount  
					 From [Music_Platform] (NOLOCK) WHERE Is_Active = 'Y'  
	...
	if(@Loglevel < 2)Exec [USPLogSQLSteps] '[USP_Get_Music_Platform_Tree_Hierarchy]', 'Step 2', 0, 'Procedure Excution Completed', 0, ''
 END