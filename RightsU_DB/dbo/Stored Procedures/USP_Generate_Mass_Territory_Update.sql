CREATE PROCEDURE [dbo].[USP_Generate_Mass_Territory_Update]
(
	@TerritoryCodes Varchar(4000),@UserId int
)
As
Begin
	...
	if(@Loglevel < 2)Exec [USPLogSQLSteps] '[USP_Generate_Mass_Territory_Update]', 'Step 1', 0, 'Started Procedure', 0, ''
			-- =============================================
		   ...
		   select 
			...
			from 
				Acq_Deal ad (NOLOCK) inner join 
				Acq_Deal_Rights adr (NOLOCK) on adr.Acq_Deal_Code=ad.Acq_Deal_Code inner join 
				Acq_Deal_Rights_Territory adrt (NOLOCK) on adrt.Acq_Deal_Rights_Code=adr.Acq_Deal_Rights_Code
			where 
			...
			( 
				...
				from
					Acq_Deal ad (NOLOCK) inner join
					Acq_Deal_Rights adr (NOLOCK) on ad.Acq_Deal_Code=adr.Acq_Deal_Code
				...
				inner join Acq_Deal_Mass_Territory_Update admtu (NOLOCK) on admtu.Acq_Deal_Code=a.Acq_Deal_Code 
				left join Acq_Deal_Mass_Territory_Update_Details  admtud (NOLOCK) on admtud.Acq_Deal_Mass_Update_Code=admtu.Acq_Deal_Mass_Update_Code
			where 
		   ...
		   select 
			...
			from 
				Syn_Deal ad (NOLOCK) inner join 
				Syn_Deal_Rights adr (NOLOCK) on adr.Syn_Deal_Code=ad.Syn_Deal_Code 
				inner join  Syn_Deal_Rights_Territory adrt (NOLOCK) on adrt.Syn_Deal_Rights_Code=adr.Syn_Deal_Rights_Code
			where 
				adrt.Territory_Type='G' and ad.Is_Active='Y' and ad.Deal_Workflow_Status='A'
				and isnull(adrt.Territory_Code,0) in (Select number from dbo.[fn_Split_withdelemiter](@TerritoryCodes,','))
			...
			( 
				...
				from
					Syn_Deal ad (NOLOCK) inner join
					Syn_Deal_Rights adr (NOLOCK) on ad.Syn_Deal_Code=adr.Syn_Deal_Code
					inner join Syn_Deal_Rights_Territory adrt (NOLOCK) on adrt.Syn_Deal_Rights_Code=adr.Syn_Deal_Rights_Code
				where 
					adrt.Territory_Type='G' 
					and isnull(adrt.Territory_Code,0) in (Select number from dbo.[fn_Split_withdelemiter](@TerritoryCodes,','))
			) as a 
				inner join Syn_Deal_Mass_Territory_Update admtu (NOLOCK) on admtu.Syn_Deal_Code=a.Syn_Deal_Code 
				left join Syn_Deal_Mass_Territory_Update_Details  admtud (NOLOCK) on admtud.Syn_Deal_Mass_Update_Code=admtu.Syn_Deal_Mass_Update_Code
			where 
				admtu.Can_Process in ('N') and isnull(admtud.Territory_Code,0)<>a.Territory_Code
	...
	if(@Loglevel < 2)Exec [USPLogSQLSteps] '[USP_Generate_Mass_Territory_Update]', 'Step 2', 0, 'Procedure Excution Completed', 0, ''
End