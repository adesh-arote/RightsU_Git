...
BEGIN
	...
	if(@Loglevel < 2)Exec [USPLogSQLSteps] '[USP_BMS_Schedule_Right_Rule_Run_Process]', 'Step 1', 0, 'Started Procedure', 0, ''
		...
		BEGIN TRY	
			...
			FROM Content_Channel_Run CCR (NOLOCK) WHERE CCR.Content_Channel_Run_Code = @Content_Channel_Run_Code AND ISNULL(CCR.Is_Archive, 'N') = 'N'
			...
			FROM Right_Rule (NOLOCK) WHERE Right_Rule_Code = @Right_RuleCode
			--select @No_of_days_hrs
			SET @IS_FIRST_AIR = 'Y'

			SELECT TOP 1 @Schedule_Item_Log_Date_Time = CAST(BST.Schedule_Item_Log_Time AS DATETIME)
			FROM BV_Schedule_Transaction BST (NOLOCK) WHERE ISNULL(BST.Timeline_ID,0) = @TimelineID
			...
			FROM BV_Schedule_Transaction BST (NOLOCK) WHERE BST.Content_Channel_Run_Code = @Content_Channel_Run_Code
			...
			IF EXISTS(SELECT * fROM BV_Schedule_Transaction BST (NOLOCK) WHERE BST.Content_Channel_Run_Code = @Content_Channel_Run_Code
			...
			BEGIN
				...
				(SELECT MAX(BST.Play_Day) fROM BV_Schedule_Transaction BST  (NOLOCK) WHERE BST.Content_Channel_Run_Code = @Content_Channel_Run_Code
			...
			BEGIN
				...
				BEGIN
					...
					from BV_Schedule_Transaction BST (NOLOCK) where BST.BV_Schedule_Transaction_Code=@BV_Schedule_Transaction_Code
		
					PRINT 'Start SimulCast'
					BEGIN				
						IF EXISTS(SELECT Channel_Code FROM Channel (NOLOCK) WHERE Channel_Code = @ChannelCode AND ISNULL(Is_Parent_Child,'') = 'C')					
						BEGIN
							...
							FROM Content_Channel_Run CCR (NOLOCK) where CCR.Content_Channel_Run_Code = @Content_Channel_Run_Code_CV AND ISNULL(CCR.Is_Archive, 'N') = 'N'

							SELECT @Parent_Channel_Code = ISNULL(C.Parent_Channel_Code,0) FROM Channel C (NOLOCK) WHERE C.Channel_Code = @ChannelCode

							IF EXISTS (SELECT TOP 1 BST.Content_Channel_Run_Code FROM BV_Schedule_Transaction BST (NOLOCK) WHERE BST.Channel_Code = @Parent_Channel_Code AND 
					...
					BEGIN
						...
						BEGIN
							...
							FROM BV_Schedule_Transaction BST (NOLOCK) WHERE ISNULL(BST.Timeline_ID,0) = @TimeLine_ID
							ORDER BY BST.Inserted_On DESC
	...
	if(@Loglevel < 2)Exec [USPLogSQLSteps] '[USP_BMS_Schedule_Right_Rule_Run_Process]', 'Step 2', 0, 'Procedure Excution Completed' , 0, ''
END