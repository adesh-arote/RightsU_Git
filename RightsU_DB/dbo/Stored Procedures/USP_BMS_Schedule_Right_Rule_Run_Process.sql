...
BEGIN
	...
	BEGIN TRY	
		...
		,@PreviousSchedule_Item_Log_Date_Time DATETIME

		--select only those right rule which are not deleted or changed
		...
		--select metadata of right rule
		SELECT @Start_Time=Start_Time, @Play_Per_Day = ISNULL(Play_Per_Day,1), @No_of_days_hrs = Duration_Of_Day, @No_Of_Repeat = No_Of_Repeat,
		@IS_FIRST_AIR = CASE WHEN ISNULL(IS_FIRST_AIR,0) > 0 THEN 'Y' ELSE 'N' END FROM Right_Rule WHERE Right_Rule_Code = @Right_RuleCode
		
		--Select Timelineid's latest scheduled datetime
		...
		--Select Timelineid's Previous original scheduled datetime
		...
		AND BST.Schedule_Item_Log_Time < @Schedule_Item_Log_Date_Time AND BST.IsIgnore = 'N'
		...
		--Insert all current Content Channel Run data into #Temp_BMS_Schedule_Right_Rule_data
		...
		--Delete All the records from temp
		...
		--And (PlayDay IS NULL AND PlayRun IS NULL)

		--select * from #Temp_BMS_Schedule_Right_Rule_data
		DECLARE @BV_Schedule_Transaction_Code INT,@Schedule_DateTime DateTime, @TimeLine_ID INT, @Ref_TimeLine_ID INT, @PlayDay INT = 0, @PlayRun INT = 0,
		...
		BEGIN 
			...
			BEGIN
				...
				--SELECT  @LastPlayDay,  @LastPlayRun,  @LastTimeLine_ID,  @LastIsIgnore,@Schedule_DateTime,@LSchedule_Start_Time,@LSchedule_End_Time,@IS_FIRST_AIR
						--,@LSchedule_Start_Time_Is_First_Air,@LSchedule_End_Time
					...
					BEGIN
						...
						Update #Temp_BMS_Schedule_Right_Rule_data 
							...
							IF((Select  COUNT(*) from #Temp_BMS_Schedule_Right_Rule_data WHERE 
								...
								DATEADD(SECOND ,-1 ,(DATEADD(HOUR,-(ISNULL(@No_of_days_hrs,24)),@Schedule_DateTimeVRightRule)))
		...
		--update play day and play run in #Temp_BMS_Schedule_Right_Rule_data for USP_BMS_Schedule_revert Process
