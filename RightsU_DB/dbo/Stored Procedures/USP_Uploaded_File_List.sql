...
BEGIN
	...
	if(@Loglevel  < 2)Exec [USPLogSQLSteps] '[USP_Uploaded_File_List]', 'Step 1', 0, 'Started Procedure', 0, '' 
		IF(@PageNo = 0)
			SET @PageNo = 1
		...
		SET @Sql = 'SELECT File_Code, SUBSTRING(File_Name, CHARINDEX(''~'', File_Name) + 1, LEN(File_Name) - CHARINDEX(''~'', File_Name) + 1) AS File_Name,
					...
					ISNULL((SELECT TOP 1 CASE WHEN Row_Num = 0 AND LTRIM(RTRIM(Err_Cols)) != '''' THEN UED.Upload_Detail_Code ELSE 0 END AS Upload_Detail_Code FROM Upload_Err_Detail UED (NOLOCK)  where UED.File_Code = FU.File_Code), 0)AS Upload_Detail_Code
					FROM Upload_Files FU (NOLOCK)  WHERE File_Code IN 
					(
	...
	if(@Loglevel  < 2)Exec [USPLogSQLSteps] '[USP_Uploaded_File_List]', 'Step 2', 0, 'Procedure Excuting Completed', 0, '' 
END