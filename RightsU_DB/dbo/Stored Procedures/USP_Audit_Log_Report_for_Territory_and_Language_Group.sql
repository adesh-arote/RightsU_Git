...
BEGIN
	...
	if(@Loglevel < 2)Exec [USPLogSQLSteps] '[USP_Audit_Log_Report_for_Territory_and_Language_Group]', 'Step 1', 0, 'Started Procedure', 0, ''
		...
		BEGIN
			...
			FROM Territory_Deal_Log TDL (NOLOCK)
			INNER JOIN Users U (NOLOCK) ON U.Users_Code = TDL.User_Code
			INNER JOIN Acq_Deal AD (NOLOCK) ON AD.Acq_Deal_Code = TDL.Deal_Code AND TDL.Deal_Type='A'
			INNER JOIN Territory T (NOLOCK) ON T.Territory_Code = TDL.Territory_Code
			...
			FROM Territory_Deal_Log TDL (NOLOCK)
			INNER JOIN Users U (NOLOCK) ON U.Users_Code = TDL.User_Code
			INNER JOIN Syn_Deal SD (NOLOCK) ON SD.Syn_Deal_Code = TDL.Deal_Code AND TDL.Deal_Type='S'
			INNER JOIN Territory T (NOLOCK) ON T.Territory_Code = TDL.Territory_Code
			CROSS APPLY dbo.fn_Split_withdelemiter(TDL.Country_Codes,',') AS TEMP
			...
			FROM #Temp T
			INNER JOIN Country C (NOLOCK) ON C.Country_Code = T.Country_Codes
		...
		BEGIN
			...
			FROM Language_Deal_Log LDL (NOLOCK)
			INNER JOIN Users U (NOLOCK) ON U.Users_Code = LDL.User_Code
			INNER JOIN Acq_Deal AD (NOLOCK) ON AD.Acq_Deal_Code = LDL.Deal_Code AND LDL.Deal_Type='A'
			INNER JOIN Language_Group LG (NOLOCK) ON LG.Language_Group_Code = LDL.Language_Group_Code
			...
			FROM Language_Deal_Log LDL (NOLOCK)
			INNER JOIN Users U (NOLOCK) ON U.Users_Code = LDL.User_Code
			INNER JOIN Syn_Deal SD (NOLOCK) ON SD.Syn_Deal_Code = LDL.Deal_Code AND LDL.Deal_Type='S'
			INNER JOIN Language_Group LG (NOLOCK) ON LG.Language_Group_Code = LDL.Language_Group_Code
			CROSS APPLY dbo.fn_Split_withdelemiter(LDL.Language_Codes,',') AS TEMP
			...
			FROM #Temp2 T
			INNER JOIN Language L (NOLOCK) ON L.Language_Code = T.Language_Codes
			WHERE (@Search_Key <> '' AND (UserName LIKE @Search_Key  OR  Agreement_No LIKE @Search_Key
	...
	if(@Loglevel < 2)Exec [USPLogSQLSteps] '[USP_Audit_Log_Report_for_Territory_and_Language_Group]', 'Step 2', 0, 'Procedure Excution Completed', 0, ''
END
