CREATE PROCEDURE [dbo].[USP_Validate_Content_Music_Import]
...
BEGIN
	...
	if(@Loglevel < 2)Exec [USPLogSQLSteps] '[USP_Validate_Content_Music_Import]', 'Step 1', 0, 'Started Procedure', 0, ''  
	--declare @DM_Master_Import_Code INT = 6264
		...
		SELECT @frameLimit = CAST(Parameter_Value AS INT) FROM System_Parameter_New WHERE Parameter_Name = 'FrameLimit'
			--CM0005: Block 2- Validation
		
			SELECT @Record_Code = @DM_Master_Import_Code, @Step_No = @Step_No + 1, @Sub_Step_No = 1, @Short_Status_Code = 'CM0005', @Proc_Name = 'USP_Validate_Content_Music_Import'
		...
		IF EXISTS (SELECT Record_Status FROM DM_Content_Music (NOLOCK) where Record_Status = 'OR' AND DM_Master_Import_Code = @DM_Master_Import_Code)
		BEGIN
	...
	if(@Loglevel < 2)Exec [USPLogSQLSteps] '[USP_Validate_Content_Music_Import]', 'Step 2', 0, 'Procedure Excuting Completed', 0, ''  
END