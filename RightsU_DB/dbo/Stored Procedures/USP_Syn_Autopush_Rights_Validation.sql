...
BEGIN
	...
	if(@Loglevel< 2)Exec [USPLogSQLSteps] '[USP_Syn_Autopush_Rights_Validation]', 'Step 1', 0, 'Started Procedure', 0, '' 
		DECLARE
		...
		BEGIN ------------------------------START IDENTIFYING SYNDICATION TARGET RIGHTS 
			...
			BEGIN
				...
				SELECT TOP 1 @AcqDealRightsCode = SecondaryDataCode FROM AcqPreReqMappingData MD (NOLOCK) WHERE MD.MappingFor = 'ACQDEALRIGHTS' AND PrimaryDataCode = @SynDealRightsCode 
			END

			INSERT INTO #DstSynRights (Syn_Deal_Rights_Code_V18)
			Select Syn_Deal_Rights_Code  FROM RightsU_Broadcast.dbo.Syn_Acq_Mapping (NOLOCK) Where Deal_Rights_Code = @AcqDealRightsCode
		...
		BEGIN --------------------------START CURSOR FOR MULTIPLE SYNDICATION RIGHTS CODE 
			...
			FROM RightsU_Broadcast.dbo.Syn_Deal_Rights dr  (NOLOCK)
		...
		END-------------------------------------------------------END ASSIGN VALUE TO VARIABLES	
			...
			PRINT 'INSERTION OF CURRENT DATA WITH RESPECTED SYNDICATION DEAL RIGHTS'
				...
				From RightsU_Broadcast.dbo.Syn_Deal_Rights_Title SDRT (NOLOCK)
				INNER JOIN AcqPreReqMappingData APRMD (NOLOCK) ON APRMD.SecondaryDataCode = SDRT.Title_Code
				...
				From RightsU_Broadcast.dbo.Syn_Deal_Rights_Platform SDRP (NOLOCK)
				INNER JOIN AcqPreReqMappingData APRMD (NOLOCK) ON APRMD.SecondaryDataCode = SDRP.Platform_Code
				...
				From (
					...
					From RightsU_Broadcast.dbo.Syn_Deal_Rights_Territory  SDRT (NOLOCK)
					INNER JOIN AcqPreReqMappingData APRMD (NOLOCK) ON (APRMD.SecondaryDataCode = SDRT.Country_Code AND SDRT.Territory_Type = 'I') OR
				...
				From (
					...
					FROM RightsU_Broadcast.dbo.Syn_Deal_Rights_Subtitling SDRS (NOLOCK)
					INNER JOIN AcqPreReqMappingData APRMD (NOLOCK) ON (APRMD.SecondaryDataCode = SDRS.Language_Code AND sdrs.Language_Type= 'I') OR
				...
				Left Join Language_Group_Details lgd (NOLOCK) On srlan.Language_Group_Code = lgd.Language_Group_Code

				INSERT INTO #CurrentDubbing_AP(SynDealRightsCode, DubLanguageCode)
				SELECT DISTINCT Syn_Deal_Rights_Code, Case When srlan.Language_Type = 'G' Then lgd.Language_Code Else srlan.Language_Code End Language_Code
				From (
					SELECT DISTINCT Syn_Deal_Rights_Code, APRMD.PrimaryDataCode AS [Language_Code], APRMD.PrimaryDataCode AS  [Language_Group_Code], Language_Type
					FROM RightsU_Broadcast.dbo.Syn_Deal_Rights_Dubbing SDRD (NOLOCK)
					INNER JOIN AcqPreReqMappingData APRMD (NOLOCK) ON (APRMD.SecondaryDataCode = SDRD.Language_Code AND SDRD.Language_Type= 'I') OR
		...
		END ---------------------------------------------------- END CHECK DST SYNDICATION
			...
			BEGIN-----------------------START INSERT DESTINATION RIGHTS DATA IN TABLE VARIABLE
				...
				From RightsU_Broadcast.dbo.Syn_Deal_Rights_Platform  (NOLOCK)
				Where Syn_Deal_Rights_Code = @SynDealRightsCodeV18
				...
				From (
					...
					From RightsU_Broadcast.dbo.Syn_Deal_Rights_Territory  (NOLOCK) 
					Where Syn_Deal_Rights_Code = @SynDealRightsCodeV18
				) As srter
				Left Join RightsU_Broadcast.dbo.Territory_Details td (NOLOCK) On srter.Territory_Code = td.Territory_Code
				...
				From (
					...
					From RightsU_Broadcast.dbo.Syn_Deal_Rights_Subtitling (NOLOCK)
					Where Syn_Deal_Rights_Code = @SynDealRightsCodeV18
				) As srlan
				Left Join RightsU_Broadcast.dbo.Language_Group_Details lgd (NOLOCK) On srlan.Language_Group_Code = lgd.Language_Group_Code

				Insert InTo @Deal_Rights_Dubbing(Deal_Rights_Code, Dubbing_Code)
				Select Syn_Deal_Rights_Code, Case When srlan.Language_Type = 'G' Then lgd.Language_Code Else srlan.Language_Code End Language_Code
				From (
					Select Syn_Deal_Rights_Code, Language_Code, Language_Group_Code, Language_Type
					From RightsU_Broadcast.dbo.Syn_Deal_Rights_Dubbing (NOLOCK)
					Where Syn_Deal_Rights_Code = @SynDealRightsCodeV18
				) As srlan
				Left Join RightsU_Broadcast.dbo.Language_Group_Details lgd (NOLOCK) On srlan.Language_Group_Code = lgd.Language_Group_Code
			...
			(Select Count(*) From Syn_Deal_Rights_Subtitling S (NOLOCK) Where S.Syn_Deal_Rights_Code = SDR.Syn_Deal_Rights_Code) SubCnt, 
			(Select Count(*) From Syn_Deal_Rights_Dubbing S (NOLOCK) Where S.Syn_Deal_Rights_Code = SDR.Syn_Deal_Rights_Code) DubCnt,
	   		Sum(                        
				Case 
		...
		Inner Join Syn_Deal SD (NOLOCK) On SDR.Syn_Deal_Code = SD.Syn_Deal_Code --And IsNull(SD.Deal_Workflow_Status,'') = 'A'
		Where 
		...
		Inner Join dbo.Syn_Deal_Rights_Title SDRT (NOLOCK) On SDR.Syn_Deal_Rights_Code=SDRT.Syn_Deal_Rights_Code
		...
		END----------------------END CHECKING TITLE WITH EPISODE EXIST OR NOT
			...
			BEGIN ------------------------------------START CHECK PLATFORM SYNDICATED OR NOT
				...
				Inner Join Syn_Deal_Rights_Platform sdrp (NOLOCK) On sdrp.Syn_Deal_Rights_Code = srt.Syn_Deal_Rights_Code
				Inner JOIN AcqPreReqMappingData APRMD (NOLOCK) ON APRMD.PrimaryDataCode = sdrp.Platform_Code
				...
				SELECT DISTINCT Platform_Code FROM Syn_Deal_Rights_Platform (NOLOCK) Where Syn_Deal_Rights_Code = @SynDealRightsCode
				)) > 0)					
				BEGIN
			...
			PRINT 'DISTINCT REGION'
				...
				From (
					...
					INNER JOIN AcqPreReqMappingData APRMD (NOLOCK) ON (APRMD.PrimaryDataCode = SDRT.Territory_Code AND SDRT.Territory_Type = 'G') OR
				...
				Left Join RightsU_Broadcast.dbo.Territory_Details td (NOLOCK) On srter.SecondaryDataCode = td.Territory_Code
			...
			BEGIN ------------------------------------START CHECK SUBTITLE SYNDICATED OR NOT
				...
				Begin
						...
						InTo #Syn_Sub_AP From (
							...
							INNER JOIN AcqPreReqMappingData APRMD (NOLOCK) ON (APRMD.PrimaryDataCode = sdrs.Language_Code AND sdrs.Language_Type = 'I') OR
						...
						LEFT JOIN RightsU_Broadcast.dbo.Language_Group_Details lgd (NOLOCK) On sub.Language_Group_Code = lgd.Language_Group_Code 
			...
			BEGIN ------------------------------------START CHECk DUBBING SYNDICATED OR NOT
				...
				Begin
						...
						From (
							...
							INNER JOIN AcqPreReqMappingData APRMD (NOLOCK) ON (APRMD.PrimaryDataCode = SDRD.Language_Code AND SDRD.Language_Type = 'I') OR
						...
						Left Join RightsU_Broadcast.dbo.Language_Group_Details lgd (NOLOCK) On dub.Language_Group_Code = lgd.Language_Group_Code 
			...
			BEGIN ----------------------------------START INSERTION OF DATA IN ERROR TABLE
				...
				Begin
					...
					From(
						...
						(
							(
							Select ', ' + C.Country_Name From RightsU_Broadcast.dbo.Country C (NOLOCK) 
						...
						(
							(
							Select ', ' + l.Language_Name From RightsU_Broadcast.dbo.Language l (NOLOCK) 
	...
	if(@Loglevel< 2)Exec [USPLogSQLSteps] '[USP_Syn_Autopush_Rights_Validation]', 'Step 2', 0, 'Procedure Excuting Completed', 0, '' 
END