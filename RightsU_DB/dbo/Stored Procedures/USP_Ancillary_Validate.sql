CREATE Procedure [dbo].[USP_Ancillary_Validate]
...
BEGIN		
	...
	if(@Loglevel < 2)Exec [USPLogSQLSteps] '[USP_Ancillary_Validate]', 'Step 1', 0, 'Started Procedure', 0, ''
		
			Select ISNULL(ADA.Acq_Deal_Ancillary_Code,0) as Acq_Deal_Ancillary_Code from Acq_Deal AD (NOLOCK)
			Inner Join Acq_Deal_Ancillary ADA (NOLOCK) ON AD.Acq_Deal_Code=ADA.Acq_Deal_Code AND ADA.Ancillary_Type_code = @Ancillary_Type_code
			...
			Left Join Acq_Deal_Ancillary_Platform_Medium ADAPM (NOLOCK) ON  ADAP.Acq_Deal_Ancillary_Platform_Code=ADAPM.Acq_Deal_Ancillary_Platform_Code
	...
	if(@Loglevel < 2)Exec [USPLogSQLSteps] '[USP_Ancillary_Validate]', 'Step 2', 0, 'Procedure Excution Completed', 0, ''
END