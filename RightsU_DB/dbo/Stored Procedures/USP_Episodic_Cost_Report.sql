...
BEGIN
	...
	if(@Loglevel < 2)Exec [USPLogSQLSteps] '[usp_Episodic_Cost_Report]', 'Step 1', 0, 'Started Procedure', 0, '' 
		--DECLARE
		--@DealCode INT,
		--@TitleCode VARCHAR(MAX),
		--@Eps_From INT,
		...
		left outer  join Acq_Deal_Cost_Title adct (NOLOCK) on adc.Acq_Deal_Cost_Code = adct.Acq_Deal_Cost_Code
		left outer  join Title t (NOLOCK) on t.Title_Code = adct.Title_Code 
		where 
	...
	if(@Loglevel < 2)Exec [USPLogSQLSteps] '[usp_Episodic_Cost_Report]', 'Step 2', 0, 'Procedure Excution Completed', 0, '' 
END