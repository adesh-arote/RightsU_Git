...
BEGIN    
	...
	if(@Loglevel < 2)Exec [USPLogSQLSteps] '[USP_Integration_Deal_Data_Generation]', 'Step 1', 0, 'Started Procedure', 0, ''  
	 DECLARE @Is_Error CHAR(1), @Error_Desc NVARCHAR(MAX)      
	 BEGIN TRY      
	  SET NOCOUNT ON;       
	  DECLARE @Deal_Code INT = 0      
	  IF NOT EXISTS(SELECT TOP 1 Acq_Deal_Code FROM Integration_Deal (NOLOCK) WHERE Record_Status = 'W' )      
	  BEGIN      
	  DECLARE CUS_Deal_Process CURSOR FOR       
	  SELECT DISTINCT Acq_Deal_Code FROM  Integration_Deal (NOLOCK) WHERE Record_Status = 'P'       
	  ...
	  BEGIN    
	   ...
	   INNER JOIN Acq_Deal_Run_Channel ADRC (NOLOCK) ON ADRT.Acq_Deal_Run_Code= ADRC.Acq_Deal_Run_Code       
	   INNER JOIN Title T (NOLOCK) ON T.Title_Code= ADRT.Title_Code               
	   ...
	   (select Top 1 Integration_Title_Code from Integration_Title (NOLOCK) where Title_Code=TDD.Title_Code) AS Integration_Title_Code      
	  ...
	  IAR.Acq_Deal_Run_Code NOT IN(select DISTINCT TADR.Acq_Deal_Run_Code from #Temp_Acq_Deal_Run TADR)  
	   ...
	   INNER JOIN Acq_Deal_Run ADR (NOLOCK) ON ADRC.Acq_Deal_Run_Code =ADR.Acq_Deal_Run_Code      
	   INNER JOIN Integration_Acq_Run IAR (NOLOCK) ON IAR.Acq_Deal_Run_Code=ADRC.Acq_Deal_Run_Code      
	   ...
	   INNER JOIN Acq_Deal_Run ADR (NOLOCK) ON ADRYR.Acq_Deal_Run_Code =ADR.Acq_Deal_Run_Code      
	   INNER JOIN Integration_Acq_Run IAR (NOLOCK) ON IAR.Acq_Deal_Run_Code=ADRYR.Acq_Deal_Run_Code       
	   where ADR.Acq_Deal_Code=@Deal_Code      
	   ...
	   FROM Integration_Acq_Run_Channel IARC1 (NOLOCK)      
	   ...
	   select Integration_Acq_Run_Channel_Code FROM Integration_Acq_Run_Channel IARC1  (NOLOCK)     
	   INNER JOIN #Temp_Acq_Deal_Run_Channel TIARC1 ON IARC1.Integration_Acq_Run_Code=TIARC1.Integration_Acq_Run_Code       
	   AND IARC1.Channel_Code=TIARC1.Channel_Code      
	   )      
	   ...
	   FROM Integration_Acq_Run_Yearwise ADRYR1  (NOLOCK)                   
	   ...
	   select Integration_Acq_Run_Yearwise_Code FROM Integration_Acq_Run_Yearwise IARC   (NOLOCK)    
	   INNER JOIN #Temp_Acq_Deal_Run_Yearwise TIARC ON IARC.Integration_Acq_Run_Code=TIARC.Integration_Acq_Run_Code       
	   AND (IARC.Start_Date=TIARC.Start_Date OR IARC.End_Date = TIARC.End_Date)      
	   )      
	...
	if(@Loglevel < 2)Exec [USPLogSQLSteps] '[USP_Integration_Deal_Data_Generation]', 'Step 2', 0, 'Procedure Excution Completed', 0, ''
END