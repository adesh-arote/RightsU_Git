...
BEGIN     
	...
	if(@Loglevel < 2)Exec [USPLogSQLSteps] '[USP_Integration_SDM_Generate_XML]', 'Step 1', 0, 'Started Procedure', 0, ''    
		SET NOCOUNT ON;  
		...
		/********************************Check Module Name****************/        
		BEGIN TRY
		SELECT TOP 1 @Integration_Config_Code=Integration_Config_Code FROM Integration_Config IC  (NOLOCK)       
				WHERE 1 =1  AND UPPER(IC.Module_Name) = UPPER(@Module_Name) AND Foreign_System_Name='SDM'                 
			IF EXISTS(SELECT * from Integration_Config (NOLOCK) WHERE Integration_Config_Code=@Integration_Config_Code AND Is_Active='Y')        
			BEGIN
				/***************************************************Title*********************************************************************/        
				...
				BEGIN
					...
					INNER JOIN Title T (NOLOCK) ON T.Title_Code= ADRTitle.Title_Code   
					...
					(        
						...
						(
							...
							(
								...
								FROM Integration_SDM_Log ESL  (NOLOCK)
		...
		IF(ISNULL(@Integration_Config_Code,0) = 0)        
		  SELECT @Integration_Config_Code = Integration_Config_Code FROM Integration_Config (NOLOCK) WHERE UPPER(Module_Name) = @Module_Name 
	...
	if(@Loglevel < 2)Exec [USPLogSQLSteps] '[USP_Integration_SDM_Generate_XML]', 'Step 2', 0, 'Procedure Excution Completed', 0, ''
END