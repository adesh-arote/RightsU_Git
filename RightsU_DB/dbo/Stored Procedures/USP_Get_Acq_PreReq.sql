...
BEGIN    
	...
	BEGIN    
		...
		END    
		INSERT INTO #PreReqData(Display_Value, Display_Text, Data_For)    
		SELECT DISTINCT Objection_Type_Code, Objection_Type_Name, 'OBT' AS Data_For FROM Title_Objection_Type  --Where Deal_Flag IN ('AS')  
	...
	END      
		INSERT INTO #PreReqData(Display_Value, Display_Text, Data_For)    
		SELECT DISTINCT Title_Objection_Status_Code, Objection_Status_Name, 'OBS' AS Data_For FROM Title_Objection_Status  --Where Deal_Flag IN ('AS')  
	...
	BEGIN    
		INSERT INTO #PreReqData(Display_Value, Display_Text, Data_For)    
		SELECT DISTINCT T.Title_Code, T.Title_Name, 'TOB' AS Data_For FROM Title T     
	...
	BEGIN    
		...
		INNER JOIN Acq_Deal AD ON V.Vendor_Code = AD.Vendor_Code
		INNER JOIN Title_Objection TOB ON AD.Acq_Deal_Code = TOB.Record_Code
		WHERE  Deal_Workflow_Status NOT IN ('AR', 'WA') ORDER BY V.Vendor_Name 
	END    
