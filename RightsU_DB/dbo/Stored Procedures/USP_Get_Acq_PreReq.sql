...
BEGIN     
 ...
 if(@Loglevel < 2)Exec [USPLogSQLSteps] '[USP_Get_Acq_PreReq]', 'Step 1', 0, 'Started Procedure', 0, ''   
  SET FMTONLY OFF      
  SET NOCOUNT ON      
      
  --DECLARE      
  --@Data_For VARCHAR(MAX) = 'TOV,OBS,OBT,TOB',      
  --@Call_From VARCHAR(3) = 'LST',      
  --@LoginUserCode INT = 136,      
  --@Acq_Deal_Code INT = 0,      
  ...
  IF(@Acq_Deal_Code > 0)      
   SELECT TOP 1 @Selected_Vendor_Code = Vendor_Code FROM Acq_Deal (NOLOCK) WHERE Acq_Deal_Code = @Acq_Deal_Code      
  ...
  IF(CHARINDEX('OBT', @Data_For) > 0)      
  BEGIN      
   IF(@Call_From = 'LST')       
   BEGIN      
    INSERT INTO #PreReqData(Display_Value, Display_Text, Data_For)      
    SELECT 0 AS Display_Value, 'Please Select' AS Display_Text, 'OBT' AS Data_For      
   END      
   INSERT INTO #PreReqData(Display_Value, Display_Text, Data_For)      
   SELECT DISTINCT Objection_Type_Code, Objection_Type_Name, 'OBT' AS Data_For FROM Title_Objection_Type  (NOLOCK) --Where Deal_Flag IN ('AS')    
   ORDER BY Objection_Type_Name      
  END      
        
  IF(CHARINDEX('OBS', @Data_For) > 0)      
  BEGIN      
   IF(@Call_From = 'LST')       
   BEGIN      
   INSERT INTO #PreReqData(Display_Value, Display_Text, Data_For)      
   SELECT 0 AS Display_Value, 'Please Select' AS Display_Text, 'OBS' AS Data_For      
  END        
   INSERT INTO #PreReqData(Display_Value, Display_Text, Data_For)      
   SELECT DISTINCT Title_Objection_Status_Code, Objection_Status_Name, 'OBS' AS Data_For FROM Title_Objection_Status (NOLOCK)  --Where Deal_Flag IN ('AS')    
   ORDER BY Objection_Status_Name      
  END      
    
  IF(CHARINDEX('TOB', @Data_For) > 0)      
  BEGIN      
   INSERT INTO #PreReqData(Display_Value, Display_Text, Data_For)      
   SELECT DISTINCT T.Title_Code, T.Title_Name, 'TOB' AS Data_For FROM Title T  (NOLOCK)    
   INNER JOIN Title_Objection TOB (NOLOCK) ON T.Title_Code = TOB.Title_Code       
   ORDER BY T.Title_Name      
  END      
  
  IF(CHARINDEX('TOV', @Data_For) > 0)      
  BEGIN      
   INSERT INTO #PreReqData(Display_Value, Display_Text, Data_For)      
   SELECT DISTINCT V.Vendor_Code, V.Vendor_Name, 'TOV' AS Data_For FROM Vendor V (NOLOCK)  
   INNER JOIN Acq_Deal AD (NOLOCK) ON V.Vendor_Code = AD.Vendor_Code  
  ...
  BEGIN      
   INSERT INTO #PreReqData(Display_Value, Display_Text, Data_For)      
   SELECT DISTINCT Role_Code, Role_Name, 'ROL' AS Data_For FROM [Role] (NOLOCK) WHERE Role_Type = 'A'      
  END      
  ...
  BEGIN      
   ...
   SELECT TOP 1 @ADM_Code = ISNULL(Master_Deal_Movie_Code_ToLink, 0) FROM Acq_Deal  (NOLOCK) WHERE Acq_Deal_Code = @Acq_Deal_Code      
  ...
  BEGIN      
   ...
   --UNION      
   SELECT DISTINCT Entity_Code, [Entity_Name], 'ENT' AS Data_For FROM Entity (NOLOCK) WHERE Is_Active  = 'Y'      
  END      
  ...
  BEGIN      
   INSERT INTO #PreReqData(Display_Value, Display_Text, Data_For)      
   SELECT DISTINCT Vendor_Code, Vendor_Name, 'VEN' AS Data_For FROM Vendor   (NOLOCK)     
   WHERE (Vendor_Code IN(SELECT Vendor_Code FROM Acq_Deal_Licensor (NOLOCK) where Acq_Deal_Code = @Acq_Deal_Code) AND @Is_Vendor_Validation_For_Deal = 'Y') OR Is_Active  = 'Y'      
  ...
  BEGIN      
   ...
   INSERT INTO #PreReqData(Display_Value, Display_Text, Data_For)      
   SELECT DISTINCT V.Vendor_Code, V.Vendor_Name, 'VEN' AS Data_For FROM Vendor V  (NOLOCK)     
   INNER JOIN Vendor_Role VR  (NOLOCK) ON V.Vendor_Code = VR.Vendor_Code AND ISNULL(VR.Is_Active, 'Y') = 'Y'      
   --INNER JOIN Temp_Role TR ON TR.Role_Code = vr.Role_Code      
   WHERE V.Is_Active  = 'Y'      
   AND V.Vendor_Code IN(select Vendor_Code from Acq_Deal (NOLOCK) WHERE  Deal_Workflow_Status NOT IN ('AR', 'WA'))      
   ORDER BY V.Vendor_Name      
  END      
  END      
  ...
  BEGIN      
   ...
   INSERT INTO #PreReqData(Display_Value, Display_Text, Data_For)      
   SELECT DISTINCT VC.Vendor_Contacts_Code, VC.Contact_Name, 'VPC' AS Data_For FROM Vendor_Contacts VC  (NOLOCK)     
  ...
  BEGIN      
   ...
   INNER JOIN Acq_Deal_Movie ADM  (NOLOCK) ON T.Title_Code = ADM.Title_Code      
   INNER JOIN Acq_Deal AD (NOLOCK) ON ADM.Acq_Deal_Code = AD.Acq_Deal_Code AND AD.Business_Unit_Code = @BusinessUnitCode      
 ...
 if(@Loglevel < 2)Exec [USPLogSQLSteps] '[USP_Get_Acq_PreReq]', 'Step 2', 0, 'Procedure Excution Completed', 0, ''   
END