CREATE PROCEDURE [dbo].[USPExportToExcelBulkImport]
...
BEGIN
	...
	if(@Loglevel< 2)Exec [USPLogSQLSteps] '[USPExportToExcelBulkImport]', 'Step 1', 0, 'Started Procedure', 0, ''
	--DECLARE
	...
	--	@AdvanceSearch NVARCHAR(MAX) = ''
		...
		BEGIN   
			...
			BEGIN
				...
				SELECT  DM_Title_Code, LTRIM(RTRIM(number)) AS StarCast from DM_Title DMT (NOLOCK)
				...
				SELECT DM_Title_Code,  LTRIM(RTRIM(number)) AS ErrorMsg from DM_Title DMT (NOLOCK)
				...
				SELECT  DM_Title_Code, LTRIM(RTRIM(number)) AS Director from DM_Title DMT (NOLOCK)
				...
				SELECT  DM_Title_Code, LTRIM(RTRIM(number)) AS MusicLabel from DM_Title DMT (NOLOCK)
			 ...
			 BEGIN
				...
				DECLARE @ImportType VARCHAR(5)
				SET @ImportType = (select TOP 1 Import_Type from DM_Master_Import d where d.DM_Master_Import_Code = @DM_Master_Import_Code)
				...
				SELECT @TotalCounter = COUNT(*) + 1  FROM DM_Title_Import_Utility  (NOLOCK) where Is_Active = 'Y' AND Import_Type = @ImportType
				...
				SELECT @BulkImp_ColNames = LEFT(@BulkImp_ColNames, LEN(@BulkImp_ColNames) - 1)
				 ...
				 EXEC ('
					  ...
					  SELECT '+ @ColNames +'
					  FROM DM_Title_Import_Utility_Data (NOLOCK) WHERE DM_Master_Import_Code = '+@DM_Master_Import_Code+' 
		...
		BEGIN
			...
			SELECT DMT.IntCode, LTRIM(RTRIM(number)) AS StarCast from DM_Music_Title DMT (NOLOCK)
			CROSS APPLY dbo.fn_Split_withdelemiter([Star_Cast],',') 
			WHERE LTRIM(RTRIM(ISNULL([Star_Cast], ''))) <> ''
			
			INSERT INTO #TempSinger(ID, Singer)
			SELECT DMT.IntCode, LTRIM(RTRIM(number)) AS Singers from DM_Music_Title DMT (NOLOCK)
			CROSS APPLY dbo.fn_Split_withdelemiter([Singers],',') 
			WHERE LTRIM(RTRIM(ISNULL([Singers], ''))) <> '' 
		
			INSERT INTO #TempErrorMsg(ID, ErrorMessage)
			SELECT DMT.IntCode, LTRIM(RTRIM(number)) AS ErrorMsg from DM_Music_Title DMT (NOLOCK)
			CROSS APPLY dbo.fn_Split_withdelemiter([Error_Message],'~') 
			WHERE LTRIM(RTRIM(ISNULL([Error_Message], ''))) <> '' 

			INSERT INTO #TempLanguage(ID, Language)
			SELECT DMT.IntCode, LTRIM(RTRIM(number)) AS Language from DM_Music_Title DMT (NOLOCK)
		...
		BEGIN
			 ...
			 SELECT DCM.IntCode, LTRIM(RTRIM(number)) AS ErrorMsg from DM_Content_Music DCM (NOLOCK)
			 CROSS APPLY dbo.fn_Split_withdelemiter([Error_Message],'~') 
			 WHERE LTRIM(RTRIM(ISNULL([Error_Message], ''))) <> '' 

			 SELECT * INTO #TempMasterImportCode 
			 FROM(
			 SELECT @DM_Master_Import_Code as DM_Master_Import_Code ,Name ,Master_Type,Master_Code,User_Action,Action_By,Action_On,Roles,Is_Ignore,Mapped_By  FROM DM_Master_Log (NOLOCK) where DM_Master_Import_Code LIKE '~' + CAST(@DM_Master_Import_Code AS VARCHAR) + '~'
			 UNION
			 SELECT @DM_Master_Import_Code as DM_Master_Import_Code,Name,Master_Type,Master_Code,User_Action,Action_By,Action_On,Roles,Is_Ignore,Mapped_By  FROM DM_Master_Log (NOLOCK) where DM_Master_Import_Code = CAST(@DM_Master_Import_Code AS VARCHAR)
			 ) as a
			
			 SELECT Distinct DCM.IntCode, MT.Music_Title_Name Into #TempMusicTitle FROM DM_Content_Music DCM  (NOLOCK)
			 LEFT JOIN #TempMasterImportCode DML ON DCM.DM_Master_Import_Code = DML.DM_Master_Import_Code
			 LEFT JOIN Music_Title MT (NOLOCK) ON MT.Music_Title_Code = DML.Master_Code
								...
								(      
									select distinct * from (      
										select distinct DCM.[IntCode], DCM.[Excel_Line_No], DCM.[Content_Name], DCM.[Episode_No], DCM.[Version_Name],  DCM.[Music_Track], DCM.[From]    
												...
												FROM DM_Content_Music  DCM (NOLOCK)
												LEFT JOIN #TempMasterImportCode DML ON  DCM.DM_Master_Import_Code  =  DML.DM_Master_Import_Code 
	...
	if(@Loglevel< 2)Exec [USPLogSQLSteps] '[USPExportToExcelBulkImport]', 'Step 2', 0, 'Procedure Excution Completed', 0, ''
END