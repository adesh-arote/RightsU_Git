CREATE PROCEDURE [dbo].[USP_RightsU_Health_Checkup]
(
	@CS_Curr_Count INT = 1
)
AS
BEGIN
	...
	if(@Loglevel<2)Exec [USPLogSQLSteps] '[USP_RightsU_Health_Checkup]', 'Step 1', 0, 'Started Procedure', 0, ''
		...
		BEGIN
			...
			CASE
					...
					ELSE 'NA'
			END AS [Record_Status] 
			FROM Syn_Deal_Rights SDR (NOLOCK) INNER JOIN Syn_Deal SD (NOLOCK) ON  SD.Syn_Deal_Code = SDR.Syn_Deal_Code  
		...
		BEGIN
			  ...
			  -- RightsU_Avail_Movies (Acquisition)
			  ...
			  FROM RightsU_Avail_Movies.dbo.Approved_Deal_Process
			  ...
			  ( SELECT DISTINCT PS.Record_Code, MAX(start_Time) AS [Process_End_Date] 
					FROM RightsU_Avail_Movies.dbo.Process_Schedule PS 
			   ...
			   WHERE
			   A.Deal_Code NOT IN (SELECT Record_Code FROM RightsU_Avail_Movies.dbo.Approved_Deal_Process WHERE Deal_Status = 'P' AND Deal_Type = 'A')

			  -- RightsU_Avail_Movies (Syndication)
			  INSERT INTO #Tmp_Pending_Avail_Generation ( Deal_Type, Deal_Code, Last_Approved, Last_Avail_Refresh)
			  ...
			  ( SELECT DISTINCT PS.Record_Code, MAX(start_Time) AS [Process_End_Date] 
			   ...
			   WHERE
			   A.Deal_Code NOT IN (SELECT Record_Code FROM RightsU_Avail_Movies.dbo.Approved_Deal_Process WHERE Deal_Status = 'P' AND Deal_Type = 'S')

			   --RightsU_Avail_Shows (Acquisition)
			  INSERT INTO #Tmp_Pending_Avail_Generation ( Deal_Type, Deal_Code, Last_Approved, Last_Avail_Refresh)
			  SELECT 'Acquisition', Record_Code ,'Last Approved  Since ' + CAST( Inserted_On AS NVARCHAR(MAX)),''
			  FROM RightsU_Avail_Shows.dbo.Approved_Deal_Process
			  WHERE Deal_Status = 'P' AND Deal_Type = 'A'
			  UNION
			  SELECT 'Acquisition', A.Deal_Code,'Last Approved Since '+ CAST( A.Process_End_Date AS NVARCHAR(MAX)), 'Last Avail Refresh Since '+ CAST( B.Process_End_Date AS NVARCHAR(MAX)) FROM (
				  SELECT DISTINCT Deal_Code , MAX(Porcess_End) as [Process_End_Date] 
			  ...
			  LEFT JOIN
			  ( SELECT DISTINCT PS.Record_Code, MAX(start_Time) AS [Process_End_Date] 
					FROM RightsU_Avail_Shows.dbo.Process_Schedule PS 
					WHERE PS.Record_Type = 'A' AND job_status = 'D' 
					GROUP BY PS.Record_Code
			  ) AS B on A.Deal_Code = B.Record_Code And A.Process_End_Date > B.Process_End_Date 
			   WHERE
			   A.Deal_Code NOT IN (SELECT Record_Code FROM RightsU_Avail_Shows.dbo.Approved_Deal_Process WHERE Deal_Status = 'P' AND Deal_Type = 'A')

			  --RightsU_Avail_Shows (Syndication)
			  INSERT INTO #Tmp_Pending_Avail_Generation ( Deal_Type, Deal_Code, Last_Approved, Last_Avail_Refresh)
			  SELECT 'Syndication', Record_Code ,'Last Approved Since ' + CAST( Inserted_On AS NVARCHAR(MAX)),''
			  FROM RightsU_Avail_Shows.dbo.Approved_Deal_Process
			  WHERE Deal_Status = 'P' AND Deal_Type = 'S'
			  ...
			  ( SELECT DISTINCT PS.Record_Code, MAX(start_Time) AS [Process_End_Date] 
			   ...
			   WHERE
			   A.Deal_Code NOT IN (SELECT Record_Code FROM RightsU_Avail_Shows.dbo.Approved_Deal_Process WHERE Deal_Status = 'P' AND Deal_Type = 'S')
		...
		BEGIN
			...
			FROM (
				...
				(SELECT TOP 1 b.Acq_Deal_Movie_Code FROM Acq_Deal_Movie b (NOLOCK) WHERE b.Acq_Deal_Movie_Code = a.Acq_Deal_Movie_Code) DealMovie
				FROM Acq_Deal_Movie a (NOLOCK)
				INNER JOIN Acq_Deal AD (NOLOCK) ON AD.Acq_Deal_Code = a.Acq_Deal_Code
			) as e 
			...
			FROM (
				...
				(SELECT TOP 1 b.Acq_Deal_Rights_Code FROM Acq_Deal_Rights_Title b (NOLOCK) WHERE b.Acq_Deal_Rights_Code = a.Acq_Deal_Rights_Code) RightTitle,
				(SELECT TOP 1 c.Acq_Deal_Rights_Code FROM Acq_Deal_Rights_Territory c (NOLOCK) WHERE c.Acq_Deal_Rights_Code = a.Acq_Deal_Rights_Code) RightTerritory,
				(SELECT TOP 1 d.Acq_Deal_Rights_Code FROM Acq_Deal_Rights_Platform d (NOLOCK) WHERE d.Acq_Deal_Rights_Code = a.Acq_Deal_Rights_Code) RightPlatform
				FROM Acq_Deal_Rights a (NOLOCK)
				inner join Acq_Deal AD  (NOLOCK) ON AD.Acq_Deal_Code = a.Acq_Deal_Code
			) as e 
			...
			FROM (
				...
				(SELECT TOP 1 b.Acq_Deal_Pushback_Code FROM Acq_Deal_Pushback_Title b (NOLOCK) WHERE b.Acq_Deal_Pushback_Code = a.Acq_Deal_Pushback_Code) PushbackTitle,
				(SELECT TOP 1 c.Acq_Deal_Pushback_Code FROM Acq_Deal_Pushback_Territory c  (NOLOCK) WHERE c.Acq_Deal_Pushback_Code = a.Acq_Deal_Pushback_Code) PushbackTerritory,
				(SELECT TOP 1 d.Acq_Deal_Pushback_Code FROM Acq_Deal_Pushback_Platform d (NOLOCK) WHERE d.Acq_Deal_Pushback_Code = a.Acq_Deal_Pushback_Code) PushbackPlatform
			...
			FROM (
				...
				(SELECT TOP 1 b.Term FROM Acq_Deal_Rights B (NOLOCK) WHERE ISNULL(B.Term,'') = '' AND B.Right_Type = 'Y' AND B.Acq_Deal_Rights_Code = A.Acq_Deal_Rights_Code) Acq_Deal_Rights_Term
				FROM Acq_Deal_Rights A (NOLOCK)
				INNER JOIN Acq_Deal AD (NOLOCK) ON AD.Acq_Deal_Code = A.Acq_Deal_Code
			) as e
			...
			FROM (
				...
				(SELECT TOP 1 b.Acq_Deal_Run_Code FROM Acq_Deal_Run_Title b (NOLOCK) WHERE b.Acq_Deal_Run_Code = a.Acq_Deal_Run_Code) RunTitle,
				(SELECT TOP 1 c.Acq_Deal_Run_Code FROM Acq_Deal_Run_Channel c (NOLOCK) WHERE c.Acq_Deal_Run_Code = a.Acq_Deal_Run_Code) RunChannel
				FROM Acq_Deal_Run a (NOLOCK)
				INNER JOIN Acq_Deal AD (NOLOCK) ON AD.Acq_Deal_Code = a.Acq_Deal_Code
			) as e 
			...
			FROM (
				...
				(SELECT TOP 1 b.Acq_Deal_Ancillary_Code FROM Acq_Deal_Ancillary_Title b (NOLOCK) WHERE b.Acq_Deal_Ancillary_Code = a.Acq_Deal_Ancillary_Code) AncillaryTitle--,
				...
				FROM Acq_Deal_Ancillary a (NOLOCK)
				INNER JOIN Acq_Deal AD (NOLOCK) ON AD.Acq_Deal_Code = a.Acq_Deal_Code
			) as e 
			...
			FROM (
				SELECT ad.Agreement_No ,
				(SELECT TOP 1 b.Acq_Deal_Cost_Code FROM Acq_Deal_Cost_Title b (NOLOCK) WHERE b.Acq_Deal_Cost_Code = a.Acq_Deal_Cost_Code) CostTitle,
				(SELECT TOP 1 c.Acq_Deal_Cost_Code FROM Acq_Deal_Cost_Costtype c (NOLOCK) WHERE c.Acq_Deal_Cost_Code = a.Acq_Deal_Cost_Code) Costtype
				FROM Acq_Deal_Cost a (NOLOCK)
				INNER JOIN Acq_Deal AD (NOLOCK) ON AD.Acq_Deal_Code = a.Acq_Deal_Code
			) as e 
			...
			+ 'is Blank'
			FROM Acq_Deal_Payment_Terms a (NOLOCK)
			INNER JOIN Acq_Deal b (NOLOCK) on a.Acq_Deal_Code = b.Acq_Deal_Code
			...
			+ 'is Blank'
			FROM Acq_Deal_Material a (NOLOCK)
			INNER JOIN Acq_Deal b (NOLOCK) on a.Acq_Deal_Code = b.Acq_Deal_Code
			...
			+ 'is Blank'
			FROM Acq_Deal_Attachment a (NOLOCK)
			INNER JOIN Acq_Deal b (NOLOCK) on a.Acq_Deal_Code = b.Acq_Deal_Code
			...
			FROM (
				...
				(SELECT TOP 1 b.Syn_Deal_Movie_Code FROM Syn_Deal_Movie b (NOLOCK) WHERE b.Syn_Deal_Movie_Code = a.Syn_Deal_Movie_Code) DealMovie
				FROM Syn_Deal_Movie a (NOLOCK)
				INNER JOIN Syn_Deal AD (NOLOCK) ON AD.Syn_Deal_Code = a.Syn_Deal_Code
			) as e 
			...
			FROM (
				...
				(SELECT TOP 1 b.Syn_Deal_Rights_Code FROM Syn_Deal_Rights_Title b (NOLOCK) WHERE b.Syn_Deal_Rights_Code = a.Syn_Deal_Rights_Code) RightTitle,
				(SELECT TOP 1 c.Syn_Deal_Rights_Code FROM Syn_Deal_Rights_Territory c (NOLOCK) WHERE c.Syn_Deal_Rights_Code = a.Syn_Deal_Rights_Code) RightTerritory,
				(SELECT TOP 1 d.Syn_Deal_Rights_Code FROM Syn_Deal_Rights_Platform d (NOLOCK) WHERE d.Syn_Deal_Rights_Code = a.Syn_Deal_Rights_Code) RightPlatform
				FROM Syn_Deal_Rights a (NOLOCK)
				inner join Syn_Deal AD (NOLOCK) ON AD.Syn_Deal_Code = a.Syn_Deal_Code
			) as e 
			WHERE e.RightPlatform IS NULL or e.RightTitle is null or e.RightTerritory is null
			UNION --REVERSE HOLDBACK
			...
			FROM (
				...
				(SELECT TOP 1 d.Syn_Deal_Rights_Code FROM Syn_Deal_Rights_Platform d (NOLOCK) WHERE d.Syn_Deal_Rights_Code = a.Syn_Deal_Rights_Code) RightPlatform
				FROM Syn_Deal_Rights a (NOLOCK)
				inner join Syn_Deal AD (NOLOCK) ON AD.Syn_Deal_Code = a.Syn_Deal_Code AND  a.Is_Pushback = 'Y'
			) as e 
			WHERE e.RightPlatform IS NULL or e.RightTitle is null or e.RightTerritory is null
			UNION --RUN DEFINATION
			SELECT e.Agreement_No,
			...
			FROM (
				...
				(SELECT TOP 1 c.Syn_Deal_Run_Code FROM Syn_Deal_Run_Platform c (NOLOCK) WHERE c.Syn_Deal_Run_Code = a.Syn_Deal_Run_Code) RunPlatform
				FROM Syn_Deal_Run a (NOLOCK)
				INNER JOIN Syn_Deal AD (NOLOCK) ON AD.Syn_Deal_Code = a.Syn_Deal_Code
			) as e 
			...
			+ ISNULL(CASE WHEN ISNULL(a.Percentage, 0) = 0 THEN 'Percentage(%) Amount '	END, '') 
			+ 'is Blank'
			FROM Syn_Deal_Payment_Terms a (NOLOCK)
			INNER JOIN Syn_Deal b (NOLOCK) on a.Syn_Deal_Code = b.Syn_Deal_Code
			...
			+ ISNULL(CASE WHEN ISNULL(a.Title_Code, 0) = 0 THEN 'Title Code '	END, '') 
			+ 'is Blank'
			FROM Syn_Deal_Material a (NOLOCK)
			INNER JOIN Syn_Deal b (NOLOCK) on a.Syn_Deal_Code = b.Syn_Deal_Code
			WHERE  ISNULL(a.Material_Medium_Code, 0) = 0 OR ISNULL(a.Material_Type_Code, 0) = 0 OR ISNULL(a.Title_Code, 0) = 0 
			UNION --ATTACHMENT
			SELECT b.Agreement_No, 
			...
			+ ISNULL(CASE WHEN ISNULL(a.Document_Type_Code, 0) = 0 THEN 'Document Type Code '	END, '') 
			+ 'is Blank'
			FROM Syn_Deal_Attachment a (NOLOCK)
			INNER JOIN Syn_Deal b  (NOLOCK) on a.Syn_Deal_Code = b.Syn_Deal_Code
			WHERE  ISNULL(a.Attachment_Title, '') = '' OR ISNULL(a.System_File_Name, '') = '' OR ISNULL(a.Document_Type_Code, 0) = 0 
			UNION --ANCILLARY
			SELECT e.Agreement_No,
			...
			FROM (
				...
				(SELECT TOP 1 b.Syn_Deal_Ancillary_Code FROM Syn_Deal_Ancillary_Title b (NOLOCK) WHERE b.Syn_Deal_Ancillary_Code = a.Syn_Deal_Ancillary_Code) AncillaryTitle--,
				...
				FROM Syn_Deal_Ancillary a (NOLOCK)
				INNER JOIN Syn_Deal AD (NOLOCK) ON AD.Syn_Deal_Code = a.Syn_Deal_Code
			) as e 
			WHERE e.AncillaryTitle IS NULL --or e.AncillaryPlatform IS NULL
			UNION --REVENUE
			...
			FROM (
				...
				(SELECT TOP 1 b.Syn_Deal_Revenue_Code FROM Syn_Deal_Revenue_Title b (NOLOCK) WHERE b.Syn_Deal_Revenue_Code = a.Syn_Deal_Revenue_Code) RevenueTitle,
				(SELECT TOP 1 c.Syn_Deal_Revenue_Code FROM Syn_Deal_Revenue_Costtype c (NOLOCK) WHERE c.Syn_Deal_Revenue_Code = a.Syn_Deal_Revenue_Code) RevenuePlatform
				FROM Syn_Deal_Revenue a (NOLOCK)
				INNER JOIN Syn_Deal AD (NOLOCK) ON AD.Syn_Deal_Code = a.Syn_Deal_Code
			) as e 
	...
	if(@Loglevel<2)Exec [USPLogSQLSteps] '[USP_RightsU_Health_Checkup]', 'Step 2', 0, 'Procedure Excuting Completed', 0, ''
END
