CREATE PROCEDURE [dbo].[USP_Acq_Bulk_Update_Final]
(
...
BEGIN
	...
	if(@Loglevel < 2)Exec [USPLogSQLSteps] '[USP_Acq_Bulk_Update_Final]', 'Step 1', 0, 'Started Procedure', 0, ''
				...
				BEGIN
					...
					BEGIN
						...
						EXEC [dbo].[USP_Validate_Rights_Duplication_UDT_Acq] @Deal_Rights_UDT,@Deal_Rights_Title_UDT,@Deal_Rights_Platform_UDT,@Deal_Rights_Territory_UDT,
																			@Deal_Rights_Subtitling_UDT,@Deal_Rights_Dubbing_UDT,'AR','',0
					...
					BEGIN
						...
						INTERSECT SELECT Platform_Code FROM Platform (NOLOCK) WHERE Is_No_Of_Run = 'Y') AS A ) = 0 
						BEGIN
								...
								SELECT @Count_TC = COUNT(*) FROM (
									...
									SELECT  ADRT.Title_Code FROM Acq_Deal_Run ADR (NOLOCK) INNER JOIN 
											Acq_Deal_Run_Title ADRT ON ADR.Acq_Deal_Run_Code = ADRT.Acq_Deal_Run_Code
						...
						BEGIN
							...
							AND SDRP.Acq_Deal_Rights_Code IN(@RCODE) AND SDRP.Acq_Deal_Rights_Code NOT IN (
							select DISTINCT SAM.Deal_Rights_Code FROM Syn_Acq_Mapping SAM (NOLOCK)
							INNER JOIN Syn_Deal_Rights SDR (NOLOCK) ON SAM.Syn_Deal_Rights_Code=SDR.Syn_Deal_Rights_Code
							...
							(
								...
								INNER JOIN Acq_Deal_Run_Title RunT (NOLOCK) ON ADRT.Title_Code=RunT.Title_Code
								Inner Join Acq_Deal_Rights_Platform ADRP (NOLOCK) ON  ADRP.Acq_Deal_Rights_Code = ADR.Acq_Deal_Rights_Code 
								Inner JOIN [Platform] p (NOLOCK) ON ADRP.Platform_Code=p.Platform_Code
						...
						IF(ISNULL(@IS_SYN_ACQ_MAP,'') = 'Y')
							...
							INNER JOIN Syn_Deal_Rights SDR (NOLOCK) ON SAM.Syn_Deal_Rights_Code=SDR.Syn_Deal_Rights_Code
							INNER JOIN Syn_Deal_Rights_Platform SDRP (NOLOCK) ON SDRP.Syn_Deal_Rights_Code=SDR.Syn_Deal_Rights_Code
							INNER JOIN Acq_Deal_Rights_Platform ADRP (NOLOCK) ON ADRP.Acq_Deal_Rights_Code=SAM.Deal_Rights_Code
							where SAM.Deal_Rights_Code IN(@RCODE) 
							AND SDRP.Platform_Code IN (select number FROM dbo.[fn_Split_withdelemiter](@Codes,','))
							AND ADRP.Platform_Code IN(select number FROM dbo.[fn_Split_withdelemiter](@Codes,','))
						END

						--Code of anchal comented by akshay for new single liner run validation
						/*
						INSERT INTO #RCodeForRunDef(Right_Code)
						select Distinct Adr.Acq_Deal_Rights_Code FROM Acq_Deal_Rights ADR
						INNER JOIN Acq_Deal_Rights_Title ADRT ON ADR.Acq_Deal_Rights_Code=ADRT.Acq_Deal_Rights_Code
				...
				BEGIN
					...
					BEGIN
						...
						(	
							...
							select DISTINCT SAM.Deal_Rights_Code FROM Syn_Acq_Mapping SAM (NOLOCK)
							INNER JOIN Syn_Deal_Rights SDR (NOLOCK) ON SAM.Syn_Deal_Rights_Code=SDR.Syn_Deal_Rights_Code
							INNER JOIN Syn_Deal_Rights_Territory SDRTT (NOLOCK) ON SDRTT.Syn_Deal_Rights_Code=SDR.Syn_Deal_Rights_Code
							where SAM.Deal_Rights_Code IN (@RCODE) 
						...
						INNER JOIN Syn_Deal_Rights SDR (NOLOCK) ON SAM.Syn_Deal_Rights_Code=SDR.Syn_Deal_Rights_Code
						INNER JOIN Syn_Deal_Rights_Territory SDRTT (NOLOCK) ON SDRTT.Syn_Deal_Rights_Code=SDR.Syn_Deal_Rights_Code
						INNER JOIN Acq_Deal_Rights_Territory ADRT (NOLOCK) ON ADRT.Acq_Deal_Rights_Code=SAM.Deal_Rights_Code
						where SAM.Deal_Rights_Code IN (@RCODE) 
						AND SDRTT.Country_Code IN (select number FROM dbo.[fn_Split_withdelemiter](@Codes,','))
						AND ADRT.Country_Code IN(select number FROM dbo.[fn_Split_withdelemiter](@Codes,','))
						END
				...
				IF(@Is_Theatrical!='Y')
					...
					IF(ISNULL(@IS_SYN_ACQ_MAP,'') = 'Y')
						...
						DELETE FROM Acq_Deal_Rights_Territory where Acq_Deal_Rights_Code IN
							(	
								...
								AND SDRP.Acq_Deal_Rights_Code NOT IN(
									...
									INNER JOIN Acq_Deal_Rights_Territory ADRT (NOLOCK) ON ADRT.Acq_Deal_Rights_Code=SAM.Deal_Rights_Code
									INNER JOIN Territory_Details TD (NOLOCK) ON ADRT.Territory_Code=TD.Territory_Code where SAM.Deal_Rights_Code IN (@RCODE) 
									AND 
									( 
									SDRT.Territory_Code IN (select number FROM dbo.[fn_Split_withdelemiter](@Codes,','))OR SDRT.Country_Code=TD.Country_Code
									)
									)
									AND SDRP.Acq_Deal_Rights_Code NOT IN(
									select Distinct RH.Acq_Deal_Rights_Code FROM Acq_Deal_Rights_Holdback RH (NOLOCK)
						...
						BEGIN
							...
							INNER JOIN Syn_Deal_Rights_Territory SDRT (NOLOCK)  ON SDRT.Syn_Deal_Rights_Code=SDR.Syn_Deal_Rights_Code
							INNER JOIN Acq_Deal_Rights_Territory ADRT (NOLOCK) ON ADRT.Acq_Deal_Rights_Code=SAM.Deal_Rights_Code
							INNER JOIN Territory_Details TD (NOLOCK) ON ADRT.Territory_Code=TD.Territory_Code
							where SAM.Deal_Rights_Code IN (@RCODE) 
							AND 
							( 
				...
				BEGIN
					...
					(	
						...
						AND SDR.Acq_Deal_Rights_Code NOT IN (
							select DISTINCT  SAM.Deal_Rights_Code FROM Syn_Acq_Mapping SAM (NOLOCK)
							INNER JOIN Syn_Deal_Rights SDR (NOLOCK) ON SAM.Syn_Deal_Rights_Code=SDR.Syn_Deal_Rights_Code
							INNER JOIN Syn_Deal_Rights_Subtitling SDRSS (NOLOCK) ON SDRSS.Syn_Deal_Rights_Code=SDR.Syn_Deal_Rights_Code
							where SAM.Deal_Rights_Code IN (@RCODE)
					...
					BEGIN
						...
						INNER JOIN Syn_Deal_Rights SDR (NOLOCK) ON SAM.Syn_Deal_Rights_Code=SDR.Syn_Deal_Rights_Code
						INNER JOIN Syn_Deal_Rights_Subtitling SDRSS (NOLOCK) ON SDRSS.Syn_Deal_Rights_Code=SDR.Syn_Deal_Rights_Code
						INNER JOIN Acq_Deal_Rights_Subtitling ADRSS (NOLOCK) ON ADRSS.Acq_Deal_Rights_Code=SAM.Deal_Rights_Code
						where SAM.Deal_Rights_Code IN (@RCODE)
						AND SDRSS.Language_Code IN(select number FROM dbo.[fn_Split_withdelemiter](@Codes, ','))
						AND ADRSS.Language_Code IN(select number FROM dbo.[fn_Split_withdelemiter](@Codes, ','))
					END
				...
				IF(ISNULL(@IS_SYN_ACQ_MAP,'') = 'Y')
					BEGIN
						...
						(	
							...
							AND SDR.Acq_Deal_Rights_Code NOT IN (
							select DISTINCT SAM.Deal_Rights_Code FROM Syn_Acq_Mapping SAM (NOLOCK)
						INNER JOIN Syn_Deal_Rights SDRR (NOLOCK) ON SAM.Syn_Deal_Rights_Code=SDRR.Syn_Deal_Rights_Code
						INNER JOIN Syn_Deal_Rights_Subtitling SDRSS (NOLOCK) ON SDRSS.Syn_Deal_Rights_Code=SDRSS.Syn_Deal_Rights_Code
							INNER JOIN Acq_Deal_Rights_Subtitling ADRS (NOLOCK) ON ADRS.Acq_Deal_Rights_Code=SAM.Deal_Rights_Code
							INNER JOIN Language_Group_Details LGD (NOLOCK) ON ADRS.Language_Group_Code=LGD.Language_Group_Code 
								where SAM.Deal_Rights_Code IN (@RCODE) 

								AND 

							( SDRSS.Language_Group_Code IN (SELECT number FROM dbo.[fn_Split_withdelemiter](@Codes, ','))

								OR 

								SDRSS.Language_Code=LGD.Language_Code
			...
			BEGIN
				...
				IF(ISNULL(@IS_SYN_ACQ_MAP,'') = 'Y')
					...
					(	
						...
						AND SDR.Acq_Deal_Rights_Code NOT IN (
							select DISTINCT  SAM.Deal_Rights_Code FROM Syn_Acq_Mapping SAM (NOLOCK)
							INNER JOIN Syn_Deal_Rights SDR (NOLOCK) ON SAM.Syn_Deal_Rights_Code=SDR.Syn_Deal_Rights_Code
							INNER JOIN Syn_Deal_Rights_Dubbing SDRS (NOLOCK) ON SDRS.Syn_Deal_Rights_Code=SDR.Syn_Deal_Rights_Code
							where SAM.Deal_Rights_Code IN(@RCODE) 
				...
				END
					...
					BEGIN
						...
						INNER JOIN Syn_Deal_Rights SDR (NOLOCK) ON SAM.Syn_Deal_Rights_Code=SDR.Syn_Deal_Rights_Code
						INNER JOIN Syn_Deal_Rights_Dubbing SDRS (NOLOCK) ON SDRS.Syn_Deal_Rights_Code=SDR.Syn_Deal_Rights_Code
						Inner join Acq_Deal_Rights_Dubbing ADRS (NOLOCK) on ADRS.Acq_Deal_Rights_Code=SAM.Deal_Rights_Code
						where SAM.Deal_Rights_Code IN(@RCODE) 
						AND SDRS.Language_Code IN (SELECT number FROM dbo.[fn_Split_withdelemiter](@Codes, ','))
						AND ADRS.Language_Code IN(SELECT number FROM dbo.[fn_Split_withdelemiter](@Codes, ','))
					END
			...
			BEGIN
				...
				BEGIN
					...
					(	
						...
						AND
						SDR.Acq_Deal_Rights_Code NOT IN (
							select DISTINCT SAM.Deal_Rights_Code FROM Syn_Acq_Mapping SAM (NOLOCK)
							INNER JOIN Syn_Deal_Rights SDR (NOLOCK) ON SAM.Syn_Deal_Rights_Code=SDR.Syn_Deal_Rights_Code
							INNER JOIN Syn_Deal_Rights_Dubbing SDRS (NOLOCK) ON SDRS.Syn_Deal_Rights_Code=SDR.Syn_Deal_Rights_Code
							INNER JOIN Acq_Deal_Rights_Dubbing ADRS (NOLOCK) ON ADRS.Acq_Deal_Rights_Code=SAM.Deal_Rights_Code
							...
							where SAM.Deal_Rights_Code IN (@RCODE) 
							AND 
							( SDRS.Language_Group_Code IN (select number FROM dbo.[fn_Split_withdelemiter](@Codes,','))	OR SDRS.Language_Code=LGD.Language_Code )
							) AND SDR.Acq_Deal_Rights_Code NOT IN(select Distinct RH.Acq_Deal_Rights_Code FROM Acq_Deal_Rights_Holdback RH
							Inner join Acq_Deal_Rights_Holdback_Dubbing HS (NOLOCK) on RH.Acq_Deal_Rights_Holdback_Code=HS.Acq_Deal_Rights_Holdback_Code
			...
			BEGIN
				...
				BEGIN
					...
					BEGIN
						...
						BEGIN
							...
							SELECT @Is_T= Is_Tentative FROM Acq_Deal_Rights (NOLOCK) WHERE Acq_Deal_Rights_Code=@RCODE
							...
							END
							select @DateCount=COUNT(*) FROM Syn_Deal_Rights (NOLOCK) where (DATEDIFF(day,@Start_Date,Right_Start_Date)<0 OR DATEDIFF(day,@End_Date,Right_End_Date)>0)
							AND Syn_Deal_Rights_Code IN(Select Syn_Deal_Rights_Code FROM Syn_Acq_Mapping (NOLOCK) where Deal_Rights_Code=@RCODE)
						END
				...
				END
					...
					BEGIN 
						...
						INNER JOIN Acq_Deal_Rights_Title ADRT (NOLOCK) ON ADR.Acq_Deal_Rights_Code=ADRT.Acq_Deal_Rights_Code 
						INNER JOIN Acq_Deal_Run_Title RunT (NOLOCK) ON ADRu.Acq_Deal_Run_Code=RunT.Acq_Deal_Run_Code AND ADRT.Title_Code  = RunT.Title_Code
						where ADR.Acq_Deal_Rights_Code IN(@RCODE)
				...
				BEGIN
					...
					@Deal_Rights_Territory_UDT,@Deal_Rights_Subtitling_UDT,@Deal_Rights_Dubbing_UDT,'AR','',0
					...
					BEGIN
						IF(@Is_Exclusive='N')
							BEGIN
								...
								IF(ISNULL(@IS_SYN_ACQ_MAP,'') = 'Y')
								BEGIN
									UPDATE Acq_Deal_Rights SET Is_Exclusive=@Is_Exclusive
									WHERE Acq_Deal_Rights_Code=@RCODE
									AND Acq_Deal_Rights_Code NOT IN (
				...
				BEGIN
					...
					BEGIN
						INSERT INTO #RCodeForSynMap(Right_Code)
						select DISTINCT  SAM.Deal_Rights_Code FROM Syn_Acq_Mapping SAM (NOLOCK) INNER JOIN Syn_Deal_Rights SDR ON SAM.Syn_Deal_Rights_Code=SDR.Syn_Deal_Rights_Code
						where SAM.Deal_Rights_Code IN(@RCODE) 
					END
					...
					IF(NOT EXISTS(select * FROM ##Error_Record where Rights_Code=@RCODE) AND NOT EXISTS(select * from #RCodeForSynMap where Right_Code=@RCODE))
					BEGIN
						if(@Codes='0'OR @Codes='-1')
						BEGIN
							IF(ISNULL(@IS_SYN_ACQ_MAP,'') = 'Y')
							BEGIN
								UPDATE Acq_Deal_Rights SET  Is_Sub_License='N',Sub_License_Code=NULL WHERE Acq_Deal_Rights_Code NOT IN (
									select DISTINCT  SAM.Deal_Rights_Code FROM Syn_Acq_Mapping SAM
									INNER JOIN Syn_Deal_Rights SDR ON SAM.Syn_Deal_Rights_Code=SDR.Syn_Deal_Rights_Code
									where SAM.Deal_Rights_Code IN(@RCODE) 
								) AND Acq_Deal_Rights_Code = @RCODE
							END
							ELSE
							BEGIN
		...
		BEGIN
			...
			Insert INTO #Result(Msg_Type,Right_Codes)
			SELECT '',number FROM dbo.[fn_Split_withdelemiter](@Right_Codes,',') where number!='0' AND number!='' AND number NOT IN(select Right_Code FROM #RightCode)
		END
			...
			BEGIN
						...
						stuff(
							reverse(
								STUFF((
									...
									LEFT JOIN Territory t (NOLOCK) ON t.Territory_Code = adrt.Territory_Code
									LEFT JOIN Country c (NOLOCK) ON c.Country_Code = adrt.Country_Code
				...
				STUFF((
					...
					(
						...
						FROM Acq_Deal_Rights_Platform (NOLOCK)
				...
				REVERSE(
						STUFF(REVERSE(STUFF((
							...
							LEFT JOIN Language_Group lg (NOLOCK) ON lg.Language_Group_Code = adrs.Language_Group_Code
							LEFT JOIN [Language] l (NOLOCK) ON l.Language_Code = adrs.Language_Code
				...
				REVERSE(
						STUFF(REVERSE(STUFF((
							Select   CASE WHEN Language_Type = 'G' THEN lg.Language_Group_Name+', '
							WHEN Language_Type = 'L' THEN l.Language_Name+', ' END
							from Acq_Deal_Rights_Dubbing adrs (NOLOCK)
							LEFT JOIN Language_Group lg  (NOLOCK) ON lg.Language_Group_Code = adrs.Language_Group_Code
							LEFT JOIN [Language] l (NOLOCK) ON l.Language_Code = adrs.Language_Code
							where adrs.Acq_Deal_Rights_Code In (@RC)
				...
				(
					(
						...
						Where  t.Deal_Workflow_Status NOT IN ('AR', 'WA') AND t.Acq_Deal_Code In (
							Select adr.Acq_Deal_Code FROM Acq_Deal_Rights adr (NOLOCK) where adr.Acq_Deal_Rights_Code=@RC
					)FOR XML PATH('')), 1, 1, ''
				) as Agreement_No
				From (
					Select T.Title_Code,
					...
					INNER JOIN Acq_Deal_Rights_Title ADRT (NOLOCK) on ADR.Acq_Deal_Rights_Code = ADRT.Acq_Deal_Rights_Code
					INNER JOIN Title T (NOLOCK) ON T.Title_Code=ADRT.Title_Code where ADR.Acq_Deal_Rights_Code=@RC
		...
		IF OBJECT_ID('tempdb..#RightCodeTab') IS NOT NULL DROP TABLE #RightCodeTab
		IF OBJECT_ID('tempdb..#SelectedCodeTab') IS NOT NULL DROP TABLE #SelectedCodeTab
		IF OBJECT_ID('tempdb..#RCodeForHoldback') IS NOT NULL DROP TABLE #RCodeForHoldback
		IF OBJECT_ID('tempdb..#RCodeForSynMap') IS NOT NULL DROP TABLE #RCodeForSynMap
		IF OBJECT_ID('tempdb..#RCodeForRunDef') IS NOT NULL DROP TABLE #RCodeForRunDef
		IF OBJECT_ID('tempdb..#RCodeGrp') IS NOT NULL DROP TABLE #RCodeGrp
	...
	if(@Loglevel < 2)Exec [USPLogSQLSteps] '[USP_Acq_Bulk_Update_Final]', 'Step 2', 0, 'Procedure Excution Completed', 0, ''
END
