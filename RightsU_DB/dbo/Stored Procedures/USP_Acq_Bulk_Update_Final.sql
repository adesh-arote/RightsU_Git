		...
		BEGIN
			...
			BEGIN
				...
				BEGIN
					...
					IF (SELECT COUNT(*) FROM ( SELECT Platform_Code from Acq_Deal_Rights_Platform WHERE Acq_Deal_Rights_Code IN (@RCODE) --SELECT number FROM dbo.[fn_Split_withdelemiter](@Codes,',')
					...
					BEGIN
							...
							SELECT TOP 1 @Acq_Deal_Code =  Acq_Deal_Code FROM Acq_Deal_Rights WHERE Acq_Deal_Rights_Code IN (@RCODE) 

							SELECT @Count_TC = COUNT(*) FROM (
								SELECT  ADRT.Title_Code FROM Acq_Deal_Rights_Title ADRT INNER JOIN 
										Acq_Deal_Rights ADR ON ADR.Acq_Deal_Rights_Code = ADRT.Acq_Deal_Rights_Code
										WHERE ADR.Acq_Deal_Code = @Acq_Deal_Code AND ADR.Acq_Deal_Rights_Code IN (@RCODE)
								INTERSECT
								SELECT  ADRT.Title_Code FROM Acq_Deal_Run ADR INNER JOIN 
							...
							IF (SELECT COUNT(*) FROM Acq_Deal_Rights ADR 
					...
					BEGIN
						...
						select Acq_Deal_Rights_Code	from Acq_Deal_Rights_Platform AS SDRP
			...
			BEGIN
				...
				BEGIN
					...
					BEGIN
						...
						LEFT JOIN Acq_Deal_Rights_Territory AS ADRT 
				...
				BEGIN
					...
					(	
						...
						NOT IN(
						select Distinct RH.Acq_Deal_Rights_Code FROM Acq_Deal_Rights_Holdback RH
						Inner join Acq_Deal_Rights_Holdback_Territory HT on RH.Acq_Deal_Rights_Holdback_Code=HT.Acq_Deal_Rights_Holdback_Code
						where RH.Acq_Deal_Rights_Code IN (@RCODE) 
					...
					DELETE FROM Acq_Deal_Rights_Territory where Acq_Deal_Rights_Code IN
						(	
							...
							AND SDRP.Acq_Deal_Rights_Code NOT IN(
								...
								INNER JOIN Acq_Deal_Rights_Territory ADRT ON ADRT.Acq_Deal_Rights_Code=SAM.Deal_Rights_Code
								INNER JOIN Territory_Details TD ON ADRT.Territory_Code=TD.Territory_Code where SAM.Deal_Rights_Code IN (@RCODE) 
								AND 
								( 
		...
		BEGIN
			...
			BEGIN
				...
				BEGIN
					...
					LEFT JOIN 
					Acq_Deal_Rights_Dubbing AS ADRD 
		...
		BEGIN
			...
			END
				...
				BEGIN 
					...
					INNER JOIN Acq_Deal_Rights_Title ADRT ON ADR.Acq_Deal_Rights_Code=ADRT.Acq_Deal_Rights_Code 
					INNER JOIN Acq_Deal_Run_Title RunT ON ADRu.Acq_Deal_Run_Code=RunT.Acq_Deal_Run_Code AND ADRT.Title_Code  = RunT.Title_Code
					where ADR.Acq_Deal_Rights_Code IN(@RCODE)
			...
			BEGIN
				...
				BEGIN
					...
					BEGIN
						...
						AND	Acq_Deal_Rights_Code NOT IN 
						(
							select DISTINCT ADRH.Acq_Deal_Rights_Code FROM Acq_Deal_Rights_Holdback ADRH where ADRH.Is_Title_Language_Right='Y'
						)
					END
					ELSE
					BEGIN
...
END
	...
	--	[Is_Title_Language],
	--	[Is_Tentative])
	--VALUES (',2343,3343~,Y,Y','TL','A',null,null,null,null,null,null,null,'',null,'N',null)

	--select * from @MyTable

	--EXEC USP_Acq_Bulk_Update @MyTable
