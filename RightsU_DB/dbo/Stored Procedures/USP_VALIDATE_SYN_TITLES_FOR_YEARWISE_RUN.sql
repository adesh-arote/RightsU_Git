...
BEGIN
	...
	if(@Loglevel< 2)Exec [USPLogSQLSteps] '[USP_VALIDATE_SYN_TITLES_FOR_YEARWISE_RUN]', 'Step 1', 0, 'Started Procedure', 0, ''
		...
		BEGIN
			...
			BEGIN
				...
				SELECT  DISTINCT SDM.Syn_Deal_Movie_Code from Syn_Deal_Rights SDR (NOLOCK)
			...
			BEGIN
				INSERT INTO @tableTemp(Title_Code) 
				SELECT  DISTINCT SDRT.Title_Code from Syn_Deal_Rights SDR (NOLOCK)
				inner join Syn_Deal_Rights_Title SDRT (NOLOCK) on SDRT.Syn_Deal_Rights_Code = SDR.Syn_Deal_Rights_Code
				where SDR.Syn_Deal_Code = @Deal_Code 
	...
	if(@Loglevel< 2)Exec [USPLogSQLSteps] '[USP_VALIDATE_SYN_TITLES_FOR_YEARWISE_RUN]', 'Step 2', 0, 'Procedure Excution Completed ', 0, ''
END