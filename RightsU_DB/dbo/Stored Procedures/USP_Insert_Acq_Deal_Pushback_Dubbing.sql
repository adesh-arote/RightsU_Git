CREATE Proc [dbo].[USP_Insert_Acq_Deal_Pushback_Dubbing](@Acq_Deal_Pushback_Code Int, @Language_Type Char(1), @Language_Code Int, @Language_Group_Code Int)
As
Begin
	Declare @Loglevel int
	select @Loglevel = Parameter_Value from System_Parameter_New where Parameter_Name='loglevel'
	if(@Loglevel < 2)Exec [USPLogSQLSteps] '[USP_Insert_Acq_Deal_Pushback_Dubbing]', 'Step 1', 0, 'Started Procedure', 0, ''
		If(@Language_Type = 'G')
		Begin
			...
			Insert InTo Acq_Deal_Pushback_Dubbing(Acq_Deal_Pushback_Code, Language_Type, Language_Code, Language_Group_Code)
		...
		SELECT SCOPE_IDENTITY() AS Acq_Deal_Pushback_Dubbing_Code
	 
	if(@Loglevel < 2)Exec [USPLogSQLSteps] '[USP_Insert_Acq_Deal_Pushback_Dubbing]', 'Step 2', 0, 'Procedure Excution Completed', 0, ''
End
