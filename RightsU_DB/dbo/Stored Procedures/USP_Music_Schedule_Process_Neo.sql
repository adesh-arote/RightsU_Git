...
BEGIN
	...
	if(@Loglevel<2)Exec [USPLogSQLSteps] '[USP_Music_Schedule_Process_Neo]', 'Step 1', 0, 'Started Procedure', 0, '' 
		SET NOCOUNT ON
		...
		BEGIN
			...
			INNER JOIN Music_Schedule_Exception MSE WITH(NOLOCK) ON MST.Music_Schedule_Transaction_Code = MSE.Music_Schedule_Transaction_Code
		...
		BEGIN
			...
			BEGIN
				...
				FROM Title_Content TC WITH(NOLOCK)
				INNER JOIN Content_Music_Link CML WITH(NOLOCK) ON CML.Title_Content_Code = TC.Title_Content_Code
				INNER JOIN Title_Content_Version TCV WITH(NOLOCK) ON TCV.Title_Content_Version_Code = CML.Title_Content_Version_Code AND TCV.Version_Code = @DefaultVersionCode
				INNER JOIN #TempScheduleData TSD ON TSD.Title_Code = TC.Title_Code AND TSD.Episode_No =  TC.Episode_No
				INNER JOIN Music_Title_Label MTL WITH(NOLOCK) ON MTL.Music_Title_Code = CML.Music_Title_Code 
				AND TSD.Schedule_Date BETWEEN MTL.Effective_From AND ISNULL(MTL.Effective_To, TSD.Schedule_Date)
				LEFT JOIN Music_Schedule_Transaction MST WITH(NOLOCK) ON CML.Content_Music_Link_Code = MST.Content_Music_Link_Code AND TSD.BV_Schedule_Transaction_Code = MST.BV_Schedule_Transaction_Code
			...
			*/
			...
			INNER JOIN Title t WITH(NOLOCK) ON t.Title_Code = TMST.Title_Code 
			INNER JOIN Music_Deal MD WITH(NOLOCK) ON MD.Music_Label_Code = TMST.Music_Label_Code
			INNER JOIN Music_Deal_Channel MDC WITH(NOLOCK) ON MDC.Music_Deal_Code = MD.Music_Deal_Code
			LEFT JOIN Music_Deal_LinkShow MDLS WITH(NOLOCK) ON MDLS.Music_Deal_Code = MD.Music_Deal_Code 
			...
			BEGIN
				...
				BEGIN --------------- RIGHT RULE PROCESSING
					...
					FROM (
						...
						INNER JOIN Right_Rule rr WITH(NOLOCK) ON TMST.RightRuleCode = rr.Right_Rule_Code
				...
				FROM (
					...
					FROM Music_Deal_Channel WITH(NOLOCK) WHERE Music_Deal_Code IN (
				...
				FROM (
					...
					FROM Music_Deal_Channel mdc WITH(NOLOCK)
				...
				SELECT NRE.MusicScheduleTransactionCode, ECM.Error_Code, 'E', 'N' FROM Error_Code_Master ECM  WITH(NOLOCK)
				INNER JOIN (
	...
	if(@Loglevel<2)Exec [USPLogSQLSteps] '[USP_Music_Schedule_Process_Neo]', 'Step 2', 0, 'Procedure Excution Completed', 0, '' 
END
