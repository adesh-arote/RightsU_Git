...
BEGIN 
	...
	if(@Loglevel< 2)Exec [USPLogSQLSteps] '[USP_Syn_Deal_Clone_UDT]', 'Step 1', 0, 'Started Procedure', 0, '' 
	   ...
	   --   VALUES(2674, 27766, 1, 1)--, (2026, 902, 1, 1) 
		  ...
		  BEGIN TRY 
			  ...
			  --WHERE ADM.Acq_Deal_Code = @Previous_Acq_Deal_Code AND CADM.Acq_Deal_Code = @New_Acq_Deal_Code 
				...
				SELECT Syn_deal_rights_code  FROM   Syn_Deal_Rights (NOLOCK) WHERE  Syn_Deal_Code = @Previous_Syn_Deal_Code 
				...
				BEGIN 
					IF( (SELECT Count(*) FROM   Syn_Deal_Rights_Title SDRT (NOLOCK) INNER JOIN #temptitle DRT  ON SDRT.Title_Code = DRT.Title_Code 
					  ...
					  BEGIN 
						 ...
						 PRINT'/**************** Insert into AT_Syn_Deal_Rights_Title_EPS ****************/'
						  ...
						  FROM   Syn_Deal_Rights_Title_EPS ADRTE (NOLOCK)
								 INNER JOIN Syn_Deal_Rights_Title ADRT (NOLOCK)
										 ON ADRTE.Syn_Deal_Rights_Title_Code =  ADRT.Syn_Deal_Rights_Title_Code 
								 INNER JOIN #temptitle DRT 
										 ON ADRT.Title_Code = DRT.Title_Code  AND ADRT.Episode_From =  DRT.Old_Episode_From  AND ADRT.Episode_To = DRT.Old_Episode_To 
								 INNER JOIN Syn_Deal_Rights_Title AtADRT (NOLOCK)
						 ...
						 PRINT'/******** Insert into AT_Syn_Deal_Rights_Holdback_Territory ********/'
						  ...
						  FROM   Syn_Deal_Rights_Blackout_Dubbing ADRBD (NOLOCK)
								 INNER JOIN Syn_Deal_Rights_Blackout ADRB (NOLOCK)
										 ON ADRBD.Syn_Deal_Rights_Blackout_Code = 
											ADRB.Syn_Deal_Rights_Blackout_Code 
								 INNER JOIN Syn_Deal_Rights_Blackout AtADRB (NOLOCK)
				...
				SELECT Syn_Deal_Run_Code FROM Syn_Deal_Run (NOLOCK) WHERE  Syn_Deal_Code = @Previous_Syn_Deal_Code 
				...
				ADRRD.Day_Code 
				FROM   Syn_Deal_Run_Repeat_On_Day ADRRD (NOLOCK)
				...
				SELECT @New_Syn_Deal_Run_Code, 
				ADRRD.Platform_Code 
				FROM   Syn_Deal_Run_Platform ADRRD (NOLOCK)
				WHERE  ADRRD.Syn_Deal_Run_Code = @Syn_Deal_Run_Code 
				...
				ADRYR.No_Of_Runs, 
				ADRYR.Year_No 
				FROM   Syn_Deal_Run_Yearwise_Run ADRYR (NOLOCK)
				...
				*/
				...
				SELECT Syn_Deal_Revenue_Code FROM Syn_Deal_Revenue (NOLOCK) WHERE  Syn_Deal_Code = @Previous_Syn_Deal_Code 
				...
				WHILE @@FETCH_STATUS = 0 
				BEGIN 
					IF( (SELECT Count(*) FROM   Syn_Deal_Revenue_Title ADRT (NOLOCK) INNER JOIN #temptitle tt ON ADRT.Title_Code = tt.Title_Code 
					...
					BEGIN 
						...
						SELECT @New_Syn_Deal_Revenue_Code
							...
							,GETDATE()
							,@User_Code
						FROM Syn_Deal_Revenue_Additional_Exp SDRAE (NOLOCK)
						...
						SELECT @New_Syn_Deal_Revenue_Code
							...
							,Percentage
							,Amount
							FROM Syn_Deal_Revenue_Commission SDRC (NOLOCK)
						...
						SELECT @New_Syn_Deal_Revenue_Code
							...
							,GETDATE()
							,@User_Code
						FROM Syn_Deal_Revenue_Costtype SDRP (NOLOCK)
						...
						SELECT @New_Syn_Deal_Revenue_Code, Platform_Code FROM Syn_Deal_Revenue_Platform SDRP (NOLOCK)
						WHERE  SDRP.Syn_Deal_Revenue_Code = @Syn_Deal_Revenue_Code
						...
						SELECT @New_Syn_Deal_Revenue_Code
							...
							,Episode_From
							,Episode_To
						FROM Syn_Deal_Revenue_Title ADRT (NOLOCK)
				...
				tt.Episode_End_To 
				FROM   Syn_Deal_Material ADM (NOLOCK)
				INNER JOIN #temptitle tt 
	...
	if(@Loglevel< 2)Exec [USPLogSQLSteps] '[USP_Syn_Deal_Clone_UDT]', 'Step 2', 0, 'Procedure Excuting Completed', 0, '' 
END