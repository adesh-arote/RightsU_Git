...
BEGIN
	...
	if(@Loglevel<2)Exec [USPLogSQLSteps] '[USP_Music_Exception_Report]', 'Step 1', 0, 'Started Procedure', 0, '' 
		...
		CASE WHEN ISNULL(MT.Movie_Album_Type,'A')='A' THEN MA.Music_Album_Name ELSE TA.Title_Name END AS Movie_Album,
		...
		REVERSE(STUFF(REVERSE(STUFF((
			...
			from Music_Schedule_Exception MTT1(NOLOCK)
			INNER JOIN Error_Code_Master TA (NOLOCK) ON MTT1.Error_Code=TA.Error_Code
		...
		INNER JOIN Content_Music_Link AS CML (NOLOCK) ON CML.Content_Music_Link_Code=MST.Content_Music_Link_Code
		INNER JOIn Title_Content TC (NOLOCK) On TC.Title_Content_Code = CML.Title_Content_Code
		INNER JOIn Title_Content_Version TCV (NOLOCK) On TCV.Title_Content_Code = TC.Title_Content_Code AND TCV.Title_Content_Version_Code = CML.Title_Content_Version_Code
		...
		INNER JOIN Title AS T (NOLOCK) ON T.Title_Code = TC.Title_Code
		INNER JOIN Music_Title AS MT (NOLOCK) ON MT.Music_Title_Code = CML.Music_Title_Code
		LEFT JOIN Music_Album AS MA (NOLOCK) ON MT.Music_Album_Code = MA.Music_Album_Code
		LEFT JOIN Title AS TA (NOLOCK) ON MT.Title_Code = TA.Title_Code
		INNER JOIN Music_Label AS ML (NOLOCK) ON ML.Music_Label_Code = MST.Music_Label_Code
		INNER JOIN Channel AS C (NOLOCK) ON C.Channel_Code = MST.Channel_Code
		LEFT JOIN Program P (NOLOCK) ON T.Program_Code = P.Program_Code
		LEFT JOIN Music_Override_Reason AS MOR (NOLOCK) ON MST.Music_Override_Reason_Code = MOR.Music_Override_Reason_Code
	...
	if(@Loglevel<2)Exec [USPLogSQLSteps] '[USP_Music_Exception_Report]', 'Step 2', 0, 'Procedure Excution Completed', 0, '' 
END