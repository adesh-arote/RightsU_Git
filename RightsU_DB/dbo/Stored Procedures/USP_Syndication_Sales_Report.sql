...
BEGIN
	...
	if(@Loglevel < 2)Exec [USPLogSQLSteps] '[USP_Syndication_Sales_Report]', 'Step 1', 0, 'Started Procedure', 0, ''
		...
		BEGIN
				...
				SELECT Territory_Code FROM Territory (NOLOCK) WHERE ISNULL(Is_Active, 'Y') ='Y' AND ISNULL(Is_Thetrical, 'N') = @IncludeDomestic
		
		END
		IF NOT EXISTS (SELECT * FROM #Temp_Countrys) AND @Country_Codes != '-1'
		BEGIN
			INSERT INTO #Temp_Countrys
			SELECT Country_Code FROM Country (NOLOCK) WHERE ISNULL(Is_Theatrical_Territory, 'N') = @IncludeDomestic AND ISNULL(Is_Active, 'Y') ='Y'
		END
		...
		INNER JOIN Syn_Deal_Movie sdm (NOLOCK) ON sd.Syn_Deal_Code = sdm.Syn_Deal_Code AND sd.Business_Unit_Code = @Business_Unit_Code 
		...
		INNER JOIN Syn_Deal_Rights_Title sdrt (NOLOCK) ON sdr.Syn_Deal_Rights_Code = sdrt.Syn_Deal_Rights_Code AND sdm.Title_Code = sdrt.Title_Code
		INNER JOIN Title t (NOLOCK) ON sdrt.Title_Code = t.Title_Code
		INNER JOIN Platform p (NOLOCK) ON p.Platform_Code = sdrp.Platform_Code
		...
		LEFT JOIN 
		(SELECT convert(decimal(18, 2), SUM(ISNULL(sdRev.DEAL_COST, 0))) Revenue, sdRev.Syn_Deal_Code  FROM Syn_Deal_Revenue sdRev (NOLOCK) GROUP BY sdRev.Syn_Deal_Code) sd_Revenue
		...
		WHERE sdr.Right_Status = 'C' AND ISNULL(sd.Agreement_Date, '9999-12-31') >= @Start_Date
			  ...
			  AND (
					(sdr.Syn_Deal_Rights_Code IN 
						(SELECT sdrt.Syn_Deal_Rights_Code FROM Syn_Deal_Rights_Territory sdrt  (NOLOCK)
						...
						OR (sdrt.Territory_Code IN (SELECT tmpTT.Territory_Code FROM #Temp_Territorys tmpTT INNER JOIN Territory tmpT (NOLOCK) on 
	...
	if(@Loglevel < 2)Exec [USPLogSQLSteps] '[USP_Syndication_Sales_Report]', 'Step 2', 0, 'Procedure Excuting Completed', 0, ''
END