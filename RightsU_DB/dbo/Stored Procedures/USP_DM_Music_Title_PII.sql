...
BEGIN  
	...
	SELECT distinct [Name], [Key], [value], [DM_Master_Type], CASE WHEN  ISNULL(RTRIM(LTRIM([value])),'') = 'NEW' THEN 'Y' ELSE 'N' END AS [Is_New]
	...
	BEGIN  
		...
		TD.Master_Name COLLATE SQL_Latin1_General_CP1_CI_AS NOT IN(SELECT Genres_Name  COLLATE SQL_Latin1_General_CP1_CI_AS FROM Genres)
		...
		INNER JOIN DM_Music_Title DMT ON DMT.Movie_Album collate SQL_Latin1_General_CP1_CI_AS = TD.Master_Name collate SQL_Latin1_General_CP1_CI_AS AND DMT.DM_Master_Import_Code = @DM_Master_Import_Code
		WHERE TD.Master_Type = 'MA' AND Is_New = 'Y' AND
		TD.Master_Name collate SQL_Latin1_General_CP1_CI_AS NOT IN (SELECT Music_Album_Name collate SQL_Latin1_General_CP1_CI_AS FROM Music_Album)
		...
		AND TD.Master_Type  = 'MA'
    
		INSERT INTO Music_Language(Language_Name, Is_Active,Inserted_By, Inserted_On)  
		SELECT DISTINCT TD.Master_Name, 'Y', @Users_Code, @CurDate FROM #Temp_DM TD  
		WHERE TD.Master_Type = 'ML' AND Is_New = 'Y' AND
		TD.Master_Name collate SQL_Latin1_General_CP1_CI_AS NOT IN(SELECT Language_Name collate SQL_Latin1_General_CP1_CI_AS FROM Music_Language)
		...
		TD.Master_Name collate SQL_Latin1_General_CP1_CI_AS NOT IN (select Music_Theme_Name collate SQL_Latin1_General_CP1_CI_AS FROM Music_Theme)
	...
	BEGIN  
		...
		INNER JOIN DM_Master_Log DM ON T.DM_Master_Log_Code = DM.DM_Master_Log_Code AND DM.Roles Like '%Singers%' AND DM.Master_Type = 'TA'  
		UNION  
		SELECT DM_Master_Code, @LyricistRoleCode As Role_Code, [Master_Name] FROM #Temp_DM  T  
		INNER JOIN DM_Master_Log DM ON T.DM_Master_Log_Code = DM.DM_Master_Log_Code AND DM.Roles like '%Lyricist%' AND DM.Master_Type = 'TA'  
		UNION  
		SELECT DM_Master_Code, @MusicComposerRoleCode As Role_Code, [Master_Name] FROM #Temp_DM  T  
		INNER JOIN DM_Master_Log DM ON T.DM_Master_Log_Code = DM.DM_Master_Log_Code AND DM.Roles like '%Composer%' AND DM.Master_Type = 'TA'  
		UNION  
	...
	BEGIN
		DECLARE CUR_DM_Music_Title_Ignore CURSOR For SELECT Master_Name from #Temp_DM
	...
	SELECT @ResolveCount = COUNT(*) FROM DM_Music_Title WHERE Record_Status = 'N' AND DM_Master_Import_Code = @DM_Master_Import_Code
	SELECT @TotalCount = COUNT(*) FROM DM_Music_Title WHERE DM_Master_Import_Code = @DM_Master_Import_Code
