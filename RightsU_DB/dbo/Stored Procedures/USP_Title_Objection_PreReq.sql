CREATE PROCEDURE [dbo].[USP_Title_Objection_PreReq]
(
	@TitleCode INT = 36672,
...
BEGIN
	 ...
	 if(@Loglevel < 2)Exec [USPLogSQLSteps] '[USP_Title_Objection_PreReq]', 'Step 1', 0, 'Started Procedure', 0, ''   
			...
			BEGIN
				...
				BEGIN
					...
					BEGIN
						...
						IF((SELECT TOP 1 Territory_Type FROM Acq_Deal_Rights_Territory (NOLOCK) where Acq_Deal_Rights_Code = @Acq_Deal_Rights_Code) = 'G')
						BEGIN
							INSERT INTO @TABLE(CountryCode, Deal_Rights_Code)
							SELECT DISTINCT Country_Code, @Acq_Deal_Rights_Code FROM Territory_Details (NOLOCK) WHERE Territory_Code IN 
							(SELECT Territory_Code FROM Acq_Deal_Rights_Territory (NOLOCK) where Acq_Deal_Rights_Code = @Acq_Deal_Rights_Code)

							INSERT INTO @Tbl_Territory (TerritoryCode, Deal_Rights_Code)
							SELECT Territory_Code, @Acq_Deal_Rights_Code FROM Acq_Deal_Rights_Territory (NOLOCK) where Acq_Deal_Rights_Code = @Acq_Deal_Rights_Code
						END
						ELSE
						BEGIN
							INSERT INTO @TABLE(CountryCode, Deal_Rights_Code)
							SELECT Country_Code, @Acq_Deal_Rights_Code FROM Acq_Deal_Rights_Territory (NOLOCK) where Acq_Deal_Rights_Code = @Acq_Deal_Rights_Code
						END
					...
					BEGIN
						...
						SELECT DISTINCT Territory_Code, Country_Code FROM Territory_Details (NOLOCK) WHERE Territory_Code = @TerritoryCode
			...
			BEGIN
				...
				BEGIN
					...
					FROM Syn_Deal_Rights ADR (NOLOCK)
						 INNER JOIN Syn_Deal_Rights_Title ADM  (NOLOCK) ON ADM.Syn_Deal_Rights_Code = ADR.Syn_Deal_Rights_Code
					...
					BEGIN
						...
						IF((SELECT TOP 1 Territory_Type FROM Syn_Deal_Rights_Territory  (NOLOCK) where Syn_Deal_Rights_Code = @Acq_Deal_Rights_Code) = 'G')
						BEGIN
							INSERT INTO @TABLE(CountryCode, Deal_Rights_Code)
							SELECT DISTINCT Country_Code, @Acq_Deal_Rights_Code FROM Territory_Details (NOLOCK) WHERE Territory_Code IN 
							(SELECT Territory_Code FROM Syn_Deal_Rights_Territory (NOLOCK) where Syn_Deal_Rights_Code = @Acq_Deal_Rights_Code)
						END
						ELSE
						BEGIN
							INSERT INTO @TABLE(CountryCode, Deal_Rights_Code)
							SELECT Country_Code, @Acq_Deal_Rights_Code FROM Syn_Deal_Rights_Territory (NOLOCK) WHERE Syn_Deal_Rights_Code = @Acq_Deal_Rights_Code
						END
						SET @Counter  = @Counter  + 1
					END
			...
			FROM Title_Objection_Type A (NOLOCK)
			INNER JOIN Title_Objection_Type B (NOLOCK) ON A.Objection_Type_Code = B.Parent_Objection_Type_Code
	 ...
	 if(@Loglevel < 2)Exec [USPLogSQLSteps] '[USP_Title_Objection_PreReq]', 'Step 2', 0, 'Procedure Excuting Completed', 0, ''   
END
