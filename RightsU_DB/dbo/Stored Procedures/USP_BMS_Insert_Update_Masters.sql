...
BEGIN
	...
	if(@Loglevel < 2)Exec [USPLogSQLSteps] '[USP_BMS_Insert_Update_Masters]', 'Step 1', 0, 'Started Procedure', 0, ''
		...
		BEGIN 	
			...
			SELECT DISTINCT   COL2,COL3,COL4 FROM  BMS_Masters_Import (NOLOCK) WHERE UPPER(COL1) = 'CHANNEL'
		...
		BEGIN 
			...
			SELECT DISTINCT   COL2,COL3 FROM  BMS_Masters_Import (NOLOCK) WHERE UPPER(COL1) = 'LANGUAGE'		
		...
		BEGIN 
			INSERT INTO #BMS_XML (BMS_Key,BMS_Desc)		
			SELECT DISTINCT   COL2,COL3 FROM    BMS_Masters_Import (NOLOCK) WHERE UPPER(COL1) = 'COUNTRY'		
		...
		PRINT 'IN GENRE'
			INSERT INTO #BMS_XML (BMS_Key,BMS_Desc)		
			SELECT DISTINCT   COL2,COL3 FROM    BMS_Masters_Import (NOLOCK) WHERE UPPER(COL1) = 'GENRE'		
		...
		BEGIN 
		INSERT INTO #BMS_XML (BMS_Key,BMS_Desc)		
			SELECT DISTINCT   COL2,COL3 FROM    BMS_Masters_Import (NOLOCK) WHERE UPPER(COL1) = 'CURRENCY'		
		...
		BEGIN 
		INSERT INTO #BMS_XML (BMS_Key,BMS_Desc)		
			SELECT DISTINCT   COL2,COL3  FROM  BMS_Masters_Import (NOLOCK) WHERE UPPER(COL1) = 'CATEGORY'		
		...
		BEGIN 
		INSERT INTO #BMS_XML (BMS_Key,BMS_Desc)		
		SELECT DISTINCT   COL2,COL3 FROM BMS_Masters_Import (NOLOCK) WHERE UPPER(COL1) = 'ENTITY'		
		...
		BEGIN 
			INSERT INTO #BMS_XML (BMS_Key,BMS_Desc)		
			SELECT DISTINCT   COL2,COL3 FROM BMS_Masters_Import (NOLOCK) WHERE UPPER(COL1) = 'RIGHT_RULE'		
		-------------------------Insert Or Update Logic For ENTITY or Licensee Master------
		 MERGE Right_Rule C 
		  USING  #BMS_XML BV
		...
		BEGIN 
			...
			SELECT DISTINCT   COL2,COL3,COL4,		
			 CASE 
						...
						THEN 'Y'
						ELSE 'N' END AS ISArc
			,COL6 FROM BMS_Masters_Import (NOLOCK) WHERE UPPER(COL1) = 'BMS_MEDIACATEGORY'				
		...
		BEGIN 
			...
			SELECT DISTINCT   COL2,COL3,COL4,		
			 CASE 
						...
						THEN 'Y'
						ELSE 'N' END AS ISArc
			FROM BMS_Masters_Import (NOLOCK) WHERE UPPER(COL1) = 'BMS_MEDIAFRAMERATES'		
		-------------------------Insert Or Update Logic For BV MEDIA CATEGORY Master------
		...
		BEGIN 
			...
			SELECT DISTINCT   COL2,COL3,COL4,		
			 CASE 
						...
						THEN 'Y'
						ELSE 'N' END AS ISArc
			,COL6 FROM BMS_Masters_Import (NOLOCK) WHERE UPPER(COL1) = 'BMS_PROGRAMCATEGORY'		
		-------------------------Insert Or Update Logic For BV MEDIA CATEGORY Master------
		...
		BEGIN 
			...
			SELECT DISTINCT   COL2,COL3,		
			 CASE 
						...
						THEN 'Y'
						ELSE 'N' END AS ISArc
			,COL5 FROM BMS_Masters_Import (NOLOCK) WHERE UPPER(COL1) = 'BMS_STATION'		
		-------------------------Insert Or Update Logic For BV MEDIA CATEGORY Master------
		 MERGE BMS_Station C 
		  USING  #BMS_XML BV
			  ON BV.BMS_Key = C.BMS_Key
		...
		COL3 FROM BMS_Masters_Import  (NOLOCK)
		...
		BEGIN 	
			...
			INSERT INTO #BMS_XML (BMS_Key,BMS_Desc)		
			SELECT DISTINCT   COL2,COL3
			FROM BMS_Masters_Import (NOLOCK) WHERE UPPER(COL1) = 'BMS_SYSLOOKUPCLASS'				
		...
		BEGIN 
			...
			SELECT DISTINCT   COL2,COL3,		
			 CASE 
						...
						THEN 'Y'
						ELSE 'N' END AS ISArc
			,COL5 FROM BMS_Masters_Import (NOLOCK) WHERE UPPER(COL1) = 'BMS_TRAFFICCATEGORY'		
		...
		BEGIN 
			...
			,COL6 FROM BMS_Masters_Import (NOLOCK) WHERE UPPER(COL1) = 'BMS_VERSIONTYPE'		
		-------------------------Insert Or Update Logic For BV MEDIA CATEGORY Master------
		...
		BEGIN 		
			...
			SELECT DISTINCT   COL2,COL6,COL3,COL4,COL5,		
			 CASE 
						...
						THEN 'Y'
						ELSE 'N' END AS IsArchived
			,COL8 FROM BMS_Masters_Import (NOLOCK) WHERE UPPER(COL1) = 'BMS_MEDIA_TYPE'		
		-------------------------Insert Or Update Logic For BV MEDIA CATEGORY Master------
		 ...
		 ,IsArchived,ForeignId)
			SELECT   COL2,COL3
				...
				,CASE 
							...
							ELSE 'N' END AS IsArchived
			,COL14		
			FROM BMS_Masters_Import (NOLOCK) WHERE UPPER(COL1) = 'BMS_PAYEE'		
		-------------------------Insert Or Update Logic For ENTITY or Licensee Master------
		 MERGE BMS_Payee C 
	...
	if(@Loglevel < 2)Exec [USPLogSQLSteps] '[USP_BMS_Insert_Update_Masters]', 'Step 2', 0, 'Procedure Excution Completed', 0, ''
END
