CREATE PROC [dbo].[USP_List_Provisional_Deal]
(
...
BEGIN	
	...
	if(@Loglevel < 2)Exec [USPLogSQLSteps] '[USP_List_Provisional_Deal]', 'Step 1', 0, 'Started Procedure', 0, '' 
		SET FMTONLY OFF
		--DECLARE
		--@SearchText NVARCHAR(MAX) = '',
		--@Agreement_No VARCHAR(50) = '',
		...
		BEGIN
			...
			SELECT A.Provisional_Deal_Code FROM (
				SELECT DISTINCT PD.Provisional_Deal_Code, PD.Last_Updated_Time FROM Provisional_Deal PD  (NOLOCK)
				INNER JOIN Provisional_Deal_Licensor PDL (NOLOCK) ON PDL.Provisional_Deal_Code = PD.Provisional_Deal_Code
				INNER JOIN Users_Business_Unit UBU (NOLOCK) ON UBU.Business_Unit_Code = PD.Business_Unit_Code AND UBU.Users_Code = @User_Code
				INNER JOIN Provisional_Deal_Title PDT  (NOLOCK) On PD.Provisional_Deal_Code = PDT.Provisional_Deal_Code
		...
		SELECT
			...
			(
				...
				From Provisional_Deal PD2 (NOLOCK)
				INNER JOIN #TempProvisionalDeal TPD2 ON TPD.Provisional_Deal_Code = PD2.Provisional_Deal_Code
				INNER JOIN Provisional_Deal_Title PDT2  (NOLOCK) On PD2.Provisional_Deal_Code = PDT2.Provisional_Deal_Code
				INNER JOIN Title T2 (NOLOCK) On PDT2.Title_Code = T2.Title_Code
				INNER JOIN [Business_Unit] BU2 (NOLOCK) ON BU2.[Business_Unit_Code] = PD2.[Business_Unit_Code]
				INNER JOIN [Deal_Type] DT2 (NOLOCK) ON DT2.[Deal_Type_Code] = PD2.[Deal_Type_Code]
			...
			(
				...
				From Provisional_Deal PD1 (NOLOCK)
				INNER JOIN #TempProvisionalDeal TPD1 ON TPD.Provisional_Deal_Code = PD1.Provisional_Deal_Code
				INNER JOIN Provisional_Deal_Licensor PDL1 (NOLOCK) ON PDL1.Provisional_Deal_Code = PD1.Provisional_Deal_Code
				INNER JOIN [Vendor] V1 (NOLOCK) ON V1.[Vendor_Code] = PDL1.Vendor_Code
				INNER JOIN [Business_Unit] BU1 (NOLOCK) ON BU1.[Business_Unit_Code] = PD1.[Business_Unit_Code]
				INNER JOIN [Deal_Type] DT1  (NOLOCK) ON DT1.[Deal_Type_Code] = PD1.[Deal_Type_Code]
			...
			INTO #TempResultData
		FROM 
			Provisional_Deal PD  (NOLOCK)
			INNER JOIN #TempProvisionalDeal TPD ON TPD.Provisional_Deal_Code = PD.Provisional_Deal_Code
			INNER JOIN [Business_Unit] BU (NOLOCK) ON BU.[Business_Unit_Code] = PD.[Business_Unit_Code]
			INNER JOIN [Deal_Type] DT (NOLOCK) ON DT.[Deal_Type_Code] = PD.[Deal_Type_Code]
	...
	if(@Loglevel < 2)Exec [USPLogSQLSteps] '[USP_List_Provisional_Deal]', 'Step 2', 0, 'Procedure Excution Completed', 0, '' 
END