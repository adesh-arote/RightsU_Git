CREATE PROCEDURE  [dbo].[USPMHNotificationMail]
@MHRequestCode INT,
...
BEGIN
	...
	if(@Loglevel< 2)Exec [USPLogSQLSteps] '[USPMHNotificationMail]', 'Step 1', 0, 'Started Procedure', 0, ''
	-----------------------------------
	...
	---------------------------------------------
		...
		DECLARE @MailSubjectCr AS NVARCHAR(MAX),
				...
				SET @RequestID = (Select RequestID from MHRequest (NOLOCK) Where MHRequestCode = @MHRequestCode)
				...
				If(@MHRequestTypeCode = 1)
					BEGIN
						...
						SELECT @Email_Config_Code=Email_Config_Code FROM Email_Config (NOLOCK) WHERE [Key]='MHCR'
				...
				ELSE IF(@MHRequestTypeCode = 2)
					BEGIN
						...
						SET @EmailConfigCode = (Select Email_Config_Code from Email_Config (NOLOCK) where Email_Type = 'MHMusicRequest' )
						SET @Subject = ''+@RequestID+' - New Music Track Request is Sent for approval';
					
						SELECT @Email_Config_Code=Email_Config_Code FROM Email_Config (NOLOCK) WHERE [Key]='MHMUR'
				...
				Else
					BEGIN
						...
						SET @EmailConfigCode = (Select Email_Config_Code from Email_Config (NOLOCK) where Email_Type = 'MHMovieRequest' )
						SET @Subject = ''+@RequestID+' - New Movie Request is Sent for approval';
					
						SELECT @Email_Config_Code=Email_Config_Code FROM Email_Config (NOLOCK) WHERE [Key]='MHMOR'
				...
				IF(@MHRequestTypeCode = 1)
					BEGIN
						...
						INNER JOIN MHRequestDetails MRD  (NOLOCK) ON MRD.MHRequestCode = MR.MHRequestCode
						INNER JOIN Users U (NOLOCK) ON U.Users_Code = MR.UsersCode
						INNER JOIN MHUsers MU (NOLOCK) ON MU.Users_Code = MR.UsersCode
		...
		--		INSERT INTO MHNotificationLog VALUES(@EmailConfigCode,GETDATE(),'N',@Emailbody,@UserCode,@VendorCode,@Subject,@ToMail, @MHRequestCode, @MHRequestTypeCode)
		--		--Select Top 10 * from Email_Notification_Log order by 1 desc
		--		Select 'Y' AS Result
		--	FETCH NEXT FROM curOuter INTO @UserCode, @ToMail
		--END
		--Close curOuter
	...
	if(@Loglevel< 2)Exec [USPLogSQLSteps] '[USPMHNotificationMail]', 'Step 2', 0, 'Procedure Excution Completed', 0, ''
END
