CREATE PROC [dbo].[USP_GetContentsRightData]
(
...
BEGIN
	...
	if(@Loglevel < 2)Exec [USPLogSQLSteps] '[USP_GetContentsRightData]', 'Step 1', 0, 'Started Procedure', 0, ''
		...
		SELECT Platform_Code INTO #PlatformForRun FROM [Platform] (NOLOCK) WHERE ISNULL(Is_No_Of_Run, 'N') = 'Y'
		...
		INNER JOIN Acq_Deal_Run_Title ADRT (NOLOCK) ON ADRT.Title_Code = TC.Title_Code AND TC.Episode_No BETWEEN ADRT.Episode_From AND ADRT.Episode_To
		...
		STUFF((
			SELECT  DISTINCT ',' + C.Channel_Name
			FROM Provisional_Deal_Run_Channel PDRC (NOLOCK)
			INNER JOIN Channel C (NOLOCK) ON C.Channel_Code = PDRC.Channel_Code	
			WHERE  PDRC.Provisional_Deal_Run_Code = PDR.Provisional_Deal_Run_Code
			FOR XML PATH('')
		), 1, 1, '') AS Channel_Names,
		'P' AS Deal_Type
		FROM Title_Content TC (NOLOCK)
		INNER JOIN Provisional_Deal_Title PDT (NOLOCK) ON PDT.Title_Code = TC.Title_Code AND TC.Episode_No BETWEEN PDT.Episode_From AND PDT.Episode_To
		INNER JOIN Provisional_Deal_Run PDR (NOLOCK) ON PDR.Provisional_Deal_Title_Code = PDT.Provisional_Deal_Title_Code
		WHERE TC.Title_Content_Code = @Title_Content_Code
		) B
		...
		BEGIN
			...
			SELECT DISTINCT adm.Acq_Deal_Code, 'A' FROM Title_Content_Mapping tcm (NOLOCK)
			INNER JOIN Acq_Deal_Movie adm (NOLOCK) ON tcm.Acq_Deal_Movie_Code = adm.Acq_Deal_Movie_Code
		...
		INNER JOIN Acq_Deal_Movie ADM (NOLOCK) ON ADM.Acq_Deal_Movie_Code = TCM.Acq_Deal_Movie_Code 
		INNER JOIN Acq_Deal AD (NOLOCK) ON AD.Acq_Deal_Code = ADM.Acq_Deal_Code
		LEFT JOIN Acq_Deal_Run ADR (NOLOCK) ON ADR.Acq_Deal_Code = AD.Acq_Deal_Code
		...
		(
			...
			Acq_Deal_Rights_Title ADRC (NOLOCK) 
			INNER JOIN Acq_Deal_Rights ADR (NOLOCK) ON ADR.Acq_Deal_Rights_Code = ADRC.Acq_Deal_Rights_Code 
		...
		INNER JOIN Title_Content_Mapping TCM (NOLOCK) ON TCM.Title_Content_Code = TC.Title_Content_Code
		INNER JOIN Provisional_Deal_Title PDT (NOLOCK) ON PDT.Provisional_Deal_Title_Code = TCM.Provisional_Deal_Title_Code 
		INNER JOIN Provisional_Deal PD (NOLOCK) ON PD.Provisional_Deal_Code = PDT.Provisional_Deal_Code
		INNER JOIN Provisional_Deal_Licensor PDL (NOLOCK) ON PD.Provisional_Deal_Code = PDL.Provisional_Deal_Code
		INNER JOIN Provisional_Deal_Run PDR (NOLOCK) ON PDR.Provisional_Deal_Title_Code = PDT.Provisional_Deal_Title_Code
		INNER JOIN Provisional_Deal_Run_Channel PDRC (NOLOCK) ON PDRC.Provisional_Deal_Run_Code = PDR.Provisional_Deal_Run_Code
		INNER JOIN Vendor V (NOLOCK) ON V.Vendor_Code = PDL.Vendor_Code
	...
	if(@Loglevel < 2)Exec [USPLogSQLSteps] '[USP_GetContentsRightData]', 'Step 2', 0, 'Procedure Excution Completed', 0, ''
END