CREATE PROC [dbo].[Avail_GetTitlesToProcessListNeo]
...
BEGIN  
	 ...
	 if(@Loglevel < 2)Exec [USPLogSQLSteps] '[Avail_GetTitlesToProcessListNeo]', 'Step 1', 0, 'Started Procedure', 0, ''
		...
		CREATE TABLE #DealType (    
		  ...
		  SELECT @AvailsType = Parameter_Value 
			...
			DROP Table #DealType
	if(@Loglevel < 2)Exec [USPLogSQLSteps] '[Avail_GetTitlesToProcessListNeo]', 'Step 2', 0, 'Procedure Excution Completed', 0, ''
END