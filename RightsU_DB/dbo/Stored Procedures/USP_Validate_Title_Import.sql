...
BEGIN  
	...
	if(@Loglevel< 2)Exec [USPLogSQLSteps] '[USP_Validate_Title_Import]', 'Step 1', 0, 'Started Procedure', 0, ''
	  SET NOCOUNT ON;    
	  ...
	  IF EXISTS (SELECT [Excel_Line_No] FROM  DM_Title (NOLOCK) WHERE ISNULL(RTRIM(LTRIM([Excel_Line_No])),'') = '')
	  BEGIN  
	  ...
	  IF EXISTS (SELECT [Original Title (Tanil/Telugu)] FROM  DM_Title (NOLOCK) WHERE ISNULL(RTRIM(LTRIM([Original Title (Tanil/Telugu)])),'') = '')  
	  BEGIN  
	  ...
	  IF EXISTS (SELECT [Title Type] FROM DM_Title (NOLOCK) WHERE ISNULL(RTRIM(LTRIM([Title Type])),'') = '')  
	  BEGIN  
	  ...
	  IF EXISTS (SELECT [Original Language (Hindi)] FROM DM_Title (NOLOCK) WHERE ISNULL(RTRIM(LTRIM([Original Language (Hindi)])),'') = '')  
	  BEGIN  
	  ...
	  IF EXISTS (SELECT  [Music_Label] FROM  DM_Title (NOLOCK) WHERE DM_Master_Import_Code = @DM_Master_Import_Code and ISNULL(RTRIM(LTRIM([Music_Label])),'') = '' AND [Title Type]=@TitleType)   
	  BEGIN  
	  ...
	  (  
	   SELECT [Original Title (Tanil/Telugu)] FROM DM_Title  (NOLOCK) 
	...
	UPDATE DM_Title SET Record_Status = 'E', [Error_Message] = ISNULL([Error_Message], '') + '~' + [Original Title (Tanil/Telugu)] + ' - Title Name is Duplicate in Database'  
	  WHERE [Original Title (Tanil/Telugu)] IN   
	  (  
	   SELECT Title_Name FROM [Title] T (NOLOCK) Where DM_Title.[Title Type] = (SELECT Deal_Type_Name FROM Deal_Type DT (NOLOCK) where DT.Deal_Type_Code  = T.Deal_Type_Code)   
	   GROUP BY Title_Name  
	...
	if(@Loglevel< 2) Exec [USPLogSQLSteps] '[USP_Validate_Title_Import]', 'Step 2', 0, 'Procedure Excution Completed', 0, ''
 END