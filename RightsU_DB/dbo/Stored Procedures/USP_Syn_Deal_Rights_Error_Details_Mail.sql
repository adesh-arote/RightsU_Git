...
BEGIN
	...
	if(@Loglevel< 2)Exec [USPLogSQLSteps] '[USP_Syn_Deal_Rights_Error_Details_Mail]', 'Step 1', 0, 'Started Procedure', 0, '' 
		SET NOCOUNT ON;
		...
		SELECT @Email_Config_Code=Email_Config_Code FROM Email_Config (NOLOCK) where [Key]='SRV'
		...
		BEGIN
			IF ((SELECT COUNT(*) FROM Syn_Deal_Rights_Error_Details	AS SDE (NOLOCK)
				INNER JOIN Syn_Deal_Rights SDR (NOLOCK) ON SDR.Syn_Deal_Rights_Code =SDE.Syn_Deal_Rights_Code
			...
			FROM Syn_Deal_Rights_Error_Details AS SDE (NOLOCK)
			INNER JOIN Syn_Deal_Rights SDR (NOLOCK) ON SDR.Syn_Deal_Rights_Code =SDE.Syn_Deal_Rights_Code
			INNER JOIN Syn_Deal SD (NOLOCK) ON SD.Syn_Deal_Code=SDR.Syn_Deal_Code
			...
			BEGIN
				IF ((SELECT COUNT(*) FROM Syn_Deal_Rights_Error_Details	AS SDE (NOLOCK)
					...
					FROM Syn_Deal_Rights_Error_Details SDE (NOLOCK)
					INNER JOIN Syn_Deal_Rights SDR  (NOLOCK) ON SDR.Syn_Deal_Rights_Code =SDE.Syn_Deal_Rights_Code
					INNER JOIN Syn_Deal SD (NOLOCK) ON SD.Syn_Deal_Code=SDR.Syn_Deal_Code
			...
			BEGIN
					...
					SELECT @Email_Config_Code, @EmailDetails, ISNULL(@To_Users_Code,''), ISNULL(@To_User_Mail_Id ,''), ISNULL(@CC_Users_Code,''), ISNULL(@CC_User_Mail_Id,''), ISNULL(@BCC_Users_Code,''), ISNULL(@BCC_User_Mail_Id,''), @MailSubject
	...
	if(@Loglevel< 2)Exec [USPLogSQLSteps] '[USP_Syn_Deal_Rights_Error_Details_Mail]', 'Step 2', 0, ' Procedure Excuting Completed', 0, '' 
END
