...
BEGIN	
	...
	if(@Loglevel < 2)Exec [USPLogSQLSteps] '[USP_Last_Month_Utilization_Report]', 'Step 1', 0, 'Started Procedure', 0, ''
		...
		BEGIN
			INSERT INTO #Temp_Channel_Code
			SELECT Channel_Code FROM Channel (NOLOCK)
		END
		...
		INNER JOIN Acq_Deal_Movie ADM (NOLOCK) ON ADM.Acq_Deal_Code= AD.Acq_Deal_Code --AND ADM.Acq_Deal_Movie_Code IN(SELECT Deal_Movie_Code FROM #BV_Trans)	
		INNER JOIN Acq_Deal_Rights ADR (NOLOCK) ON ADR.Acq_Deal_Code= ADM.Acq_Deal_Code --AND ADR.Acq_Deal_Rights_Code IN(SELECT Deal_Right_Code FROM #BV_Trans)	                       
		INNER JOIN Acq_Deal_Rights_Title ADRT (NOLOCK) ON ADR.Acq_Deal_Rights_Code= ADRT.Acq_Deal_Rights_Code AND  ADRT.title_code = ADM.Title_Code
		INNER JOIN Acq_Deal_Run ADMR (NOLOCK) ON ADMR.Acq_Deal_Code= ADR.Acq_Deal_Code 
		INNER JOIN Acq_Deal_Run_Title ADMRT (NOLOCK) ON ADMR.Acq_Deal_Run_Code= ADMRT.Acq_Deal_Run_Code AND ADMRT.title_code  = ADRT.Title_Code
		INNER JOIN Acq_Deal_Run_Channel ADMRC (NOLOCK) on ADMR.Acq_Deal_Run_Code= ADMRC.Acq_Deal_Run_Code 
		...
		LEFT JOIN Acq_Deal_Run_Yearwise_Run ADRY (NOLOCK) ON ADRY.Acq_Deal_Run_Code = ADMR.Acq_Deal_Run_Code		
		...
		LEFT JOIN BV_Schedule_Transaction BST (NOLOCK) ON  trr.Deal_Right_Code = bst.Deal_Movie_Rights_Code AND trr.Deal_Movie_Code = bst.Deal_Movie_Code
		INNER JOIN 
		(
		...
		) AS tmp_Run_Channel ON tmp_Run_Channel.Channel_Code =  trr.Channel_Code AND trr.Deal_Movie_Code=tmp_Run_Channel.Deal_Movie_Code 
	...
	if(@Loglevel < 2)Exec [USPLogSQLSteps] '[USP_Last_Month_Utilization_Report]', 'Step 2', 0, 'Procedure Excution Completed', 0, ''
END