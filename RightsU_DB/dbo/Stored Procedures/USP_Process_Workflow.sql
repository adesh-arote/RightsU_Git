...
--	@Remarks NVARCHAR(MAX)
	...
	if(@Loglevel<2)Exec [USPLogSQLSteps] '[USP_Process_Workflow]', 'Step 1', 0, 'Started Procedure', 0, ''
	INSERT INTO QueryTest(Sql_SELECT)
	SELECT 'USP_Process_Workflow: @Module_Code: '+CAST(@Module_Code AS VARCHAR)+ ' @Record_Code: '+ CAST(@Record_Code AS varchar)+ ' Login: '+CAST(@Login_User AS VARCHAR)
		+' UserAction: '+CAST(@User_Action AS VARCHAR)
		BEGIN TRY
			BEGIN TRANSACTION;		
			...
			FROM Module_Workflow_Detail (NOLOCK) WHERE Module_Code = @Module_Code AND Record_Code = @Record_Code And Is_Done = 'N' 
			ORDER By Role_Level
			...
			BEGIN
				...
				ELSE IF(@Module_Code = 262)
					UPDATE AL_Recommendation SET Workflow_Status = @User_Action, Last_Updated_Time = GETDATE(), Last_Action_By = @Login_User WHERE AL_Recommendation_Code = @Record_Code
				ELSE IF(@Module_Code = 265)
					UPDATE AL_Purchase_Order SET Workflow_Status = @User_Action, Updated_On = GETDATE() WHERE AL_Purchase_Order_Code = @Record_Code
			...
			BEGIN
				...
				DECLARE @Version VARCHAR(15)
					Update Acq_Deal Set Deal_Workflow_Status = @User_Action, Last_Updated_Time = GETDATE() Where Acq_Deal_Code = @Record_Code
					SELECT @Version=[Version] from Acq_Deal (NOLOCK) WHERE Acq_Deal_Code=@Record_Code
					IF(@User_Action = 'A')
					BEGIN
				...
				BEGIN
					...
					ELSE IF(@Module_Code = 163)
						EXEC DBO.USP_AT_Music_Deal @Record_Code, @Is_Error OUT
					--ELSE IF(@Module_Code = 262)
						--IF required AT Process needs to be called from here
				END
				IF(@Is_Email_Required = 'Y')
				BEGIN
					EXEC USP_SendMail_To_NextApprover_New @Record_Code, @Module_Code, 'Y', 'Y', @Is_Error Out
	...
	if(@Loglevel<2)Exec [USPLogSQLSteps] '[USP_Process_Workflow]', 'Step 2', 0, 'Procedure Excuting Completed', 0, ''
END
