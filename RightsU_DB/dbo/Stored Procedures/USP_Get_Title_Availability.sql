...
BEGIN
	...
	if(@Loglevel < 2)Exec [USPLogSQLSteps] '[USP_Get_Title_Availability]', 'Step 1', 0, 'Started Procedure', 0, ''
		...
		from Title t (NOLOCK)
		LEFT OUTER JOIN Title_Geners (NOLOCK) AS tg ON t.Title_Code =tg.Title_Code
		LEFT OUTER JOIN Genres (NOLOCK) AS g ON tg.Genres_Code = g.Genres_Code
		LEFT OUTER JOIN [Language] (NOLOCK) AS l ON t.Title_Language_Code = l.Language_Code
		...
		t.Duration_In_Min,t.Year_Of_Production,t.Language_Name	INTO #tem
			FROM Avail_Acq (NOLOCK) AS a 
			INNER JOIN Avail_Acq_Details (NOLOCK) AS d ON a.Avail_Acq_Code = d.Avail_Acq_Code
			INNER JOIN #temp t ON a.Title_Code = t.Title_Code
			LEFT OUTER JOIN Avail_Acq_Language (NOLOCK) AS l ON a.Avail_Acq_Code = l.Avail_Acq_Code
		...
		Select ar.Acq_Deal_Code,ar.Restriction_Remarks,adrp.Platform_Code,ADRT.Title_Code INTO #tempRight from Acq_Deal_Rights (NOLOCK) AS ar
		INNER JOIN Acq_Deal_Rights_Platform (NOLOCK) AS adrp ON adrp.Acq_Deal_Rights_Code = ar.Acq_Deal_Rights_Code
		INNER JOIN Acq_Deal_Rights_Title (NOLOCK) AS ADRT ON ADRT.Acq_Deal_Rights_Code = ar.Acq_Deal_Rights_Code
		...
		from #tempAvail AS tem
			INNER JOIN Acq_Deal (NOLOCK) AS ad ON tem.Acq_Deal_Code = ad.Acq_Deal_Code
	...
	if(@Loglevel < 2)Exec [USPLogSQLSteps] '[USP_Get_Title_Availability]', 'Step 2', 0, 'Procedure Excution Completed', 0, ''
END