...
BEGIN
	...
	if(@Loglevel < 2)Exec [USPLogSQLSteps] '[USP_Generate_Title_Content]', 'Step 1', 0, 'Started Procedure', 0, ''
		SET NOCOUNT ON
		SET FMTONLY OFF
		...
		BEGIN TRY
			...
			BEGIN
				...
				IF EXISTS (SELECT * FROM Acq_Deal (NOLOCK) WHERE Acq_Deal_Code = @Acq_Deal_Code AND LTRIM(RTRIM(Deal_Workflow_Status)) = 'A')
				BEGIN
				...
				BEGIN
					...
					(
						...
						INNER JOIN Title_Content_Mapping TCM (NOLOCK) ON TCM.Title_Content_Code = TC.Title_Content_Code
						AND TCM.Acq_Deal_Movie_Code = @AcqDealMovieCode
						INNER JOIN BV_Schedule_Transaction BST (NOLOCK) ON BST.Deal_Movie_Code = TCM.Acq_Deal_Movie_Code 
	...
	if(@Loglevel < 2)Exec [USPLogSQLSteps] '[USP_Generate_Title_Content]', 'Step 2', 0, 'Procedure Excution Completed', 0, ''
END