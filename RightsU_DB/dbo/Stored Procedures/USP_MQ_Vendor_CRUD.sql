...
BEGIN      
	 ...
	 BEGIN  
		  ...
		  BEGIN
				PRINT 'Vendor Name not exists'  
				SELECT TOP 1 @Is_Error = 'Y', @errorMessage = ISNULL(@errorMessage,'') + '108;Duplicate Vendor Name;'  FROM Vendor V WHERE V.Vendor_Name = @Vendor_Name  AND V.Is_Active = 'Y'
		  END
		  --------------------------------------------------------------------------------------------------------------  
	...
	BEGIN TRY
		 ...
		 BEGIN  
			  ...
			  SELECT @RightsU_Vendor_ID , Country_Code , Is_Theatrical_Territory FROM Country WHERE Country_Code IN (SELECT number FROM dbo.fn_Split_withdelemiter(@Country_Code,','))  
			  --------------------------------------------------------------------------------------------------------------  
			  ...
			  SELECT @RightsU_Vendor_ID, Role_Code, 'Y' FROM [Role] WHERE Role_Code IN (select number from #temp_Role_Code)  
			  --------------------------------------------------------------------------------------------------------------  
		 END  
		 ...
		 BEGIN  
			  ...
			  INSERT INTO Vendor_Country (Vendor_Code, Country_Code, Is_Theatrical)  
			  SELECT @RightsU_Vendor_ID , Country_Code , Is_Theatrical_Territory FROM Country WHERE Country_Code IN   
			  (SELECT id FROM #temp_Country  
			   EXCEPT  
			   SELECT country_Code FROM Vendor_Country  WHERE Vendor_Code = @RightsU_Vendor_ID)  
			  --------------------------------------------------------------------------------------------------------------  
			  ...
			  INSERT INTO Vendor_Role(Vendor_Code, Role_Code, Is_Active)  
			  SELECT @RightsU_Vendor_ID, Role_Code, 'Y' FROM [Role] WHERE Role_Code IN   
			  (SELECT number FROM #temp_Role_Code  
			  EXCEPT  
			  SELECT Role_Code FROM Vendor_Role  WHERE Vendor_Code = @RightsU_Vendor_ID)  
			  --------------------------------------------------------------------------------------------------------------  
			  DROP TABLE #temp_Country    
			  DROP TABLE #temp_Role_Code 
		 END 	
