...
BEGIN
	...
	if(@Loglevel < 2)Exec [USPLogSQLSteps] '[USP_List_IPR_Opp]', 'Step 1', 0, 'Started Procedure', 0, ''
		SET FMTONLY OFF;
		SET NOCOUNT ON;	
		...
		(
			...
			FROM IPR_Opp IOp (NOLOCK)
			INNER JOIN IPR_REP IR (NOLOCK) ON IOp.IPR_Rep_Code = IR.IPR_Rep_Code
			INNER JOIN IPR_REP_CLASS IRC (NOLOCK) ON IRC.IPR_Rep_Code = IR.IPR_Rep_Code
			LEFT JOIN IPR_CLASS IC (NOLOCK) ON IC.IPR_Class_Code = IOp.IPR_Class_Code OR IC.IPR_Class_Code = IRC.IPR_Class_Code
			LEFT JOIN IPR_Opp_Business_Unit IPR_BU (NOLOCK) ON IOp.IPR_Opp_Code=IPR_BU.IPR_Opp_Code
			LEFT JOIN IPR_Opp_Channel IPR_C (NOLOCK) ON IOp.IPR_Opp_Code=IPR_C.IPR_Opp_Code
		...
		SELECT DISTINCT
			...
			INNER JOIN IPR_REP_CLASS IRC (NOLOCK) ON IRC.IPR_Rep_Code = IR.IPR_Rep_Code
			LEFT JOIN IPR_CLASS IC (NOLOCK) ON IC.IPR_Class_Code = IOp.IPR_Class_Code
	...
	if(@Loglevel < 2)Exec [USPLogSQLSteps] '[USP_List_IPR_Opp]', 'Step 2', 0, 'Procedure Excution Completed', 0, ''
END