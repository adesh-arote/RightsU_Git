...
BEGIN  
	...
	if(@Loglevel < 2)Exec [USPLogSQLSteps] '[USP_Get_Title_Language]', 'Step 1', 0, 'Started Procedure', 0, ''
		--DECLARE @Title_Codes VARCHAR(MAX)
		--SET @Title_Codes = '6415,6416'

		DECLARE @Count INT = 0, @ReturnString NVARCHAR(200) = ''
		select @Count = COUNT(Distinct T.Title_Language_Code) from Title T  (NOLOCK) 
		where T.Title_Code in (SELECT number FROM fn_Split_withdelemiter(@Title_Codes, ','))
		IF(@Count = 0)
			SET @ReturnString = ''
		ELSE IF(@Count = 1)
			select TOP 1 @ReturnString = l.Language_Name from Title T (NOLOCK) 
			INNER JOIN Language L (NOLOCK) on L.Language_Code = T.Title_Language_Code
			where T.Title_Code in (SELECT number FROM fn_Split_withdelemiter(@Title_Codes, ','))
		ELSE
	...
	if(@Loglevel < 2)Exec [USPLogSQLSteps] '[USP_Get_Title_Language]', 'Step 2', 0, 'Procedure Excution Completed', 0, ''
END
