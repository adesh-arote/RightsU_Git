...
Begin
	...
	if(@Loglevel < 2)Exec [USPLogSQLSteps] '[USP_Get_PlatformCodes_For_Ancillary]', 'Step 1', 0, 'Started Procedure', 0, ''
		--DECLARE 
	...
	-- ============================================= -- ============================================= 
		...
		Begin
			...
			Inner Join Acq_Deal ad (NOLOCK) ON adr.Acq_Deal_Code = ad.Acq_Deal_Code And ADR.Actual_Right_Start_Date IS NOT NULL
		...
		BEGIN
			SELECT @Required_Codes =  STUFF((
				...
				FROM Acq_deal_Rights_Platform (NOLOCK) where Acq_deal_Rights_cODE = @Deal_Rights_Code
			FOR XML PATH('')
			), 1, 1, '');
	...
	if(@Loglevel < 2)Exec [USPLogSQLSteps] '[USP_Get_PlatformCodes_For_Ancillary]', 'Step 2', 0, 'Procedure Excution Completed', 0, ''
End