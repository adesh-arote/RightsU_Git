...
BEGIN
	...
	if(@Loglevel < 2)Exec [USPLogSQLSteps] '[USP_BMS_Deal_Data_Generation]', 'Step 1', 0, 'Started Procedure', 0, ''
		-- ==================================[BMS_Asset_Ref_Key]===========
		...
		-- =============================================
			...
			IF NOT EXISTS(SELECT TOP 1 BMS_Process_Deals_Code FROM  BMS_Process_Deals (NOLOCK) WHERE Record_Status = 'W')
			BEGIN
			DECLARE CUS_Deal_Process CURSOR FOR 
						SELECT Acq_Deal_Code FROM  BMS_Process_Deals (NOLOCK) WHERE Record_Status = 'P' ORDER BY BMS_Process_Deals_Code ASC
				...
				BEGIN
					...
					FROM BMS_Deal BV (NOLOCK) WHERE ISNULL(BV.Acq_Deal_Code,0) = @Deal_Code
					PRINT '1'
			...
			--		UPDATE #TempData Set Episode_Number = NULL WHERE Deal_Type_Code IN (1, 20, 21, 22)
				...
				/******** Update Total Runs and Content Start and End Date *********************************************************************/
					...
					SELECT @License_Fees = SUM(Deal_Cost) FROM Acq_Deal_Cost (NOLOCK) WHERE Acq_Deal_Code = @Deal_Code
					PRINT '4'
					BEGIN TRY
						BEGIN TRAN	
						...
						INSERT INTO BMS_Asset 
								...
								(
									(
										...
										AND
										(
								 			temp.RU_Title_Code IN (select RU_Title_Code FROM BMS_Asset BA (NOLOCK))-- WHERE BA.RU_Title_Code = temp.RU_Title_Code)
											AND CAST(temp.Episode_Number AS INT) NOT IN (SELECT CAST(Episode_Number AS INT) FROM BMS_Asset BA (NOLOCK) WHERE BA.RU_Title_Code = temp.RU_Title_Code)
										)
									)
									OR
									temp.RU_Title_Code NOT IN (select RU_Title_Code FROM BMS_Asset BA (NOLOCK) ) --WHERE BA.RU_Title_Code = temp.RU_Title_Code)
								)						
						PRINT '8'
							...
							INNER JOIN #TempData temp ON --BA.BMS_Deal_Code = temp.CurIden_BMS_Deal AND
								...
								(
									...
									BA.RU_ProgramCategory_Code NOT IN(						
										SELECT ISNULL(Columns_Value_Code,0) FROM Map_Extended_Columns (NOLOCK) WHERE Columns_Code = 1 And Record_Code = temp.RU_Title_Code 
									)
								)
						...
						BEGIN
							...
							FROM BMS_Deal_Content BDC (NOLOCK) WHERE BDC.BMS_Deal_Code = @BMS_Deal_Code
							...
							(
								...
								(
									SELECT BDC.[Start_Date] FROM BMS_Deal_Content BDC  (NOLOCK)
								...
								(
									SELECT BDC.[End_Date] FROM BMS_Deal_Content  BDC  (NOLOCK)
									WHERE BDC.BMS_Asset_Code = TD.CurIden_BMS_Asset 
									AND BDC.BMS_Deal_Code = @BMS_Deal_Code
									AND UPPER(BDC.Is_Archived) = 'FALSE'
								)
						...
						BEGIN		
							...
							*/		
							...
							INNER JOIN BMS_Deal_Content BDC (NOLOCK) ON BDCR.BMS_Deal_Content_Code = BDC.BMS_Deal_Content_Code 				
							AND BDC.BMS_Deal_Code = @BMS_Deal_Code							
	...
	if(@Loglevel < 2)Exec [USPLogSQLSteps] '[USP_BMS_Deal_Data_Generation]', 'Step 2', 0, 'Procedure Excution Completed', 0, ''
END