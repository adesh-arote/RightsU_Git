...
BEGIN
	...
	if(@Loglevel <2)Exec [USPLogSQLSteps] '[USP_RollBack_Music_Deal]', 'Step 1', 0, 'Started Procedure', 0, ''
		SET NOCOUNT ON 
		...
		BEGIN TRY
			...
			SELECT TOP 1 @AT_Music_Deal_Code = ISNULL(AT_Music_Deal_Code, 0) FROM AT_Music_Deal (NOLOCK) WHERE Music_Deal_Code = @Music_Deal_Code
			...
			SELECT 
				[Music_Deal_Channel_Code], @Music_Deal_Code, [Channel_Code], [Defined_Runs], [Scheduled_Runs]
			FROM AT_Music_Deal_Channel (NOLOCK) WHERE [AT_Music_Deal_Code] = @AT_Music_Deal_Code
			...
			SELECT 
				[Music_Deal_Country_Code], @Music_Deal_Code, [Country_Code]
			FROM AT_Music_Deal_Country (NOLOCK) WHERE [AT_Music_Deal_Code] = @AT_Music_Deal_Code
			...
			SELECT 
				[Music_Deal_Language_Code], @Music_Deal_Code, [Music_Language_Code]
			FROM AT_Music_Deal_Language (NOLOCK) WHERE [AT_Music_Deal_Code] = @AT_Music_Deal_Code
			...
			SELECT 
				[Music_Deal_LinkShow_Code], @Music_Deal_Code, [Title_Code]
			FROM AT_Music_Deal_LinkShow (NOLOCK) WHERE [AT_Music_Deal_Code] = @AT_Music_Deal_Code
			...
			SELECT 
				[Music_Deal_Vendor_Code], @Music_Deal_Code, [Vendor_Code]
			FROM AT_Music_Deal_Vendor (NOLOCK) WHERE [AT_Music_Deal_Code] = @AT_Music_Deal_Code
	...
	if(@Loglevel <2)Exec [USPLogSQLSteps] '[USP_RollBack_Music_Deal]', 'Step 2', 0, 'Procedure Excuting Completed', 0, ''
END