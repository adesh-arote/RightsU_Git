...
BEGIN
	...
	if(@Loglevel< 2)Exec [USPLogSQLSteps] '[USP_Syn_Bulk_Populate]', 'Step 1', 0, 'Started Procedure', 0, '' 
	--SET FMTONLY OFF
		...
		Create TABLE #temp1(Display_Text NVARCHAR(MAX),Display_Value VARCHAR(50),PlatformCodes VARCHAR(MAX));
		...
		SELECT @Is_Theatrical=Is_Theatrical_Right,@Syn_Deal_Code=Syn_Deal_Code FROM Syn_Deal_Rights (NOLOCK) WHERE Syn_Deal_Rights_Code IN(SELECT TOP 1 Right_Code FROM #RightCodes)
		SELECT @RCount=COUNT(*) FROM #RightCodes
		INSERT INTO #TitleCodes
		SELECT Title_Code FROM Syn_Deal_Rights_Title (NOLOCK) WHERE Syn_Deal_Rights_Code IN(SELECT Right_Code FROM #RightCodes)
		...
		SELECT Syn_Deal_Movie_Code FROM Syn_Deal_Movie (NOLOCK) WHERE Title_Code IN(SELECT Title_Code FROM #TitleCodes)
		...
		BEGIN
			...
			BEGIN
				...
				SELECT Country_Name,Country_Code FROM Country (NOLOCK) WHERE Country_Code IN(SELECT number FROM dbo.[fn_Split_withdelemiter](@RequiredCodes,',') WHERE number!='')
			END
			ELSE IF(@Type='T')
			BEGIN
				INSERT INTO #temp
				...
				IF(@RequiredCodes!='')
				INSERT INTO #temp1(Display_Text,Display_Value)
				SELECT Territory_Name,Territory_Code FROM Territory (NOLOCK) WHERE Territory_Code IN(SELECT number FROM dbo.[fn_Split_withdelemiter](@RequiredCodes,',') WHERE number!='')
			END
			...
			print 'SL'
				...
				INSERT INTO #temp1(Display_Text,Display_Value)
				SELECT Language_Name,Language_Code FROM Language (NOLOCK) WHERE Language_Code IN(SELECT number FROM dbo.[fn_Split_withdelemiter](@SubTitle_Lang_Code,',') WHERE number!='')
			END
			ELSE IF(@Type='SG')
			BEGIN
				INSERT INTO #temp
				...
				IF(@SubTitle_Lang_Code!='')
				INSERT INTO #temp1(Display_Text,Display_Value)
				SELECT Language_Group_Name,Language_Group_Code FROM Language_Group (NOLOCK) WHERE Language_Group_Code IN(SELECT number FROM dbo.[fn_Split_withdelemiter](@SubTitle_Lang_Code,',') WHERE number!='')
			END
			ELSE IF(@Type='DL')
			BEGIN
				INSERT INTO #temp
				...
				IF(@Dubb_Lang_Code!='')
				INSERT INTO #temp1(Display_Text,Display_Value)
				SELECT Language_Name,Language_Code FROM Language (NOLOCK) WHERE Language_Code IN(SELECT number FROM dbo.[fn_Split_withdelemiter](@Dubb_Lang_Code,',') 
				WHERE number!='')
			END
			ELSE IF(@Type='DG')
			BEGIN
				...
				IF(@Dubb_Lang_Code!='')
				INSERT INTO #temp1(Display_Text,Display_Value)
				SELECT Language_Group_Name,Language_Group_Code FROM Language_Group (NOLOCK) WHERE Language_Group_Code 
		...
		BEGIN
			...
			BEGIN			
				...
				SELECT DISTINCT Platform_Code FROM Syn_Deal_Rights_Platform (NOLOCK) WHERE Syn_Deal_Rights_Code IN(SELECT Right_Code FROM #RightCodes) 			
			...
			BEGIN
				INSERT INTO #temp(RequiredCodes)
				SELECT DISTINCT Country_Code FROM Syn_Deal_Rights_Territory (NOLOCK) WHERE Syn_Deal_Rights_Code IN(SELECT Right_Code FROM #RightCodes) 
			...
			BEGIN
				INSERT INTO #temp(RequiredCodes)
				SELECT DISTINCT Territory_Code FROM Syn_Deal_Rights_Territory (NOLOCK) WHERE Syn_Deal_Rights_Code IN(SELECT Right_Code FROM #RightCodes) 
				GROUP BY Territory_Code having COUNT(*)=@RCount
				SELECT @RequiredCodes= Stuff((SELECT ',' + Cast(rc.RequiredCodes AS VARCHAR(MAX))	FROM #temp rc FOR XML PATH('')), 1, 1, '')
				IF(@RequiredCodes!='')
				INSERT INTO #temp1(Display_Text,Display_Value)
				SELECT Territory_Name,Territory_Code FROM Territory (NOLOCK) WHERE Territory_Code IN(SELECT number FROM dbo.[fn_Split_withdelemiter](@RequiredCodes,',') 
				WHERE number!='')
			END
			ELSE IF(@Type='I' AND @Is_Theatrical='Y')
			BEGIN
			...
			BEGIN
				...
				SELECT DISTINCT Language_Code FROM Syn_Deal_Rights_Subtitling (NOLOCK) WHERE Syn_Deal_Rights_Code IN(SELECT Right_Code FROM #RightCodes) 
				...
				IF(@SubTitle_Lang_Code!='')
				INSERT INTO #temp1(Display_Text,Display_Value)
				SELECT Language_Name,Language_Code FROM Language (NOLOCK) WHERE Language_Code IN(SELECT number FROM dbo.[fn_Split_withdelemiter](@SubTitle_Lang_Code,',') 
				WHERE number!='')
			END
			ELSE IF(@Type='SG')
			BEGIN
				INSERT INTO #temp(SubTitle_Lang_Code)
				SELECT DISTINCT Language_Group_Code FROM Syn_Deal_Rights_Subtitling (NOLOCK) WHERE Syn_Deal_Rights_Code IN(SELECT Right_Code FROM #RightCodes) 
				GROUP BY Language_Group_Code having COUNT(*)=@RCount
				SELECT @SubTitle_Lang_Code=Stuff((SELECT ',' + Cast(rc.SubTitle_Lang_Code AS VARCHAR(MAX))	FROM #temp rc FOR XML PATH('')), 1, 1, '')
				IF(@SubTitle_Lang_Code!='')
				INSERT INTO #temp1(Display_Text,Display_Value)
				SELECT Language_Group_Name,Language_Group_Code FROM Language_Group (NOLOCK) WHERE Language_Group_Code IN(SELECT number FROM dbo.[fn_Split_withdelemiter]
				(@SubTitle_Lang_Code,',') WHERE number!='')
			END
			ELSE IF(@Type='DL')
			BEGIN
				INSERT INTO #temp(Dubb_Lang_Code)
				SELECT DISTINCT Language_Code FROM Syn_Deal_Rights_Dubbing (NOLOCK) WHERE Syn_Deal_Rights_Code IN(SELECT Right_Code FROM #RightCodes) 
				GROUP BY Language_Code having COUNT(*)=@RCount
			...
			BEGIN
				INSERT INTO #temp(Dubb_Lang_Code)
				SELECT DISTINCT Language_Group_Code FROM Syn_Deal_Rights_Dubbing (NOLOCK) WHERE Syn_Deal_Rights_Code IN(SELECT Right_Code FROM #RightCodes) 
				GROUP BY Language_Group_Code having COUNT(*)=@RCount
				SELECT @Dubb_Lang_Code=Stuff((SELECT ',' + Cast(rc.Dubb_Lang_Code AS VARCHAR(MAX))	FROM #temp rc FOR XML PATH('')), 1, 1, '')
				IF(@Dubb_Lang_Code!='')
				INSERT INTO #temp1(Display_Text,Display_Value)
		...
		BEGIN
			...
			INSERT INTO #temp1(Display_Text,Display_Value)
			SELECT Sub_License_Name,Sub_License_Code FROM Sub_License (NOLOCK)
		END
	...
	if(@Loglevel< 2)Exec [USPLogSQLSteps] '[USP_Syn_Bulk_Populate]', 'Step 2', 0, 'Procedure Excuting Completed', 0, '' 
END