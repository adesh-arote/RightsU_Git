CREATE PROC [dbo].[Avail_GetAcquisitionDealRightNeo]
(    
 --@Acq_Deal_Code  INT,
...
BEGIN
	...
	if(@Loglevel < 2)Exec [USPLogSQLSteps] '[Avail_GetAcquisitionDealRightNeo]', 'Step 1', 0, 'Started Procedure', 0, ''
		--Country: this will return all country codes for given Acq_Deal_Rights_Code
	SELECT CountryCode --, Country_Name CountryName
	FROM 
	(SELECT DISTINCT 
					CASE 
	...
	SELECT P.Platform_Code PlatformCode--, P.Platform_Hiearachy PlatformName 
	...
	select	Is_Exclusive IsExclusive, 
			...
			ADR.Acq_Deal_Rights_Code As RightCode
	...
	if(@Loglevel < 2)Exec [USPLogSQLSteps] '[Avail_GetAcquisitionDealRightNeo]', 'Step 2', 0, 'Procedure Execution Completed', 0, ''
END