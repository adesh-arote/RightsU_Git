...
BEGIN
	...
	BEGIN
		SELECT DCR.*, DRT.Title_Code, BMA.Episode_Number, DRC.Channel_Code INTO #tempDCR
			...
			INNER JOIN @Deal_Run_Title DRT ON BMA.RU_Title_Code = DRT.Title_Code AND CAST(BMA.Episode_Number AS INT) BETWEEN DRT.Episode_From AND DRT.Episode_To
			LEFT JOIN @Deal_Run_Channel DRC ON DCR.RU_Channel_Code = DRC.Channel_Code
			...
			DYR.No_Of_Runs, SUM(DCR.Utilised_Run) AS No_Of_Schd_Run
			FROM #tempDCR DCR
			...
			HAVING SUM(DCR.Utilised_Run) > DYR.No_Of_Runs

			IF((SELECT TOP 1 Run_Definition_Type FROM Acq_Deal_Run (NOLOCK)	WHERE Acq_Deal_Run_Code in (SELECT Deal_Run_Code FROM @Deal_Run_Channel )) = 'C')
			BEGIN
				INSERT INTO #TEMP(Title_Name, Episode_No, StartDate, EndDate, Channel_Name, No_Of_Runs, No_Of_Schd_Run)
				SELECT tl.Title_Name, DCR.Episode_Number, 'NA' AS StartDate ,'NA' AS EndDate, Chnl.Channel_Name, DRC.Min_Runs AS No_Of_Runs, SUM(DCR.Utilised_Run) AS No_Of_Schd_Run 
				FROM #tempDCR DCR
				...
				HAVING  SUM(DCR.Utilised_Run) > DRC.Min_Runs
			END
	END
