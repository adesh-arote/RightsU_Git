...
BEGIN
	...
	if(@Loglevel  < 2)Exec [USPLogSQLSteps] '[USP_AT_Music_Deal]', 'Step 1', 0, 'Started Procedure', 0, ''
		SET NOCOUNT ON 
		...
		BEGIN TRY
			...
			SELECT 
				...
				[Right_Rule_Code], [Link_Show_Type], [Business_Unit_Code], [Deal_Type_Code], [Deal_Workflow_Status], [Work_Flow_Code], [Parent_Deal_Code], 
				[Inserted_By], [Inserted_On], [Last_Updated_Time], [Last_Action_By], [Lock_Time], [Remarks],[Agreement_Cost],[Channel_Category_Code],[Channel_Or_Category], [Music_Deal_Code]
			FROM Music_Deal (NOLOCK) WHERE Music_Deal_Code = @Music_Deal_Code
			...
			SELECT 
				@AT_Music_Deal_Code, [Channel_Code], [Defined_Runs], [Scheduled_Runs], [Music_Deal_Channel_Code]
			FROM Music_Deal_Channel (NOLOCK) WHERE Music_Deal_Code = @Music_Deal_Code
			...
			SELECT 
				@AT_Music_Deal_Code, [Country_Code], [Music_Deal_Country_Code]
			FROM Music_Deal_Country (NOLOCK) WHERE Music_Deal_Code = @Music_Deal_Code
			...
			SELECT 
				@AT_Music_Deal_Code, [Music_Language_Code], [Music_Deal_Language_Code]
			FROM Music_Deal_Language (NOLOCK) WHERE Music_Deal_Code = @Music_Deal_Code
			...
			SELECT 
				@AT_Music_Deal_Code, [Title_Code], [Music_Deal_LinkShow_Code]
			FROM Music_Deal_LinkShow (NOLOCK) WHERE Music_Deal_Code = @Music_Deal_Code
			...
			SELECT 
				@AT_Music_Deal_Code, [Vendor_Code], [Music_Deal_Vendor_Code]
			FROM Music_Deal_Vendor (NOLOCK) WHERE Music_Deal_Code = @Music_Deal_Code
			SET @Is_Error = 'N'
	...
	if(@Loglevel  < 2)Exec [USPLogSQLSteps] '[USP_AT_Music_Deal]', 'Step 2', 0, 'Procedure Excution Completed', 0, ''
END