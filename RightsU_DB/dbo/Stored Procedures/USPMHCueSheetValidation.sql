...
BEGIN  
	...
	if(@Loglevel< 2)Exec [USPLogSQLSteps] '[USPMHCueSheetValidation]', 'Step 1', 0, 'Started Procedure', 0, ''
		SET NOCOUNT ON
		...
		BEGIN TRANSACTION
			...
			FROM MHCueSheet (NOLOCK) WHERE [UploadStatus] = 'P'
			...
			BEGIN
				...
				[DurationFrame],[ExcelLineNo],[RecordStatus],[ErrorMessage],[TitleCode],[TitleContentCode],[MusicTitleCode],[IsApprove] 
				FROM MHCueSheetSong (NOLOCK) WHERE MHCueSheetCode = @MHCueSheetCode
				...
				BEGIN
					...
					SELECT @TitleCode = Title_Code FROM Title (NOLOCK) WHERE  Deal_Type_Code IN (SELECT number FROM fn_Split_withdelemiter(@dealTypeCode, ',')) AND Title_Name COLLATE DATABASE_DEFAULT = 
					(SELECT TitleName FROM #Temp_MHCueSheetSong WHERE MHCueSheetSongCode = @MHCueSheetSongCode) COLLATE DATABASE_DEFAULT

					SELECT @Music_Title_Code = MT.Music_Title_Code FROM Music_Title MT (NOLOCK) WHERE MT.Is_Active = 'Y' AND MT.Music_Title_Name COLLATE DATABASE_DEFAULT = 
					...
					BEGIN
						...
						SELECT Title_Content_Code, Title_Code, Episode_No FROM Title_Content (NOLOCK) WHERE Title_Code = @TitleCode
					...
					ELSE
					BEGIN	
						IF EXISTS (SELECT * FROM MHCueSheetSong (NOLOCK) TV WHERE  TV.MHCueSheetSongCode = @MHCueSheetSongCode AND ISNULL(TV.TitleName, '') = '')
						...
						ELSE
							BEGIN
								IF NOT EXISTS (Select * FROM Title_Content (NOLOCK) Where Episode_Title COLLATE DATABASE_DEFAULT IN (Select TitleName from MHCueSheetSong (NOLOCK) where MHCueSheetSongCode = @MHCueSheetSongCode))
								BEGIN
					...
					ELSE
					BEGIN
						IF EXISTS (SELECT * FROM MHCueSheetSong TV (NOLOCK) WHERE  TV.MHCueSheetSongCode = @MHCueSheetSongCode AND ISNULL(TV.EpisodeNo, '') = '')
						...
						--Added by rahul
							BEGIN
								IF NOT EXISTS (Select * FROM Title_Content (NOLOCK) Where Episode_Title COLLATE DATABASE_DEFAULT IN (Select TitleName from MHCueSheetSong (NOLOCK) where MHCueSheetSongCode = @MHCueSheetSongCode) AND Episode_No = (Select Episode_No from MHCueSheetSong where MHCueSheetSongCode = @MHCueSheetSongCode))
								BEGIN
					...
					ELSE
					BEGIN 
						IF EXISTS (SELECT * FROM MHCueSheetSong TV (NOLOCK) WHERE  TV.MHCueSheetSongCode = @MHCueSheetSongCode AND ISNULL(TV.MusicTrackName, '') = '')
					...
					BEGIN
					IF EXISTS (SELECT * FROM MHCueSheetSong TV (NOLOCK) WHERE  TV.MHCueSheetSongCode = @MHCueSheetSongCode AND ISNULL(TV.SongType, '') = '')
						BEGIN
							UPDATE TV SET  TV.RecordStatus = 'E',TV.ErrorMessage =  ISNULL(TV.ErrorMessage, '') + 'MHSTB~' --Music Hub Song Type is Blank
							FROM MHCueSheetSong TV 
							WHERE  TV.MHCueSheetSongCode = @MHCueSheetSongCode 
						END
					ELSE IF Not Exists(select * from MHMusicSongType (NOLOCK) where SongType = (Select top 1 SongType from MHCueSheetSong (NOLOCK) where MHCueSheetSongCode = @MHCueSheetSongCode AND SongType IS NOT NULL))
					BEGIN
						DECLARE @SongType NVARCHAR(50)
						SET @SongType = (Select top 1 SongType from MHCueSheetSong (NOLOCK) where MHCueSheetSongCode = @MHCueSheetSongCode)
					...
					BEGIN
						...
						SET @MusicTrackName = (Select TOP 1 MusicTrackName FROM MHCueSheetSong (NOLOCK) WHERE MHCueSheetSongCode = @MHCueSheetSongCode)
						SET @MovieAlbum = (Select TOP 1 MovieAlbum FROM MHCueSheetSong (NOLOCK) WHERE MHCueSheetSongCode = @MHCueSheetSongCode)
						SET @MovieAlbumCode = (Select Music_Album_Code from Music_Album (NOLOCK) where Music_Album_Name = (Select MovieAlbum from MHCueSheetSong  (NOLOCK) Where MHCueSheetSongCode = @MHCueSheetSongCode))
										
						IF EXISTS (SELECT * FROM MHCueSheetSong TV (NOLOCK) WHERE  TV.MHCueSheetSongCode = @MHCueSheetSongCode AND ISNULL(TV.MovieAlbum, '') = '')
							BEGIN
								UPDATE TV SET  TV.RecordStatus = 'E',TV.ErrorMessage =  ISNULL(TV.ErrorMessage, '') + 'MHMAB~' --Music Hub Music/Album is Blank
								FROM MHCueSheetSong TV 
								WHERE  TV.MHCueSheetSongCode = @MHCueSheetSongCode
							END
						ELSE IF NOT EXISTS(Select * FROM Music_Album (NOLOCK) where Music_Album_Name = @MovieAlbum) 
							BEGIN
								UPDATE TV SET  TV.RecordStatus = 'W', TV.ErrorMessage =  ISNULL(TV.ErrorMessage, '~') + 'MHMANF~' -- Music Hub Movie/Album Not Found
								FROM MHCueSheetSong TV --#Temp_MHCueSheetSong TV
								WHERE TV.MHCueSheetSongCode = @MHCueSheetSongCode
							END
						ELSE IF NOT EXISTS (SELECT * FROM Music_Title (NOLOCK) WHERE Music_Title_Name = @MusicTrackName AND Music_Album_Code = @MovieAlbumCode)
							BEGIN
				...
				--*********************************** FINAL UPLOAD STATUS STARTS ***********************************
					...
					IF EXISTS(SELECT TV.RecordStatus FROM MHCueSheetSong TV (NOLOCK)  WHERE MHCueSheetCode = @MHCueSheetCode and TV.RecordStatus = 'E')
					BEGIN
					...
					END
					ELSE IF EXISTS(SELECT TV.RecordStatus FROM MHCueSheetSong TV (NOLOCK)  WHERE MHCueSheetCode = @MHCueSheetCode and TV.RecordStatus= 'W')
					BEGIN
					...
					END
					ELSE IF NOT EXISTS(SELECT TV.RecordStatus FROM MHCueSheetSong TV (NOLOCK) WHERE MHCueSheetCode = @MHCueSheetCode and TV.RecordStatus IN ('W','E'))
					BEGIN
						IF NOT EXISTS (SELECT TV.RecordStatus FROM MHCueSheetSong TV (NOLOCK) WHERE MHCueSheetCode = @MHCueSheetCode)
						BEGIN
					...
					UPDATE TV SET TV.ErrorRecords = ( SELECT count(*) From MHCueSheetSong (NOLOCK) WHERE MHCueSheetCode = @MHCueSheetCode and RecordStatus in ('E','W') ) 
			...
			[MHCueSheetCode],[RequestID] ,[FileName],[UploadStatus],[VendorCode] ,[TotalRecords],[ErrorRecords] ,[CreatedBy],[CreatedOn] ,[ApprovedBy] ,[ApprovedOn] )
			SELECT [MHCueSheetCode],[RequestID] ,[FileName],[UploadStatus],[VendorCode] ,[TotalRecords],[ErrorRecords] ,[CreatedBy],[CreatedOn] ,[ApprovedBy] ,[ApprovedOn]
			FROM MHCueSheet (NOLOCK) WHERE [UploadStatus] = 'R'
			...
			FROM Title_Content_Version (NOLOCK) GROUP BY Title_Content_Code
			...
			BEGIN
				...
				PRINT 'Step 4 : #temp_MHCueSheet ie RowNo = ' +   CAST(@RowNo_MH AS NVARCHAR(MAX)) + ' And MHCueSheetCode = ' +   CAST(@MHCueSheetCode AS NVARCHAR(MAX))
					...
					(
					SELECT MusicTitleCode FROM MHCueSheetSong (NOLOCK) WHERE MHCueSheetCode = @MHCueSheetCode
					EXCEPT
					SELECT DISTINCT MTL.Music_Title_Code FROM MHCueSheet MR (NOLOCK)
					   INNER JOIN MHCueSheetSong MRD (NOLOCK) ON MR.MHCueSheetCode = MRD.MHCueSheetCode 
					   INNER JOIN Music_Title_Label MTL (NOLOCK) ON MTL.Music_Title_Code = MRD.MusicTitleCode
					   INNER JOIN Music_Deal MD (NOLOCK) ON MD.Music_Label_Code = MTL.Music_Label_Code
					...
					*/
					...
					(
						SELECT DISTINCT MRD.TitleCode FROM  MHCueSheetSong MRD (NOLOCK) WHERE MRD.MHCueSheetCode = @MHCueSheetCode
						EXCEPT
						SELECT TitleCode FROM MHRequest (NOLOCK) WHERE MHRequestTypeCode = 1
					)
	...
	if(@Loglevel< 2)Exec [USPLogSQLSteps] '[USPMHCueSheetValidation]', 'Step 2', 0, 'Procedure Excution Completed', 0, ''
END
