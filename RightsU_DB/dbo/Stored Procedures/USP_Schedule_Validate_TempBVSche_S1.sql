CREATE PROC [dbo].[usp_Schedule_Validate_TempBVSche_S1]  
(  
 @File_Code BIGINT,  
...
BEGIN  
	...
	if(@Loglevel< 2)Exec [USPLogSQLSteps] '[usp_Schedule_Validate_TempBVSche_S1]', 'Step 1', 0, 'Started Procedure', 0, ''
	...
	PRINT '--===============4.0 PROCEES ALL MATCHED PROGRAMMIDS --==============='  
	 --===============4.1 Inserting all matched PROGRAMMIDS records into BV_Schedule_Transaction --===============  
	  ...
	  FROM Temp_BV_Schedule tbs  (NOLOCK) 
	   ...
	   INNER JOIN Title_Content_Mapping dmc (NOLOCK) ON dmc.Title_Content_Code = tc.Title_Content_Code  
	   LEFT JOIN ACQ_Deal_Movie DM  (NOLOCK) ON DM.ACQ_deal_movie_code = DMC.ACQ_deal_movie_code AND ISNULL(dmc.Deal_For, 'A') = 'A'  
	   LEFT JOIN ACQ_Deal D (NOLOCK) ON D.ACQ_deal_code = DM.ACQ_deal_code   
	   LEFT JOIN Provisional_Deal_Title pdt (NOLOCK) ON pdt.Provisional_Deal_Title_Code = DMC.Provisional_Deal_Title_Code AND ISNULL(dmc.Deal_For, 'A') = 'P'  
	   LEFT JOIN Provisional_Deal  pd (NOLOCK) ON pd.Provisional_Deal_Code = pdt.Provisional_Deal_Code   
	 ...
	 PRINT '----------  PROCESS THOSE TITLES WHOSE DEALS ARE NOT APPROVED ---------------'  
	  ...
	  SELECT @EmailMsg_New_Deal =  Email_Msg FROM Email_Notification_Msg (NOLOCK) WHERE LTRIM(RTRIM(Email_Msg_For)) = 'Deal_Not_Approve' AND [Type] = 'S'  
	...
	if(@Loglevel< 2)Exec [USPLogSQLSteps] '[usp_Schedule_Validate_TempBVSche_S1]', 'Step 2', 0, 'Procedure Excuting Complete', 0, ''
END