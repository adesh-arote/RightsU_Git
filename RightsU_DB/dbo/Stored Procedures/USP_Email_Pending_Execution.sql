CREATE PROC [dbo].[USP_Email_Pending_Execution]
AS
BEGIN
	Declare @Loglevel int;
	select @Loglevel = Parameter_Value from System_Parameter_New where Parameter_Name='loglevel'
	if(@Loglevel < 2)Exec [USPLogSQLSteps] '[USP_Email_Pending_Execution]', 'Step 1', 0, 'Started Procedure', 0, '' 
		...
		SELECT @Days_Freq = ECDA.Mail_Alert_Days FROM email_config EC (NOLOCK)
		INNER JOIN Email_Config_Detail ECD (NOLOCK) ON ECD.Email_Config_Code = EC.Email_Config_Code
		INNER JOIN Email_Config_Detail_Alert ECDA (NOLOCK) ON ECDA.Email_Config_Detail_Code = ECD.Email_Config_Detail_Code
		WHERE EC.[Key] = 'PEX'

		SELECT @Deal_Tag_Code = Deal_Tag_Code from Deal_Tag (NOLOCK) where Deal_Tag_Description = 'Pending For Execution'
		...
		FROM Email_Config EC  (NOLOCK)
			INNER JOIN Email_Config_Detail ECD (NOLOCK) ON  EC.Email_Config_Code=ECD.Email_Config_Code
			INNER JOIN Email_Config_Detail_User ECDU (NOLOCK) ON ECDU.Email_Config_Detail_Code=ECD.Email_Config_Detail_Code
		WHERE EC.[Key]='PEX'
		...
		UPDATE AD SET AD.Titles =  STUFF((
			SELECT DISTINCT ',' +  T.Title_Name FROM Syn_Deal_Movie ADM (NOLOCK)
			LEFT JOIN Title T (NOLOCK) ON T.Title_Code = ADM.Title_Code
		...
		BEGIN  
			 ...
			 BEGIN
					SELECT @TO = STUFF((
						SELECT  DISTINCT ';' + Email_Id FROM Users (NOLOCK) WHERE Users_Code IN (SELECT number FROM DBO.fn_Split_withdelemiter(@User_Codes, ',') WHERE number <> ',')
					FOR XML PATH('')), 1, 1, '')
	
					SELECT @CC = STUFF((
						SELECT DISTINCT ';' + Email_Id FROM Users (NOLOCK) WHERE Users_Code IN (SELECT number FROM DBO.fn_Split_withdelemiter(@CC_Users, ',') WHERE number <> ',')
					FOR XML PATH('')), 1, 1, '')
			 END
			 ELSE IF (@User_Type = 'G')
			 BEGIN
					SELECT @TO = STUFF((
						SELECT DISTINCT ';' + Email_Id FROM Users (NOLOCK) where Security_Group_Code = @Security_Group_Code and Is_Active = 'Y'
					FOR XML PATH('')), 1, 1, '')
	...
	if(@Loglevel < 2)Exec [USPLogSQLSteps] '[USP_Email_Pending_Execution]', 'Step 2', 0, 'Procedure Excution Completed', 0, '' 
END
