CREATE PROCEDURE [dbo].[USP_Acq_Deal_Clone] 
...
BEGIN
	...
	if(@Loglevel < 2)Exec [USPLogSQLSteps] '[USP_Acq_Deal_Clone]', 'Step 1', 0, 'Started Procedure', 0, ''
		...
		--Declare cursor for Rights
				DECLARE rights_cursor CURSOR FOR 
					SELECT Acq_Deal_Rights_Code FROM Acq_Deal_Rights (NOLOCK) WHERE Acq_Deal_Code = @Previous_Acq_Deal_Code
				OPEN rights_cursor
	 ...
	 ;
	if(@Loglevel < 2)Exec [USPLogSQLSteps] '[USP_Acq_Deal_Clone]', 'Step 2', 0, 'Procedure Excution Completed', 0, ''
END
