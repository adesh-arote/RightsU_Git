...
BEGIN
	...
	if(@Loglevel< 2)Exec [USPLogSQLSteps] '[USPMHStatusHistoryList]', 'Step 1', 0, 'Started Procedure', 0, ''
		IF(OBJECT_ID('TEMPDB..#TempRightsPagingData') IS NOT NULL)
			...
			From MHRequest MR (NOLOCK)
				Inner Join MHRequestType MRT (NOLOCK) on MR.MHRequestTypeCode=MRT.MHRequestTypeCode
				Inner Join  Title T (NOLOCK) on MR.TitleCode=T.Title_Code
				Left Join  MHRequestStatus MRS (NOLOCK) on MR.MHRequestStatusCode=MRS.MHRequestStatusCode
	...
	if(@Loglevel< 2)Exec [USPLogSQLSteps] '[USPMHStatusHistoryList]', 'Step 2', 0, 'Procedure Excution Completed', 0, ''
END