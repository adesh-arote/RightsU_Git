CREATE PROCEDURE [dbo].[USPMHGetLabelWiseUsage] 
@UsersCode INT,
...
BEGIN
	...
	if(@Loglevel< 2)Exec [USPLogSQLSteps] '[USPMHGetLabelWiseUsage]', 'Step 1', 0, 'Started Procedure', 0, ''
		...
		IF(@LabelName = 'Others')
			BEGIN
				...
				LEFT JOIN Music_Title_Label MTL (NOLOCK) ON MTL.Music_Title_Code = MRD.MusicTitleCode AND (Effective_To IS NULL OR Effective_To BETWEEN Effective_From AND GETDATE())
		...
		IF(@ShowName = '')
			BEGIN
				...
				INNER JOIN MHRequest MR (NOLOCK) ON MRD.MHRequestCode = MR.MHRequestCode AND MR.TitleCode IS NOT NULL AND ((CAST(MR.RequestedDate AS DATE)   BETWEEN REPLACE(CONVERT(NVARCHAR,@FromDate, 106),' ', '-') AND REPLACE(CONVERT(NVARCHAR,@ToDate, 106),' ', '-')))
				LEFT JOIN Music_Title_Label MTL (NOLOCK) ON MTL.Music_Title_Code = MRD.MusicTitleCode
				INNER JOIN Music_Title MT (NOLOCK) ON MT.Music_Title_Code = MTL.Music_Title_Code
				LEFT JOIN Title T (NOLOCK) ON T.Title_Code = MR.TitleCode
				LEFT JOIN Music_Language ML (NOLOCK) ON ML.Music_Language_Code = MT.Language_Code
				WHERE MR.MHRequestTypeCode = 1 AND MusicTitleCode IN (SELECT Music_Title_Code FROM Music_Title_Label (NOLOCK) WHERE Music_Label_Code IN (SELECT Music_Label_Code FROM MUSIC_Label WHERE Music_Label_Name COLLATE DATABASE_DEFAULT IN(Select LabelName from #tempLabelName1)-- @LabelName
				) AND
				(Effective_To IS NULL OR Effective_To BETWEEN Effective_From AND GETDATE()))
				AND MR.VendorCode In (Select Vendor_Code From MHUsers (NOLOCK) Where Users_Code = @UsersCode)
				ORDER BY ShowName
		...
		ELSE IF(@ShowName = 'Others')
			BEGIN
				...
				LEFT JOIN Title T (NOLOCK) ON T.Title_Code = MR.TitleCode
				LEFT JOIN Music_Language ML (NOLOCK) ON ML.Music_Language_Code = MT.Language_Code
				WHERE MR.MHRequestTypeCode = 1 AND MusicTitleCode IN (SELECT Music_Title_Code FROM Music_Title_Label (NOLOCK) WHERE Music_Label_Code IN (SELECT Music_Label_Code FROM MUSIC_Label (NOLOCK) WHERE  Music_Label_Name COLLATE DATABASE_DEFAULT IN(Select LabelName from #tempLabelName1)
				) AND
				(Effective_To IS NULL OR Effective_To BETWEEN Effective_From AND GETDATE()))
				AND MR.VendorCode In (Select Vendor_Code From MHUsers (NOLOCK) Where Users_Code = @UsersCode)
	...
	if(@Loglevel< 2)Exec [USPLogSQLSteps] '[USPMHGetLabelWiseUsage]', 'Step 2', 0, 'Procedure Excution Completed', 0, ''
END