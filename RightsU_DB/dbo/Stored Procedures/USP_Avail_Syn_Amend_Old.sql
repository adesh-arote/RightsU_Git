...
BEGIN
	...
	if(@Loglevel < 2)Exec [USPLogSQLSteps] '[USP_Avail_Syn_Amend_Old]', 'Step 1', 0, 'Started Procedure', 0, ''
		DECLARE 
		@Title_Code INT,
		@Platform_Code INT,
		@Country_Code INT,
		...
		FOR SELECT AVS.Title_Code,AVS.Platform_Code,AVS.Country_Code,AVSD.Rights_Start_Date,AVSD.Rights_End_Date
			...
			LEFT JOIN Avail_Syn_Language AVSL (NOLOCK) ON AVS.Avail_Syn_Code=AVSL.Avail_Syn_Code
	...
	if(@Loglevel < 2)Exec [USPLogSQLSteps] '[USP_Avail_Syn_Amend_Old]', 'Step 2', 0, 'Procedure Excution Completed', 0, ''
END