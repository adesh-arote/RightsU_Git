...
Select @BUCode = Business_Unit_Code from Acq_Deal Where Deal_Workflow_Status NOT IN ('AR', 'WA') AND Acq_Deal_Code IN
(
	Select Distinct Acq_Deal_Code from Acq_Deal_Movie  Where Acq_Deal_Movie_Code=ISNULL(@DealMovieCode,0)
)
	IF(@IsDisplay_Date='Y')
	BEGIN
...
IF((Select COUNT(@BUCode) from Business_Unit Where Business_Unit_Code = @BUCode AND RTRIM(LTRIM(Business_Unit_Name)) = 'Hindi Movies')>0)
 BEGIN  
 if(@dBug='D')
	PRINT 'HINDI'
	IF(@IsDisplay_Date='N')
	...
	insert into #ScheduleAsRun_YearWiseDetails_Report
     select 
	   ...
	   from Acq_Deal_Movie ADM 
		inner join Acq_Deal AD on AD.Acq_Deal_Code = ADM.Acq_Deal_Code AND ADM.Acq_Deal_Movie_Code = @DealMovieCode
		inner join Acq_Deal_Run_Title ADRT on ADRT.Title_Code = ADM.Title_Code
		inner join Acq_Deal_Run_Channel ADRC on ADRC.Acq_Deal_Run_Code = ADRT.Acq_Deal_Run_Code AND  ((@ChannelCode <>'' AND ADRc.Channel_Code in (SELECT NUMBER FROM dbo.fn_Split_withdelemiter(@ChannelCode,',')) ) or @ChannelCode ='' )
		inner join Acq_Deal_Run_Yearwise_Run ADYR on ADYR.Acq_Deal_Run_Code = ADRC.Acq_Deal_Run_Code
		WHERE AD.Deal_Workflow_Status NOT IN ('AR', 'WA')
	END
END       
...
/*          
          Select * from Title Where Title_Name Like 'Jab%' --6157
Select * from Title Where Title_Name Like 'skyfall%' --600	4
Select * from Acq_Deal_MOvie Where Title_Code =6004
Select * from Acq_Deal_MOvie Where Title_Code IN(2046,2048)
Select * from Channel
          
EXEC [dbo].[USP_ChannelWiseConsumption] '6157','4,9','N','MOVIE'

EXEC [USP_ChannelWiseConsumption_Yearwise_Sub]  '1746','8','Y','Y','N'
