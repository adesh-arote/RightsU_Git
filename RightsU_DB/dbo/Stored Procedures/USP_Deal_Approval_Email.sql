...
BEGIN
	...
	if(@Loglevel < 2)Exec [USPLogSQLSteps] '[USP_Deal_Approval_Email]', 'Step 1', 0, 'Started Procedure', 0, ''
		 --'AP' = Approved
		 --'DA' = Details Added
		 --'RJ' = Rejected
		...
		SELECT @Email_Config_Code = Email_Config_Code FROM Email_Config (NOLOCK) where [Key] = 'NDA'
		...
		BEGIN
			...
			(
				...
				(
					SELECT  msh.Record_Code,u.Security_Group_Code,Count(*) AS MSHCount FROM Module_Status_History msh  (NOLOCK)
					INNER JOIN Users u (NOLOCK) ON u.Users_Code = msh.Status_Changed_By where CONVERT(DATE,Status_Changed_On)=Convert(date,GETDATE()) AND Status='A' AND msh.Module_Code=30
					AND msh.Record_Code NOT IN
					(
						SELECT h.Record_Code FROM Module_Status_History h (NOLOCK) WHERE CONVERT(DATE,h.Status_Changed_On)!=CONVERT(DATE,GETDATE()) 
						AND Status='A' AND h.Module_Code=30
					) GROUP BY msh.Record_Code,u.Security_Group_Code
				) as tmpMSH  
				INNER JOIN 
				(
					Select mwd.Record_Code AS mwdRecord_Code, mwd.Group_Code AS mwdGroup_Code from Module_Workflow_detail mwd (NOLOCK)
					Inner join 
					(
						select Record_Code,Max(Role_Level) MAx_Role_Level from Module_Workflow_detail (NOLOCK) where Module_Code=30 Group by Record_Code
					) as a 
			 ...
			 where  AD.Deal_Workflow_Status NOT IN ('AR', 'WA') AND ad.Acq_Deal_Code IN(SELECT Acq_Deal_Code FROM Acq_Deal_Rights (NOLOCK) where Is_Sub_License='Y')
			 ) AS finalDeal 
			...
			BEGIN
				...
				--FETCH NEXT FROM curOuter Into @Users_Email_Id
					...
					--SET @RowTitleCodeOld = ''
						...
						select Agreement_No,Deal_Desc,Deal_Type,Agreement_Date,Party
						from #DealDetails
						--Where Business_Unit_Code IN (select DISTINCT UBU.Business_Unit_Code from Users_Business_Unit UBU INNER JOIN Users U on U.Users_Code = UBU.Users_Code
						--where U.Email_Id=@To_User_Mail_Id)
						...
						BEGIN
								...
								INSERT INTO @Email_Config_Users_UDT(Email_Config_Code, Email_Body, To_User_Mail_Id,CC_User_Mail_Id, BCC_User_Mail_Id, [Subject])
								SELECT @Email_Config_Code, @EmailUser_Body, ISNULL(@To_User_Mail_Id ,''), ISNULL(@CC_User_Mail_Id ,''), ISNULL(@BCC_User_Mail_Id ,''), @MailSubjectCr
		...
		BEGIN
				...
				(SELECT count(*)  FROM 
					(
					select  DISTINCT Title_Code from Acq_Deal_Movie (NOLOCK)  WHERE Acq_Deal_Code =  AD.Acq_Deal_Code
					EXCEPT
					SELECT DISTINCT Title_Code FROM Acq_Deal_Rights_Title (NOLOCK) WHERE  Acq_Deal_Rights_Code IN ( SELECT Acq_Deal_Rights_Code FROM Acq_Deal_Rights WHERE Acq_Deal_Code = AD.Acq_Deal_Code)
					) AS A
				) 
					...
					BEGIN
							...
							BEGIN	
								...
								SELECT Agreement_No ,Agreement_Date ,Deal_Desc ,Primary_Vendor, Title_Names ,Deal_Creation_Date ,Deal_Added_Since FROM #DealDetailsAdded
										WHERE Title_Names IS NOT NULL AND  Business_Unit_Code IN ( SELECT Business_Unit_Codes FROM Email_Config_Detail_User ECDU (NOLOCK) INNER JOIN Email_Config_Detail ECD (NOLOCK) ON ECDU.Email_Config_Detail_Code = ECD.Email_Config_Detail_Code INNER JOIN Email_Config EC ON ECD.Email_Config_Code = EC.Email_Config_Code  WHERE ToUser_MailID = @Users_Email_Id AND  EC.[Key] = 'NDA')
				...
				Branch_DetailsAdded_Stage_Two:
					...
					BEGIN
						...
						(SELECT count(*)  FROM 
							(
							select  DISTINCT Title_Code from Syn_Deal_Movie (NOLOCK)  WHERE Syn_Deal_Code =  AD.Syn_Deal_Code
							EXCEPT
							SELECT DISTINCT Title_Code FROM Syn_Deal_Rights_Title (NOLOCK) WHERE  Syn_Deal_Rights_Code IN ( SELECT Syn_Deal_Rights_Code FROM Syn_Deal_Rights (NOLOCK) WHERE Syn_Deal_Code = AD.Syn_Deal_Code)
							) AS A
						) 
						AND ad.Inserted_On < DATEADD(day,@Details_AD_RJ_Stage_Since_Days, GETDATE()) AND ad.Is_Active = 'Y'
	...
	if(@Loglevel < 2)Exec [USPLogSQLSteps] '[USP_Deal_Approval_Email]', 'Step 2', 0, 'Procedure Excution Completed', 0, ''
END