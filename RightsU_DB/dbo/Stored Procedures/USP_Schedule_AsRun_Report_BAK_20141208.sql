...
BEGIN  
	...
	if(@Loglevel<2)Exec [USPLogSQLSteps] '[USP_Schedule_AsRun_Report_BAK_20141208]', 'Step 1', 0, 'Started Procedure', 0, ''
	...
	SET FMTONLY OFF;
		...
		AND D.Deal_Type_Code in (select deal_type_code from Deal_Type (NOLOCK) WHERE deal_type_name = ''Movie'' AND is_active = ''Y'' ) 
		AND (D.is_active = ''Y'') 
	...
	if(@Loglevel<2)Exec [USPLogSQLSteps] '[USP_Schedule_AsRun_Report_BAK_20141208]', 'Step 2', 0, 'Procedure Excuting Completed', 0, ''
END