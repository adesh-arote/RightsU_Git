CREATE PROCEDURE [dbo].[USP_List_Country]
(
@SysLanguageCode INT
)
AS
BEGIN
	...
	if(@Loglevel < 2)Exec [USPLogSQLSteps] '[USP_List_Country]', 'Step 1', 0, 'Started Procedure', 0, ''
	 ...
	 SELECT @No = SLM.Message_Desc FROM System_Message SM (NOLOCK)
		...
		INTO #TempCountry 
		FROM Country C1 (NOLOCK)
		LEFT JOIN Country C2 (NOLOCK) ON C1.Parent_Country_Code = C2.Country_Code

		DECLARE @CountryCode INT = 0


		UPDATE TC SET TC.Language_Names = STUFF((
				SELECT ', ' + CAST(l.Language_Name AS NVARCHAR(1000)) FROM Country_Language CL (NOLOCK)
				INNER JOIN [Language] L (NOLOCK) ON L.Language_Code = CL.Language_Code
	...
	if(@Loglevel < 2)Exec [USPLogSQLSteps] '[USP_List_Country]', 'Step 2', 0, 'Procedure Excution Completed', 0, ''
END