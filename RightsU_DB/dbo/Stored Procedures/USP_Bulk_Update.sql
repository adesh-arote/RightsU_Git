...
BEGIN
	...
	if(@Loglevel < 2)Exec [USPLogSQLSteps] '[USP_Bulk_Update]', 'Step 1', 0, 'Started Procedure', 0, ''
		...
		BEGIN
			...
			BEGIN
				...
				from #temp1 AS t 
				LEFT JOIN Syn_Deal_Rights_Platform AS SDRP (NOLOCK)
			...
			BEGIN
				...
				BEGIN
					...
					select Distinct SDR.Syn_Deal_Rights_Code FROM Syn_Deal_Rights SDR (NOLOCK)
					inner join Syn_Deal_Rights_Title SDRT (NOLOCK) ON SDR.Syn_Deal_Rights_Code=SDRT.Syn_Deal_Rights_Code
					INNER JOIN Syn_Deal_Run RunT (NOLOCK) ON SDRT.Title_Code=RunT.Title_Code
					Inner Join Syn_Deal_Rights_Platform SDRP (NOLOCK) ON  SDRP.Syn_Deal_Rights_Code = SDR.Syn_Deal_Rights_Code 
					Inner JOIN [Platform] p (NOLOCK) ON SDRP.Platform_Code=p.Platform_Code
				...
				(	
					...
					from Syn_Deal_Rights_Platform AS SDRP (NOLOCK)
		...
		BEGIN
			...
			BEGIN
				...
				from #temp1 AS t 
				LEFT JOIN Syn_Deal_Rights_Territory (NOLOCK) AS SDRT 
			...
			BEGIN
				...
				(	
					select 
					Syn_Deal_Rights_Code
					from Syn_Deal_Rights_Territory AS SDRT (NOLOCK)
		...
		BEGIN 
			...
			BEGIN
				...
				(	
					...
					(
						select SD.Syn_Deal_Rights_Code,SD.Territory_Code  from Syn_Deal_Rights_Territory AS SD (NOLOCK) where Syn_Deal_Rights_Code
				...
				from #temp1 AS t 
				LEFT JOIN 
				Syn_Deal_Rights_Territory AS SDRT (NOLOCK)
			...
			BEGIN
				...
				inner join Territory_Details TD (NOLOCK) ON TD.Territory_Code=SDRT.Territory_Code
				inner join Syn_Deal_Rights_Holdback SDRH (NOLOCK) ON SDRH.Syn_Deal_Rights_Code=SDR.Syn_Deal_Rights_Code 
				inner join Syn_Deal_Rights_Holdback_Territory SDRHT (NOLOCK) ON SDRHT.Syn_Deal_Rights_Holdback_Code=SDRH.Syn_Deal_Rights_Holdback_Code 
				where SDRH.Syn_Deal_Rights_Code IN(select number FROM dbo.[fn_Split_withdelemiter](@Right_Codes,',')) 
				...
				(	
					...
					from Syn_Deal_Rights_Territory AS SDRP (NOLOCK)
		...
		BEGIN 
			...
			BEGIN
				...
				from #temp1 AS t 
				LEFT JOIN Syn_Deal_Rights_Subtitling AS SDRS  (NOLOCK)
		...
		BEGIN 
			...
			BEGIN
				...
				(
					...
					(SELECT LGD.Language_Code FROM Language_Group_Details LGD (NOLOCK) WHERE LGD.Language_Group_Code
				...
				(	
					...
					(
						select SD.Syn_Deal_Rights_Code,SD.Language_Group_Code  from Syn_Deal_Rights_Subtitling AS SD (NOLOCK) where Syn_Deal_Rights_Code
						IN(select number FROM dbo.[fn_Split_withdelemiter](@Right_Codes,','))
					)AS SDT
				...
				from #temp1 AS t 
				LEFT JOIN 
				Syn_Deal_Rights_Subtitling AS SDRS  (NOLOCK)
			...
			BEGIN
				INSERT INTO #RCodeForHB(Right_Code)
				select Distinct SDR.Syn_Deal_Rights_Code FROM Syn_Deal_Rights SDR (NOLOCK)
				inner join Syn_Deal_Rights_Subtitling SDRT (NOLOCK) ON SDRT.Syn_Deal_Rights_Code=SDR.Syn_Deal_Rights_Code
				inner join Language_Group_Details TD (NOLOCK) ON TD.Language_Group_Code=SDRT.Language_Group_Code
				inner join Syn_Deal_Rights_Holdback SDRH (NOLOCK) ON SDRH.Syn_Deal_Rights_Code=SDR.Syn_Deal_Rights_Code 
				inner join Syn_Deal_Rights_Holdback_Subtitling SDRHT (NOLOCK) ON SDRHT.Syn_Deal_Rights_Holdback_Code=SDRH.Syn_Deal_Rights_Holdback_Code 
				where SDRH.Syn_Deal_Rights_Code IN(select number FROM dbo.[fn_Split_withdelemiter](@Right_Codes,',')) 
		...
		BEGIN
			...
			BEGIN
				...
				LEFT JOIN Syn_Deal_Rights_Dubbing AS SDRS  (NOLOCK)
				ON SDRS.Syn_Deal_Rights_Code = t.R_Code AND SDRS.Language_Code = t.I_Code
			...
			BEGIN
				...
				select Distinct SDR.Syn_Deal_Rights_Code FROM Syn_Deal_Rights SDR  (NOLOCK)
				inner join Syn_Deal_Rights_Holdback SDRH (NOLOCK) ON SDRH.Syn_Deal_Rights_Code=SDR.Syn_Deal_Rights_Code 
				inner join Syn_Deal_Rights_Holdback_Dubbing SDRHS (NOLOCK) ON SDRHS.Syn_Deal_Rights_Holdback_Code=SDRH.Syn_Deal_Rights_Holdback_Code 
				where SDRH.Syn_Deal_Rights_Code IN(select number FROM dbo.[fn_Split_withdelemiter](@Right_Codes,',')) 
				AND SDRHS.Language_Code IN(select number FROM dbo.[fn_Split_withdelemiter](@Codes,','))
		...
		BEGIN
			...
			BEGIN
				...
				(
					...
					select SDT.Syn_Deal_Rights_Code,TDD.Language_Code from
					(
						select SD.Syn_Deal_Rights_Code,SD.Language_Group_Code  from Syn_Deal_Rights_Dubbing AS SD (NOLOCK) where Syn_Deal_Rights_Code
						IN(select number FROM dbo.[fn_Split_withdelemiter](@Right_Codes,','))
					)AS SDT
					inner join Language_Group_Details AS TDD (NOLOCK) ON SDT.Language_Group_Code=TDD.Language_Group_Code
			...
			BEGIN
				INSERT INTO #RCodeForHB(Right_Code)
				select Distinct SDR.Syn_Deal_Rights_Code FROM Syn_Deal_Rights SDR (NOLOCK)
				inner join Syn_Deal_Rights_Dubbing SDRT (NOLOCK) ON SDRT.Syn_Deal_Rights_Code=SDR.Syn_Deal_Rights_Code
				inner join Language_Group_Details TD (NOLOCK) ON TD.Language_Group_Code=SDRT.Language_Group_Code
				inner join Syn_Deal_Rights_Holdback SDRH (NOLOCK) ON SDRH.Syn_Deal_Rights_Code=SDR.Syn_Deal_Rights_Code 
				inner join Syn_Deal_Rights_Holdback_Dubbing SDRHT (NOLOCK) ON SDRHT.Syn_Deal_Rights_Holdback_Code=SDRH.Syn_Deal_Rights_Holdback_Code 
				where SDRH.Syn_Deal_Rights_Code IN(select number FROM dbo.[fn_Split_withdelemiter](@Right_Codes,',')) 
				AND SDRHT.Language_Code IN(select number FROM dbo.[fn_Split_withdelemiter](@Codes,',')) AND SDRHT.Language_Code=TD.Language_Code
		...
		BEGIN
			...
			WHILE @@FETCH_STATUS = 0
			BEGIN
				if(EXISTS(select * from Syn_Deal_Rights (NOLOCK) where Syn_Deal_Rights_Code=@R_CODE AND (Right_Start_Date!=@Start_Date OR Right_End_Date != @End_Date 
				...
				--Can not change Tentative state as Run Definition is already added. To change rights period, delete Run Definition first.
					...
					select DISTINCT SDR.Syn_Deal_Rights_Code FROM Syn_Deal_Rights SDR (NOLOCK)
					inner join Syn_Deal_Run SDRu (NOLOCK) ON SDR.Syn_Deal_Code=SDRu.Syn_Deal_Code 
					inner join Syn_Deal_Rights_Title SDRT (NOLOCK) ON SDR.Syn_Deal_Rights_Code=SDRT.Syn_Deal_Rights_Code AND SDRT.Title_Code=SDRu.Title_Code
			...
			BEGIN
				...
				STUFF((
					...
					(
						...
						FROM Syn_Deal_Rights_Platform (NOLOCK) 
				...
				REVERSE(
						STUFF(REVERSE(STUFF((
							Select   CASE WHEN Language_Type = 'G' THEN lg.Language_Group_Name+', '
							WHEN Language_Type = 'L' THEN l.Language_Name+', ' END
							from Syn_Deal_Rights_Dubbing adrs (NOLOCK)
							LEFT JOIN Language_Group lg (NOLOCK) ON lg.Language_Group_Code = adrs.Language_Group_Code
							LEFT JOIN [Language] l (NOLOCK) ON l.Language_Code = adrs.Language_Code
							where adrs.Syn_Deal_Rights_Code In (@RC)
				...
				(
					(
						...
						Where t.Syn_Deal_Code In (
							Select adr.Syn_Deal_Code FROM Syn_Deal_Rights adr (NOLOCK) where adr.Syn_Deal_Rights_Code=@RC
					)FOR XML PATH('')), 1, 1, ''
				) as Agreement_No
				From (
					Select T.Title_Code,
					...
					INNER JOIN Title T (NOLOCK) ON T.Title_Code=ADRT.Title_Code where ADR.Syn_Deal_Rights_Code=@RC
	...
	if(@Loglevel < 2)Exec [USPLogSQLSteps] '[USP_Bulk_Update]', 'Step 2', 0, 'Procedure Excution Completed', 0, ''
END