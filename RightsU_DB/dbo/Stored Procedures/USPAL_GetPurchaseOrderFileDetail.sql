CREATE PROCEDURE [dbo].[USPAL_GetPurchaseOrderFileDetail]
(
...
BEGIN
	...
	IF OBJECT_ID('tempdb..#TempFileDetail') IS NOT NULL DROP TABLE #TempFileDetail
	...
	CREATE TABLE #TempFileDetail
	(
		TitleName	NVARCHAR(MAX),	
		FileDetail NVARCHAR(MAX),
		Row_Num INT
	)
	...
	DECLARE @Distributor NVARCHAR(MAX) = ''
		...
		INSERT INTO #TempFileDetail (TitleName, FileDetail, Row_Num)
		SELECT [TitleName], [File details], ROW_NUMBER() OVER(ORDER BY [TitleName] ASC) AS Row_Num
		FROM #TempPurchaseOrderData tpod 
		INNER JOIN
		(SELECT * FROM ( SELECT tb.AL_Booking_Sheet_Code, tb.TitleName, tb.Columns_Name, tb.Columns_Value, tb.Title_Code, tb.Title_Content_Code, tb.EpisodeNo, tb.EpisodeTitle FROM #TempBookingData tb ) a
		...
		SELECT FileDetail AS FileDetailRemarks FROM		
		(SELECT DISTINCT FileDetail FROM #TempFileDetail WHERE ISNULL(FileDetail,'')<>'') AS T
