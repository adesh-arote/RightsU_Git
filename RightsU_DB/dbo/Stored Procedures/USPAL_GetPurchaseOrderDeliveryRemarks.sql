CREATE PROCEDURE [dbo].[USPAL_GetPurchaseOrderDeliveryRemarks]
(
...
BEGIN
	...
	IF OBJECT_ID('tempdb..#TempDeliveryRemarksData') IS NOT NULL DROP TABLE #TempDeliveryRemarksData
	...
	CREATE TABLE #TempDeliveryRemarksData
	(
		TitleName	NVARCHAR(MAX),
		MasterDeliveryDate NVARCHAR(MAX),	
		DeliveryPaymentRemarks NVARCHAR(MAX),
		Row_Num INT
	)
	...
	DECLARE @AL_Booking_Sheet_Code INT = 0, @Title_Code INT = 0, @Title_Content_Code INT = 0 , @Total_OEM_Count INT = 0;
	...
	DECLARE @Distributor NVARCHAR(MAX) = ''
		...
		INSERT INTO #TempDeliveryRemarksData (TitleName, MasterDeliveryDate, DeliveryPaymentRemarks, Row_Num)
		SELECT [TitleName],[Master delivery date], [Delivery and Payment remarks], ROW_NUMBER() OVER(ORDER BY [TitleName] ASC) AS Row_Num
		FROM #TempPurchaseOrderData tpod 
		INNER JOIN
		(SELECT * FROM ( SELECT tb.AL_Booking_Sheet_Code, tb.TitleName, tb.Columns_Name, tb.Columns_Value, tb.Title_Code, tb.Title_Content_Code, tb.EpisodeNo, tb.EpisodeTitle FROM #TempBookingData tb ) a
  ...
  --      SELECT CAST(RowNo AS NVARCHAR(10)) + '. ' + DeliveryPaymentRemarks AS DeliveryPaymentRemarks FROM
		--(SELECT 'Dellivery Date - ' + (SELECT MAX(MasterDeliveryDate) FROM #TempDeliveryRemarksData) AS DeliveryPaymentRemarks, 1 AS RowNo
		--UNION 
		--SELECT DISTINCT DeliveryPaymentRemarks, ROW_NUMBER() OVER(ORDER BY [TitleName] ASC) + 1 AS RowNo FROM #TempDeliveryRemarksData) AS T
		--ORDER BY RowNo ASC

		SELECT DeliveryPaymentRemarks AS DeliveryPaymentRemarks FROM
		(SELECT '1. Delivery Date - ' + (SELECT MAX(MasterDeliveryDate) FROM #TempDeliveryRemarksData) AS DeliveryPaymentRemarks, 1 AS RowNo
		UNION 
		SELECT DISTINCT DeliveryPaymentRemarks, 2 AS RowNo FROM #TempDeliveryRemarksData) AS T
		ORDER BY RowNo ASC
