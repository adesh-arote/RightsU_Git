CREATE PROCEDURE [dbo].[USP_MIGRATE_TO_NEW]
(	
	@DEALCODE INT=0,
	@dBug CHAR(1)='N'
)
...
BEGIN
	...
	if(@Loglevel < 2)Exec [USPLogSQLSteps] '[USP_MIGRATE_TO_NEW]', 'Step 1', 0, 'Started Procedure', 0, '' 
	...
	BEGIN TRAN
		...
		select top 1 @Business_unit_code =Business_Unit_Code from Business_Unit (NOLOCK)
		--ROLLBACK TRAN
		--RETURN
	...
	BEGIN TRY
		...
		*/
		...
		FROM Rightsu_vmpl_live_27March_2015.dbo.deal  D (NOLOCK)
		Inner Join Rightsu_vmpl_live_27March_2015.dbo.Vendor v (NOLOCK) On d.customer_code = v.vendor_code
			Inner Join Vendor vNew (NOLOCK) On vNew.Vendor_Name = v.vendor_name
		---Category , Currency,Payment Terms, Entity---
		Inner join Rightsu_vmpl_live_27March_2015.dbo.Category c (NOLOCK) on d.category_code=c.category_code
		inner join Category cNew (NOLOCK) on cNew.Category_Name=c.category_name
		inner join Rightsu_vmpl_live_27March_2015.dbo.Currency curr (NOLOCK) on d.currency_code=curr.currency_code
		inner join Currency currNew (NOLOCK) on currNew.Currency_Name=curr.currency_name
		inner join Rightsu_vmpl_live_27March_2015.dbo.Deal_Type dt (NOLOCK) on d.deal_for_code=dt.deal_type_code
		inner join Deal_Type dtNew (NOLOCK) on dtNew.Deal_Type_Name=dt.Deal_Type_Name
		---Category , Currency,Payment Terms---
		WHERE d.deal_code=@DEALCODE

		/*Update Content Type*/	
		Update Acq_Deal
		SET Role_Code = (Select Role_Code FROM Role (NOLOCk) WHERE Role_Name = 'Assignment')
		WHERE Content_Type = 'A'

		Update Acq_Deal
		SET Role_Code = (Select Role_Code FROM Role (NOLOCK) WHERE Role_Name = 'License')
		WHERE Content_Type = 'L'

		Update Acq_Deal
		SET Role_Code = (Select Role_Code FROM Role (NOLOCK) WHERE Role_Name = 'Own Production')
		...
		*/
		...
		FROM Rightsu_vmpl_live_27March_2015.dbo.deal_movie dm (NOLOCK)
		inner join Rightsu_vmpl_live_27March_2015.dbo.deal d (NOLOCK) on d.deal_code=dm.deal_code
		Inner Join Rightsu_vmpl_live_27March_2015.dbo.Title tit (NOLOCK) on dm.title_code = tit.title_code  
		Inner Join Title titNew (NOLOCK) On tit.english_title = titNew.Title_Name and 
		...
		*/
		...
		FROM Rightsu_vmpl_live_27March_2015.dbo.Deal_Licensor D (NOLOCK)
		Inner Join Rightsu_vmpl_live_27March_2015.dbo.Vendor v (NOLOCK) On d.licensor_customer_code = v.vendor_code
		Inner Join Vendor vNew (NOLOCK) On vNew.Vendor_Name = v.vendor_name
		...
		SELECT 
			...
			dpt.Last_Action_By
		FROM Rightsu_vmpl_live_27March_2015.dbo.Deal_Payment_Terms dpt (NOLOCK)
		Inner join Rightsu_vmpl_live_27March_2015.dbo.Payment_Terms p (NOLOCK) on dpt.payment_terms_code=p.payment_terms_code
		inner join Payment_Terms pNew (NOLOCK) on pNew.Payment_Terms=p.Payment_Terms
		inner join Rightsu_vmpl_live_27March_2015.dbo.Cost_Type c (NOLOCK) on c.cost_type_code=dpt.Cost_Type_Code
		...
		*/
		...
		FROM Rightsu_vmpl_live_27March_2015.dbo.Deal_Movie DM (NOLOCK)
		INNER JOIN Rightsu_vmpl_live_27March_2015.dbo.Deal_Movie_Rights DMR (NOLOCK) ON DM.deal_movie_code=DMR.deal_movie_code 
		INNER JOIN Rightsu_vmpl_live_27March_2015.dbo.Deal_Movie_Rights_Territory DMRT (NOLOCK) ON DMR.deal_movie_rights_code=DMRT.deal_movie_rights_code 
		...
		FROM Rightsu_vmpl_live_27March_2015.dbo.Deal_Movie DM (NOLOCK)
			INNER JOIN Rightsu_vmpl_live_27March_2015.dbo.Deal_Movie_Rights DMR (NOLOCK) ON DM.deal_movie_code=DMR.deal_movie_code 
		INNER JOIN Rightsu_vmpl_live_27March_2015.dbo.Deal_Movie_Rights_Territory DMRT (NOLOCK) ON DMR.deal_movie_rights_code=DMRT.deal_movie_rights_code 
		left join Rightsu_vmpl_live_27March_2015.dbo.International_Territory it (NOLOCK) on it.international_territory_code=dmrt.territory_code
		left join Country c (NOLOCK) on c.Country_Name=it.international_territory_name
		left join Rightsu_vmpl_live_27March_2015.dbo.territory_group tg (NOLOCK) on tg.territory_group_code=dmrt.territory_group_code
		left join Territory t (NOLOCK) on t.Territory_Name=tg.territory_group_name
		WHERE DM.deal_code=@DEALCODE	
		...
		FROM Rightsu_vmpl_live_27March_2015.dbo.Deal_Movie DM (NOLOCK)
		INNER JOIN Rightsu_vmpl_live_27March_2015.dbo.Deal_Movie_Rights DMR (NOLOCK) ON DM.deal_movie_code=DMR.deal_movie_code 
		INNER JOIN Rightsu_vmpl_live_27March_2015.dbo.Deal_Movie_Rights_Subtitling DMRS (NOLOCK) ON DMR.deal_movie_rights_code=DMRS.deal_movie_rights_code 
		left join Rightsu_vmpl_live_27March_2015.dbo.Language l (NOLOCK) on l.language_code=dmrs.language_code
		left join Language lnew (NOLOCK) on lNew.Language_Name=l.Language_Name
		Left join Rightsu_vmpl_live_27March_2015.dbo.Language_Group lg (NOLOCK) on lg.Language_Group_Code=dmrs.Language_Group_Code
		...
		FROM Rightsu_vmpl_live_27March_2015.dbo.Deal_Movie DM (NOLOCK)
		INNER JOIN Rightsu_vmpl_live_27March_2015.dbo.Deal_Movie_Rights DMR (NOLOCK) ON DM.deal_movie_code=DMR.deal_movie_code 
		INNER JOIN Rightsu_vmpl_live_27March_2015.dbo.Deal_Movie_Rights_Dubbing DMRD (NOLOCK) ON DMR.deal_movie_rights_code=DMRD.deal_movie_rights_code 
		left join Rightsu_vmpl_live_27March_2015.dbo.Language l (NOLOCK) on l.language_code=dmrd.language_code
		left join Language lnew (NOLOCK) on lNew.Language_Name=l.Language_Name
		Left join Rightsu_vmpl_live_27March_2015.dbo.Language_Group lg (NOLOCK) on lg.Language_Group_Code=dmrd.Language_Group_Code
		Left join Language_Group lgNew (NOLOCK) on lgNew.Language_Group_Name=lg.Language_Group_Name
		LEFT JOIN Language_Group_Details LGD (NOLOCK) ON lgNew.Language_Group_Code=LGD.Language_Group_Code
		WHERE DM.deal_code=@DEALCODE	
		...
		BEGIN -- REGION RUN Groupwise Run / Title / Channel / Repeat On Day / Yearwise Run
			...
			FROM Rightsu_vmpl_live_27March_2015.dbo.Deal_Movie dm (NOLOCK)
			inner join Rightsu_vmpl_live_27March_2015.dbo.Deal d (NOLOCK) on d.deal_code=dm.deal_code
			Inner Join Rightsu_vmpl_live_27March_2015.dbo.Title tit1 (NOLOCK) on dm.title_code = tit1.title_code  
			Inner Join Title titNew (NOLOCK) On tit1.english_title = titNew.Title_Name and titNew.deal_type_code=tit1.dealtype_code
			INNER JOIN Rightsu_vmpl_live_27March_2015.dbo.Deal_Movie_Rights DMR (NOLOCK) ON DM.deal_movie_code=DMR.deal_movie_code 
			INNER JOIN Rightsu_vmpl_live_27March_2015.dbo.Deal_Movie_Rights_Run DMRR (NOLOCK) ON DMR.deal_movie_rights_code=DMRR.deal_movie_rights_code
			WHERE DM.deal_code=@DEALCODE And titNew.Title_Code Not In (
				Select tit.Title_Code From DM_Runs_Allocation dmr (NOLOCK)
				Inner Join Title tit (NOLOCK) On dmr.Title = tit.Title_Name 
			...
			INNER JOIN Rightsu_vmpl_live_27March_2015.dbo.Deal_Movie_Rights DMR (NOLOCK) ON DM.deal_movie_code=DMR.deal_movie_code 
			INNER JOIN Rightsu_vmpl_live_27March_2015.dbo.Deal_Movie_Rights_Run DMRR (NOLOCK) ON DMR.deal_movie_rights_code=DMRR.deal_movie_rights_code
			INNER JOIN Rightsu_vmpl_live_27March_2015.dbo.Deal_Movie_Rights_Run_Channel DMRRC (NOLOCK) ON DMRRC.deal_movie_rights_run_code=DMRR.deal_movie_rights_run_code
			left join Rightsu_vmpl_live_27March_2015.dbo.Channel c (NOLOCK) on c.channel_code=DMRRC.channel_code
			left join Channel cNew (NOLOCK) on c.channel_name=cnew.channel_name
			WHERE DM.deal_code=@DEALCODE
		 ...
		 ORDER BY run_definition_group_code,channel_code
			...
			INNER JOIN Rightsu_vmpl_live_27March_2015.dbo.Deal_Movie_Rights DMR (NOLOCK) ON DM.deal_movie_code=DMR.deal_movie_code 
			INNER JOIN Rightsu_vmpl_live_27March_2015.dbo.Deal_Movie_Rights_Run DMRR (NOLOCK) ON DMR.deal_movie_rights_code=DMRR.deal_movie_rights_code
			INNER JOIN Rightsu_vmpl_live_27March_2015.dbo.Deal_Movie_Rights_Run_Repeat_On_Day DMRRC (NOLOCK) ON DMRRC.deal_movie_rights_run_code=DMRR.deal_movie_rights_run_code
			WHERE DM.deal_code=@DEALCODE
		 GROUP BY DMRR.run_definition_group_code,Day_Code
			...
			INNER JOIN Rightsu_vmpl_live_27March_2015.dbo.Deal_Movie_Rights DMR (NOLOCK) ON DM.deal_movie_code=DMR.deal_movie_code 
			INNER JOIN Rightsu_vmpl_live_27March_2015.dbo.Deal_Movie_Rights_Run DMRR (NOLOCK) ON DMR.deal_movie_rights_code=DMRR.deal_movie_rights_code
			INNER JOIN Rightsu_vmpl_live_27March_2015.dbo.Deal_Movie_Rights_Run_Yearwise_Run DMRRC (NOLOCK) ON DMRRC.deal_movie_rights_run_code=DMRR.deal_movie_rights_run_code
		WHERE DM.deal_code=@DEALCODE
		...
		BEGIN -- REGION Ancillary Groupwise Title / Platform / Platform_Medium
			...
			*/
			...
			FROM Rightsu_vmpl_live_27March_2015.dbo.Deal_Movie_Ancillary DMA (NOLOCK)
			INNER JOIN  Rightsu_vmpl_live_27March_2015.dbo.Deal_Movie DM (NOLOCK) ON DM.deal_movie_code=DMA.deal_movie_code
			INNER JOIN  Rightsu_vmpl_live_27March_2015.dbo.Deal D (NOLOCK) ON D.deal_code=DM.deal_code
			Inner Join Rightsu_vmpl_live_27March_2015.dbo.Title tit1 (NOLOCK) on dm.title_code = tit1.title_code 
			Inner Join Title titNew (NOLOCK) On tit1.english_title = titNew.Title_Name 
			and ((d.deal_type='M' and titnew.Reference_Flag is null) or (d.deal_type='S' and titnew.Reference_Flag ='T'))--titNew.deal_type_code=tit.dealtype_code
			WHERE dm.deal_code=@DealCode
			...
			FROM Rightsu_vmpl_live_27March_2015.dbo.Deal_Movie_Ancillary_Medium DMAM (NOLOCK)
			INNER JOIN Rightsu_vmpl_live_27March_2015.dbo.Deal_Movie_Ancillary DMA (NOLOCK) ON DMA.Deal_Movie_Ancillary_Code=DMAM.Deal_Movie_Ancillary_Code
			INNER JOIN  Rightsu_vmpl_live_27March_2015.dbo.Deal_Movie DM (NOLOCK) ON DM.deal_movie_code=DMA.deal_movie_code 
			WHERE deal_code=@DealCode
		...
		SELECT DISTINCT AP.Ancillary_Type_code,Duration,ISNULL([Day],0),ISNULL(Remarks,''),Group_No 
			FROM Rightsu_vmpl_live_27March_2015.dbo.Deal_Movie_Ancillary DMA (NOLOCK)
			INNER JOIN  Rightsu_vmpl_live_27March_2015.dbo.Ancillary_Platform AP (NOLOCK) ON AP.Ancillary_Platform_code=DMA.Ancillary_Platform_code
			INNER JOIN  Rightsu_vmpl_live_27March_2015.dbo.Deal_Movie DM (NOLOCK) ON DM.deal_movie_code=DMA.deal_movie_code 
			WHERE deal_code=@DEALCODE
		OPEN CUR_DMA_ADA	
		...
		BEGIN
			...
			BEGIN
				 ...
				 */
				 ...
				 INNER JOIN Acq_Deal_Ancillary_Platform DB (NOLOCK) ON DB.Ancillary_Platform_code=TEMP.Ancillary_Platform_code 
		...
		--if(
			...
			Begin
				...
				FOR 
					...
					INNER JOIN Rightsu_vmpl_live_27March_2015.dbo.Deal_Movie_Rights_Run DMRR (NOLOCK) ON DMR.deal_movie_rights_code=DMRR.deal_movie_rights_code
					WHERE DM.deal_code=@DEALCODE and titNew.Title_Code Not In (
						Select Title_Code From DM_Runs_Allocation dmr  (NOLOCK)
						Inner Join Title tit (NOLOCK) On dmr.Title = tit.Title_Name 
						Where dmr.[Acquisition Deal No] = @Excel_Deal_Code
					)
					GROUP BY
				...
				FOR
					SELECT distinct
						STUFF(
							(
								Select Distinct ',' + CAST(DMRR1.run_definition_group_code as Varchar) From Rightsu_vmpl_live_27March_2015.dbo.Deal_Movie_Rights_Run DMRR1
								Inner Join Rightsu_vmpl_live_27March_2015.dbo.Deal_Movie_Rights DMR1 (NOLOCK) ON DMR1.deal_movie_rights_code=DMRR1.deal_movie_rights_code
								Inner Join Rightsu_vmpl_live_27March_2015.dbo.Deal_Movie DM1 (NOLOCK) ON DMR1.deal_movie_code=DM1.deal_movie_code 
										   And DM1.deal_code = @DEALCODE And IsNull(DMR1.right_start_date, GetDate()) = IsNull(DMR.right_start_date, getdate()) 
										   And IsNull(DMR.right_end_date, getdate()) = isnull(DMR1.right_end_date, getdate())
								FOR XML PATH('')
							), 1, 1, ''
		...
		FROM (
			Select * From Rightsu_vmpl_live_27March_2015.dbo.DEAL_MOvie_Rights r (NOLOCK) WHERE DEAL_MOvie_Code in
			(
				SELECT DEAL_MOvie_Code FROM Rightsu_vmpl_live_27March_2015.dbo.Deal_Movie (NOLOCK) WHERE deal_code=@DEALCODE
			) 
		) as a
		inner join Rightsu_vmpl_live_27March_2015.dbo.DEAL_Movie dm  (NOLOCK) on dm.deal_movie_code=a.deal_movie_code
		inner join Rightsu_vmpl_live_27March_2015.dbo.DEAL d (NOLOCK) on dm.deal_code=d.deal_code
		Left Join (
			Select Distinct Max(Deal_Movie_Rights_Restriction_Remark) Deal_Movie_Rights_Restriction_Remark, dmr1.Group_No From Rightsu_vmpl_live_27March_2015.dbo.Deal_Movie_Rights_Restriction_Remark dmrrr (NOLOCK)
			Inner Join Rightsu_vmpl_live_27March_2015.dbo.Deal_Movie_Rights_Restriction_Remark_Details dmrrrd (NOLOCK) On dmrrr.Deal_Movie_Rights_Restriction_Remark_Code = dmrrrd.Deal_Movie_Rights_Restriction_Remark_Code
			Inner Join Rightsu_vmpl_live_27March_2015.dbo.Deal_Movie_Rights dmr1 (NOLOCK) On dmr1.deal_movie_rights_code = dmrrrd.Deal_Movie_Rights_Code And dmr1.DEAL_MOvie_Code in
			(
				SELECT DEAL_MOvie_Code FROM Rightsu_vmpl_live_27March_2015.dbo.Deal_Movie (NOLOCK) WHERE deal_code=@DEALCODE
			) 
			...
			BEGIN
				...
				*/
				...
				FOR  select dmrh.Holdback_Type,
					 ...
					 FROM Rightsu_vmpl_live_27March_2015.dbo.deal_movie DM (NOLOCK)
					 INNER JOIN Rightsu_vmpl_live_27March_2015.dbo.deal_movie_rights DMR (NOLOCK) ON DM.deal_movie_code=DMR.deal_movie_code 
					 INNER JOIN Rightsu_vmpl_live_27March_2015.dbo.Deal_Movie_Rights_Holdback DMRH (NOLOCK) ON DMR.deal_movie_rights_code=DMRH.Deal_movie_rights_code 
				...
				BEGIN
					...
					BEGIN
						...
						IF EXISTS (SELECT TOP 1 Acq_Deal_Rights_Holdback_code from Acq_Deal_Rights_Holdback (NOLOCK) where 
						...
						BEGIN
							...
							SELECT top 1 @Acq_Deal_Rights_Holdback_code=@Acq_Deal_Rights_HoldBack_Code from Acq_Deal_Rights_Holdback (NOLOCK) where 
							Acq_Deal_Rights_Code=@Acq_Deal_Rights_Code and Holdback_Type=@Holdback_Type and HB_Run_After_Release_No=@HB_Run_After_Release_No  and 
							HB_Run_After_Release_Units=@HB_Run_After_Release_Units and isnull(Holdback_On_Platform_Code,0)=isnull(@Holdback_On_Platform_Code,0) and 
						...
						Rightsu_vmpl_live_27March_2015.dbo.Deal_Movie_Rights_HoldBack_Languages dmrhl  (NOLOCK)
						inner join Rightsu_vmpl_live_27March_2015.dbo.Language l (NOLOCK) on l.language_code=dmrhl.language_code
						inner join Language lnew (NOLOCK) on l.language_name=lnew.language_name
						...
						from 
						Rightsu_vmpl_live_27March_2015.dbo.Deal_Movie_Rights_HoldBack_Territory dmrht (NOLOCK)
						...
						left join Rightsu_vmpl_live_27March_2015.dbo.territory_group tg (NOLOCK) on tg.territory_group_code=dmrht.territory_code
						left join Territory t (NOLOCK) on t.Territory_Name=tg.territory_group_name
						left join Territory_Details td (NOLOCK) on t.Territory_code=td.Territory_code
						left join Country c (NOLOCK) on c.country_code=td.country_code
		...
		BEGIN
			...
			BEGIN
				...
				FROM Rightsu_vmpl_live_27March_2015.dbo.Deal_Rights_RHB_Title drrt (NOLOCK)
				inner join Rightsu_vmpl_live_27March_2015.dbo.Deal_Rights_RHB drr (NOLOCK) on drr.Deal_Rights_RHB_Code=drrt.Deal_Rights_RHB_Code
				inner join Rightsu_vmpl_live_27March_2015.dbo.Deal d (NOLOCK) on d.deal_code=drr.deal_code
				Inner Join Rightsu_vmpl_live_27March_2015.dbo.Title tit (NOLOCK) on drrt.title_code = tit.title_code  
				Inner Join Title titNew  (NOLOCK) On tit.english_title = titNew.Title_Name 
				and ((d.deal_type='M' and titnew.Reference_Flag is null) or (d.deal_type='S' and titnew.Reference_Flag ='T'))--titNew.deal_type_code=tit.dealtype_code
				...
				FROM Rightsu_vmpl_live_27March_2015.dbo.Deal_Rights_RHB_Platform drrp (NOLOCK)
				...
				FROM Rightsu_vmpl_live_27March_2015.dbo.Deal_Rights_RHB_Territory drht (NOLOCK)
				left join Rightsu_vmpl_live_27March_2015.dbo.International_Territory it (NOLOCK) on it.international_territory_code=drht.International_Territory_Code
				left join Country c (NOLOCK) on c.Country_Name=it.international_territory_name
				left join Rightsu_vmpl_live_27March_2015.dbo.territory_group tg (NOLOCK) on tg.territory_group_code=drht.territory_group_code
				left join Territory t (NOLOCK) on t.Territory_Name=tg.territory_group_name
				WHERE Deal_Rights_RHB_Code=@Deal_Rights_RHB_Code_cur
				...
				FROM Rightsu_vmpl_live_27March_2015.dbo.Deal_Rights_RHB_Subtitling drrs (NOLOCK)
				Left join Rightsu_vmpl_live_27March_2015.dbo.Language l (NOLOCK) on l.language_code=drrs.language_code
				Left join Language lnew (NOLOCK) on lNew.Language_Name=l.Language_Name
				Left join Rightsu_vmpl_live_27March_2015.dbo.Language_Group lg (NOLOCK) on lg.Language_Group_Code=drrs.Language_Group_Code
				Left join Language_Group lgNew (NOLOCK) on lgNew.Language_Group_Name=lg.Language_Group_Name
				LEFT JOIN Language_Group_Details LGD (NOLOCK) ON lgNew.Language_Group_Code=LGD.Language_Group_Code
				WHERE Deal_Rights_RHB_Code=@Deal_Rights_RHB_Code_cur
				...
				SELECT 				
					...
					,Case When lgnew.Language_Group_Code > 0 Then lgnew.Language_Group_Code Else Null End
				FROM Rightsu_vmpl_live_27March_2015.dbo.Deal_Rights_RHB_Dubbing drrd (NOLOCK)
				Left join Rightsu_vmpl_live_27March_2015.dbo.Language l (NOLOCK) on l.language_code=drrd.language_code
				Left join Language lnew (NOLOCK) on lNew.Language_Name=l.Language_Name
				Left join Rightsu_vmpl_live_27March_2015.dbo.Language_Group lg (NOLOCK) on lg.Language_Group_Code=drrd.Language_Group_Code
				Left join Language_Group lgNew (NOLOCK) on lgNew.Language_Group_Name=lg.Language_Group_Name
				LEFT JOIN Language_Group_Details LGD (NOLOCK) ON lgNew.Language_Group_Code=LGD.Language_Group_Code
				WHERE Deal_Rights_RHB_Code=@Deal_Rights_RHB_Code_cur
		...
		Inner Join Title titNew (NOLOCK) On tit.english_title = titNew.Title_Name 
		and ((d.deal_type='M' and titnew.Reference_Flag is null) or (d.deal_type='S' and titnew.Reference_Flag ='T'))--titNew.deal_type_code=tit.dealtype_code
		INNER JOIN Rightsu_vmpl_live_27March_2015.dbo.deal_movie_cost DMC (NOLOCK) ON DM.deal_movie_code=DMC.deal_movie_code
		WHERE DM.deal_code=@DEALCODE
		...
		FROM Rightsu_vmpl_live_27March_2015.dbo.deal_movie_cost dmc (NOLOCK)
		inner join Rightsu_vmpl_live_27March_2015.dbo.deal_movie dm (NOLOCK) on dm.deal_movie_code=dmc.deal_movie_code
		inner join Rightsu_vmpl_live_27March_2015.dbo.deal d (NOLOCK) on d.deal_code=dm.deal_code
		inner join Rightsu_vmpl_live_27March_2015.dbo.Currency curr (NOLOCK) on dmc.currency_code=curr.currency_code
		inner join Currency currNew (NOLOCK) on currNew.Currency_Name=curr.currency_name
		left join Rightsu_vmpl_live_27March_2015.dbo.Royalty_Recoupment rr (NOLOCK) on rr.royalty_recoupment_code=dmc.royalty_recoupment_code
		left join Royalty_Recoupment rrNew (NOLOCK) on rrNew.Royalty_Recoupment_Name=rr.Royalty_Recoupment_Name
		left join Rightsu_vmpl_live_27March_2015.dbo.Cost_Center cc (NOLOCK) on cc.cost_center_id=dmc.cost_center_id
		left join Cost_Center ccNew (NOLOCK) on ccNew.Cost_Center_Name=cc.Cost_Center_Name
		WHERE d.deal_code=@DEALCODE
		group by 
			currNew.currency_code,
			dmc.exchange_rate,
		...
		BEGIN
			...
			BEGIN
				...
				--print 'ADD Deal_Movie_Cost Title'			
				 ...
				 from   Rightsu_vmpl_live_27March_2015.dbo.deal_movie_cost_additional_exp  dmca (NOLOCK)
				 inner join Rightsu_vmpl_live_27March_2015.dbo.Additional_Expense a (NOLOCK) on a.additional_expense_code= dmca.additional_expense_code
				 inner join Additional_Expense aNew (NOLOCK) on anew.Additional_Expense_Name=a.additional_expense_name
				 inner join Rightsu_vmpl_live_27March_2015.dbo.deal_movie_cost dmc (NOLOCK) on dmc.deal_movie_cost_code=dmca.deal_movie_cost_code
				 inner join Rightsu_vmpl_live_27March_2015.dbo.deal_movie dm (NOLOCK) on dm.deal_movie_code=dmc.deal_movie_code
				 ...
				 from  Rightsu_vmpl_live_27March_2015.dbo.deal_movie_cost_commission dmcc (NOLOCK)
				 inner join Rightsu_vmpl_live_27March_2015.dbo.deal_movie_cost dmc (NOLOCK) on dmc.deal_movie_cost_code=dmcc.deal_movie_cost_code
				 inner join Rightsu_vmpl_live_27March_2015.dbo.deal_movie dm (NOLOCK) on dm.deal_movie_code=dmc.deal_movie_code
				 left join Rightsu_vmpl_live_27March_2015.dbo.Cost_Type ct  (NOLOCK) on ct.cost_type_code=dmcc.cost_type_code
				 left join Cost_Type ctNew (NOLOCK) on ct.cost_type_name=ctNew.cost_type_name
				 left join Rightsu_vmpl_live_27March_2015.dbo.Entity e (NOLOCK) on e.entity_code=dmcc.vendor_code
				 left join Entity eNew (NOLOCK) on e.entity_name=eNew.Entity_Name
				 left join Rightsu_vmpl_live_27March_2015.dbo.Vendor v (NOLOCK) on v.vendor_code=dmcc.vendor_code
				...
				eNew.Entity_Code
				 from Rightsu_vmpl_live_27March_2015.dbo.deal_movie_cost_variable_cost dmcvc (NOLOCK)
				inner join Rightsu_vmpl_live_27March_2015.dbo.deal_movie_cost dmc (NOLOCK) on dmc.deal_movie_cost_code=dmcvc.deal_movie_cost_code
				inner join Rightsu_vmpl_live_27March_2015.dbo.deal_movie dm (NOLOCK) on dm.deal_movie_code=dmc.deal_movie_code
				left join Rightsu_vmpl_live_27March_2015.dbo.Entity e (NOLOCK) on e.entity_code=dmcvc.licensr_code
				left join Entity eNew (NOLOCK) on e.entity_name=eNew.Entity_Name
				left join Rightsu_vmpl_live_27March_2015.dbo.Vendor v (NOLOCK) on v.vendor_code=dmcvc.licensr_code
				left join Vendor vNew (NOLOCK) on v.vendor_name=vNew.vendor_name
				where dm.deal_code=@DEALCODE and dmc.is_group=@is_group
				...
				from Rightsu_vmpl_live_27March_2015.dbo.deal_movie_cost_costtype dmcc (NOLOCK)
				inner join Rightsu_vmpl_live_27March_2015.dbo.deal_movie_cost dmc (NOLOCK) on dmc.deal_movie_cost_code=dmcc.deal_movie_cost_code
				inner join Rightsu_vmpl_live_27March_2015.dbo.deal_movie dm (NOLOCK) on dm.deal_movie_code=dmc.deal_movie_code
				inner join Rightsu_vmpl_live_27March_2015.dbo.Cost_Type c (NOLOCK) on c.cost_type_code=dmcc.cost_type_code
				inner join Cost_Type cNew (NOLOCK) on cNew.Cost_Type_Name=c.Cost_Type_Name
				where dm.deal_code=@DEALCODE and dmc.is_group=@is_group
		...
		*/
		...
		Declare cur_tit Cursor For 
			Select Acq_Deal_Rights_Title_Code From Acq_Deal_Rights_Title (NOLOCK)
		Open cur_tit
		...
		*/
		...
		,status_changed_by,status_changed_on,remarks 
		from Rightsu_vmpl_live_27March_2015.dbo.Module_Status_History (NOLOCK)
		where Record_Code=@DEALCODE and module_code=30
		...
		from Rightsu_vmpl_live_27March_2015.dbo.Module_Workflow_Detail (NOLOCK) 
	...
	*/
	...
	if(@Loglevel < 2)Exec [USPLogSQLSteps] '[USP_MIGRATE_TO_NEW]', 'Step 2', 0, 'Procedure Excution Completed', 0, '' 
END