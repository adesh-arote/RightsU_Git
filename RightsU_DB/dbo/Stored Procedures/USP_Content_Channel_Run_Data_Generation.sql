...
BEGIN
	...
	if(@Loglevel < 2)Exec [USPLogSQLSteps] '[USP_Content_Channel_Run_Data_Generation]', 'Step 1', 0, 'Started Procedure', 0, ''
	...
	-- =============================================
		...
		BEGIN TRY	
				...
				FROM BMS_Deal BV (NOLOCK) WHERE ISNULL(BV.Acq_Deal_Code,0) = @Acq_Deal_Code
				...
				* INTO #TempData FROM 
				(
					...
					INNER JOIN Acq_Deal_Movie ADM (NOLOCK) ON ADM.Acq_Deal_Code=AD.Acq_Deal_Code AND AD.Is_Master_Deal = 'Y'
					...
					INNER JOIN Acq_Deal_Rights ADR (NOLOCK) ON AD.Acq_Deal_Code = ADR.Acq_Deal_Code AND ADR.Actual_Right_Start_Date IS NOT NULL
					...
					LEFT JOIN Acq_Deal_Run_Yearwise_Run ADRYR (NOLOCK) ON ADRun.Acq_Deal_Run_Code = ADRYR.Acq_Deal_Run_Code AND ADRun.Is_Yearwise_Definition = 'Y'
				...
				INTO #TempData_New
				from #TempData TD
				INNER JOIN Title_Content_Mapping TCM (NOLOCK) ON TCM.Acq_Deal_Movie_Code=TD.Acq_Deal_Movie_Code
				...
				FROM Content_Channel_Run (NOLOCK) WHERE Acq_Deal_Code=@Acq_Deal_Code AND Is_Archive='N'
	...
	if(@Loglevel < 2)Exec [USPLogSQLSteps] '[USP_Content_Channel_Run_Data_Generation]', 'Step 2', 0, 'Procedure Excution Completed', 0, ''
END