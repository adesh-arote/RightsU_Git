...
BEGIN
	...
	if(@Loglevel < 2)Exec [USPLogSQLSteps] '[USP_Insert_Email_Notification_Log]', 'Step 1', 0, 'Started Procedure', 0, ''
		...
		SELECT distinct 'Email_NTF', EC.Email_Type, @Module_Code, @Record_Code, 0, To_User_Mail_Id, CC_User_Mail_Id, BCC_User_Mail_Id, [Subject], Email_Body, GETDATE(), GETDATE(),0 ,'P' , 0, 0, 0, 0 
		FROM  #Email_Config_Users_UDT A
		LEFT JOIN Email_Config EC ON EC.Email_Config_Code = A.Email_Config_Code
 
	if(@Loglevel < 2)Exec [USPLogSQLSteps] '[USP_Insert_Email_Notification_Log]', 'Step 2', 0, 'Procedure Excution Completed', 0, ''
END