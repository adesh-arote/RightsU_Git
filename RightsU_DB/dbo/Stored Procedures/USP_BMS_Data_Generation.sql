...
-- =============================================
	...
	BEGIN
		...
		FETCH NEXT FROM CUS_Deal_Process INTO @Deal_Code
			...
			END
			IF OBJECT_ID('tempdb..#runCounter') IS NOT NULL
			BEGIN
				DROP TABLE #runCounter
			END
			IF OBJECT_ID('tempdb..#MatchDealRuns') IS NOT NULL
			BEGIN
				DROP TABLE #MatchDealRuns
			END
			IF OBJECT_ID('tempdb..#DiffRuns') IS NOT NULL
			BEGIN
				DROP TABLE #DiffRuns
			END
			IF OBJECT_ID('tempdb..#DiffRunsYear') IS NOT NULL
			BEGIN
				DROP TABLE #DiffRunsYear
			END
			IF OBJECT_ID('tempdb..#tblChannelUpdate') IS NOT NULL
			BEGIN
				DROP TABLE #tblChannelUpdate
			END
			
			CREATE TABLE #runCounter
			(
				CNT INT,RN INT,Acq_Deal_Run_Code INT,SD Datetime,ED Datetime,DiffCNT INT
			)
			CREATE TABLE #MatchDealRuns
			(
				RN_Code INT,Acq_Deal_Run_Code INT,SDate Datetime,EDate Datetime,Title_Code INT,Add_Or_Sub  VARCHAR(1),DiffCNT INT
			)
			CREATE TABLE #tblChannelUpdate
			(
				Acq_Deal_Run_Code INT,Channel_Code INT,Title_Code INT,Changed_Runs INT
			)
			 /********************************Delcare Variables****************/
			...
			SELECT 
						...
						CASE
							...
							WHEN Is_Yearwise_Definition = 'Y' AND Is_Channel_Definition_Rights = 'Y' AND Run_Type <> 'U'
								...
								THEN CASE WHEN Channel_Weightage_YearWise = 0 THEN 0 ELSE  
									Channel_Weightage_YearWise END
				...
				0 [CurIden_BMS_Deal_Content_Rights],
				0 [CurIden_BMS_Asset],
				0 [Min_Plays],
				0 [Max_Plays],
				...
				* INTO #TempData FROM (
				SELECT 
					...
					NULL [BMS_Asset_Code], CAST(NULL AS VARCHAR(50)) [BMS_Asset_Ref_Key], 'PRO' [Asset_Type],--CAST(NULL AS Decimal(38,0)) [BMS_Asset_Ref_Key]
...
--/*-------------------------------------------------------Mismatch Runs Updation ----------------------------------------------------------------------------------------------------------------*/
--/*--------------------------------------Insertion of mismatch runs records on channel runs in #DifffRuns table------------------------- */
--		Select  SUM(Total_Runs) as a,Min_Runs,Acq_Deal_Run_Code,RU_Channel_Code,RU_Title_Code,'N' as Is_Changed, (SUM(Total_Runs) - Min_runs) as DiffCNT  into #DiffRuns 
--		from #TempData
--		WHERE ISNULL(Is_Yearwise_Definition,'N') = 'Y'
--		GROUP BY Acq_Deal_Run_Code,RU_Channel_Code ,Min_Runs,RU_Title_Code
--		HAVING SUM(Total_Runs) <> Min_Runs 

--		Select  SUM(Total_Runs) as a,No_Of_Runs,Acq_Deal_Run_Code,Right_Start_Date,Right_End_Date,RU_Title_Code,'N' as Is_Changed, (SUM(Total_Runs) - No_Of_Runs) as DiffCNT  into #DiffRunsYear 
--		from #TempData
--		WHERE ISNULL(Is_Yearwise_Definition,'N') = 'Y'
--		GROUP BY Acq_Deal_Run_Code,Right_Start_Date,Right_End_Date,No_Of_Runs,RU_Title_Code
--		HAVING SUM(Total_Runs) <> No_Of_Runs 

--		/* Start Cursor on #DiffRuns ------------------------------------------------------------------------------------------------------------*/
--			DECLARE @Total_Runs INT,@No_Of_Runs INT, @Acq_Deal_Run_Code INT,@SD Datetime,@ED Datetime,@RU_Title_Code INT
--			DECLARE CUS_Validate_Runs CURSOR FOR 
--				SELECT a,No_Of_Runs,Acq_Deal_Run_Code,Right_Start_Date,Right_End_Date,RU_Title_Code FROM  #DiffRunsYear WHere Is_Changed = 'N'
--			OPEN CUS_Validate_Runs
--			FETCH NEXT FROM CUS_Validate_Runs INTO @Total_Runs,@No_Of_Runs,@Acq_Deal_Run_Code,@SD,@ED,@RU_Title_Code
--			WHILE(@@FETCH_STATUS = 0)
--			BEGIN
--			DECLARE @CHANNELCODE INT
--				IF(@Total_Runs > @No_Of_Runs)
--				BEGIN
--					INSERT INTO #runCounter(CNT,RN,Acq_Deal_Run_Code,SD,ED,DiffCNT)
--					Select TD.RU_Channel_Code AS CNT,TD.RN,TD.Acq_Deal_Run_Code,TD.Right_Start_Date,TD.Right_End_Date,ts.DiffCNT 
--					from #TempData TD
--					INNER JOIN #DiffRunsYear ts ON TD.Acq_Deal_Run_Code = ts.Acq_Deal_Run_Code AND TD.Right_Start_Date = ts.Right_Start_Date AND TD.Right_End_Date = ts.Right_End_Date
--					Where TD.Acq_Deal_Run_Code = @Acq_Deal_Run_Code AND TD.RU_Title_Code = @RU_Title_Code AND ts.a > ts.No_Of_Runs

--					SELECT TOP 1 @CHANNELCODE = TD.RU_Channel_Code from #TempData TD
--					INNER JOIN #DiffRuns DFR ON TD.Acq_Deal_Run_Code = DFR.Acq_Deal_Run_Code AND TD.RU_Channel_Code = DFR.RU_Channel_Code
--					WHERE TD.Acq_Deal_Run_Code = @Acq_Deal_Run_Code AND TD.RU_Title_Code = @RU_Title_Code AND TD.Right_Start_Date = @SD AND TD.Right_End_Date = @ED AND
--					DFR.DiffCNT < TD.Total_Runs AND TD.Total_Runs <> 0
--					ORDER BY TD.RU_Channel_Code DESC
...
--					END
--				END	
--				/*Start Cursor on Channel wise counter to find counter wise records ----------------------------------------------------------------------*/
--				DECLARE @SDate Datetime,@EDate Datetime ,@Counter INT = 1
--				DECLARE CUS_Run_Counter CURSOR FOR 
--					SELECT DISTINCT SD,ED FROM  #runCounter 
--				OPEN CUS_Run_Counter 
--				FETCH NEXT FROM CUS_Run_Counter INTO @SDate,@EDate
--				WHILE(@@FETCH_STATUS = 0)
--				BEGIN
--					IF(@Total_Runs > @No_Of_Runs)
--					BEGIN
--						INSERT INTO #MatchDealRuns(RN_Code,Acq_Deal_Run_Code,SDate,EDate,Title_Code,Add_Or_Sub,DiffCNT)
--						SELECT top 1 RN,@Acq_Deal_Run_Code,@SD,@ED,@RU_Title_Code,'S',DiffCNT FROM #runCounter WHere CNT = @CHANNELCODE AND SD = @SDate AND ED = @EDate

--						UPDATE #DiffRunsYear SET Is_Changed = 'Y'  WHere Acq_Deal_Run_Code = @Acq_Deal_Run_Code AND Right_Start_Date = @SD AND Right_End_Date = @ED AND a > No_Of_Runs
--					END
...
--				FETCH NEXT FROM CUS_Run_Counter INTO @SDate,@EDate
--				END
--				CLOSE CUS_Run_Counter
--				DEALLOCATE CUS_Run_Counter

--				TRUNCATE TABLE #runCounter

--			FETCH NEXT FROM CUS_Validate_Runs INTO @Total_Runs,@No_Of_Runs,@Acq_Deal_Run_Code,@SD,@ED,@RU_Title_Code
--			END
--			CLOSE CUS_Validate_Runs
--			DEALLOCATE CUS_Validate_Runs

--/*--------------------------------------Updation (+/-) of records in #TempData which having mismatch bifurcation Runs----------------------------------------*/
--			UPDATE TD SET TD.Total_Runs = TD.Total_Runs - MDR.DiffCNT , TD.Is_Update = 'Y'
--			FROM #TempData TD 
--			INNER JOIN #MatchDealRuns MDR ON TD.RN = MDR.RN_Code AND MDR.Add_Or_Sub = 'S'

			
--			UPDATE TD SET TD.Total_Runs = TD.Total_Runs - (MDR.DiffCNT) , TD.Is_Update = 'Y'
--			FROM #TempData TD 
--			INNER JOIN #MatchDealRuns MDR ON TD.RN = MDR.RN_Code AND MDR.Add_Or_Sub = 'A'

--			-------------------------------Updation in Channel Table---------------------------------------------------------------------------------------------
--			INSERT INTO #tblChannelUpdate(Acq_Deal_Run_Code,Channel_Code,Title_Code,Changed_Runs)
--			Select Distinct Acq_Deal_Run_Code,RU_Channel_Code,RU_Title_Code,SUM(Total_Runs) AS Changed_Runs from #TempData Where Acq_Deal_Run_Code IN( 
--				Select Acq_Deal_Run_Code FROM #TempData Where IS_Update = 'Y' AND Acq_Deal_Code = @Deal_Code
--			)
--			GROUP BY Acq_Deal_Run_Code,RU_Channel_Code,RU_Title_Code

			
--/*-------------------------------------------------------Mismatch Runs Updation END----------------------------------------------------------------------------------------------------------------*/

			UPDATE #TempData SET Start_Date = (SELECT Min(Start_Date) FROM #TempData), End_Date = (SELECT Max(End_Date) FROM #TempData)
			
			--SELECT * 
			--UPDATE td SET td.Start_Date = tmp.StDate, td.End_Date = tmp.EnDate
			--FROM #TempData td
			--INNER JOIN (
			--	SELECT RU_Title_Code, Episode_Number, MIN(Start_Date) StDate, MAX(End_Date) EnDate 
			--	FROM #TempData
			--	GROUP BY RU_Title_Code, Episode_Number
			--) AS tmp ON td.RU_Title_Code = tmp.RU_Title_Code AND td.Episode_Number = tmp.Episode_Number

			--DELETE FROM #TempData WHERE CAST(End_Date AS DATE) < CAST(GETDATE() AS DATE)
		...
		/******** Update Total Runs and Content Start and End Date *********************************************************************/
			...
			BEGIN TRY
				...
				BEGIN	
					...
					SELECT 
						DISTINCT 
							...
							[Acquisition_Date], 'P',GETDATE(),GETDATE(),null,'Y'
					FROM #TempData
				...
				INSERT INTO BMS_Asset 
						...
						SELECT DISTINCT 							
							[BMS_Asset_Ref_Key],  
							--CONVERT(VARCHAR,dateadd(SECOND,(CAST([Duration_In_Min] AS INT) *60) + ([Duration_In_Min] %1)  ,'00:00:00'), 108), 
							[Duration_In_Min],
						...
						(
							(
								...
								(
									...
									AND CAST(temp.Episode_Number AS VARCHAR(100)) NOT IN (SELECT CAST(Episode_Number AS VARCHAR(100)) FROM BMS_Asset BA WHERE BA.RU_Title_Code = temp.RU_Title_Code)
								)
							)
							OR
				...
				DECLARE @AssetPrefix VARCHAR(20)=''
				...
				UPDATE temp SET temp.CurIden_BMS_Asset = BA.BMS_Asset_Code
					...
					UPDATE BA SET BA.Duration = [Duration_In_Min]
					--CONVERT(VARCHAR, dateadd(SECOND,(CAST([Duration_In_Min] AS INT) *60) + (PARSENAME([Duration_In_Min] % 1,1))  ,'00:00:00'), 108)
					...
					,BA.[BMS_Asset_Ref_Key] = @AssetPrefix+CAST(temp.CurIden_BMS_Asset as VARCHAR)
					--,Record_Status = 'P'
					--,BA.Updated_On=GETDATE()
					FROM BMS_Asset BA
					INNER JOIN #TempData temp ON --BA.BMS_Deal_Code = temp.CurIden_BMS_Deal AND
						BA.RU_Title_Code = temp.RU_Title_Code
						AND ISNULL(BA.Episode_Number, 0) = ISNULL(temp.Episode_Number, 0)
				...
				PRINT '9'	
					SELECT 					
						DISTINCT Total_Runs,Episode_Number,CurIden_BMS_Deal,CurIden_BMS_Deal_Content,CurIden_BMS_Deal_Content_Rights
				...
				BEGIN				
					...
					SELECT 
						DISTINCT 
							...
							[Asset_Type], [Title_Name],null,null, 
							--[Right_Start_Date] ,Right_End_Date,					
							'P','false',GETDATE(),null,'Y'
					FROM #TempData_New		
				...
				BEGIN
					INSERT INTO #Temp_BMS_Deal_Content(
							BMS_Deal_Content_Code,BMS_Deal_Code,BMS_Asset_Code,[Start_Date],End_Date,Yearwise_No
						)
					SELECT DISTINCT BDC.BMS_Deal_Content_Code,BDC.BMS_Deal_Code,BDC.BMS_Asset_Code,null,null,0
						--BDC.[Start_Date],End_Date,
						--ROW_NUMBER() OVER (Partition BY  BMS_Asset_Code ORDER BY Start_Date) YearWise_No
					...
					INNER JOIN #TempData_New temp ON temp.CurIden_BMS_Deal = BDC.BMS_Deal_Code
					AND BDC.BMS_Asset_Code = temp.CurIden_BMS_Asset	
					--AND BDC.YearWise_No = temp.YearWise_No
					...
					SELECT	BDC.BMS_Deal_Content_Code,BDC.BMS_Deal_Code,BDC.BMS_Asset_Code,null,null,0
						--BDC.[Start_Date],End_Date,
						--ROW_NUMBER() OVER (Partition BY  BMS_Asset_Code order by BMS_Asset_Code asc) YearWise_No ------Change the order by  start date to bms_Asset_Code
					FROM BMS_Deal_Content BDC WHERE BDC.BMS_Deal_Code = @BMS_Deal_Code
					--AND UPPER(Is_Archived) = 'FALSE' 
					...
					--Select * from #Temp_BMS_Deal_Content
				...
				INSERT INTO BMS_Deal_Content_Rights (
					...
					,Min_Runs,Max_Runs,Acq_Deal_Run_Code,Is_Archived,Created_On,Updated_On,Is_Active
					)
				...
				SELECT
					DISTINCT 
						...
						,Acq_Deal_Run_Code,'false',GETDATE(),null,'Y'
				FROM #TempData_New
				END
				ELSE
				BEGIN		
					...
					*/		
					SELECT 
					BDCR.BMS_Deal_Content_Rights_Code, BDCR.BMS_Deal_Content_Code,BDCR.RU_Channel_Code,BDCR.BMS_Asset_Code, BDCR.YearWise_No,BDC.Is_Archived
					...
					UPDATE BDCR 
						SET 
							...
							BDCR.Acq_Deal_Run_Code=temp.Acq_Deal_Run_Code,
						--	,CASE WHEN Run_Definition_Type = 'S' OR Run_Definition_Type = 'N' THEN Min_Plays ELSE NULL END--Run_Definition_Type
						--,CASE WHEN Run_Definition_Type = 'S' OR Run_Definition_Type = 'N' THEN Max_Plays ELSE NULL END--Run_Definition_Type				
							BMS_Station_Code=temp.BMS_Station_Key,
							BDCR.Updated_On=GETDATE()
					...
					--WHERE 				
					----UPPER(TBVDCR.Is_Archived) = 'FALSE' AND
					--(
					--	--TBVDCR.BMS_Asset_Code NOT IN
					--	--(
					--	--	SELECT temp.CurIden_BMS_Asset 
					--	--	FROM #TempData_New temp WHERE temp.RU_Channel_Code = TBVDCR.RU_Channel_Code
					--	--)
					--	--OR
					...
					--	WHERE 
					--	--UPPER(BVDCR.Is_Archived) = 'FALSE' AND 
				...
				--UPDATE ADRC SET  ADRC.Min_Runs = TD.Changed_Runs from Acq_Deal_Run_Channel ADRC 
				--INNER JOIN #tblChannelUpdate TD ON ADRC.Acq_Deal_Run_Code = TD.Acq_Deal_RUn_Code AND ADRC.Channel_Code = TD.Channel_Code 
				...
				/************************Set BMS_Deal_Content Is_Active Flag ***********************************************************/
					...
					UPDATE BDC SET BDC.Is_Active='N'--BDC.Is_Archived = 'true',BDC.Record_Status = 'A'
					--SELECT *
					FROM BMS_Deal_Content BDC			
					WHERE BDC.BMS_Deal_Code = @BMS_Deal_Code 
					--AND UPPER(BDC.Is_Archived) = 'FALSE'
				...
				/************************Set BMS_Deal_Content_Rights Is_Active Flag ***********************************************************/
					...
					UPDATE TBVDCR SET TBVDCR.Is_Active='N'--TBVDCR.Is_Archived= 'true',TBVDCR.Record_Status = 'A'
					FROM BMS_Deal_Content_Rights TBVDCR
					INNER JOIN #TempData_New temp ON TBVDCR.BMS_Deal_Content_Code=temp.CurIden_BMS_Deal_Content
					WHERE 				
					--UPPER(TBVDCR.Is_Archived) = 'FALSE' AND
					...
					--IF Title Delete from Run defination
						--UPDATE BVDCR SET BVDCR.Is_Active='N'--BVDCR.Is_Archived= 'true',BVDCR.Record_Status = 'A'
						--FROM BMS_Deal_Content_Rights BVDCR
						--WHERE 
						----UPPER(BVDCR.Is_Archived) = 'FALSE' AND 						
						--BVDCR.BMS_Deal_Content_Code IN
						...
						--UPDATE BVDCR SET BVDCR.Is_Active='N'
						--FROM BMS_Deal_Content_Rights BVDCR
						--WHERE
						--BVDCR.BMS_Deal_Content_Code IN
						--(
						--	SELECT BDC.BMS_Deal_Content_Code FROM BMS_Deal_Content BDC (NOLOCK)
						--	WHERE BDC.Is_Active='N'
						--)
