...
BEGIN    
	...
	SELECT @CBFC_Code = Columns_Code FROM Extended_Columns WHERE Columns_Name = 'CBFC Rating'
	...
	SELECT DM_Master_Log_Code AS IntCode, Master_Code, Name FROM DM_Master_Log WHERE Master_Type ='TG' AND ISNULL(Master_Code, 0) > 0      
	UNION    
	...
	BEGIN	
		...
		SELECT 0 AS IntCode, Columns_Value_Code, Columns_Value FROM Extended_Columns_Value  WHERE Columns_Code = @CBFC_Code
	END
	...
	@CBFC_Rating VARCHAR(100) = ''
		...
		BEGIN        
			...
			BEGIN                                  
				...
				SELECT @titleCount = Count(*) From Title WHERE Title_Name = @EngTitle AND Deal_Type_Code = @deal_type_code and Reference_Key is null
