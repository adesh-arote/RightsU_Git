...
BEGIN 
	...
	if(@Loglevel < 2)Exec [USPLogSQLSteps] '[USP_Deal_Expiry_Email]', 'Step 1', 0, 'Started Procedure', 0, ''
		...
		SELECT @Email_Config_Code = Email_Config_Code FROM Email_Config (NOLOCK) where [Key] = @Alert_Type
		...
		PRINT 'ACE1'
			...
			(		 
				...
				INNER JOIN Acq_Deal Ad (NOLOCK) ON Ad.Acq_Deal_Code = adr.Acq_Deal_Code AND Ad.Deal_Workflow_Status = 'A' AND AD.Is_Master_Deal = 'Y'
				...
				Inner Join Acq_Deal_Rights_Territory adrc (NOLOCK) On adr.Acq_Deal_Rights_Code = adrc.Acq_Deal_Rights_Code
			...
			FROM (
				...
				CASE t1.Territory_Type
					...
					(
						...
						Inner Join Territory c (NOLOCK) On t2.Territory_Code = c.Territory_Code Where t2.Acq_Deal_Rights_Code = t1.Acq_Deal_Rights_Code 
						FOR XML PATH('') ), 1, 1, '')
					)	 
					Else
					(
						stuff((SELECT DISTINCT ', ' + cast(c.Country_Name as NVARCHAR(max))FROM #ACQ_EXPIRING_DEALS t2
						Inner Join Country c (NOLOCK) On t2.Country_Code = c.Country_Code Where t2.Acq_Deal_Rights_Code = t1.Acq_Deal_Rights_Code 
						FOR XML PATH('') ), 1, 1, '')
					)
				END	as CountryNames,
				...
				(
					...
					FROM Vendor v1  (NOLOCK)
					inner join Acq_Deal_Licensor a1 (NOLOCK) on v1.Vendor_Code=a1.Vendor_Code 
					WHERE a1.Acq_Deal_Code=AD.Acq_Deal_Code FOR XML PATH('') ), 1, 1, '') 
				) as Vendor_Name
				FROM #ACQ_EXPIRING_DEALS t1
				INNER join  Acq_Deal AD (NOLOCK) on ad.Acq_Deal_Code= t1.Acq_Deal_Code AND AD.Is_Master_Deal = 'Y'
				INNER JOIN Title T  (NOLOCK) ON T.Title_Code = t1.Title_Code
			...
			FROM (
				...
				CASE t1.Territory_Type
					...
					(
						...
						Inner Join Territory c (NOLOCK) On t2.Territory_Code = c.Territory_Code Where t2.Syn_Deal_Rights_Code = t1.Syn_Deal_Rights_Code 
						FOR XML PATH('') ), 1, 1, '')
					)	 
					Else
					(
						stuff((SELECT DISTINCT ', ' + cast(c.Country_Name as NVARCHAR(max))FROM #Syn_EXPIRING_DEALS t2
						Inner Join Country c (NOLOCK) On t2.Country_Code = c.Country_Code Where t2.Syn_Deal_Rights_Code = t1.Syn_Deal_Rights_Code 
						FOR XML PATH('') ), 1, 1, '')
					)
				END	as CountryNames,
				t1.IsProcessed, IsNull(Business_Unit_Code,0) as Business_Unit_Code,
				...
				INNER join  Syn_Deal AD (NOLOCK) on ad.Syn_Deal_Code= t1.Syn_Deal_Code
		...
		BEGIN	
			...
			BEGIN
				...
				SELECT @BU_Name = Business_Unit_Name FROM Business_Unit (NOLOCK) WHERE Business_Unit_Code =  @Business_Unit_Code
	...
	if(@Loglevel < 2)Exec [USPLogSQLSteps] '[USP_Deal_Expiry_Email]', 'Step 2', 0, 'Procedure Excution Completed', 0, ''
END