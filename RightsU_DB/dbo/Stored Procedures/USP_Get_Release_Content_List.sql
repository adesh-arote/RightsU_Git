...
BEGIN
	...
	if(@Loglevel  < 2)Exec [USPLogSQLSteps] '[USP_Get_Release_Content_List]', 'Step 1', 0, 'Started Procedure', 0, ''
		SET NOCOUNT ON
		...
		BEGIN
			...
			BEGIN
				...
				BEGIN
					...
					IF EXISTS (
						SELECT * FROM Title_Content TC (NOLOCK)
				...
				BEGIN
					...
					SELECT @MinEpisodeNo = Min(TC.Episode_No), @MaxEpisodeNo = Max(TC.Episode_No) FROM Title_Content TC (NOLOCK)
					INNER JOIN Title_Content_Mapping TCM (NOLOCK) ON TCM.Title_Content_Code = TC.Title_Content_Code
					AND TCM.Acq_Deal_Movie_Code = @AcqDealMovieCode AND TC.Episode_No < @EpisodeFrom 
		...
		BEGIN
			...
			INNER JOIN Title T (NOLOCK) ON T.Title_Code = TADM.Title_Code
	...
	if(@Loglevel  < 2)Exec [USPLogSQLSteps] '[USP_Get_Release_Content_List]', 'Step 2', 0, 'Procedure Excution Completed', 0, ''
END