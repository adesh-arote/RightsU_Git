...
BEGIN
	...
	if(@Loglevel < 2)Exec [USPLogSQLSteps] '[USP_BMS_Deserialize_Masters]', 'Step 1', 0, 'Started Procedure', 0, ''
		SET NOCOUNT ON;	
	   ...
	   -- SET @XML_Data = @strXml   
		...
		BEGIN
			...
			INSERT INTO #Temp_Updated_Keys(BMS_Key, Code)
			SELECT BMS_Key, REPLACE(RU_Code, 'RUBMSV', '')  AS Code  FROM BMS_Response_Data (NOLOCK) WHERE Type_For = 'RUV'
		...
		BEGIN
			...
			INSERT INTO #Temp_Updated_Keys(BMS_Key, Code)
			SELECT BMS_Key, REPLACE(RU_Code, 'RUBMSC', '')  AS Code  FROM BMS_Response_Data (NOLOCK) WHERE Type_For = 'RUC'
		...
		BEGIN              
			...
			INSERT INTO #Temp_Updated_Keys(BMS_Key, Code)
			SELECT BMS_Key, REPLACE(RU_Code, 'RUBMSRR', '')  AS Code  FROM BMS_Response_Data (NOLOCK) WHERE Type_For = 'RURR'
		...
		BEGIN                     
			...
			INSERT INTO #Temp_Updated_Keys(BMS_Key, Code)
			SELECT BMS_Key, REPLACE(RU_Code, 'RUBMSECV', '')  AS Code  FROM BMS_Response_Data (NOLOCK) WHERE Type_For = 'RUECV'
		...
		BEGIN               
			INSERT INTO #Temp_Updated_Keys(BMS_Key, Code)
			SELECT BMS_Key, REPLACE(RU_Code, ' ', '')  AS Code  FROM BMS_Response_Data (NOLOCK) WHERE Type_For = 'RUBW'
	...
	if(@Loglevel < 2)Exec [USPLogSQLSteps] '[USP_BMS_Deserialize_Masters]', 'Step 2', 0, 'Procedure Excution Completed', 0, ''
END