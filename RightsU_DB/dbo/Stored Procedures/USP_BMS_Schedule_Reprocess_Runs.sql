...
BEGIN  
	...
	if(@Loglevel < 2)Exec [USPLogSQLSteps] '[USP_BMS_Schedule_Reprocess_Runs]', 'Step 1', 0, 'Started Procedure', 0, ''
		SET NOCOUNT ON;
		BEGIN TRY
		--BEGIN TRAN
		...
		IF EXISTS (select * from Content_Channel_Run (NOLOCK) where Run_Def_Type = 'S' AND 
		...
		BEGIN
			...
			LEFT Join Content_Channel_Run t (NOLOCK) On BST.Content_Channel_Run_Code = t.Content_Channel_Run_Code AND ISNULL(t.Is_Archive, 'N') = 'N'
		...
		BEGIN
			...
			SELECT DISTINCT BST.Content_Channel_Run_Code, BST.Channel_Code, BST.IsPrime, BST.Schedule_Item_Log_Date, BST.Timeline_ID, BST.Program_Episode_ID,
							BST.Schedule_Item_Log_Time, BST.IsIgnore, t.Run_Def_Type, t.Title_Content_Code, t.Acq_Deal_Run_Code, t.Rights_Start_Date, t.Rights_End_Date
			FROM BV_Schedule_Transaction BST (NOLOCK)
			Inner Join Content_Channel_Run t (NOLOCK) On BST.Content_Channel_Run_Code = t.Content_Channel_Run_Code AND ISNULL(t.Is_Archive, 'N') = 'N'
	...
	if(@Loglevel < 2)Exec [USPLogSQLSteps] '[USP_BMS_Schedule_Reprocess_Runs]', 'Step 2', 0, 'Procedure Excution Completed', 0, ''
END