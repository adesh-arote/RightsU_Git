...
BEGIN	
	...
	if(@Loglevel < 2)Exec [USPLogSQLSteps] '[USP_Get_Unutilized_Run] ', 'Step 1', 0, 'Started Procedure', 0, ''
		...
		Acq_Deal_Run_Yearwise_Run (NOLOCK)
		...
		FROM Acq_Deal_Run_Yearwise_Run AS ADRYR (NOLOCK)
		INNER JOIN Acq_Deal_Run_Title AS ADRT (NOLOCK) ON ADRYR.Acq_Deal_Run_Code = ADRT.Acq_Deal_Run_Code
		...
		OPEN cPointer
			...
			BEGIN
				...
				DECLARE @ParameterValue as NVARCHAR(20)
					...
					SELECT ADR.Acq_Deal_Run_Code FROM Acq_Deal_Run ADR (NOLOCK)
					INNER JOIN Acq_Deal AD (NOLOCK) ON  ADR.Acq_Deal_Code = Ad.Acq_Deal_Code AND  AD.Deal_Workflow_Status NOT IN ('AR', 'WA') AND AD.Business_Unit_Code = @Business_Unit_Code
	...
	if(@Loglevel < 2)Exec [USPLogSQLSteps] '[USP_Get_Unutilized_Run] ', 'Step 2', 0, 'Procedure Excution Completed', 0, ''
END
