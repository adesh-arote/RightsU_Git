CREATE PROCEDURE [dbo].[USPMHMailNotification]
	@MHRequestCode INT,
...
BEGIN
	...
	if(@Loglevel< 2)Exec [USPLogSQLSteps] '[USPMHMailNotification]', 'Step 1', 0, 'Started Procedure', 0, ''
		...
		BEGIN TRY
			...
			BEGIN
				...
				BEGIN
					SELECT DISTINCT
						...
						,@ShowName = (select TOP 1 T.Title_Name from Title T (NOLOCK) Where T.Title_Code = MR.TitleCode)
						...
						,@TelecastDate = CASE WHEN CAST(MR.TelecastFrom AS DATE) =  CAST(MR.TelecastTo AS DATE) 
										...
										END
						,@MusicLabel = STUFF((SELECT DISTINCT ', ' + CAST(ML.Music_Label_Name AS NVARCHAR) FROM MHRequestDetails MRD1 (NOLOCK)
							 INNER JOIN Music_Title_Label MTL (NOLOCK) ON MRD1.MusicTitleCode = MTL.Music_Title_Code 
							 INNER JOIN Music_Label ML (NOLOCK) ON ML.Music_Label_Code = MTL.Music_Label_Code
							 Where MRD1.MHRequestCode = MR.MHRequestCode
							 FOR XML PATH('')), 1, 1, '')
						,@NoOfSongs =CAST((SELECT COUNT(*) FROM MHRequestDetails MRD (NOLOCK) WHERE MRD.MHRequestCode = MR.MHRequestCode ) AS NVARCHAR)
						...
						,@SongsApproved = (SELECT COUNT(*) FROM MHRequestDetails MRD1 (NOLOCK) WHERE MR.MHRequestCode = MRD1.MHRequestCode AND MRD1.IsApprove = 'Y')
						,@VendorCode = MR.VendorCode
					 FROM MHRequest MR (NOLOCK)
						INNER JOIN MHRequestStatus MRS (NOLOCK) ON MR.MHRequestStatusCode = MRS.MHRequestStatusCode 
						LEFT JOIN Title T (NOLOCK) ON T.Title_Code = MR.TitleCode
						LEFT JOIN Channel c (NOLOCK) ON c.Channel_Code = MR.ChannelCode
						LEFT JOIN Users U (NOLOCK) ON MR.ApprovedBy = U.Users_Code
					...
					SELECT @Email_Config_Code =  Email_Config_Code FROM Email_Config (NOLOCK) WHERE UPPER(Email_Type) = 'MHCONSUMPTIONREQUEST'
				END
				...
				BEGIN
					SELECT DISTINCT
					 ...
					 SELECT @Email_Config_Code =  Email_Config_Code FROM Email_Config (NOLOCK) WHERE UPPER(Email_Type) = 'MHMUSICREQUEST'
				END
				...
				BEGIN
					 ...
					 SELECT @Email_Config_Code =  Email_Config_Code FROM Email_Config (NOLOCK) WHERE UPPER(Email_Type) = 'MHMOVIEREQUEST'
				END
			END
			ELSE
			BEGIN
				...
				FROM MHCUESHEET MC (NOLOCK)
					INNER JOIN Vendor V (NOLOCK) ON V.Vendor_Code = MC.VendorCode
					INNER JOIN Users U (NOLOCK) ON U.Users_Code = MC.SubmitBy
				...
				SELECT @Email_Config_Code =  Email_Config_Code FROM Email_Config (NOLOCK) WHERE UPPER(Email_Type) = 'MHCUESHEETUPLOAD'
			END
			...
			SELECT MU.Users_Code,U.Email_Id, UPPER(LEFT(U.First_Name, 1))+LOWER(SUBSTRING(U.First_Name, 2, LEN(U.First_Name))) +' '+UPPER(LEFT(U.Last_Name, 1))+LOWER(SUBSTRING(U.Last_Name, 2, LEN(U.Last_Name))) AS UserName FROM MHUsers MU (NOLOCK)
			INNER JOIN Users U (NOLOCK) ON U.Users_Code = MU.Users_Code WHERE MU.Vendor_Code = @VendorCode AND U.Is_Active = 'Y'
			OPEN curOuter 
	...
	if(@Loglevel< 2)Exec [USPLogSQLSteps] '[USPMHMailNotification]', 'Step 2', 0, 'Procedure Excution Completed', 0, ''
END
GO
