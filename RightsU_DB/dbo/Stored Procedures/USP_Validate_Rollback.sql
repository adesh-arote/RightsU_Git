CREATE PROCEDURE [dbo].[USP_Validate_Rollback]
(
	@Deal_Code INT,
	@Type CHAR(1),
	@User_Code INT
...
BEGIN
	...
	if(@Loglevel < 2)Exec [USPLogSQLSteps] '[USP_Validate_Rollback]', 'Step 1', 0, 'Started Procedure', 0, ''
		...
		BEGIN
			...
			SELECT TOP 1 @Deal_Type_Code = Deal_Type_Code FROM Acq_Deal WHERE Acq_Deal_Code = @Deal_Code
			...
			BEGIN
				...
				SELECT DISTINCT TC.Title_Content_Code INTO #CurrentVersionData FROM Acq_Deal AD (NOLOCK)
				...
				BEGIN
					SELECT DISTINCT TC.Title_Content_Code INTO #PreviousVersionData FROM AT_Acq_Deal atAD (NOLOCK)
					INNER JOIN AT_Acq_Deal_Movie atADM (NOLOCK) ON atADM.AT_Acq_Deal_Code = atAD.AT_Acq_Deal_Code
					INNER JOIN Title_Content TC (NOLOCK) ON TC.Title_Code = atADM.Title_Code AND TC.Episode_No BETWEEN atADM.Episode_Starts_From AND atADM.Episode_End_To
					...
					BEGIN
						IF NOT EXISTS (
							SELECT * FROM Title_Content_Mapping TCM (NOLOCK)
							INNER JOIN #LinkedContent LC ON LC.Title_Content_Code = TCM.Title_Content_Code
							INNER JOIN Acq_Deal_Movie ADM (NOLOCK) ON ADM.Acq_Deal_Movie_Code = TCM.Acq_Deal_Movie_Code AND ADM.Acq_Deal_Code <> @Deal_Code
						)
						BEGIN
	...
	if(@Loglevel < 2)Exec [USPLogSQLSteps] '[USP_Validate_Rollback]', 'Step 2', 0, 'Procedure Excution Completed', 0, ''
END