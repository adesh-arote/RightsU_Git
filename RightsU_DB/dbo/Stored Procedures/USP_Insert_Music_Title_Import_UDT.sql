...
SET NOCOUNT ON;		
		...
		IF NOT EXISTS(SELECT TOP 1 Record_Status FROM DM_Music_Title WHERE ISNULL(RTRIM(LTRIM(Record_Status)),'') = 'E')
		BEGIN
		PRINT 'IF Condition'	
		...
		(
			SELECT [Theme] MusicThemeName FROM DM_Music_Title WHERE ISNULL([Theme], '') <> ''
		) AS a
		CROSS APPLY
		(
		...
		(
			SELECT [Title_Language] LanName FROM DM_Music_Title WHERE ISNULL([Title_Language], '') <> ''
		) AS a
		CROSS APPLY
		(
		...
		(
			SELECT [Music_Version] Music_Version FROM DM_Music_Title WHERE ISNULL([Music_Version], '') <> ''
		) AS a

		INSERT INTO #Temp_Genres(Name, RoleExists)
		SELECT  LTRIM(RTRIM(Replace(a.Genres, ' ', ' '))), 'N' FROM
		(
			SELECT DISTINCT [Genres] Genres FROM DM_Music_Title WHERE ISNULL([Genres], '') <> ''
		) AS a
		...
		(
			SELECT DISTINCT [Movie_Album] Movie_Album, [Music_Album_Type] Music_Album_Type FROM DM_Music_Title WHERE ISNULL([Movie_Album], '') <> '' AND ISNULL([Music_Album_Type], '') <> ''
		) AS a
		...
		(
			SELECT [Singers] SinName FROM DM_Music_Title WHERE ISNULL([Singers], '') <> ''
		) AS a
		CROSS APPLY
		(
		...
		SELECT DISTINCT LTRIM(RTRIM(Replace(b.Lyricist, ' ', ' '))), 'N' FROM(
			SELECT [Lyricist] FROM DM_Music_Title WHERE ISNULL([Lyricist], '') <> ''
		) AS a
		CROSS APPLY
		(
		...
		SELECT DISTINCT LTRIM(RTRIM(Replace(b.Music_Director, ' ', ' '))), 'N' FROM(
			SELECT [Music_Director] FROM DM_Music_Title WHERE ISNULL([Music_Director], '') <> ''
		) AS a
		CROSS APPLY
		(
		...
		(
			SELECT [Star_Cast] SCName FROM DM_Music_Title WHERE ISNULL([Star_Cast], '') <> ''
		) AS a
		CROSS APPLY
		(
		...
		SELECT DISTINCT LTRIM(RTRIM(Replace(b.SCName, ' ', ' '))), 'N' FROM
		(
			SELECT [Movie_Star_Cast] SCName FROM DM_Music_Title WHERE ISNULL([Movie_Star_Cast], '') <> ''
		) AS a
		CROSS APPLY
		(
			SELECT LTRIM(RTRIM(Replace(number, ' ', ' '))) SCName FROM DBO.fn_Split_withdelemiter(a.SCName, ',')
		...
		SELECT LTRIM(RTRIM(Name)), 'N', 'Y' FROM #Temp_Singers WHERE Name collate SQL_Latin1_General_CP1_CI_AS NOT IN (SELECT talent_name collate SQL_Latin1_General_CP1_CI_AS FROM Talent)
		UNION
		SELECT LTRIM(RTRIM(Name)), 'N', 'Y' FROM #Temp_Lyricist WHERE Name collate SQL_Latin1_General_CP1_CI_AS NOT IN (SELECT talent_name collate SQL_Latin1_General_CP1_CI_AS FROM Talent)
		UNION
		SELECT LTRIM(RTRIM(Name)), 'N', 'Y' FROM #Temp_Music_Director WHERE Name collate SQL_Latin1_General_CP1_CI_AS NOT IN (SELECT talent_name collate SQL_Latin1_General_CP1_CI_AS FROM Talent)
		UNION
		SELECT LTRIM(RTRIM(Name)), 'N', 'Y' FROM #Temp_Star_Cast WHERE Name collate SQL_Latin1_General_CP1_CI_AS NOT IN (SELECT talent_name collate SQL_Latin1_General_CP1_CI_AS FROM Talent)
		UNION 
		SELECT LTRIM(RTRIM(Name)), 'N', 'Y' FROM #Temp_Movie_Star_Cast WHERE Name collate SQL_Latin1_General_CP1_CI_AS NOT IN (SELECT talent_name collate SQL_Latin1_General_CP1_CI_AS FROM Talent )
		...
		SELECT LTRIM(RTRIM(Name)), 'Y' FROM #Temp_Genres WHERE Name collate SQL_Latin1_General_CP1_CI_AS NOT IN (SELECT Genres_Name collate SQL_Latin1_General_CP1_CI_AS FROM Genres)
		...
		WHERE Name collate SQL_Latin1_General_CP1_CI_AS NOT IN (SELECT Music_Album_Name collate SQL_Latin1_General_CP1_CI_AS FROM Music_Album)
 
 		INSERT INTO Music_Language(Language_Name,Is_Active)
		SELECT LTRIM(RTRIM(Name)), 'Y' FROM #Temp_Music_Language WHERE Name collate SQL_Latin1_General_CP1_CI_AS NOT IN (SELECT Language_Name collate SQL_Latin1_General_CP1_CI_AS FROM Music_Language)
		
		INSERT INTO Music_Theme(Music_Theme_Name,Is_Active)
		SELECT LTRIM(RTRIM(Name)), 'Y' FROM #Temp_Music_Theme WHERE Name collate SQL_Latin1_General_CP1_CI_AS NOT IN (SELECT Music_Theme_Name collate SQL_Latin1_General_CP1_CI_AS FROM Music_Theme)
		...
		DECLARE CUR_Title CURSOR For
			...
			FROM DM_Music_Title WHERE Record_Status = 'N' -- AND [Title/ Dubbed Title (Hindi)] = 'Aag Ka Gola'
		OPEN CUR_Title
		...
		BEGIN																														
			...
			SELECT @Genres_Code = Genres_Code FROM Genres WHERE LTRIM(RTRIM(Genres_Name)) collate SQL_Latin1_General_CP1_CI_AS = LTRIM(RTRIM(ISNULL(@Genres, '')))	collate SQL_Latin1_General_CP1_CI_AS
			SELECT @Music_Album_Code = Music_Album_Code FROM Music_Album WHERE LTRIM(RTRIM(Music_Album_Name)) collate SQL_Latin1_General_CP1_CI_AS = LTRIM(RTRIM(ISNULL(@Movie_Album, '')))	collate SQL_Latin1_General_CP1_CI_AS
			...
			WHERE tmsc.Movie_Star_Cast_Code NOT IN (SELECT mat.Talent_Code FROM Music_Album_Talent mat WHERE mat.Music_Album_Code = @Music_Album_Code)
