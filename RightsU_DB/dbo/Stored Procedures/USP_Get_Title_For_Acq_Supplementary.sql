CREATE PROCEDURE [dbo].[USP_Get_Title_For_Acq_Supplementary]
	@ACQ_DEAL_CODE int, @title_Code int = 0
AS
BEGIN
	Declare @Loglevel int;
	select @Loglevel = Parameter_Value from System_Parameter_New where Parameter_Name='loglevel'
	if(@Loglevel < 2)Exec [USPLogSQLSteps] '[USP_Get_Title_For_Acq_Supplementary]', 'Step 1', 0, 'Started Procedure', 0, ''
		...
		Begin
			...
			And T.Title_code NOT IN (
				SELECT ads.Title_code FROM Acq_Deal_Supplementary ads (NOLOCK) 
	...
	if(@Loglevel < 2)Exec [USPLogSQLSteps] '[USP_Get_Title_For_Acq_Supplementary]', 'Step 2', 0, 'Procedure Excution Completed', 0, ''
END