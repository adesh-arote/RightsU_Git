...
--ORDER BY VWB.Channel_Code
	...
	SELECT DISTINCT
		...
		VWB.Title_Code 
	FROM VW_BMS_Schedule_Acq_Rights_Data VWB
	INNER JOIN BV_Schedule_Transaction BST ON VWB.Title_Code = BST.Title_Code AND ISNULL(BST.Title_Code,0) > 0 	
	INNER JOIN BMS_Schedule_Runs BSR ON BSR.Acq_Deal_Run_Code = BST.Acq_Deal_Run_Code AND BSR.Channel_Code = VWB.Channel_Code  	
	AND VWB.Acq_Deal_Code = @Acq_Deal_Code 
	ORDER BY VWB.Channel_Code
