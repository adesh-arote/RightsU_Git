...
BEGIN
	...
	if(@Loglevel < 2)Exec [USPLogSQLSteps] '[USP_Content_Music_Link_Bulk_Insert_UDT]', 'Step 1', 0, 'Started Procedure', 0, ''
		SET NOCOUNT ON;
		...
		BEGIN
			...
			ELSE IF(@TC_Code > 0)
			BEGIN
				IF NOT EXISTS (SELECT * FROM Title_Content_Version (NOLOCK) WHERE Title_Content_Code = @TC_Code AND Version_Code = @versionCode)
				BEGIN
					INSERT INTO Title_Content_Version(Title_Content_Code, Version_Code, Duration)
					SELECT Title_Content_Code, @versionCode, Duration  FROM Title_Content (NOLOCK) WHERE Title_Content_Code = @TC_Code
				END

				SELECT TOP 1 @titleContentVersionCode = Title_Content_Version_Code FROM Title_Content_Version (NOLOCK) WHERE Title_Content_Code = @TC_Code AND Version_Code = @versionCode
	...
	if(@Loglevel < 2)Exec [USPLogSQLSteps] '[USP_Content_Music_Link_Bulk_Insert_UDT]', 'Step 2', 0, 'Procedure Excution Completed', 0, ''
END