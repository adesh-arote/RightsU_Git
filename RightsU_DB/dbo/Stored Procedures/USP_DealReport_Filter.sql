...
BEGIN      
	...
	if(@Loglevel  < 2)Exec [USPLogSQLSteps] '[USP_DealReport_Filter]', 'Step 1', 0, 'Started Procedure', 0, ''
	...
	--@MilestoneNatureCode  = '0'
	 ...
	 --Delete From #Temp_Title Where number = 0      
	  IF @Acq_Deal_Code > 0  
		BEGIN  
	 SET @Agreement_No = (SELECT AD.Agreement_No from Acq_Deal AD (NOLOCK) where AD.Acq_Deal_Code = @Acq_Deal_Code)  
	 END  
  
	IF @Syn_Deal_Code > 0  
		BEGIN  
	 SET @Agreement_No = (SELECT AD.Agreement_No from Syn_Deal AD (NOLOCK) where AD.Syn_Deal_Code = @Syn_Deal_Code)  
	 END
	  -----Music Title------------------------------------
	...
	SET @TitleNames = Case when @Title_Code = '' Then 'NA' ELSE @TitleNames END
	 ...
	 FROM ProjectMilestone P   (NOLOCK)     
	...
	SET @Content_Names_ID =ISNULL(STUFF((SELECT DISTINCT ',' + Co.Episode_Title      
	 ...
	 FROM Vendor V (NOLOCK)  WHERE V.Vendor_Code IN (select number from dbo.fn_Split_withdelemiter(@MHProduction_House_Code,',') Where number NOT IN ('0',''))      
	 FOR XML PATH(''), TYPE      
	 ).value('.', 'NVARCHAR(MAX)'),1,1,''), '')    
	 ...
	 FROM MHRequestStatus S  (NOLOCK) WHERE S.MHRequestStatusCode IN (select number from dbo.fn_Split_withdelemiter(@MHStatus_Code,',') Where number NOT IN ('0',''))      
	 ...
	 FROM Milestone_Nature MN  (NOLOCK)    
	...
	if(@Loglevel < 2)Exec [USPLogSQLSteps] '[USP_DealReport_Filter]', 'Step 2', 0, 'Procedure Excution Completed', 0, ''
END