...
BEGIN
	...
	if(@Loglevel < 2)Exec [USPLogSQLSteps] '[USP_Get_Title_Availability_LanguageWise]', 'Step 1', 0, 'Started Procedure', 0, ''
		...
		from Avail_Acq AA (NOLOCK)
		INNER JOIN Avail_Acq_Details AAD (NOLOCK) ON AA.Avail_Acq_Code = AAD.Avail_Acq_Code
		INNER JOIN Title t (NOLOCK) ON AA.Title_Code = t.Title_Code
		WHERE ((AA.Title_Code in (select number from dbo.fn_Split_withdelemiter(@Title_Code,',') ) AND @Title_Code <> '0') OR @Title_Code = '0')
		AND ((AA.Platform_Code in ( Select Platform_Code from [Platform] (NOLOCK) Where Base_Platform_Code in (select number from dbo.fn_Split_withdelemiter(@Platform_Code,','))) AND @Platform_Code <> '0') OR @Platform_Code = '0')
		...
		(Select CONVERT(varchar(12), Rights_Start_Date,106) from Avail_Acq_Details (NOLOCK) Where Avail_Acq_Code = AA.Avail_Acq_Code) AS Right_Start_Date,
		(Select CONVERT(varchar(12), Rights_End_Date,106) from Avail_Acq_Details (NOLOCK) Where Avail_Acq_Code = AA.Avail_Acq_Code) AS Right_End_Date, [Type] = AAL.Dubbing_Subtitling ,AAL.Language_Code
		from Avail_Acq AA (NOLOCK)
		INNER JOIN Avail_Acq_Language AAL (NOLOCK) ON AA.Avail_Acq_Code = AAL.Avail_Acq_Code
		WHERE ((AA.Title_Code in (select number from dbo.fn_Split_withdelemiter(@Title_Code,',') ) AND @Title_Code <> '0') OR @Title_Code = '0')
		AND ((AA.Platform_Code in ( Select Platform_Code from [Platform] (NOLOCK) Where Base_Platform_Code in (select number from dbo.fn_Split_withdelemiter(@Platform_Code,','))) AND @Platform_Code <> '0') OR @Platform_Code = '0')
		AND ((AA.Country_Code in (select number from dbo.fn_Split_withdelemiter(@Country_Code,',') ) AND @Country_Code <> '0') OR @Country_Code = '0')
		...
		(Select *,
				...
				(Select Distinct ', ' + C.Country_Name
					...
					INNER JOIN Country C (NOLOCK) ON b.Country_Code = C.Country_Code
				...
				(Select Distinct ', ' + CAST(L.Language_Name as NVARCHAR)
					From #tempTitleAvail b
					INNER JOIN [Language] L (NOLOCK) ON b.Language_Code=L.Language_Code
					Where a.Title_Code = b.Title_Code And a.Right_Start_Date = b.Right_Start_Date AND ISNULL(a.Rights_End_Date,'')=ISNULL(b.Rights_End_Date,'')
					AND a.[Type] = b.[Type]
		...
		INNER JOIN Title t (NOLOCK) ON t.Title_Code = ta.Title_Code
	...
	if(@Loglevel < 2)Exec [USPLogSQLSteps] '[USP_Get_Title_Availability_LanguageWise]', 'Step 2', 0, 'Procedure Excution Completed', 0, ''
END