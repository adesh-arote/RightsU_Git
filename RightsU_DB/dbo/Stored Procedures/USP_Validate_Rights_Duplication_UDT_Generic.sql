...
Begin
	...
	if(@Loglevel < 2)Exec [USPLogSQLSteps] '[USP_Validate_Rights_Duplication_UDT_Generic]', 'Step 1', 0, 'Started Procedure', 0, ''
		...
		BEGIN 
			...
			BEGIN
				...
				BEGIN
					...
					If(IsNull(@CCodes, '') <> '')
						...
						FROM Country (NOLOCK) WHERE Country_Code IN 
						(
					...
					If(IsNull(@SCodes, '') <> '')
						...
						FROM [Language] (NOLOCK) WHERE Language_Code IN 
						(
					...
					*/
					...
					FROM (
						...
						INNER JOIN [Platform] P (NOLOCK) ON P.Parent_Platform_Code=TPC.Platform_Code OR P.Base_Platform_Code=TPC.Platform_Code
						RIGHT JOIN
						(
							SELECT
							 Title_Code
		  ...
		  SELECT  DISTINCT
			...
			INNER JOIN Title T (NOLOCK) ON T.Title_Code=DP.Title_Code
			INNER JOIN [Platform] P (NOLOCK) ON P.Platform_Code=DP.Platform_Code
		END
		ELSE
		--IF(@Debug='N')
		BEGIN
		...
		--					 ,ErrorMSG,Right_Type,Episode_From,Episode_To,Deal_Type_Code
			...
			Select Title_Name
				...
				(Select Distinct ', ' + CAST(L.Language_Name as NVARCHAR)
					From @Dup_Records_Language b
					INNER JOIN [Language] L (NOLOCK) ON b.Subtitling_Language=L.Language_Code
					Where a.Title_Code = b.Title_Code And a.Right_Start_Date = b.Right_Start_Date AND ISNULL(a.Right_End_Date,'')=ISNULL(b.Right_End_Date,'')
					And a.Episode_From = b.Episode_From And a.Episode_To = b.Episode_To  and isnull(a.Is_Sub_License,'')=isnull(b.Is_Sub_License,'') and isnull(a.Is_Title_Language_Right,'')=isnull(b.Is_Title_Language_Right,'')
					And a.ErrorMSG = b.ErrorMSG
				...
				(Select Distinct ', ' + CAST(L.Language_Name as NVARCHAR) 
					From @Dup_Records_Language b 
					INNER JOIN [Language] L (NOLOCK) ON b.Dubbing_Language=L.Language_Code
					Where a.Title_Code = b.Title_Code And a.Right_Start_Date = b.Right_Start_Date AND ISNULL(a.Right_End_Date,'')=ISNULL(b.Right_End_Date,'')
					And a.Episode_From = b.Episode_From And a.Episode_To = b.Episode_To and isnull(a.Is_Sub_License,'')=isnull(b.Is_Sub_License,'') and isnull(a.Is_Title_Language_Right,'')=isnull(b.Is_Title_Language_Right,'')
					And a.ErrorMSG = b.ErrorMSG
				...
				From (
					...
					,DRL.IsPushback
					From @Dup_Records_Language DRL
					INNER JOIN Title T (NOLOCK) ON T.Title_Code=DRL.Title_Code
					--WHERE (DRL.Is_Title_Language_Right <> 'N' AND DRL.Dubbing_Language is not null AND DRL.Subtitling_Language is not null)
					Group By T.Title_Code, T.Title_Name, Right_Start_Date, Right_End_Date,Is_Sub_License,Is_Title_Language_Right
	...
	if(@Loglevel < 2)Exec [USPLogSQLSteps] '[USP_Validate_Rights_Duplication_UDT_Generic]', 'Step 2', 0, 'Procedure Excuting Completed', 0, ''
END
