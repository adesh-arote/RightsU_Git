...
BEGIN
	...
	if(@Loglevel<2)Exec [USPLogSQLSteps] '[USP_Music_Schedule_Process]', 'Step 1', 0, 'Started Procedure', 0, '' 
		...
		SELECT @Email_Config_Code=Email_Config_Code FROM Email_Config (NOLOCK) where [Key]='MSE'
		...
		BEGIN
			...
			INNER JOIN Title_Content TC (NOLOCK) ON TC.Ref_BMS_Content_Code = BST.Program_Episode_ID AND TC.Title_Code = BST.Title_Code
		...
		BEGIN
			...
			BEGIN
				...
				INNER JOIN Title_Content_Version TCV (NOLOCK) ON TCV.Title_Content_Version_Code = CML.Title_Content_Version_Code AND TCV.Version_Code = @DefaultVersionCode
				INNER JOIN #TempScheduleData TSD ON TSD.Title_Code = TC.Title_Code AND TSD.Episode_No =  TC.Episode_No
				LEFT JOIN Music_Title_Label MTL (NOLOCK) ON MTL.Music_Title_Code = CML.Music_Title_Code 
			...
			*/
			...
			SELECT TOP 1 @CurrentTitleDealType = Deal_Type_Code FROM Title T (NOLOCK) WHERE T.Title_Code = @TitleCode
			...
			INNER JOIN Music_Deal MD (NOLOCK) ON MD.Music_Label_Code = TMST.Music_Label_Code
			INNER JOIN Music_Deal_Channel MDC (NOLOCK) ON MDC.Music_Deal_Code = MD.Music_Deal_Code
			LEFT JOIN Music_Deal_LinkShow MDLS (NOLOCK) ON MDLS.Music_Deal_Code = MD.Music_Deal_Code 
			...
			BEGIN
				...
				DELETE MSE FROM Music_Schedule_Transaction MST
				INNER JOIN Music_Schedule_Exception MSE (NOLOCK) ON MSE.Music_Schedule_Transaction_Code = MST.Music_Schedule_Transaction_Code
			...
			BEGIN
				...
				SELECT DISTINCT MSE.Music_Schedule_Transaction_Code, ECM.Error_Code, ECM.Upload_Error_Code FROM Music_Schedule_Exception MSE (NOLOCK)
				INNER JOIN #TempMusicScheduleTransaction MST ON MST.MusicScheduleTransactionCode = MSE.Music_Schedule_Transaction_Code
				INNER JOIN Error_Code_Master ECM (NOLOCK) ON ECM.Error_Code = MSE.Error_Code
				...
				BEGIN
					...
					BEGIN
						...
						BEGIN
							...
							FROM Music_Schedule_Transaction MST (NOLOCK)
							INNER JOIN Content_Music_Link CML (NOLOCK) ON CML.Content_Music_Link_Code = MST.Content_Music_Link_Code
							INNER JOIN BV_Schedule_Transaction BST (NOLOCK) ON MST.BV_Schedule_Transaction_Code = BST.BV_Schedule_Transaction_Code 
						...
						SET @CountSchedule = 0
						SELECT @CountSchedule = COUNT(Music_Schedule_Transaction_Code) 
						FROM Music_Schedule_Transaction (NOLOCK) WHERE  Music_Deal_Code = @MusicDealCode AND Is_Ignore = 'N' AND Channel_Code = @ChannelCode
						...
						BEGIN
							...
							SELECT @Scheduled_Runs = ISNULL(SUM(Scheduled_Runs), 0) FROM Music_Deal_Channel (NOLOCK) WHERE Music_Deal_Code = @MusicDealCode
							...
							BEGIN
								IF EXISTS (
									SELECT * FROM Music_Deal_Channel (NOLOCK) 
					...
					BEGIN
						...
						SELECT @MusicScheduleTransactionCode, ECM.Error_Code, 'E', 'N' FROM Error_Code_Master ECM  (NOLOCK)
						INNER JOIN (
						...
						INNER JOIN Music_Schedule_Exception MSE (NOLOCK) ON MSE.Music_Schedule_Transaction_Code = @MusicScheduleTransactionCode
				...
				BEGIN
					SELECT @RunType = Run_Type FROM Music_Deal MD (NOLOCK) WHERE Music_Deal_Code = @MusicDealCode
		...
		-----------------------------------------------------------
			...
			DECLARE curMailData CURSOR FOR
				...
				INNER JOIN Title T (NOLOCK) ON T.Title_Code = BST.Title_Code AND T.Title_Code = TC.Title_Code
				INNER JOIN Music_Title MT (NOLOCK) ON MT.Music_Title_Code = CML.Music_Title_Code
				LEFT JOIN Channel C (NOLOCK) ON C.Channel_Code = MST.Channel_Code
				LEFT JOIN Music_Label ML (NOLOCK) ON ML.Music_Label_Code = MST.Music_Label_Code
				LEFT JOIN Music_Album MA (NOLOCK) ON MA.Music_Album_Code = MT.Music_Album_Code
				LEFT JOIN Music_Deal MD (NOLOCK) ON MD.Music_Deal_Code = MST.Music_Deal_Code
			...
			BEGIN
				...
				BEGIN
					...
					SELECT TOP 1 @mailBody = Template_Desc FROM Email_Template (NOLOCK) WHERE Template_For = 'MS'
				
			
					SELECT TOP 1 @DefaultSiteUrl = DefaultSiteUrl FROM System_Param (NOLOCK)
					...
					BEGIN	
						...
						SELECT  @User_Email_Id = ISNULL(STUFF((SELECT ';' + A.number FROM dbo.fn_Split_withdelemiter(@User_Email_Id,';') AS A INNER JOIN Users U (NOLOCK) on U.Email_Id = A.number FOR XML PATH('')), 1, 1, '') ,''),
								@To_Users_Code = ISNULL( STUFF(( SELECT ',' + CAST(U.Users_Code AS NVARCHAR(MAX)) FROM dbo.fn_Split_withdelemiter(@User_Email_Id,';') AS A INNER JOIN Users U (NOLOCK) on U.Email_Id = A.number FOR XML PATH('')), 1, 1, '') ,''),
								@CC_User_Mail_Id = ISNULL(STUFF((SELECT ';' + A.number FROM dbo.fn_Split_withdelemiter(@CC_User_Mail_Id,';') AS A INNER JOIN Users U (NOLOCK) on U.Email_Id = A.number FOR XML PATH('')), 1, 1, '') ,''),
								@CC_Users_Code = ISNULL( STUFF(( SELECT ',' + CAST(U.Users_Code AS NVARCHAR(MAX)) FROM dbo.fn_Split_withdelemiter(@CC_User_Mail_Id,';') AS A INNER JOIN Users U (NOLOCK) on U.Email_Id = A.number FOR XML PATH('')), 1, 1, '') ,''),
								@BCC_User_Mail_Id = ISNULL(STUFF((SELECT ';' + A.number FROM dbo.fn_Split_withdelemiter(@BCC_User_Mail_Id,';') AS A INNER JOIN Users U (NOLOCK) on U.Email_Id = A.number FOR XML PATH('')), 1, 1, '') ,''),
								@BCC_Users_Code = ISNULL( STUFF(( SELECT ',' + CAST(U.Users_Code AS NVARCHAR(MAX)) FROM dbo.fn_Split_withdelemiter(@BCC_User_Mail_Id,';') AS A INNER JOIN Users U (NOLOCK) on U.Email_Id = A.number FOR XML PATH('')), 1, 1, '') ,'')
	...
	if(@Loglevel<2)Exec [USPLogSQLSteps] '[USP_Music_Schedule_Process]', 'Step 2', 0, 'Procedure Excution Completed', 0, '' 
END