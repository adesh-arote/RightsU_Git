...
BEGIN
	...
	*/
	...
	SELECT DISTINCT MIN(Role_Level) FROM Module_Workflow_Detail WHERE Is_Done = 'N' GROUP BY Role_Level
	...
	BEGIN
		...
		FROM (SELECT  Record_Code, MIN(Role_Level) AS Role_Level FROM Module_Workflow_Detail WHERE Is_Done ='N' AND MODULE_Code = @Module_Code GROUP BY Record_Code)   MWD
		INNER JOIN Acq_Deal AD ON AD.Acq_Deal_Code = MWD.Record_Code
		...
		Select DISTINCT BU.Business_Unit_Name,'Level ' + CAST(MWD.Role_level + 1 AS VARCHAR) FROM Acq_Deal AD
		INNER JOIN Module_Workflow_Detail MWD ON MWD.Record_Code = AD.Acq_Deal_Code
		INNER JOIN Business_Unit BU ON BU.Business_unit_code = AD.Business_Unit_code
	...
	BEGIN
		...
		SELECT DISTINCT BU.Business_Unit_Name,'Level ' + CAST(MWD.Role_Level AS VARCHAR),COUNT(MWD.Role_Level)
		FROM (SELECT  Record_Code, MIN(Role_Level) AS Role_Level FROM Module_Workflow_Detail WHERE Is_Done ='N' AND MODULE_Code = @Module_Code GROUP BY Record_Code) MWD
		INNER JOIN Syn_Deal SD ON SD.Syn_Deal_Code = MWD.Record_Code
		...
		INSERT INTO #TEMPDATA(Business_Unit,R_Level)
		Select DISTINCT BU.Business_Unit_Name,'Level ' + CAST(MWD.Role_level + 1 AS VARCHAR) FROM Syn_Deal SD
		INNER JOIN Module_Workflow_Detail MWD ON MWD.Record_Code = SD.Syn_Deal_Code
		INNER JOIN Business_Unit BU ON BU.Business_unit_code = SD.Business_Unit_code
		WHERE BU.Business_Unit_Code IN (select number from fn_Split_withdelemiter(@Content_Category,','))
