...
BEGIN      
	...
	if(@Loglevel < 2)Exec [USPLogSQLSteps] '[USP_MQ_Customer_CRUD]', 'Step 1', 0, 'Started Procedure', 0, ''
		 SET NOCOUNT ON      
		 --U = UPDATE  
		 --C = CREATE  
		 ...
		 BEGIN  
			  ...
			  BEGIN 
				   ...
				   ELSE
				   BEGIN
					   IF NOT EXISTS (SELECT * FROM Vendor (NOLOCK) WHERE Vendor_Code = @RightsU_Customer_ID AND Party_Type = 'C')--AND Is_Active = 'Y')  
					   BEGIN  
					   ...
					   SELECT TOP 1 @Is_Error = 'Y', @errorMessage = ISNULL(@errorMessage,'') + '108;Duplicate Customer Name;'  FROM Vendor V  (NOLOCK)
			  ...
			  BEGIN
					PRINT 'Customer Name not exists'  
					SELECT TOP 1 @Is_Error = 'Y', @errorMessage = ISNULL(@errorMessage,'') + '108;Duplicate Customer Name;'  FROM Vendor V (NOLOCK) WHERE V.Vendor_Name = @Customer_Name  AND V.Is_Active = 'Y'
			  END
			  --------------------------------------------------------------------------------------------------------------  
		...
		BEGIN TRY
			 ...
			 BEGIN  
				  ...
				  SELECT @RightsU_Customer_ID , Country_Code , Is_Theatrical_Territory FROM Country (NOLOCK) WHERE Country_Code IN (SELECT number FROM dbo.fn_Split_withdelemiter(@Country_Code,','))  
				  --------------------------------------------------------------------------------------------------------------  
			 ...
			 BEGIN  
				  ...
				  INSERT INTO Vendor_Country (Vendor_Code, Country_Code, Is_Theatrical)  
				  SELECT @RightsU_Customer_ID , Country_Code , Is_Theatrical_Territory FROM Country (NOLOCK) WHERE Country_Code IN   
				  (SELECT id FROM #temp_Country  
				   EXCEPT  
				   SELECT country_Code FROM Vendor_Country (NOLOCK)  WHERE Vendor_Code = @RightsU_Customer_ID)  
				  --------------------------------------------------------------------------------------------------------------  
	...
	if(@Loglevel < 2)Exec [USPLogSQLSteps] '[USP_MQ_Customer_CRUD]', 'Step 2', 0, 'Procedure Excution Completed', 0, ''
END