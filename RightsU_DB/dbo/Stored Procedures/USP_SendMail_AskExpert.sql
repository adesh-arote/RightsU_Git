...
BEGIN  
	...
	if(@Loglevel< 2)Exec [USPLogSQLSteps] '[USP_SendMail_AskExpert]', 'Step 1', 0, 'Started Procedure', 0, ''
		SET FMTONLY OFF   
		...
		BEGIN TRY    
			...
			SELECT @Email_Config_Code = Email_Config_Code FROM Email_Config (NOLOCK) where [Key] = 'AAE'
			...
			SELECT @User_Name = U.First_Name  FROM Users U (NOLOCK) WHERE U.Users_Code = @User_Code  
			...
			SELECT TOP 1 @body = Template_Desc FROM Email_template (NOLOCK) WHERE Template_For = 'AEX'  
			SELECT TOP 1 @body_RightsUTeam = Template_Desc FROM Email_template (NOLOCK) WHERE Template_For = 'AEX_RightsTeam'  
			...
			INTO #Temp 
			from [dbo].[UFN_Get_Bu_Wise_User]('AAE')  WHERE BuCode IN(select Business_Unit_Code from Users_Business_Unit  (NOLOCK)
	...
	if(@Loglevel< 2)Exec [USPLogSQLSteps] '[USP_SendMail_AskExpert]', 'Step 2', 0, 'Procedure Excuting Completed', 0, ''
END