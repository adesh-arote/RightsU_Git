CREATE PROC [dbo].[USP_Email_Notification]
-- =============================================
...
BEGIN
	...
	if(@Loglevel < 2)Exec [USPLogSQLSteps] '[USP_Email_Notification]', 'Step 1', 0, 'Started Procedure', 0, '' 
	select @Loglevel = Parameter_Value from System_Parameter_New where Parameter_Name='loglevel'
			...
			--select  @Key, @Email_Config_Code, @Notification_Day, @Notification_Frequency, @Notification_Time, @IsNotification_Frequency
				...
				SELECT TOP 1 @Created_Time = Created_Time FROM Email_Notification_Log (NOLOCK) where Email_Config_Code = @Email_Config_Code order by 1 desc
				...
				FROM Email_Notification_Log (NOLOCK) where Email_Config_Code = @Email_Config_Code
	...
	if(@Loglevel < 2)Exec [USPLogSQLSteps] '[USP_Email_Notification]', 'Step 2', 0, 'Procedure Excution Completed', 0, '' 
END
