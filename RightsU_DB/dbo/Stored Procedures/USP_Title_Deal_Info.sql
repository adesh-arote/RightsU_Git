...
BEGIN  
	...
	if(@Loglevel< 2)Exec [USPLogSQLSteps] '[USP_Title_Deal_Info]', 'Step 1', 0, 'Started Procedure', 0, ''
		 SET FMTONLY OFF  
		 ...
		 Where   AD.Business_Unit_Code IN(select Business_Unit_Code from Users_Business_Unit (NOLOCK) where Users_Code=@User_Code)  
		 UNION  
		 ...
		 Where SD.Business_Unit_Code IN (select Business_Unit_Code from Users_Business_Unit (NOLOCK) where Users_Code=@User_Code)  
		 ...
		 OPEN CUR_Right  
		  FETCH NEXT FROM CUR_Right InTo @Right_Code,@Module_Code  
		   ...
		   BEGIN  
			  SET @Codes = ''  
			  IF(@Module_Code = 30)       
			   SELECT @Codes = @Codes + CAST(Platform_Code AS VARCHAR) + ',' FROM Acq_Deal_Rights_Platform (NOLOCK) Where Acq_Deal_Rights_Code = @Right_Code  
			  ELSE        
			   SELECT @Codes = @Codes + CAST(Platform_Code AS VARCHAR) + ',' FROM Syn_Deal_Rights_Platform (NOLOCK) Where Syn_Deal_Rights_Code = @Right_Code  
		 ...
		 IF EXISTS(SELECT TOP 1  Mail_alert_days FROM  Deal_Expiry_Email (NOLOCK) Where Alert_Type = 'A' AND ISNULL(Mail_alert_days,0) > 0)  
		  SELECT TOP 1 @Acq_Mail_alert_days = Mail_alert_days FROM  Deal_Expiry_Email (NOLOCK) Where Alert_Type = 'A'   
		 IF EXISTS(SELECT TOP 1  Mail_alert_days FROM  Deal_Expiry_Email  (NOLOCK) Where Alert_Type = 'S' AND ISNULL(Mail_alert_days,0) > 0)  
		  SELECT TOP 1 @Syn_Mail_alert_days = Mail_alert_days FROM  Deal_Expiry_Email (NOLOCK) Where Alert_Type = 'S'   
		 ...
		 (  
		  SELECT T.Title_Language_Code FROM  Title T (NOLOCK) WHERE T.Title_Code = @Title_Code  
		 )  
	...
	if(@Loglevel< 2)Exec [USPLogSQLSteps] '[USP_Title_Deal_Info]', 'Step 2', 0, 'Procedure Excuion Completed', 0, ''
END