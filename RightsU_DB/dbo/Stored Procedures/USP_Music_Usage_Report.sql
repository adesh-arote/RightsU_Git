...
BEGIN  
	...
	if(@Loglevel<2)Exec [USPLogSQLSteps] '[USP_Music_Usage_Report]', 'Step 1', 0, 'Started Procedure', 0, ''
		...
		INNER JOIN Role R (NOLOCK) ON R.Role_Code = MTT.Role_Code  
		INNER JOIN Talent TA (NOLOCK) ON MTT.Talent_Code=TA.Talent_Code  

		INSERT INTO #TEMP (MT_Code,
		...
		SELECT DISTINCT
			...
			CASE WHEN ISNULL(MT.Movie_Album_Type,'A')='A' THEN (CASE WHEN ISNULL(MT.Music_Album_Code, 0) = 0 THEN MT.Movie_Album ELSE MA.Music_Album_Name END) ELSE TA.Title_Name END Movie_Album,
			ML.Music_Label_Name, MTY.Music_Type_Name, 
			...
			REVERSE(STUFF(REVERSE(STUFF((   
				...
				FROM Music_Title_Theme MTT3 (NOLOCK)  
				INNER JOIN Music_Theme MT1 (NOLOCK) ON MT1.Music_Theme_Code=MTT3.Music_Theme_Code  
		...
		FROM   
			Content_Music_Link CML (NOLOCK)  
			INNER JOIN Title_Content TC (NOLOCK) ON TC.Title_Content_Code = CML.Title_Content_Code 
			INNER JOIN Title_Content_Version TCV (NOLOCK) ON TCV.Title_Content_Version_Code = CML.Title_Content_Version_Code AND TCV.Title_Content_Code = TC.Title_Content_Code
			...
			INNER JOIN Title T (NOLOCK) ON T.Title_Code = CCR.Title_Code   
			INNER JOIN Music_Title MT (NOLOCK) ON MT.Music_Title_Code = CML.Music_Title_Code  
			INNER JOIN Music_Title_Label MTL (NOLOCK) ON MTL.Music_Title_Code=MT.Music_Title_Code  
			LEFT JOIN Music_Album MA (NOLOCK) ON MA.Music_Album_Code = MT.Music_Album_Code
			LEFT JOIN Title TA (NOLOCK) ON TA.Title_Code = MT.Title_Code
			LEFT JOIN Music_Type MTY (NOLOCK) ON MT.Music_Type_Code=MTY.Music_Type_Code  
			LEFT JOIN Program P (NOLOCK) ON T.Program_Code = P.Program_Code
			LEFT JOIN Music_Title_Theme MTT (NOLOCK) ON MTT.Music_Title_Code = MT.Music_Title_Code  
			LEFT JOIN BV_Schedule_Transaction BST (NOLOCK) ON Tc.Title_Code = BST.Title_Code AND TC.Episode_No = BST.Program_Episode_Number  
			...
			LEFT JOIN Channel C (NOLOCK) ON C.channel_code = BST.Channel_Code  
			LEFT JOIN Genres G (NOLOCK) ON G.Genres_Code = MT.Genres_Code  
			LEFT JOIN Music_Label ML (NOLOCK) ON MTL.Music_Label_Code = ML.Music_Label_Code   
		WHERE  AD.Deal_Workflow_Status NOT IN ('AR', 'WA') AND (@Title_Code='' OR (@Title_Code<>''AND TC.Title_Code in (SELECT number FROM dbo.fn_Split_withdelemiter('' + @Title_Code +'',','))))  
			...
			(SELECT MTT_TMP.Music_Title_Code FROM Music_Title_Talent MTT_TMP (NOLOCK) WHERE MTT_TMP.Talent_Code IN 
	...
	if(@Loglevel<2)Exec [USPLogSQLSteps] '[USP_Music_Usage_Report]', 'Step 2', 0, 'Procedure Excution Completed', 0, ''
END