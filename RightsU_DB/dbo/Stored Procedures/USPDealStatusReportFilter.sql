CREATE Procedure [dbo].[USPDealStatusReportFilter]
...
BEGIN
	...
	if(@Loglevel< 2)Exec [USPLogSQLSteps] '[USPDealStatusReportFilter]', 'Step 1', 0, 'Started Procedure', 0, ''
		...
		SET @AgreementNumber = (Select top 1 Agreement_No from Acq_Deal (NOLOCK) WHERE Acq_Deal_Code = @AgreementNo) 
	...
	if(@Loglevel< 2)Exec [USPLogSQLSteps] '[USPDealStatusReportFilter]', 'Step 2', 0, 'Procedure Excution Completed', 0, ''
END
