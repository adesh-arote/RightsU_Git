...
BEGIN   
	...
	if(@Loglevel  < 2)Exec [USPLogSQLSteps] '[USP_BMS_Asset_Mapping_UDT]', 'Step 1', 0, 'Started Procedure', 0, ''
		...
		BEGIN
			...
			,(SELECT Columns_Value_Code FROM Map_Extended_Columns (NOLOCK) WHERE Columns_Code = 1 And Record_Code = t.Title_Code)
			,(SELECT Ref_BMS_Code FROM [dbo].[Extended_Columns_Value]  (NOLOCK)
			WHERE Columns_Value_Code In (SELECT Columns_Value_Code FROM Map_Extended_Columns (NOLOCK) WHERE Columns_Code = 1 And Record_Code = t.Title_Code))
	...
	if(@Loglevel  < 2)Exec [USPLogSQLSteps] '[USP_BMS_Asset_Mapping_UDT]', 'Step 2', 0, 'Procedure Excution Completed', 0, ''
END