	...
	if(@Loglevel< 2)Exec [USPLogSQLSteps] '[USPRUR_Title_Expiry]', 'Step 1', 0, 'Started Procedure', 0, ''
		...
		BEGIN
			...
			SELECT Acq_Deal_Rights_Code, Country_Code FROM (
				...
				FROM #RigtsTitle adr
				INNER JOIN Acq_Deal_Rights_Territory tg (NOLOCK) ON adr.Acq_Deal_Rights_Code = tg.Acq_Deal_Rights_Code AND tg.Territory_Type = 'G'
				INNER JOIN Territory_Details td (NOLOCK) ON tg.Territory_Code = td.Territory_Code
				INNER JOIN @Deal_Rights_Territory drt  ON td.Country_Code = drt.Country_Code
				UNION ALL
				SELECT adr.Acq_Deal_Rights_Code, tg.Country_Code 
				FROM #RigtsTitle adr
				INNER JOIN Acq_Deal_Rights_Territory tg (NOLOCK) ON adr.Acq_Deal_Rights_Code = tg.Acq_Deal_Rights_Code AND tg.Territory_Type = 'I'
			...
			INNER JOIN Title t (NOLOCK) On t.Title_Code = data.Title_Code
			INNER JOIN Country c (NOLOCK) On c.Country_Code = data.Country_Code
			INNER JOIN Platform p (NOLOCK) On p.Platform_Code = data.Platform_Code
