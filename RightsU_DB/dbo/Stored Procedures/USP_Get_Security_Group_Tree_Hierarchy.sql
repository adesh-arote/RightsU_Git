...
Begin 
	...
	if(@Loglevel < 2)Exec [USPLogSQLSteps] '[USP_Get_Security_Group_Tree_Hierarchy]', 'Step 1', 0, 'Started Procedure', 0, ''  
	 ...
	 Begin            
	  ...
	  From System_Module (NOLOCK) Where Module_Code in (Select number From DBO.fn_Split_withdelemiter(@ModuleCodes, ',')) order by Module_Name             
              
	  While((             
	   Select Count(*) From System_Module (NOLOCK) Where Module_Code In (Select Parent_Module_Code From #TempPF) And Module_Code Not In (Select Module_Code From #TempPF)            
	  ) > 0)            
	  Begin            
               
	   Insert InTo #TempPF            
	   Select Module_Code, ISNULL(Parent_Module_Code, 0), Is_Sub_Module            
	   From System_Module (NOLOCK) Where Module_Code in (Select Parent_Module_Code From #TempPF) And Module_Code Not In (Select Module_Code From #TempPF)            
	 ...
	 ISNULL(STUFF((SELECT ',' + convert(varchar(Max),S.Module_Right_Code) FROM        
			(select * from System_Module_Right (NOLOCK))As S Where S.[Module_Code] = System_Module.Module_Code AND System_Module.Is_Sub_Module = 'N'         
			 FOR XML PATH('')),1,1,''),' ') As [Module_Right_Codes],        
        
	   ISNULL(STUFF((SELECT ',' + convert(varchar(Max),SA.Right_Name) FROM        
			(select s.Module_Right_Code,s.Module_Code,s.Right_Code,sr.Right_Name from System_Module_Right S inner join System_Right sr on S.Right_Code = sr.Right_Code)As SA  Where SA.[Module_Code] = System_Module.Module_Code --AND System_Module.Is_Sub_Module = 'N'      
			 FOR XML PATH('')),1,1,''),' ') As [Right_Codes],        
        
        
	 (select ISNULL(count(*),0) From System_Module_Right SMR (NOLOCK)  where SMR.Module_Code = System_Module.Module_Code AND System_Module.Is_Sub_Module = 'N' Group By SMR.Module_Code) RightsCount      
	 From System_Module (NOLOCK) Where Is_Active = 'Y'             
	...
	if(@Loglevel < 2)Exec [USPLogSQLSteps] '[USP_Get_Security_Group_Tree_Hierarchy]', 'Step 2', 0, 'Procedure Excution Completed', 0, ''  
End