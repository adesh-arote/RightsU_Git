...
BEGIN      
	...
	if(@Loglevel<2)Exec [USPLogSQLSteps] '[USP_Schedule_Process]', 'Step 1', 0, 'Started Procedure', 0, ''
		...
		BEGIN
			...
			SELECT BV_Schedule_Transaction_Code, Program_Episode_ID, Program_Version_ID, Program_Episode_Title, Program_Episode_Number, Program_Title, 
				Program_Category, Schedule_Item_Log_Date, Schedule_Item_Log_Time, Schedule_Item_Duration, Scheduled_Version_House_Number_List,
				Found_Status, File_Code, Title_Code, IsProcessed, IsException
			FROM BV_Schedule_Transaction (NOLOCK) WHERE 1=1 AND File_Code = @File_Code AND ISNULL(IsProcessed, 'N') = 'N'
		...
		BEGIN
			...
			OR
			(
				BST.Deal_Movie_Code IN(select Acq_Deal_Movie_Code From Acq_Deal_Movie (NOLOCK) Where Acq_Deal_Code = @Deal_Code))
			)
		...
		BEGIN ------------------------------------ 3.0 MAIN_CURSOR_FOR_UPDATE_SCHEDULE_COUNT ------------------------------------      
			...
			PRINT 'In Coursor'
				...
				BEGIN --Insert Temp_BV_Schedule Data into temporary #DealRunData table
					...
					SELECT TOP 1 @title_code_HouseID = tc.Title_Code, @Title_Content_Code = tc.Title_Content_Code FROM Title_Content tc (NOLOCK)
					...
					FROM Content_Channel_Run CCR (NOLOCK)
						INNER JOIN Acq_Deal AD (NOLOCK) ON AD.Acq_Deal_Code = CCR.Acq_Deal_Code
					...
					FROM Content_Channel_Run CCR (NOLOCK)
						INNER JOIN Provisional_Deal pd (NOLOCK) ON pd.Provisional_Deal_Code = CCR.Provisional_Deal_Code
					...
					INNER JOIN Acq_Deal_Rights ADR (NOLOCK) ON ADR.Acq_Deal_Code = drd.Deal_Code AND drd.Deal_Type = 'A'
						AND (
					...
					INNER JOIN Acq_Deal_Rights_Blackout ADRB (NOLOCK) ON ADRB.Acq_Deal_Rights_Code = ADR.Acq_Deal_Rights_Code
				...
				BEGIN  
					...
					BEGIN
						...
						(      
							...
							a.Deal_Type
							FROM #DealRunData a
							LEFT JOIN Content_Channel_Run ccr  (NOLOCK) ON ((ccr.Acq_Deal_Run_Code = a.Deal_Run_Code AND a.Deal_Type = 'A')
				...
				BEGIN --===============8.0 INNER_CURSOR_FOR_UPDATE_SCHEDULE_RUN_IN_FIFO_ORDER ===============--      
					...
					IF((SELECT CASE WHEN ISNULL(MIN(CCR.Deal_Type), 'A') = 'A' THEN COUNT(Ad.Business_Unit_Code) ELSE  COUNT(PD.Business_Unit_Code) END
						FROM Content_Channel_Run CCR (NOLOCK)
						LEFT JOIN Acq_Deal AD (NOLOCK) ON AD.Acq_Deal_Code = CCR.Acq_Deal_Code AND ISNULL(CCR.Deal_Type, 'A') = 'A' AND CCR.Acq_Deal_Code = @VW_DCode_Cr
						...
						WHERE  AD.Deal_Workflow_Status NOT IN ('AR', 'WA') AND ((AD.Business_Unit_Code IN ( SELECT Business_Unit_Code FROM Business_Unit (NOLOCK) WHERE  Business_Unit_Name LIKE '%English Movies%')
						AND ISNULL(CCR.Deal_Type, 'A') = 'A') OR (PD.Business_Unit_Code IN ( SELECT Business_Unit_Code FROM Business_Unit (NOLOCK) WHERE  Business_Unit_Name
						LIKE '%English Movies%') AND ISNULL(CCR.Deal_Type, 'A') = 'P')) AND ISNULL(CCR.Is_Archive,'N') = 'N') > 0
						AND
						(
							SELECT COUNT(Channel_Code) FROM Channel (NOLOCK) WHERE Order_For_schedule > 50 AND Channel_Code = @Channel_Code
						) > 0
					...
					BEGIN
						...
						SELECT @d =  CONVERT(DATE, Schedule_Item_Log_Date), @t = Schedule_Item_Log_Time  FROM BV_Schedule_Transaction (NOLOCK)
							WHERE BV_Schedule_Transaction_Code =  @BV_Schedule_Transaction_Code

						SELECT @PrimeStartTime = Prime_Start_Time, @Time_Lag_Simulcast = Time_Lag_Simulcast FROM Content_Channel_Run (NOLOCK)
						...
						SELECT @Channel_Group_code = Channel_Group FROM Channel (NOLOCK) WHERE Channel_Code = @Channel_Code
						IF(
							(
								SELECT COUNT(BV_Schedule_Transaction_code) FROM BV_Schedule_Transaction (NOLOCK)
								...
								AND IsProcessed = 'Y' AND Channel_Code IN (SELECT ISNULL(Channel_Code,0) FROM Channel (NOLOCK) WHERE Channel_Code not IN (@Channel_Code)
					...
					IF(
						(
							SELECT COUNT(BV_Schedule_Transaction_Code)  FROM BV_Schedule_Transaction (NOLOCK) WHERE
					...
					BEGIN
						...
						BEGIN
							...
							BEGIN
								...
								FROM BV_Schedule_Transaction (NOLOCK) WHERE BV_Schedule_Transaction_Code =  @BV_Schedule_Transaction_Code
								...
								SELECT TOP 1
									...
									FROM Content_Channel_Run (NOLOCK)  WHERE Acq_Deal_Run_Code = @VW_DMR_Run_Code_Cr AND Channel_Code = @Channel_Code
								...
								BEGIN
									...
									FROM Content_Channel_Run (NOLOCK) WHERE Content_Channel_Run_Code = @VW_CCRCode_Cr  AND ISNULL(Is_Archive,'N') = 'N'
								...
								BEGIN
									...
									BEGIN
										...
										FROM Content_Channel_Run adr (NOLOCK) WHERE Content_Channel_Run_Code = @VW_CCRCode_Cr AND ISNULL(Is_Archive,'N') = 'N'
									...
									BEGIN      
										...
										FROM Content_Channel_Run adr (NOLOCK) WHERE
							...
							BEGIN -----START ONLY IF Shared, Not Applicable
								...
								FROM Content_Channel_Run (NOLOCK) WHERE Content_Channel_Run_Code = @VW_CCRCode_Cr
		...
		UPDATE Email_Notification_Schedule
		SET 
		Title_Name = (SELECT Title_Name FROM Title (NOLOCK) WHERE title_code = CCR.Title_Code),
		Right_Start_Date = (SELECT MIN(CCR1.Rights_Start_Date) FROM Content_Channel_Run CCR1 (NOLOCK) WHERE CCR1.Title_Content_Code = CCR.Title_Content_Code 
		...
		Right_End_Date = (SELECT MAX(CCR2.Rights_End_Date) FROM Content_Channel_Run CCR2 (NOLOCK) WHERE CCR2.Title_Content_Code = CCR.Title_Content_Code
		...
		Available_Channels = (
								...
								(
									(
										SELECT DISTINCT ', ' + C.channel_name from Content_Channel_Run CCR1 (NOLOCK)
										INNER JOIN Channel C  (NOLOCK) on C.Channel_Code = CCR1.Channel_Code
		...
		Channel_Name = (SELECT channel_name AS Channel_Name FROM Channel (NOLOCK) WHERE channel_code = bv.Channel_Code)
		FROM Email_Notification_Schedule Email 
		INNER JOIN BV_Schedule_Transaction bv ON bv.BV_Schedule_Transaction_Code = Email.BV_Schedule_Transaction_Code
		INNER JOIN Content_Channel_Run CCR ON CCR.Content_Channel_Run_Code = bv.Content_Channel_Run_Code
		WHERE bv.File_Code = @File_Code
	...
	if(@Loglevel<2)Exec [USPLogSQLSteps] '[USP_Schedule_Process]', 'Step 2', 0, 'Procedure Excuting Completed', 0, ''
END