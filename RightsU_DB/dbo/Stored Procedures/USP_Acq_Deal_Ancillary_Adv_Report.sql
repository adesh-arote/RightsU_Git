...
BEGIN  
	...
	if(@Loglevel < 2)Exec [USPLogSQLSteps] '[USP_Acq_Deal_Ancillary_Adv_Report]', 'Step 1', 0, 'Started Procedure', 0, ''
	 --DECLARE  
	 --@Agreement_No VARCHAR(MAX),  
	 --@Title_Codes VARCHAR(MAX),  
	 --@Platform_Codes VARCHAR(MAX),  
	 ...
	 BEGIN  
	  ...
	  SELECT @Acq_Deal_Code = Acq_Deal_Code FROM Acq_deal (NOLOCK) WHERE Deal_Workflow_Status NOT IN ('AR', 'WA') AND Agreement_No = @Agreement_No AND Business_Unit_Code = @Business_Unit_Code  
	 ...
	 BEGIN  
	  ...
	  SELECT ADAT.Acq_Deal_Ancillary_Code, ADAT.Title_Code FROM Acq_Deal_Ancillary_title ADAT (NOLOCK)
	  INNER JOIN Acq_Deal_Ancillary ADA (NOLOCK) ON ADA.Acq_Deal_Ancillary_Code = ADAT.Acq_Deal_Ancillary_Code  
	  INNER JOIN Acq_Deal AD (NOLOCK) ON AD.Acq_Deal_Code = ADA.Acq_Deal_Code AND Business_Unit_Code = @Business_Unit_Code  
	 ...
	 BEGIN  
	  ...
	  SELECT  Acq_Deal_Code  FROM Acq_deal (NOLOCK) WHERE Deal_Workflow_Status NOT IN ('AR', 'WA') AND Business_Unit_Code = @Business_Unit_Code  
  
	  INSERT INTO @Ancillary_Deal (Acq_Deal_Ancillary_Code)  
	  SELECT Acq_Deal_Ancillary_Code FROM Acq_Deal_Ancillary (NOLOCK) WHERE Acq_deal_Code IN (SELECT Acq_Deal_Code FROM @AcqDeal)  
	  END  
	  ...
	  INSERT INTO @Ancillary_Deal (Acq_Deal_Ancillary_Code, Title_Code)  
	  SELECT ADT.Acq_Deal_Ancillary_Code, ADT.Title_Code FROM Acq_Deal_Ancillary ADA (NOLOCK)
	  INNER JOIN Acq_Deal_Ancillary_title ADT (NOLOCK) ON ADT.Acq_Deal_Ancillary_Code = ADA.Acq_Deal_Ancillary_Code  
	 ...
	 INNER JOIN Acq_Deal ad (NOLOCK) ON ada.Acq_Deal_Code = ad.Acq_Deal_Code  
	 ...
	 FROM Acq_Deal_Ancillary ADA (NOLOCK)
	  ...
	  INNER JOIN Acq_Deal_Ancillary_Platform ADP (NOLOCK) ON ADA.Acq_Deal_Ancillary_Code = ADP.Acq_Deal_Ancillary_Code  
	  INNER JOIN Acq_Deal_Ancillary_title ADT (NOLOCK) ON ADT.Title_Code = AD.Title_Code  
	  ...
	  INNER JOIN Platform P (NOLOCK) On P.Platform_Code = ADP.Platform_Code  
	  INNER JOIN Title T (NOLOCK) On ADT.Title_Code = T.Title_Code  
	  INNER JOIN Ancillary_Type AT (NOLOCK) ON AT.Ancillary_Type_Code = ADA.Ancillary_Type_Code  
	  INNER JOIN #GetDealTypeCondition GDTC ON AD.Deal_Type_Code = GDTC.Deal_Type_Code  
	  WHERE   
	 ...
	 if(@Loglevel < 2)Exec [USPLogSQLSteps] '[USP_Acq_Deal_Ancillary_Adv_Report]', 'Step 2', 0, 'Procedure Excution Completed', 0, ''
END