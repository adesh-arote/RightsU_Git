...
BEGIN	
	...
	if(@Loglevel < 2)Exec [USPLogSQLSteps] '[USP_Validate_Title_Talent_UDT]', 'Step 1', 0, 'Started Procedure', 0, ''
		SET FMTONLY OFF
		CREATE TABLE #Temp 
		(
			Talent_Code INT,
		...
		BEGIN
			...
			AND AD.Deal_Type_Code IN(SELECT DISTINCT Deal_Type_Code FROM [Role] (NOLOCK) WHERE Role_Code IN(SELECT DISTINCT Role_Code FROM @Title_Talent_Role))
			...
			INNER JOIN Title T  (NOLOCK) ON  SDTC.Title_Code = T.Title_Code AND T.Deal_Type_Code = SDTC.Deal_Type_Code	
			INNER JOIN [Role] R  (NOLOCK) ON R.Deal_Type_Code = SDTC.Deal_Type_Code AND R.Role_Code IN(SELECT DISTINCT Role_Code FROM @Title_Talent_Role)
	...
	if(@Loglevel < 2)Exec [USPLogSQLSteps] '[USP_Validate_Title_Talent_UDT]', 'Step 2', 0, 'Procedure Excution Completed', 0, ''
END