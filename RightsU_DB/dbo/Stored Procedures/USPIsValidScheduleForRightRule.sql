...
BEGIN
	...
	FROM Content_Channel_Run WHERE Content_Channel_Run_Code = @VW_CCRCode_Cr --ACq_Deal_RUN_Code = @VW_DMR_Run_Code_Cr
	...
	FROM Right_Rule WHERE Right_Rule_Code = @Right_Rule_Code
	...
	BEGIN
		...
		--	HRS_Logic:
			...
			IF( (SELECT COUNT(BV_Schedule_Transaction_Code) FROM BV_Schedule_Transaction 
					WHERE  Content_Channel_Run_Code = @VW_CCRCode_Cr -- Deal_Movie_Code IN (@ACQ_DEAL_Movie_Code)
					AND Title_Code IN (@Title_Code)
					AND ISNULL(IsProcessed, 'N') = 'Y' 
					AND ISNULL(IsIgnore, 'N') = 'N'
			...
			BEGIN
				...
				BEGIN
					SELECT @Count_schedule = COUNT(*) FROM BV_Schedule_Transaction BST 
...
DECLARE @RST Char(1)
set @RST = 'Y'
EXEC [dbo].[USPIsValidScheduleForRightRule] 177149,946,'Nov 17, 2014','11:00 PM',2431,509,1,@RST output
select @RST
*/