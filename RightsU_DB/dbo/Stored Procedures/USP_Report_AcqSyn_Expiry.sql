...
BEGIN
	...
	if(@Loglevel<2)Exec [USPLogSQLSteps] '[USP_Report_AcqSyn_Expiry]', 'Step 1', 0, 'Started Procedure', 0, ''
		 --DECLARE
		...
		BEGIN
			...
			INNER JOIN Acq_Deal_Movie ADM (NOLOCK) ON AD.Acq_Deal_Code = ADM.Acq_Deal_Code AND AD.Is_Master_Deal = 'N' AND AD.Deal_Type_Code !=  @Deal_Type_Code_Music
			WHERE  AD.Deal_Workflow_Status NOT IN ('AR', 'WA') AND AD.Master_Deal_Movie_Code_ToLink IN (SELECT Acq_Deal_Movie_Code FROM Acq_Deal_Movie ADM1 (NOLOCK) WHERE ADM1.Title_Code IN (Select number from #Temp_Title) OR @titleCodes = '0')
		END

		SELECT ADM.Acq_Deal_Movie_Code, ADM.Title_Code, L.Language_Name INTO #Temp_Title_Languages FROM Acq_Deal_Movie ADM (NOLOCK)
		...
		BEGIN
			...
			SELECT DISTINCT Country_Code FROM Territory_Details (NOLOCK) WHERE Territory_Code in (SELECT Territory_Code FROM #Temp_Territory)
		END
		...
		BEGIN
			...
			BEGIN
				...
				SELECT  MD.Record_Code,MD.Status_Changed_On FROM Acq_Deal AD (NOLOCK)
			...
			BEGIN 
				...
				SELECT Acq_Deal_Code FROM Acq_Deal (NOLOCK) Where  Deal_Workflow_Status NOT IN ('AR', 'WA') AND Business_Unit_Code = @BusinessUnit_Code
			END
			...
			BEGIN
				SET @SQL_Acq = 'INSERT INTO #Temp_Right_Code (Right_Code,Deal_Code,Status_Changed_On)
					...
					INNER JOIN Acq_Deal_Rights_Title ADRT (NOLOCK) ON ADR.Acq_Deal_Rights_Code = ADRT.Acq_Deal_Rights_Code '
			END
			ELSE IF(@Deal_Type = 'AMT')
			BEGIN
				SET @SQL_Acq = 'INSERT INTO #Temp_Right_Code (Right_Code,Deal_Code,Status_Changed_On)
					SELECT DISTINCT ADR.Acq_Deal_Rights_Code , AD.Acq_Deal_Code, AD.Status_Changed_On
					FROM Acq_Deal_Rights ADR (NOLOCK)
					INNER JOIN #Acq_Deals AD ON AD.Acq_Deal_Code = ADR.Acq_Deal_Code
					INNER JOIN Acq_Deal_Material ADM ON ADM.Acq_Deal_Code = AD.Acq_Deal_Code
					INNER JOIN Acq_Deal_Rights_Title ADRT (NOLOCK) ON ADR.Acq_Deal_Rights_Code = ADRT.Acq_Deal_Rights_Code '
			END
			ELSE IF(@Deal_Type = 'APT')
			BEGIN
				SET @SQL_Acq = 'INSERT INTO #Temp_Right_Code (Right_Code,Deal_Code,Status_Changed_On)
					SELECT DISTINCT ADR.Acq_Deal_Rights_Code , AD.Acq_Deal_Code, AD.Status_Changed_On
					FROM Acq_Deal_Rights ADR (NOLOCK)
					INNER JOIN #Acq_Deals AD ON AD.Acq_Deal_Code = ADR.Acq_Deal_Code
					INNER JOIN Acq_Deal_Payment_Terms ADPT (NOLOCK) ON ADPT.Acq_Deal_Code = AD.Acq_Deal_Code
					INNER JOIN Acq_Deal_Rights_Title ADRT (NOLOCK) ON ADR.Acq_Deal_Rights_Code = ADRT.Acq_Deal_Rights_Code '
			END
			ELSE
			BEGIN
			...
			ELSE IF(@Deal_Type = 'AHB')
			BEGIN
				SET @Acq_Cond ='WHERE Actual_Right_End_Date IS NOT NULL AND Holdback_Release_Date IS NOT NULL AND DATEDIFF(d, GETDATE(), ISNULL(Holdback_Release_Date , GETDATE())) BETWEEN 0 AND '''+ @expiryDays +''' '				
			END
			ELSE IF(@Deal_Type = 'ARHB')
			BEGIN
				SET @Acq_Cond ='WHERE ADP.Right_End_Date IS NOT NULL AND DATEDIFF(d, GETDATE(), ISNULL(ADP.Right_End_Date , GETDATE())) BETWEEN 0 AND '''+ @expiryDays +''' '				
			END
			ELSE IF(@Deal_Type = 'AMT')
			BEGIN
				SET @Acq_Cond ='WHERE Actual_Right_Start_Date IS NOT NULL AND
			...
			INNER JOIN Acq_Deal_Rights_Territory ADRC (NOLOCK) ON ADR.Right_Code = ADRC.Acq_Deal_Rights_Code
			LEFT JOIN Territory_Details TD (NOLOCK) ON 
			...
			INNER JOIN Acq_Deal_Movie ADM (NOLOCK) ON AD.Acq_Deal_Code = ADM.Acq_Deal_Code AND AD.Business_Unit_Code = @BusinessUnit_Code
			INNER JOIN Acq_Deal_Rights ADR (NOLOCK) ON (ADM.Acq_Deal_Code = ADR.Acq_Deal_Code OR AD.Acq_Deal_Code = ADR.Acq_Deal_Code)
			...
			BEGIN
				SET @Acq_Right_sql = 'UPDATE #Temp_Rights SET expDays = DATEDIFF(d, GETDATE(), ISNULL(Right_End_Date , GETDATE()))'
			END
			ELSE IF(@Deal_Type = 'AMT' OR @Deal_Type = 'APT')
			BEGIN
				SET @Acq_Right_sql = 'UPDATE #Temp_Rights SET expDays = DATEDIFF(d, GETDATE(), ISNULL(Right_Start_Date , GETDATE()))'
			END
			EXEC(@Acq_Right_sql)
		...
		BEGIN
			...
			BEGIN
				...
				SELECT MD.Record_Code, MAX(MD.Status_Changed_On)
				FROM   Module_Status_History MD (NOLOCK)
				WHERE  MD.Module_Code = 35  AND  MD.Status = 'A' 
				AND CONVERT(date,MD.Status_Changed_On,103) BETWEEN  cast(@StartDate as varchar(50))  AND  cast(@EndDate as varchar(50))
				GROUP BY  MD.Record_Code 

				INSERT INTO #Syn_Deals (Syn_Deal_Code, Status_Changed_On)
				SELECT  MD.Record_Code, MD.Status_Changed_On FROM Syn_Deal SD (NOLOCK)
			...
			BEGIN 
				...
				SELECT Syn_Deal_Code FROM Syn_Deal (NOLOCK) Where Business_Unit_Code = @BusinessUnit_Code
			END
			...
			BEGIN
				SET @SQL_Syn = 'INSERT INTO #Temp_Right_Code (Right_Code,Deal_Code,Status_Changed_On)
					...
					FROM Syn_Deal_Rights SDR (NOLOCK)
					INNER JOIN #Syn_Deals SD ON SD.Syn_Deal_Code = SDR.Syn_Deal_Code
					INNER JOIN Syn_Deal_Rights_Holdback SRHB (NOLOCK) ON SRHB.Syn_Deal_Rights_Code = SDR.Syn_Deal_Rights_Code
					INNER JOIN Syn_Deal_Rights_Title SDRT (NOLOCK) ON SDR.Syn_Deal_Rights_Code = SDRT.Syn_Deal_Rights_Code '
			END
			ELSE IF(@Deal_Type = 'SRHB')
			BEGIN
				SET @SQL_Syn = 'INSERT INTO #Temp_Right_Code (Right_Code,Deal_Code,Status_Changed_On)
					SELECT DISTINCT SDR.Syn_Deal_Rights_Code , SD.Syn_Deal_Code , SD.Status_Changed_On
					FROM Syn_Deal_Rights SDR (NOLOCK)
			...
			ELSE IF(@Deal_Type = 'SHB')
			BEGIN
				SET @Syn_Cond ='WHERE Actual_Right_End_Date IS NOT NULL AND SRHB.Holdback_Release_Date IS NOT NULL AND DATEDIFF(d, GETDATE(), ISNULL(SRHB.Holdback_Release_Date , GETDATE())) BETWEEN 0 AND '''+ @expiryDays +''' '					
			END	
			ELSE IF(@Deal_Type = 'SRHB')
			BEGIN
				SET @Syn_Cond ='WHERE Actual_Right_End_Date IS NOT NULL AND SDR.Actual_Right_End_Date IS NOT NULL AND SDR.Is_Pushback = ''Y'' AND DATEDIFF(d, GETDATE(), ISNULL(SDR.Actual_Right_End_Date , GETDATE())) BETWEEN 0 AND '''+ @expiryDays +''' '					
			END	
			...
			INNER JOIN Syn_Deal_Rights_Territory SDRC (NOLOCK) ON SDR.Right_Code = SDRC.Syn_Deal_Rights_Code
			LEFT JOIN Territory_Details TD (NOLOCK) ON 
			...
			INNER JOIN Syn_Deal_Rights SDR (NOLOCK) ON SD.Syn_Deal_Code = SDR.Syn_Deal_Code AND SD.Business_Unit_Code = @BusinessUnit_Code
			INNER JOIN Syn_Deal_Rights_Title SDRT (NOLOCK) ON SDR.Syn_Deal_Rights_Code = SDRT.Syn_Deal_Rights_Code
			INNER JOIN #Temp_Right_Code trc ON SDR.Syn_Deal_Rights_Code = trc.Right_Code
			INNER JOIN Syn_Deal_Rights_Platform SDRP (NOLOCK) ON SDR.Syn_Deal_Rights_Code = SDRP.Syn_Deal_Rights_Code
		...
		FROM #Temp_Rights TR
		INNER JOIN Country C (NOLOCK) ON C.Country_Code = TR.Country_Code
		...
		INNER JOIN Vendor V (NOLOCK) ON RC.Vendor_Code = V.Vendor_Code
		INNER JOIN Deal_Type TG (NOLOCK) ON RC.Deal_Type_Code = TG.Deal_Type_Code
		...
		INNER JOIN Title T (NOLOCK) ON RC.Title_Code = T.title_code
		LEFT JOIN Sub_License SL (NOLOCK) ON RC.Sub_License_Code = SL.Sub_License_Code
	...
	if(@Loglevel<2)Exec [USPLogSQLSteps] '[USP_Report_AcqSyn_Expiry]', 'Step 2', 0, 'Procedure Excuting Completed', 0, ''
END