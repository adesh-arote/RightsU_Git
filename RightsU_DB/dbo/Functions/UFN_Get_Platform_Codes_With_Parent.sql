CREATE Function [dbo].[UFN_Get_Platform_Codes_With_Parent]
(
	@PFNames as varchar(2000)	
)
...
-- =============================================
BEGIN
	--DECLARE @PFNames as varchar(2000)='Gaming Rights -- Promotional'
	--select  *From Platform where Parent_Platform_Code IN(106)
	--select  *From Platform where Platform_Code IN(106)
	----SELECT @PFCodes = @PFCodes + ',' + CAST( platform_Code as varchar)
	----FROM Platform where Platform_Hiearachy like '%non sta%' AND IS_LAST_LEVEL='Y'
	
	----SELECT @PFCodes
	
	--Declare @TempPF As Table(
	--	Platform_Code Int,
	--	Parent_Platform_Code Int,
	--	Base_Platform_Code Int,
	--	Is_Display Varchar(2),
	...
	--	TempCnt Int,
	--	TableCnt Int,
	--	Platform_Hiearachy Varchar(1000),
	--  Platform_Count Int
	--)
	...
	Declare @TempPFChild As Table(
		Child_Platform_Code Int,
		IsUse Varchar(1)
	)

	Declare @TempPFSubChild As Table(
		Sub_Child_Platform_Code Int,
		IsUse Varchar(1)
	)

	Insert InTo @TempPF
	Select platform_code, ISNULL(Parent_Platform_Code, 0), ISNULL(base_platform_code, 0), 'Y', Is_Last_Level, 0, 0, Platform_Hiearachy, 0
	From [Platform] Where Platform_Hiearachy in (Select number From DBO.fn_Split_withdelemiter(IsNull(@PFNames, ''), ','))
	AND @PFNames <> 'All Platforms'  
	order by platform_name 
	
	Insert InTo @TempPF
	Select  platform_code, ISNULL(Parent_Platform_Code, 0), ISNULL(base_platform_code, 0), 'Y', Is_Last_Level, 0, 0, Platform_Hiearachy, 0
	From [Platform] Where @PFNames = 'All Platforms' AND Is_Last_Level = 'Y'
	order by platform_name
	
	--Insert InTo @TempPFParent
	--Select Distinct ISNULL(Parent_Platform_Code, 0), 'N' From @TempPF
	
	Insert InTo @TempPFChild
	Select distinct P.Platform_Code, 'N'
	FROM Platform P
	INNER JOIN @TempPF TP ON TP.Platform_Code = P.Base_Platform_Code
	WHERE TP.Parent_Platform_Code = 0 AND TP.Base_Platform_Code = 0 AND @PFNames <> 'All Platforms'  

	Insert InTo @TempPFChild
	Select distinct P.Platform_Code, 'N'
	FROM Platform P
	INNER JOIN @TempPF TP ON TP.Platform_Code = P.Parent_Platform_Code
	WHERE TP.Parent_Platform_Code <> 0 AND TP.Base_Platform_Code <> 0 AND @PFNames <> 'All Platforms'  

	Insert InTo @TempPF
	Select distinct platform_code, ISNULL(Parent_Platform_Code, 0), ISNULL(base_platform_code, 0), 'N', Is_Last_Level, 0, 0, Platform_Hiearachy, 0
	From [Platform] p 
	Where platform_code In (Select ISNULL(Child_Platform_Code, 0) From @TempPFChild) --order by platform_name 

	Insert InTo @TempPFSubChild
	Select distinct P.Platform_Code, 'N'
	FROM Platform P
	INNER JOIN @TempPF TP ON TP.Platform_Code = P.Parent_Platform_Code
	WHERE TP.Parent_Platform_Code <> 0 AND TP.Base_Platform_Code <> 0 AND @PFNames <> 'All Platforms' AND Is_Display = 'N'  

	Insert InTo @TempPF
	Select distinct platform_code, ISNULL(Parent_Platform_Code, 0), ISNULL(base_platform_code, 0), 'N', Is_Last_Level, 0, 0, Platform_Hiearachy, 0
	From [Platform] p 
	Where platform_code In (Select ISNULL(Sub_Child_Platform_Code, 0) From @TempPFSubChild) --order by platform_name 
	--Insert InTo @TempPFSubChild
	--Select P.Platform_Code, 'N'
	--FROM Platform P
	--INNER JOIN @TempPFChild TP ON TP.Child_Platform_Code = P.Parent_Platform_Code
	--WHERE P.Parent_Platform_Code = TP.Child_Platform_Code 

	
	--select  *From @TempPFSubChild
	...
	DELETE FROM @TempPF Where Is_Last_Level <> 'Y'
	
	--Declare @RowCnt Int = 0
	--Select @RowCnt = Count(*) From @TempPF
	
	--Update @TempPF Set Platform_Count = @RowCnt
	--SELECT * FROM @TempPF order by Platform_Code  --where Is_Display='Y'
	RETURN
END
/*
...
EXEC USP_Get_Platform_With_Parent 
'117,118,119,120,121,122,126,127,128,129,130,131,194,195,196,197,198,199,200,201,202,203,204,205,206,207,208,209,210,211,212,213'
--*/
