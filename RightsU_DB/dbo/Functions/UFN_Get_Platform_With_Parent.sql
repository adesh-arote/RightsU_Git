...
BEGIN
	...
	--;WITH ABC AS(			
	--		SELECT 
	--		(Select Count(*) From @TempPF B Where B.Parent_Platform_Code = A.Platform_Code And Is_Display = 'Y') AS One,
	--		(Select Count(*) From [Platform] C Where C.Parent_Platform_Code = A.Platform_Code) AS Two,
	--		A.Platform_Code Platform_Code
	--		FROM @TempPF A
	--	) 
	--	UPDATE @TempPF SET TempCnt = (SELECT ONE FROM ABC WHERE Platform_Code=ABC.Platform_Code),
  ...
  --    	Update t Set 
	...
	Begin
		...
		--FROM ABC X 
		--INNER JOIN @TempPF Y ON X.Platform_Code=Y.Platform_Code
