CREATE TABLE [dbo].[Music_Deal] (
    [Music_Deal_Code]       INT             IDENTITY (1, 1) NOT NULL,
    ...
    CONSTRAINT [FK_Music_Deal_Deal_Tag] FOREIGN KEY ([Deal_Tag_Code]) REFERENCES [dbo].[Deal_Tag] ([Deal_Tag_Code]),
    CONSTRAINT [FK_Music_Deal_Deal_Type] FOREIGN KEY ([Deal_Type_Code]) REFERENCES [dbo].[Deal_Type] ([Deal_Type_Code]),
    CONSTRAINT [FK_Music_Deal_Entity] FOREIGN KEY ([Entity_Code]) REFERENCES [dbo].[Entity] ([Entity_Code]),
    CONSTRAINT [FK_Music_Deal_Music_Label] FOREIGN KEY ([Music_Label_Code]) REFERENCES [dbo].[Music_Label] ([Music_Label_Code]),
...
GO
EXECUTE sp_addextendedproperty @name = N'MS_Description', @value = N'C = Channel, G = Category', @level0type = N'SCHEMA', @level0name = N'dbo', @level1type = N'TABLE', @level1name = N'Music_Deal', @level2type = N'COLUMN', @level2name = N'Channel_Or_Category';
