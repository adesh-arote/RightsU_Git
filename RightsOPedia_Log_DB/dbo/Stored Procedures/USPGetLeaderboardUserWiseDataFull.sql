...
CREATE PROCEDURE [dbo].[USPGetLeaderboardUserWiseDataFull] 
@rptStartDate Date,
@rptEndDate Date
AS 
  BEGIN 
   ...
   CREATE TABLE #SecurityGroup
	...
	(SELECT Distinct t.TraceId,t.userid AS  UserId, 
					...
					CONVERT(DATE,t.RequestDateTime_IST) As RptDate,p.isunique

				FROM   VWTrace t WITH(NOLOCK)
						...
						WHERE 
						t.method = 'POST' AND  t.issuccess = 1  AND CONVERT(DATE,t.RequestDateTime_IST) between CONVERT(DATE, @rptStartDate) AND CONVERT(DATE, @rptEndDate)) As Temp
