...
CREATE PROCEDURE [dbo].[USPGetLeaderboardUserWiseDataFull_Prod] 
@rptStartDate Date,
@rptEndDate Date
AS 
  BEGIN 
   ...
   CREATE TABLE #SecurityGroup
	...
	From [RightsU]..SecurityGroup (NOLOCk) 
	...
	(SELECT Distinct t.TraceId,t.userid AS  UserId, 
				...
				FROM   trace t WITH(NOLOCK)
							INNER JOIN [RightsU]..pointsystemurl pu WITH(NOLOCK) 
									ON pu.pointsystemurl COLLATE DATABASE_DEFAULT = t.requestmethod COLLATE DATABASE_DEFAULT   
							INNER JOIN [RightsU]..pointsystem p WITH(NOLOCK)
									ON p.pointsystemurlid = pu.pointsystemurlid  --AND  p.isunique = 0
							INNER JOIN [RightsU]..users u WITH(NOLOCK)
									ON u.users_code = t.userid 
						INNER JOIN #SecurityGroup SG WITH(NOLOCK) ON SG.Security_Group_Code = u.Security_Group_Code
						WHERE 
						t.method = 'POST' AND  t.issuccess = 1  AND CONVERT(DATE,t.RequestDateTime) between CONVERT(DATE, '01-Jan-2020') AND CONVERT(DATE, '05-Mar-2020')) As Temp
