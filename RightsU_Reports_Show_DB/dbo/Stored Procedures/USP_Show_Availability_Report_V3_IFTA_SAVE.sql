CREATE PROCEDURE [dbo].[USP_Show_Availability_Report_V3_IFTA_SAVE]
(
...
--+''',@Is_Digital='''+ISNULL(Cast(@Is_Digital AS varchar), '')
   ...
   END		
	...
	print 'ad'
	--SET @strSQL =
	INSERT INTO Show_Avail_Data(Title_Name,Episode_From,Episode_To,Platform_Name,COL1,COL2,COL3,Right_Start_Date,Rights_End_Date,Title_Language_Names,Sub_Language_Names,Dub_Language_Names,Genres_Name,Star_Cast,Director,Duration_In_Min,Year_Of_Production,Restriction_Remark,Sub_Deal_Restriction_Remark,Remarks,Rights_Remarks,Exclusive,Sub_License,Platform_Avail,Ancillary_Rights,BU_Code)
		SELECT DISTINCT 
			...
			trr.Sub_License_Name AS Sub_License, 'Yes' Platform_Avail,'' AS Ancillary_Rights,CAST(@BU_Code AS VARCHAR)
		FROM 
			#Temp_Main_Ctr tm
		...
		Sub_Deal_Restriction_Remark, Remarks, Rights_Remarks, Exclusive, 'No' AS Sub_License, Platform_Avail, '' AS Ancillary_Rights,CAST(@BU_Code AS VARCHAR)
